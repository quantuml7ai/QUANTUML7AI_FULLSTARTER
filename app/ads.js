...
export function HomeBetweenBlocksAd({ slotKey, slotKind }) {
  ...
  try {
    ...
    /*
      Браузер запретил autoplay со звуком.
      Запускаем разрешённый muted autoplay и синхронизируем
      только локальное визуальное состояние кнопки рекламы.
    */
    applyMutedToVideo(v, true);

    mutedRef.current = true;
    setMuted((previousMuted) =>
      previousMuted === true ? previousMuted : true
    );
