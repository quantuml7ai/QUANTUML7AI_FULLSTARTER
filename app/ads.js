...
function isIOSWebKitRuntime() {
  ...
  try {
    ...
    return !!((classicIOS || desktopModeIPad) && /AppleWebKit/i.test(ua));
  } catch {
    return false;
  }
}
...
const SITE_AD_AUDIBLE_DOCUMENT_SOURCE = 'site-ads-audible-play';
...
function syncMutedPrefEverywhere(nextMuted, id, source = 'site-ads') {
  ...
  const userSet =
    normalizedSource === 'site-ads-toggle' ||
    normalizedSource === 'site-ads-surface-activate' ||
    normalizedSource === SITE_AD_AUDIBLE_DOCUMENT_SOURCE ||
    normalizedSource.endsWith('-toggle');
  writeMutedPrefToDocument(next, userSet);
  writeMutedPrefToStorage(next);
  emitMutedPref(next, id, normalizedSource);
  return next;
}
...
export function HomeBetweenBlocksAd({ slotKey, slotKind }) {
  ...
  /*
   * HYDRATION CONTRACT:
   * buildInternalSlotKey() intentionally depends on browser page identity
   * (hostname + pathname) and on the process-local instance counter.
   * Therefore it must not run during SSR or the first hydration render.
   *
   * We keep the existing key format and uniqueness semantics intact,
   * but create the key once, after mount, in the browser.
   */
  const internalKeyRef = useRef(null);
  const [internalSlotKey, setInternalSlotKey] = useState('');
  const selectedAdUrlRef = useRef(null);
  // Fail closed until IntersectionObserver proves that the mounted slot is outside the viewport.
  const isInViewportRef = useRef(true);
  const pendingAdRepickRef = useRef(false);
  const [adMountRevision, setAdMountRevision] = useState(0);
  ...
  const adAudibleDocumentPublishedRef = useRef(false);
  ...
  /*
   * The selected ad URL is browser/runtime data: it depends on clientId,
   * current time bucket, current page slot key and the live ad config.
   * Starting with null guarantees identical SSR and first client output.
   * The rotation clock still runs after mount, but replacement selection is committed
   * only after the slot has been observed outside the real viewport.
   */
  ...
  useEffect(() => {
    ...
    const key = internalKeyRef.current;
    setInternalSlotKey((prev) => (prev === key ? prev : key));
    ...
    selectedAdUrlRef.current = nextUrl || null;
    setUrl(nextUrl || null);
    return undefined;
  }, [slotKey, slotKind]);
  ...
  const emitAdPlayToCoordinator = useCallback((source = 'site_ad') => {
    ...
    const nativeVideoIsActuallyPlaying =
      source === 'site_ad_video' &&
      Boolean(videoRef.current) &&
      videoRef.current.paused === false &&
      videoRef.current.ended === false;
    const externalAudiblePlayCommand =
      source === 'site_ad_youtube' || source === 'site_ad_tiktok';
    const becameAudible =
      nextMuted === false &&
      (nativeVideoIsActuallyPlaying || externalAudiblePlayCommand);

    if (becameAudible && !adAudibleDocumentPublishedRef.current) {
      adAudibleDocumentPublishedRef.current = true;
      syncMutedPrefEverywhere(
        false,
        playerIdRef.current,
        SITE_AD_AUDIBLE_DOCUMENT_SOURCE
      );
    }

    emitSiteMediaPlay(source, el, playerIdRef.current, nextMuted, adPlayEventTsRef);
  }, []);
  ...
  const repickAdUrl = useCallback(() => {
    if (!internalSlotKey) return null;

    // Runtime contract: once a creative is mounted, do not even choose its successor
    // while any part of the slot is visible. Selection is deferred until the observer
    // confirms that the slot is outside the real viewport. The initial empty -> ad fill
    // is allowed because it is not a replacement of an already visible creative.
    if (selectedAdUrlRef.current && isInViewportRef.current) {
      pendingAdRepickRef.current = true;
      return selectedAdUrlRef.current;
    }
    ...
    pendingAdRepickRef.current = false;
    selectedAdUrlRef.current = nextUrl;
    setMediaInfo({ media: { kind: 'skeleton', src: null }, mediaHref: '', clickHref: nextUrl });
    setUrl(nextUrl);
    // Remount even when the click URL stays the same: a campaign may contain multiple
    // media creatives, and AdCard must pick the next one only while this slot is offscreen.
    setAdMountRevision((value) => value + 1);
    return nextUrl;
  }, [internalSlotKey]);
  ...
  try {
    if (
      ...
      normalizeSiteMuted(mutedRef.current) !== false
    ) {
      return;
    }

    /*
      iOS/WebKit policy rejection of audible autoplay becomes shared muted=true.
      Persist it before the muted retry so focus/recovery and the next media do not
      keep re-attempting audible autoplay until the user explicitly enables sound.
      Other runtimes keep the previous local-only muted retry behavior.
    */
    const blockedAudibleAutoplay =
      isIOSWebKitRuntime() &&
      String(err?.name || '') === 'NotAllowedError';

    const nextMuted = blockedAudibleAutoplay
      ? syncMutedPrefEverywhere(
          true,
          playerIdRef.current,
          'site-ads-autoplay-fallback'
        )
      : true;
  ...
  useEffect(() => {
    ...
    const onMuted = (e) => {
      ...
      // Incoming shared-mute events are applied locally without re-emitting them.
      // Re-broadcasting from every mounted ad creates an event echo between ad slots.
      const next = normalizeSiteMuted(detail.muted);
  ...
  useEffect(() => {
    ...
    if (!el || !isBrowser() || typeof IntersectionObserver === 'undefined') {
      // Without a real observer we cannot prove offscreen state, so keep rotation locked.
    ...
    const focusObs = new IntersectionObserver(([entry]) => {
      const ratio = Number(entry?.intersectionRatio || 0);
      const inViewport = !!entry?.isIntersecting && ratio > 0;
      isInViewportRef.current = inViewport;
      setIsFocused(ratio >= 0.6);

      if (!inViewport && pendingAdRepickRef.current) {
        repickAdUrl();
      }
  ...
  useEffect(() => {
    if (!isBrowser() || !internalSlotKey) return undefined;
  ...
  }, [adsLinksRefreshTick, conf, internalSlotKey, repickAdUrl]);
  ...
  return (
    <>
      ...
      >
        <AdCard
          key={`site-ad:${internalSlotKey}:${adMountRevision}`}
          url={url}
