...
export default function GamePage() {
  ...
  const supportActionHandledRef = useRef(false)
  ...
  useEffect(() => {
    if (!mounted || supportActionHandledRef.current || typeof window === 'undefined') return
    const params = new URLSearchParams(window.location.search || '')
    if (params.get('ql7Action') !== 'metastudio') return
    supportActionHandledRef.current = true
    openMetaStudio()
    try {
      params.delete('ql7Action')
      const nextQuery = params.toString()
      const nextUrl = `${window.location.pathname}${nextQuery ? `?${nextQuery}` : ''}${window.location.hash || '#metastudio'}`
      window.history.replaceState(window.history.state, '', nextUrl)
    } catch {}
  }, [mounted, openMetaStudio])
  ...
  const metaStudioPortal = mounted && metaStudioOpen
    ? createPortal(
        <div className="metastudioOverlay" role="presentation" data-ql7-render-managed="1" data-ql7-visual-scope="overlay">
          <section
  ...
  return (
    <>
      ...
      <main className="game-page-root" data-ql7-render-managed="1" data-ql7-visual-scope="route">
        <section className="panel game-panel" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
  ...
  mix-blend-mode: screen;
      ...
      <section
        className="marquee-wrap no-gutters"
          data-ql7-render-managed="1" data-ql7-visual-scope="panel"
        aria-hidden="true"
      >
      ...
      {/* НИЖНИЕ ИКОНКИ */}
      <div className="ql7-icons-row ql7-footer-social-layout" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
        <Link
          href="/privacy"
