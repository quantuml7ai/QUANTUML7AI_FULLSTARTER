...
/* ---- INBOX tabs ---- */
:root{
  --ql7-quantum-messenger-sticky-top:max(0px,env(safe-area-inset-top));
  --ql7-quantum-messenger-sticky-height:92px;
  --ql7-support-operator-sticky-gap:6px;
}
html[data-tma="1"]{
  --ql7-quantum-messenger-sticky-top:var(--tma-inbox-sticky-top,45px);
}
.inboxHeader{
  position:sticky; top:var(--ql7-quantum-messenger-sticky-top); z-index:48;
...
/* TMA: Quantum Messenger title + tabs remain one sticky owner. */
html[data-tma="1"] .inboxHeader{
  top:var(--ql7-quantum-messenger-sticky-top);
}
.inboxTabBtn{
  position:relative;
...
.dmThreadHeader[data-support-thread="1"] .ql7SupportOperator{
  overflow:visible;
}
...
@media (max-width:560px){
  ...
  .supportRailNotice span{ width:min(100%, 116px); max-width:min(100%, 116px); min-height:26px; padding:0 3px; font-size:clamp(6.8px, 2vw, 9px); letter-spacing:0; }
}
...
/* QL7 Support server-only rich cards */
.ql7SupportCard{
  width:min(100%, 680px);
  min-width:0;
  ...
  overflow:hidden;
  border:1px solid rgba(111,226,255,.34);
  border-radius:18px;
  background:
    radial-gradient(circle at 9% 0%, rgba(71,212,255,.15), transparent 34%),
    linear-gradient(145deg, rgba(6,17,32,.96), rgba(10,25,43,.93));
  box-shadow:inset 0 0 0 1px rgba(255,232,144,.07), 0 16px 42px rgba(0,0,0,.3), 0 0 28px rgba(67,202,255,.09);
  color:#eafcff;
  overflow-wrap:anywhere;
}
.ql7SupportCard[dir="rtl"]{ text-align:right; }
.ql7SupportCardHeader{ display:flex; align-items:flex-start; justify-content:space-between; gap:12px; min-width:0; }
.ql7SupportCardHeader h3{ margin:4px 0 0; color:#fff4ba; font-size:clamp(14px, 3.8vw, 19px); line-height:1.2; }
.ql7SupportCardEyebrow{ color:#70e6ff; font-size:9px; font-weight:950; letter-spacing:.18em; }
.ql7SupportCardStatus{ flex:0 1 auto; max-width:44%; padding:5px 8px; overflow:hidden; border:1px solid rgba(255,222,112,.4); border-radius:999px; color:#ffe89a; font-size:10px; font-weight:900; line-height:1.1; text-overflow:ellipsis; white-space:nowrap; }
.ql7SupportCardSummary{ margin:11px 0 0; color:#d8f6ff; font-size:13px; line-height:1.48; white-space:pre-wrap; }
.ql7SupportCardSection{ margin-top:14px; padding-top:12px; border-top:1px solid rgba(127,224,255,.14); }
.ql7SupportCardSection h4{ margin:0 0 8px; color:#8deeff; font-size:11px; line-height:1.25; letter-spacing:.045em; text-transform:uppercase; }
.ql7SupportCardSection ul,.ql7SupportCardSection ol{ margin:0; padding-inline-start:19px; display:grid; gap:6px; color:#dff8ff; font-size:12px; line-height:1.4; }
.ql7SupportCardWarnings{ border-color:rgba(255,176,95,.24); }
.ql7SupportCardWarnings h4,.ql7SupportCardWarnings li{ color:#ffd0a2; }
.ql7SupportCardSnapshot blockquote{ margin:0 0 10px; padding:10px 12px; border-inline-start:3px solid rgba(255,222,112,.72); border-radius:8px; background:rgba(255,255,255,.035); color:#f0fbff; font-size:12px; line-height:1.45; white-space:pre-wrap; }
.ql7SupportCardSnapshot dl,.ql7SupportCardFacts{ margin:0; display:grid; grid-template-columns:minmax(78px,.42fr) minmax(0,1fr); gap:7px 10px; font-size:11px; line-height:1.35; }
.ql7SupportCardSnapshot dt,.ql7SupportCardFacts dt{ color:#80dff6; font-weight:850; }
.ql7SupportCardSnapshot dd,.ql7SupportCardFacts dd{ min-width:0; margin:0; color:#f2fbff; }
.ql7SupportCardFacts dd{ display:grid; gap:2px; }
.ql7SupportCardFacts small{ color:#8ea9b8; font-size:9px; }
.ql7SupportCardMediaGrid{ margin-top:12px; display:grid; grid-template-columns:repeat(auto-fit,minmax(min(180px,100%),1fr)); gap:8px; min-width:0; }
.ql7SupportCardMedia{ min-width:0; margin:0; overflow:hidden; border:1px solid rgba(111,226,255,.19); border-radius:12px; background:#02070d; }
.ql7SupportCardImage img,.ql7SupportCardVideo video{ display:block; width:100%; height:auto; max-height:min(46vh,420px); object-fit:contain; background:#000; }
.ql7SupportCardVideo video{ aspect-ratio:16/9; }
.ql7SupportCardEmbed iframe{ display:block; width:100%; aspect-ratio:16/9; border:0; background:#000; }
.ql7SupportCardMediaLink,.ql7SupportCardOpenPost{ display:flex; align-items:center; justify-content:center; min-height:38px; padding:8px 12px; border:1px solid rgba(91,228,255,.35); border-radius:11px; background:linear-gradient(135deg,rgba(0,175,225,.18),rgba(75,61,181,.17)); color:#bff6ff; font-size:11px; font-weight:900; line-height:1.3; text-decoration:none; overflow-wrap:anywhere; }
.ql7SupportCardOpenPost{ width:max-content; max-width:100%; margin-top:11px; }
.ql7SupportCardMediaLink:hover,.ql7SupportCardOpenPost:hover{ border-color:rgba(255,221,104,.62); color:#fff2ad; }
.ql7SupportCardReviewMessage{ margin:0 0 10px; padding:10px 12px; border:1px solid rgba(255,197,74,.2); border-radius:10px; background:rgba(255,183,39,.055); color:#eafaff; font-size:12px; line-height:1.48; }
.ql7SupportCardEvidenceLabel{ margin:0 0 6px; color:#8deeff; font-size:10px; font-weight:900; letter-spacing:.04em; text-transform:uppercase; }
.ql7SupportCardAudio{ padding:8px; }
.ql7SupportCardAudio audio{ display:block; width:100%; max-width:100%; }
.ql7SupportCardMedia figcaption{ padding:8px 10px; border-top:1px solid rgba(111,226,255,.14); color:#b8dbe8; font-size:10px; line-height:1.4; background:rgba(4,14,26,.92); }
.ql7SupportCardValueBadge{ display:inline-flex; align-items:center; min-height:22px; padding:3px 8px; border:1px solid rgba(100,229,255,.32); border-radius:999px; background:rgba(28,130,175,.12); color:#bff6ff; font-size:10px; font-weight:850; }
.ql7SupportCardValueBadge--warning{ border-color:rgba(255,184,91,.42); background:rgba(176,91,25,.14); color:#ffd5a3; }
.ql7SupportChoicePanel{ margin-top:14px; display:grid; gap:10px; }
.ql7SupportChoiceGrid{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px; }
.ql7SupportChoiceButton{ min-width:0; min-height:48px; padding:10px 12px; display:grid; gap:3px; align-content:center; border:1px solid rgba(102,226,255,.36); border-radius:13px; background:linear-gradient(145deg,rgba(17,72,105,.34),rgba(26,29,70,.38)); color:#e9fbff; text-align:start; cursor:pointer; box-shadow:inset 0 0 0 1px rgba(255,255,255,.025); transition:transform .16s ease,border-color .16s ease,filter .16s ease; }
.ql7SupportChoiceButton:hover:not(:disabled){ transform:translateY(-1px); border-color:rgba(255,221,103,.66); filter:brightness(1.08); }
.ql7SupportChoiceButton:focus-visible,.ql7SupportChoiceSend:focus-visible{ outline:2px solid #88edff; outline-offset:2px; }
.ql7SupportChoiceButton:disabled{ cursor:default; opacity:.58; }
.ql7SupportChoiceButton[aria-pressed="true"]{ border-color:rgba(94,255,161,.62); background:rgba(30,121,80,.24); }
.ql7SupportChoiceLabel{ min-width:0; color:#effcff; font-size:12px; font-weight:900; line-height:1.28; overflow-wrap:anywhere; }
.ql7SupportChoiceButton small{ color:#9fc6d5; font-size:10px; line-height:1.3; }
.ql7SupportChoiceButton--other{ width:100%; min-height:40px; }
.ql7SupportChoiceOtherEditor{ margin-top:8px; display:grid; gap:8px; }
.ql7SupportChoiceOtherEditor textarea{ width:100%; min-width:0; resize:vertical; box-sizing:border-box; padding:10px 11px; border:1px solid rgba(111,226,255,.28); border-radius:11px; background:rgba(2,10,20,.72); color:#ecfbff; font:inherit; font-size:12px; line-height:1.45; }
.ql7SupportChoiceSend{ min-height:38px; padding:8px 14px; justify-self:end; border:1px solid rgba(255,219,97,.52); border-radius:11px; background:linear-gradient(135deg,rgba(202,153,32,.25),rgba(30,111,145,.3)); color:#fff1ac; font-size:11px; font-weight:900; cursor:pointer; }
.ql7SupportChoiceSend:disabled{ opacity:.48; cursor:default; }
.ql7SupportChoiceState{ margin:0; color:#b9eaff; font-size:10px; line-height:1.35; }
.ql7SupportChoiceState--sent{ color:#9cf3be; }
.ql7SupportChoiceState--error{ color:#ffb1b1; }
.ql7SupportCardTableScroll{ max-width:100%; overflow:auto; border:1px solid rgba(111,226,255,.16); border-radius:12px; }
.ql7SupportCardTableSection table{ width:100%; min-width:420px; border-collapse:collapse; font-size:11px; }
.ql7SupportCardTableSection th,.ql7SupportCardTableSection td{ padding:9px 10px; border-bottom:1px solid rgba(111,226,255,.12); text-align:start; vertical-align:top; }
.ql7SupportCardTableSection th{ position:sticky; top:0; z-index:1; background:rgba(12,40,62,.98); color:#86eaff; font-weight:900; }
.ql7SupportCardTableSection td{ color:#ecfbff; background:rgba(4,15,28,.72); }
.ql7SupportCardTimeline{ list-style:none !important; padding:0 !important; }
.ql7SupportCardTimeline li{ position:relative; padding-inline-start:18px; display:grid; gap:2px; }
.ql7SupportCardTimeline li::before{ content:""; position:absolute; inset-inline-start:2px; top:.45em; width:7px; height:7px; border-radius:50%; background:#70e6ff; box-shadow:0 0 10px rgba(112,230,255,.55); }
.ql7SupportCardTimeline time{ color:#8ea9b8; font-size:9px; }
.ql7SupportCardActionRail{ margin-top:14px; display:flex; flex-wrap:wrap; gap:8px; }
.ql7SupportCardAction{ min-height:36px; padding:7px 12px; display:inline-flex; align-items:center; justify-content:center; border:1px solid rgba(100,225,255,.36); border-radius:11px; background:rgba(17,65,92,.35); color:#c9f8ff; font-size:11px; font-weight:900; text-decoration:none; }
.ql7SupportCardAction--primary{ border-color:rgba(255,217,91,.5); color:#fff0a6; }
.ql7SupportCardAsOf{ margin-top:14px; padding-top:10px; border-top:1px solid rgba(127,224,255,.12); color:#819eac; font-size:9px; line-height:1.3; }
@media (max-width:360px){
  .ql7SupportCard{ padding:11px; border-radius:15px; }
  .ql7SupportCardHeader{ gap:7px; }
  .ql7SupportCardStatus{ max-width:40%; font-size:8px; }
  .ql7SupportCardSnapshot dl,.ql7SupportCardFacts{ grid-template-columns:1fr; gap:3px; }
  .ql7SupportChoiceGrid{ grid-template-columns:1fr; }
  .ql7SupportCardSnapshot dd,.ql7SupportCardFacts dd{ margin-bottom:6px; }
}
@media (prefers-reduced-motion:reduce){
  .ql7SupportCard *{ scroll-behavior:auto !important; }
}

/* QL7 Support 777 premium cards, shared DM media and runtime-gated input */
.ql7SupportCardTitleBlock{min-width:0;display:grid;gap:8px;}
.ql7SupportCardHeader h3{margin:0;color:#fff0a8;font-size:clamp(16px,3.7vw,22px);font-weight:950;line-height:1.18;text-wrap:balance;text-shadow:0 0 18px rgba(255,218,101,.16);}
.ql7SupportCardSummary{margin:0;color:#d9f7ff;font-size:clamp(12px,2.8vw,14px);line-height:1.56;}
.ql7SupportCard--clarification_choices{background:radial-gradient(circle at 15% 0%,rgba(50,196,255,.13),transparent 42%),radial-gradient(circle at 88% 10%,rgba(255,199,69,.1),transparent 38%),linear-gradient(148deg,rgba(15,31,54,.98),rgba(4,12,25,.98));}
.ql7SupportChoiceGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;}
.ql7SupportChoiceButton{position:relative;overflow:hidden;min-height:62px;padding:13px 14px 13px 17px;border-color:rgba(108,224,255,.42);background:linear-gradient(145deg,rgba(19,88,127,.34),rgba(31,28,81,.48));box-shadow:inset 0 0 0 1px rgba(255,255,255,.035),0 10px 24px rgba(0,0,0,.18);}
.ql7SupportChoiceButton::before{content:"";position:absolute;inset-block:10px;inset-inline-start:7px;width:3px;border-radius:999px;background:linear-gradient(180deg,#65e7ff,#ffd966);box-shadow:0 0 12px rgba(101,231,255,.45);}
.ql7SupportChoiceButton:hover:not(:disabled){transform:translateY(-2px);border-color:rgba(255,221,103,.78);box-shadow:inset 0 0 0 1px rgba(255,255,255,.06),0 14px 30px rgba(0,0,0,.25),0 0 22px rgba(74,218,255,.12);}
.ql7SupportChoiceButton--other{min-height:48px;border-style:dashed;background:linear-gradient(145deg,rgba(126,89,34,.2),rgba(25,37,75,.38));}
.ql7SupportChoicePanel[data-ql7-support-choice-rail="five-full-width"] .ql7SupportChoiceGrid{grid-template-columns:minmax(0,1fr)!important}
.ql7SupportChoicePanel[data-ql7-support-choice-rail="five-full-width"] .ql7SupportChoiceOther{width:100%}
.ql7SupportChoicePanel[data-ql7-support-choice-rail="five-full-width"] .ql7SupportChoiceButton--other{width:100%!important}
.ql7SupportPremiumTable{display:grid;overflow:hidden;border:1px solid rgba(104,220,255,.21);border-radius:15px;background:rgba(2,10,22,.58);box-shadow:inset 0 0 24px rgba(50,186,255,.035);}
.ql7SupportDataRow{display:grid;grid-template-columns:minmax(132px,.42fr) minmax(0,1fr);gap:14px;padding:11px 13px;border-bottom:1px solid rgba(103,219,255,.12);align-items:start;}
.ql7SupportDataRow:last-child{border-bottom:0;}
.ql7SupportDataLabel{color:#7ee7ff;font-size:10px;font-weight:950;line-height:1.35;letter-spacing:.035em;text-transform:uppercase;}
.ql7SupportDataValue{min-width:0;color:#f0fbff;font-size:12px;font-weight:700;line-height:1.45;overflow-wrap:anywhere;}
.ql7SupportDataValue small{display:block;margin-top:3px;color:#89a9ba;font-size:9px;font-weight:600;}
.ql7SupportDataRow--warning .ql7SupportDataValue{color:#ffd8a3;}
.ql7SupportDataRow--danger{background:rgba(170,43,56,.11);}
.ql7SupportDataRow--danger .ql7SupportDataValue{color:#ffb1b9;}
.ql7SupportDataRow--accent .ql7SupportDataValue{color:#fff0a1;}
.ql7SupportEvidencePanel{margin:0 0 12px;padding:12px 13px;border:1px solid rgba(255,208,83,.2);border-radius:13px;background:linear-gradient(140deg,rgba(112,76,14,.12),rgba(4,15,30,.62));}
.ql7SupportEvidencePanel>strong{display:block;margin-bottom:7px;color:#ffd977;font-size:10px;text-transform:uppercase;letter-spacing:.055em;}
.ql7SupportEvidencePanel blockquote{margin:0;padding:0;border:0;background:transparent;color:#effbff;font-size:12px;line-height:1.5;white-space:pre-wrap;}
.ql7SupportRulesNotice{margin-top:12px;padding:12px 13px;border:1px solid rgba(255,190,78,.25);border-radius:13px;background:linear-gradient(145deg,rgba(117,54,18,.14),rgba(17,24,45,.52));}
.ql7SupportRulesNotice strong{color:#ffd38d;font-size:11px;}
.ql7SupportRulesNotice p{margin:5px 0 0;color:#dceef5;font-size:11px;line-height:1.48;}
.ql7SupportSharedMediaSection{margin-top:14px;padding-top:14px;border-top:1px solid rgba(127,224,255,.14);}
.ql7SupportSharedMediaSection .dmMediaGrid{margin-top:0;}
.ql7SupportCard .dmMediaBox{width:100%;max-width:100%;}
.ql7SupportCard .videoCard.dmMediaBox,.ql7SupportCard .videoCard.dmMediaBox .mediaBoxItem{min-height:350px;max-height:min(72vh,650px);}
.supportRailNotice{min-width:0;display:flex!important;align-items:center;gap:8px;max-width:min(62vw,520px);padding:5px 10px;border:1px solid rgba(87,219,255,.22);border-radius:999px;background:rgba(13,44,68,.32);color:#bdefff;font-size:10px;line-height:1.25;}
.supportRailNotice span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.supportRailNotice.isRuntimeLocked{border-color:rgba(255,190,76,.38);background:linear-gradient(135deg,rgba(114,63,12,.25),rgba(18,41,68,.38));color:#ffe2a0;}
.supportRailCountdown{flex:0 0 auto;min-width:34px;padding:3px 7px;border:1px solid rgba(255,218,105,.46);border-radius:999px;background:rgba(255,190,50,.12);color:#fff0ad;text-align:center;font-variant-numeric:tabular-nums;}
@media(max-width:520px){
  .ql7SupportChoiceGrid{grid-template-columns:1fr;}
  .ql7SupportDataRow{grid-template-columns:1fr;gap:4px;padding:10px 11px;}
  .supportRailNotice{max-width:42vw;padding-inline:8px;}
  .supportRailNotice span{display:none;}
}
@media(prefers-reduced-motion:reduce){
  .ql7SupportChoiceButton{transition:none!important;}
  .ql7SupportChoiceButton:hover:not(:disabled){transform:none;}
}


/* QL7 Support V8 styles moved to ql7SupportGlobalStyles.js and mounted globally by ForumLayout. */
