...
@media (pointer: coarse), (max-width: 860px){
  /*
   * Mobile QCast EQ:
   * возвращаем только лёгкую transform-анимацию полос.
   * Shimmer, glow/filter и тяжёлый canvas-визуализатор
   * на мобильном не включаем.
   */
  .qcastCtrlShimmer{
    animation:none;
    opacity:.18;
  ...
  /*
   * Анимируем полосы только во время реального playback.
   * data-on="1" уже выставляется React-компонентом QCastPlayer.
   */
  ...
  /*
   * Pause / stop:
   * гарантированно возвращаем статическое состояние.
   */
  .qcastCtrlEQ[data-on="0"] .qcastCtrlBar{
    animation:none;
    transform:scaleY(.12);
...
/*
 * Accessibility имеет последний приоритет.
 * Если пользователь включил Reduce Motion,
 * мобильный override не должен снова запускать EQ.
 */
@media (prefers-reduced-motion: reduce){
