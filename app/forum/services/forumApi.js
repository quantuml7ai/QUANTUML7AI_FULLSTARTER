...
async function serverForumMediaFeedPagePost(path, body = {}, headers = {}, signal = null) {
  try {
    const requestInit = {
      method: 'POST',
      cache: 'no-store',
    ...
    const retryable = !r.ok && (status === 408 || status === 425 || status === 429 || status >= 500)
    return {
      ...json,
      ok: !!json?.ok && r.ok,
      status,
      retryable: json?.retryable === true || retryable,
    }
  } catch (error) {
    const aborted = String(error?.name || '') === 'AbortError'
    return { ok: false, error: aborted ? 'aborted' : 'network', status: 0, retryable: true }
  }
}
...
export const api = {
  ...
  // Cheap Mongo-primary revision check. When `since` is provided the same request
  // also returns only public-safe post edit/delete deltas from the canonical change journal.
  async rev(q = {}) {
    try {
      const params = new URLSearchParams()
      const since = Math.max(0, Math.trunc(Number(q?.since || 0) || 0))
      const limit = Math.max(1, Math.min(512, Math.trunc(Number(q?.limit || 128) || 128)))
      if (since > 0) params.set('since', String(since))
      if (since > 0) params.set('limit', String(limit))
      const r = await fetch('/api/forum/rev' + (params.toString() ? `?${params}` : ''), { cache: 'no-store' })
      const json = await r.json().catch(() => ({}))
      return {
        ok: !!json?.ok && r.ok,
        rev: Number(json?.rev || 0) || 0,
        cursorRev: Number(json?.cursorRev ?? json?.rev ?? 0) || 0,
        events: Array.isArray(json?.events) ? json.events : [],
        hasMore: !!json?.hasMore,
      }
    } catch {
      return { ok: false, rev: 0, cursorRev: 0, events: [], hasMore: false, error: 'network' }
    }
  },
  ...
  async mediaFeedPage(q = {}) {
    ...
    const requestSignal = source?.requestSignal || null
    const { requestSignal: _requestSignal, ...body } = source
    const actor = withForumActor({
      mode: readForumGeoFeedModePreference(),
      sort: readForumFeedSortPreference(body.sort || 'random'),
      randomSeed: body.randomSeed || body.seed || sessionForumFeedSeed(),
    ...
    return serverForumMediaFeedPagePost('/api/forum/media-feed/page', actor.body, actor.headers, requestSignal)
  },
  ...
  // Батч-мутации
  async mutate(batch, userId, options = null) {
    try {
      const actorId =
        userId ??
        batch?.userId ??
      ...
      const r = await fetch(url, { method: 'POST', headers, body, keepalive: !!options?.keepalive });
