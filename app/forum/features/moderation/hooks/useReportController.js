...
export default function useReportController({
  ...
  const handleReportSelect = useCallback(async (reason) => {
    ...
    try {
      ...
      if (res?.action === 'deleted' || res?.action === 'deleted_and_locked' || res?.alreadyDeleted || res?.action === 'already_deleted') {
        const deletedIds = Array.from(new Set([
          ...(Array.isArray(res?.deletedPostIds) ? res.deletedPostIds : []),
        ...
        persistTombstones((prev) => ({
          ...
          posts: {
            ...(prev?.posts || {}),
            ...Object.fromEntries(deletedIds.map((id) => [id, Date.now() + tombstoneTtlMs])),
          },
        }))
