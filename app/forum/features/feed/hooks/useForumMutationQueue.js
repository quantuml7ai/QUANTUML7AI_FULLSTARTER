...
const VIEW_BATCH_FLUSH_MS = 10000
const VIEW_BATCH_MAX_IDS = 32
const VIEW_RETRY_DELAYS_MS = Object.freeze([5000, 15000, 30000, 60000])
const VIEW_OUTBOX_SUFFIX = ':view-outbox:v1'

function normalizeActorId(value) {
  return String(value || '').trim()
}

function viewOutboxStorageKey(queueStorageKey, actorId) {
  const actor = normalizeActorId(actorId).toLowerCase()
  if (!actor) return ''
  return `${String(queueStorageKey || 'forum:queue')}${VIEW_OUTBOX_SUFFIX}:${encodeURIComponent(actor)}`
}

function readViewOutbox(key) {
  if (!key || typeof localStorage === 'undefined') return { posts: [], topics: [] }
  try {
    const parsed = JSON.parse(localStorage.getItem(key) || '{}')
    return {
      posts: Array.from(new Set((Array.isArray(parsed?.posts) ? parsed.posts : []).map(String).map((id) => id.trim()).filter(Boolean))),
      topics: Array.from(new Set((Array.isArray(parsed?.topics) ? parsed.topics : []).map(String).map((id) => id.trim()).filter(Boolean))),
    }
  } catch {
    return { posts: [], topics: [] }
  }
...
function writeViewOutbox(key, posts, topics) {
  if (!key || typeof localStorage === 'undefined') return
  try {
    const postIds = Array.from(new Set((Array.isArray(posts) ? posts : []).map(String).map((id) => id.trim()).filter(Boolean)))
    const topicIds = Array.from(new Set((Array.isArray(topics) ? topics : []).map(String).map((id) => id.trim()).filter(Boolean)))
    if (!postIds.length && !topicIds.length) {
      localStorage.removeItem(key)
      return
    }
    localStorage.setItem(key, JSON.stringify({ version: 1, posts: postIds, topics: topicIds }))
  } catch {}
}
...
export default function useForumMutationQueue({
  ...
  const flushRequestedWhileBusyRef = useRef(false)
  const pendingViewsPostsRef = useRef(new Set())
  const pendingViewsTopicsRef = useRef(new Set())
  const pendingViewsActorRef = useRef('')
  const viewRetryAttemptRef = useRef(0)
  const flushSoonTimerRef = useRef(null)
  const flushSoonDueAtRef = useRef(0)
  ...
  const getActiveActorId = useCallback(() => normalizeActorId(
    authRef?.current?.asherId ||
    authRef?.current?.accountId ||
    getForumUserIdFn?.() ||
    '',
  ), [authRef, getForumUserIdFn])

  const persistPendingViews = useCallback(() => {
    if (!isBrowserFn?.()) return
    const actorId = pendingViewsActorRef.current
    const key = viewOutboxStorageKey(queueStorageKey, actorId)
    if (!key) return
    writeViewOutbox(
      key,
      Array.from(pendingViewsPostsRef.current || []),
      Array.from(pendingViewsTopicsRef.current || []),
    )
  }, [isBrowserFn, queueStorageKey])

  const activatePendingViewActor = useCallback((actorInput) => {
    if (!isBrowserFn?.()) return false
    const actorId = normalizeActorId(actorInput)
    if (pendingViewsActorRef.current === actorId) return false

    if (pendingViewsActorRef.current) persistPendingViews()
    pendingViewsPostsRef.current.clear()
    pendingViewsTopicsRef.current.clear()
    pendingViewsActorRef.current = actorId
    viewRetryAttemptRef.current = 0

    const key = viewOutboxStorageKey(queueStorageKey, actorId)
    if (!key) return true
    const restored = readViewOutbox(key)
    restored.posts.forEach((id) => pendingViewsPostsRef.current.add(id))
    restored.topics.forEach((id) => pendingViewsTopicsRef.current.add(id))
    return true
  }, [isBrowserFn, persistPendingViews, queueStorageKey])

  const requestFlushSoon = useCallback((delayMs = 180) => {
    const delay = Math.max(0, Number(delayMs || 0))
    const dueAt = Date.now() + delay
    if (flushSoonTimerRef.current && flushSoonDueAtRef.current && flushSoonDueAtRef.current <= dueAt) return
    if (flushSoonTimerRef.current) clearTimeout(flushSoonTimerRef.current)
    flushSoonDueAtRef.current = dueAt
    flushSoonTimerRef.current = setTimeout(() => {
      flushSoonTimerRef.current = null
      flushSoonDueAtRef.current = 0
      try {
  ...
  const schedulePendingViewFlush = useCallback(() => {
    const count = pendingViewsPostsRef.current.size + pendingViewsTopicsRef.current.size
    if (!count) return
    requestFlushSoon(count >= VIEW_BATCH_MAX_IDS ? 0 : VIEW_BATCH_FLUSH_MS)
  }, [requestFlushSoon])

  const schedulePendingViewRetry = useCallback(() => {
    const count = pendingViewsPostsRef.current.size + pendingViewsTopicsRef.current.size
    if (!count) {
      viewRetryAttemptRef.current = 0
      return
    ...
    const attempt = Math.min(viewRetryAttemptRef.current, VIEW_RETRY_DELAYS_MS.length - 1)
    viewRetryAttemptRef.current = Math.min(viewRetryAttemptRef.current + 1, VIEW_RETRY_DELAYS_MS.length)
    requestFlushSoon(VIEW_RETRY_DELAYS_MS[attempt])
  }, [requestFlushSoon])
  ...
  const flushMutations = useCallback(async (options = null) => {
    if (busyRef.current) {
      // A mutation can be enqueued while another forum batch is still in flight.
      // Remember that edge instead of dropping the flush request; the current
      // batch will re-arm one drain pass from finally.
      flushRequestedWhileBusyRef.current = true
      return
    }

    const userId = normalizeActorId(
      authRef?.current?.asherId ||
      authRef?.current?.accountId ||
      getForumUserIdFn?.() ||
      '',
    )
    activatePendingViewActor(userId)
    ...
    try {
      // Forum write ownership must follow the same actor precedence as DM,
      // media moderation and owner actions. In Telegram Mini App sessions
      // asherId is the post-auth/canonical identity; accountId can still carry
      // a stale wallet-storage value until the auth bridge converges.
      const resp = await api?.mutate?.({ ops: toSend }, userId, { keepalive: !!options?.keepalive })
...
if (resp && Array.isArray(resp.applied)) {
  ...
  for (const it of applied) {
    if (it?.error || it?.op !== 'create_topic') continue

    const cid = String(it.cid || '').trim()
    const topicId = String(it.topic?.id || it.topicId || '').trim()

    if (!cid || !topicId || cid === topicId) continue

    topicAckByCid.set(cid, {
      topicId,
      topic: it.topic || null,
    })
  }
...
const leftover = current
  ...
  .map((op) => {
    if (op?.type !== 'create_post' || !op?.payload) return op

    const rawTopicId = String(op.payload.topicId || '').trim()
    const topicAck = topicAckByCid.get(rawTopicId)

    if (!topicAck) return op
    ...
    return {
      ...
      payload: {
        ...
        // Pending operation больше не должна идти на tmp_t_*.
        topicId: topicAck.topicId,

        // Оставляем CID как fail-closed recovery bridge.
        ...(String(op.payload.topicCid || '').trim()
          ? {}
          : { topicCid: rawTopicId }),
      },
    }
...
saveQueue(leftover)
        ...
        persistPendingViews()
        if ((!pendingPosts.length || hasAckedPostViews) && (!pendingTopics.length || hasAckedTopicViews)) {
          viewRetryAttemptRef.current = 0
        } else {
          schedulePendingViewRetry()
        }
        ...
        persistSnap((prev) => {
          ...
          for (const it of applied) {
            ...
            if (it.op === 'view_topic' && it.topicId != null) {
              ...
              const views = Number(it.views ?? 0)
              if (Number.isFinite(views)) {
                next.topics = (next.topics || []).map((t) => (String(t.id) === id ? mergeTopicViews(t, views) : t))
              }
...
if (
  clearOverlay.createTopics.size ||
  clearOverlay.createPosts.size ||
  topicAckByCid.size
) {
  const creates = {
    topics: (next.creates.topics || [])
      .filter(
        (t) =>
          !clearOverlay.createTopics.has(
            String(t.id || t.cid || ''),
          ),
      ),

    posts: (next.creates.posts || [])
      .filter(
        (p) =>
          !clearOverlay.createPosts.has(
            String(p.id || p.cid || ''),
          ),
      )
      .map((p) => {
        const rawTopicId = String(p.topicId || '').trim()
        const topicAck = topicAckByCid.get(rawTopicId)

        return topicAck
          ? { ...p, topicId: topicAck.topicId }
          : p
      }),
  }
...
for (const [cid, topicAck] of topicAckByCid.entries()) {
  try {
    topicReconcileRef?.current?.({
      cid,
      topicId: topicAck.topicId,
      topic: topicAck.topic,
    })
  } catch {}
}
      ...
      } else if (pendingPosts.length || pendingTopics.length) {
        persistPendingViews()
        schedulePendingViewRetry()
      }
    } catch (e) {
      ...
      if (flushRequestedWhileBusyRef.current) {
        flushRequestedWhileBusyRef.current = false
        requestFlushSoon(0)
      }
    }
  }, [
    activatePendingViewActor,
    api,
    authRef,
    compactOps,
    getForumUserIdFn,
...
topicReconcileRef,
  ...
  useEffect(() => {
    ...
    const actorChanged = activatePendingViewActor(actorId)
    if (actorChanged && (pendingViewsPostsRef.current.size || pendingViewsTopicsRef.current.size)) requestFlushSoon(0)

    const flushDurableViews = () => {
      persistPendingViews()
      if (!pendingViewsPostsRef.current.size && !pendingViewsTopicsRef.current.size) return
      try { flushMutationsRef.current?.({ keepalive: true }) } catch {}
    }
    const onVisibilityChange = () => {
      try {
        if (document.hidden) flushDurableViews()
        else if (pendingViewsPostsRef.current.size || pendingViewsTopicsRef.current.size) requestFlushSoon(0)
      } catch {}
    }
    const onOnline = () => {
      if (pendingViewsPostsRef.current.size || pendingViewsTopicsRef.current.size) requestFlushSoon(0)
    }

    window.addEventListener('pagehide', flushDurableViews)
    window.addEventListener('online', onOnline)
    document.addEventListener('visibilitychange', onVisibilityChange)
    return () => {
      persistPendingViews()
      window.removeEventListener('pagehide', flushDurableViews)
  ...
  const enqueuePendingPostView = useCallback((postId) => {
    ...
    activatePendingViewActor(actorId)
    const before = pendingViewsPostsRef.current.size
    pendingViewsPostsRef.current.add(id)
    if (pendingViewsPostsRef.current.size === before) return
    persistPendingViews()
    schedulePendingViewFlush()
  }, [activatePendingViewActor, getActiveActorId, persistPendingViews, schedulePendingViewFlush])
  ...
  const enqueuePendingTopicView = useCallback((topicId) => {
    ...
    if (!id || !actorId) return
    activatePendingViewActor(actorId)
    const before = pendingViewsTopicsRef.current.size
    pendingViewsTopicsRef.current.add(id)
    if (pendingViewsTopicsRef.current.size === before) return
    persistPendingViews()
    schedulePendingViewFlush()
  }, [activatePendingViewActor, getActiveActorId, persistPendingViews, schedulePendingViewFlush])
