...
export default function useTopicDiscoveryModel({
  ...
  const results = useMemo(() => {
    ...
    const pickMedia = (post, rawText, stickers = []) => {
      ...
      const posterUrl = String(post?.posterUrl || post?.media?.posterUrl || '').trim()
      if (videoUrl) return buildSearchVideoMediaFn(videoUrl, posterUrl)
      ...
      for (const url of urls) {
        if (isVideoUrlFn(url) || isYouTubeUrlFn(url) || isTikTokUrlFn(url)) return buildSearchVideoMediaFn(url, String(post?.posterUrl || '').trim())
      }
      for (const url of urls) {
