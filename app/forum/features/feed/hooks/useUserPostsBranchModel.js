...
function dispatchServerItemsMerge(payload, { projectionOwner = '', reset = false } = {}) {
  try {
...
export default function useUserPostsBranchModel({
  ...
  const repliesById = useMemo(() => {
    const map = new Map()
    if (!String(authorFilterUserId || '').trim()) return map
...
export function useForumProfileBranchRuntime({
  ...
  const [serverBranchPosts, setServerBranchPosts] = useState([])
 ...
 const normalizedProfileBranchUserId = useMemo(
    () => String(profileBranchUserId || '').trim(),
    [profileBranchUserId],
  )

  const profilePostsSource = useMemo(() => {
    if (profileBranchMode !== 'posts') return EMPTY_PROFILE_POSTS
    ...
    const isDeleted = (post) => {
      ...
      return Boolean(id && deletedPosts?.[id])
    }
    if (!serverBranchPosts.length) return (posts || []).filter((post) => !isDeleted(post))

    const byId = new Map()

    for (const post of (posts || [])) {
      if (isDeleted(post)) continue
      const id = String(
        post?.id ||
        post?.postId ||
        ''
    ...
    for (const post of serverBranchPosts) {
      if (isDeleted(post)) continue
      const id = String(
        post?.id ||
        post?.postId ||
  ...
  useEffect(() => {
    ...
    const safeBranchSort =
      mode === 'topics'
        ? normalizeProfileBranchSort(topicSort)
        : normalizeProfileBranchSort(postSort)

    const key = `${mode}:${userId}:${safeBranchSort}`
    const projectionOwner = forumProfileBranchProjectionOwner(mode, userId, safeBranchSort)
    let cancelled = false
    ...
    const load = async () => {
      try {
        ...
        if (mode === 'posts') {
          const branchPosts = merged.posts.map((post) => ({
            ...post,

            // userPostsPage is already alias-aware.
            // If this row came back for this opened profile branch,
            // this branch userId is the authoritative canonical author.
            canonicalAuthorId: userId,
          }))

          setServerBranchPosts(branchPosts)

          dispatchServerItemsMerge({
            ...merged,
            posts: branchPosts,
          }, { projectionOwner, reset: true })
        } else {
          setServerBranchTopics(merged.topics)
          dispatchServerItemsMerge(merged, { projectionOwner, reset: true })
        }
