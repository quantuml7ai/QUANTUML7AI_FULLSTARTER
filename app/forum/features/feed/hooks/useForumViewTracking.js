...
export default function useForumViewTracking({
  ...
  useEffect(() => {
    ...
    const pendingRemovedCards = new Set()
    let removedFlushRaf = 0

    const publishTrackingState = () => {
      if (process.env.NODE_ENV === 'production') return
      try {
        window.__forumViewTrackingState = () => {
          let observedConnected = 0
          let observedDisconnected = 0
          observedCards.forEach((card) => {
            if (card?.isConnected) observedConnected += 1
            else observedDisconnected += 1
          })
          let focusedDisconnected = 0
          focused.forEach((rec) => {
            if (rec?.el && !rec.el.isConnected) focusedDisconnected += 1
          })
          return {
            observedConnected,
            ...
            focusedDisconnected,
            pendingRemoved: pendingRemovedCards.size,
            timers: Array.from(focused.values()).filter((rec) => !!rec?.t).length,
          }
        }
      } catch {}
    }
    ...
    // Poster/image loading remains browser-owned; no global speculative prewarm runtime.
    // Keep the hot view-IntersectionObserver path free of document-wide card scans
    // and duplicate Image() preloaders.
    ...
    const unobserveCard = (card) => {
      try {
        if (!(card instanceof Element)) return
        io.unobserve(card)
        observedCards.delete(card)
        const postId = String(card.getAttribute('data-forum-post-id') || card.dataset?.forumPostId || '').trim()
        if (postId) {
          clearFocusedTimer(postId)
          focused.delete(postId)
        }
        try { delete card.dataset.__viewObs } catch {}
      } catch {}
    }

    const collectRemovedCards = (node) => {
      try {
        if (!(node instanceof Element)) return
        if (node.matches?.(CARD_SELECTOR)) pendingRemovedCards.add(node)
        node.querySelectorAll?.(CARD_SELECTOR)?.forEach?.((card) => pendingRemovedCards.add(card))
      } catch {}
    }

    const flushRemovedCards = () => {
      removedFlushRaf = 0
      const cards = Array.from(pendingRemovedCards)
      pendingRemovedCards.clear()
      cards.forEach((card) => {
        if (card?.isConnected) {
    ...
    const scheduleRemovedCardsFlush = () => {
      if (removedFlushRaf) return
      try { removedFlushRaf = window.requestAnimationFrame(flushRemovedCards) } catch { flushRemovedCards() }
    }
      ...
      mo = new MutationObserver((mutList) => {
        ...
        for (const m of mutList) {
          ...
          ;(m.removedNodes || []).forEach((n) => {
            hasRemoved = true
            collectRemovedCards(n)
          })
        }
        if (hasRemoved) scheduleRemovedCardsFlush()
        publishTrackingState()
      })
    ...
    return () => {
      if (removedFlushRaf) {
        try { window.cancelAnimationFrame(removedFlushRaf) } catch {}
        removedFlushRaf = 0
      }
      pendingRemovedCards.clear()
      observedCards.clear()
      ...
      if (process.env.NODE_ENV !== 'production') {
        try { delete window.__forumViewTrackingState } catch {}
      }
    }
  }, [
    authAccountId,
