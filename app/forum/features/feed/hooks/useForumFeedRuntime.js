...
} from '../utils/transientProjectionRetention'
...
function dispatchTopicRootPostsMerge(posts, topicId, { projectionOwner = '', reset = false } = {}) {
  ...
  if (!cleanPosts.length && !(reset && String(projectionOwner || '').trim())) return
  try {
    window.dispatchEvent(new CustomEvent('forum:server-items-merge', {
      detail: {
        source: 'topic_roots_server_hydrate',
        posts: cleanPosts,
        projectionOwner: String(projectionOwner || '').trim(),
...
const threadBranchLoadKeyRef = useRef('')
const threadOpenHydrationKeyRef = useRef('')
const selectedTopicTransitionRef = useRef(String(selectedTopicId || '').trim())
const activeTopicProjectionOwnerRef = useRef('')
const activeThreadProjectionOwnerRef = useRef('')
...
useEffect(() => {
  ...
  const previousTopicId = String(selectedTopicTransitionRef.current || '').trim()
  const isOptimisticReconcile =
    previousTopicId.startsWith('tmp_t_') &&
    !!topicId &&
    !topicId.startsWith('tmp_t_')

  if (isOptimisticReconcile) return

  setVisibleThreadPostsCount(threadPageSize)
}, [postSort, selectedTopicId, setVisibleThreadPostsCount, threadPageSize, threadRoot])

const loadTopicRootsPage = useCallback(async ({ reset = false, preserveReady = false } = {}) => {
  const topicId = String(selectedTopicId || '').trim()
  const optimisticTopic = topicId.startsWith('tmp_t_')

  if (!topicId || optimisticTopic || threadRoot || typeof api?.threadPage !== 'function') {
    return {
      ok: false,
      skipped: true,
      reason: optimisticTopic ? 'optimistic_topic' : 'inactive',
    }
  }
...
setTopicRootsState((prev) => ({
  key: topicId,
  loading: true,
  loaded: reset ? Boolean(preserveReady) : Boolean(prev.loaded && prev.key === topicId),
  ...
  const loadThreadBranchPage = useCallback(async () => {
    ...
    const targetPostId = String(
      threadRoot?.__threadTargetPostId ||
      threadRoot?.id ||
      threadRoot?.postId ||
      '',
    ).trim()
    const rootPostId = String(
      threadRoot?.__threadCanonicalRootId ||
      threadRoot?.rootPostId ||
      threadRoot?.thread?.rootPostId ||
      targetPostId,
    ).trim()
    ...
    try {
      const page = await api.threadPage({
        ...
        topicId,
        rootPostId,
        targetPostId: targetPostId || rootPostId,
        pageSize: 160,
        limit: 160,
        sort: postSort || 'new',
        feedMode: 'world',
      ...
      const posts = normalizeThreadPagePosts(page)
        .filter((post) => String(post.topicId || topicId) === topicId)
      dispatchTopicRootPostsMerge(posts, topicId, {
        projectionOwner: forumThreadProjectionOwner(topicId, rootPostId),
        reset: true,
      })
...
useEffect(() => {
  ...
  selectedTopicTransitionRef.current = topicId
  ...
  const timer = setTimeout(() => {
    loadTopicRootsPage({
      reset: true,
      preserveReady: isOptimisticReconcile,
    })
  }, 0)
  ...
  useEffect(() => {
    ...
    if (!threadRoot) {
      threadBranchLoadKeyRef.current = ''
      threadOpenHydrationKeyRef.current = ''
      return undefined
    }
    if (threadRoot?.__threadOpening === true) return undefined

    const topicId = String(selectedTopicId || '').trim()
    const targetPostId = String(threadRoot?.__threadTargetPostId || threadRoot?.id || threadRoot?.postId || '').trim()
    const canonicalRootPostId = String(
      threadRoot?.__threadCanonicalRootId ||
      threadRoot?.rootPostId ||
      threadRoot?.thread?.rootPostId ||
      targetPostId,
    ...
    const loadKey = [
      ...
      targetPostId,
      String(postSort || 'new'),
      String(contentRefreshToken || 0),
    ].join('|')

    const openHydrationKey = [
      topicId,
      canonicalRootPostId,
      targetPostId,
      String(threadRoot?.__threadHydratedSort || ''),
    ].join('|')
    if (
      threadRoot?.__threadOpenHydrated === true &&
      String(threadRoot?.__threadHydratedSort || '') === String(postSort || 'new') &&
      threadOpenHydrationKeyRef.current !== openHydrationKey
    ) {
      threadOpenHydrationKeyRef.current = openHydrationKey
      threadBranchLoadKeyRef.current = loadKey
      return undefined
    }
    if (threadBranchLoadKeyRef.current === loadKey) return undefined
    threadBranchLoadKeyRef.current = loadKey
  ...
  // Transient server pages are scoped to the currently reachable topic/thread.
  // Releasing a scope removes only projection-only entities; canonical/home ids survive.
  useEffect(() => {
    const topicOwner = forumTopicRootsProjectionOwner(selectedTopicId)
    const previousTopicOwner = activeTopicProjectionOwnerRef.current
    if (previousTopicOwner && previousTopicOwner !== topicOwner) {
      dispatchForumTransientProjectionRelease(previousTopicOwner)
    }
    activeTopicProjectionOwnerRef.current = topicOwner

    const threadOwner = forumThreadProjectionOwner(
      selectedTopicId,
      threadRoot?.__threadCanonicalRootId ||
        threadRoot?.rootPostId ||
        threadRoot?.thread?.rootPostId ||
    ...
    const previousThreadOwner = activeThreadProjectionOwnerRef.current
    if (previousThreadOwner && previousThreadOwner !== threadOwner) {
      dispatchForumTransientProjectionRelease(previousThreadOwner)
    }
    activeThreadProjectionOwnerRef.current = threadOwner
  }, [
    selectedTopicId,
    threadRoot?.__threadCanonicalRootId,
    threadRoot?.id,
  ...
  useEffect(() => () => {
    dispatchForumTransientProjectionRelease(activeThreadProjectionOwnerRef.current)
    dispatchForumTransientProjectionRelease(activeTopicProjectionOwnerRef.current)
    activeThreadProjectionOwnerRef.current = ''
    activeTopicProjectionOwnerRef.current = ''
  }, [])
