...
function dispatchDeepLinkServerMerge(options = {}) {
  if (typeof window === 'undefined') return
  const cleanTopicId = String(options?.topicId || '').trim()
  ...
  const topics = cleanTopicId
    ? [{ id: cleanTopicId, topicId: cleanTopicId, title: '', description: '', source: 'server_deeplink_locate' }]
    : []
  const hasTopicsProjection = topics.length > 0
  const projectionOwner = String(options?.projectionOwner || '').trim()
  const reset = options?.reset === true
  if (!cleanPosts.length && !topics.length && !(reset && projectionOwner && (hasPostsProjection || hasTopicsProjection))) return
  try {
    window.dispatchEvent(new CustomEvent('forum:server-items-merge', {
      detail: {
        ...(hasTopicsProjection ? { topics } : null),
        ...(hasPostsProjection ? { posts: cleanPosts } : null),
        reason: options?.reason || 'deeplink_server_hydrate',
        rev: Date.now(),
        projectionOwner,
...
export default function useForumDeepLinkFlow({
  ...
  useEffect(() => {
    ...
    if (!postId) return
    let activeThreadProjectionOwner = ''
    let activeTopicProjectionOwner = ''
    ...
    const fail = () => {
      ...
      st.done = true
      st.failed = true
      if (activeThreadProjectionOwner) {
        dispatchForumTransientProjectionRelease(activeThreadProjectionOwner)
        activeThreadProjectionOwner = ''
      }
      if (activeTopicProjectionOwner) {
        dispatchForumTransientProjectionRelease(activeTopicProjectionOwner)
        activeTopicProjectionOwner = ''
      }
    ...
    ;(async () => {
      ...
      // The deep-link target is the eventual thread head, so it shares the same
      // projection owner as openThreadForPost and remains valid after query cleanup.
      activeThreadProjectionOwner = forumThreadProjectionOwner(topicId, postId)
      activeTopicProjectionOwner = forumTopicRootsProjectionOwner(topicId)
      try {
        ...
        dispatchDeepLinkServerMerge({
          topicId,
          reason: 'deeplink_topic_hydrate',
          projectionOwner: activeTopicProjectionOwner,
          reset: true,
        })
        dispatchDeepLinkServerMerge({
          posts: hydratePosts,
          reason: 'deeplink_branch_hydrate',
          projectionOwner: activeThreadProjectionOwner,
          reset: true,
        })
      } catch {}
      ...
      // The selected-topic runtime now owns this same logical projection key.
      activeTopicProjectionOwner = ''
      ...
      try {
        ...
        // openThreadForPost now owns the same projection key. Do not release it
        // from the deep-link helper after successful handoff.
        activeThreadProjectionOwner = ''
      } catch {}
