...
function dispatchServerItemsMerge(posts, source = 'thread_open', { projectionOwner = '', reset = false } = {}) {
...
export default function useThreadOpenNavigation({
  ...
  useEffect(() => {
    ...
    const onAuthoritativePostDeleted = (event) => {
      const detail = event?.detail && typeof event.detail === 'object' ? event.detail : {}
      const deletedIds = new Set([
        ...(Array.isArray(detail.deletedPostIds) ? detail.deletedPostIds : []),
        detail.postId,
      ].map(str).filter(Boolean))
      const currentRootId = str(threadRootRef.current?.id || threadRootRef.current?.postId)
      if (!currentRootId || !deletedIds.has(currentRootId)) return

      threadOpenSeqRef.current += 1
      ...
      try { if (navPendingThreadRootRef) navPendingThreadRootRef.current = null } catch {}
      try { setThreadRoot(null) } catch {}
      setOpeningState({ active: false, targetId: '', status: 'idle', error: '' })
    }

    window.addEventListener('forum:authoritative-post-deleted', onAuthoritativePostDeleted)
    return () => window.removeEventListener('forum:authoritative-post-deleted', onAuthoritativePostDeleted)
  }, [isBrowserFn, navPendingThreadRootRef, setOpeningState, setThreadRoot])

  const createThreadSeed = useCallback((post) => {
    const id = str(post?.id || post?.postId)
    if (!id) return null
    const canonicalRootPostId = str(
      post?.__threadCanonicalRootId ||
      post?.rootPostId ||
      post?.thread?.rootPostId,
    )
    return {
      ...post,
      id,
      ...
      ...(canonicalRootPostId ? { rootPostId: canonicalRootPostId, __threadCanonicalRootId: canonicalRootPostId } : null),
      __threadTargetPostId: id,
  ...
  const hydrateThreadTarget = useCallback(async (seed, seq, attempt = 0, sortOverride = '') => {
    ...
    const initialCanonicalRootPostId = str(
      seed?.__threadCanonicalRootId ||
      seed?.rootPostId ||
      seed?.thread?.rootPostId ||
      targetId,
    )
    const initialProjectionOwner = forumThreadProjectionOwner(topicId, initialCanonicalRootPostId)
    ...
    const applyReady = (targetPost = null, canonicalRootPostId = targetId) => {
      if (!isCurrent()) return
      const canonicalRootId = str(canonicalRootPostId || targetId) || targetId
      const nextRoot = {
        ...
        rootPostId: canonicalRootId,
        __threadCanonicalRootId: canonicalRootId,
        __threadTargetPostId: targetId,
        __threadHydratedSort: effectivePostSort,
        __threadOpenHydrated: true,
    ...
    try {
      ...
      const projectionOwner = forumThreadProjectionOwner(topicId, rootPostId)
      const locatedPosts = []
      ...
      dispatchServerItemsMerge([seed, ...locatedPosts], 'thread_open_locate', { projectionOwner })
      if (initialProjectionOwner && initialProjectionOwner !== projectionOwner) {
        dispatchForumTransientProjectionRelease(initialProjectionOwner)
      }
      try {
        setThreadRoot({
          ...seed,
          rootPostId,
          __threadCanonicalRootId: rootPostId,
          __threadTargetPostId: targetId,
          __threadOpening: true,
          __threadSeed: true,
        })
      ...
      dispatchServerItemsMerge([seed, ...pagePosts], 'thread_open_page', { projectionOwner })
      applyReady(targetPost, rootPostId)
    } catch (error) {
      if (!isCurrent()) return
  ...
  const openThreadForPost = useCallback((post, opts = {}) => {
    ...
    const projectionOwner = forumThreadProjectionOwner(topicId, canonicalRootId)
    threadOpenSeqRef.current = seq
    try { setPostSort?.('new') } catch {}
    setOpeningState({ active: true, targetId: rootId, status: 'locating', error: '' })
    dispatchServerItemsMerge([seed], 'thread_open_seed', { projectionOwner, reset: true })
    ...
    if (!topic) {
      dispatchForumTransientProjectionRelease(projectionOwner)
  ...
  useEffect(() => {
    ...
    if (pendingId) {
      ...
      if (seed) {
        ...
        const projectionOwner = forumThreadProjectionOwner(seed.topicId || selId, canonicalRootId)
        dispatchServerItemsMerge([seed], 'thread_open_pending_seed', { projectionOwner, reset: true })
        try { hydrateThreadTarget(seed, seq, 0, 'new') } catch {}
      }
      return
    }
