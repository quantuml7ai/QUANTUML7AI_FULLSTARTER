...
export default function useForumMutationActions({
  ...
  const reactMut = useCallback(async (post, kind) => {
    ...
    try {
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('forum:post-reaction-overlay', {
          detail: { postId, state: nextState, likes, dislikes },
        }))
      }
    } catch {}
  ...
  const delTopicOwn = useCallback(async (topic) => {
    ...
    const alreadyDeleted = String(r?.error || '') === 'forum_err_not_found'
    if (r?.ok || alreadyDeleted) {
      // A second client may still render an entity that another linked session
      // already deleted. Treat authoritative not-found as idempotent success and
      // keep the local tombstone instead of resurrecting stale projection state.
  ...
  const delPostOwn = useCallback(async (post) => {
    ...
    const alreadyDeleted = String(r?.error || '') === 'forum_err_not_found'
    if (r?.ok || alreadyDeleted) {
      const deletedIds = Array.from(new Set((Array.isArray(r?.applied) ? r.applied : [])
        .flatMap((item) => [
          ...(Array.isArray(item?.deletedPostIds) ? item.deletedPostIds : []),
          ...(Array.isArray(item?.deleted) ? item.deleted : []),
          item?.postId,
        ])
        .concat(postId)
      ...
      if (deletedIds.length > 1) {
        persistTombstones((prev) => ({
          ...
          posts: {
            ...(prev.posts || {}),
            ...Object.fromEntries(deletedIds.map((id) => [id, Date.now()])),
          },
        }))
      }
      try {
