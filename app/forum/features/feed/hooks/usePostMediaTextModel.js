...
export function extractPostPrewarmHints({ text = '', posterUrl = '', limit = 4 } = {}) {
  const raw = String(text || '')
  const lines = raw.split(/\r?\n/)
  const images = Array.from(new Set([
    ...lines.map((line) => String(line || '').trim()).filter((line) => IMG_RE.test(line)),
    ...collectMatches(lines, IMG_RE),
  ]))
  const hints = []
  const poster = String(posterUrl || '').trim()
  if (poster) hints.push({ kind: 'poster', src: poster })
  for (const src of images) hints.push({ kind: 'image', src })
