...
function mergeFreshProjectionEntity(prev, item, id) {
  ...
  return str(fresh.__ql7ServerFeedSurface).toLowerCase() === 'home'
    ? promoteForumTransientProjectionMetadata(out)
    : out
}

function keepPreviousBranchPost(item) {
  if (!item || typeof item !== 'object') return false
  const projectionOwners = item?.[FORUM_TRANSIENT_PROJECTION_OWNERS_FIELD]
  if (Array.isArray(projectionOwners) && projectionOwners.length > 0) return true
    ...
    const hydrateProfilesForItems = async (posts, topics) => {
      ...
      try {
        ...
        if (pm?.ok && pm?.map && typeof pm.map === 'object') {
          ...
          vipIds = Array.from(new Set([
            ...ids.map((rawId) => str(aliases?.[rawId] || rawId)),
            ...Object.keys(pm.map || {}).map(str),
          ].filter(Boolean)))
          setProfileBump((x) => x + 1)
        }
      } catch {}
      try { queueVipProbes(vipIds) } catch {}
    }
