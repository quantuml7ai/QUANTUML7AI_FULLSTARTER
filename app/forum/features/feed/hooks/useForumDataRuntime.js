...
function mergeByForumId(prevItems, incomingItems) {
  return mergeForumCanonicalProjection(prevItems, incomingItems)
}
...
export default function useForumDataRuntime({
  ...
  // Transient server projections (thread/topic/media/profile/published) may hydrate
  // the canonical client store while their surface is mounted, but must release
  // projection-only entities when that surface closes. Canonical/home merges
  // promote matching ids and therefore survive a later projection release.
  React.useEffect(() => {
    if (!isBrowserFn()) return undefined
    const onMerge = (event) => {
      try {
        const detail = event?.detail && typeof event.detail === 'object' ? event.detail : {}
        const hasTopicsProjection = Array.isArray(detail.topics)
        const hasPostsProjection = Array.isArray(detail.posts)
        const incomingTopics = hasTopicsProjection ? detail.topics : []
        const incomingPosts = hasPostsProjection ? detail.posts : []
        const projectionOwner = String(detail.projectionOwner || '').trim()
        const projectionReset = detail.projectionReset === true
        const hasProjectionWork = projectionOwner && projectionReset && (hasTopicsProjection || hasPostsProjection)
        if (!incomingTopics.length && !incomingPosts.length && !hasProjectionWork) return
        persistSnap((prev) => ({
          ...prev,
          topics: projectionOwner
            ? (hasTopicsProjection
                ? mergeForumTransientProjection(prev?.topics || [], incomingTopics, { owner: projectionOwner, reset: projectionReset })
                : (prev?.topics || []))
            : (incomingTopics.length ? mergeByForumId(prev?.topics || [], incomingTopics) : (prev?.topics || [])),
          posts: projectionOwner
            ? (hasPostsProjection
                ? mergeForumTransientProjection(prev?.posts || [], incomingPosts, { owner: projectionOwner, reset: projectionReset })
                : (prev?.posts || []))
            : (incomingPosts.length ? mergeByForumId(prev?.posts || [], incomingPosts) : (prev?.posts || [])),
          // Hydration/branch merge events may use Date.now() as a local event stamp.
          // They are not authoritative forum revisions and must never advance snap.rev.
    ...
    const onRelease = (event) => {
      try {
        const detail = event?.detail && typeof event.detail === 'object' ? event.detail : {}
        const projectionOwner = String(detail.owner || '').trim()
        if (!projectionOwner) return
        persistSnap((prev) => {
          const topics = releaseForumTransientProjection(prev?.topics || [], projectionOwner)
          const posts = releaseForumTransientProjection(prev?.posts || [], projectionOwner)
          if (topics === prev?.topics && posts === prev?.posts) return prev
          return { ...prev, topics, posts }
        })
      } catch {}
    }
    window.addEventListener('forum:server-items-merge', onMerge)
    window.addEventListener('forum:server-items-release', onRelease)
    return () => {
      window.removeEventListener('forum:server-items-merge', onMerge)
      window.removeEventListener('forum:server-items-release', onRelease)
    }
  ...
  const changeRevRef = React.useRef(0)
