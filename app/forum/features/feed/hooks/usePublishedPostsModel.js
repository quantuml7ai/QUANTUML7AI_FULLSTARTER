...
function dispatchServerItemsMerge(posts, { projectionOwner = '', reset = false } = {}) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('forum:server-items-merge', {
      detail: {
        posts: items,
        rev: Date.now(),
        source: 'published-posts-server-bridge',
...
export default function usePublishedPostsModel({
  ...
  const projectionOwner = forumPublishedProjectionOwner(meId, safePostSort)
  ...
  }, [active, buildSortedPublished, meId, posts, resolveProfileAccountIdFn])
  ...
  const loadPublishedPostsPage = useCallback(async (options = {}) => {
    ...
    if (!active || !authorId || typeof fn !== 'function') return { ok: false, skipped: true, reason: 'inactive_or_missing_api_or_user' }
    const current = serverStateRef.current || {}
    // A reset supersedes any older page request (for example after sort/user
    // changes). Non-reset pagination still preserves the single-flight guard.
    if (current.loading && !reset) return { ok: false, skipped: true, reason: 'loading' }
    ...
    if (reset) dispatchForumTransientProjectionRelease(projectionOwner)
    setServerState((prev) => ({
      ...prev,
          key: serverKey,
          loading: true,
    ...
    try {
      ...
      dispatchServerItemsMerge(items, { projectionOwner, reset })
  ...
  }, [active, api, meId, pageLimit, projectionOwner, safePostSort, serverKey, isPostTombstonedNow])

  useEffect(() => {
    if (!active || !meId || typeof api?.userPostsPage !== 'function') {
  ...
  }, [active, api, loadPublishedPostsPage, meId, safePostSort, serverKey])

  useEffect(() => {
    if (!projectionOwner) return undefined
    if (!active) {
      dispatchForumTransientProjectionRelease(projectionOwner)
      return undefined
    }
    return () => dispatchForumTransientProjectionRelease(projectionOwner)
  }, [active, projectionOwner])
