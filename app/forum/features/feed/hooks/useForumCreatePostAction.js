...
export default function useForumCreatePostAction({
  ...
  const createPost = async () => {
    ...
    let uid = ''
    if (pendingVideo) {
      r = await requireAuthStrict?.()
      if (!r) return fail()
      // Forum security/write ownership follows the authenticated actor directly.
      // Profile aliases are presentation cache and must never rebind a moderation receipt.
      uid = String(r.asherId || r.accountId || '').trim()
    }
    ...
    // 2) Preserve the legacy auth timing for non-video posts; video authenticates earlier only for the pre-upload gate.
    if (!r) {
      r = await requireAuthStrict?.()
      if (!r) return fail()
      uid = String(r.asherId || r.accountId || '').trim()
    ...
    const p = {
      ...
      ...(videoUrlToSend ? { videoModerationStatus: 'approved' } : {}),
    }
...
pushOp('create_post', {
  ...
  parentId,
      ...
      id: tmpId,
      ...(videoPosterUrlToSend ? { posterUrl: videoPosterUrlToSend } : {}),
      ...(videoUrlToSend ? { videoApprovalToken } : {}),
    })
    ...
    emitPostCreated?.(p.id, selectedTopicId)
