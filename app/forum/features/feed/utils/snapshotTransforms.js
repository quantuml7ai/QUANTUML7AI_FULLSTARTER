...
import { mergeForumEntityPreserving, stripForumTransientProjectionMetadata } from './postMerge'
...
export function applyForumFullSnapshot(prev, snapshot, tombstones) {
  ...
  const topics = mergeStableOrder(prev.topics || [], snapshotTopics, (_prevTopic, freshTopic) => stripForumTransientProjectionMetadata({ ...freshTopic }))
  const posts = mergeStableOrder(prev.posts || [], snapshotPosts, (prevPost, freshPost) => stripForumTransientProjectionMetadata({
...
export function applyForumEvents(prev, events, tombstones, overlay) {
  ...
  for (const evt of (events || [])) {
    ...
    if (kind === 'post_edit') {
      ...
      if (!prior || typeof evt?.data?.text !== 'string') continue
      const text = String(evt.data.text)
      postsById.set(id, { ...prior, text, textSnippet: text.slice(0, 480) })
      continue
    }

    if (kind === 'post_deleted') {
      const ids = Array.from(new Set([
        ...(Array.isArray(evt.deletedPostIds) ? evt.deletedPostIds : []),
        ...(Array.isArray(evt.deleted) ? evt.deleted : []),
        evt.id,
      ].map((value) => String(value || '').trim()).filter(Boolean)))
      ids.forEach((id) => deletedPosts.add(id))
      continue
    }
