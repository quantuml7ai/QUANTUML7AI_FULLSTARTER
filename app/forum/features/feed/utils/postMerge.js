...
export const FORUM_TRANSIENT_PROJECTION_ONLY_FIELD = '__ql7TransientProjectionOnly'
export const FORUM_TRANSIENT_PROJECTION_OWNERS_FIELD = '__ql7TransientProjectionOwners'

export function promoteForumTransientProjectionMetadata(item) {
  if (!item || typeof item !== 'object') return item
  if (!Object.prototype.hasOwnProperty.call(item, FORUM_TRANSIENT_PROJECTION_ONLY_FIELD)) return item
  const out = { ...item }
  delete out[FORUM_TRANSIENT_PROJECTION_ONLY_FIELD]
  return out
}

export function stripForumTransientProjectionMetadata(item) {
  if (!item || typeof item !== 'object') return item
  const hasOnly = Object.prototype.hasOwnProperty.call(item, FORUM_TRANSIENT_PROJECTION_ONLY_FIELD)
  const hasOwners = Object.prototype.hasOwnProperty.call(item, FORUM_TRANSIENT_PROJECTION_OWNERS_FIELD)
  if (!hasOnly && !hasOwners) return item
  const out = { ...item }
  delete out[FORUM_TRANSIENT_PROJECTION_ONLY_FIELD]
  delete out[FORUM_TRANSIENT_PROJECTION_OWNERS_FIELD]
  return out
}
...
export function mergeForumEntityPreserving(prevItem, nextItem) {
  ...
  if (id) out.id = id
  if (str(nextItem?.__ql7ServerFeedSurface).toLowerCase() === 'home') {
    return promoteForumTransientProjectionMetadata(out)
  }
  return out
}
