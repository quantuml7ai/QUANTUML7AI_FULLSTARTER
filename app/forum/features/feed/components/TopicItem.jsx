...
const TopicItem = React.memo(function TopicItem({
  ...
  useRenderManagedScope(ref, { kind: 'card', marginProfile: 'near100', initialNear: true })
  React.useEffect(() => {
  ...
  return (
    <div
      ...
      data-ql7-visual-scope="card" data-ql7-render-managed="1"
      data-feed-card="1"
        ...
        <div className="flex flex-col gap-3">
          {shouldRenderAuthorRow && (
            <div className="topicUserRow">
              <div
                ref={avatarRef}
                className="avaMini ql7AvatarBadgeHost"
              ...
              >
                ...
                <AvatarBadgeOverlay
                  vipActive={isVipAuthor}
                  showInfo={!!(rawUserId || authorId)}
                />
              </div>
              <button
                type="button"
                className={cls('nick-badge nick-animate')}
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  onUserInfoToggle?.(rawUserId, avatarRef.current, userPreview)
                ...
                translate="no"
              >
                <ForumNickText className="nick-text" textValue={displayNickname}>{displayNickname}</ForumNickText>
              </button>
