    ...
    : undefined
      ...
      {canLoadMoreThread && (
        <div className="loadMoreFooter">
          ...
          <LoadMoreSentinel
            ...
            pending={!!topicRootsLoadingActive}
            hasMore={!!canLoadMoreThread}
            loadKey={`${String(threadRoot?.id || 'topic_roots')}:${renderVisibleCount}:${Number(flatLength || 0)}:${Number(!!topicRootsHasMore)}:${Number(!!threadHasMore)}`}
            onVisible={() => {
      ...
      {(!threadRoot && !topicRootsPending && !topicRootsLoadingActive && flatLength === 0) && (
        <div className="meta forumFeedEmptyState">
          <span className="forumFeedEmptyStateText">{t('forum_no_posts_yet')}</span>
        </div>
      )}
    </div>
  )
