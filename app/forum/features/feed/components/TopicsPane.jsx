      ...
      >
        ...
        {isProfileTopicsBranch && !topicsLoading && !topicsHasMore && !(visibleTopics || []).length && (
          <div className="meta forumFeedEmptyState">
            <span className="forumFeedEmptyStateText">{t?.('forum_no_topics_yet') || t?.('forum_no_posts_yet')}</span>
          </div>
        )}
      </div>
      {!showInitialTopicsSkeleton && topicsHasMore && (
        <div className="loadMoreFooter">
          ...
          <LoadMoreSentinel
            ...
            loadKey={`${profileBranchMode || 'topics'}:${String(branchUserId || '')}:${Number((visibleTopics || []).length)}:${Number(sortedTopicsLength || 0)}`}
            onVisible={() =>
