...
function ImageCarousel({
  ...
  const activeSrc = images[clampIndex(activeIndex, images.length)] || ''
  ...
  return (
    <div className="postImagesCarousel" style={{ marginTop: 8 }}>
      ...
      >
        {/* eslint-disable-next-line @next/next/no-img-element -- the original media URL is reused as a decorative, transform-free ambient layer. */}
        <img
          key={`ambient:${activeSrc}`}
          src={activeSrc}
          alt=""
          aria-hidden="true"
          draggable={false}
          ...
          data-ambient-media-backdrop="post-image"
        />
        <div
        ...
        <div className="postVideo" style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {videoLines.map((src, i) => (
            <div
              ...
              data-stable-shell="1"
              data-windowing-keepalive={isVideoFeed ? "0" : "media"}
              style={{ margin: 0 }}
            >
{String(videoModerationStatus || '').toLowerCase() === 'pending' && (
  <span className="ql7VideoModerationBadge" aria-label="moderation">moderation</span>
)}
<VideoMediaComponent
