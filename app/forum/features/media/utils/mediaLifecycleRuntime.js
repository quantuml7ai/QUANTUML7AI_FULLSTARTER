...
function __isIOSWebKitMediaRuntime() {
  try {
    const ua = String((typeof navigator !== 'undefined' ? navigator.userAgent : '') || '')
    const classicIOS = /iP(hone|ad|od)/i.test(ua)
    const desktopModeIPad =
      /Macintosh/i.test(ua) &&
      Number((typeof navigator !== 'undefined' ? navigator.maxTouchPoints : 0) || 0) > 1
...
;(() => {
  // iOS/WebKit keeps the shared sound choice across reloads. If audible autoplay is later
  // rejected by WebKit, the coordinator persists muted=true until the next user sound choice.
  // Other runtimes keep the existing fresh-page muted safety policy for splash/BG audio.
  try {
    if (typeof window === 'undefined') return
    if (__isIOSWebKitMediaRuntime()) {
      const persistedMuted = readMutedPrefFromStorage()
      const initialMuted = typeof persistedMuted === 'boolean' ? persistedMuted : true
      writeMutedPrefToDocument(initialMuted, false)
      if (typeof persistedMuted !== 'boolean') writeMutedPrefToStorage(initialMuted)
      return
    }
