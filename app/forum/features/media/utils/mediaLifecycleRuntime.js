...
function __isChromeDesktopMediaRuntime() {
  try {
...
;(() => {
  // Every fresh Forum runtime starts from one shared muted state. Explicit sound choices
  // remain valid for the current runtime, while a hard reload is muted again on every browser.
  try {
    if (typeof window === 'undefined') return
...
const __SOFT_RESIDENT_POST_VIDEO = !__isChromeDesktopMediaRuntime()
...
const __MEDIA_RUNTIME_PROFILE = (() => {
  try {
    ...
    const dm = Number((typeof navigator !== 'undefined' ? navigator?.deviceMemory : 0) || 0)
    const lowMem = Number.isFinite(dm) && dm > 0 && dm <= 3
    return { isIOS, isAndroid, isChromeDesktop: __isChromeDesktopMediaRuntime(), coarse: coarse || isIOS || isAndroid, lowMem }
  } catch {
    return { isIOS: false, isAndroid: false, isChromeDesktop: false, coarse: true, lowMem: true }
  }
})()
...
const __MAX_ACTIVE_VIDEO_ELEMENTS = (() => {
  try {
    ...
    if (__MEDIA_RUNTIME_PROFILE.isChromeDesktop) return 2
    return 3
  } catch {
    return 2
  }
...
export function __enforceActiveVideoCap(exceptEl) {
  try {
    // QL7_FORUM_MEDIA_DETACHED_APP_GUARDS_R27_FINAL
    // Prune stale strong references before viewport geometry or media mutations.
    for (let i = __activeVideoLRU.length - 1; i >= 0; i -= 1) {
      const el = __activeVideoLRU[i]
      if (el?.isConnected) continue
      __activeVideoLRU.splice(i, 1)
      if (el) __activeVideoEls.delete(el)
    }
    for (const el of __activeVideoEls) {
      if (!el?.isConnected) __activeVideoEls.delete(el)
    }
    let guard = 0
    while (__activeVideoLRU.length > __MAX_ACTIVE_VIDEO_ELEMENTS && guard < 128) {
      ...
      const protectMargin = victimIsPostFeedVideo
        ? (__MEDIA_RUNTIME_PROFILE.isIOS ? 1600 : (__MEDIA_RUNTIME_PROFILE.coarse ? 1300 : (__MEDIA_RUNTIME_PROFILE.isChromeDesktop ? 420 : 760)))
        : 180
...
// QL7_FORUM_NATIVE_SRC_BUDGET_R25_STRICT_LIFECYCLE
const strictNativeSrcCapDetach = (() => {
  try {
    return isPostFeedVideo && String(el?.dataset?.__strictNativeSrcCapDetach || '') === '1'
  } catch {
    return false
  }
})()
...
// QL7_FORUM_NATIVE_VIDEO_POSTER_FIRST_LIGHT_PREWARM_R24_FIX2_RUNTIME
// Poster owns far/offscreen presentation. This shared lifecycle helper may retain a
// connected native forum source only inside a short hysteresis runway, and it must
// never upgrade a paused/prewarmed forum source from metadata to auto.
const shellVisible = isPostFeedVideo ? __isVideoNearViewport(el, __MEDIA_RUNTIME_PROFILE.isChromeDesktop ? 220 : 520) : false
const postLightResidentPx = (() => {
  if (!isPostFeedVideo) return 420
  if (__MEDIA_RUNTIME_PROFILE.isIOS) return 760
  if (__MEDIA_RUNTIME_PROFILE.coarse) return 680
  if (__MEDIA_RUNTIME_PROFILE.isChromeDesktop) return 420
  return 620
})()
const postLightHysteresisPx = (() => {
  if (!isPostFeedVideo) return 420
  if (__MEDIA_RUNTIME_PROFILE.isIOS) return 860
  if (__MEDIA_RUNTIME_PROFILE.coarse) return 760
  if (__MEDIA_RUNTIME_PROFILE.isChromeDesktop) return 480
  return 700
})()
const nearViewport = __isVideoNearViewport(el, isPostFeedVideo ? postLightResidentPx : 420)
const postPrewarmRunway = isPostFeedVideo
  ? __isVideoNearViewport(el, postLightHysteresisPx)
  : false
...
const protectableForcedPostSurface =
  isPostFeedVideo &&
  hardUnloadRequested &&
  !strictNativeSrcCapDetach &&
  !!el?.isConnected &&
  (
    nearViewport ||
    shellVisible ||
    hadPostPlaybackIntent ||
...
if (shouldSoftUnload) {
  ...
  try {
    ...
    el.dataset.__hardDetached = '0'
    el.preload = isPostFeedVideo ? 'metadata' : (keepWarmFetchOnSoftUnload ? 'auto' : 'metadata')
    el.dataset.__lastUnloadTs = String(nowTs)
  } catch {}
...
try {
  ...
  try { delete el.dataset.__strictNativeSrcCapDetach } catch {}
}
...
export function __restoreVideoEl(el) {
  ...
  try {
    ...
    delete el.dataset.__strictNativeSrcCapDetach
  } catch {}
...
export const NativeSafeVideoPlayer = React.forwardRef(function NativeSafeVideoPlayer(props, ref) {
