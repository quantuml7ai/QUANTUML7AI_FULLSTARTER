...
export function commandExternalVideo(frame, action, options = {}) {
  ...
  try {
    ...
    if (normalized) {
      ...
      const hasLiveSrc = !!String(frame.getAttribute('src') || '').trim()
      if (action === 'play' && !hasLiveSrc) frame.setAttribute('src', normalized)
    }
  } catch {}
  ...
  try {
    const postGenericMethod = (method) => {
      frame.contentWindow?.postMessage?.({ method }, '*')
      frame.contentWindow?.postMessage?.(method, '*')
    }
    if (action === 'mute' || action === 'unmute') {
      postGenericMethod(action)
      emitExternalVideoState(frame, { muted: action === 'mute' })
      return true
    }
    if (action === 'play') {
      if (nextMuted === true) postGenericMethod('mute')
      if (nextMuted === false) postGenericMethod('unmute')
      postGenericMethod('play')
      emitExternalVideoState(frame, { paused: false, muted: nextMuted })
      return true
    }
    if (action === 'pause') {
      postGenericMethod('pause')
      emitExternalVideoState(frame, { paused: true })
      return true
    }
  } catch {}
