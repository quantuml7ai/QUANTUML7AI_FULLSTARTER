...
function markProgrammaticVideoSnap(reason = 'video_feed_first_card_snap') {
  try {
    window.__forumProgrammaticScrollTs = Date.now()
    window.__forumProgrammaticScrollReason = String(reason || 'video_feed_first_card_snap')
  } catch {}
}

function firstVideoFeedPostCard(root, anchor) {
  try {
    const grid =
      root?.querySelector?.('[data-ql7-video-feed-grid="1"]') ||
      anchor?.parentElement?.querySelector?.('[data-ql7-video-feed-grid="1"]') ||
      document.querySelector?.('[data-ql7-video-feed-grid="1"]') ||
      null
    const fromGrid =
      grid?.querySelector?.('[data-feed-card="1"][data-feed-kind="post"]') ||
      grid?.querySelector?.('[data-video-feed-card="1"]')?.closest?.('[data-feed-card="1"]') ||
      null
    if (fromGrid instanceof Element) return fromGrid
    const fromRoot =
      root?.querySelector?.('[data-video-feed-card="1"]')?.closest?.('[data-feed-card="1"]') ||
      root?.querySelector?.('[data-feed-card="1"][data-feed-kind="post"]') ||
      null
    if (fromRoot instanceof Element) return fromRoot
  } catch {}
  return null
}
...
export function snapVideoFeedToFirstCardTop({
  ...
  const run = () => {
    try {
      ...
      if (anchorOnly && anchor) {
        ...
        if (anchorRect) {
          ...
          if (useInner) {
            ...
            markProgrammaticVideoSnap('video_feed_anchor_snap')
      ...
      const card = firstVideoFeedPostCard(root, anchor)
      if (!card) {
        if (opts?.fallbackToAnchor === true) {
      ...
      if (useInner) {
        ...
        markProgrammaticVideoSnap('video_feed_first_card_snap')
        scrollEl.scrollTop = Math.max(0, targetTop)
      } else {
