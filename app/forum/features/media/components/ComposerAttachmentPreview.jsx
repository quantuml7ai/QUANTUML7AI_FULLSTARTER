...
export default function ComposerAttachmentPreview({
  ...
  React.useEffect(() => {
    setVideoReady(false)
    setVideoPoster('')
    if (!pendingVideo || typeof URL === 'undefined') return undefined
    ...
    if (pendingVideoPosterBlob instanceof Blob && pendingVideoPosterBlob.size) {
      try {
        posterObjectUrl = URL.createObjectURL(pendingVideoPosterBlob)
        setVideoPoster(posterObjectUrl)
      } catch {}
    }
  ...
  }, [pendingVideo, pendingVideoPosterBlob])
