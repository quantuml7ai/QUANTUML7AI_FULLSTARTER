...
export default function ComposerAttachmentPreview({
  ...
  React.useEffect(() => {
    ...
    createNativeVideoPoster({
      source: pendingVideo,
      cacheKey: pendingVideo,
      mirrorX: pendingVideoMirror,
      signal: controller.signal,
      sourceName: 'composer-video',
    }).then((record) => {
      if (disposed || !record?.blob?.size) return
      posterObjectUrl = URL.createObjectURL(record.blob)
      if (!disposed) setVideoPoster(posterObjectUrl)
    }).catch(() => {})
