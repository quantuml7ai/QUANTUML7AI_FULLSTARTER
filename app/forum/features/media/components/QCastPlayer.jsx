...
export default function QCastPlayer({
  ...
  const qcastFxProfile = React.useMemo(() => {
    if (typeof window === 'undefined') return { viz: false, boom: false, burst: 2, pool: 0, fullscreen: true }
    try {
  ...
  const spawnFx = React.useCallback((kind, origin) => {
    ...
    for (let i = 0; i < burst; i++) {
      ...
      const dx = Math.round((Math.random() - 0.5) * 360)
      ...
      const dur = Math.round(1100 + Math.random() * 900)
      const delay = Math.round(i * (10 + Math.random() * 18))
      const hue = Math.round((Math.random() - 0.5) * 44)
  ...
  }, [GOOD_SET, BAD_SET, FX_VARIANTS, FX_BURST_BASE, qcastFxProfile.fullscreen, rearmPooledFx])
  ...
  const applyPreset = React.useCallback((st, nextPreset) => {
    ...
    if (nextPreset === 'rock') {
      ...
      setG(0, 0); setG(1, 0); setG(2, 0); setG(3, 0); setG(4, 0); setG(5, 0)
    }
  }, [])
  ...
  React.useEffect(() => {
    if (!qcastFxProfile.viz) return undefined
    ...
    const resize = () => {
      try {
        ...
        const h = Math.max(32, Math.round(rect?.height || 0))
    ...
    try {
      if (typeof ResizeObserver !== 'undefined') {
        ...
        if (hostRef.current) st.ro.observe(hostRef.current)
      }
    } catch {}
    ...
    const paint = () => {
      ...
      try {
        ...
        if (!isPlaying || !st.analyser || !st.data) return
        ...
        const step = Math.max(1, Math.floor(st.data.length / bars))
        const baseY = h - 14

        for (let i = 0; i < bars; i++) {
          const sample = st.data[Math.min(st.data.length - 1, i * step)] || 0
          const amp = sample / 255
          const bh = Math.max(6, Math.round(amp * (h * 0.34)))
          const x = i * (barW + gap)
          const y = baseY - bh

          ctx2d.fillStyle = i % 2 === 0 ? 'rgba(97, 214, 255, 0.18)' : 'rgba(255, 110, 214, 0.14)'
          ctx2d.fillRect(x, y, barW, bh)

          ctx2d.fillStyle = i % 2 === 0 ? 'rgba(125, 228, 255, 0.82)' : 'rgba(255, 156, 224, 0.72)'
          ctx2d.fillRect(x, y + Math.max(0, bh - 10), barW, Math.min(10, bh))
        }

        const glow = ctx2d.createLinearGradient(0, 0, 0, h)
        glow.addColorStop(0, 'rgba(84, 202, 255, 0.10)')
        glow.addColorStop(0.55, 'rgba(32, 110, 168, 0.04)')
    ...
    return () => {
      ...
      st.ro = null
      try {
        ctx2d.clearRect(0, 0, st.w || 0, st.h || 0)
      } catch {}
    }
  }, [isPlaying, qcastFxProfile.viz])
  ...
  React.useEffect(() => {
    ...
    const ensureAudioGraph = async () => {
      ...
      if (!st.ctx) {
        ...
        try {
          ...
          if (st.panner) {
            st.master.connect(st.panner)
            st.panner.connect(st.ctx.destination)
          } else {
    ...
    return () => {
      ...
      try {
        if (Array.isArray(st.filters)) st.filters.forEach((f) => f?.disconnect?.())
      } catch {}
  ...
  }, [desiredMuted, mutedEventName, readQcastMutedPref, syncMasterGain, writeQcastMutedPref])
  ...
  React.useEffect(() => {
    ...
    const onMutedSync = (e) => {
      try {
        ...
        if (source !== 'qcast' && next && hasManualUnmute) return
        if (source !== 'qcast' && next && userUnmuteHoldUntil > Date.now()) return
        if (source !== 'qcast' && next && persistedPref === false) return
  ...
  const toggleSurround = async () => {
    ...
    try {
      if (waRef.current.panner) waRef.current.panner.pan.value = (!surround) ? 0.18 : 0
    } catch {}
  }
    ...
    >
      ...
      {FX_POOL > 0 ? (
        <div className="qcastFxLayer" data-scope={qcastFxProfile.fullscreen ? 'viewport' : 'card'} aria-hidden="true">
          ...
          {qcastFxProfile.boom && BOOM_POOL > 0
        ...
        <div className="qcastBottomBar">
          ...
          <div className="qcastBottomRight qcastDesktopOnly">
            ...
            {preview && typeof onRemove === 'function' && (
              <button type="button" className="qcastBtn qcastBtn--danger" onClick={onRemove} title="Remove" aria-label="Remove">
                ✕
              </button>
            )}
          </div> 
          ...
          <div className="qcastSettingsBody">
            <div className="qcastSettingRow">
              ...
              <input
                ...
                className="qcastRange qcastRange--vol"
                aria-label="Volume"
              />
              ...
              <div className="qcastPresetChips" role="group" aria-label="EQ Preset">
                {['custom', 'rock', 'pop', 'classic'].map((p) => (
                  <button
                    key={p}
                    type="button"
                    className={`qcastChip ${preset === p ? 'active' : ''}`}
                    onClick={(e) => {
                      e.preventDefault()
                      e.stopPropagation()
                      setPresetSafe(p)
            ...
            <div className="qcastSettingRow">
              ...
              <button
                type="button"
                className={`qcastToggle ${surround ? 'on' : ''}`}
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  toggleSurround()
                }}
                aria-label="Surround toggle"
              >
