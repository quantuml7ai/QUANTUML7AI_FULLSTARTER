  ...
  const qcastFxProfile = React.useMemo(() => {
    ...
    try {
      ...
      return {
        ...
        pool: mobileLean ? 6 : 24,
        fullscreen: fullscreenFx,
      };
    } catch {
      return { viz: false, boom: false, burst: 2, pool: 6, fullscreen: true };
    }
  }, []);
...
const isQCastControlTarget = React.useCallback((target) => {
  try { return !!target?.closest?.('button, input, a, textarea, select, [role="button"], .qcastSettings'); } catch { return false; }
}, []);
...
const playWithSoundFromSurface = React.useCallback((gestureType = 'click') => {
  ...
  const now = Date.now();
  const gestureUntil = String(now + 5200);
  const leaseUntil = String(now + 90_000);
  const unmuteHoldUntil = String(now + (8 * 60 * 60 * 1000));
  const storedVolume = Number(audio.dataset?.__lastAudibleVolume || 0);
  const currentVolume = Number(audio.volume);
  const nextVolume = Number.isFinite(storedVolume) && storedVolume > 0.01
    ? storedVolume
    : (Number.isFinite(currentVolume) && currentVolume > 0.01 ? currentVolume : Math.max(0.01, Number(volume || 1)));

  let graphKick = null;
  let started = null;
  try {
    // Safari hot path: resume WebAudio and call HTMLAudioElement.play() in the
    // same trusted event before React state/storage/event fanout.
    try { graphKick = waRef.current?.__ensure?.(); } catch {}
    try {
      const ctx = waRef.current?.ctx;
      if (ctx?.state === 'suspended' && typeof ctx.resume === 'function') {
    ...
    audio.dataset.__surfaceGestureEvent = String(gestureType || 'unknown');
    audio.dataset.__surfaceGestureAt = String(now);
    try { audio.dataset.__surfaceGestureUserActive = navigator?.userActivation?.isActive ? '1' : '0'; } catch {}
    audio.dataset.__audibleGestureUntil = gestureUntil;
    audio.dataset.__userGestureUntil = gestureUntil;
    audio.dataset.__manualLeaseUntil = leaseUntil;
    audio.dataset.__userUnmuteHoldUntil = unmuteHoldUntil;
    audio.dataset.__qcastManualUnmute = '1';
    for (const key of ['__audibleAutoplayBlocked','__audibleAutoplayFallbackAt','__audibleAutoplayFallbackUntil','__audibleAutoplayFallbackPending','__autoplayFallbackMuted','__iosAudibleAutoplayAttempted','__iosAudibleAutoplaySucceeded','__skipMutePersistUntil','__userPaused','__userPausedAt','__suppressedPlayUntil']) delete audio.dataset[key];
    if (host?.dataset) {
      host.dataset.__surfaceGestureEvent = String(gestureType || 'unknown');
      host.dataset.__surfaceGestureAt = String(now);
      host.dataset.__audibleGestureUntil = gestureUntil;
      host.dataset.__userGestureUntil = gestureUntil;
      host.dataset.__manualLeaseUntil = leaseUntil;
      host.dataset.__userUnmuteHoldUntil = unmuteHoldUntil;
      host.dataset.__qcastManualUnmute = '1';
      for (const key of ['__audibleAutoplayBlocked','__audibleAutoplayFallbackAt','__audibleAutoplayFallbackUntil','__audibleAutoplayFallbackPending','__autoplayFallbackMuted','__iosAudibleAutoplayAttempted','__iosAudibleAutoplaySucceeded','__userPaused','__userPausedAt','__suppressedPlayUntil']) delete host.dataset[key];
    }
    audio.muted = false;
    audio.defaultMuted = false;
    audio.volume = Math.min(1, Math.max(0.01, nextVolume));
    audio.removeAttribute?.('muted');
    audio.preload = 'auto';
    if (audio.ended) audio.currentTime = 0;
    audio.dataset.__lastAudibleVolume = String(audio.volume);
    audio.dataset.__persistMuteUntil = String(now + 4200);
    if (host?.dataset) host.dataset.__persistMuteUntil = String(now + 4200);
    syncMasterGain();
    started = audio.play?.();
    audio.dataset.__surfacePlayResult = 'pending';
  } catch (error) {
    try {
      audio.dataset.__surfacePlayResult = 'threw';
      audio.dataset.__surfacePlayError = String(error?.name || error?.message || 'play_threw').slice(0, 120);
    } catch {}
    return false;
  }
  ...
  if (started && typeof started.then === 'function') {
    started.then(() => {
      try { audio.dataset.__surfacePlayResult = 'resolved'; } catch {}
      syncMasterGain();
      try { window.dispatchEvent(new CustomEvent('site-media-play', { detail: { source: 'qcast', element: host || null, manual: true, id: playerIdRef.current, gestureType } })); } catch {}
    }).catch((error) => {
      try {
        audio.dataset.__surfacePlayResult = 'rejected';
        audio.dataset.__surfacePlayError = String(error?.name || error?.message || 'play_rejected').slice(0, 120);
      } catch {}
    });
  } else {
    try { audio.dataset.__surfacePlayResult = audio.paused ? 'unknown' : 'resolved-sync'; } catch {}
  }
  try { if (graphKick && typeof graphKick.catch === 'function') graphKick.catch(() => {}); } catch {}
  try { window.dispatchEvent(new CustomEvent(mutedEventName, { detail: { muted: false, source: 'qcast', id: playerIdRef.current } })); } catch {}
  return true;
}, [mutedEventName, syncMasterGain, volume, writeMuted, writeQcastMutedPref]);

const handleQCastTouchStart = React.useCallback((event) => {
  if (isQCastControlTarget(event?.target)) return;
...
}, [isQCastControlTarget]);

const handleQCastTouchMove = React.useCallback((event) => {
  const touch = event?.touches?.[0] || event?.changedTouches?.[0];
  if (!touch) return;
...
const handleQCastTouchEnd = React.useCallback((event) => {
  if (isQCastControlTarget(event?.target) || surfaceTouchRef.current.moved) return;
  try { event?.stopPropagation?.(); } catch {}
  surfaceTouchRef.current.touchHandledAt = Date.now();
  playWithSoundFromSurface('touchend');
}, [isQCastControlTarget, playWithSoundFromSurface]);

const handleQCastClick = React.useCallback((event) => {
  if (isQCastControlTarget(event?.target)) return;
  try { event?.stopPropagation?.(); } catch {}
  const audio = audioRef.current;
  if (Date.now() - Number(surfaceTouchRef.current.touchHandledAt || 0) < 900 && audio && !audio.paused && !audio.muted) return;
...
React.useEffect(() => {
  ...
  return (
    <div
      ...
      onTouchStartCapture={handleQCastTouchStart}
      onTouchMoveCapture={handleQCastTouchMove}
      onTouchEndCapture={handleQCastTouchEnd}
      onClickCapture={handleQCastClick}
     ...
     <audio ref={audioRef} src={src} preload="metadata" playsInline crossOrigin="anonymous" referrerPolicy="no-referrer" data-qcast-audio="1" className="qcastAudio" />
      {isIosSafariBrowser ? (
        <button
          ...
          type="button"
          aria-label="Play QCast with sound"
          data-ios-safari-media-gesture="qcast"
          style={{
            position: 'absolute',
            inset: 0,
            zIndex: 1,
