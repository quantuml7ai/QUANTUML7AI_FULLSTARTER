...
import { schedulePosterReleaseAfterPresentedFrame } from '../../../../../lib/forumVideoPosterPresentation'
...
export const NativeSafeVideoPlayer = React.forwardRef(function NativeSafeVideoPlayer(
  ...
  const ambientPosterUrl = String(poster || '').trim()
  const mirrorPosterUrl = shouldMirrorVideo ? ambientPosterUrl : ''
  const [mirrorPosterVisible, setMirrorPosterVisible] = React.useState(() => !!mirrorPosterUrl)
  ...
  React.useEffect(() => {
    const node = localRef.current
    const canShowPoster = !!mirrorPosterUrl && (!node || node.paused || node.ended)
    setMirrorPosterVisible(canShowPoster)
  }, [mirrorPosterUrl, src, shouldMirrorVideo])
  ...
  return (
    ...
    >
      {ambientPosterUrl ? (
        // eslint-disable-next-line @next/next/no-img-element -- immutable poster asset reused as a decorative ambient layer.
        <img
          src={ambientPosterUrl}
          alt=""
          aria-hidden="true"
          draggable={false}
          loading="lazy"
          ...
          className="ql7AmbientMediaBackdrop"
          data-ambient-media-backdrop="native-safe-video"
        />
      ) : null}
      ...
      <video
        ref={assignRef}
        src={src}
        poster={shouldMirrorVideo ? undefined : (poster || undefined)}
        ...
        onPlaying={(event) => {
          // Keep the already-mirrored poster covering the transformed <video> until
          // the browser is actually presenting decoded frames. Hiding on `play` is
          // too early and can expose the video's CSS-mirrored native poster briefly.
          setMirrorPosterVisible(false)
          try { onPlaying?.(event) } catch {}
        }}
        onPause={(event) => {
          setPlaying(false)
      ...
      {mirrorPosterVisible && mirrorPosterUrl ? (
        // Front-camera poster pixels are already mirrored at extraction time.
        // Keep them outside the scaleX(-1) video element so the poster is not mirrored twice.
        // eslint-disable-next-line @next/next/no-img-element -- R2/blob poster must render byte-for-byte without Next image transforms.
        <img
          src={mirrorPosterUrl}
          alt=""
          aria-hidden="true"
          draggable={false}
          data-front-camera-poster-layer="1"
          className="ql7NativeSafeMirrorPoster"
          style={{ objectFit: fit }}
        />
      ) : null}
...
const renderPoster = String(nativeVideoPoster || '').trim() || undefined
const feedPosterUrl = isPostVideo ? String(renderPoster || '').trim() : ''
const mirrorFeedPosterUrl = isPostVideo && shouldMirrorVideo ? feedPosterUrl : ''
const [feedPosterVisible, setFeedPosterVisible] = React.useState(() => !!feedPosterUrl)
const posterReleaseCancelRef = React.useRef(null)

const cancelPendingPosterRelease = React.useCallback(() => {
  try { posterReleaseCancelRef.current?.() } catch {}
  posterReleaseCancelRef.current = null
}, [])

React.useEffect(() => {
  cancelPendingPosterRelease()
  setFeedPosterVisible(!!feedPosterUrl)
  return cancelPendingPosterRelease
}, [cancelPendingPosterRelease, feedPosterUrl, src, shouldMirrorVideo])
      ...
      isPostVideo
        ? (
          coordinatorOwnsPostLifecycle
            ? (postPlaybackIntent ? 'auto' : (wantsWarm ? 'metadata' : 'none'))
...
try {
  ...
  if (isNewMediaNode && isPostVideo) {
    ...
    // Poster-first: with no src attached the coordinator owns the first metadata kick.
...
const videoNode = (
  <video
    ...
    poster={shouldMirrorVideo ? undefined : renderPoster}
    ...
    onPlaying={(event) => {
      // Keep the poster as the presentation owner until the browser has actually
      // presented a decoded video frame. `play`/`playing` alone can still expose a
      // black compositor frame on mobile Chromium/WebKit while the first frame lands.
      cancelPendingPosterRelease()
      if (feedPosterUrl) {
        posterReleaseCancelRef.current = schedulePosterReleaseAfterPresentedFrame(
          event.currentTarget,
          () => {
            posterReleaseCancelRef.current = null
            setFeedPosterVisible(false)
          },
        )
      }
      try { onPlayingProp?.(event) } catch {}
    }}
    onPause={() => {
      cancelPendingPosterRelease()
    }}
    onError={(event) => {
      cancelPendingPosterRelease()
      if (feedPosterUrl) setFeedPosterVisible(true)
      try { onVideoError(event) } catch {}
    }}
    {...rest}
  />
)
    ...
    >
      {feedPosterUrl ? (
        ...
        <img
          ...
          referrerPolicy="no-referrer"
          className="ql7AmbientMediaBackdrop"
          data-ambient-media-backdrop="post-video"
        />
      ) : null}
      {videoNode}

      {feedPosterVisible && feedPosterUrl ? (
        // Poster remains above the native video until the first real frame is presented.
        // Front-camera poster pixels are already mirror-correct, so this independent
        // layer also avoids applying the video's scaleX(-1) transform a second time.
        // eslint-disable-next-line @next/next/no-img-element -- immutable poster asset; no image transform is allowed here.
        <img
          src={feedPosterUrl}
          alt=""
          aria-hidden="true"
          ...
          decoding="async"
          fetchPriority="high"
          data-forum-video-poster-layer="1"
          data-front-camera-feed-poster-layer={mirrorFeedPosterUrl ? '1' : undefined}
          style={{
            position: 'absolute',
            inset: 0,
            zIndex: 2,
