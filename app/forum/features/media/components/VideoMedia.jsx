...
const mirrorFeedPosterUrl = isPostVideo && shouldMirrorVideo ? String(renderPoster || '').trim() : ''
const [mirrorFeedPosterVisible, setMirrorFeedPosterVisible] = React.useState(() => !!mirrorFeedPosterUrl)

React.useEffect(() => {
  const node = ref.current
  const canShowPoster = !!mirrorFeedPosterUrl && (!node || node.paused || node.ended)
  setMirrorFeedPosterVisible(canShowPoster)
}, [mirrorFeedPosterUrl, src, shouldMirrorVideo])
...
const videoNode = (
  <video
    ...
    onPlaying={(event) => {
      // Persisted front-camera poster pixels are already mirror-correct. The video
      // itself remains scaleX(-1), so release the independent poster only once real
      // decoded playback is on screen. This avoids applying the mirror twice.
      if (mirrorFeedPosterUrl) setMirrorFeedPosterVisible(false)
  ...
  return (
    ...
    >
      ...
      {mirrorFeedPosterVisible && mirrorFeedPosterUrl ? (
        // Front-camera poster pixels are already mirrored during poster extraction.
        // Render them outside the scaleX(-1) <video> so poster and decoded video
        // each receive exactly one horizontal mirror.
        // eslint-disable-next-line @next/next/no-img-element -- immutable R2 poster asset; no image transform is allowed here.
        <img
          src={mirrorFeedPosterUrl}
          alt=""
          aria-hidden="true"
          draggable={false}
          data-front-camera-feed-poster-layer="1"
          style={{
            position: 'absolute',
            inset: 0,
            zIndex: 2,
