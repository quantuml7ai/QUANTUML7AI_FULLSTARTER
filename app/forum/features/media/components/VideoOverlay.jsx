...
function getOverlayCameraVideoConstraints() {
  if (isMobileCameraRuntime()) {
    // The first mobile front-camera open must negotiate exactly like the working flip-back path:
    // select the user-facing camera, but do not force portrait width/height/frame-rate presets.
    return {
...
export default function VideoOverlay({
  ...
  const [previewPoster, setPreviewPoster] = React.useState('')

  React.useEffect(() => {
    setPreviewPoster('')
    if (!open || st !== 'preview' || mediaKind !== 'video' || !activePreviewUrl || typeof URL === 'undefined') return undefined
    const controller = new AbortController()
    let disposed = false
    let objectUrl = ''
    createNativeVideoPoster({
      source: activePreviewUrl,
      cacheKey: activePreviewUrl,
      mirrorX: previewMirror,
      signal: controller.signal,
      sourceName: 'composer-video-overlay',
    }).then((record) => {
      if (disposed || !record?.blob?.size) return
      objectUrl = URL.createObjectURL(record.blob)
      if (!disposed) setPreviewPoster(objectUrl)
    }).catch(() => {})
    return () => {
      disposed = true
  ...
  }, [open, st, mediaKind, activePreviewUrl, previewMirror])
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        streamRef.current = effectiveStream
        try {
          await enumerateVideoInputs()
          const s = effectiveStream.getVideoTracks?.()[0]?.getSettings?.()
  ...
  const flipCamera = async () => {
    ...
    try {
      ...
      try {
        const s = finalVideoTrack.getSettings?.() || newVideoTrack.getSettings?.()
  ...
  return createPortal(
    <div
      ...
      data-ql7-visual-scope="media-ui"
      role="dialog"
      aria-modal="true"
      tabIndex={-1}
      onKeyDown={(e) => {
        ...
        <div style={{ width: '100%', height: '100%', aspectRatio: aspect, overflow: 'hidden' }}>
          {(st === 'live' || st === 'recording' || st === 'processing') ? (
            <div className={fixMirrorClass}>
              <LivePreview
                streamRef={streamRef}
                mirror={(facing === 'user' || facing === 'front') && !streamRef?.current?.__ql7CameraFixed}
              />
            </div>
          ) : (
            <div className={fixMirrorClass}>
