...
export default function VideoOverlay({
  ...
  React.useEffect(() => {
    setPreviewPoster('')
    if (!open || st !== 'preview' || mediaKind !== 'video' || !(previewPosterBlob instanceof Blob) || typeof URL === 'undefined') return undefined
    let objectUrl = ''
    try {
      objectUrl = URL.createObjectURL(previewPosterBlob)
      setPreviewPoster(objectUrl)
    } catch {}
    return () => {
      if (objectUrl) { try { URL.revokeObjectURL(objectUrl) } catch {} }
    }
  }, [open, st, mediaKind, previewPosterBlob])
