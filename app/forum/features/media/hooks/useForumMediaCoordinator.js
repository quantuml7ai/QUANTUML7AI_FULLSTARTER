  ...
  if (!kickMediaLoad(mediaEl, {
    ...
    const runQl7MediaLifecycleSweep = (reason = 'periodic') => {
      try {
        ...
        const snap = readQl7VideoFeedPressure(reason);
        const rows = Array.isArray(snap.rows) ? snap.rows : [];
        const budget = isChromeDesktopRuntime ? 2 : (isCoarseUi || isIOSUi ? 3 : 3);
        const now = Date.now();
        const withSrc = rows.filter((row) => row.hasSrc);
        ...
        const detachRow = (row, detachReason) => {
          try {
            const video = Array.from(document.querySelectorAll('video[data-forum-media="video"]'))[row.index];
            if (!(video instanceof HTMLVideoElement)) return false;
            if (protectRow(row)) return false;
        ...
        srcGroups.forEach((group) => {
          ...
          group
            ...
            .slice(1)
            .forEach((row) => detachRow(row, 'duplicate_src_sweep'));
        });
        const refreshed = readQl7VideoFeedPressure(`${reason}_after_duplicate`);
        const attached = (refreshed.rows || [])
          .filter((row) => row.hasSrc)
          .sort((a, b) => {
            const pa = protectRow(a) ? 1 : 0;
            const pb = protectRow(b) ? 1 : 0;
            if (pa !== pb) return pb - pa;
        ...
        try { enforcePostNativeSrcCap(nativePrewarmEl, reason); } catch {}
        return readQl7VideoFeedPressure(`${reason}_done`);
      } catch {
        return null;
      }
    };
