...
import { isTelegramMiniAppRuntime } from '../utils/telegramMiniAppSwipeLock'
export default function useForumMediaCoordinator({ emitDiag }) {
  ...
  useEffect(() => {
    ...
    const isTelegramMiniAppMediaRuntime = (() => {
      try {
...
const enforcePostNativeSrcCap = (keepEl = null, reason = 'post_native_src_cap') => {
  try {
    ...
    const nearProtectPx = (() => {
      try {
        if (isIOSUi) return Math.max(640, Math.min(980, Math.round(viewportH * 0.78)));
        if (isCoarseUi) return Math.max(520, Math.min(820, Math.round(viewportH * 0.66)));
        return Math.max(320, Math.min(560, Math.round(viewportH * 0.42)));
      } catch {
        return isIOSUi ? 640 : (isCoarseUi ? 520 : 320);
      }
    })();
      ...
      if (item.hardProtected || item.nearProtected) {
        if (item.nearProtected) {
          softDemoteNativePostVideo(item.node, 'post_native_src_cap_soft_demote', {
            ...
            keepResident: true,
            keepPrewarm: item.ready && item.nearProtected,
  ...
  setMutedPref(e.detail.muted, source, false);
    ...
    const onMediaPauseCaptured = (e) => {
      ...
      if (isAudibleAutoplayPolicyPause) {
        trace('audible_autoplay_pause_to_muted_resume', target, {
...
const scheduleYouTubeApiInitRetry = (iframe, reason = 'yt_api_wait') => {
  try {
    ...
    const shouldGuardAudibleAutoplayFallback = () => {
      try {
        // Apple-specific policy is limited to ordinary iPhone/iPad browsers.
        // Telegram Mini App keeps its existing autoplay path untouched.
        return !!(isIOSUi && !isTelegramMiniAppMediaRuntime);
      } catch {
        return false;
      }
    };
  ...
  if (!kickMediaLoad(mediaEl, {
      ...
      try {
        ...
        const snapshot = {
          ...
          scheduler: {
            ...
            nativePrewarmSrc: String(nativePrewarmEl?.currentSrc || nativePrewarmEl?.getAttribute?.('src') || nativePrewarmEl?.dataset?.__src || ''),
    ...
    const runQl7MediaLifecycleSweep = (reason = 'periodic') => {
      try {
        ...
        const budget = isChromeDesktopRuntime ? 2 : (isCoarseUi || isIOSUi ? 3 : 3);
        const now = Date.now();
        ...
        rows.filter((row) => row.hasSrc).forEach((row) => {
          if (!row.srcKey) return;
        ...
        const attached = rows.filter((row) => row.hasSrc && !detachedRows.has(row)).sort((a, b) => {
          const pa = protectRow(a) ? 1 : 0;
          const pb = protectRow(b) ? 1 : 0;
          if (pa !== pb) return pb - pa;
