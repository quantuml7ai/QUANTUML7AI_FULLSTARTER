...
import { selectNativeSrcBudgetSurvivors } from '../utils/mediaStatePolicy'
import {
...
export default function useForumMediaCoordinator({ emitDiag }) {
  ...
  // QL7_FORUM_NATIVE_VIDEO_POSTER_FIRST_LIGHT_PREWARM_R24_FIX2_FINAL
  // Native forum video presentation is poster-first: only near-viewport metadata may prewarm;
  // real play/decode starts through the unchanged active autoplay/manual-play pipeline.
  useEffect(() => {
    if (!isBrowser()) return;

    const selector = '[data-forum-media]';
    const isManualOnlyExternalFrame = (el) => {
      ...
      return String(el.getAttribute?.('data-forum-external-manual-only') || '') === '1';
    };
    const hasManualExternalIntent = (el) => {
      try {
        if (!(el instanceof Element)) return false;
        const now = Date.now();
        return (
          Number(el.dataset?.__manualLeaseUntil || 0) > now ||
          Number(el.dataset?.__userGestureIntentUntil || 0) > now ||
          Number(el.dataset?.__audibleGestureUntil || 0) > now
        );
      } catch {}
      return false;
    };
    ...
    const isChromeDesktopRuntime = (() => {
      try {
        const ua = String(navigator?.userAgent || '');
        const isChrome = /Chrome\//i.test(ua) || /Chromium\//i.test(ua);
        const isEdge = /Edg\//i.test(ua);
        const isOpera = /OPR\//i.test(ua) || /Opera/i.test(ua);
        const isMobile = /Android|iP(hone|ad|od)/i.test(ua);
        const coarse = !!window?.matchMedia?.('(pointer: coarse)')?.matches;
        return !!(isChrome && !isEdge && !isOpera && !isMobile && !coarse);
      } catch {
        return false;
      }
    })();
      ...
      window.dumpForumMediaInternals = () => {
        try {
          ...
          return {
            ...
            nativePosterFirstLightPrewarm: true,
    ...
    const isUserSoundSource = (source) => {
      ...
      return (
        ...
        s === 'media_element' ||
        s === 'forum-ads-toggle' ||
        s === 'forum-ad-surface-activate' ||
        s === 'site-ads-surface-activate' ||
        s === 'user' ||
        s.startsWith('user-') ||
        s.endsWith('-toggle')
      );
    ...
    // iOS/WebKit восстанавливает persisted sound choice; другие runtime сохраняют fresh-page muted safety.
    // После пользовательского выбора все контуры читают document/window, storage — shared persistent mirror.
    let mutedPref = null;
    try {
  ...
  setMutedPref(!!next, 'qcast');
    ...
    const applyMutedPref = (el) => {
      ...
      const fallbackLocked =
        shouldGuardAudibleAutoplayFallback() &&
        isAudibleAutoplayFallbackActive(el) &&
        !hasConfirmedSoundGestureNow(el);
      const want = desiredMuted() || fallbackLocked;
  ...
  try {
    ...
    document.querySelectorAll('[data-forum-media]').forEach((el) => {
      ...
      const fallbackLocked =
        ...
        isAudibleAutoplayFallbackActive(el) &&
        !hasConfirmedSoundGestureNow(el);
      const effectiveWant = want || fallbackLocked;

      if (el instanceof HTMLVideoElement || el instanceof HTMLAudioElement) {
        if (shouldSkipMutePersist(el)) return;
        if (el.muted !== effectiveWant) el.muted = effectiveWant;
        el.defaultMuted = effectiveWant;
        if (effectiveWant) el.setAttribute('muted', '');
        else el.removeAttribute('muted');
        return;
      }
      ...
      if (kind === 'qcast') {
        ...
        if (a instanceof HTMLAudioElement) {
          ...
          const qcastFallbackLocked =
            shouldGuardAudibleAutoplayFallback() &&
            (isAudibleAutoplayFallbackActive(a) || isAudibleAutoplayFallbackActive(el)) &&
            !hasConfirmedSoundGestureNow(a) &&
            !hasConfirmedSoundGestureNow(el);
          const qcastWant = want || qcastFallbackLocked;
          if (a.muted !== qcastWant) a.muted = qcastWant;
          a.defaultMuted = qcastWant;
          if (qcastWant) a.setAttribute('muted', '');
      ...
      if (kind === 'youtube' || kind === 'tiktok' || kind === 'iframe') {
        if (isManualOnlyExternalFrame(el) && !String(el.getAttribute?.('src') || '').trim()) return;
        try { commandExternalVideo(el, effectiveWant ? 'mute' : 'unmute'); } catch {}
      }
    });
    ...
    if (window.__forumYtPlayers && window.__forumYtPlayers instanceof Map) {
      window.__forumYtPlayers.forEach((player, iframe) => {
        try {
          const fallbackLocked =
            shouldGuardAudibleAutoplayFallback() &&
            isAudibleAutoplayFallbackActive(iframe) &&
            !hasConfirmedSoundGestureNow(iframe);
          if (want || fallbackLocked) player?.mute?.();
          else player?.unMute?.();
        } catch {}
      });
    }
    ...
    const MAX_CONCURRENT_LOAD_PENDING = (() => {
      try {
        ...
        if (lowMem) return 1;
        if (coarse) return 1;
        if (isChromeDesktopRuntime) return 1;
        return 2;
      } catch {
        return 2;
      }
...
// QL7_FORUM_NATIVE_SRC_BUDGET_R25_STRICT_FINAL
const POST_NATIVE_SRC_CAP = 2; // one active native video + one lightweight metadata-prepared neighbor
const getNativeSrcCapCooldownMs = () => {
  if (isIOSUi) return 2200;
  if (isCoarseUi) return 1800;
  if (isChromeDesktopRuntime) return 900;
  return 1200;
};
const readNativeSrcCapBlockedUntil = (el) => {
  try {
    const media = getMediaStateNode(el);
    return Number(media?.dataset?.__nativeSrcCapBlockedUntil || 0);
  } catch {
    return 0;
  }
};
const isNativeSrcCapCooldownActive = (el) => readNativeSrcCapBlockedUntil(el) > Date.now();
const markNativeSrcCapCooldown = (el, reason = 'resident_cap_overflow') => {
  const media = getMediaStateNode(el);
  if (!(media instanceof HTMLVideoElement)) return 0;
  const until = Date.now() + getNativeSrcCapCooldownMs();
  try {
    media.dataset.__nativeSrcCapBlockedUntil = String(until);
    media.dataset.__nativeSrcCapBlockedReason = String(reason || 'resident_cap_overflow');
  } catch {}
  return until;
};
const readPendingLoads = (force = false) => {
      try {
        ...
        const nodes = Array.from(mediaRegistry)
          .map((owner) => getMediaStateNode(owner) || (owner instanceof HTMLMediaElement ? owner : null))
          .filter((node, index, list) => node instanceof HTMLMediaElement && list.indexOf(node) === index);
        mediaRegistryHotPathReads += 1;
        ...
        const candidates = Array.from(document.querySelectorAll(managedForumVideoSelector))
          .filter((node) => {
            try {
              ...
              if (!hasAttachedMediaSrc(node)) return false;
    ...
    // QL7_FORUM_MEDIA_RETENTION_R26_FINAL
    // `currentSrc` is browser pipeline state, not proof that a managed feed video
    // still owns a DOM src attribute. Chromium may keep it populated after src
    // removal, so source residency is measured from the physical attribute.
    const readAttachedMediaSrc = (el) => {
      const media = getMediaStateNode(el);
      if (!(media instanceof HTMLMediaElement)) return '';
      try {
        const attrSrc = String(media.getAttribute?.('src') || '').trim();
        if (media instanceof HTMLVideoElement && isManagedForumVideoKind(media)) return attrSrc;
        return attrSrc || String(media.currentSrc || '').trim();
      } catch {
        return '';
      }
    };
    const hasAttachedMediaSrc = (el) => !!readAttachedMediaSrc(el);
    ...
    const getSourceRewarmCooldownMs = () => {
      if (isIOSUi) return 6200;
      if (isCoarseUi) return 5200;
      if (isChromeDesktopRuntime) return 4000;
      return 4600;
    };
    const markSourceRewarmCooldown = (el, reason = 'hard_detach') => {
      const media = getMediaStateNode(el);
      if (!(media instanceof HTMLVideoElement) || !isManagedForumVideoKind(media)) return 0;
      const srcKey = getMediaSrcKey(media);
      if (!srcKey) return 0;
      const now = Date.now();
      const prev = srcKickState.get(srcKey) || {};
      const until = Math.max(Number(prev?.rewarmBlockedUntil || 0), now + getSourceRewarmCooldownMs());
      srcKickState.set(srcKey, { ...prev, rewarmBlockedUntil: until, touchedAt: now });
      sourceRewarmBlockCount += 1;
      pruneSrcKickState(now);
      trace('native_src_rewarm_cooldown', media, { reason, blockedForMs: Math.max(0, until - now) });
      return until;
    };
    const canKickLoad = (
      el,
      ...
      if (srcKey && !bypassSrcLimiter) {
        try {
          ...
          const rewarmBlockedUntil = Number(state?.rewarmBlockedUntil || 0);
          if (rewarmBlockedUntil > now) {
            trace('load_kick_skip_src_rewarm_cooldown', media, {
              channel,
              blockedForMs: rewarmBlockedUntil - now,
            });
            return false;
          }
...
const getHtmlMediaNetworkSnapshot = (el) => {
  ...
  try {
    snap.hasSrc = hasAttachedMediaSrc(media);
...
const clampPostNativeWarmBuffer = (el, reason = 'warm_buffer_clamp') => {
  ...
  try {
    ...
    if (media.paused) media.preload = 'metadata';
    return true;
  } catch {
    return false;
  }
...
const enforcePostNativeSrcCap = (keepEl = null, reason = 'post_native_src_cap') => {
  try {
    mediaRegistryHotPathReads += 1;
    const nodes = Array.from(mediaRegistry)
      .map((owner) => getMediaStateNode(owner) || (owner instanceof HTMLVideoElement ? owner : null))
    ...
    const rows = nodes.map((node) => {
      ...
      const activeMatch = !!(activeOwner && owner instanceof Element && (
        activeOwner === owner ||
        activeOwner === node ||
        activeOwner.contains?.(owner) ||
        owner.contains?.(activeOwner)
      ...
      const centerDist = getOwnerCenterDist(owner || node);
      ...
      return {
        ...
        owner,
        isKeep,
        active: activeMatch || String(node.dataset?.__active || '') === '1',
        playing,
        nativePrewarm: node === nativePrewarmEl || String(node.dataset?.__nativePrewarm || '') === '1',
        visiblePx,
        gapPx,
        centerDist,
        ready,
    ...
    const survivors = new Set(
      selectNativeSrcBudgetSurvivors(rows, POST_NATIVE_SRC_CAP).map((row) => row.node),
    );
    const victims = rows.filter((row) => !survivors.has(row.node));
    ...
    victims.forEach((item) => {
      ...
      try {
        markNativeSrcCapCooldown(media, reason);
        media.dataset.__strictNativeSrcCapDetach = '1';
        media.dataset.__nativeSrcCapReason = String(reason || 'post_native_src_cap');
        media.dataset.__prewarm = '0';
        media.dataset.__resident = '0';
        media.dataset.__nativePrewarm = '0';
      } catch {}
      try { clearLoadPending(media, 'resident_cap_overflow', false); } catch {}
      if (nativePrewarmEl === media) {
        try { clearNativePauseRecovery(media); } catch {}
        nativePrewarmEl = null;
        nativePrewarmTs = 0;
      }
      cancelUnload(media);
      hardUnloadMedia(media, 'resident_cap_overflow');
      trace('post_native_src_cap_release', media, {
        reason,
        ...
        visiblePx: Math.round(Number(item.visiblePx || 0)),
        gapPx: Number.isFinite(Number(item.gapPx)) ? Math.round(Number(item.gapPx)) : 999999,
        centerDist: Number.isFinite(Number(item.centerDist)) ? Math.round(Number(item.centerDist)) : 999999,
      });
    });
  } catch {}
};
...
const kickMediaLoad = (
  ...
  const media = getMediaStateNode(el);
  if (!(media instanceof HTMLMediaElement)) return false;
  if (media instanceof HTMLVideoElement && isManagedForumVideoKind(media) && !media.isConnected) {
    try { clearLoadPending(media, 'detached_load_guard', false); } catch {}
    trace('load_kick_skip_detached', media, { channel });
    return false;
  }
  ...
  try {
    if (!hasAttachedMediaSrc(media)) {
      __restoreVideoEl(media);
      if (media instanceof HTMLVideoElement && isManagedForumVideoKind(media)) {
        try { enforcePostNativeSrcCap(media, `${channel}_restore_attach`); } catch {}
      }
    }
  } catch {}
  ...
  try {
    if (media instanceof HTMLVideoElement) {
      ...
      media.preload = mode;
      if (mode === 'metadata') {
        const clearMetadataPending = () => {
          try { clearLoadPending(media, 'metadata_ready', false); } catch {}
          try { media.removeEventListener('loadedmetadata', clearMetadataPending); } catch {}
          trace('native_metadata_prewarm_ready', media, {
            channel,
            readyState: Number(media.readyState || 0),
            networkState: Number(media.networkState || 0),
          });
        };
        if (Number(media.readyState || 0) >= 1) clearMetadataPending();
        else {
          try { media.addEventListener('loadedmetadata', clearMetadataPending, { once: true }); } catch {}
        }
      }
    }
    media.load?.();
    if (media instanceof HTMLVideoElement && isManagedForumVideoKind(media)) {
      try { enforcePostNativeSrcCap(media, `${channel}_load_attach`); } catch {}
    }
    return true;
  } catch {
...
try {
  ...
  if (el instanceof HTMLVideoElement) {
    ...
    const allowNearViewportRestore =
      ...
      (() => {
        try {
          ...
          return getOwnerViewportGapPx(el) <= getNativePrewarmGapLimit();
        } catch {
          return false;
        }
      })();
    const keepWarm = highPriorityReason || allowNearViewportRestore;
    const lightNativePrewarm = isPostVideo && isPrewarmOnly && !highPriorityReason;

    if (lightNativePrewarm) {
      if (isNativeSrcCapCooldownActive(el)) {
        trace('native_prewarm_skip_src_cap_cooldown', el, {
          reason,
          blockedForMs: Math.max(0, readNativeSrcCapBlockedUntil(el) - Date.now()),
        });
        return false;
      }
      return prepareNativePriorityPrewarm(el, `ensure_${reasonTag || 'prewarm'}`);
    }

    try { el.dataset.__resident = keepWarm ? '1' : '0'; } catch {}
    try { el.dataset.__prewarm = keepWarm ? '1' : '0'; } catch {}
    try {
      el.preload = lightNativePrewarm
        ? 'metadata'
        : (keepWarm ? 'auto' : (isPostVideo ? 'none' : 'metadata'));
    } catch {}

    // Poster-first forum rule:
    // beyond the light runway, low-priority native prewarm must not attach src.
    // inside the runway, only metadata is allowed; active play upgrades to auto later.
    ...
    if (!el.getAttribute('src')) {
      ...
      __restoreVideoEl(el);
      if (isPostVideo) {
        try { enforcePostNativeSrcCap(el, `${reasonTag || 'candidate'}_restore_attach`); } catch {}
      }
    }
  } else {
...
if (stalledPending && readyRetryCount < maxReadyRetryCount && (now - lastBoostTs) > 900) {
  ...
  if (isPostVideo) {
    ...
    if (!el.getAttribute('src')) {
      ...
      try { enforcePostNativeSrcCap(el, 'candidate_retry_restore_attach'); } catch {}
      armReadyReplay(el);
      return false;
    }
...
const hasAttachedSrcForRetry = hasAttachedMediaSrc(el);
...
const onMutedEvent = (e) => {
  ...
  const isAuthoritativeMuteSource =
    ...
    source === 'ios-webkit-autoplay-fallback' ||
    source === 'forum-ads-autoplay-fallback' ||
    source === 'site-ads-autoplay-fallback' ||
    source === 'site-ads-surface-activate' ||
    source === 'site-ads-toggle';
  ...
  setMutedPref(e.detail.muted, source, false);
    ...
    const onMediaPauseCaptured = (e) => {
      ...
      const isAudibleAutoplayPolicyPause =
      ...
      if (isAudibleAutoplayPolicyPause) {
        const fallbackSource = isAdMediaElement(target)
          ? 'forum-ads-autoplay-fallback'
          : 'ios-webkit-autoplay-fallback';
        // WebKit already rejected the audible state by pausing playback. Persist
        // the fallback globally before recovery so every next attempt is muted
        // until an explicit user sound action overwrites the shared document.
        setMutedPref(true, fallbackSource);
        trace('audible_autoplay_pause_to_global_mute_resume', target, {
        ...
        try {
          markSkipMutePersist(target, 2600);
          markAudibleAutoplayFallback(target);
          target.muted = true;
        ...
        clearCoordinatorPlayIntent(owner);
        cancelUnload(owner);
        scheduleNativePauseRecovery(target, owner, 'audible_autoplay_policy_muted_resume');
        return;
      }
      ...
      if (isNativeAutoplayPause) {
        const stableIosFallbackPause =
          shouldGuardAudibleAutoplayFallback() &&
          isAudibleAutoplayFallbackActive(target) &&
          target.muted;
        if (
          stableIosFallbackPause &&
          Date.now() - Number(target.dataset?.__iosMutedPauseRecoveryAt || 0) < 8000
        ) {
          trace('pause_recover_muted_fallback_skip_repeat', target, { ownerMatchesActive });
          return;
        }
...
const onMediaLoadedCaptured = (e) => {
  ...
  try {
    ...
    const releaseDetachedManagedVideoPipeline = (media, reason = 'detached_release') => {
      if (!(media instanceof HTMLVideoElement)) return false;
      if (media.isConnected || !isManagedForumVideoKind(media)) return false;
      try { markSourceRewarmCooldown(media, reason); } catch {}
      try { clearLoadPending(media, reason, false); } catch {}
      ...
      try {
        ...
        media.dataset.__strictNativeSrcCapDetach = '1';
        media.dataset.__pendingHardUnload = '1';
      } catch {}
      ...
      try {
        ...
        delete media.dataset.__strictNativeSrcCapDetach;
      } catch {}
      mediaRegistryDetachedReleaseCount += 1;
      trace('detached_native_pipeline_release', media, { reason });
      return true;
    };
    const cleanupObservedMediaNode = (node, reason = 'removed') => {
      ...
      forEachMediaOwner(node, (owner) => {
        ...
        try { mediaRegistry.delete(owner); } catch {}
        ...
        if (kind === 'qcast') {
          ...
          if (audio instanceof HTMLAudioElement) {
            ...
            // QCast audio participates in the shared active-media LRU through
            // startHtmlMedia/playMedia. Once its owner is confirmed detached,
            // release that strong reference before tearing down the media pipeline.
            try { __dropActiveVideoEl(audio); } catch {}
            try {
              audio.dataset.__forceHardUnload = '1';
              audio.dataset.__pendingHardUnload = '1';
            } catch {}
            withSystemPause(audio, () => {
              try { __unloadVideoEl(audio); } catch {}
            });
            try {
              delete audio.dataset.__pendingHardUnload;
              delete audio.dataset.__forceHardUnload;
            } catch {}
          }
        }
        if (owner instanceof HTMLVideoElement || owner instanceof HTMLAudioElement) {
          ...
          // Both HTMLVideoElement and HTMLAudioElement can enter the shared active
          // media LRU, so confirmed removal must release either element kind.
          ...
          if (owner instanceof HTMLVideoElement) {
            ...
            // A disconnected managed video is never a soft-resident surface. Hard
            // reset its native pipeline before the last strong registry references
            // are released, so decoder/range buffers cannot survive DOM removal.
            if (!owner.isConnected && isManagedForumVideoKind(owner)) {
              try { releaseDetachedManagedVideoPipeline(owner, 'detached_registry_release'); } catch {}
            }
          }
        }
      });
    };
    const selfHealDisconnectedMediaRegistry = (reason = 'registry_self_heal') => {
      let healed = 0;
      const releasesBefore = mediaRegistryDetachedReleaseCount;
      try {
        for (const owner of mediaRegistry) {
          if (!(owner instanceof Element)) {
            mediaRegistry.delete(owner);
            healed += 1;
            continue;
          }
          if (owner.isConnected) continue;
          // Fail-closed: release application-owned strong roots before secondary cleanup.
          mediaRegistry.delete(owner);
          pendingRemovedMediaOwners.delete(owner);
          try { observed.delete(owner); } catch {}
          try { cleanupObservedMediaNode(owner, `${reason}_disconnected`); } catch {}
          // If generic cleanup misses the exact owner, still kill a live detached native pipeline.
          if (
            owner instanceof HTMLVideoElement &&
            !owner.isConnected &&
            isManagedForumVideoKind(owner) &&
            String(owner.getAttribute?.('src') || '').trim()
          ) {
            try { releaseDetachedManagedVideoPipeline(owner, `${reason}_fallback_release`); } catch {}
          }
          healed += 1;
        }
      } catch {}
      if (healed > 0) {
        mediaRegistrySelfHealCount += healed;
        trace('media_registry_self_heal', null, {
          reason,
          healed,
          hardReleased: Math.max(0, mediaRegistryDetachedReleaseCount - releasesBefore),
        });
      }
      return healed;
    };
    const sweepDetachedMediaState = (reason = 'detached_sweep', force = false) => {
      ...
      try { selfHealDisconnectedMediaRegistry(reason); } catch {}
      try {
      ...
      try {
        if (nativePrewarmEl instanceof HTMLVideoElement && !nativePrewarmEl.isConnected) {
          const detachedPrewarm = nativePrewarmEl;
          clearNativePauseRecovery(detachedPrewarm);
          nativePrewarmEl = null;
          nativePrewarmTs = 0;
          try { releaseDetachedManagedVideoPipeline(detachedPrewarm, 'detached_native_prewarm_release'); } catch {}
        }
      } catch {}
    ...
    const hasConfirmedSoundGestureNow = (el) => {
      try {
        const owner = getOwnerNode(el);
        const now = Date.now();
        const latestGestureAt = Math.max(
          Number(el?.dataset?.__surfaceGestureAt || 0),
          Number(owner?.dataset?.__surfaceGestureAt || 0),
          Number(el?.dataset?.__lastManualMuteTs || 0),
          Number(owner?.dataset?.__lastManualMuteTs || 0),
        );
        if (latestGestureAt > 0 && now - latestGestureAt < 8000) return true;
        if (String(el?.dataset?.__qcastManualUnmute || '') === '1') return true;
        if (String(owner?.dataset?.__qcastManualUnmute || '') === '1') return true;
        return false;
      } catch {
        return false;
      }
    ...
    const restoreUserSoundAfterSafeAutoplay = (el, reason = 'autoplay_started') => {
      try {
        ...
        if (
          ...
          trace('restore_user_sound_deferred_after_audible_fallback', el, { reason });
          return;
        }
        if (!hasDocumentSoundUnlock()) return;
        markSkipMutePersist(el, 1400);
        el.dataset.__restoreUserSoundAfterAutoplay = String(Date.now());
        el.muted = false;
        el.defaultMuted = false;
        el.removeAttribute('muted');
        clearAudibleAutoplayFallback(el);
    ...
    const hasMediaUserLeaseNow = (el) => {
      try {
        ...
        const leaseUntil = Math.max(
          ...
          Number(el?.dataset?.__audibleGestureUntil || 0),
          Number(owner?.dataset?.__manualLeaseUntil || 0),
          Number(owner?.dataset?.__userGestureUntil || 0),
          Number(owner?.dataset?.__audibleGestureUntil || 0),
        );
    ...
    const startHtmlMedia = (el, reason = 'play') => {
      ...
      const ordinaryIosAutoplay = shouldGuardAudibleAutoplayFallback() && !hasConfirmedSoundGestureNow(el);
      const fallbackLocked = ordinaryIosAutoplay && isAudibleAutoplayFallbackActive(el);
      if (fallbackLocked) enforceStableIosMutedFallback(el);
      if (
        ordinaryIosAutoplay &&
        String(el.dataset?.__playRequested || '') === '1' &&
        requestNow - Number(el.dataset?.__playRequestStartedAt || 0) < 3600
      ) {
        trace('ios_play_skip_inflight', el, { reason, muted: !!el.muted });
        return;
      }
      if (ordinaryIosAutoplay && !el.paused && !el.ended) {
        trace('ios_play_skip_already_playing', el, { reason, muted: !!el.muted });
        return;
      }
      ...
      try {
        ...
        try {
          ...
          el.dataset.__playRequestStartedAt = String(requestNow);
          ...
          if (
            ...
            el.dataset.__iosAudibleAutoplayAttempted = '1';
            el.dataset.__iosAudibleAutoplayAttemptedAt = String(requestNow);
          }
        } catch {}
...
try {
  if (ordinaryIosAutoplay && !el.muted) el.dataset.__iosAudibleAutoplaySucceeded = '1';
} catch {}
...
if (String(el?.dataset?.__warmReady || '') === '1') {
  ...
  try { enforcePostNativeSrcCap(el, 'play_started'); } catch {}
            ...
            const iosAudibleFallback =
              shouldGuardAudibleAutoplayFallback() &&
              desiredMuted() === false &&
              !hasConfirmedSoundGestureNow(el);
            const shouldPromoteBlockedAudibleAutoplay =
              shouldGuardAudibleAutoplayFallback() &&
              desiredMuted() === false &&
              String(err?.name || '') === 'NotAllowedError';

            if (shouldPromoteBlockedAudibleAutoplay) {
              const fallbackSource = isAdMediaElement(el)
                ? 'forum-ads-autoplay-fallback'
                : 'ios-webkit-autoplay-fallback';
              setMutedPref(true, fallbackSource);
              trace('audible_play_reject_global_mute', el, {
                reason,
                source: fallbackSource,
              });
            }
            ...
            try {
              ...
              if (iosAudibleFallback || shouldPromoteBlockedAudibleAutoplay) {
                markAudibleAutoplayFallback(el);
                try { if (qcastHost instanceof Element) markAudibleAutoplayFallback(qcastHost); } catch {}
                trace('audible_play_reject_to_muted_retry', el, { reason });
              } else {
...
if (String(el?.dataset?.__warmReady || '') === '1') {
  ...
  try { enforcePostNativeSrcCap(el, 'play_retry_started'); } catch {}
    ...
    const destroyTrackedYouTubePlayer = (iframe) => {
      try {
    ...
    const isNoCookieYouTubeSrc = (value) => {
      try {
        const url = new URL(String(value || ''), window.location.href);
        return /(^|\.)youtube-nocookie\.com$/i.test(url.hostname || '');
      } catch {
        return /youtube-nocookie\.com/i.test(String(value || ''));
      }
    };
    const getExpectedYouTubeSrc = (iframe) => {
      const raw = String(iframe?.getAttribute?.('data-src') || iframe?.getAttribute?.('src') || '').trim();
      if (!raw) return '';
      return String(iframe?.getAttribute?.('data-forum-yt-host-fallback') || '') === '1'
        ? ensureYouTubeStandardEmbedSrc(raw)
        : normalizeYouTubeEmbedSrc(raw);
    };
    const cooldownDeadYouTubeFrame = (iframe, reason = 'yt_dead_cooldown') => {
      try {
        if (!(iframe instanceof HTMLIFrameElement)) return;
        destroyTrackedYouTubePlayer(iframe);
        const src = iframe.getAttribute('data-src') || iframe.getAttribute('src') || '';
        if (src && !iframe.getAttribute('data-src')) iframe.setAttribute('data-src', src);
        iframe.removeAttribute('data-forum-iframe-active');
        iframe.removeAttribute('data-forum-iframe-loaded');
        iframe.removeAttribute('data-forum-loaded-src');
        iframe.setAttribute('data-forum-yt-failed-until', String(Date.now() + (isIOSUi ? 9000 : (isCoarseUi ? 7600 : 5600))));
        if (iframe.getAttribute('src')) iframe.setAttribute('src', '');
        emitExternalVideoState(iframe, { ready: false, paused: true });
        traceCandidate('youtube_dead_cooldown', iframe, { reason });
      } catch {}
    };
    const fallbackYouTubeToStandardHost = (iframe, reason = 'yt_host_fallback') => {
      try {
        if (!(iframe instanceof HTMLIFrameElement) || !iframe.isConnected) return false;
        const current = String(iframe.getAttribute('src') || iframe.getAttribute('data-src') || '').trim();
        if (!current || !isNoCookieYouTubeSrc(current)) return false;
        const attempts = Number(iframe.getAttribute('data-forum-yt-host-fallback-count') || 0);
        if (attempts >= 1) return false;
        const fallbackSrc = ensureYouTubeStandardEmbedSrc(current);
        if (!fallbackSrc || fallbackSrc === current) return false;
        destroyTrackedYouTubePlayer(iframe);
        iframe.setAttribute('data-forum-yt-host-fallback', '1');
        iframe.setAttribute('data-forum-yt-host-fallback-count', String(attempts + 1));
        iframe.setAttribute('data-src', fallbackSrc);
        iframe.setAttribute('data-forum-iframe-loaded', '0');
        iframe.removeAttribute('data-forum-loaded-src');
        iframe.setAttribute('data-forum-iframe-active', '1');
        iframe.setAttribute('data-forum-last-active-ts', String(Date.now()));
        iframe.setAttribute('src', fallbackSrc);
        emitExternalVideoState(iframe, { ready: false, paused: true });
        traceCandidate('youtube_host_fallback', iframe, { reason });
        setTimeout(() => {
          try {
            if (iframe.isConnected && !isUserPaused(iframe)) playMedia(iframe);
          } catch {}
        }, isIOSUi ? 720 : 520);
        return true;
      } catch {
    ...
    const scheduleYouTubePlaybackHealthCheck = (iframe, reason = 'youtube_play') => {
      try {
        if (!(iframe instanceof HTMLIFrameElement)) return;
        const now = Date.now();
        iframe.setAttribute('data-forum-yt-play-attempt-ts', String(now));
        const activeToken = String(now);
        iframe.setAttribute('data-forum-yt-health-token', activeToken);
        setTimeout(() => {
          try {
            if (!(iframe instanceof HTMLIFrameElement) || !iframe.isConnected) return;
            if (iframe.getAttribute('data-forum-yt-health-token') !== activeToken) return;
            if (isUserPaused(iframe)) return;
            const visiblePx = getOwnerVisiblePx(iframe);
            const nearEnough = isNearViewportElement(iframe, isIOSUi ? 840 : (isCoarseUi ? 720 : 560));
            if (active !== iframe && visiblePx < 96 && !nearEnough) return;
            const lastPlaying = Number(iframe.getAttribute('data-forum-yt-playing-ts') || 0);
            const attemptTs = Number(iframe.getAttribute('data-forum-yt-play-attempt-ts') || 0);
            if (lastPlaying >= attemptTs) return;
            traceCandidate('youtube_playback_stalled', iframe, { reason, visiblePx });
            if (fallbackYouTubeToStandardHost(iframe, 'yt_playback_stalled')) return;
            cooldownDeadYouTubeFrame(iframe, 'yt_playback_stalled');
          } catch {}
        }, isIOSUi ? 3600 : (isCoarseUi ? 3200 : 2600));
      } catch {}
    };
...
const scheduleYouTubeApiInitRetry = (iframe, reason = 'yt_api_wait') => {
  try {
    ...
    const now = Date.now();
    if (
      reason === 'yt_api_load_timeout' &&
      isNoCookieYouTubeSrc(iframe.getAttribute('src') || iframe.getAttribute('data-src') || '') &&
      Number(iframe.getAttribute('data-forum-yt-host-fallback-count') || 0) < 1
    ) {
      if (fallbackYouTubeToStandardHost(iframe, reason)) return;
    }
    ...
    const initYouTubePlayer = async (iframe) => {
      ...
      const expectedSrc = getExpectedYouTubeSrc(iframe);
        ...
        try {
          const player = new YT.Player(iframe, {
            events: {
              ...
              onStateChange: (evt) => {
                try {
                  ...
                  if (state === YT.PlayerState?.PLAYING) {
                    try {
                      iframe.setAttribute('data-forum-yt-playing-ts', String(Date.now()));
                      iframe.dataset.__externalActualPlaying = '1';
                      iframe.dataset.__externalActualMuted = player?.isMuted?.() ? '1' : '0';
                    } catch {}
                  ...
                  if (state === YT.PlayerState?.ENDED) {
                    try { iframe.dataset.__externalActualPlaying = '0'; } catch {}
              ...
              onError: (evt) => {
                try {
                  const code = Number(evt?.data || 0);
                  traceCandidate('youtube_player_error', iframe, { code });
                  emitExternalVideoState(iframe, { paused: true, ready: false, error: code });
                  if (!fallbackYouTubeToStandardHost(iframe, `yt_player_error_${code}`)) {
                    cooldownDeadYouTubeFrame(iframe, `yt_player_error_${code}`);
                  }
                } catch {}
              }
            },
    ...
    const pendingRemovedMediaOwners = new Set();
    let mediaRemovalRaf = 0;
    let mediaLifecycleSweepCount = 0;
    let mediaLifecycleSnapshotCount = 0;
    let mediaLifecycleGeometryReadCount = 0;
    let mediaLifecycleFastPathCount = 0;
    let mediaGlobalDiagnosticScanCount = 0;
    let mediaRegistrySelfHealCount = 0;
    let mediaRegistryDetachedReleaseCount = 0;
    let mediaRegistryHotPathReads = 0;
    let mediaActivationDeferrals = 0;
    let mediaScrollSettleRuns = 0;
    let hotScrollNativeScanSkips = 0;
    let coordinatorLastScrollEventTs = 0;
    let mediaScrollSettleTimer = 0;
    let coordinatorScrollMotionTop = 0;
    let coordinatorScrollMotionTs = 0;
    let coordinatorScrollVelocityPxMs = 0;
    let nativeAutoCandidate = null;
    let nativeAutoCandidateSinceTs = 0;
    let nativeAutoCandidateTimer = 0;
    ...
    const readCoordinatorScrollTop = () => {
      try {
        if (coordinatorScrollEl instanceof Element) return Number(coordinatorScrollEl.scrollTop || 0);
      } catch {}
    ...
    const recordCoordinatorScrollMotion = () => {
      const now = Date.now();
      const top = readCoordinatorScrollTop();
      if (coordinatorScrollMotionTs > 0) {
        const elapsed = Math.max(1, now - coordinatorScrollMotionTs);
        const instantVelocity = Math.abs(top - coordinatorScrollMotionTop) / elapsed;
        coordinatorScrollVelocityPxMs = Math.max(
          instantVelocity,
          Number(coordinatorScrollVelocityPxMs || 0) * 0.72,
        );
      }
      coordinatorScrollMotionTop = top;
      coordinatorScrollMotionTs = now;
    };
    const getCoordinatorScrollVelocity = () => {
      const age = Math.max(0, Date.now() - Number(coordinatorLastScrollEventTs || 0));
      if (age >= 140) return 0;
      return Number(coordinatorScrollVelocityPxMs || 0) * Math.max(0, 1 - (age / 140));
    };
    ...
    const getNearQueuePlacementFromRect = (el, dir = 1, rect = null) => {
      try {
        const viewportH = Number(window?.innerHeight || document?.documentElement?.clientHeight || 0) || 0;
    ...
    const getNearQueuePlacement = (el, dir = 1) => {
      try {
        return getNearQueuePlacementFromRect(el, dir, el?.getBoundingClientRect?.());
      } catch {
        return { band: 9, top: 0, bottom: 0, order: getMediaDomOrder(el) };
      }
    };
    ...
    const shouldGuardAudibleAutoplayFallback = () => {
      try {
        // Apple media policy applies to the whole iOS runtime, including embedded
        // WKWebView/Telegram. A refused audible autoplay becomes one shared MUTED
        // document state instead of another automatic attempt to restore sound.
        return !!isIOSUi;
      } catch {
        return false;
      }
    };
    const markAudibleAutoplayFallback = (el, ms = 0) => {
      try {
        if (!(el instanceof Element)) return;
        el.dataset.__audibleAutoplayBlocked = '1';
        el.dataset.__audibleAutoplayFallbackAt = String(Date.now());
        if (Number(ms || 0) > 0) {
          el.dataset.__audibleAutoplayFallbackUntil = String(Date.now() + Math.max(1800, Number(ms || 0)));
        } else {
    ...
    const isAudibleAutoplayFallbackActive = (el) => {
      try {
        if (!(el instanceof Element)) return false;
        if (String(el?.dataset?.__audibleAutoplayBlocked || '') === '1') return true;
        return Number(el?.dataset?.__audibleAutoplayFallbackUntil || 0) > Date.now();
      } catch {
        return false;
      }
    };
    const clearAudibleAutoplayFallback = (el) => {
      try {
        if (!(el instanceof Element)) return;
    ...
    const enforceStableIosMutedFallback = (el) => {
      try {
        if (!(el instanceof HTMLMediaElement)) return false;
        if (!shouldGuardAudibleAutoplayFallback()) return false;
        if (!isAudibleAutoplayFallbackActive(el) || hasConfirmedSoundGestureNow(el)) return false;
        markSkipMutePersist(el, 3200);
        el.muted = true;
        el.defaultMuted = true;
        el.setAttribute('muted', '');
        el.dataset.__autoplayFallbackMuted = '1';
    ...
    const isExternalPlaybackActuallyRunning = (el) => {
      try {
        if (!(el instanceof HTMLIFrameElement)) return false;
        const kind = String(el.getAttribute('data-forum-media') || '');
        if (kind === 'youtube') {
          const player = ytPlayers?.get?.(el) || window?.__forumYtPlayers?.get?.(el) || null;
          const state = Number(player?.getPlayerState?.());
          if (state === 1) return true;
        }
        return String(el.dataset?.__externalActualPlaying || '') === '1';
      } catch {
        return false;
      }
    };
    const isExternalPlaybackActuallyAudible = (el) => {
      try {
        if (!(el instanceof HTMLIFrameElement)) return false;
        const kind = String(el.getAttribute('data-forum-media') || '');
        if (kind === 'youtube') {
          const player = ytPlayers?.get?.(el) || window?.__forumYtPlayers?.get?.(el) || null;
          return Number(player?.getPlayerState?.()) === 1 && player?.isMuted?.() === false;
        }
        return String(el.dataset?.__externalActualPlaying || '') === '1' &&
          String(el.dataset?.__externalActualMuted || '') === '0';
      } catch {
        return false;
      }
    };
...
const shouldRetainHtmlMedia = (el) => {
  ...
  try {
    ...
    if (String(mediaDataset?.__strictNativeSrcCapDetach || '') === '1') return false;
  ...
  return false;
    ...
    const getUnloadDelay = (el, reason = 'timeout') => {
      if (!isIframeLike(el)) {
        ...
        if (reason === 'resident_cap' || reason === 'resident_cap_overflow' || reason === 'error_blocked') return 0;
    ...
    const getLoadedYouTubeIframes = () => {
      try {
        return getLoadedIframes().filter((frame) => {
          try { return String(frame.getAttribute('data-forum-media') || '') === 'youtube'; } catch {}
          return false;
        });
      } catch {
        return [];
    ...
    const getIframeSnapshot = () => {
      try {
        ...
        const youtubeLoaded = all.filter((frame) => {
          try {
            return String(frame.getAttribute('data-forum-media') || '') === 'youtube' &&
    ...
    const getNearPostSurfaceLimit = () => {
      if (isIOSUi) return 900;
      if (isCoarseUi) return 780;
      if (isChromeDesktopRuntime) return 360;
      return 620;
    };

    const getHardDetachMargin = () => {
      if (isIOSUi) return 1250;
      if (isCoarseUi) return 1080;
      if (isChromeDesktopRuntime) return 620;
      return 900;
    };
      ...
      try {
        if (media instanceof HTMLVideoElement || media instanceof HTMLAudioElement) {
          ...
          state.managedPostVideo =
            ...
            isManagedForumVideoKind(media) &&
            String(media.getAttribute?.('data-forum-media') || '') === 'video';
          state.hasSrc = hasAttachedMediaSrc(media);
    ...
    const canHardDetachHtmlMedia = (el, reason = 'timeout') => {
      ...
      if (reasonKey === 'resident_cap_overflow') {
        return { allowed: true, state, blockReason: 'strict_native_src_budget' };
      }
    ...
    const softDemoteNativePostVideo = (el, reason = 'soft_demote', options = {}) => {
      ...
      try {
        ...
        if (!state.playing) media.preload = 'metadata';
      } catch {}
    ...
    const blockHardDetachWithSoftDemote = (el, reason = 'timeout', traceEvent = 'hard_unload_blocked_near_post_video') => {
      ...
      softDemoteNativePostVideo(check.state.media || el, reason, {
        ...
        keepResident: true,
        keepPrewarm: check.state.nativePrewarm,
...
const pauseForeignMedia = (keepEl = null) => {
  try {
    ...
    const hardUnloadMedia = (...args) => {
      ...
      if (el instanceof HTMLVideoElement || el instanceof HTMLAudioElement) {
        ...
        try {
          ...
          if (detachHard && el instanceof HTMLVideoElement && isManagedForumVideoKind(el)) {
            try { markSourceRewarmCooldown(el, unloadReason); } catch {}
          }
      ...
      if (
        ...
        const forceYoutubeCapDetach =
          isYoutubeFrame &&
          String(el.getAttribute?.('data-forum-youtube-cap-detach') || '') === '1';
        const iframeActive =
          el === active ||
        ...
        if (!forceYoutubeCapDetach && (iframeVisiblePx > visibleProtectPx || (iframeActive && iframeVisiblePx > 0) || iframeNear)) {
          try { el.removeAttribute('data-forum-iframe-active'); } catch {}
      ...
      if (kind === 'youtube') {
        ...
        if (softIframeCooldown) {
          try { el.removeAttribute('data-forum-iframe-active'); } catch {}
          try { el.removeAttribute('data-forum-youtube-cap-detach'); } catch {}
    ...
    const enforceYouTubeIframeResidentCap = (keepEl = null, reason = 'youtube_resident_cap') => {
      const cap = Math.max(1, Math.min(IFRAME_RESIDENT_CAP || 2, 2));
      const loaded = getLoadedYouTubeIframes();
      const extra = loaded.length - cap;
      if (extra <= 0) return;
      ...
      const ranked = loaded
        .map((frame, index) => {
          ...
          let manualLease = false;
          let lastActiveTs = 0;
          try {
            visiblePx = getOwnerVisiblePx(frame);
            centerDist = getOwnerCenterDist(frame);
            activeFrame = frame === active;
            manualLease = hasMediaUserLeaseNow(frame);
            lastActiveTs = Number(frame.getAttribute('data-forum-last-active-ts') || 0);
          } catch {}
          const keepScore = frame === keepEl ? 10000000 : 0;
          const activeScore = activeFrame ? 5000000 : 0;
          const leaseScore = manualLease ? 3500000 : 0;
          const visibleScore = Number(visiblePx || 0) * 1200;
          const centerPenalty = Number.isFinite(centerDist) ? centerDist : 100000;
          const recencyScore = Math.min(200000, Math.max(0, lastActiveTs % 1000000) / 5);
          return {
            frame,
            index,
            visiblePx,
            centerDist,
            activeFrame,
            score: keepScore + activeScore + leaseScore + visibleScore + recencyScore - centerPenalty,
          };
        })
        .sort((a, b) => {
      ...
      const survivors = new Set(ranked.slice(0, cap).map((item) => item.frame));
      const victims = ranked
        .filter((item) => !survivors.has(item.frame))
        .slice(0, extra);
      if (!victims.length) return;
      ...
      victims.forEach((item) => {
        ...
        cancelUnload(frame);
        trace('youtube_iframe_cap_release', frame, {
          reason,
          visiblePx: item.visiblePx,
          centerDist: item.centerDist,
          cap,
        });
        try { frame.setAttribute('data-forum-youtube-cap-detach', '1'); } catch {}
        hardUnloadMedia(frame, 'resident_cap');
        try { frame.removeAttribute('data-forum-youtube-cap-detach'); } catch {}
      });
      emitMediaDiag('youtube_iframe_resident_cap', {
        cap,
    ...
    const enforceIframeResidentCap = (keepEl = null) => {
      ...
      enforceYouTubeIframeResidentCap(keepEl, 'iframe_resident_cap');
    ...
    const getNativePrewarmGapLimit = () => {
      const viewportH = Number(window?.innerHeight || document?.documentElement?.clientHeight || 0) || 0;
      // Poster owns the far/offscreen presentation. Only a small near-viewport runway
      // may attach src and fetch metadata; real frame decode begins on active play.
      if (isIOSUi) return Math.max(420, Math.min(720, Math.round(viewportH * 0.62)));
      if (isCoarseUi) return Math.max(360, Math.min(640, Math.round(viewportH * 0.54)));
      return Math.max(220, Math.min(420, Math.round(viewportH * 0.34)));
    };
    ...
    const getNativePrewarmPipelineState = (el) => {
      ...
      try {
        state.hasSrc = hasAttachedMediaSrc(media);
        state.metadataReady = Number(media.readyState || 0) >= 1;
        state.ready = state.metadataReady || String(media.dataset?.__warmReady || '') === '1';
        state.loadPending = String(media.dataset?.__loadPending || '') === '1';
        const since = Number(media.dataset?.__loadPendingSince || 0);
    ...
    const isNativePrewarmEligible = (el) => {
      try {
        ...
        if (isNativeSrcCapCooldownActive(media)) return false;
    ...
    const scheduleNativePauseRecovery = (media, owner = null, reason = 'native_pause_recover') => {
      try {
        ...
        const now = Date.now();
        const stableIosFallback =
          shouldGuardAudibleAutoplayFallback() &&
          isAudibleAutoplayFallbackActive(media) &&
          media.muted;
        if (
          stableIosFallback &&
          now - Number(media.dataset?.__iosMutedPauseRecoveryAt || 0) < 8000
        ) {
          trace('pause_recover_active_native_skip_stable_fallback', media, { reason });
          return false;
        }
        if (stableIosFallback) media.dataset.__iosMutedPauseRecoveryAt = String(now);
    ...
    const getNativeWarmOwnerLostDelay = (media) => {
      try {
        const visiblePx = getOwnerVisiblePx(media);
        const gapPx = getOwnerViewportGapPx(media);
        const nearRunway = visiblePx > 0 || gapPx <= Math.max(
          getNativePrewarmGapLimit(),
          isIOSUi ? 760 : (isCoarseUi ? 680 : 460),
        );
        return nearRunway
          ? (isIOSUi ? 560 : (isCoarseUi ? 480 : 340))
          : (isIOSUi ? 260 : (isCoarseUi ? 220 : 180));
      } catch {
        return isIOSUi ? 560 : (isCoarseUi ? 480 : 340);
      }
    };
    const releaseNativePrewarmExcept = (keepEl = null, reason = 'native_prewarm_replace') => {
      try {
        ...
        if (!prev.isConnected) {
          nativePrewarmEl = null;
          nativePrewarmTs = 0;
          try { clearNativePauseRecovery(prev); } catch {}
          try { releaseDetachedManagedVideoPipeline(prev, 'detached_native_prewarm_release'); } catch {}
          return;
        }
        ...
        // Keep one strict metadata-warm owner, but do not tear down the previous source
        // inside the one-card mobile runway: this preserves cheap metadata readiness
        // during small scroll corrections while the poster owns visual presentation.
    ...
    const prepareNativePriorityPrewarm = (el, reason = 'native_priority_prewarm') => {
      ...
      if (!media.isConnected) {
        ...
        try { clearNativePauseRecovery(media); } catch {}
        trace('native_prewarm_skip_detached', media, { reason });
        return false;
      }
      if (isNativeSrcCapCooldownActive(media)) {
        trace('native_prewarm_skip_src_cap_cooldown', media, {
          reason,
          blockedForMs: Math.max(0, readNativeSrcCapBlockedUntil(media) - Date.now()),
        });
        return false;
      }
      ...
      if (prev instanceof HTMLVideoElement && prev !== media && prev.isConnected && !isActiveNativeOwner(prev)) {
        ...
        const nextInFocusRunway =
          ...
          nextGap <= Math.max(getNativePrewarmGapLimit(), isIOSUi ? 420 : 340);
      ...
      if (Number(media.readyState || 0) >= 1 || String(media.dataset?.__warmReady || '') === '1') {
        trace('native_prewarm_metadata_ready', media, { reason });
        return true;
      }

      const alreadyLoadingOrBuffered = (() => {
        try {
          const hasSrc = hasAttachedMediaSrc(media);
          const networkState = Number(media.networkState || 0);
      ...
      if (!kicked && String(media.dataset?.__loadPending || '') !== '1') {
        ...
        trace('native_prewarm_kick_skip', media, { reason, claimRolledBack: '1' });
        return false;
      }
      ...
      trace('native_prewarm_metadata_kick', media, {
        reason,
        readyState: Number(media.readyState || 0),
        networkState: Number(media.networkState || 0),
        gapPx: getOwnerViewportGapPx(media),
    ...
    const externalWarmOrigins = new Set();
    const warmExternalProviderOrigin = (rawSrc) => {
      try {
        const u = new URL(String(rawSrc || ''), window.location.href);
        if (!/^https?:$/.test(u.protocol)) return false;
        const origin = u.origin;
        if (!origin) return false;
        if (externalWarmOrigins.has(origin)) return true;
        if (externalWarmOrigins.size >= 8) return false;
        ...
        if (!existing) {
          ...
          preconnect.setAttribute('data-ql7-forum-provider-preconnect', '1');
          head.appendChild(preconnect);
        }

        const dnsExists = Array.from(document.querySelectorAll('link[rel="dns-prefetch"]'))
          .some((link) => String(link.getAttribute('href') || '') === `//${u.host}`);
        if (!dnsExists) {
          const dns = document.createElement('link');
          dns.rel = 'dns-prefetch';
          dns.href = `//${u.host}`;
          dns.setAttribute('data-ql7-forum-provider-preconnect', '1');
          head.appendChild(dns);
        }

        externalWarmOrigins.add(origin);
        return true;
      } catch {
        return false;
      }
    ...
    const prepareExternalMedia = (el, reason = 'prewarm') => {
      ...
      const kind = String(el.getAttribute('data-forum-media') || '');
      if (kind !== 'youtube' && kind !== 'tiktok' && kind !== 'iframe') return false;
      if (isManualOnlyExternalFrame(el) && !hasManualExternalIntent(el)) {
        trace('iframe_manual_only_skip_prepare', el, { reason });
        return false;
      }
      const now = Date.now();
      ...
      if (lastPrewarmTs > 0 && (now - lastPrewarmTs) < 700) {
        ...
        if (passivePrepare) return el.getAttribute('data-forum-origin-prewarmed') === '1';
      }
      ...
      const nextSrc =
        kind === 'youtube'
          ? (
              String(el.getAttribute('data-forum-yt-host-fallback') || '') === '1'
                ? ensureYouTubeStandardEmbedSrc(raw)
      ...
      if (!hadSrc && passivePrepare) {
        if (!canSpendExternalPrewarmBudget(el, reason)) return false;
        const originWarmed = warmExternalProviderOrigin(nextSrc);
        try { el.setAttribute('data-forum-origin-prewarmed', originWarmed ? '1' : '0'); } catch {}
        try { el.setAttribute('data-forum-prewarm-ts', String(now)); } catch {}
        trace('iframe_origin_prewarm', el, {
          kind,
          reason,
          originWarmed,
          gate: 'passive_origin_only',
        });
        emitMediaDiag('iframe_origin_prewarm', { kind, reason, originWarmed, ...getIframeSnapshot() });
        return originWarmed;
      }
...
const waitExternalIframeLoad = (iframe, expectedSrc = '', timeoutMs = 1400) => {
  ...
  return new Promise((resolve) => {
    ...
    const getStableAutoplayMuted = (el) => {
      try {
        if (desiredMuted()) return true;
        if (
          shouldGuardAudibleAutoplayFallback() &&
          !hasConfirmedSoundGestureNow(el) &&
          (
            isAudibleAutoplayFallbackActive(el) ||
            String(el?.dataset?.__audibleAutoplayFallbackPending || '') === '1'
          )
        ) return true;
      } catch {}
      return false;
    };
    const scheduleAppleExternalMutedFallback = (el, reason = 'external_autoplay') => {
      try {
        if (!(el instanceof HTMLIFrameElement)) return;
        if (!shouldGuardAudibleAutoplayFallback()) return;
        if (desiredMuted() || hasConfirmedSoundGestureNow(el)) return;
        if (isAudibleAutoplayFallbackActive(el)) return;
        if (String(el.dataset?.__audibleAutoplayFallbackPending || '') === '1') return;
        if (String(el.dataset?.__iosAudibleAutoplayAttempted || '') === '1') return;

        el.dataset.__iosAudibleAutoplayAttempted = '1';
        el.dataset.__audibleAutoplayFallbackPending = '1';
        const token = String(Date.now());
        el.dataset.__audibleAutoplayAttemptToken = token;
        setTimeout(() => {
          try {
            if (!(el instanceof HTMLIFrameElement) || !el.isConnected) return;
            if (el.dataset?.__audibleAutoplayAttemptToken !== token) return;
            delete el.dataset.__audibleAutoplayFallbackPending;
            if (desiredMuted() || hasConfirmedSoundGestureNow(el)) return;
            const ownerMatchesActive = active === el || active?.contains?.(el) || el.contains?.(active);
            if (!ownerMatchesActive || getOwnerVisiblePx(el) <= 0) return;
            if (isExternalPlaybackActuallyAudible(el)) {
              el.dataset.__iosAudibleAutoplaySucceeded = '1';
              trace('external_audible_autoplay_confirmed', el, { reason });
              return;
            }
            const fallbackSource = isAdMediaElement(el)
              ? 'forum-ads-autoplay-fallback'
            ...
            setMutedPref(true, fallbackSource);
            markAudibleAutoplayFallback(el);
            commandExternalVideo(el, 'play', { muted: true });
            trace('external_audible_to_muted_stable', el, {
              reason,
              source: fallbackSource,
            });
          } catch {}
...
if (el instanceof HTMLVideoElement) {
  ...
  if (!hasSrc) {
    ...
    __restoreVideoEl(el);
    restoredNow = true;
    try { enforcePostNativeSrcCap(el, 'play_restore_attach'); } catch {}
  }
...
return;
      ...
      const kind = el.getAttribute('data-forum-media');
      if (isManualOnlyExternalFrame(el) && !hasManualExternalIntent(el)) {
        trace('play_skip_external_manual_only', el);
        return;
      }
      if (kind === 'qcast') {
        const a = el.querySelector?.('audio');
        if (a instanceof HTMLAudioElement) {
          try {
            ...
            const qcastFallbackLocked =
              ...
              !hasConfirmedSoundGestureNow(a) &&
              !hasConfirmedSoundGestureNow(el);
            const nextQcastMuted = qcastFallbackLocked
              ? true
              : (typeof persistedQcastMuted === 'boolean' ? persistedQcastMuted : desiredMuted());
      ...
      if (kind === 'youtube') {
        try {
          const failedUntil = Number(el.getAttribute('data-forum-yt-failed-until') || 0);
          if (failedUntil > Date.now()) {
            trace('play_skip_youtube_cooldown', el, { blockedForMs: failedUntil - Date.now() });
            return;
          }
          try { el.removeAttribute('data-forum-yt-failed-until'); } catch {}
          const raw = String(el.getAttribute('data-src') || el.getAttribute('src') || '');
          const ds = getExpectedYouTubeSrc(el) || ensureYouTubeEmbedSrc(raw);
          ...
          if (ds && !readyForYouTubeApi && !isYouTubeIframeReadyForApi(el, ds)) {
            trace('youtube_iframe_load_timeout_soft_continue', el, {
              loaded: readyForYouTubeApi ? '1' : '0',
            });
          }
          ...
          const kickYoutube = () => {
            try {
              if (
                shouldGuardAudibleAutoplayFallback() &&
                isAudibleAutoplayFallbackActive(el) &&
                isExternalPlaybackActuallyRunning(el)
              ) return;
              commandExternalVideo(el, 'play', { muted: getStableAutoplayMuted(el) });
            } catch {}
          };
          const useYoutubeApi =
            USE_YOUTUBE_IFRAME_API &&
            isYouTubeIframeReadyForApi(el, getExpectedYouTubeSrc(el));
          const player = useYoutubeApi ? await initYouTubePlayer(el) : null;
          if (useYoutubeApi && !player) return;
          if (player) {
            try {
              if (getStableAutoplayMuted(el)) player?.mute?.();
              else player?.unMute?.();
              player?.playVideo?.();
            } catch {}
            scheduleYouTubePlaybackHealthCheck(el, 'youtube_viewport_autoplay');
          }
          kickYoutube();
          scheduleAppleExternalMutedFallback(el, 'youtube_viewport_autoplay');
          if (!shouldGuardAudibleAutoplayFallback()) {
      ...
      if (kind === 'tiktok' || kind === 'iframe') {
        ...
        kickExternalFrame();
        scheduleAppleExternalMutedFallback(el, `${kind}_viewport_autoplay`);
        if (!shouldGuardAudibleAutoplayFallback()) {
    ...
    const getCandidateMetricsFromRect = (el, ratioOverride = null, rect = null) => {
      if (!(el instanceof Element)) return null;
      const viewportH = Number(window?.innerHeight || document?.documentElement?.clientHeight || 0) || 0;
      const viewportCenter = viewportH / 2;
      try {
        ...
        const center = (Number(rect.top || 0) + Number(rect.bottom || 0)) / 2;
        const centerDist = Math.abs(center - viewportCenter);
        const gapPx = rect.bottom < 0
          ? Math.abs(Number(rect.bottom || 0))
          : (rect.top > viewportH ? Number(rect.top || 0) - viewportH : 0);
        const score = (visiblePx * 1.15) + (ratio * 420) - (centerDist * 0.45);
        return { el, ratio, score, visiblePx, centerDist, gapPx };
      } catch {}
      return null;
    };

    const pickMostVisible = (metricsByElement = null) => {
      ...
      let bestGapPx = Number.POSITIVE_INFINITY;
      for (const [el, r] of ratios.entries()) {
        if (isUserPaused(el)) continue;
        if (isManualOnlyExternalFrame(el)) continue;
        if (isMediaSrcBlocked(el)) continue;
        const metrics = metricsByElement?.get?.(el) || getCandidateMetrics(el, r);
        if (!metrics) continue;
        if (metrics.score > bestScore) {
          ...
          bestGapPx = Number(metrics.gapPx ?? 0);
          best = el;
        }
      }
      if (!best) {
      ...
      return { el: best, ratio: bestRatio, score: bestScore, visiblePx: bestVisiblePx, centerDist: bestCenterDist, gapPx: bestGapPx };
    };
    ...
    const isStartableCandidate = (el, ratio, visiblePx, centerDist) => {
      if (!(el instanceof Element)) return false;
      if (isManualOnlyExternalFrame(el)) return false;
    ...
    const isReadyCandidate = (el) => {
      ...
      if (kind === 'youtube' || kind === 'tiktok' || kind === 'iframe') {
        ...
        if (kind === 'youtube') {
          const failedUntil = Number(frame.getAttribute('data-forum-yt-failed-until') || 0);
          if (failedUntil > Date.now()) return false;
          if (failedUntil > 0) {
            try { frame.removeAttribute('data-forum-yt-failed-until'); } catch {}
          }
        }
    ...
    // G7 FIX2: native autoplay follows a short candidate-commit window while the feed
    // is moving. This is not a scroll-stop debounce: a stable winner can start during
    // continued scrolling, while transient fling candidates never attach an MP4 src.
    const getSpeculativePrewarmSettleMs = (el) => {
      const kind = getMediaKind(el);
      if (kind === 'youtube' || kind === 'tiktok' || kind === 'iframe') {
        return isIOSUi ? 360 : (isCoarseUi ? 300 : 140);
      }
      return isIOSUi ? 180 : (isCoarseUi ? 140 : 80);
    };
    const isSpeculativePrewarmScrollHot = (el) => {
      if (hasManualLease(el)) return false;
      const settleMs = getSpeculativePrewarmSettleMs(el);
      const sinceScroll = Date.now() - Number(coordinatorLastScrollEventTs || 0);
      return coordinatorLastScrollEventTs > 0 && sinceScroll < settleMs;
    };
    const schedulePredictivePrewarmAfterScroll = () => {
      try {
        if (mediaScrollSettleTimer) clearTimeout(mediaScrollSettleTimer);
        const settleMs = isIOSUi ? 180 : (isCoarseUi ? 140 : 80);
        const sinceScroll = coordinatorLastScrollEventTs > 0
          ? Math.max(0, Date.now() - coordinatorLastScrollEventTs)
          : settleMs;
        const delay = Math.max(24, settleMs - sinceScroll);
        mediaScrollSettleTimer = setTimeout(() => {
          mediaScrollSettleTimer = 0;
          mediaScrollSettleRuns += 1;
          scheduleNativePrewarmScan('scroll_settled_native_predictive_scan');
        }, delay);
      } catch {
        mediaScrollSettleTimer = 0;
      }
    ...
    const clearNativeAutoCandidateCommit = (candidate = null) => {
      if (candidate && nativeAutoCandidate && nativeAutoCandidate !== candidate) return;
      if (nativeAutoCandidateTimer) {
        try { clearTimeout(nativeAutoCandidateTimer); } catch {}
      }
      nativeAutoCandidateTimer = 0;
      nativeAutoCandidate = null;
      nativeAutoCandidateSinceTs = 0;
    };
    const isNativeAutoCommitScrollHot = (candidate) => {
      if (!isNativePostVideoCandidate(candidate) || hasManualLease(candidate)) return false;
      const sinceScroll = Date.now() - Number(coordinatorLastScrollEventTs || 0);
      return coordinatorLastScrollEventTs > 0 && sinceScroll < 140;
    };
    const getNativeAutoCommitDelayMs = () => {
      const velocity = getCoordinatorScrollVelocity();
      const fastVelocity = isIOSUi ? 0.75 : (isCoarseUi ? 0.9 : 1.2);
      if (velocity >= fastVelocity) return isIOSUi ? 96 : (isCoarseUi ? 80 : 64);
      return isIOSUi ? 48 : (isCoarseUi ? 40 : 32);
    };
    const commitNativeAutoCandidate = () => {
      nativeAutoCandidateTimer = 0;
      const tracked = nativeAutoCandidate;
      if (!(tracked instanceof Element) || !tracked.isConnected) {
        clearNativeAutoCandidateCommit();
        return false;
      }

      const picked = pickMostVisible();
      const candidate = picked?.el;
      if (candidate !== tracked) {
        clearNativeAutoCandidateCommit();
        if (candidate instanceof Element) shouldDeferNativeAutoCandidate(candidate, picked);
        return false;
      }

      const elapsed = Date.now() - Number(nativeAutoCandidateSinceTs || 0);
      const requiredMs = getNativeAutoCommitDelayMs();
      if (elapsed < requiredMs) {
        nativeAutoCandidateTimer = setTimeout(commitNativeAutoCandidate, Math.max(16, requiredMs - elapsed));
        return false;
      }
      if (
        active ||
        isUserPaused(candidate) ||
        hasSuppressedPlayback(candidate) ||
        isMediaSrcBlocked(candidate) ||
        !isStartableCandidate(candidate, picked.ratio, picked.visiblePx, picked.centerDist)
      ) {
        clearNativeAutoCandidateCommit();
        return false;
      }
      ...
      prepareNativePriorityPrewarm(candidate, 'stable_scroll_candidate');
      active = candidate;
      activeSinceTs = Date.now();
      cancelUnload(active);
      traceCandidate('candidate_stable_scroll_activate', candidate, {
        ratio: picked.ratio,
        score: picked.score,
        visiblePx: picked.visiblePx,
        centerDist: picked.centerDist,
        dwellMs: elapsed,
        velocityPxMs: getCoordinatorScrollVelocity(),
      });
      emitMediaDiag('media_focus_switch', {
        ratio: picked.ratio,
        prevRatio: 0,
        score: picked.score,
        reason: 'stable_scroll_candidate',
      });
      clearNativeAutoCandidateCommit(candidate);
      playMedia(active);
      return true;
    };
    function shouldDeferNativeAutoCandidate(candidate, metrics = null) {
      if (!(candidate instanceof Element) || !isNativeAutoCommitScrollHot(candidate)) return false;
      if (active === candidate) return false;
      const now = Date.now();
      if (nativeAutoCandidate !== candidate) {
        clearNativeAutoCandidateCommit();
        nativeAutoCandidate = candidate;
        nativeAutoCandidateSinceTs = now;
      }
      if (!nativeAutoCandidateTimer) {
        const delay = getNativeAutoCommitDelayMs();
        nativeAutoCandidateTimer = setTimeout(commitNativeAutoCandidate, delay);
      }
      const elapsed = now - Number(nativeAutoCandidateSinceTs || now);
      const requiredMs = getNativeAutoCommitDelayMs();
      if (elapsed >= requiredMs) return false;
      mediaActivationDeferrals += 1;
      traceCandidate('candidate_defer_transient_scroll', candidate, {
        ratio: Number(metrics?.ratio || 0),
        visiblePx: Number(metrics?.visiblePx || 0),
        centerDist: Number(metrics?.centerDist || 0),
        dwellMs: elapsed,
        requiredMs,
        velocityPxMs: getCoordinatorScrollVelocity(),
      });
    ...
    io = new IntersectionObserver(
      (entries) => {
        // IntersectionObserver already gives geometry for this delivery. Keep it
        // through the rAF arbitration so we do not synchronously force layout again.
        const metricsByElement = new Map();
        // обновляем ratios
        for (const entry of entries) {
          const el = entry.target;
          const r = entry.isIntersecting ? (entry.intersectionRatio || 0) : 0;
          const entryMetrics = getCandidateMetricsFromRect(el, r, entry.boundingClientRect);
          if (entryMetrics) metricsByElement.set(el, entryMetrics);
          if (r <= 0) {
            ratios.delete(el);
        ...
        rafId = requestAnimationFrame(() => {
          rafId = 0;
          const { el: candidate, ratio, score, visiblePx, centerDist, gapPx } = pickMostVisible(metricsByElement);
          const candidateMetrics = { ratio, score, visiblePx, centerDist, gapPx };
          const deferNativeAutoCandidate = shouldDeferNativeAutoCandidate(candidate, candidateMetrics);
          ...
          if (candidate && ratio >= PREWARM_RATIO && !isReadyCandidate(candidate)) {
            ...
            const externalCanPrepare =
              ...
              (
                ...
                (
                  Number(candidateMetrics?.gapPx ?? getOwnerViewportGapPx(candidate)) <= externalRunwayPx &&
                  centerDist <= getPriorityCenterMaxDist(candidate) + externalRunwayPx
                )
              );
            const prepared = isNativeCandidate
              ? (deferNativeAutoCandidate ? false : prepareNativePriorityPrewarm(candidate, 'early_native_candidate'))
              : (
                  externalCanPrepare
                    ? (
                        isExternalCandidate
          ...
          if (active) {
            ...
            const activeMetrics = metricsByElement.get(active) || getCandidateMetrics(active, ar);
            ...
            if (
              ...
              if (
                ...
                const now = Date.now();
                if (deferNativeAutoCandidate) return;
            ...
            if (
              ...
              softDemoteNativePostVideo(active, 'below_stop_ratio_soft_hold_visible', {
                clearLoadPending: false,
                keepResident: true,
                keepPrewarm: releaseSurface.nativePrewarm,
          ...
          // На iPhone сохраняем мягкий relaxed-start для metadata-prepared / poster-backed кандидата.
    ...
    const prewarmAhead = (el, reason = 'near_viewport') => {
      ...
      if (kind === 'youtube' || kind === 'tiktok' || kind === 'iframe') {
        if (isManualOnlyExternalFrame(el)) {
          traceCandidate('external_manual_only_skip_prewarm', el, { reason });
          return false;
        }
        // Keep speculative iframe boot off the hot-scroll path, but never delay the
        // actual focused candidate: the main IO path below owns immediate autoplay.
        if (isSpeculativePrewarmScrollHot(el)) {
          mediaActivationDeferrals += 1;
          traceCandidate('external_prewarm_skip_scroll_hot', el, { reason });
          return false;
        }
      ...
      try {
        ...
        const rows = nodes
          .map((node) => {
            ...
            if (isNativeSrcCapCooldownActive(node)) return null;
    ...
    const scheduleNativePrewarmScan = (reason = 'predictive_native_scan') => {
      try {
        const now = Date.now();
        const nativeSettleMs = isIOSUi ? 180 : (isCoarseUi ? 140 : 80);
        const scrollHot = coordinatorLastScrollEventTs > 0 && (now - coordinatorLastScrollEventTs) < nativeSettleMs;
        if (scrollHot && reason !== 'scroll_settled_native_predictive_scan') {
          hotScrollNativeScanSkips += 1;
          return;
        }
        if (nativePrewarmScanRaf) return;
        const minScanGap = isIOSUi ? 180 : (isCoarseUi ? 220 : 160);
        if ((now - Number(nativePrewarmScanLastTs || 0)) < minScanGap) return;
      ...
      (entries) => {
        ...
        const intersecting = (entries || [])
          ...
          .map((entry) => {
            ...
            const metrics = getCandidateMetricsFromRect(
              target,
              Number(entry.intersectionRatio || 0),
              entry.boundingClientRect,
            );
            const placement = getNearQueuePlacementFromRect(target, dir, entry.boundingClientRect);
            return {
              entry,
              metrics,
              placement,
    ...
    const observeOne = (el) => {
      try {
        ...
        if (!el.isConnected) {
          ...
          pendingRemovedMediaOwners.delete(el);
          const detachedMedia = getMediaStateNode(el);
          if (detachedMedia instanceof HTMLMediaElement) {
            try { __dropActiveVideoEl(detachedMedia); } catch {}
          }
          if (
            detachedMedia instanceof HTMLVideoElement &&
            !detachedMedia.isConnected &&
            isManagedForumVideoKind(detachedMedia)
          ) {
            try { releaseDetachedManagedVideoPipeline(detachedMedia, 'observe_detached_guard'); } catch {}
          }
          return;
        }
        ...
        if (isNativePostVideoCandidate(el)) {
          ...
          if (
            ...
            if (isNativeSrcCapCooldownActive(media)) {
              trace('observe_native_src_cap_cooldown', media, {
                visiblePx,
                gapPx,
                blockedForMs: Math.max(0, readNativeSrcCapBlockedUntil(media) - Date.now()),
              });
            ...
            if (visiblePx > Math.max(40, Math.round(getStartVisiblePx(media) * 0.32))) {
              trace('observe_native_visible_prewarm', media, { visiblePx, gapPx });
    ...
    const publishMediaRegistryState = () => {
      if (process.env.NODE_ENV === 'production') return;
      try {
        window.__forumMediaRegistryState = () => {
          let connected = 0;
          let disconnected = 0;
          mediaRegistry.forEach((owner) => {
            if (owner?.isConnected) connected += 1;
            else disconnected += 1;
          });
          return {
            registry: mediaRegistry.size,
            connected,
            disconnected,
            pendingRemoved: pendingRemovedMediaOwners.size,
            sweeps: mediaLifecycleSweepCount,
            snapshots: mediaLifecycleSnapshotCount,
            geometryReads: mediaLifecycleGeometryReadCount,
            fastPathSweeps: mediaLifecycleFastPathCount,
            globalDiagnosticScans: mediaGlobalDiagnosticScanCount,
            selfHealed: mediaRegistrySelfHealCount,
            detachedHardReleased: mediaRegistryDetachedReleaseCount,
            sourceRewarmBlocked: sourceRewarmBlockCount,
            hotPathRegistryReads: mediaRegistryHotPathReads,
            activationDeferrals: mediaActivationDeferrals,
            scrollSettleRuns: mediaScrollSettleRuns,
            hotScrollNativeScanSkips,
          };
        };
      } catch {}
    };

    const collectRemovedMediaOwners = (node) => {
      try {
        if (!(node instanceof Element)) return;
        forEachMediaOwner(node, (owner) => {
          if (owner instanceof Element) pendingRemovedMediaOwners.add(owner);
        });
      } catch {}
    };

    const flushRemovedMediaOwners = () => {
      mediaRemovalRaf = 0;
      const owners = Array.from(pendingRemovedMediaOwners);
      pendingRemovedMediaOwners.clear();
      owners.forEach((owner) => {
        if (!(owner instanceof Element)) return;
        if (owner.isConnected) {
          try { observed.add(owner); } catch {}
          mediaRegistry.add(owner);
          try { io?.observe?.(owner); } catch {}
          try { nearIo?.observe?.(owner); } catch {}
          return;
        }
        cleanupObservedMediaNode(owner, 'mutation_removed_confirmed');
      });
      publishMediaRegistryState();
    };

    const scheduleRemovedMediaOwnersFlush = () => {
      if (mediaRemovalRaf) return;
      try {
        mediaRemovalRaf = requestAnimationFrame(flushRemovedMediaOwners);
      } catch {
        mediaRemovalRaf = setTimeout(flushRemovedMediaOwners, 0);
      }
    };
...
const onExternalMediaPlay = (e) => {
  ...
  promoteExternalActive(candidate, `${source}_external_play`, { manual });
    ...
    const onCoordinatorScroll = () => {
      coordinatorLastScrollEventTs = Date.now();
      recordCoordinatorScrollMotion();
      updateCoordinatorScrollDirection();
      // Candidate commit is independent from scroll-stop; predictive scans still settle.
      hotScrollNativeScanSkips += 1;
      schedulePredictivePrewarmAfterScroll();
    };
    try {
    ...
    const recoverVisibleHtmlMedia = (reason = 'visibility_recover') => {
      try {
        ...
        mediaRegistry.forEach((owner) => {
          ...
          const hasSrcNow = hasAttachedMediaSrc(mediaEl);
          if (!hasSrcNow && __hasLazyVideoSourceWithoutSrc(mediaEl)) {
            if (isNativePostVideoCandidate(mediaEl)) {
              if (isNativeSrcCapCooldownActive(mediaEl)) {
                trace('visibility_restore', mediaEl, {
                  reason,
                  mode: 'native_src_cap_cooldown',
                  blockedForMs: Math.max(0, readNativeSrcCapBlockedUntil(mediaEl) - Date.now()),
                });
                return;
              }
              trace('visibility_restore', mediaEl, { reason, mode: 'native_budgeted_prewarm' });
              prepareNativePriorityPrewarm(mediaEl, `${reason}_native_recover`);
              return;
            }
...
if (hasSrcNow && readyStateNow === 0 && networkEmpty && mediaEl.dataset?.__loadPending !== '1') {
  ...
  if (!kickMediaLoad(mediaEl, {
    ...
    const normalizeQl7MediaSrc = (raw) => {
      try {
        const text = String(raw || '').trim();
        if (!text) return '';
        const url = new URL(text, window.location.href);
        url.hash = '';
        return `${url.hostname.toLowerCase()}${String(url.pathname || '').replace(/\/+$/, '').toLowerCase()}${url.search ? url.search.toLowerCase() : ''}`;
      } catch {
        return String(raw || '').trim().toLowerCase();
      }
    };

    const getVideoFeedCards = () => {
      try {
        const grid = document.querySelector('[data-ql7-video-feed-grid="1"]');
        if (!grid) return [];
        return Array.from(grid.querySelectorAll('[data-feed-card="1"]'))
    ...
    const readQl7VideoFeedPressure = (reason = 'manual') => {
      mediaGlobalDiagnosticScanCount += 1;
      publishMediaRegistryState();
      try {
        const grid = document.querySelector('[data-ql7-video-feed-grid="1"]');
        const cards = getVideoFeedCards();
        const videos = grid
          ? Array.from(grid.querySelectorAll('video'))
          : Array.from(document.querySelectorAll('video[data-forum-media="video"]'));
        const iframes = grid
          ? Array.from(grid.querySelectorAll('iframe'))
          : Array.from(document.querySelectorAll('iframe[data-forum-media]'));
        const videoRows = videos.map((node, index) => {
          const owner = getOwnerNode(node) || node.closest?.('[data-feed-card="1"]') || node;
          const rect = owner?.getBoundingClientRect?.();
          const srcAttr = String(node.getAttribute?.('src') || '').trim();
          const currentSrc = String(node.currentSrc || '').trim();
          const src = readAttachedMediaSrc(node);
          const dataSrc = String(node.dataset?.__src || node.getAttribute?.('data-src') || '').trim();
          const kind = String(node.getAttribute?.('data-forum-video') || (isAdMediaElement(node) ? 'ad' : 'post'));
          const id = String(owner?.id || owner?.getAttribute?.('data-ad-slot-key') || node.dataset?.__mid || `video:${index}`);
          const top = Number(rect?.top || 0);
          const bottom = Number(rect?.bottom || 0);
          ...
          const gapPx = bottom < 0 ? Math.abs(bottom) : (top > vh ? top - vh : 0);
          return {
            index,
            id,
            kind,
            ...
            currentSrc,
            dataSrc,
            srcKey: normalizeQl7MediaSrc(src || dataSrc),
            hasSrc: !!src,
            loadPending: String(node.dataset?.__loadPending || '') === '1',
            active: String(node.dataset?.__active || '') === '1',
            prewarm: String(node.dataset?.__prewarm || '') === '1',
            nativePrewarm: String(node.dataset?.__nativePrewarm || '') === '1' || node === nativePrewarmEl,
            srcCapBlockedUntil: Number(node.dataset?.__nativeSrcCapBlockedUntil || 0),
            paused: !!node.paused,
            ended: !!node.ended,
            readyState: Number(node.readyState || 0),
            networkState: Number(node.networkState || 0),
            currentTime: Number(node.currentTime || 0),
            duration: Number(node.duration || 0),
            visiblePx: Math.round(visiblePx),
            gapPx: Number.isFinite(gapPx) ? Math.round(gapPx) : 999999,
            top: Math.round(top),
            bottom: Math.round(bottom),
          };
        });
        const withSrc = videoRows.filter((row) => row.hasSrc);
        const srcCounts = new Map();
        withSrc.forEach((row) => {
          if (!row.srcKey) return;
          srcCounts.set(row.srcKey, (srcCounts.get(row.srcKey) || 0) + 1);
        });
        const duplicateSrcWithSrc = Array.from(srcCounts.entries())
          .filter(([, count]) => count > 1)
          .map(([srcKey, count]) => ({ srcKey, count }));
        const iframeRows = iframes.map((node, index) => ({
          index,
          kind: String(node.getAttribute?.('data-forum-media') || node.getAttribute?.('data-kind') || 'iframe'),
          hasSrc: !!String(node.getAttribute?.('src') || '').trim(),
          src: String(node.getAttribute?.('src') || '').slice(0, 180),
        }));
        const mem = (() => {
          try {
            ...
            if (!m) return null;
            return {
              usedMB: Math.round(Number(m.usedJSHeapSize || 0) / 1048576),
              totalMB: Math.round(Number(m.totalJSHeapSize || 0) / 1048576),
              limitMB: Math.round(Number(m.jsHeapSizeLimit || 0) / 1048576),
            };
          } catch {
            return null;
          }
        })();
        let registryConnected = 0;
        let registryDisconnected = 0;
        try {
          mediaRegistry.forEach((owner) => {
            if (owner instanceof Element && owner.isConnected) registryConnected += 1;
            else registryDisconnected += 1;
          });
        } catch {}
        const sourceRewarmBlocked = (() => {
          const now = Date.now();
          let count = 0;
          try {
            srcKickState.forEach((state) => {
              if (Number(state?.rewarmBlockedUntil || 0) > now) count += 1;
            });
          } catch {}
          return count;
        })();
        const snapshot = {
          ...
          nativeSrcBudget: {
            cap: POST_NATIVE_SRC_CAP,
            attached: withSrc.length,
            overflow: Math.max(0, withSrc.length - POST_NATIVE_SRC_CAP),
            cooldown: videoRows.filter((row) => Number(row.srcCapBlockedUntil || 0) > Date.now()).length,
            sourceRewarmBlocked,
          },
          mediaRegistry: {
            size: mediaRegistry.size,
            connected: registryConnected,
            disconnected: registryDisconnected,
            selfHealed: mediaRegistrySelfHealCount,
            detachedHardReleased: mediaRegistryDetachedReleaseCount,
          },
          postVideos: videoRows.filter((row) => row.kind === 'post').length,
          postVideosWithSrc: withSrc.filter((row) => row.kind === 'post').length,
          adVideos: videoRows.filter((row) => row.kind === 'ad').length,
          adVideosWithSrc: withSrc.filter((row) => row.kind === 'ad').length,
          videosLoadPending: videoRows.filter((row) => row.loadPending).length,
          iframes: iframeRows.length,
          iframesWithSrc: iframeRows.filter((row) => row.hasSrc).length,
          duplicateSrcWithSrc,
          active: videoRows.find((row) => row.active || (!row.paused && !row.ended)) || null,
          next: videoRows.filter((row) => row.hasSrc && !row.active && row.visiblePx <= 0).sort((a, b) => a.gapPx - b.gapPx)[0] || null,
          rows: videoRows,
          iframesRows: iframeRows,
          scheduler: {
            owner: 'useForumMediaCoordinator',
            nativePrewarmSrc: String(readAttachedMediaSrc(nativePrewarmEl) || nativePrewarmEl?.dataset?.__src || ''),
            nativePrewarmCurrentSrc: String(nativePrewarmEl?.currentSrc || ''),
            nativePrewarmConnected: nativePrewarmEl instanceof HTMLVideoElement ? !!nativePrewarmEl.isConnected : null,
            activeKind: String(active?.getAttribute?.('data-forum-media') || ''),
          },
          memory: mem,
        };
        return snapshot;
      } catch (err) {
        return { error: String(err?.message || err || 'pressure_read_failed') };
      }
    };

    const buildQl7MediaLifecycleSnapshot = (reason = 'periodic') => {
      mediaLifecycleSnapshotCount += 1;
      const rows = [];
      let index = 0;
      let grid = null;
      try { grid = document.querySelector('[data-ql7-video-feed-grid="1"]'); } catch {}
      mediaRegistry.forEach((owner) => {
        if (!(owner instanceof Element) || !owner.isConnected) return;
        let node = null;
        if (owner instanceof HTMLVideoElement) node = owner;
        else {
          try { node = owner.querySelector?.('video'); } catch {}
        }
        if (!(node instanceof HTMLVideoElement)) return;
        if (grid instanceof Element) {
          if (!grid.contains(owner) && !grid.contains(node)) return;
        } else if (!node.matches?.('video[data-forum-media="video"]')) {
          return;
        }
        const src = readAttachedMediaSrc(node);
        const currentSrc = String(node.currentSrc || '').trim();
        ...
        rows.push({
          ...
          owner,
          node,
          kind: String(node.getAttribute?.('data-forum-video') || (isAdMediaElement(node) ? 'ad' : 'post')),
          src,
          currentSrc,
          dataSrc,
          srcKey: normalizeQl7MediaSrc(src || dataSrc),
      ...
      return { reason, ts: Date.now(), rows };
    };

    const measureQl7MediaLifecycleRow = (row, viewportH) => {
      if (!row?.hasSrc || !(row?.owner instanceof Element) || !(viewportH > 0)) return row;
      try {
        mediaLifecycleGeometryReadCount += 1;
        const rect = row.owner.getBoundingClientRect?.();
        if (!rect) return row;
        const top = Number(rect.top || 0);
        const bottom = Number(rect.bottom || 0);
        row.visiblePx = Math.max(0, Math.min(bottom, viewportH) - Math.max(top, 0));
        row.gapPx = bottom < 0 ? Math.abs(bottom) : (top > viewportH ? top - viewportH : 0);
        row.top = Math.round(top);
        row.bottom = Math.round(bottom);
        row.visiblePx = Math.round(row.visiblePx);
        row.gapPx = Number.isFinite(row.gapPx) ? Math.round(row.gapPx) : 999999;
        row.geometryMeasured = true;
      } catch {}
      return row;
    };

    const installQl7MediaDiagnostics = () => {
      try {
        window.__ql7VideoFeedPressure = () => readQl7VideoFeedPressure('manual');
        window.__ql7AdMediaRuntimeState = () => {
          const snap = readQl7VideoFeedPressure('ad_runtime');
          return {
            ts: snap.ts,
            adVideos: snap.rows?.filter?.((row) => row.kind === 'ad') || [],
            duplicateSrcWithSrc: snap.duplicateSrcWithSrc || [],
          };
        };
        window.__ql7MediaPressureWatch = (ms = 1500) => {
          try { clearInterval(window.__ql7MediaPressureWatchTimer); } catch {}
          const delay = Math.max(500, Number(ms || 1500));
          const tick = () => {
            const s = readQl7VideoFeedPressure('watch');
            const row = {
              scrollY: s.scrollY,
              cards: s.cards,
              videosWithSrc: s.videosWithSrc,
              postVideosWithSrc: s.postVideosWithSrc,
              adVideosWithSrc: s.adVideosWithSrc,
              iframesWithSrc: s.iframesWithSrc,
              loadPending: s.videosLoadPending,
              duplicateSrc: (s.duplicateSrcWithSrc || []).length,
              srcCap: s.nativeSrcBudget?.cap ?? '',
              srcOverflow: s.nativeSrcBudget?.overflow ?? '',
              sourceRewarmBlocked: s.nativeSrcBudget?.sourceRewarmBlocked ?? '',
              registryDisconnected: s.mediaRegistry?.disconnected ?? '',
              registrySelfHealed: s.mediaRegistry?.selfHealed ?? '',
              detachedHardReleased: s.mediaRegistry?.detachedHardReleased ?? '',
              usedMB: s.memory?.usedMB ?? '',
            };
            try { console.table([row]); } catch { console.log('[QL7 media pressure]', row); }
            return s;
          };
          tick();
          window.__ql7MediaPressureWatchTimer = setInterval(tick, delay);
          return window.__ql7MediaPressureWatchTimer;
        };
      } catch {}
    };

    const runQl7MediaLifecycleSweep = (reason = 'periodic') => {
      try {
        mediaLifecycleSweepCount += 1;
        const registryHealed = selfHealDisconnectedMediaRegistry(`lifecycle_${reason}`);
        const snap = buildQl7MediaLifecycleSnapshot(reason);
        const rows = Array.isArray(snap.rows) ? snap.rows : [];
        const budget = POST_NATIVE_SRC_CAP;
        const now = Date.now();
        const detachedRows = new Set();
        const protectRow = (row) => {
          if (!row) return false;
          if (row.visiblePx > 0) return true;
          if (row.active || row.nativePrewarm) return true;
          if (!row.paused && !row.ended) return true;
          if (row.gapPx <= (isChromeDesktopRuntime ? 260 : (isCoarseUi ? 760 : 520))) return true;
          return false;
        };
        const detachRow = (row, detachReason) => {
          try {
            const video = row?.node;
            if (!(video instanceof HTMLVideoElement)) return false;
            if (detachedRows.has(row) || protectRow(row)) return false;
            const lastDetach = Number(video.dataset?.__ql7LastSweepDetachTs || 0);
            if (lastDetach > 0 && (now - lastDetach) < 1200) return false;
            video.dataset.__forceHardUnload = '1';
            video.dataset.__ql7LastSweepDetachTs = String(now);
            video.dataset.__ql7LastSweepDetachReason = String(detachReason || reason);
            video.dataset.__prewarm = '0';
            video.dataset.__resident = '0';
            video.dataset.__nativePrewarm = '0';
            scheduleHardUnload(video, 0, detachReason || reason);
            detachedRows.add(row);
            row.hasSrc = false;
            return true;
          } catch {
            return false;
        ...
        const attachedRows = rows.filter((row) => row.hasSrc);
        const srcGroups = new Map();
        attachedRows.forEach((row) => {
          if (!row.srcKey) return;
          const list = srcGroups.get(row.srcKey) || [];
          list.push(row);
          srcGroups.set(row.srcKey, list);
        });
        const needsDuplicateArbitration = Array.from(srcGroups.values()).some((group) => Array.isArray(group) && group.length > 1);
        const needsBudgetArbitration = attachedRows.length > budget;

        // Only rows with a live native src can be detached by this sweep. If the
        // current src set already satisfies both duplicate and budget policy,
        // geometry cannot change the outcome. Skip layout reads entirely.
        if (!needsDuplicateArbitration && !needsBudgetArbitration) {
          mediaLifecycleFastPathCount += 1;
          try { enforcePostNativeSrcCap(nativePrewarmEl, reason); } catch {}
          publishMediaRegistryState();
          return {
            reason,
            ts: snap.ts,
            rows: rows.map(({ owner: _owner, node: _node, ...row }) => row),
            detached: 0,
            registry: mediaRegistry.size,
            registryHealed,
            fastPath: true,
        ...
        // When arbitration is required, measure only rows that actually own a src.
        // No-src poster rows never participate in duplicate/budget detachment.
        const viewportH = Number(window?.innerHeight || document?.documentElement?.clientHeight || 0) || 0;
        attachedRows.forEach((row) => measureQl7MediaLifecycleRow(row, viewportH));

        srcGroups.forEach((group) => {
          if (!Array.isArray(group) || group.length <= 1) return;
          group.slice().sort((a, b) => {
            const pa = protectRow(a) ? 1 : 0;
            const pb = protectRow(b) ? 1 : 0;
            if (pa !== pb) return pb - pa;
            return a.gapPx - b.gapPx;
          }).slice(1).forEach((row) => detachRow(row, 'duplicate_src_sweep'));
        });
        const attached = attachedRows.filter((row) => !detachedRows.has(row)).sort((a, b) => {
          const pa = protectRow(a) ? 1 : 0;
          const pb = protectRow(b) ? 1 : 0;
          if (pa !== pb) return pb - pa;
          if (a.visiblePx !== b.visiblePx) return b.visiblePx - a.visiblePx;
          return a.gapPx - b.gapPx;
        });
        attached.slice(budget).forEach((row) => detachRow(row, 'attached_budget_sweep'));
        try { enforcePostNativeSrcCap(nativePrewarmEl, reason); } catch {}
        publishMediaRegistryState();
        return {
          reason,
    ...
    installQl7MediaDiagnostics();
    let ql7LifecycleSweepRaf = 0;
    let ql7LifecycleSweepIdleTimer = 0;
    const readRecentForumScrollAge = () => {
      try {
        const now = Date.now();
        const last = Math.max(
    ...
    const readLifecycleSweepIdleMs = () => (isIOSUi ? 620 : (isCoarseUi ? 560 : 420));
    const isLifecycleScrollSettling = () => readRecentForumScrollAge() < readLifecycleSweepIdleMs();
    const scheduleQl7MediaLifecycleSweep = (reason = 'scroll', options = null) => {
      if (ql7LifecycleSweepRaf) return;
      const force = !!options?.force;
      const deferUntilIdle = !!options?.deferUntilIdle || String(reason || '').includes('scroll');
      try {
        if (!force && (deferUntilIdle || isLifecycleScrollSettling())) {
          if (ql7LifecycleSweepIdleTimer) {
            try { clearTimeout(ql7LifecycleSweepIdleTimer); } catch {}
            ql7LifecycleSweepIdleTimer = 0;
          }
          const waitMs = Math.max(140, readLifecycleSweepIdleMs() - readRecentForumScrollAge());
          ql7LifecycleSweepIdleTimer = setTimeout(() => {
            ql7LifecycleSweepIdleTimer = 0;
            scheduleQl7MediaLifecycleSweep(`${reason}_idle`, { force: true });
          }, Number.isFinite(waitMs) ? waitMs : readLifecycleSweepIdleMs());
          return;
        }
        ql7LifecycleSweepRaf = requestAnimationFrame(() => {
          ql7LifecycleSweepRaf = 0;
          runQl7MediaLifecycleSweep(reason);
        });
      } catch {
        ql7LifecycleSweepRaf = setTimeout(() => {
          ql7LifecycleSweepRaf = 0;
          runQl7MediaLifecycleSweep(reason);
        }, 80);
      }
    };
    const ql7LifecycleSweepTimer = setInterval(() => {
      try { selfHealDisconnectedMediaRegistry('timer_registry_fast_heal'); } catch {}
      if (isLifecycleScrollSettling()) {
        scheduleQl7MediaLifecycleSweep('timer_after_scroll', { deferUntilIdle: true });
        return;
      }
      runQl7MediaLifecycleSweep('timer');
    }, isChromeDesktopRuntime ? 1200 : 1800);
    const onQl7LifecycleScroll = () => scheduleQl7MediaLifecycleSweep('window_scroll');
    window.addEventListener('scroll', onQl7LifecycleScroll, { passive: true });
      ...
      mo = new MutationObserver((mutations) => {
        ...
        for (const m of mutations) {
          ...
          for (const n of (m.removedNodes || [])) {
            ...
            touchedMedia = true;
            collectRemovedMediaOwners(n);
          }
        }
        if (!touchedMedia) return;
        if (pendingRemovedMediaOwners.size) scheduleRemovedMediaOwnersFlush();
    ...
    return () => {
      try { clearInterval(ql7LifecycleSweepTimer); } catch {}
      try { window.removeEventListener('scroll', onQl7LifecycleScroll); } catch {}
      if (ql7LifecycleSweepIdleTimer) {
        try { clearTimeout(ql7LifecycleSweepIdleTimer); } catch {}
        ql7LifecycleSweepIdleTimer = 0;
      }
      if (ql7LifecycleSweepRaf) {
        try { cancelAnimationFrame(ql7LifecycleSweepRaf); } catch {}
        try { clearTimeout(ql7LifecycleSweepRaf); } catch {}
        ql7LifecycleSweepRaf = 0;
      }
      ...
      if (mediaScrollSettleTimer) {
        try { clearTimeout(mediaScrollSettleTimer); } catch {}
        mediaScrollSettleTimer = 0;
      }
      clearNativeAutoCandidateCommit();
      ...
      if (mediaRemovalRaf) {
        try { cancelAnimationFrame(mediaRemovalRaf); } catch {}
        try { clearTimeout(mediaRemovalRaf); } catch {}
        mediaRemovalRaf = 0;
      }
      pendingRemovedMediaOwners.clear();
      try { Array.from(mediaRegistry).forEach((owner) => cleanupObservedMediaNode(owner, 'coordinator_cleanup')); } catch {}
      mediaRegistry.clear();
      io?.disconnect?.();
      nearIo?.disconnect?.();
      if (process.env.NODE_ENV !== 'production') {
        try { delete window.__forumMediaRegistryState; } catch {}
      }
