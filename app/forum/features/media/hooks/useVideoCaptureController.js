...
function getCameraVideoConstraints() {
  if (isMobileRecorderRuntime()) {
    // Match the proven flip-camera reacquisition path on mobile.
    // Width/height hints made the first iPhone front-camera stream negotiate a narrow 16:9 preset,
    // while returning to the same front camera by facing/device id used its normal native preset.
    return {
