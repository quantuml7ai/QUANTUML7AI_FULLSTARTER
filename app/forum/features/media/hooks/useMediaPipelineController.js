...
export default function useMediaPipelineController({
  ...
  const formatMediaPhase = useCallback((phase) => {
    ...
    if (p === 'Prepared') return t('media_pipeline_ready') || t('forum_media_ready') || t('media_pipeline_verifying')
  ...
  const beginMediaPipeline = useCallback((phase = 'Moderating', externalController = null, options = null) => {
    const continueExisting = options?.continueExisting === true
    let ac = null

    if (continueExisting) {
      ac = externalController || mediaAbortRef.current || ensureMediaAbortController()
      if (externalController && mediaAbortRef.current !== externalController) {
        mediaAbortRef.current = externalController
      }
      try { mediaCancelRef.current = false } catch {}
    } else {
      ac = externalController || ensureMediaAbortController()
    ...
    setMediaPhase(phase)
    if (continueExisting) {
      setMediaPct((previous) => Math.max(1, Number(previous || 0)))
    } else {
      setMediaPct(1)
    }
    ...
    if (phaseKey === 'uploading') {
      startSoftProgress(continueExisting ? 94 : 72, 160, continueExisting ? 99 : 94)
  ...
  }, [stopMediaProg, clearMediaAbortController])
  ...
  useEffect(() => {
    ...
    const preparedVideoParked = localVideoPreviewOnly && String(mediaPhase || '').toLowerCase() === 'prepared'
    if (preparedVideoParked) {
      stopMediaProg()
      setMediaBarOn(true)
      setMediaPct((p) => Math.max(94, Number(p || 0)))
  ...
  }, [hasComposerMedia, stopMediaProg, mediaPipelineOn, pendingImgs, pendingVideo, mediaPhase])
