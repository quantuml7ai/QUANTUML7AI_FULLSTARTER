...
export default function useVideoFeedWindowing({
  ...
  const { win: vfWin, measureRef: vfMeasureRef } = useForumWindowing({
    ...
    scrollToTopOnHardReset: false,
    mediaKeepaliveEnabled: true,
    // The last scroll frame already owns a bounded, viewport-safe window.
    // Do not trigger a second direction-neutral shrink immediately after touch
    // inertia ends; later real measurements / TTL expiry still recalc normally.
