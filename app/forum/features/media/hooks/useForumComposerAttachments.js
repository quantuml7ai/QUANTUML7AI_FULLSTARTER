...
import { enterVideoHeavyActivity, mapVideoPrepareProgress } from '../../../../../lib/videoPipelineProgress'
import { preparePreparedVideoArtifacts } from '../services/moderatePreparedVideoForUpload'
    ...
    try {
      ...
      if (vidFiles.length) {
        ...
        try {
          ...
          const previewFile = preparedOnSelect ? prepared?.file : vf
          if (!(previewFile instanceof Blob)) throw new Error('forum_video_preview_file_missing')

          let preparedArtifacts = null
          if (preparedOnSelect) {
            ensureHeavyActivity()
            preparedArtifacts = await preparePreparedVideoArtifacts({
              file: previewFile,
              surface: videoModerationSurface,
              userId: String(videoModerationActorId || ''),
              signal: controller.signal,
              mirrorX: false,
              sourceName: String(previewFile?.name || vf?.name || 'forum-video.mp4'),
              onProgress: startHevcProgress,
            })
            if (selectionCancelled()) return
          }
          ...
          const previewMeta = {
            previewUrl: committedPreviewUrl,
          ...
          // Publish the canonical blob URL synchronously. React state/ref mirroring happens
          // later in an effect; Send must never refetch this prepared URL into a new Blob
          // and lose the exact-file WeakMap proof in that window.
          try { if (pendingVideoRef) pendingVideoRef.current = committedPreviewUrl } catch {}
          ...
          if (preparedOnSelect) {
            // HEVC/H.265 heavy work is finished here. Release execution ownership now so
            // the composer is not held busy while the user decides when to Send. The
            // progress card remains parked at Prepared and is continued at Uploading.
            if (pipelineStarted) {
              try { endMediaPipeline?.() } catch {}
              pipelineStarted = false
            }
            try { setMediaPhase?.('Prepared') } catch {}
            try { setMediaPct?.((previous) => Math.max(94, Number(previous || 0))) } catch {}
            try { setVideoProgress?.(100) } catch {}
          } else {
            try { setMediaPhase?.('idle') } catch {}
            try { setMediaPct?.(0) } catch {}
            try { setVideoProgress?.(0) } catch {}
