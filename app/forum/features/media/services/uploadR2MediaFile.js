...
import { getForumVerifiedVideoUploadProof, prepareForumVideoForUpload } from '../../../../../lib/forumClientVideoOptimizer'
import { enterVideoHeavyActivity } from '../../../../../lib/videoPipelineProgress'
import { getPreparedVideoArtifacts, preparePreparedVideoArtifacts } from './moderatePreparedVideoForUpload'
...
export default async function uploadR2MediaFile({
  ...
  const resolvedModerationSurface = resolveModerationSurface(kind, moderationSurface)
  const requestedVideo = /^video\//i.test(String(contentType || file?.type || '')) || /_video$/i.test(String(kind || ''))
  const artifactContext = {
    surface: resolvedModerationSurface,
    userId: String(userId || ''),
    mirrorX: !!videoPolicy?.mirrorX,
  }
  const sourceProof = requestedVideo ? getForumVerifiedVideoUploadProof(file) : null
  let videoArtifacts = requestedVideo ? getPreparedVideoArtifacts(file, artifactContext) : null
  if (sourceProof?.source === 'optimizer-output' && !videoArtifacts) {
  ...
  if (sourceProof?.source === 'optimizer-output' && videoArtifacts) {
    preparation = {
      file,
      filename: String(filename || file?.name || 'video.mp4'),
      contentType: 'video/mp4',
      isVideo: true,
      optimized: true,
      bypassReason: 'prepared-artifacts',
      policyId: sourceProof.policyId || null,
      durationSec: Number(sourceProof.durationSec) || null,
      width: Number(sourceProof.width) || null,
      height: Number(sourceProof.height) || null,
      frameRate: Number(sourceProof.frameRate) || null,
    }
  } else {
    try {
      if (requestedVideo) releaseHeavy = enterVideoHeavyActivity()
      preparation = await prepareForumVideoForUpload({
        file,
        kind,
        filename,
        contentType,
      ...
      const candidateFile = preparation?.file || file
      if (preparation?.isVideo) {
        videoArtifacts = getPreparedVideoArtifacts(candidateFile, artifactContext)
        if (!videoArtifacts) {
          videoArtifacts = await preparePreparedVideoArtifacts({
            ...
            signal,
            mirrorX: !!videoPolicy?.mirrorX,
            sourceName: String(preparation?.filename || filename || candidateFile?.name || 'video.mp4'),
            onProgress: onPrepareProgress,
          })
        }
      }
  ...
  if (preparation?.isVideo) {
    videoModeration = getPreparedVideoArtifacts(uploadFile, artifactContext)
    if (
      !videoModeration ||
      videoModeration.file !== uploadFile ||
      videoModeration?.status !== 'approved' ||
      !videoModeration?.moderationReceipt ||
      !videoModeration?.mediaSha256
    ) {
      throw new Error('R2 video gateway requires exact-file pre-upload moderation approval')
    }
  }
  ...
  try {
    ...
    const fallbackResult = await uploadForumVideoViaServer({
      ...
      userId,
      filename: resolvedFilename,
      moderationSurface: resolvedModerationSurface,
      moderationReceipt: videoModeration?.moderationReceipt,
      mediaSha256: videoModeration?.mediaSha256,
      signal,
      onUploadProgress,
