...
export async function resolveComposerMediaPayload({
  ...
  let videoPosterUrlToSend = ''
  let videoModerationSource = null
  let videoApprovalToken = ''
  const fail = (msg) => {
    try { onFail?.(msg) } catch {}
    return { failed: true, imageUrlsToSend: [], videoUrlToSend: '', videoPosterUrlToSend: '', audioUrlToSend: '', videoMetaToSend: null, videoModerationSource: null, videoApprovalToken: '' }
  }
  ...
  // React's state->ref effect can lag behind the user's Send click. The synchronous
  // ref is accepted only when current composer metadata binds it to the same preview URL;
  // this prevents both exact-file loss on fast Send and stale-ref reuse after removal.
  const pendingVideoState = String(pendingVideo || '')
  const pendingVideoRefValue = String(pendingVideoRef?.current || '')
  let pendingVideoInfo = null
  try { pendingVideoInfo = pendingVideoInfoRef?.current || null } catch {}
  const refMatchesCurrentPreview = !!(
    pendingVideoRefValue &&
    String(pendingVideoInfo?.previewUrl || '') === pendingVideoRefValue
  )
  const pendingVideoCurrent = refMatchesCurrentPreview ? pendingVideoRefValue : (pendingVideoState || pendingVideoRefValue)
  let pendingVideoSendMeta = null
  try {
    pendingVideoSendMeta = pendingVideoCurrent
      ? pendingVideoBlobMetaRef.current?.get?.(pendingVideoCurrent) || null
      : null
  } catch {}
  if (!pendingVideoSendMeta) {
    try {
      if (
        pendingVideoInfo?.preparedOnSelect &&
        pendingVideoInfo?.preparedFile instanceof Blob &&
        String(pendingVideoInfo?.previewUrl || '') === pendingVideoCurrent
      ) {
        pendingVideoSendMeta = pendingVideoInfo
      }
    } catch {}
  }
  const preparedVideoAtSend = !!(pendingVideoSendMeta?.preparedOnSelect && pendingVideoSendMeta?.preparedFile instanceof Blob)
  const hasLocalBlobMedia =
    localImageUrls.length > 0 ||
    (pendingVideoCurrent && /^blob:/.test(pendingVideoCurrent)) ||
  ...
  if (hasLocalBlobMedia) {
    const ac = (() => {
      try {
        if (preparedVideoAtSend && !localImageUrls.length) {
          return beginMediaPipeline?.('Uploading', null, { continueExisting: true })
        }
  ...
  if (pendingVideoCurrent) {
    try {
      if (/^blob:/.test(pendingVideoCurrent)) {
        let localMeta = pendingVideoSendMeta
        if (!localMeta) {
          try {
            localMeta = pendingVideoBlobMetaRef.current?.get?.(pendingVideoCurrent) || null
          } catch {}
        }
        if (!localMeta) {
          try { localMeta = pendingVideoInfoRef?.current || null } catch {}
        }

        let fileBlob = localMeta?.preparedFile instanceof Blob ? localMeta.preparedFile : null
        if (localMeta?.preparedOnSelect && !fileBlob) {
          throw new Error('prepared_video_exact_file_missing')
        }
        if (!fileBlob) {
        ...
        videoModerationSource = fileBlob
        ...
        const readSafeVideoMeta = (candidate) => {
          ...
          const posterUrl = String(candidate?.posterUrl || '').trim()
          if (!source && !facingMode && !frontCameraMirror && !posterUrl) return null
          return {
            source,
        ...
        const result = await uploadR2MediaFile({
          file: fileBlob,
          kind: 'forum_video',
          userId: String(videoModerationActorId || viewerId || ''),
          moderationSurface: videoModerationSurface,
          filename: sourceName,
          ...
          videoPolicy: {
            ...
            mirrorX: !!videoMetaToSend?.frontCameraMirror,
          },
        ...
        videoApprovalToken = String(result?.videoApprovalToken || '')
        videoPosterUrlToSend = String(result?.posterUrl || '')
        if (videoPosterUrlToSend) {
          videoMetaToSend = { ...(videoMetaToSend || {}), posterUrl: videoPosterUrlToSend }
        }
        ...
        videoModerationSource = pendingVideoCurrent
        try {
          const meta =
          ...
          videoMetaToSend = meta
            ? {
                ...
                frontCameraMirror,
                mirrorVideo: frontCameraMirror,
                ...(String(meta?.posterUrl || '').trim() ? { posterUrl: String(meta.posterUrl).trim() } : {}),
              }
            : null
        } catch {}
      }
  ...
  return {
    ...
    audioUrlToSend,
    videoMetaToSend,
    videoModerationSource: videoModerationSource || videoUrlToSend || null,
    videoApprovalToken,
  }
}
