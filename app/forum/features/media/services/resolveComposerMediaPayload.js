...
import { readCachedNativeVideoPoster } from '../../../../../lib/nativeVideoPoster'
  ...
  if (pendingVideoCurrent) {
    try {
      if (/^blob:/.test(pendingVideoCurrent)) {
        let localMeta = null
        try {
          localMeta = pendingVideoBlobMetaRef.current?.get?.(String(pendingVideoCurrent || '')) || null
        } catch {}
        ...
        const result = await uploadR2MediaFile({
          ...
          signal,
          videoPoster: readCachedNativeVideoPoster(pendingVideoCurrent, { mirrorX: !!videoMetaToSend?.frontCameraMirror }),
          videoPolicy: {
            mode: 'video-required',
