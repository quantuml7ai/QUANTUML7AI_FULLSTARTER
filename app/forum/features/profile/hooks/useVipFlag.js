...
const VIP_PROBE_TTL_MS = 20 * 60 * 1000
...
function pruneVipProbeCache(now = Date.now()) {
  ...
  while (vipProbeOnce.size > VIP_PROBE_CACHE_LIMIT) {
    const firstKey = vipProbeOnce.keys().next().value
    if (firstKey === undefined) break
    vipProbeOnce.delete(firstKey)
  }
}

function hasRecentVipProbe(uid, now = Date.now()) {
  pruneVipProbeCache(now)
  const ts = Number(vipProbeOnce.get(uid) || 0)
  return ts > 0 && (now - ts) <= VIP_PROBE_TTL_MS
}

function markVipProbe(uid, now = Date.now()) {
  if (!uid) return
  vipProbeOnce.delete(uid)
  vipProbeOnce.set(uid, now)
  pruneVipProbeCache(now)
}
