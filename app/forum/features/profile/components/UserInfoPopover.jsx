...
const USER_INFO_ENTER_MS = 420
const USER_INFO_EXIT_MS = 320
const USER_INFO_EXIT_SAFETY_MS = 96
...
export default function UserInfoPopover({
  ...
  const motionEnterRafARef = useRef(null)
  const motionEnterRafBRef = useRef(null)
  const motionExitTimerRef = useRef(null)
  const motionSkipExitRef = useRef(false)
  const positionRef = useRef({ top: 0, left: 0, slideX: -420, placement: 'bottom', ready: false })
  ...
  const [bioTranslationAnimation, setBioTranslationAnimation] = useState({ src: '', key: 0 })
  const [position, setPosition] = useState(positionRef.current)
  const [presenceOnline, setPresenceOnline] = useState(false)
  const [motionMounted, setMotionMounted] = useState(false)
  const [motionPhase, setMotionPhase] = useState('closed')
  const [motionUserId, setMotionUserId] = useState('')
  ...
  const clearMotionEnterFrames = useCallback(() => {
    if (motionEnterRafARef.current) {
      cancelAnimationFrame(motionEnterRafARef.current)
      motionEnterRafARef.current = null
    }
    if (motionEnterRafBRef.current) {
      cancelAnimationFrame(motionEnterRafBRef.current)
      motionEnterRafBRef.current = null
    }
  }, [])

  const clearMotionExitTimer = useCallback(() => {
    if (motionExitTimerRef.current) {
      clearTimeout(motionExitTimerRef.current)
      motionExitTimerRef.current = null
    }
  }, [])

  const finishMotionExit = useCallback(() => {
    clearMotionEnterFrames()
    clearMotionExitTimer()
    setMotionMounted(false)
  ...
  }, [clearMotionEnterFrames, clearMotionExitTimer])
    ...
    async (uid) => {
      ...
      const promise = (async () => {
        ...
        const payload = {
          ...
          officialStatus: json?.officialStatus === true,
          about: json?.about || '',
          stats: {
  ...
  useEffect(() => {
    ...
    if (open && targetUserId && hasAnchor) {
      clearMotionEnterFrames()
      clearMotionExitTimer()
      motionSkipExitRef.current = false
      setMotionUserId(targetUserId)
      setMotionMounted(true)
      setMotionPhase('pre-enter')
      return undefined
    }

    if (open || !motionMounted) return undefined

    if (motionSkipExitRef.current) {
      motionSkipExitRef.current = false
      finishMotionExit()
      return undefined
    }
    ...
    try {
      ...
      if (popoverRef.current?.contains?.(activeElement)) activeElement?.blur?.()
    } catch {}
    ...
    setMotionPhase('exit')
    motionExitTimerRef.current = setTimeout(
      finishMotionExit,
      USER_INFO_EXIT_MS + USER_INFO_EXIT_SAFETY_MS,
    )

    return () => clearMotionExitTimer()
  }, [
    anchorRef,
    clearMotionEnterFrames,
    clearMotionExitTimer,
  ...
  useEffect(() => {
    if (!open || !motionMounted || !position.ready || motionPhase !== 'pre-enter') return undefined
    ...
    motionEnterRafARef.current = requestAnimationFrame(() => {
      motionEnterRafARef.current = null
      motionEnterRafBRef.current = requestAnimationFrame(() => {
        motionEnterRafBRef.current = null
        setMotionPhase('open')
      })
    })

    return () => clearMotionEnterFrames()
  }, [clearMotionEnterFrames, motionMounted, motionPhase, open, position.ready])
  ...
  useEffect(() => {
    if (!open || !rawUserId) return undefined
  ...
  const updatePosition = useCallback(() => {
    ...
    const next = {
      ...
      slideX: Math.round(-(left + popW + 20)),
      placement,
      ready: true,
    }
    ...
    if (
      ...
      positionRef.current.slideX === next.slideX &&
  ...
  useEffect(() => {
    if (!open || !motionMounted) return
    schedulePositionUpdate()
  }, [open, motionMounted, schedulePositionUpdate, status, data, translatedBio, showOriginal])
  ...
  const handleToggleTranslate = useCallback(
    async (e) => {
      ...
      if (translatedBio) {
        setBioTranslationAnimation((prev) => ({
          src: `${String(rawUserId || '')}\u0000${String(data.about || '')}`,
          key: prev.key + 1,
        }))
    ...
    [data?.about, rawUserId, registerAction, translatedBio],
  )
  ...
  const openDirectMessage = useCallback((event) => {
    event?.preventDefault?.()
    event?.stopPropagation?.()

    const targetUserId = String(
      data?.accountId ||
      resolveProfileAccountId(rawUserId) ||
      rawUserId ||
      '',
    ).trim()
    ...
    // Не держим UserInfoPopover под auth overlay.
    // Сам переход в DM ниже произойдёт только после подтверждённой авторизации.
    // Здесь presentation-exit намеренно пропускается: auth overlay должен получить экран сразу.
    motionSkipExitRef.current = true
    onClose?.()

    void runAuthorizedClientAction({
      actionKey: `forum-user-info-dm:${targetUserId}`,
      source: 'forum-user-info-dm',
      action: () => {
        notifySearchPopoverNavigation('dm')

        try {
          window.dispatchEvent(new CustomEvent('forum:head-hide-once'))
          window.dispatchEvent(new CustomEvent('inbox:open-dm', {
            detail: {
              userId: targetUserId,
              source: 'user-info-popover',
            },
          }))
        } catch {}

        onOpenDirectMessage?.({ userId: targetUserId })
      },
    })
  }, [
    data?.accountId,
    notifySearchPopoverNavigation,
    onClose,
    onOpenDirectMessage,
    rawUserId,
    registerAction,
  ...
  const handleMotionTransitionEnd = useCallback((event) => {
    if (event.target !== event.currentTarget) return
    if (event.propertyName !== 'transform') return
    if (!open && motionPhase === 'exit') finishMotionExit()
  }, [finishMotionExit, motionPhase, open])

  const hasLiveTarget = !!(open && anchorRef?.current && String(rawUserId || '').trim())
  const shouldRenderMotion = motionMounted || hasLiveTarget
  if (!shouldRenderMotion) return null

  const liveTargetId = String(rawUserId || '').trim()
  const visualTargetMatches = !open || (!!liveTargetId && liveTargetId === motionUserId)
  const visualMotionPhase = motionMounted ? motionPhase : 'pre-enter'
  const stats = data?.stats || {}
  const showTranslated = translatedBio && !showOriginal
  const displayBio = showTranslated ? translatedBio : data?.about || ''
  const bioTranslationSource = `${String(rawUserId || '')}\u0000${String(data?.about || '')}`
  const bioTranslationAnimationEnabled = !!data?.about && bioTranslationAnimation.src === bioTranslationSource

  const popover = (
    <div
      ref={popoverRef}
      className={presenceOnline ? 'userInfoPopover userInfoPopover--onlinePulse' : 'userInfoPopover'}
      data-ql7-visual-scope="popover"
      data-placement={position.placement}
      data-motion={visualMotionPhase}
      aria-hidden={visualMotionPhase === 'exit' ? 'true' : undefined}
      style={{
        top: position.top,
        left: position.left,
        visibility: position.ready && visualTargetMatches ? 'visible' : 'hidden',
        '--user-info-slide-x': `${position.slideX}px`,
        '--user-info-enter-ms': `${USER_INFO_ENTER_MS}ms`,
        '--user-info-exit-ms': `${USER_INFO_EXIT_MS}ms`,
      }}
    ...
    >
      {data?.officialStatus === true && (
        <span className="userInfoOfficialStatus" data-ql7-official-status="true">
          <img src="/status/official.png" alt="Official" draggable="false" />
          <span className="userInfoOfficialShine" aria-hidden="true" />
        </span>
      )}
      <div className="userInfoBioRow">
        <div className="userInfoBioHeader">
          <div className="userInfoBioLabelWrap">
            <div className="userInfoBioLabel">{t?.('forum_user_popover_bio')}:</div>
            <span className="userInfoBioLabelRail" aria-hidden="true" />
          </div>
        </div>
        <div className="userInfoBioActionsRow">
          ...
          <div className="userInfoTranslateSlot">
            <button
              type="button"
      ...
      {status === 'ready' && (
        <TypewriterText
          ...
          text={displayBio}
          animate={bioTranslationAnimationEnabled}
          animationKey={bioTranslationAnimation.key}
          renderHtml={safeRich}
        />
      )}
