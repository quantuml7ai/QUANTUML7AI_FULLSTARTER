...
import useVipFlag, { queueVipProbes } from '../../profile/hooks/useVipFlag'
...
function SubscriptionUserRow({ row, ownerUserId, copy, onOpenUserInfo }) {
  ...
  const isVip = useVipFlag(
    rowUserId,
    row?.vipActive ?? row?.isVip ?? row?.vip ?? row?.vipUntil ?? null,
  )
  ...
  return (
    <button
      ...
      onClick={(event) => onOpenUserInfo?.(rowUserId, event.currentTarget, {
        userId: rowUserId,
        nickname,
        icon: row?.icon || '',
        avatar: row?.icon || '',
        ...
        sourceId: String(ownerUserId || ''),
      })}
    >
      <span className="subsFamilyRowRail" aria-hidden="true" />
      <span className="subsFamilyAvatar avaMini ql7AvatarBadgeHost">
        {!useAnonymousAvatar ? (
          <AvatarEmoji userId={rowUserId} pIcon={row?.icon} className="ql7AvatarMediaClip" />
        ) : (
          <span
        ...
        <AvatarBadgeOverlay vipActive={isVip} showInfo />
      </span>
      <span className="subsFamilyRowMain">
        <span className="subsFamilyNickWrap">
          <span className={cls('nick-badge nick-animate')}>
            <ForumNickText className="nick-text" textValue={nickname}>{nickname}</ForumNickText>
          </span>
        </span>
      </span>
    </button>
...
export default function SubscriptionsPopover({
  ...
  const loadPage = useCallback(async ({ append = false, cursor = '' } = {}) => {
    ...
    try {
      ...
      const vipIds = pageUsers.map((user) => String(user?.userId || '').trim()).filter(Boolean)
      try {
        if (vipIds.length) queueVipProbes(vipIds)
      } catch {}
  ...
  }, [apiClient, debouncedQuery, mode, open, userId])
  ...
  const openUserInfo = useCallback((rowUserId, anchorEl, userPreview = null) => {
    const uid = String(rowUserId || '').trim()
    if (!uid || !anchorEl) return
    onOpenUserInfo?.(uid, anchorEl, userPreview)
  }, [onOpenUserInfo])
