...
import ForumNickText from '../../../shared/components/ForumNickText'
...
import {
  ...
  forumSearchProjectionOwner,
  forumThreadProjectionOwner,
  forumTopicRootsProjectionOwner,
} from '../../feed/utils/transientProjectionRetention'

const SEARCH_MODES = ['posts', 'topics', 'people']
const EMPTY_SEARCH_INDEX_SOURCE = Object.freeze([])
...
function dispatchServerItemsMerge(options = {}) {
  if (typeof window === 'undefined') return
  const hasPostsProjection = Array.isArray(options?.posts)
  const hasTopicsProjection = Array.isArray(options?.topics)
  const cleanPosts = hasPostsProjection ? options.posts.filter((item) => item && item.id) : []
  const cleanTopics = hasTopicsProjection ? options.topics.filter((item) => item && item.id) : []
  const projectionOwner = String(options?.projectionOwner || '').trim()
  const reset = options?.reset === true
  if (!cleanPosts.length && !cleanTopics.length && !(reset && projectionOwner && (hasPostsProjection || hasTopicsProjection))) return
  try {
    window.dispatchEvent(new CustomEvent('forum:server-items-merge', {
      detail: {
        ...(hasPostsProjection ? { posts: cleanPosts } : null),
        ...(hasTopicsProjection ? { topics: cleanTopics } : null),
        reason: options?.reason || 'search_navigation',
        rev: Date.now(),
        projectionOwner,
        projectionReset: reset,
      },
    }))
  } catch {}
}
...
function SearchMediaPreview({ media, t }) {
    ...
    return (
      <span className={cls('searchPortalMedia', kind === 'sticker' && 'searchPortalMedia--sticker')}>
        {/\.gif(?:$|[?#])/i.test(String(media.url || '')) ? (
          <ViewportAnimatedImage className="searchPortalMediaObject" animatedSrc={media.url} alt="" loading="lazy" decoding="async" referrerPolicy="no-referrer" />
        ) : (
  ...
  if (kind === 'video' && isDirectVideoUrl(media.url)) {
    return (
      <span className="searchPortalMedia searchPortalMedia--video">
        <video className="searchPortalMediaObject" src={media.url} poster={media.thumb || undefined} data-search-preview-video="true" muted loop playsInline preload="metadata" aria-label={label} />
        <span className="searchPortalMediaBadge">{label}</span>
      </span>
    )
  }
...
function AuthorIdentity({ entity, avatarRef = null, size = 'regular', className = '', showInfo = false }) {
  ...
  return (
    <span className={cls('searchPortalAuthor ql7AvatarBadgedIdentity', `searchPortalAuthor--${size}`, className)}>
      <span ref={avatarRef} className="avaMini searchPortalAuthorAvatar ql7AvatarBadgeHost" aria-hidden="true">
        <AvatarEmoji
          userId={authorId || rawUserId}
          pIcon={icon}
          className="ql7AvatarMediaClip"
        />
        <AvatarBadgeOverlay vipActive={isVipAuthor} showInfo={showInfo} />
      </span>
      <span className={cls('nick-badge nick-animate searchPortalNickBadge')} translate="no">
        <ForumNickText className="nick-text truncate" textValue={nickname || shortId(authorId || rawUserId)}>{nickname || shortId(authorId || rawUserId)}</ForumNickText>
      </span>
    </span>
  )
}
...
function UserResultCard({ user, onOpen }) {
  ...
  return (
    ...
    >
      <AuthorIdentity entity={user} avatarRef={avatarRef} size="regular" showInfo />
    </button>
  )
}
...
export default function ForumSearchSortControls({
  ...
  const searchProjectionOwner = forumSearchProjectionOwner(query)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        dispatchServerItemsMerge({
          ...
          projectionOwner: searchProjectionOwner,
          reset: true,
        })
        setServerSearch({
          query: queryText,
    ...
    return () => {
      alive = false
      window.clearTimeout(timer)
      dispatchForumTransientProjectionRelease(searchProjectionOwner)
    }
  }, [drop, query, searchProjectionOwner])

  const searchIndexesActive = !!drop && !!query
  const searchTopicsSource = useMemo(
    () => (searchIndexesActive ? (data?.topics || EMPTY_SEARCH_INDEX_SOURCE) : EMPTY_SEARCH_INDEX_SOURCE),
    [data?.topics, searchIndexesActive],
  )
  const searchPostsSource = useMemo(
    () => (searchIndexesActive ? (data?.posts || EMPTY_SEARCH_INDEX_SOURCE) : EMPTY_SEARCH_INDEX_SOURCE),
    [data?.posts, searchIndexesActive],
  )
  const topicsById = useMemo(
    () => new Map((searchTopicsSource || []).map((topic) => [String(topic?.id || ''), topic])),
    [searchTopicsSource],
  )
  const postsById = useMemo(
    () => new Map((searchPostsSource || []).map((post) => [String(post?.id || ''), post])),
    [searchPostsSource],
  )
  const postCountByTopic = useMemo(() => {
    const map = new Map()
    for (const post of searchPostsSource || []) {
  ...
  const searchModel = useMemo(() => {
    if (!searchIndexesActive) return { posts: [], topics: [], people: [] }
    ...
    if (!topicIds.size) {
      for (const topic of searchTopicsSource || []) {
  ...
  }, [postsById, query, results, searchIndexesActive, searchPostsSource, searchTopicsSource, serverSearch, topicsById])
  ...
  const openTopic = useCallback((topic) => {
    ...
    dispatchServerItemsMerge({
      topics: [topic],
      reason: 'search_topic_navigation',
      projectionOwner: forumTopicRootsProjectionOwner(topic.id),
      reset: true,
    })
    ...
    try {
      ...
      if (locate?.ok) {
        ...
        if (topic) {
          dispatchServerItemsMerge({
            topics: [topic],
            reason: 'search_post_topic_navigation',
            projectionOwner: forumTopicRootsProjectionOwner(topicId),
            reset: true,
          })
        }
        dispatchServerItemsMerge({
          ...
          projectionOwner: forumThreadProjectionOwner(topicId, postId),
          reset: true,
        })
      }
    } catch {}
