...
export default function ComposerEmojiPanel({
  ...
  return (
    <div
      ...
      data-ql7-visual-surface={emojiTab === 'stickers' ? 'vip-stickers' : 'emoji-picker'}
      style={{
        maxHeight: 250,
        overflowY: 'auto',
        ...
        <div key={emojiTab} className="emojiPanelMode" data-emoji-mode={emojiTab}>
          {emojiTab === 'stickers' ? (
            ...
            <div className="emojiGrid">
              {VIP_EMOJI.map((emojiItem, emojiIndex) => (
                <button
                  key={emojiItem}
                  type="button"
                ...
                >
                  {typeof emojiItem === 'string' && emojiItem.startsWith('/')
                    ? <ViewportAnimatedImage animatedSrc={emojiItem} alt="" className="vipEmojiIcon" width={64} height={64} initialPoster={emojiIndex >= DENSE_INITIAL_WARM_COUNT} marginProfile="near50" loading="lazy" decoding="async" />
