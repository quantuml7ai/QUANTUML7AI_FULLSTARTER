...
import Ql7SupportComposerRuntimePanel from '../../dm/components/Ql7SupportComposerRuntimePanel'
import { isQl7SupportEmergencyText, normalizeQl7SupportInputPolicy } from '../../../../../lib/ql7-support/inputPolicy.js'
...
export default function ComposerActionRail({
  ...
  const [policyNow, setPolicyNow] = React.useState(() => Date.now())
  const rawSupportPolicy = dmSupportMode && ql7SupportUi?.inputPolicy && typeof ql7SupportUi.inputPolicy === 'object'
    ? ql7SupportUi.inputPolicy
    : null
  const supportPolicy = rawSupportPolicy
    ? normalizeQl7SupportInputPolicy(rawSupportPolicy, { now: () => policyNow, locale: rawSupportPolicy?.locale || 'en' })
    : null
  const readyAtMs = Number(
    supportPolicy?.blockedUntilMs ||
    Date.parse(supportPolicy?.blockedUntil || '') ||
    supportPolicy?.readyAtMs ||
    Date.parse(supportPolicy?.readyAt || '') ||
    Date.parse(supportPolicy?.expiresAt || '') ||
    0
  )
  const emergencyText = isQl7SupportEmergencyText(text)
  const supportPolicyBlocked = Boolean(supportPolicy && supportPolicy.allowed === false && readyAtMs > policyNow && !(supportPolicy.emergencyOverride !== false && emergencyText))
  const supportPolicySeconds = supportPolicyBlocked ? Math.max(1, Math.ceil((readyAtMs - policyNow) / 1000)) : 0
  React.useEffect(() => {
    if (!rawSupportPolicy || (rawSupportPolicy?.canSend !== false && rawSupportPolicy?.allowed !== false) || readyAtMs <= 0) return undefined
    const timer = window.setInterval(() => setPolicyNow(Date.now()), 250)
    return () => window.clearInterval(timer)
  }, [rawSupportPolicy, readyAtMs])
  React.useEffect(() => { setPolicyNow(Date.now()) }, [readyAtMs, supportPolicy?.canSend])
  const sendDisabled =
    !!postingRef?.current ||
    cooldownLeft > 0 ||
  ...
  if (dmSupportMode) {
    return (
      <Ql7SupportComposerRuntimePanel
        textLen={textLen}
        textLimit={textLimit}
        policy={supportPolicy}
        blocked={supportPolicyBlocked}
        seconds={supportPolicySeconds}
  ...
  return (
    <div className="topRail" role="toolbar" aria-label={t('forum_composer_actions')}>
      <div className="railInner">
        ...
        <div className="railItem voiceRailItem">
          <button
            type="button"
