...
import useComposerSafetyPreview from '../hooks/useComposerSafetyPreview.js'
...
export default function ComposerCore({
  ...
  const pendingVideoPosterBlob = React.useMemo(() => {
    const src = String(pendingVideo || '').trim()
    if (!src) return null
    let meta = null
    try { meta = pendingVideoBlobMetaRef?.current?.get?.(src) || null } catch {}
    try { if (!meta) meta = pendingVideoInfoRef?.current || null } catch {}
    return meta?.posterBlob instanceof Blob ? meta.posterBlob : null
  }, [pendingVideo, pendingVideoBlobMetaRef, pendingVideoInfoRef])

  return (
    <>
      <div className="forumComposer" data-voice-recording={recState === 'rec' ? '1' : undefined}>
        {!dmSupportMode ? (
        ...
        <div className="taWrap" data-active={composerActive}>
          <ComposerMediaProgressBar
            ...
            pipelineKind={pendingVideo || ['processing', 'verifying', 'prepared'].includes(String(mediaPhase || '').toLowerCase())
              ? 'video'
              : (pendingAudio ? 'audio' : 'media')}
          />
          <ComposerActionRail
            text={text}
            textLimit={textLimit}
        ...
        <ComposerAttachmentPreview
          ...
          pendingVideoPosterBlob={pendingVideoPosterBlob}
