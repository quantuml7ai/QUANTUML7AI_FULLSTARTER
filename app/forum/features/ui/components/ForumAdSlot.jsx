...
import { commandExternalVideo } from '../../media/utils/externalVideoBridge'
...
function activateAdFrame(frame, kind) {
  ...
  try {
    ...
    frame.dataset.__userGestureIntentUntil = String(now + 5200)
    frame.dataset.__audibleGestureUntil = String(now + 5200)
    frame.setAttribute('data-forum-iframe-active', '1')
  } catch {}
  try {
    if (commandExternalVideo(frame, 'play', { muted: false })) return
  } catch {}
  if (kind === 'youtube') {
