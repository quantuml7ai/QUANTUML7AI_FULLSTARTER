...
export default function ForumOverlayStack({
  ...
  const overlayPreviewPosterBlob = React.useMemo(() => {
    if (overlayMediaKind === 'image') return null
    const src = String(overlayPreviewUrl || '').trim()
    let meta = null
    try { meta = src ? pendingVideoBlobMetaRef?.current?.get?.(src) || null : null } catch {}
  ...
  return (
    <>
      ...
      <VideoOverlay
        ...
        previewPosterBlob={overlayPreviewPosterBlob}
