...
export default function QCoinInline({ t, userKey, vipActive, anchorRef }) {
  ...
  const numericBalance = Number(q.balanceDisplay ?? q.balance ?? 0)
  const hasAuthIdentity = !!userKey || numericBalance > 0
  const clsVal = hasAuthIdentity ? 'qcoinValue live' : 'qcoinValue paused'
  const liveVisualStyle = hasAuthIdentity ? { color: '#ffd700', animation: 'none' } : null
  ...
  return (
    <div className="qcoinRow qcoinCol" translate="no">
      <div className="qcoinTop">
        <span className="qcoinLabel ql7QCoinGoldLabel">Q COIN</span>
        <span
      ...
      <span
        ...
        role="button"
        tabIndex={0}
        style={{ cursor: 'pointer', ...(liveVisualStyle || {}) }}
        title={walletTitle}
