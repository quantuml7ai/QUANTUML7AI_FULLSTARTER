  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const checkedAt = Number(response?.checkedAt || Date.now()) || Date.now()
        const confirmedMap = {}
        for (const id of list) {
          const vip = map?.[id] || null
          if (!vip || vip?.available === false) continue
          const vipUntil = Number(vip?.untilMs || 0) || 0
          const vipActive = vipUntil > 0 ? vipUntil > Date.now() : !!vip?.active
          const vipCheckedAt = Number(vip?.checkedAt || checkedAt) || checkedAt
          try { mergeProfileCacheFn(id, { vipActive, isVip: vipActive, vipUntil, vipCheckedAt }) } catch {}
          confirmedMap[id] = { ...vip, available: true, active: vipActive, untilMs: vipUntil, checkedAt: vipCheckedAt }
        }
