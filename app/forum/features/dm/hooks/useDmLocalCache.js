...
import { dedupeDmDialogs, dedupeDmThreadMessagesExact } from '../utils/dmLoaders.js'

const DM_LS_DIALOGS_MAX = 150
const DM_LS_THREAD_MAX = 150


function dmThreadNamespace(uid) {
  return String(uid || '').trim() === 'ql7-support' ? 'system-support' : 'peer-dm'
}

function dmThreadStorageKey(meId, uid) {
  return `dm:thread:${dmThreadNamespace(uid)}:${meId}:${uid}`
}
...
export default function useDmLocalCache({
  ...
  useLayoutEffect(() => {
    ...
    try {
      ...
      const cacheKey = dmThreadStorageKey(meId, uid)
      const raw = localStorage.getItem(cacheKey)
      const parsed = raw ? JSON.parse(raw) : null
      const items = dedupeDmThreadMessagesExact(Array.isArray(parsed?.items) ? parsed.items : [])
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (let i = localStorage.length - 1; i >= 0; i -= 1) {
        ...
        if (key.startsWith(prefix) && key.includes(`:${meId}:`)) localStorage.removeItem(key)
      }
    } catch {}
  ...
  useEffect(() => {
    ...
    try {
      dmThreadMemRef.current.set(uid, {
        items: dedupeDmThreadMessagesExact(Array.isArray(dmThreadItems) ? dmThreadItems : []),
  ...
  useEffect(() => {
    ...
    dmThreadPersistTimerRef.current = setTimeout(() => {
      try {
        if (String(dmActiveThreadUidRef.current || '').trim() !== uid) return
        const cacheKey = dmThreadStorageKey(meId, uid)
        const itemsAll = dedupeDmThreadMessagesExact(Array.isArray(dmThreadItems) ? dmThreadItems : [])
        const items = itemsAll.length > DM_LS_THREAD_MAX
          ? itemsAll.slice(itemsAll.length - DM_LS_THREAD_MAX)
          : itemsAll
        localStorage.setItem(cacheKey, JSON.stringify({
          ts: Date.now(),
          cacheKey: `${dmThreadNamespace(uid)}:${meId}:${uid}`,
          cursor: dmThreadCursor || null,
          hasMore: !!dmThreadHasMore,
          peerSeenTs: Number(dmThreadSeenTs || 0),
          items,
