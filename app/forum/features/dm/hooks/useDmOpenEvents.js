...
export default function useDmOpenEvents({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const deepLinkUser = String(deepLink.searchParams.get('dmUser') || '').trim()
      const deepLinkInbox = String(deepLink.searchParams.get('inbox') || '').trim()
      if (deepLinkUser && deepLinkInbox === 'messages') {
        onOpenDm({ detail: { userId: deepLinkUser, source: 'admin-email' } })
        for (const key of ['dmUser', 'inbox', 'supportCase', 'supportMessage', 'supportTopic', 'correlation']) deepLink.searchParams.delete(key)
        const cleanHref = `${deepLink.pathname}${deepLink.search}${deepLink.hash}`
