...
import {
  ...
  forumInboxRepliesProjectionOwner,
} from '../../feed/utils/transientProjectionRetention.js'
...
export default function useForumDmRuntime({
  ...
  const dmThreadHasMoreRef = useRef(true)
  ...
  useEffect(() => { dmThreadHasMoreRef.current = !!dmThreadHasMore }, [dmThreadHasMore])
  // DM history belongs to one peer generation. Switching peer starts a
  // fresh thread session before useDmLoadLifecycle performs its page-one read.
  useEffect(() => {
    const previousOwner = dmThreadOwnerRef.current
    const nextOwner = advanceDmThreadOwner(previousOwner, ownerInputFor(dmWithUserId))
    dmThreadOwnerRef.current = nextOwner
    if (Number(nextOwner?.generation || 0) === Number(previousOwner?.generation || 0)) return
  ...
  // dmThreadLoadingRef stores peer+generation ownership; UI loading boolean is intentionally separate.
  ...
  const {
    ...
    posts: dataPosts,
    tombstones,
    presentationActive: inboxOpen && inboxTab === 'replies',
    postSort,
    visibleRepliesCount,
    resolveProfileAccountIdFn,
  })
  ...
  const serverRepliesRequestSeqRef = useRef(0)

  const inboxRepliesProjectionOwner = forumInboxRepliesProjectionOwner(meId, postSort || 'new')

  const emitServerRepliesMerge = useCallback((posts, rev = 0, { reset = false } = {}) => {
    if (typeof window === 'undefined') return
    ...
    window.dispatchEvent(new CustomEvent('forum:server-items-merge', {
      detail: {
        ...
        projectionOwner: inboxRepliesProjectionOwner,
        projectionReset: reset === true,
      },
    }))
  }, [inboxRepliesProjectionOwner])
  ...
  const loadInboxRepliesPage = useCallback(async ({ reset = false, reason = 'manual' } = {}) => {
    ...
    const seq = serverRepliesRequestSeqRef.current + 1
    serverRepliesRequestSeqRef.current = seq
    if (reset) dispatchForumTransientProjectionRelease(inboxRepliesProjectionOwner)
    ...
    try {
      ...
      if (serverRepliesRequestSeqRef.current !== seq) return { ok: false, skipped: true, reason: 'stale' }
      ...
      emitServerRepliesMerge(rows, response.rev, { reset })
      ...
      if (serverRepliesRequestSeqRef.current === seq) {
  ...
  useEffect(() => {
    ...
    if (serverRepliesKeyRef.current !== key) {
      ...
      serverRepliesAutoAttemptKeyRef.current = key
  ...
  useEffect(() => {
    const activeRepliesSurface = !!inboxOpen && inboxTab === 'replies' && !!meId
    if (activeRepliesSurface) return undefined

    serverRepliesKeyRef.current = ''
    serverRepliesAutoAttemptKeyRef.current = ''
    serverRepliesRequestSeqRef.current += 1
    serverRepliesInFlightRef.current = false
    setServerRepliesToMe([])
    setServerRepliesCursor(null)
    setServerRepliesHasMore(false)
    setServerRepliesLoading(false)
    setServerRepliesLoaded(false)
    dispatchForumTransientProjectionRelease(inboxRepliesProjectionOwner)
    return undefined
  }, [inboxOpen, inboxRepliesProjectionOwner, inboxTab, meId])

  useEffect(() => () => {
    // Invalidate an in-flight response before releasing this owner; otherwise a
    // stale sort/user request could repopulate the just-released projection.
    serverRepliesRequestSeqRef.current += 1
    serverRepliesInFlightRef.current = false
    dispatchForumTransientProjectionRelease(inboxRepliesProjectionOwner)
  }, [inboxRepliesProjectionOwner])
  ...
  useEffect(() => {
    if (!inboxOpen || inboxTab !== 'published') return
    // Published is fresh-on-entry and monotonic within that surface session.
    // Projection growth from a successful next page must never collapse it back.
    setVisiblePublishedCount(publishedPageSize)
  }, [inboxOpen, inboxTab, meId, postSort, setVisiblePublishedCount, publishedPageSize])
  ...
  const reopenDeletedDmDialog = useCallback((uid, rawUid = '') => {
    ...
    setDmThreadItems([])
    setDmThreadCursor(null)
    dmThreadHasMoreRef.current = false
  ...
  const fetchDmThreadRealtime = useCallback(async (withUserId, reason = 'dm-realtime-thread') => {
    ...
    const run = async () => {
      ...
      // Realtime/latest reads reconcile only the newest window. The older-history
      // cursor belongs exclusively to page-one session bootstrap + older paging.
      setDmThreadSeenTs(Number(payload.peerSeenTs || 0))
      if (supportRequest) {
  ...
  useEffect(() => {
    ...
    // useDmLoadLifecycle owns the immediate normal-peer read. Keep only the two
    // convergence retries here; Support retains its exact direct t=0 behavior.
    const openDelays = isQl7SupportPeerId(uid) ? [0] : [250, 900]
  ...
  }, [mounted, meId, runDmRealtimeRefresh, applyDmRealtimeDeleteImpulse])
  ...
  const {
    ...
    active: inboxOpen && inboxTab === 'published',
    meId,
    posts: dataPosts,
    tombstones,
    postSort,
