...
const EMPTY_INBOX_REPLY_COUNT_SOURCE = Object.freeze([])
...
export default function useInboxRepliesModel({
  ...
  const replyCountSource = useMemo(
    () => (presentationActive && meId ? (posts || EMPTY_INBOX_REPLY_COUNT_SOURCE) : EMPTY_INBOX_REPLY_COUNT_SOURCE),
    [meId, posts, presentationActive],
  )
  const repliesById = useMemo(() => {
    const map = new Map()
    for (const post of replyCountSource) {
  ...
  const sortedRepliesToMe = useMemo(
    () => {
      // Preserve repliesToMe while hidden because unread badges consume it, but
      // defer reply-counter aggregation and presentation sorting until visible.
      if (!presentationActive) return repliesToMe
      const score = (post) => {
    ...
    [postSort, presentationActive, repliesById, repliesToMe],
  )
