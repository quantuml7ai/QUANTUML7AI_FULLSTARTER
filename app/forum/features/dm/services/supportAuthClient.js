...
export async function translateQl7SupportCard({ deliveryReceiptId = '', targetLocale = '', signal = null } = {}) {
  const sourceReceiptId = str(deliveryReceiptId)
  if (!sourceReceiptId) return null
  const targetLang = String(targetLocale || 'en').split(/[-_]/)[0] || 'en'
  const clientMutationId = `support-card-translate:${sourceReceiptId}:${targetLang}`.slice(0, 160)
  const result = await fetchQl7SupportAuthenticated('/api/dm/support-card-translate', {
    method: 'POST',
    cache: 'no-store',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ deliveryReceiptId: sourceReceiptId, targetLang, clientMutationId }),
    signal,
  }, { signal, waitTimeoutMs: 12000, retryOnFreshAuth: true })
