...
export async function sendDmComposerMessage({
  ...
  const buildAttachments = ({ imageUrls = pendingImgs, audioUrl = '', videoUrl = '', videoMeta = null } = {}) => {
    ...
    return [
      ...
      ...(vv ? [{ url: vv, type: 'video', moderationStatus: 'pending', ...(safeVideoMeta || {}) }] : []),
    ].filter(Boolean)
  }
  ...
  let videoModerationSource = optimisticVideoUrl || null
  ...
  try {
    ...
    if (needsResolve) {
      ...
      videoModerationSource = media?.videoModerationSource || finalVideoUrl || videoModerationSource
    ...
    if (finalVideoUrl && !videoModerationSource) videoModerationSource = finalVideoUrl
    ...
    const supportStateTimers = new Set()
    const supportSeenEvents = new Set()
    let supportStateBusy = false
    let supportStateStopped = false
    let supportPollInterval = 0
    let supportReplayOffsetMs = 0
    let supportCommittedSeen = false
    const stopSupportReplay = () => {
      supportStateStopped = true
      for (const timer of supportStateTimers) {
        try { window.clearTimeout(timer) } catch {}
      }
      supportStateTimers.clear()
    }
    const emitRuntimeEvent = (event = {}) => {
      const runtimeState = String(event?.state || 'idle')
      ...
      emitSupportUi(runtimeState, {
        ...
        correlationId: String(event?.correlationId || clientMutationId),
        detailCode: String(event?.detailCode || ''),
        finalMessageId: String(event?.finalMessageId || ''),
        changedAt: Date.parse(event?.changedAt || '') || Date.now(),
    ...
    const queueRuntimeHistory = (state = {}) => {
      ...
      for (const event of history) {
        ...
        const delay = supportReplayOffsetMs
        supportReplayOffsetMs += 140
        queued += 1
        try {
          const timer = window.setTimeout(() => {
            supportStateTimers.delete(timer)
            if (!supportStateStopped) emitRuntimeEvent(event)
          }, delay)
          supportStateTimers.add(timer)
        } catch {}
      }
      if (!queued && state?.state) emitRuntimeEvent(state)
      supportReplayOffsetMs = Math.min(supportReplayOffsetMs, 1120)
    }
    ...
    if (!resp?.ok || !j?.ok) {
      ...
      if (j?.supportBroadcast) {
        ...
        if (!dmSupportMode && finalVideoUrl && typeof moderateVideoSource === 'function') {
          const moderationEntityId = realId
          const moderationActorId = uid
          const moderationSource = videoModerationSource || finalVideoUrl
          void (async () => {
            const result = await moderateVideoSource(moderationSource, {
              surface: 'dm',
              entityId: moderationEntityId,
              actorId: moderationActorId,
              mediaUrl: finalVideoUrl,
              fallbackSource: finalVideoUrl,
              clientRequestId: `dm-video-${clientMutationId}`,
            })
            ...
            if (status === 'approved') {
              ...
              setDmThreadItems((prev) => (prev || []).map((m) =>
                String(m?.id || '') === String(moderationEntityId)
                  ? { ...m, attachments: approveAttachments(m?.attachments) }
                  : m
              ))
              setDmDialogs((prev) => dedupeDmDialogs((prev || []).map((d) => {
                const last = d?.lastMessage || null
                if (!last || String(last?.id || '') !== String(moderationEntityId)) return d
                return { ...d, lastMessage: { ...last, attachments: approveAttachments(last?.attachments) } }
              }), uid))
              return
            }
            if (status === 'rejected' || status === 'gone') {
              setDmThreadItems((prev) => (prev || []).filter((m) => String(m?.id || '') !== String(moderationEntityId)))
              setDmDialogs((prev) => dedupeDmDialogs((prev || []).filter((d) => String(d?.lastMessage?.id || '') !== String(moderationEntityId)), uid))
              try { dmDialogsCacheRef.current.clear() } catch {}
              try { dmThreadCacheRef.current.clear() } catch {}
              try { loadDmDialogs(null, { force: true, refresh: true, throttleMs: 0 }) } catch {}
              try { loadDmThread(dmTarget, null, { force: true, refresh: true, throttleMs: 0 }) } catch {}
              if (status === 'rejected') {
