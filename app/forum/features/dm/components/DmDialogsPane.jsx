...
export default function DmDialogsPane({
  ...
  const visibleDialogs = supportActive ? (dmDialogs || []) : (dmDialogs || []).filter((dialog) => String(dialog?.userId || '').trim() !== 'ql7-support')
  ...
  return (
    <>
      {supportActive && <div className="dmSupportLauncher" aria-label={supportHint}>
        <button
          type="button"
      ...
      {supportActive && <div className="dmSupportLauncherRail" aria-hidden="true" />}
