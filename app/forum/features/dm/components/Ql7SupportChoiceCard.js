...
import { buildQl7SupportAuthHeaders, buildQl7SupportRouteContext, fetchQl7SupportAuthenticated, readQl7SupportAuthSnapshot } from '../services/supportAuthClient'
import { QL7_SUPPORT_ID } from '../../../../../lib/ql7-support/systemActor'
import { countQl7SupportGraphemesV11, QL7_SUPPORT_USER_INPUT_MAX_GRAPHEMES_V11 } from '../../../../../lib/ql7-support/limitsV11'
import { dedupeQl7SupportChoicesV12 } from '../../../../../lib/ql7-support/v12/choiceDiversityV12.js'
...
export default function Ql7SupportChoiceCard({ card }) {
  ...
  const otherCount = countQl7SupportGraphemesV11(otherText, s(card?.locale) || 'en')
  const otherValid = otherCount >= 1 && otherCount <= QL7_SUPPORT_USER_INPUT_MAX_GRAPHEMES_V11

  const send = React.useCallback(async (option, text) => {
    const clean = s(text)
    if (!clean || busy) return
    if (countQl7SupportGraphemesV11(clean, s(card?.locale) || 'en') > QL7_SUPPORT_USER_INPUT_MAX_GRAPHEMES_V11) {
      setError(s(card?.labels?.tooLong) || `Maximum ${QL7_SUPPORT_USER_INPUT_MAX_GRAPHEMES_V11} characters.`)
      return
    }
  ...
  const options = dedupeQl7SupportChoicesV12(Array.isArray(card?.options) ? card.options : []).filter((option) => s(option?.id) && s(option?.label)).slice(0, 4)
  ...
  return h('div', { className: 'ql7SupportChoicePanel', role: 'group', 'aria-label': s(card?.title) || 'Choose the closest option', 'data-ql7-support-choice-panel': '1', 'data-ql7-support-choice-layout': 'stacked-full-width', 'data-ql7-support-choice-rail': choiceCount === 5 ? 'five-full-width' : 'compact', 'data-ql7-support-choice-count': String(choiceCount) },
    ...
    other ? h('div', { className: 'ql7SupportChoiceOther' },
      ...
      otherOpen ? h('div', { className: 'ql7SupportChoiceOtherEditor' },
        ...
        h('div', { id: 'ql7-support-other-count', className: `ql7SupportChoiceCounter${otherCount > 600 ? ' ql7SupportChoiceCounter--error' : ''}` }, `${otherCount}/${QL7_SUPPORT_USER_INPUT_MAX_GRAPHEMES_V11}`),
