...
import ViewportAnimatedImage from '../../../../../components/visual-runtime/ViewportAnimatedImage.jsx'
...
const VIDEO_STYLE = Object.freeze({ minHeight: 315, maxHeight: 'min(64.8vh, 585px)', background: 'transparent' })
...
function isAnimatedRasterUrl(value) {
  return /(?:\.gif|[-_]animated(?:[-_][^/?#]+)?\.(?:webp|avif|png))(?:$|[?#])/i.test(String(value || ''))
}
...
export default function DmMediaRenderer({
  ...
  const stickerGrid = mediaGrid(source, safeStickers.map((item, index) => {
    ...
    return h('div', {
      ...
      'data-kind': 'sticker',
    }, h(isAnimatedRasterUrl(src) ? ViewportAnimatedImage : Image, isAnimatedRasterUrl(src) ? {
      animatedSrc: src,
      alt: '',
      width: 512,
      height: 512,
  ...
  const imageGrid = mediaGrid(source, safeImages.map((item, index) => {
    ...
    return h('figure', {
      ...
      'data-ambient-media-backdrop': 'dm-image',
    }), h(isAnimatedRasterUrl(src) ? ViewportAnimatedImage : Image, isAnimatedRasterUrl(src) ? {
      animatedSrc: src,
      alt: str(item?.alt),
      width: 1200,
  ...
  const videoGrid = mediaGrid(source, safeVideos.map((item, index) => {
    ...
    const moderationStatus = str(item?.moderationStatus).toLowerCase()
    return h('div', {
      key: `${keyPrefix}:vid:${index}:${src}`,
      className: VIDEO_SHELL_CLASS,
      'data-kind': 'video',
      'data-video-moderation-status': moderationStatus || undefined,
    }, moderationStatus === 'pending'
      ? h('span', { className: 'ql7VideoModerationBadge', 'aria-label': 'moderation' }, 'moderation')
      : null, h(VideoPlayer, {
      src,
      poster: str(item?.posterUrl || item?.poster) || undefined,
