...
import DmThreadHeader from './DmThreadHeader.jsx'
import Ql7SupportOperator from './Ql7SupportOperator.jsx'
import DmThreadAlerts from './DmThreadAlerts.jsx'
import DmThreadLoadMore from './DmThreadLoadMore.jsx'
import DmThreadMessageRow from './DmThreadMessageRow.jsx'
import DmDialogsPane from './DmDialogsPane.jsx'
...
export default function DmMessagesPane({
  ...
  if (dmWithUserId) {
    const isSupportThread = isQl7SupportId(String(dmWithUserId || ''))
    ...
    const railNode = <div className="dmThreadHeaderRail" aria-hidden="true" />
    ...
    if (isSupportThread) {
      return (
        <div className="dmSupportThreadStage" data-ql7-operator-sticky-stage="1">
          <div className="dmSupportThreadAlertsSlot">{alertsNode}</div>
          {headerNode}
          <div
            className="ql7SupportOperatorStickySlot"
            data-ql7-operator-sticky-slot="media-only"
            data-ql7-operator-sticky-target="static-video-only"
            data-ql7-operator-native-slot="identity-plate-right"
            data-ql7-operator-native-fit="identity-plate-expands-to-media"
          >
            <Ql7SupportOperator
              state={ql7SupportUi?.supportUiState || 'idle'}
              t={t}
              caseId={ql7SupportUi?.caseId}
              correlationId={ql7SupportUi?.correlationId}
              onDeliveredAnimationEnd={onSupportOperatorDelivered}
            />
          </div>
          {railNode}
          {threadNode}
