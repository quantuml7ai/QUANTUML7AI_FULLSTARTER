...
import { shortId } from '../../../shared/utils/formatters.js'
import {
  isQl7SupportId,
...
export default function DmThreadHeader({
  ...
  return (
    <div
      className="dmThreadHeader"
      data-ql7-visual-scope="header"
      data-support-thread={isSupportThread ? '1' : '0'}
      data-ql7-support-identity-sticky={isSupportThread ? '0' : undefined}
    >
      <div className="dmThreadAvatar ql7AvatarBadgeHost" onClick={openProfile}>
        <AvatarEmoji
          userId={threadUid}
          pIcon={resolveIconForDisplay(threadUid, '')}
          className="dmThreadAvatarImg ql7AvatarMediaClip"
        />
        <AvatarBadgeOverlay
          vipActive={isVipAuthor && !isSupportThread}
          showInfo={!!threadUid}
        />
      </div>
      <div className="dmThreadMeta">
        <div className="dmThreadUser">
          <button
            type="button"
            className={cls('nick-badge nick-animate dmThreadNick')}
            translate="no"
            onClick={openProfile}
            aria-label={isSupportThread ? (t?.('ql7_support_verified') || nick) : undefined}
          >
            <ForumNickText className="nick-text" textValue={nick || shortId(threadUid)}><bdi>{nick || shortId(threadUid)}</bdi></ForumNickText>
          </button>
