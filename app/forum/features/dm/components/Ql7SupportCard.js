...
import { adaptQl7SupportCardForRender } from '../../../../../lib/ql7-support/presentation.js'
import { localizeQl7SupportBadgeRailLabel } from '../../../../../lib/ql7-support/semanticBadgeRegistry.js'
import { getQl7SupportActionDescriptor, getQl7SupportRouteNavigationHref } from '../../../../../lib/ql7-support/topicActionRegistry.js'
import { buildQl7SupportAuthHeaders, fetchQl7SupportAuthenticated, readQl7SupportAuthSnapshot } from '../services/supportAuthClient.js'
...
function actionHref(action = {}) {
  ...
  const routeHref = getQl7SupportRouteNavigationHref(routeId)
...
function resolvedAction(action = {}) {
  const descriptor = getQl7SupportActionDescriptor(str(action?.routeId)) || {}
...
function BadgeRail({ items = [], locale = 'en' }) {
  ...
  for (const badge of items) {
    const label = localizeQl7SupportBadgeRailLabel(badge, locale)
...
function MatrixDataTable({ columns = [], rows = [], label = 'Details', locale = 'en' }) {
  const safeColumns = columns
    .filter((column) => str(column?.key) && str(column?.label))
    .slice(0, 8)
  const safeRows = rows
    .filter((row) => row && typeof row === 'object' && safeColumns.some((column) => hasValue(row[column.key])))
    .slice(0, 16)
  if (safeColumns.length < 2 || !safeRows.length) return null
  const template = `minmax(150px,1.35fr) repeat(${safeColumns.length - 1},minmax(92px,.8fr))`
  const rowStyle = { gridTemplateColumns: template }
  const minWidth = `${Math.max(520, 150 + (safeColumns.length - 1) * 104)}px`
  return h('div', { className: 'ql7SupportMatrixScroll' },
    h('div', { className: 'ql7SupportMatrixTable ql7SupportPremiumTable', role: 'table', 'aria-label': label, style: { minWidth } },
      h('div', { className: 'ql7SupportMatrixRow ql7SupportMatrixHeader', role: 'row', style: rowStyle }, safeColumns.map((column) =>
        h('span', { key: column.key, className: 'ql7SupportMatrixCell', role: 'columnheader' }, str(column.label)))),
      safeRows.map((row, rowIndex) => h('div', { key: row.key || `matrix-${rowIndex}`, className: 'ql7SupportMatrixRow', role: 'row', style: rowStyle }, safeColumns.map((column) =>
        h('span', { key: column.key, className: 'ql7SupportMatrixCell', role: 'cell', 'data-label': str(column.label) }, hasValue(row[column.key]) ? renderValue(row[column.key], locale, column.format) : '—')))),
    ),
  )
}
...
function TableSection({ table, labels = {}, locale = 'en' }) {
  ...
  const matrixColumns = columns.filter((column) => !['label', 'value'].includes(str(column?.key)))
  const matrix = matrixColumns.length >= 2 && rows.some((row) => matrixColumns.some((column) => hasValue(row?.[column.key])))
  if (matrix) {
    return h('section', { className: 'ql7SupportCardSection ql7SupportCardTableSection' },
      ...
      h(MatrixDataTable, { columns: matrixColumns, rows, label: str(table.title || labels?.details) || 'Details', locale }),
    )
  }
...
export function collectQl7SupportCardTextForTranslate(card) {
  ...
  try {
    const normalized = adaptQl7SupportCardForRender(card)
...
export default function Ql7SupportCard({ card, VideoPlayer, VoicePlayer }) {
  ...
  let normalized
  try { normalized = adaptQl7SupportCardForRender(card) } catch { return null }
