...
export default function DmThreadMessageRow({
  ...
  const [dmTranslationAnimation, setDmTranslationAnimation] = React.useState({ src: '', key: 0 })
  ...
  const dmTranslationAnimationEnabled = !supportCard && dmTranslationAnimation.src === dmTranslateSource
  ...
  const onDmTranslateToggle = async (e) => {
    ...
    if (dmTrState?.isTranslated) {
      if (!supportCard) {
        setDmTranslationAnimation((prev) => ({ src: dmTranslateSource, key: prev.key + 1 }))
      }
        ...
        {dmShowRawText && (
          <div className="dmTextFrame">
            {!!dmShowRawText && (
              <TypewriterText
                as="div"
                className="dmTextContent"
                text={dmDisplayText}
                animate={dmTranslationAnimationEnabled}
                animationKey={dmTranslationAnimation.key}
                renderHtml={safeHtml}
              />
            )}
