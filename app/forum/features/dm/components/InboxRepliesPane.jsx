...
import { resolveProfileAccountId } from '../../profile/utils/profileCache.js'
    ...
    <>
      ...
      {!showInitialRepliesSkeleton && repliesHasMore && (
        <div className="loadMoreFooter">
          ...
          <LoadMoreSentinel
            ...
            pending={!!serverRepliesLoading}
            hasMore={!!repliesHasMore}
            loadKey={`inbox:${Number((visibleRepliesToMe || []).length)}:${Number(sortedRepliesLength || 0)}:${Number(!!serverRepliesHasMore)}`}
            onVisible={() => {
      ...
      {!serverRepliesLoading && sortedRepliesLength === 0 && (
        <div className="meta forumFeedEmptyState">
          <span className="forumFeedEmptyStateText">
            {t('forum_inbox_empty')}
          </span>
        </div>
      )}
