...
import { readAndMigrateForumRulesAcceptance, writeForumRulesAcceptance } from '../../lib/migrations/forumRulesStorage'
...
export default function ForumPage() {
  ...
  useEffect(() => {
    try {
      ...
      const ok = readAndMigrateForumRulesAcceptance(window.localStorage)
  ...
  const handleAccept = () => {
    try {
      if (typeof window !== 'undefined') {
        writeForumRulesAcceptance(window.localStorage, '1')
      }
    } catch {}
    setShowRules(false)
  }
  ...
  return (
    <div className="iso-forum-page">
      ...
      <style jsx>{`
        ...
        /* Обёртка форума: использует тот же непрерывный global layout cap.
           Никаких собственных tablet/desktop ступеней и viewport-gutter скачков. */
        .iso-forum-shell {
          flex: 1 1 auto;
          min-height: 0;
          width: 100%;
          ...
          margin: 0 auto;
          padding-left: var(--gutter, 1px);
          padding-right: var(--gutter, 1px);
          border: 0;
          overflow: clip;
        }
