...
// === Q COIN: client-side live ticker with rare sync ===
function useQCoinLive(userKey){
  ...
  const lastUiRef       = React.useRef(0);                   // последнее локальное действие
  const lastSyncRef     = React.useRef(0);                   // когда последний раз синкались
  const becameActiveRef = React.useRef(true);                // нужно ли немедленный sync
  const displayRef      = React.useRef(server.balance);      // текущее отображаемое значение
  ...
  const markUi = React.useCallback(function(){
    ...
    becameActiveRef.current = true; // триггерим немедленный sync
  }, []);

  // Считаем открытие страницы «активностью», чтобы тикер сразу начал тикать
React.useEffect(function(){
  lastUiRef.current = Date.now();
  becameActiveRef.current = true; // дёрнем ранний sync и включим тиканье по hadUiRecently
}, []);


  // Стартовый GET (с timeout и санитайзингом чисел)
  React.useEffect(function(){
    var dead = false;
    ...
    async function load(){
      try{
        ...
        var r = await fetch('/api/qcoin/get', {
        ...
        if (!dead && j && j.ok){
          var inc0   = (j.incPerSec !== undefined && j.incPerSec !== null) ? j.incPerSec : INC_PER_SEC;
          var grace0 = (j.graceMs   !== undefined && j.graceMs   !== null) ? j.graceMs   : GRACE_MS;

          var safeInc0     = Math.max(0, Number(inc0));
          var safeGrace0   = Math.max(60000, Number(grace0));                 // ≥ 1 мин
          var clampedInc0  = Math.min(safeInc0,  INC_PER_SEC * 100);          // потолок 100x
          var clampedGr0   = Math.min(safeGrace0,24*60*60*1000);              // ≤ 24ч

          setServer(function(s){
            return {
              ...s,
              startedAt:   (j.startedAt    !== undefined && j.startedAt    !== null) ? j.startedAt    : s.startedAt,
              lastActiveAt:(j.lastActiveAt !== undefined && j.lastActiveAt !== null) ? j.lastActiveAt : s.lastActiveAt,
              lastConfirmAt:(j.lastConfirmAt!== undefined && j.lastConfirmAt!== null) ? j.lastConfirmAt : 0,
              seconds: Number((j.seconds !== undefined && j.seconds !== null) ? j.seconds : 0),
              balance: Number((j.balance !== undefined && j.balance !== null) ? j.balance : 0),
              paused: !!j.paused,
              loading: false,
              incPerSec: clampedInc0,
              graceMs: clampedGr0,
            };
          });

          displayRef.current   = Number((j.balance !== undefined && j.balance !== null) ? j.balance : 0);
          lastSyncRef.current  = Date.now();
          // считаем, что вкладка открыта/видна сейчас
          lastUiRef.current    = Date.now();
          becameActiveRef.current = false;
        } else if (!dead){
          setServer(function(s){ return { ...s, loading:false }; });
        }
      } catch(e){
        if (!dead) setServer(function(s){ return { ...s, loading:false }; });
      }
    }
  ...
  React.useEffect(function(){
    if (typeof window==='undefined') return;
    var root = document.querySelector('.forum_root') || document.body;
    var onAny = function(){ markUi(); };
    var onVis = function(){ if (document.visibilityState === 'visible') markUi(); };
  ...
  // Интервальный тик + heartbeat: тикаем каждую секунду, пока в грейсе
  React.useEffect(function(){
    var id = setInterval(async function(){
      var now = Date.now();

      var graceMs = (server.graceMs !== undefined && server.graceMs !== null) ? server.graceMs : GRACE_MS;
      var lastC   = server.lastConfirmAt || 0;
      var lastUi  = lastUiRef.current || 0;

      // «Последняя активность» = максимум из серверной и локальной (открытие вкладки тоже активность)
      var effectiveLast = (lastC > lastUi) ? lastC : lastUi;
      var withinGrace   = (now - effectiveLast) < graceMs;

      // Если в грейсе — тикаем визуально каждую секунду
      var incPerSec = (server.incPerSec !== undefined && server.incPerSec !== null) ? server.incPerSec : INC_PER_SEC;
      ...
      // Триггеры синка
      var needPeriodicSync  = (now - (lastSyncRef.current || 0)) >= SYNC_MS;
      var needImmediateSync = becameActiveRef.current;

      // Мягкий backoff при ошибках (до +10s)
      var backoffDelayMs = Math.min(5, syncErrors.current) * 2000;
      ...
      if ((needPeriodicSync || needImmediateSync) && uid && !heartbeatInFlight.current){
        ...
        var active = (now - (lastUiRef.current || 0)) < 60000;

        try{
          var controller = new AbortController();
          var timeoutId  = setTimeout(function(){ controller.abort(); }, 8000);

          var res = await fetch('/api/qcoin/heartbeat', {
            method:'POST',
            headers:{
              'content-type':'application/json',
              'x-forum-user-id': uid,
            },
            body: JSON.stringify({ active: !!active, now: now }),
            signal: controller.signal
          });
          ...
          if (j && j.ok){
            var incFromSrv   = (j.incPerSec !== undefined && j.incPerSec !== null) ? j.incPerSec : incPerSec;
            var graceFromSrv = (j.graceMs   !== undefined && j.graceMs   !== null) ? j.graceMs   : graceMs;

            var safeInc   = Math.max(0, Number(incFromSrv));
            var safeGrace = Math.max(60000, Number(graceFromSrv));
            var clampedInc   = Math.min(safeInc,   INC_PER_SEC * 100);
            var clampedGrace = Math.min(safeGrace, 24 * 60 * 60 * 1000);
            ...
            setServer(function(s){
              return {
                ...
                startedAt:    (j.startedAt    !== undefined && j.startedAt    !== null) ? j.startedAt    : s.startedAt,
                lastActiveAt: (j.lastActiveAt !== undefined && j.lastActiveAt !== null) ? j.lastActiveAt : s.lastActiveAt,
                lastConfirmAt:(j.lastConfirmAt!== undefined && j.lastConfirmAt!== null) ? j.lastConfirmAt : s.lastConfirmAt,
                seconds: Number((j.seconds !== undefined && j.seconds !== null) ? j.seconds : s.seconds),
                balance: Number((j.balance !== undefined && j.balance !== null) ? j.balance : s.balance),
                paused: !!j.paused,
            ...
            // подтягиваем локальный дисплей к серверу
            displayRef.current = Number((j.balance !== undefined && j.balance !== null) ? j.balance : displayRef.current);
            setDisplayBalance(displayRef.current);
  ...
  return {
    ...server,
    balanceDisplay: displayBalance,  // <-- в UI показывай ЭТО
    open:  function(){ setServer(function(s){ return { ...s, modal:true  }; }); },
    close: function(){ setServer(function(s){ return { ...s, modal:false }; }); },
  };
}
...
// === live-обновления через SSE (refresh только по факту картинок/удаления) ===
...
React.useEffect(() => {
  ...
  const deb = (fn, ms=160) => { clearTimeout(window.__forumSSEDebounce__); window.__forumSSEDebounce__ = setTimeout(fn, ms); };

  es.onmessage = (e) => {
    try {
      const evt = JSON.parse(e.data);
      // ждём только наши типы
      if (!evt || !evt.type) return;

      // обновляем ТОЛЬКО если есть смысл
      if (evt.type === 'post_created') {
        if (evt.hasImages === true) {
          deb(() => { refresh?.(); });
        }
      } else if (evt.type === 'post_deleted') {
        // удаление — обновляем (можно сузить по evt.hadImages===true, если добавишь этот флаг на сервер)
        deb(() => { refresh?.(); });
      }
    } catch { /* игнорим мусор */ }
  };
  ...
  es.onerror = () => {
    try { es.close(); } catch {}
    // можно добавить редкий fallback-пуллинг, если нужно:
    // if (!window.__forumPoll) window.__forumPoll = setInterval(()=>refresh?.(), 60000);
  };
...
// === END flat ===
 ...
 // ===== ПУНКТ 5: Агрегаты по темам из постов снапшота =====
  ...
  const [topicFilterId, setTopicFilterId] = useState(null);   // фильтр выбранной темы из поиска
...
toast.ok('Тема создана')
  ...
  const createPost = async () => {
    if (!rl.allowAction()) { toast.warn(t('forum_too_fast') || 'Слишком часто'); return; }
const plain = String(text || '').trim().slice(0, 180);
...
if (!body || !sel?.id) return;
  ...
  // ссылки — только для админов (проверяем тело поста)
  if (!isAdm && hasAnyLink(body)) {
    toast.warn(t('forum_links_admin_only') || 'Ссылки может отправлять только администратор');
    return;
  }
