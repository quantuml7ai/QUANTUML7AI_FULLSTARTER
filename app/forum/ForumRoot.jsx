...
import { isQl7SupportActive } from '../../lib/ql7-support/config/featureFlag'
import {
  isQl7SupportEphemeralEntryMessage,
...
const topicReconcileSurfaceSkipRef = useRef(null)
const {
  tombstones,
  persistTombstones,
  persistSnap,
...
useEffect(() => {
  ...
  for (const post of posts) {
    ...
    const actorId = String(remembered?.actorId || auth?.asherId || auth?.accountId || viewerId || '').trim()
...
useEffect(() => {
  topicReconcileRef.current = ({ cid, topicId, topic }) => {
    const optimisticId = String(cid || '').trim()
    const authoritativeId = String(topicId || topic?.id || '').trim()

if (!optimisticId || !authoritativeId) return

topicReconcileSurfaceSkipRef.current = {
  from: optimisticId,
  to: authoritativeId,
}
...
setSel((prev) => {
      ...
      // Это всё ещё та же логическая тема, поэтому не считаем
      // authoritative-ID повторным "открытием" темы.
...
const hydrateQl7SupportRuntimeState = useCallback(async () => {
  try {
    ...
    setQl7SupportUiEvent({
      supportUiState: String(state.state || state?.stateReceipt?.phase || 'idle'),
      caseId: String(state.caseId || ''),
      correlationId: String(state.correlationId || state?.stateReceipt?.correlationId || ''),
      attemptId: String(state.attemptId || state?.stateReceipt?.attemptId || ''),
      sequence: Number(state.sequence ?? state?.stateReceipt?.sequence ?? 0),
      stateVersion: Number(state.stateVersion ?? state?.stateReceipt?.stateVersion ?? 0),
      phaseRank: Number(state.phaseRank ?? state?.stateReceipt?.phaseRank ?? 0),
      terminal: state.terminal === true || state?.stateReceipt?.terminal === true,
      stateReceipt: state?.stateReceipt && typeof state.stateReceipt === 'object' ? state.stateReceipt : null,
      changedAt: Date.parse(state.changedAt || state?.stateReceipt?.changedAtServerUtc || '') || Date.now(),
...
const openQl7SupportThread = useCallback(() => {
  ...
  let entryNonce = supportEntryNonceRef.current
  try {
    const storageKey = 'ql7_support_entry_session'
    entryNonce = String(window.sessionStorage?.getItem(storageKey) || entryNonce || '').trim()
    if (!entryNonce) {
      entryNonce = `entry-session:${globalThis.crypto?.randomUUID?.() || `${Date.now()}:${Math.random().toString(36).slice(2)}`}`
      window.sessionStorage?.setItem(storageKey, entryNonce)
    }
  } catch {
    if (!entryNonce) entryNonce = `entry-session:${Date.now()}`
  }
  ...
  try {
    setDmThreadItems((prev) => {
      const rows = Array.isArray(prev) ? prev.filter((item) => !isQl7SupportEphemeralEntryMessage(item)) : []
      return rows
    })
  } catch {}
  ...
  try {
    setQl7SupportUiEvent({
      supportUiState: 'composing',
      caseId: '',
      correlationId: entryNonce,
      changedAt: Date.now(),
      inputPolicy: null,
    })
  } catch {}
    ...
    void fetchQl7SupportAuthenticated('/api/dm/support-entry', {
      ...
      .then((result) => {
        if (!result?.response?.ok || !result?.data?.ok) {
          ...
          const errorCode = String(result?.data?.error || result?.data?.code || `support_entry_http_${status}`)
          if (status === 409) {
            setQl7SupportUiEvent({ supportUiState: 'composing', correlationId: entryNonce, detailCode: 'support_entry_reconciliation_required', changedAt: Date.now() })
            return refreshDmThreadRealtime?.(uid, 'support-entry-conflict-reconcile')
          }
          setQl7SupportUiEvent({ supportUiState: 'temporarily_unavailable', correlationId: entryNonce, detailCode: errorCode, changedAt: Date.now() })
          return null
        }
        return refreshDmThreadRealtime?.(uid, result?.data?.deduped ? 'support-entry-reconciled' : 'support-entry-ack')
      })
      .catch((error) => {
        setQl7SupportUiEvent({ supportUiState: 'temporarily_unavailable', correlationId: entryNonce, detailCode: String(error?.code || error?.message || 'support_entry_failed'), changedAt: Date.now() })
        return null
      })
  } catch {}
}, [
...
useEffect(() => {
  ...
  try {
    setDmThreadItems((prev) => (
      Array.isArray(prev) ? prev.filter((item) => !isQl7SupportEphemeralEntryMessage(item)) : prev
    ))
  } catch {}
  try {
    const snapshot = readQl7SupportAuthSnapshot()
...
useEffect(() => {
  ...
  supportIdleTimerRef.current = window.setTimeout(() => {
    ...
    if (supportLatestAnchor !== expectedAnchor) return
    const snapshot = readQl7SupportAuthSnapshot()
    void fetchQl7SupportAuthenticated('/api/dm/support-entry', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-forum-locale': String(locale || ''), ...buildQl7SupportAuthHeaders(snapshot) },
      body: JSON.stringify({
        action: 'idle_nudge',
        locale,
        anchorId: String(supportLatestMessage?.id || ''),
        routeContext: buildQl7SupportRouteContext(),
      }),
    }, { waitTimeoutMs: 12000, retryOnFreshAuth: true })
      .then((result) => {
        if (!result?.response?.ok || !result?.data?.ok || result?.data?.skipped) return null
        return refreshDmThreadRealtime?.(QL7_SUPPORT_ID, 'support-idle-nudge-ack')
      })
      .catch(() => null)
  }, delay)
  return () => {
...
}, [auth?.accountId, auth?.walletAddress, dmSupportMode, locale, meId, refreshDmThreadRealtime, supportActive, supportLatestAnchor, supportLatestMessage, dmThreadItems])
...
useEffect(() => {
  ...
  const previousSurfaceKey = String(lastSurfaceSyncKeyRef.current || '')

  if (!previousSurfaceKey) {
    lastSurfaceSyncKeyRef.current = surfaceSyncKey
    return
  }

  if (previousSurfaceKey === surfaceSyncKey) return

  const pendingReconcile = topicReconcileSurfaceSkipRef.current

  if (pendingReconcile) {
    const splitSurfaceKey = (value) => {
      const raw = String(value || '')
      const separator = raw.indexOf('|')

      return separator >= 0
        ? {
            topicId: raw.slice(0, separator),
            tail: raw.slice(separator + 1),
          }
        : {
            topicId: raw,
            tail: '',
    ...
    const before = splitSurfaceKey(previousSurfaceKey)
    const after = splitSurfaceKey(surfaceSyncKey)

    const from = String(pendingReconcile.from || '').trim()
    const to = String(pendingReconcile.to || '').trim()

    const isPureTopicIdReconcile =
      !!from &&
      !!to &&
      before.topicId === from &&
      after.topicId === to &&
    ...
    if (isPureTopicIdReconcile) {
      topicReconcileSurfaceSkipRef.current = null
      lastSurfaceSyncKeyRef.current = surfaceSyncKey
      return
    }

    if (after.topicId !== from) {
      topicReconcileSurfaceSkipRef.current = null
    }
  }
...
const composerBusy = React.useMemo(() => {
  ...
  const busyPhase = !!(phaseBusy || (mediaPipelineOn && phase !== 'ready' && phase !== 'idle' && phase !== 'prepared'))
  const videoBusy = (
    videoState === 'live' ||
...
const {
  ...
  attachmentsArgs: {
    ...
    endMediaPipeline,
    mediaCancelRef,
    videoModerationSurface: dmMode ? 'dm' : 'forum',
    videoModerationActorId: dmMode
      ? String(resolveProfileAccountId(auth?.asherId || auth?.accountId || viewerId) || auth?.asherId || auth?.accountId || viewerId || '')
      : String(auth?.asherId || auth?.accountId || ''),
  },
  text,
  pendingImgs,
  pendingSticker,
...
useEffect(() => {
  ...
  const consumeProfileBranch = (rawDetail = null) => {
    let detail = rawDetail && typeof rawDetail === 'object' ? rawDetail : null
    if (!detail) {
      try {
        detail = JSON.parse(window.sessionStorage?.getItem('ql7_forum_profile_branch_after_navigation_v1') || 'null')
      } catch {}
    }
    try {
      const params = new URLSearchParams(window.location.search || '')
      const queryUserId = String(params.get('profileUser') || '').trim()
      if (queryUserId) {
        detail = {
          mode: params.get('profileMode') === 'topics' ? 'topics' : 'posts',
          userId: queryUserId,
          nickname: String(params.get('profileNick') || '').trim(),
        }
      }
    } catch {}
    ...
    const payload = { userId, nickname: String(detail?.nickname || '').trim() }
    if (detail?.mode === 'topics') openProfileTopicsBranch(payload)
    else openProfilePostsBranch(payload)

    try { window.sessionStorage?.removeItem('ql7_forum_profile_branch_after_navigation_v1') } catch {}
    try {
      const cleanUrl = new URL(window.location.href)
      for (const key of ['profileMode', 'profileUser', 'profileNick']) cleanUrl.searchParams.delete(key)
      window.history.replaceState(window.history.state, '', `${cleanUrl.pathname}${cleanUrl.search}${cleanUrl.hash}` || '/forum')
    } catch {}
    return true
  }

  const onProfileBranchOpen = (event) => {
    consumeProfileBranch(event?.detail || null)
  }

  window.addEventListener('forum:profile-branch-open', onProfileBranchOpen)
  let hasPendingBranch = false
  try {
    const params = new URLSearchParams(window.location.search || '')
    hasPendingBranch = !!String(params.get('profileUser') || '').trim()
      || !!window.sessionStorage?.getItem('ql7_forum_profile_branch_after_navigation_v1')
  } catch {}
  if (hasPendingBranch) bridgeTimer = window.setTimeout(() => consumeProfileBranch(), 80)

  return () => {
    window.removeEventListener('forum:profile-branch-open', onProfileBranchOpen)
    if (bridgeTimer) window.clearTimeout(bridgeTimer)
  }
}, [openProfilePostsBranch, openProfileTopicsBranch])
...
const topicsPaneLoading = profileBranchMode === 'topics'
  ? profileBranchTopicsLoading
  : data?.rev == null
  const {
    ...
    topicsLoading: topicsPaneLoading,
    setVisibleTopicsCount,
    topicPageSize: TOPIC_PAGE_SIZE,
