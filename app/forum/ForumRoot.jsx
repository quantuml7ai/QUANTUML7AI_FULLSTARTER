...
const FORUM_NOTIFY_ICON = '/icon-192.png'
const FORUM_NOTIFY_PROMPT_KEY = 'forum:notifications:prompted:'

function supportsBrowserNotifications() {
  return typeof window !== 'undefined' && 'Notification' in window
}

function readNotificationPrompted(viewerId) {
  if (!supportsBrowserNotifications()) return false
  const key = `${FORUM_NOTIFY_PROMPT_KEY}${String(viewerId || '').trim() || 'anon'}`
  try {
...
function writeNotificationPrompted(viewerId) {
...
function compactNotificationText(value, maxLen = 132) {
...
const forumNotificationRefs = useRef(new Map());
const dmNotificationSnapshotRef = useRef(new Map());
const replyNotificationSnapshotRef = useRef(new Set());
const dmNotificationBootedRef = useRef(false);
const replyNotificationBootedRef = useRef(false);
...
const openInboxFromNotification = useCallback((target = {}) => {
  const tab = String(target?.tab || 'messages') === 'replies' ? 'replies' : 'messages'
  const uid = tab === 'messages' ? String(target?.uid || '').trim() : ''
  try { window.focus?.() } catch {}
  try { pushNavStateStable(`notify_${tab}${uid ? `_${uid}` : ''}`) } catch {}
  ...
  try { setInboxTab(tab) } catch {}
  try { setDmWithUserId(uid) } catch {}
  try { requestAlignInboxStartUnderTabs?.() } catch {}
}, [
  pushNavStateStable,
  openOnly,
  closeUserInfoPopover,
...
const showBrowserNotification = useCallback((payload = {}) => {
  ...
  if (!tag) return null
  try {
    const prev = forumNotificationRefs.current.get(tag)
    prev?.close?.()
  } catch {}
  try {
    const notification = new Notification(
      String(payload?.title || t('forum_notifications_title') || 'Quantum Messenger'),
      {
        body: String(payload?.body || '').trim(),
        tag,
        icon: FORUM_NOTIFY_ICON,
        badge: FORUM_NOTIFY_ICON,
        lang: String(locale || 'en'),
        dir: uiDir === 'rtl' ? 'rtl' : 'ltr',
        renotify: true,
        silent: false,
      },
    )
    notification.onclick = (event) => {
      ...
      openInboxFromNotification(payload?.target || {})
    }
    notification.onclose = () => {
      try {
        if (forumNotificationRefs.current.get(tag) === notification) {
          forumNotificationRefs.current.delete(tag)
        }
      } catch {}
    }
    forumNotificationRefs.current.set(tag, notification)
    return notification
  } catch {
    return null
  }
}, [locale, openInboxFromNotification, t, uiDir])

useEffect(() => {
  const notificationMap = forumNotificationRefs.current
  return () => {
    try {
      for (const notification of notificationMap.values()) {
        notification?.close?.()
      }
...
useEffect(() => {
  dmNotificationSnapshotRef.current = new Map()
  replyNotificationSnapshotRef.current = new Set()
  dmNotificationBootedRef.current = false
  replyNotificationBootedRef.current = false
  try {
    for (const notification of forumNotificationRefs.current.values()) {
      notification?.close?.()
    }
    forumNotificationRefs.current.clear()
  } catch {}
}, [viewerId])

useEffect(() => {
  if (!mounted || !viewerId || !supportsBrowserNotifications()) return
  if (window.Notification?.permission !== 'default') return
  if (readNotificationPrompted(viewerId)) return
  let cancelled = false
  const timerId = window.setTimeout(async () => {
    if (cancelled) return
    if (document.hidden) return
    writeNotificationPrompted(viewerId)
    try {
...
useEffect(() => {
  if (!mounted || !meId || !dmDialogsLoaded) return
  ...
  for (const dialog of (dmDialogs || [])) {
    ...
    snapshot.set(uid, { lastTs, lastFrom })
    const prev = dmNotificationSnapshotRef.current.get(uid)
    if (!dmNotificationBootedRef.current) continue
    if (lastFrom === String(meId)) continue
    if (prev && Number(prev?.lastTs || 0) >= lastTs) continue
    incomingDialogs.push({ uid, lastTs, last })
  }
  dmNotificationSnapshotRef.current = snapshot
  if (!dmNotificationBootedRef.current) {
    dmNotificationBootedRef.current = true
    return
  }
  if (!incomingDialogs.length) return
  const pageFocused = !document.hidden && !!document.hasFocus?.()
  const candidates = incomingDialogs
    .filter((entry) => !(pageFocused && inboxOpen && inboxTab === 'messages' && String(dmWithUserId || '') === String(entry.uid || '')))
    .sort((a, b) => Number(b?.lastTs || 0) - Number(a?.lastTs || 0))
  if (!candidates.length) return
  const primary = candidates[0]
  const previewRaw = stripMediaUrlsFromText(String(primary?.last?.text || ''))
  const preview = compactNotificationText(previewRaw || t('forum_attachment'))
  const author = resolveNickForDisplay(primary?.uid, '') || shortId(primary?.uid)
  const body = candidates.length > 1
    ? `${candidates.length} ${t('forum_notifications_new_messages')}\n${author}: ${preview}`
    : `${author}\n${preview}`
  showBrowserNotification({
    tag: 'forum-dm',
    title: t('forum_notifications_title'),
    body,
    target: { tab: 'messages', uid: primary?.uid || '' },
  })
}, [
  mounted,
  meId,
...
useEffect(() => {
  ...
  const nextIds = new Set((sortedRepliesToMe || []).map((post) => String(post?.id || '')).filter(Boolean))
  const newReplies = []
  for (const post of (sortedRepliesToMe || [])) {
    const id = String(post?.id || '')
    if (!id) continue
    if (!replyNotificationBootedRef.current) continue
    if (replyNotificationSnapshotRef.current.has(id)) continue
    newReplies.push(post)
  }
  replyNotificationSnapshotRef.current = nextIds
  if (!replyNotificationBootedRef.current) {
    replyNotificationBootedRef.current = true
    return
  }
  if (!newReplies.length) return
  const pageFocused = !document.hidden && !!document.hasFocus?.()
  if (pageFocused && inboxOpen && inboxTab === 'replies') return
  const orderedReplies = newReplies.slice().sort((a, b) => Number(b?.ts || 0) - Number(a?.ts || 0))
  const primary = orderedReplies[0]
  const authorId = primary?.userId || primary?.accountId || ''
  const author = resolveNickForDisplay(authorId, primary?.nickname || '') || shortId(authorId)
  const previewRaw = stripMediaUrlsFromText(String(primary?.text || primary?.message || primary?.body || ''))
  const preview = compactNotificationText(previewRaw || t('forum_attachment'))
  const body = orderedReplies.length > 1
    ? `${orderedReplies.length} ${t('forum_notifications_new_replies')}\n${author}: ${preview}`
    : `${author}\n${preview}`
  showBrowserNotification({
    tag: 'forum-replies',
    title: t('forum_notifications_title'),
