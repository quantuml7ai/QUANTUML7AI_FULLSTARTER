...
const REMOUNT_FOOTPRINT_RECHECK_MS = 180
const REMOUNT_FOOTPRINT_MIN_HOLD_MS = 1600
const REMOUNT_FOOTPRINT_RELEASE_IDLE_MS = 2600
const REMOUNT_FOOTPRINT_EPSILON_PX = 2
const MEDIA_KEEPALIVE_RUNWAY_ITEMS = 4
const MEDIA_KEEPALIVE_EXPIRY_SLOP_MS = 12
const WINDOWING_KEEPALIVE_OPT_OUT_SELECTOR = '[data-windowing-keepalive="0"]'
...
function readItemLayoutFootprint(node) {
  ...
  const boxHeight = Math.max(rectHeight, offsetHeight)
  let marginHeight = 0
  let rowGap = 0
  let reservationMinHeight = boxHeight
  ...
  try {
    ...
    const verticalChrome =
      readCssPx(style?.paddingBlockStart || style?.paddingTop) +
      readCssPx(style?.paddingBlockEnd || style?.paddingBottom) +
      readCssPx(style?.borderBlockStartWidth || style?.borderTopWidth) +
      readCssPx(style?.borderBlockEndWidth || style?.borderBottomWidth)
    const measuredCssMinHeight = boxSizing === 'border-box'
      ? boxHeight
      : Math.max(0, boxHeight - verticalChrome)
    reservationMinHeight = Math.max(readCssPx(style?.minHeight), measuredCssMinHeight)
  ...
  return {
    height: boxHeight + marginHeight,
    boxHeight,
    reservationMinHeight,
    rowGap,
  }
}

function readItemNaturalLayoutFootprint(node, reservation = null) {
  const style = node?.style
  const appliedMinHeight = String(reservation?.appliedMinHeight || '')
  if (!style || !appliedMinHeight || style.minHeight !== appliedMinHeight) {
    return readItemLayoutFootprint(node)
  }

  const currentMinHeight = style.minHeight
  try {
    // Measure the content without allowing our remount reservation to become
    // the new cached height. The original min-height is restored before paint.
    style.minHeight = String(reservation?.originalInlineMinHeight || '')
    return readItemLayoutFootprint(node)
  } finally {
...
function createRemountFootprintMetric(footprint) {
  const boxHeight = Number(footprint?.boxHeight || 0)
  const minHeight = Number(footprint?.reservationMinHeight || 0)
  const footprintHeight = Number(footprint?.height || 0)
  if (!Number.isFinite(boxHeight) || boxHeight <= 1) return null
  if (!Number.isFinite(minHeight) || minHeight <= 1) return null
  if (!Number.isFinite(footprintHeight) || footprintHeight <= 1) return null
  return { boxHeight, minHeight, footprintHeight }
}
...
function isInsideWindowingKeepaliveOptOut(node, boundary = null) {
  try {
    if (typeof Element === 'undefined' || !(node instanceof Element)) return false
    const optOut = node.closest?.(WINDOWING_KEEPALIVE_OPT_OUT_SELECTOR)
    if (!optOut) return false
    if (!boundary) return true
    if (optOut === boundary) return true
    return !!boundary.contains?.(optOut)
  } catch {
    return false
  }
}
...
function hasStableMediaShell(node) {
  try {
    if (typeof Element === 'undefined' || !(node instanceof Element)) return false
    if (isInsideWindowingKeepaliveOptOut(node)) return false
...
function containsStableMediaShell(node) {
  try {
    ...
    const stableDescendants = node.querySelectorAll?.(STABLE_MEDIA_SHELL_SELECTOR) || []
    for (const candidate of stableDescendants) {
      if (!isInsideWindowingKeepaliveOptOut(candidate, node)) return true
    }
    return false
  } catch {
    return false
...
// Keepalive controls DOM retention; it must not disable layout stability.
// Video Feed intentionally opts out of media keepalive, but its intrinsic media
// resize must still be kept out of the active-scroll spacer calculation.
function hasStableLayoutShell(node) {
  try {
    if (typeof Element === 'undefined' || !(node instanceof Element)) return false
    return !!node.matches?.(STABLE_MEDIA_SHELL_SELECTOR)
  } catch {
...
function containsStableLayoutShell(node) {
  try {
    if (typeof Element === 'undefined' || !(node instanceof Element)) return false
    if (hasStableLayoutShell(node)) return true
...
export function isForumWindowingMediaKeepaliveSensitive(node) {
  try {
    return hasStableMediaShell(node) || containsStableMediaShell(node)
  } catch {
    return false
  }
}

function clampKeepaliveExpansionToBudget({
  baseStart,
  baseEnd,
  expandedStart,
  expandedEnd,
  ...
  const safeBaseStart = clamp(Number(baseStart || 0), 0, safeTotal)
  const safeBaseEnd = clamp(Number(baseEnd || 0), safeBaseStart, safeTotal)
  const safeExpandedStart = clamp(Number(expandedStart || 0), 0, safeBaseStart)
  const safeExpandedEnd = clamp(Number(expandedEnd || 0), safeBaseEnd, safeTotal)
  const baseSize = Math.max(0, safeBaseEnd - safeBaseStart)
  const safeBudget = clamp(Math.max(baseSize, Number(budget || 0)), baseSize, safeTotal)

  if ((safeExpandedEnd - safeExpandedStart) <= safeBudget) {
    return { start: safeExpandedStart, end: safeExpandedEnd }
  }

  let start = safeBaseStart
  let end = safeBaseEnd
  let remaining = Math.max(0, safeBudget - baseSize)
  let leftDemand = Math.max(0, safeBaseStart - safeExpandedStart)
  let rightDemand = Math.max(0, safeExpandedEnd - safeBaseEnd)

  const takeLeft = (limit = remaining) => {
    const take = Math.min(leftDemand, remaining, Math.max(0, Number(limit || 0)))
    start -= take
    remaining -= take
    leftDemand -= take
  }
  const takeRight = (limit = remaining) => {
    const take = Math.min(rightDemand, remaining, Math.max(0, Number(limit || 0)))
    end += take
    remaining -= take
    rightDemand -= take
  }
  ...
  if (direction > 0) {
    ...
    const leftShare = Math.ceil(remaining / 2)
    takeLeft(leftShare)
    takeRight()
    takeLeft()
  }
  ...
  return {
    start: clamp(start, 0, safeTotal),
    end: clamp(end, 0, safeTotal),
  }
}
...
export default function useForumWindowing({
  ...
  recalcOnScrollSettle = true,
}) {
  const heightsRef = useRef(new Map())
  const itemRowGapRef = useRef(0)
  const measuredNodesRef = useRef(new Map())
  // QL7_FORUM_WINDOWING_SCROLL_STABILITY_R27A_PATCH_B_FINAL
  // React callback refs must be stable across renders. Recreating them forces
  // ref(null) -> ref(node), tears down ResizeObserver and synchronously rereads
  // layout for every mounted card even when the DOM node did not change.
  const measureHandlerRef = useRef(null)
  const measureCallbackCacheRef = useRef(new Map())
  // First-mount measurements are grouped into one animation frame so a React
  // commit cannot fan out into a burst of forced layout reads on iOS/WebKit.
  const measureQueueRef = useRef(new Map())
  ...
  const confirmedFootprintsRef = useRef(new Map())
  const pendingFootprintsRef = useRef(new Map())
  const remountCandidatesRef = useRef(new Set())
  const remountReservationsRef = useRef(new Map())
  const remountReservationTimerRef = useRef(0)
  const lastScrollWallTsRef = useRef(0)
  const stableShrinkRef = useRef(new Map())
  const mediaKeepaliveRef = useRef(new Map())
  const mediaKeepaliveTimerRef = useRef({ id: 0, deadline: 0 })
  ...
  const clearMediaKeepaliveExpiryTimer = useCallback(() => {
    ...
    if (scheduled?.id) {
      try { clearTimeout(scheduled.id) } catch {}
    }
    mediaKeepaliveTimerRef.current = { id: 0, deadline: 0 }
  }, [])

  const scheduleMediaKeepaliveExpiry = useCallback((requestRecalc) => {
    if (!mediaKeepaliveEnabled || !isBrowserFn()) {
    ...
    mediaKeepaliveRef.current.forEach((until, key) => {
      const deadline = Number(until || 0)
      if (!Number.isFinite(deadline) || deadline <= now) return
      if (!keyToIndexRef.current.has(key)) return
    ...
    const scheduled = mediaKeepaliveTimerRef.current
    if (scheduled?.id && Math.abs(Number(scheduled.deadline || 0) - nextDeadline) <= MEDIA_KEEPALIVE_EXPIRY_SLOP_MS) {
      return
    }

    clearMediaKeepaliveExpiryTimer()
    const delay = Math.max(8, Math.ceil(nextDeadline - now) + MEDIA_KEEPALIVE_EXPIRY_SLOP_MS)
    const id = setTimeout(() => {
      mediaKeepaliveTimerRef.current = { id: 0, deadline: 0 }
      const expiredAt = Date.now()
      let expired = 0
      mediaKeepaliveRef.current.forEach((until, key) => {
        if (!keyToIndexRef.current.has(key) || Number(until || 0) <= expiredAt) {
          mediaKeepaliveRef.current.delete(key)
          expired += 1
        }
      })
      if (expired > 0) {
        emitWindowingDiag('windowing_media_keepalive_expired', {
          expired,
  ...
  const scheduleRecalc = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      try {
        ...
        if (mediaKeepaliveEnabled) {
          const keepaliveNow = Date.now()
          let expired = 0
          mediaKeepaliveRef.current.forEach((until, key) => {
            if (!keyToIndexRef.current.has(key) || Number(until || 0) <= keepaliveNow) {
              mediaKeepaliveRef.current.delete(key)
              expired += 1
            }
          })
          if (expired > 0) {
            emitWindowingDiag('windowing_media_keepalive_pruned', {
              expired,
              remaining: mediaKeepaliveRef.current.size,
            })
          }
          scheduleMediaKeepaliveExpiry(scheduleRecalc)
        } else if (mediaKeepaliveRef.current.size > 0) {
          mediaKeepaliveRef.current.clear()
          clearMediaKeepaliveExpiryTimer()
        }
        ...
        setWin((prev) => {
          ...
          try {
            ...
            const keepaliveBaseStart = nextStart
            const keepaliveBaseEnd = nextEnd
            let kept = 0
            if (mediaKeepaliveEnabled) mediaKeepaliveRef.current.forEach((until, key) => {
              if (Number(until || 0) <= now) return
              const index = Number(keyToIndexRef.current.get(key))
            ...
            if (kept > 0) {
              const keepaliveBudget = clamp(
                Math.max(
                  keepaliveBaseEnd - keepaliveBaseStart,
                  minViewportSafeWindowSize,
                  nextMaxRender + MEDIA_KEEPALIVE_RUNWAY_ITEMS,
                ),
                1,
                total,
              )
              const bounded = clampKeepaliveExpansionToBudget({
                baseStart: keepaliveBaseStart,
                baseEnd: keepaliveBaseEnd,
              ...
              const trimmed = Math.max(0, (nextEnd - nextStart) - (bounded.end - bounded.start))
              nextStart = bounded.start
              nextEnd = bounded.end
          ...
          if (total > 0) {
            ...
            // Keepalive may have widened a previous window that sticky shrink then preserves
            // as the next base window. Re-apply the bounded runway against the protected
            // viewport itself so an already-wide retained shell set cannot bypass the cap.
            // Explicit reveal/deeplink locks remain authoritative and are never trimmed here.
            if (mediaKeepaliveEnabled && mediaKeepaliveRef.current.size > 0 && !targetLockRef.current?.key) {
              const finalKeepaliveBudget = clamp(
                Math.max(minViewportSafeWindowSize, nextMaxRender + MEDIA_KEEPALIVE_RUNWAY_ITEMS),
                1,
                total,
              )
              const bounded = clampKeepaliveExpansionToBudget({
                baseStart: protectedVisibleStart,
                baseEnd: protectedVisibleEnd,
                expandedStart: nextStart,
                expandedEnd: nextEnd,
                total,
                budget: finalKeepaliveBudget,
                direction,
              })
              const trimmed = Math.max(0, (nextEnd - nextStart) - (bounded.end - bounded.start))
              if (trimmed > 0) {
                nextStart = bounded.start
                nextEnd = bounded.end
                emitWindowingDiag('windowing_media_keepalive_final_cap', {
                  trimmed,
                  budget: finalKeepaliveBudget,
                  start: nextStart,
                  end: nextEnd,
  ...
  const applyPendingMeasuredHeights = useCallback((reason = 'flush') => {
    ...
    try {
      pendingHeightsRef.current.forEach((height, key) => {
        ...
        const pendingFootprint = pendingFootprintsRef.current.get(key)
        if (pendingFootprint) confirmedFootprintsRef.current.set(key, pendingFootprint)
        applied += 1
      ...
      pendingFootprintsRef.current.clear()
  ...
  const scheduleMeasuredNodeUpdate = useCallback((key, node, update) => {
    if (!key || !node || typeof update !== 'function') return

    measureQueueRef.current.set(key, { node, update })
    if (measureRafRef.current) return

    measureRafRef.current = requestAnimationFrame(() => {
      measureRafRef.current = 0
      const batch = Array.from(measureQueueRef.current.entries())
      measureQueueRef.current.clear()

      for (const [batchKey, entry] of batch) {
        try {
          if (measuredNodesRef.current.get(batchKey) !== entry.node) continue
  ...
  const clearRemountReservationTimer = useCallback(() => {
    if (!remountReservationTimerRef.current) return
    try { clearTimeout(remountReservationTimerRef.current) } catch {}
    remountReservationTimerRef.current = 0
  }, [])

  const releaseRemountReservation = useCallback((key, reason = 'release') => {
    const reservation = remountReservationsRef.current.get(key)
    if (!reservation) return false

    try {
      const node = reservation.node
      if (node?.style && node.style.minHeight === reservation.appliedMinHeight) {
        node.style.minHeight = String(reservation.originalInlineMinHeight || '')
      }
    } catch {}

    remountReservationsRef.current.delete(key)
    emitWindowingDiag('remount_footprint_release', {
      key,
      reason,
      reservedBoxHeight: Math.round(Number(reservation.boxHeight || 0)),
      lastNaturalBoxHeight: Math.round(Number(reservation.lastNaturalBoxHeight || 0)),
    })
    return true
  }, [emitWindowingDiag])

  const clearAllRemountReservations = useCallback((reason = 'clear') => {
    clearRemountReservationTimer()
    Array.from(remountReservationsRef.current.keys()).forEach((key) => {
      releaseRemountReservation(key, reason)
    })
    remountCandidatesRef.current.clear()
  }, [clearRemountReservationTimer, releaseRemountReservation])

  const scheduleRemountReservationCheck = useCallback(() => {
    if (remountReservationTimerRef.current || remountReservationsRef.current.size === 0) return

    remountReservationTimerRef.current = setTimeout(() => {
      remountReservationTimerRef.current = 0
      remountReservationsRef.current.forEach((reservation, key) => {
        try {
          const node = reservation?.node
          const update = reservation?.update
          if (!node || measuredNodesRef.current.get(key) !== node || !node.isConnected) {
            releaseRemountReservation(key, 'detached')
            return
          }
          if (typeof update === 'function') scheduleMeasuredNodeUpdate(key, node, update)
        } catch {}
      })
    }, REMOUNT_FOOTPRINT_RECHECK_MS)
  }, [releaseRemountReservation, scheduleMeasuredNodeUpdate])

  const handleMeasuredNode = useCallback((key, node) => {
    if (!key) return
    try {
      if (!node) {
        const confirmedFootprint = confirmedFootprintsRef.current.get(key)
        const confirmedHeight = Number(heightsRef.current.get(key) || 0)
        releaseRemountReservation(key, 'virtualized_unmount')
        if (confirmedFootprint && Number.isFinite(confirmedHeight) && confirmedHeight > 1) {
          remountCandidatesRef.current.add(key)
        }

        measuredNodesRef.current.delete(key)
        measureQueueRef.current.delete(key)
        ...
        pendingFootprintsRef.current.delete(key)
      ...
      let remountReservation = null
      if (remountCandidatesRef.current.has(key)) {
        remountCandidatesRef.current.delete(key)
        const metric = confirmedFootprintsRef.current.get(key)
        const reservedMinHeight = Number(metric?.minHeight || 0)
        const reservedBoxHeight = Number(metric?.boxHeight || 0)
        if (node?.style && Number.isFinite(reservedMinHeight) && reservedMinHeight > 1 &&
          Number.isFinite(reservedBoxHeight) && reservedBoxHeight > 1) {
          const originalInlineMinHeight = String(node.style.minHeight || '')
          const originalInlinePx = readCssPx(originalInlineMinHeight)
          const appliedMinHeightPx = Math.max(reservedMinHeight, originalInlinePx)
          const appliedMinHeight = `${appliedMinHeightPx}px`

          // This write happens in the callback ref during the React commit, before
          // the remounted row can paint at its transient cold-media height. No
          // layout read is performed here. All useForumWindowing consumers inherit
          // the same physical-footprint continuity rule.
          node.style.minHeight = appliedMinHeight
          remountReservation = {
            node,
            boxHeight: reservedBoxHeight,
            minHeight: appliedMinHeightPx,
            appliedMinHeight,
            originalInlineMinHeight,
            reservedAt: Date.now(),
            lastNaturalBoxHeight: 0,
            lastHoldDiagAt: 0,
            update: null,
          }
          remountReservationsRef.current.set(key, remountReservation)
          emitWindowingDiag('remount_footprint_reserved', {
            key,
            boxHeight: Math.round(reservedBoxHeight),
            minHeight: Math.round(appliedMinHeightPx),
          })
        }
      }
      ...
      const update = () => {
        try {
          ...
          const activeReservation = remountReservationsRef.current.get(key)
          const footprint = readItemNaturalLayoutFootprint(node, activeReservation)
          ...
          const stableMediaShell = hasStableLayoutShell(node)
          const mediaSensitiveCard = isForumWindowingMediaKeepaliveSensitive(node)
          const cardContainsStableMediaShell = !stableMediaShell && containsStableLayoutShell(node)
          if (mediaKeepaliveEnabled && mediaSensitiveCard) {
            ...
            if (isNearViewportNode(node, keepaliveMargin)) {
              ...
              if (until > prevUntil) {
                mediaKeepaliveRef.current.set(key, until)
                scheduleMediaKeepaliveExpiry(scheduleRecalc)
              }
            }
          }
          ...
          if (activeReservation && activeReservation.node === node) {
            const naturalBoxHeight = Number(footprint.boxHeight || 0)
            activeReservation.lastNaturalBoxHeight = naturalBoxHeight

            const recovered =
              Number.isFinite(naturalBoxHeight) &&
              naturalBoxHeight >= (Number(activeReservation.boxHeight || 0) - REMOUNT_FOOTPRINT_EPSILON_PX)

            if (recovered) {
              releaseRemountReservation(key, 'natural_recovered')
            } else {
              const now = Date.now()
              const lastScrollTs = Number(lastScrollWallTsRef.current || 0)
              const recentScrollAge = lastScrollTs > 0
                ? Math.max(0, now - lastScrollTs)
                : Number.POSITIVE_INFINITY
              const reservationAge = Math.max(0, now - Number(activeReservation.reservedAt || now))
              const keepReservation =
                reservationAge < REMOUNT_FOOTPRINT_MIN_HOLD_MS ||
                isScrollActiveNow() ||
                recentScrollAge < REMOUNT_FOOTPRINT_RELEASE_IDLE_MS

              if (keepReservation) {
                if ((now - Number(activeReservation.lastHoldDiagAt || 0)) >= 600) {
                  activeReservation.lastHoldDiagAt = now
                  emitWindowingDiag('remount_footprint_hold', {
                    key,
                    reservedBoxHeight: Math.round(Number(activeReservation.boxHeight || 0)),
                    naturalBoxHeight: Math.round(naturalBoxHeight),
                    reservationAge: Math.round(reservationAge),
                    recentScrollAge: Number.isFinite(recentScrollAge) ? Math.round(recentScrollAge) : null,
                  })
                }
                if (rowGapChanged) scheduleRecalc()
                scheduleRemountReservationCheck()
                return
              }

              // A row that is still genuinely shorter after a full idle horizon
              // is no longer a cold-remount transient. Hand it back to the normal
              // stable-shrink/height-cache path instead of pinning blank space.
              releaseRemountReservation(key, 'idle_natural_shrink')
            }
          }
          ...
          if (
            ...
            if (!mayApplyShrink) {
              ...
              const pendingFootprint = createRemountFootprintMetric(footprint)
              if (pendingFootprint) pendingFootprintsRef.current.set(key, pendingFootprint)
          ...
          const shouldDeferMeasuredHeightDuringScroll =
            stableMediaShell || cardContainsStableMediaShell
          ...
          pendingFootprintsRef.current.delete(key)
          heightsRef.current.set(key, nextHeight)
          const confirmedFootprint = createRemountFootprintMetric(footprint)
          if (confirmedFootprint) confirmedFootprintsRef.current.set(key, confirmedFootprint)
          markHeightIndexDirty()
      ...
      if (remountReservation) {
        remountReservation.update = update
        scheduleRemountReservationCheck()
      }

      scheduleMeasuredNodeUpdate(key, node, update)

      if (!rosRef.current.get(key) && typeof ResizeObserver !== 'undefined') {
        const ro = new ResizeObserver(() => {
          // ResizeObserver already runs after layout. If it wins the race with
          // the queued first-mount read, consume that queued read and apply only
          // the freshest geometry once.
          measureQueueRef.current.delete(key)
          update()
        })
        ro.observe(node)
    ...
    mediaKeepaliveEnabled,
    markHeightIndexDirty,
    releaseRemountReservation,
    scheduleAnchorFlush,
    scheduleMeasuredNodeUpdate,
    scheduleMediaKeepaliveExpiry,
    scheduleRecalc,
    scheduleRemountReservationCheck,
    scrollSettleMs,
  ...
  measureHandlerRef.current = handleMeasuredNode

  const measureRef = useCallback((rawKey) => {
    const key = normalizeKey(rawKey, '')
    if (!key) return null

    const cached = measureCallbackCacheRef.current.get(key)
    if (cached) return cached

    const callback = (node) => {
      try { measureHandlerRef.current?.(key, node) } catch {}
    }
    measureCallbackCacheRef.current.set(key, callback)
    return callback
  }, [])

  useEffect(() => {
    // Keep the callback cache bounded to the currently rendered window. Callback
    // refs for nodes that survived the commit stay stable; callbacks for cards
    // that really left the virtualized DOM are released instead of accumulating
    // across a long infinite-feed session.
    const renderedKeys = new Set(itemKeys.slice(win.start, win.end))
    measureCallbackCacheRef.current.forEach((_, key) => {
      if (!renderedKeys.has(key) && !measuredNodesRef.current.has(key)) {
        measureCallbackCacheRef.current.delete(key)
      }
    })
  }, [itemKeys, win.end, win.start])
  ...
  useEffect(() => {
    ...
    measureCallbackCacheRef.current.forEach((_, key) => {
      if (!activeKeys.has(key)) measureCallbackCacheRef.current.delete(key)
    })

    measureQueueRef.current.forEach((_, key) => {
      if (!activeKeys.has(key)) measureQueueRef.current.delete(key)
    })
    ...
    pendingFootprintsRef.current.forEach((_, key) => {
      if (!activeKeys.has(key)) pendingFootprintsRef.current.delete(key)
    })
    confirmedFootprintsRef.current.forEach((_, key) => {
      if (!activeKeys.has(key)) confirmedFootprintsRef.current.delete(key)
    })
    remountCandidatesRef.current.forEach((key) => {
      if (!activeKeys.has(key)) remountCandidatesRef.current.delete(key)
    })
    Array.from(remountReservationsRef.current.keys()).forEach((key) => {
      if (!activeKeys.has(key)) releaseRemountReservation(key, 'item_removed')
    })
  ...
  useEffect(() => {
    ...
    const pendingFootprints = pendingFootprintsRef.current
    const stableShrinks = stableShrinkRef.current
    const onScroll = () => {
      try {
        const vp = readViewportState()
        const now = getNow()
        lastScrollWallTsRef.current = Date.now()
        ...
        scrollActivity.settleTimer = setTimeout(() => {
          ...
          if (recalcOnScrollSettle) {
            ...
            // real scroll frame. Only retire stale velocity here. Keeping the last
            // direction avoids a direction-neutral spacer move after iOS inertia.
            scrollStateRef.current = {
              ...scrollStateRef.current,
              velocity: 0,
            }
    ...
    const onResize = () => {
      try {
        ...
        if (prevLayoutKey !== nextLayoutKey) {
          ...
          clearAllRemountReservations('breakpoint_reset')
          try { heightsRef.current.clear() } catch {}
          try { pendingHeights.clear() } catch {}
          try { confirmedFootprintsRef.current.clear() } catch {}
          try { pendingFootprints.clear() } catch {}
  ...
  useEffect(() => {
    ...
    hardResetRef.current = () => {
      ...
      clearAllRemountReservations('hard_reset')
      try { heightsRef.current.clear() } catch {}
      try { pendingHeightsRef.current.clear() } catch {}
      try { confirmedFootprintsRef.current.clear() } catch {}
      try { pendingFootprintsRef.current.clear() } catch {}
      try { stableShrinkRef.current.clear() } catch {}
      try { mediaKeepaliveRef.current.clear() } catch {}
      clearMediaKeepaliveExpiryTimer()
      markHeightIndexDirty()
  ...
  useEffect(() => {
    ...
    const measureQueue = measureQueueRef.current
    const measureCallbackCache = measureCallbackCacheRef.current
    const pendingHeights = pendingHeightsRef.current
    const confirmedFootprints = confirmedFootprintsRef.current
    const pendingFootprints = pendingFootprintsRef.current
    const stableShrinks = stableShrinkRef.current
    ...
    return () => {
      ...
      try {
        ...
        anchorFlushTimerRef.current = 0
        pendingAnchorDeltaRef.current = 0
        clearAllRemountReservations('hook_unmount')
        pendingHeights.clear()
        confirmedFootprints.clear()
  ...
  }, [clearAllRemountReservations, clearMediaKeepaliveExpiryTimer, listId])
