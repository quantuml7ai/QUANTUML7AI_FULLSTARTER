...
export default function ForumHeaderPanel({
  ...
  const headerNickLabel = String(nickShown || '').trim()
  const headerNickLooksLikeId =
    /^0x[a-f0-9]{8,}$/i.test(headerNickLabel) ||
    /^[a-z0-9_-]{22,}$/i.test(headerNickLabel) ||
    headerNickLabel.includes('…') ||
    headerNickLabel.includes('...')
  const headerNickFitStyle = headerNickLabel && !headerNickLooksLikeId
    ? {
        fontSize: `${Math.max(10.8, Math.min(14, 112 / Math.max(headerNickLabel.length, 1))).toFixed(2)}px`,
        textOverflow: 'clip',
      }
    : undefined
  ...
  return (
    <>
      ...
      <div
        className={cls('head', headHidden && !headPinned && 'head--collapsed')}
        data-ql7-visual-scope="header"
        data-ql7-visual-collapsed={headHidden && !headPinned ? '1' : '0'}
      >
        <div className="headInner" style={{ width: '100%' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <button
              className={cls(
                'nick-badge nick-animate avaNick',
                headerNickLabel && !headerNickLooksLikeId && 'avaNick--profileName'
              )}
              title={idShown || '—'}
              onClick={copyId}
              translate="no"
            >
              <ForumNickText className="nick-text" style={headerNickFitStyle} textValue={nickShown || t('forum_not_signed')}>
