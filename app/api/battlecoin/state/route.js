...
// Private order settlement must always observe a fresh Binance result. This map
// shares only a still-pending public request; it never retains a resolved value,
// a TTL cache entry or a last-known-good fallback.
const activeMarketFreshInFlight = new Map()

async function fetchActiveMarketFreshSingleFlight(symbol) {
  const active = String(symbol || '').toUpperCase().trim()
  if (!active) return []

  const inFlight = activeMarketFreshInFlight.get(active)
  if (inFlight) return inFlight

  const request = Promise.resolve()
    .then(() => fetchActiveMarket(active))
    .finally(() => {
      if (activeMarketFreshInFlight.get(active) === request) {
        activeMarketFreshInFlight.delete(active)
      }
    })

  activeMarketFreshInFlight.set(active, request)
  return request
}
  ...
  try {
    ...
    if (scope === 'market') {
      return withRouteTelemetry({
        ...
        telemetry.setResultCount(symbols.length)
