...
import mongoTransactionContext from '../../../lib/mongo/transaction-context.cjs'

const { withMongoOperationContext, withMongoTransaction } = mongoTransactionContext
...
async function beginMarketEconomic({ routeId, operationType, userId, targetId, item, quantity, priceMicro = 0, idempotencyKey, action, source }) {
  const sourceEventId = `metamarket:${action}:${userId}:${item.itemId}:${String(idempotencyKey || '')}`
  return economicRoute.beginVerifiedEconomicOperation({
    routeId,
    operationType,
    actorAccountId: userId,
    targetAccountId: targetId || userId,
    amount: Math.abs(Number(priceMicro || 0)) / 1_000_000,
    orderId: String(idempotencyKey || ''),
    sourceEventId,
    idempotencyKey: `economic:${routeId}:${userId}:${String(idempotencyKey || sourceEventId)}`,
    sourceType: 'market-order',
    sourceOwner: 'app/api/metamarket/_transactions.js',
    sourceEvidence: { action, userId, targetId: targetId || userId, itemId: item.itemId, quantity, priceMicro: Number(priceMicro || 0), source: normalizeSource(source) },
    metadata: { itemId: item.itemId, quantity, action },
  })
}
...
async function quoteTransactionInContext({ action: rawAction, itemId, userId, recipientId = '', quantity: rawQuantity = 1 }) {
...
async function buyItemInContext({ userId, itemId, quantity: rawQuantity = 1, idempotencyKey, source, req }) {
  const item = getMetaMarketItem(itemId)
  if (!item) throw makeHttpError('item_not_found', 404)
  ...
  try {
    const result = await withMetaMarketLocks([K.lockItem(item.itemId), K.lockUser(userId)], async () => {
      ...
      const economicStarted = await beginMarketEconomic({ routeId: 'metamarket.buy', operationType: 'debit', userId, item, quantity, priceMicro, idempotencyKey, action: 'buy', source })
      if (economicStarted.authorization.replay) return economicStarted.authorization.replayResult
      try {
      ...
      const balanceQcoin = await writeQcoinBalanceMicro(userId, balanceMicro - priceMicro, { economicRouteId: 'metamarket.buy', operationType: 'debit', idempotencyKey: economicStarted.authorization.envelope.idempotencyKey, sourceEventId: economicStarted.authorization.envelope.sourceEventId, decisionReceipt: economicStarted.authorization.decisionReceipt })
      ...
      await economicRoute.commitVerifiedEconomicOperation(economicStarted, committedResult)
      return committedResult
      })
      } catch (error) {
        await economicRoute.abortVerifiedEconomicOperation(economicStarted, error?.message || 'metamarket_buy_failed').catch(() => {})
        throw error
      }
    })
    await finishIdempotency(idem.key, result).catch(() => {})
    return result
  } catch (error) {
...
async function sellItemInContext({ userId, itemId, quantity: rawQuantity = 1, tokenId = '', idempotencyKey, source, req }) {
  const item = getMetaMarketItem(itemId)
  if (!item) throw makeHttpError('item_not_found', 404)
  const quantity = normalizeQuantity(rawQuantity)
  ...
  try {
    const result = await withMetaMarketLocks([K.lockItem(item.itemId), K.lockUser(userId)], async () => {
      ...
      const economicStarted = await beginMarketEconomic({ routeId: 'metamarket.sell', operationType: 'credit', userId, item, quantity, priceMicro, idempotencyKey, action: 'sell', source })
      if (economicStarted.authorization.replay) return economicStarted.authorization.replayResult
      try {
      return await withMongoTransaction(async () => {
      ...
      const balanceMicro = await readQcoinBalanceMicro(userId)
      const balanceQcoin = await writeQcoinBalanceMicro(userId, balanceMicro + priceMicro, { economicRouteId: 'metamarket.sell', operationType: 'credit', idempotencyKey: economicStarted.authorization.envelope.idempotencyKey, sourceEventId: economicStarted.authorization.envelope.sourceEventId, decisionReceipt: economicStarted.authorization.decisionReceipt })
      await writeLedgerEvent({
        txId,
        type: 'SELL',
        item,
      ...
      return committedResult
        ...
        await economicRoute.abortVerifiedEconomicOperation(economicStarted, error?.message || 'metamarket_sell_failed').catch(() => {})
        throw error
      }
    })
    await finishIdempotency(idem.key, result).catch(() => {})
...
async function giftItemInContext({ userId, itemId, recipientId, quantity: rawQuantity = 1, tokenId = '', idempotencyKey, source, req }) {
  const item = getMetaMarketItem(itemId)
  if (!item) throw makeHttpError('item_not_found', 404)
  const quantity = normalizeQuantity(rawQuantity)
  const recipient = await resolveRecipientId(recipientId)
  ...
  try {
    const result = await withMetaMarketLocks([K.lockItem(item.itemId), K.lockUser(userId), K.lockUser(recipient)], async () => {
      ...
      const economicStarted = await beginMarketEconomic({ routeId: 'metamarket.gift', operationType: 'ownership_transfer', userId, targetId: recipient, item, quantity, priceMicro: 0, idempotencyKey, action: 'gift', source })
      if (economicStarted.authorization.replay) return economicStarted.authorization.replayResult
      try {
      return await withMongoTransaction(async () => {
      ...
      return committedResult
        ...
        await economicRoute.abortVerifiedEconomicOperation(economicStarted, error?.message || 'metamarket_gift_failed').catch(() => {})
        throw error
      }
    })
    await finishIdempotency(idem.key, result).catch(() => {})
...
export function quoteTransaction(input) {
  return withMongoOperationContext(() => quoteTransactionInContext(input))
}

export function buyItem(input) {
  return withMongoOperationContext(() => buyItemInContext(input))
}

export function sellItem(input) {
  return withMongoOperationContext(() => sellItemInContext(input))
}

export function giftItem(input) {
  return withMongoOperationContext(() => giftItemInContext(input))
}
