...
async function handle(req, method) {
  ...
  try {
    ...
    stage = 'resolve_canonical_identity'
    let accountId = String(tgId)
    let identityLinkConflicted = false
    try {
      const resolvedAccountId = await profilePrimary.resolveCanonicalAccountId(tgId)
      accountId = resolvedAccountId || String(tgId)
    } catch (error) {
      if (String(error?.code || '') !== 'IDENTITY_LINK_CONFLICT') throw error

      // Authentication itself is still valid: Telegram initData was verified
      // above. An ambiguous legacy Telegram -> wallet link must not select one
      // wallet arbitrarily or prevent the Telegram principal from signing in.
      // Keep the session Telegram-only and leave Mongo untouched for a later
      // explicit identity-link reconciliation.
      identityLinkConflicted = true
      console.warn('[TMA_IDENTITY_LINK_CONFLICT]', {
        stage,
        reason: String(error?.details?.reason || 'ambiguous_link').slice(0, 80),
      })
    }

    if (!identityLinkConflicted) {
      stage = 'persist_canonical_profile'
    ...
    console.error('[TMA_AUTO_FAILED]', {
      stage,
      code: String(e?.code || e?.message || 'tma_auto_failed').slice(0, 120),
    })
    return NextResponse.json(
