...
async function handleStatus(accountIdRaw) {
  ...
  const accountId = String(await profilePrimary.resolveCanonicalAccountId(rawAccountId) || rawAccountId).trim()
  ...
  if (!tgId) {
    tgId = await findLegacyRedisTelegramId(accountId)
      ...
      .catch(() => '')
    if (tgId) {
      await profilePrimary.assertTelegramLinkAvailable(accountId, tgId)
...
export async function POST(req) {
  try {
    ...
    const identityConflict = e?.code === 'IDENTITY_LINK_CONFLICT'
    return new Response(
      JSON.stringify({ ok: false, error: identityConflict ? 'IDENTITY_LINK_CONFLICT' : 'STATUS_FAILED' }),
      { status: identityConflict ? 409 : 500, headers: { 'content-type': 'application/json' } },
    )
  }
}
