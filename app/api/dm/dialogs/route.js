...
export async function GET(req) {
  try {
    ...
    const visibleItems = isQl7SupportActive() ? payload.items : (Array.isArray(payload.items) ? payload.items.filter((item) => !isQl7SupportId(item?.userId)) : [])
    await dmPrimary.markDeliveredForItems({ items: visibleItems, receiverIds })

    return ok({ ...payload, items: visibleItems, storagePrimary: 'mongo', supportActive: isQl7SupportActive() })
  } catch (e) {
