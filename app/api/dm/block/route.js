...
export async function POST(req) {
  ...
  try {
    ...
    if (isQl7SupportId(userId) && !isQl7SupportActive()) return bad('ql7_support_disabled', 404)
    if (isQl7SupportId(userId)) return bad('ql7_support_cannot_block', 403)

    await addAliasesFor(userId, [rawIdInput, rawId])
    const ids = await expandAliasIds([userId, rawIdInput, rawId])
    await dmPrimary.setBlock(me, ids)

    return ok({ userId, storagePrimary: 'mongo' })
  } catch (e) {
    return bad(e?.message || 'block_failed', e?.status || 500)
  }
}
