...
function exactAccountQuotaKey(date, accountId) {
  const exact = String(accountId || '').trim()
  if (!exact) return null
  const digest = createHash('sha256').update(exact).digest('hex').slice(0, 40)
  return `aiq:${date}:account:${digest}`
}

function getRawAccountIdFromReq(req, body) {
  const query = req.nextUrl.searchParams
  const queryId = query.get('id') || query.get('accountId')
  if (queryId) return String(queryId).trim() || null

  const bodyId = body?.accountId
  if (bodyId) return String(bodyId).trim() || null
  ...
  try {
    ...
    const pick = (name) =>
      cookieString
        ...
        ?.split('=')[1]
    return String(pick('asherId') || pick('accountId') || pick('wallet') || '').trim() || null
  } catch {
    return null
  }
}
...
async function readIdentityUsage({ date, ip, accountId, rawAccountId = '' }) {
  const ipKey = ipQuotaKey(date, ip)
  const accountKey = accountQuotaKey(date, accountId)
  const resolved = rawAccountId || accountId
    ? await identityContract.resolve(rawAccountId || accountId, {
        mode: 'ai-quota',
        source: 'app/api/aiquota/usage/route.js',
      }).catch(() => null)
    : null
  const readCandidates = quotaAccountReadCandidates(rawAccountId || accountId, [
    accountId,
    ...(resolved?.aliasSet || []),
    ...(resolved?.qcoinLookupOrder || []),
  ])
  const accountKeys = Array.from(new Set([
    accountKey,
    ...readCandidates.map((candidate) => exactAccountQuotaKey(date, candidate)),
  ].filter(Boolean)))
  const [rawIpUsed, rawAccountValues] = await Promise.all([
    redis.get(ipKey),
    Promise.all(accountKeys.map((key) => redis.get(key))),
  ])

  const ipUsedSec = clampQuotaUsed(rawIpUsed, AI_QUOTA_LIMIT_SEC)
  const accountUsedSec = Math.max(
    0,
    ...rawAccountValues.map((value) => clampQuotaUsed(value, AI_QUOTA_LIMIT_SEC)),
  )
...
export async function GET(req) {
  try {
    ...
    const rawAccountId = getRawAccountIdFromReq(req, null)
    const accountId = normalizeQuotaAccount(rawAccountId)
    ...
    const identity = await readIdentityUsage({ date, ip, accountId, rawAccountId })
    return NextResponse.json(
      quotaResponse({
        ip,
        date,
...
export async function POST(req) {
  ...
  try {
    ...
    const date = yyyymmdd()
    const ip = selectQuotaClientIp(req.headers)
    const rawAccountId = getRawAccountIdFromReq(req, body)
    const accountId = normalizeQuotaAccount(rawAccountId)
    const vip = await getVipStatus(req, accountId)
