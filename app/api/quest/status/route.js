...
// ---- ENV adapters (С‚Рµ Р¶Рµ РїСЂР°РІРёР»Р°, С‡С‚Рѕ РІ /api/quest/progress) ----
...
export async function GET(req) {
  try {
    ...
    // С‡РёС‚Р°РµРј РїСЂРѕРіСЂРµСЃСЃ Рё СѓС‡РёС‚С‹РІР°РµРј РѕР±Р° РєР»СЋС‡Р° вЂ” "1" Рё "quest-1"
    const prog = await readProgress(uid)
    const byNum  = prog?.[cardNum] || {}
    const byRaw  = prog?.[rawCardId] || {}
    // РІС‹Р±РёСЂР°РµРј В«Р±РѕР»РµРµ РїРѕР»РЅСѓСЋВ» Р·Р°РїРёСЃСЊ
    const chosen =
    ...
    return NextResponse.json({
      ok: true,
      cardId: cardNum,     // РЅРѕСЂРјР°Р»РёР·РѕРІР°РЅРЅС‹Р№ РЅРѕРјРµСЂ
      claimed,
      done,
      need,
    })
