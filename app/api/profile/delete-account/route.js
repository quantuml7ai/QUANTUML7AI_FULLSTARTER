...
const LATEST_WALLET_SESSION_PREFIX = 'wallet_session_latest:'
...
function latestWalletSessionKey(identity) {
  const value = clean(identity).toLowerCase()
  return value ? `${LATEST_WALLET_SESSION_PREFIX}${value}` : ''
}
...
async function readWalletStoreValue(key) {
  const redis = getRedis()
  if (redis) return redis.get(key)
  const row = walletSessionMemory.get(key)
  if (!row) return null
  if (Number(row.expiresAt || 0) <= Date.now()) {
    walletSessionMemory.delete(key)
    return null
  }
  return row.value || null
}
async function readWalletSession(token) { return readWalletStoreValue(walletSessionKey(token)) }
function latestSessionToken(record) {
  if (!record) return ''
  return clean(typeof record === 'string' ? record : record?.token)
}
function latestSessionStatus(record) {
...
async function requireVerifiedWalletSession(body = {}) {
  ...
  const latestKeys = Array.from(new Set([
    latestWalletSessionKey(session.walletAddress),
    latestWalletSessionKey(session.accountId),
  ].filter(Boolean)))
  const latestRecords = await Promise.all(latestKeys.map(readWalletStoreValue))
  if (latestRecords.some((record) => (
    record && (latestSessionToken(record) !== token || latestSessionStatus(record) !== 'active')
  ))) return null
  return {
    token,
...
export async function POST(req) {
  try {
    ...
    // The verified transport determines the deletion boundary. If both proofs
    // are present in a Telegram WebView, prefer the fresh TMA signature so a
    // cached Wallet session cannot turn a Telegram-side delete into a Wallet
    // account delete.
    const verifiedTelegramSession = await requireVerifiedTelegramSession(req, body)
    const verifiedSession = verifiedTelegramSession || await requireVerifiedWalletSession(body)
    ...
    const identityScope = verifiedSession.provider === 'telegram-mini-app' ? 'telegram' : 'wallet'
    const deletionSubjectId = identityScope === 'telegram'
      ? clean(verifiedSession.telegramId)
      : clean(verifiedSession.walletAddress || verifiedSession.accountId)
    if (!deletionSubjectId) return json({ ok: false, error: 'deletion_subject_missing' }, 401)
    ...
    const syntheticSmokeSeed = syntheticSmokeMode
      ? (
        ...
        extractSyntheticSmokeSeed(verifiedSession?.telegramId)
      )
      : ''

    const result = await accountDeletionPrimary.deleteAccount({
      accountId: deletionSubjectId,
      rawIds,
      ...
      requestMeta: {
        ...
        walletProvider: identityScope === 'wallet' ? verifiedSession.provider : null,
        telegramProvider: identityScope === 'telegram' ? verifiedSession.provider : null,
        canonicalAccountId: actorAccountId,
