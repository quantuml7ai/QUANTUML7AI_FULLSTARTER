...
import { resolveCanonicalAccountId, resolveCanonicalAccountIds } from '../_identity.js'
import { isOfficialProfile } from '../_officialStatus.js'
...
export async function GET(req) {
  try {
    ...
    const officialStatusPromise = isOfficialProfile({
      rawUserId,
      accountId,
      resolveConfiguredIds: resolveCanonicalAccountIds,
    }).catch(() => false)
    ...
    return NextResponse.json({
      ...
      officialStatus: officialStatus === true,
      lastActiveAt: Number(qcoinState?.lastActiveAt || 0),
      presenceOfflineAt: Number(qcoinState?.presenceOfflineAt || 0),
