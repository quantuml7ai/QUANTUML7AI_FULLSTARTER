...
export async function POST(req) {
  try {
    ...
    const nextGender = profilePrimary.normUserGender(body?.gender)
    ...
    const nextBirthYear = profilePrimary.normUserBirthYear(body?.birthYear)
    ...
    const currentProfile = await profilePrimary.getUserProfile(accountId)
    const saved = await profilePrimary.setUserNick(accountId, nick)
    const savedIcon = await profilePrimary.setUserAvatar(accountId, iconRaw)
    const savedGender = currentProfile?.gender
      ? String(currentProfile.gender)
      : (nextGender ? await profilePrimary.setUserGender(accountId, nextGender) : '')
    const savedBirthYear = Number(currentProfile?.birthYear || 0) || (
      nextBirthYear ? await profilePrimary.setUserBirthYear(accountId, nextBirthYear) : 0
    )

    await writeCanonicalAliases(accountId, [
      userId,
      body?.userId,
      rawUserId,
      sourceUserId,
