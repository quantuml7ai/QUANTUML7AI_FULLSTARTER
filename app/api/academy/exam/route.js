...
export async function POST(req) {
  try {
    ...
    if (wasCorrect) {
      ...
      const sourceEventId = `academy:exam:${uid}:${blockId}:${done}`
      await economicRoute.executeVerifiedEconomicOperation({
        routeId: 'academy.exam.reward',
        ...
        sourceOwner: 'app/api/academy/exam/route.js',
        sourceEvidence: { uid, blockId, done, correct, total, wasCorrect: true, vip: isVip === true, awarded },
        writer: (decisionReceipt) => qcoinPrimary.incrementBalance({
          uid,
          amount: awarded,
          eventKind: 'academy_exam_reward',
          route: '/api/academy/exam',
          economicRouteId: 'academy.exam.reward',
          sourceEventId,
          idempotencyKey: sourceEventId,
          decisionReceipt,
          meta: { blockId, correct, vip: isVip ? 1 : 0 },
        }),
      })
    }
    ...
    await academyPrimary.writeExamState({ userId: uid, blockId, aliases, state: {
      done,
      correct,
      cooldownUntil,
      updatedAt: now,
