...
async function resolvePaymentAccountId(accountId) {
  const normalized = normalizeAccountId(accountId)
  if (!normalized) return null
  try {
    return (await profilePrimary.resolveCanonicalAccountId(normalized)) || normalized
  } catch {
    return normalized
  }
}
...
export async function POST(req) {
  try {
    ...
    const accountId = await resolvePaymentAccountId(normalizedAccountId)
    ...
    if (purpose === 'vip') {
      ...
      const PRICE_USD = Number(process.env.PLAN_PRICE_USD || '19.99')
    ...
    const internalId = String(await paymentsPrimary.nextInvoiceId())
    ...
    if (purpose === 'vip' && externalId) {
      await paymentsPrimary.saveLegacySnapshot(`invoice:${externalId}`, {
