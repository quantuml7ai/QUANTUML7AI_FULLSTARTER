...
function extractVipAccountId(orderId) {
  try {
    ...
    if (lastColon === -1) {
      return canonicalUserId.normalizePrincipalSyntax(withoutPrefix) || null
    }
    const accountPart = withoutPrefix.slice(0, lastColon)
    return canonicalUserId.normalizePrincipalSyntax(accountPart) || null
  } catch {
    return null
  }
}
...
function parseAdsOrder(orderId) {
  ...
  return {
    type: 'ads',
    accountId: canonicalUserId.normalizePrincipalSyntax(accountIdRaw) || null,
    pkgType: pkgTypeRaw.trim().toUpperCase(),
    ts: tsPart,
  }
}

async function resolveHumanAccountId(raw) {
...
export async function POST(req) {
  ...
  try {
    ...
    const parsedVipAccountId = extractVipAccountId(orderId)
    const parsedAdsInfo = parseAdsOrder(orderId)
    const vipAccountId = parsedVipAccountId
      ? await resolveHumanAccountId(parsedVipAccountId)
      : null
    const adsInfo = parsedAdsInfo?.accountId
      ? {
          ...parsedAdsInfo,
          accountId: await resolveHumanAccountId(parsedAdsInfo.accountId),
        }
      : parsedAdsInfo
    ...
    if (!internalId && type === 'vip') {
      ...
      if (finished) {
        ...
        const vipSourceEventId = `pay:vip:legacy:${paymentId || externalInvoiceId || orderId}`
        await economicRoute.executeVerifiedEconomicOperation({
          routeId: 'vip.payment.activation', operationType: 'entitlement_extend', actorAccountId: vipAccountId, targetAccountId: vipAccountId,
          amount: days, entitlementId: 'vip', paymentProvider: 'nowpayments', paymentReceiptId: paymentId || externalInvoiceId || orderId,
          sourceEventId: vipSourceEventId, idempotencyKey: vipSourceEventId, sourceType: 'payment', sourceOwner: 'app/api/pay/webhook/route.js',
          sourceEvidence: { orderId, paymentId, externalInvoiceId, status, success: finished, days, legacy: true },
          writer: (decisionReceipt) => addVipDays(vipAccountId, days, { paymentId: paymentId || externalInvoiceId || '', economicRouteId: 'vip.payment.activation', operationType: 'entitlement_extend', idempotencyKey: vipSourceEventId, decisionReceipt }),
        })
    ...
    if (!internalId && type === 'ads') {
      ...
      const adsSourceEventId = `pay:ads:legacy:${paymentId || externalInvoiceId || orderId}`
      const adsEconomic = await economicRoute.executeVerifiedEconomicOperation({
        routeId: 'ads.payment.activation', operationType: 'entitlement_activate', actorAccountId: accountId, targetAccountId: accountId,
        packageId: pkgType, paymentProvider: 'nowpayments', paymentReceiptId: paymentId || externalInvoiceId || orderId,
        sourceEventId: adsSourceEventId, idempotencyKey: adsSourceEventId, sourceType: 'payment', sourceOwner: 'app/api/pay/webhook/route.js',
        sourceEvidence: { orderId, paymentId, externalInvoiceId, status: String(statusRaw || '').toLowerCase(), success: finished, pkgType, legacy: true },
        writer: (decisionReceipt) => grantAdsPackageForAccount({ accountId, pkgType, note: `legacy-webhook:${orderId}`, economicRouteId: 'ads.payment.activation', operationType: 'entitlement_activate', idempotencyKey: adsSourceEventId, decisionReceipt }),
      })
      const pkg = adsEconomic.result
    ...
    if (type === 'vip') {
      ...
      if (!invoice.activated && invoice.accountId && days > 0) {
        const vipSourceEventId = `pay:vip:${paymentId || externalInvoiceId || invoice.id}`
        await economicRoute.executeVerifiedEconomicOperation({
          routeId: 'vip.payment.activation', operationType: 'entitlement_extend', actorAccountId: invoice.accountId, targetAccountId: invoice.accountId,
          amount: days, entitlementId: 'vip', paymentProvider: 'nowpayments', paymentReceiptId: paymentId || externalInvoiceId || invoice.id,
          sourceEventId: vipSourceEventId, idempotencyKey: vipSourceEventId, sourceType: 'payment', sourceOwner: 'app/api/pay/webhook/route.js',
          sourceEvidence: { invoiceId: invoice.id, orderId: invoice.orderId || orderId, paymentId, externalInvoiceId, status: invoice.lastStatus, success: true, days },
          writer: (decisionReceipt) => addVipDays(invoice.accountId, days, { paymentId: paymentId || externalInvoiceId || '', economicRouteId: 'vip.payment.activation', operationType: 'entitlement_extend', idempotencyKey: vipSourceEventId, decisionReceipt }),
        })
    ...
    if (type === 'ads') {
      ...
      if (!invoice.activated && invoice.accountId && pkgType) {
        const adsSourceEventId = `pay:ads:${paymentId || externalInvoiceId || invoice.id}`
        const adsEconomic = await economicRoute.executeVerifiedEconomicOperation({
          routeId: 'ads.payment.activation', operationType: 'entitlement_activate', actorAccountId: invoice.accountId, targetAccountId: invoice.accountId,
          packageId: pkgType, paymentProvider: 'nowpayments', paymentReceiptId: paymentId || externalInvoiceId || invoice.id,
          sourceEventId: adsSourceEventId, idempotencyKey: adsSourceEventId, sourceType: 'payment', sourceOwner: 'app/api/pay/webhook/route.js',
          sourceEvidence: { invoiceId: invoice.id, orderId: invoice.orderId || orderId, paymentId, externalInvoiceId, status: invoice.lastStatus, success: true, pkgType },
          writer: (decisionReceipt) => grantAdsPackageForAccount({ accountId: invoice.accountId, pkgType, note: `invoice:${invoice.id}`, economicRouteId: 'ads.payment.activation', operationType: 'entitlement_activate', idempotencyKey: adsSourceEventId, decisionReceipt }),
        })
        pkg = adsEconomic.result
