...
import { isVipNowSideEffectFree } from '@/lib/subscriptions'
...
export async function POST(req) {
  try {
    ...
    // Р·Р°С‰РёС‚Р° РѕС‚ Р·Р»РѕСѓРїРѕС‚СЂРµР±Р»РµРЅРёСЏ
    const MAX = 250
    ...
    const pairs = await Promise.all(
      list.map(async (id) => {
        try {
          const vip = await isVipNowSideEffectFree(id) // { active, untilISO, daysLeft }
    ...
    const payload = { ok: true, count: list.length, map }
