...
const VIDEO_LINE_RE = /^(?:blob:[^\s]+|https?:\/\/[^\s]+(?:\.(?:webm|mp4|mov|m4v|mkv)(?:[?#].*)?|[?&]filename=[^&#]+\.(?:webm|mp4|mov|m4v|mkv)(?:$|[&#])|\/uploads\/video|\/forum\/video|\/api\/forum\/uploadVideo))/i
function textHasVideo(s) {
  if (!s) return false
  const lines = String(s).split(/\r?\n/).map((x) => x.trim()).filter(Boolean)
  return lines.some((line) => VIDEO_LINE_RE.test(line))
}
function videoUrlsFromText(s) {
  if (!s) return []
  return String(s).split(/\r?\n/).map((x) => x.trim()).filter((line) => line && VIDEO_LINE_RE.test(line))
}
...
const VIEW_BATCH_CONCURRENCY = 6

async function resolveViewBatch(ids, userId, incrementFn) {
  const out = []
  for (let offset = 0; offset < ids.length; offset += VIEW_BATCH_CONCURRENCY) {
    const chunk = ids.slice(offset, offset + VIEW_BATCH_CONCURRENCY)
    const rows = await Promise.all(chunk.map(async (id) => {
      const result = await incrementFn(id, userId)
      return { id, inc: !!result?.inc, views: result?.views }
    }))
    out.push(...rows)
  }
  return out
  ...
  try {
    ...
    for (const op of ops) {
      ...
      try {
        ...
        const publicationOps = new Set(['create_topic', 'create_post', 'edit_post'])
        const restrictedMutationOps = new Set(['create_topic', 'create_post', 'edit_post', 'delete_post', 'delete_topic', 'react', 'set_reaction'])
        if (restrictedMutationOps.has(opType)) {
          const actionId = (opType === 'react' || opType === 'set_reaction') ? 'forum.react' : 'forum.publish'
          const restriction = await restrictionGuard.guardBusinessAction({ accountId: userId, actionId })
          if (!restriction.allowed) {
            results.push({ op: opType, error: restriction.error || 'account_quarantined', status: restriction.status || 423, expiresAt: restriction.expiresAt || '', reasonCode: restriction.reasonCode || '', opId: op.opId })
            continue
          }
        }
        if (publicationOps.has(opType)) {
          const composerText = opType === 'create_topic'
            ? `${String(p.title || '')}
${String(p.description || '')}`.trim()
          ...
          composer = await composerGate.evaluateComposerSubmit({
            ...
            surface: 'forum',
            text: composerText,
            targeted: opType !== 'create_topic' && Boolean(p.parentId || p.id),
            clientMutationId: String(op.opId || p.cid || ''),
            locale: String(body?.locale || request.headers.get('x-forum-locale') || request.headers.get('accept-language') || 'und').split(',')[0].trim(),
            quotedRanges: Array.isArray(p?.quotedRanges) ? p.quotedRanges : [],
            conversationReferences: [String(p?.parentId || p?.topicId || p?.id || '')].filter(Boolean),
            context: { route: '/api/forum/mutate', operationType: opType, topicId: String(p?.topicId || p?.topicCid || '') },
          })
          if (!composer.allowed) {
            results.push({
              op: opType,
              ...
              securityCaseId: String(composer.securityCaseId || ''),
              opId: op.opId,
            })
            continue
          }
          composerDelivery = composerGate.createComposerDeliveryBinding(composer, {
            kind: opType === 'create_topic' ? 'forum_topic' : 'forum_post',
            operationId: String(op.opId || p.cid || composer.receipt?.decisionId || ''),
            entityId: opType === 'edit_post' ? String(p.id || '') : '',
          })
        }
        const prepareComposerPublication = async () => {
          if (!composer || composerPrepared) return
        ...
        if (op.type === 'create_topic') {
          ...
          const nickname  = trimStr(p.nickname || '', 80)
          const icon      = trimStr(p.icon || '', MAX_ICON)
          const posterCandidate = trimStr(p.posterUrl || '', 2048)
          const posterUrl = isR2PublicUrl(posterCandidate) && /\.(?:webp|jpe?g)(?:[?#].*)?$/i.test(posterCandidate)
            ? posterCandidate
            : ''
          ...
          let videoModerationStatus = ''
          await prepareComposerPublication()
          const videoUrls = videoUrlsFromText(text)
          if (videoUrls.length) {
            const videoApprovalToken = String(p.videoApprovalToken || '').trim()
            if (videoUrls.length !== 1 || !videoApprovalToken || !isR2PublicUrl(videoUrls[0])) {
              results.push({ op: 'create_post', error: 'video_moderation_approval_required', opId: op.opId })
              continue
            }
            try {
          ...
          const { post, rev } = await createPost({
            topicId, parentId, text, userId, nickname, icon, posterUrl, videoModerationStatus, ts: Date.now(), _geoOrigin: forumGeoOrigin,
            composerPolicyBinding: composerDelivery?.documentFields,
          })
...
try {
  await patchSnapshotPartial({ rev, patch: { posts: { [String(postId)]: { views } } } })
          ...
          let changed = false
          const rows = await resolveViewBatch(ids, userId, incrementTopicViewsUnique)
          for (const { id, inc, views } of rows) {
            viewsMap[id] = views
...
if (changed && isForumViewRealtimeEnabled()) {
  ...
  results.push({
    ...
    opId: op.opId,
          ...
          let changed = false
          const rows = await resolveViewBatch(ids, userId, incrementPostViewsUnique)
          for (const { id, inc, views } of rows) {
            viewsMap[id] = views
            if (Number.isFinite(Number(views))) {
...
try {
  await patchSnapshotPartial({
    ...
    patch: { posts: { [String(postId)]: { likes: r.likes, dislikes: r.dislikes } } }
        ...
        if (composer) {
          const opResults = results.slice(beforeResultCount)
          const sent = opResults.some((row) => String(row?.op || '') === opType && !row?.error && row?.duplicate !== true)
          if (sent) {
            const composerOutcome = await composerGate.commitComposerOutcome(composer, { sent: true })
            const last = results[results.length - 1]
            if (last && String(last.op || '') === opType) {
              last.composerDecision = composer.decision || 'ALLOW'
              last.composerClass = composer.classId || ''
              last.composerDecisionId = String(composer.receipt?.decisionId || '')
              last.warningCommitted = composerOutcome?.warningCommitted === true
              last.policyOutcomePending = composerOutcome?.policyOutcomePending === true
            }
          } else if (composerPrepared) {
            await composerGate.cancelComposerOutcome(composer, { reason: `forum_${opType}_definitive_not_sent` })
        ...
        if (composerPrepared && composer) {
          try {
            await composerGate.cancelComposerOutcome(composer, { reason: `forum_${String(op?.type || 'unknown')}_failed` })
          } catch (cancelError) {
            console.error('[FORUM_COMPOSER_OUTBOX_CANCEL_FAILED]', {
              op: String(op?.type || 'unknown'),
              code: String(cancelError?.code || cancelError?.message || 'cancel_failed').slice(0, 160),
            })
          }
        }
