    ...
    for (const op of ops) {
      try {
        ...
        if (op.type === 'create_topic') {
          ...
          // NEW: публикуем событие о новой теме
          ...
          // Публикуем событие для live-обновлений (без клиентской фильтрации по hasImages)
          ...
          // попробуем отдать свежий счётчик
          let views = null
          ...
          await publishForumEvent({
            type: 'view_topic',
            topicId,
            views, // может быть undefined, клиент переживёт
          })
          ...
          // === УНИКАЛЬНЫЕ РЕАКЦИИ: один пользователь = либо like, либо dislike (переключатель) ===
          // Снимаем противоположную реакцию и ставим текущую. Счётчики считаем по кардинальности множеств.
          ...
          try {
            ...
            // поддерживаем совместимость с существующими числовыми ключами
            try {
              await Promise.all([
