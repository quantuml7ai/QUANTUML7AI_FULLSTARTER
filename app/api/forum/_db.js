...
export async function createPost({ topicId, parentId, text, userId, nickname, icon, posterUrl = '', videoModerationStatus = '', ts, _geoOrigin }) {
  const result = await forumPrimary.createPost({ topicId, parentId, text, userId, nickname, icon, posterUrl, videoModerationStatus, ts, _geoOrigin })
...
export async function getMediaLockUntilForIdentity(userId) {
  ...
  const values = await Promise.all(ids.map((id) => getMediaLockUntil(id).catch(() => 0)))
  return values.reduce((max, value) => Math.max(max, parseIntSafe(value, 0)), 0)
}
