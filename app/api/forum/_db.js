...
export function normNick(raw) {
  ...
  return s.slice(0, 24) // максимум 24 символа (подправь при желании)
}
export function nickKeyLower(raw) {
  return normNick(raw).toLowerCase()
}

/** Проверка: свободен ли ник (без учёта владельца) */
export async function isNickFree(nick) {
  const key = K.nickIdx(nickKeyLower(nick))
  const owner = await redis.get(key)
  return !owner
}
/** Свободен ли ник для КОНКРЕТНОГО пользователя (разрешаем, если владелец = он же) */
...
/** Прочитать текущий ник пользователя */
 ...
 * - резервирует новый через SET NX
 */
export async function setUserNick(userId, newNick) {
  ...
  // если ник не меняется — ничего не делаем
  if (oldNick && nickKeyLower(oldNick) === nickKeyLower(nn)) {
    return oldNick
  }
  // попытка резервировать новый ник
  const ok = await redis.set(newKey, String(userId), { nx: true })
  if (!ok) throw new Error('nick_taken')

  // записываем ник юзеру и освобождаем старый (если владелец совпадает)
