...
async function hydrateUsersSafe(ids) {
  ...
  let hydrated = []
  try {
    // Canonical profile hydration performs verified identity reads. Do not
    // replace the whole page with anonymous cards just because a larger
    // followers page needs more than the former fixed 1800 ms deadline.
    hydrated = await getUsersPublicMini(cleanIds)
  } catch {
    hydrated = []
  }
