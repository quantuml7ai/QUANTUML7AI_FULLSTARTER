...
// NSFWJS resizes non-square tensors directly to 224x224. For portrait/landscape
// video frames that destroys camera geometry and can turn repeated texture/background
// patterns into correlated Porn spikes. Keep legacy image/avatar behavior unchanged,
// but fit VIDEO frames into the model square without stretching. Mid-gray padding is
// intentionally neutral; the returned tensor stays in 0..255 because NSFWJS performs
// its own /255 normalization.
const VIDEO_NSFW_INPUT_SIZE = 224;
function fitVideoNsfwTensor(imgTensor) {
  const [height, width, channels] = Array.isArray(imgTensor?.shape) ? imgTensor.shape : [];
  if (!height || !width || channels !== 3) return imgTensor;

  const size = VIDEO_NSFW_INPUT_SIZE;
  if (height === size && width === size) return imgTensor;

  const scale = Math.min(size / width, size / height);
  const targetWidth = Math.max(1, Math.min(size, Math.round(width * scale)));
  const targetHeight = Math.max(1, Math.min(size, Math.round(height * scale)));
  const padX = size - targetWidth;
  const padY = size - targetHeight;
  ...
  return tf.tidy(() => {
    const resized = tf.image.resizeBilinear(imgTensor.toFloat(), [targetHeight, targetWidth], true);
    return tf.pad(resized, [[top, bottom], [left, right], [0, 0]], 127.5);
  });
}
...
// --------------------
function nsfwDecision(scores, { video = false } = {}) {
  ...
  // Video uploads get a deliberately small Porn hard-block relaxation only.
  // 0.62 -> 0.69 (+11.3%), 0.95 -> 1.06 (+11.6%), 0.22 -> 0.25 (+13.6%).
  // REVIEW thresholds stay unchanged, so borderline frames are downgraded to
  // review evidence rather than silently treated as clean. Image/avatar moderation
  // keeps the legacy thresholds exactly. Hentai/gore/violence are untouched.
  const pornHardBlock = video ? 0.69 : 0.62;
  const pornComboTotal = video ? 1.06 : 0.95;
  const pornComboFloor = video ? 0.25 : 0.22;
  ...
  if (Porn >= pornHardBlock) return { decision: 'block', reason: 'porn' };
  ...
  if (ps >= pornComboTotal && Porn >= pornComboFloor) return { decision: 'block', reason: 'porn' };
...
// model/reasons/STRICT policy and all non-Porn categories remain unchanged. Video
// Porn hard-block thresholds are only modestly relaxed (10-15%); review thresholds
// stay unchanged. A video-level
...
export async function POST(req) {
  ...
  try {
    ...
    for (const f of files) {
      ...
      const nsfwTensor = isVideoModeration ? fitVideoNsfwTensor(imgTensor) : imgTensor;

      let nsfw = null;
      try {
        const preds = await model.classify(nsfwTensor);
        nsfw = {};
        for (const p of (preds || [])) {
          nsfw[p.className] = clamp01(p.probability);
        }
      } finally {
        if (nsfwTensor !== imgTensor) nsfwTensor.dispose?.();
        imgTensor.dispose?.();
      }

      const nsfwRes = nsfwDecision(nsfw || {}, { video: isVideoModeration });
