...
import { notifyQl7PurchaseStatus, notifyQl7QcoinCredited } from '../../../../../lib/ql7-support/events.js'
...
export async function POST(req) {
  ...
  try {
    ...
    if (!success) {
      ...
      await saveInvoice(invoice)
      await notifyQl7PurchaseStatus({ userId: invoice.accountId, userAliases: [invoice.rawAccountId, invoice.accountId], status: invoice.status, product: 'qcoin', amount: invoice.qcoinAmount, currency: invoice.currency, invoiceId: invoice.internalId, paymentId: paymentId || invoice.paymentId || '', occurredAt: invoice.updatedAt }).catch(() => {})
      return NextResponse.json({
        ok: true,
        type: 'qcoin_topup',
        activated: false,
    ...
    if (underpayFiat === null || underpayFiat > UNDERPAY_TOLERANCE_FIAT) {
      ...
      await saveInvoice(invoice)
      await notifyQl7PurchaseStatus({ userId: invoice.accountId, userAliases: [invoice.rawAccountId, invoice.accountId], status: 'pending', product: 'qcoin', amount: invoice.qcoinAmount, currency: invoice.currency, invoiceId: invoice.internalId, paymentId: paymentId || invoice.paymentId || '', reasonCode: 'underpaid', occurredAt: invoice.updatedAt }).catch(() => {})
      return NextResponse.json({
        ok: true,
        type: 'qcoin_topup',
        activated: false,
    ...
    if (!locked) {
      await notifyQl7PurchaseStatus({ userId: invoice.accountId, userAliases: [invoice.rawAccountId, invoice.accountId], status: 'pending', product: 'qcoin', amount: invoice.qcoinAmount, currency: invoice.currency, invoiceId: invoice.internalId, paymentId: paymentId || invoice.paymentId || '', reasonCode: 'busy', occurredAt: invoice.updatedAt }).catch(() => {})
    ...
    try {
      ...
      let newBalance
      try {
        const sourceEventId = `topup:webhook:${invoice.internalId}:${paymentId || externalInvoiceId || invoice.internalId}`
        const idempotencyKey = `qcoin:topup:paid:${paymentId || externalInvoiceId || invoice.internalId}`
        const economic = await economicRoute.executeVerifiedEconomicOperation({
          routeId: 'qcoin.topup.webhook',
          operationType: 'credit',
          actorAccountId: invoice.accountId,
          targetAccountId: invoice.accountId,
          amount: credit,
          paymentProvider: 'nowpayments',
          paymentReceiptId: paymentId || externalInvoiceId || invoice.internalId,
          sourceEventId,
          idempotencyKey,
          sourceType: 'payment',
          sourceOwner: 'app/api/qcoin/topup/webhook/route.js',
          sourceEvidence: { invoiceId: invoice.internalId, externalId: invoice.externalId, paymentId: paymentId || invoice.paymentId || '', priceAmount: invoice.priceAmount, currency: invoice.currency, status: invoice.lastStatus, underpayFiat },
          writer: (decisionReceipt) => qcoinPrimary.incrementBalance({
            ...
            economicRouteId: 'qcoin.topup.webhook',
            sourceEventId,
            idempotencyKey,
            decisionReceipt,
            meta: { invoiceId: invoice.internalId, externalId: invoice.externalId, paymentId: paymentId || invoice.paymentId || '', priceAmount: invoice.priceAmount, currency: invoice.currency, status: invoice.lastStatus },
          }),
        })
        ...
        await notifyQl7PurchaseStatus({ userId: invoice.accountId, userAliases: [rawInvoiceAccountId, invoice.rawAccountId, invoice.accountId], status: 'credit_failed', product: 'qcoin', amount: credit, currency: invoice.currency, invoiceId: invoice.internalId, paymentId: paymentId || invoice.paymentId || '', reasonCode: 'credit_failed', occurredAt: invoice.updatedAt }).catch(() => {})
        return NextResponse.json({
          ok: false,
      ...
      await notifyQl7PurchaseStatus({ userId: invoice.accountId, userAliases: [rawInvoiceAccountId, invoice.rawAccountId, invoice.accountId], status: 'paid', product: 'qcoin', amount: credit, currency: invoice.currency, invoiceId: invoice.internalId, paymentId: paymentId || invoice.paymentId || '', occurredAt: nowIso }).catch(() => {})
