...
const uid = requireUserId(req)
  // ADD: VIP-флаг + эффективная скорость
  ...
  if (effectiveActive && !s.paused) {
    ...
    if (wholeSeconds > 0) {
      ...
      addedBalance = wholeSeconds * RATE   // ADD: удвоенное начисление для VIP
  ...
  return NextResponse.json({
    ...
    addedSeconds,
    added: addedBalance,
    incPerSec: RATE,     // ADD: чтобы фронт знал свою реальную скорость
    effectiveActive,
    anyClientAlive,
    graceMs: GRACE_MS,
    vip: isVip ? 1 : 0,
