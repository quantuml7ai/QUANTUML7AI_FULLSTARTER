...
export async function POST(req) {
  const uid = (req.headers.get('x-forum-user-id') || '').trim()
  if (!uid) return NextResponse.json({ ok:false, error:'NO_USER' }, { status:400 })

  let body = {}
  try { body = await req.json() } catch {}
  const now = Number(body?.now || Date.now())
  const activeFlag = !!body?.active
  const cid = (req.headers.get('x-forum-client-id') || '').trim()
