...
const ACADEMY_VIP_STATUS_TTL_MS = 5 * 60 * 1000
const ACADEMY_EXAM_STATE_TTL_MS = 5 * 60 * 1000
const ACADEMY_RUNTIME_CACHE_LIMIT = 64
const academyVipStatusCache = new Map()
const academyVipStatusInflight = new Map()
const academyExamStateCache = new Map()
const academyExamStateInflight = new Map()

function academyRuntimeAccountKey(accountId) {
  return String(accountId || '').trim().toLowerCase()
}

function academyExamStateKey(blockId, accountId, isVip) {
  return [academyRuntimeAccountKey(accountId) || 'guest', Number(blockId) || 0, isVip ? 'vip' : 'base'].join(':')
}

function setBoundedAcademyCache(cache, key, value) {
  cache.delete(key)
  cache.set(key, value)
  while (cache.size > ACADEMY_RUNTIME_CACHE_LIMIT) {
...
function ql7PrimeVipStatus(accountId, status) {
  const key = academyRuntimeAccountKey(accountId)
  if (!key) return
  const isVip = !!(status && status.isVip)
  const untilISO = (status && status.untilISO) || null
  const untilMs = untilISO ? Date.parse(untilISO) : NaN
  const ttlExpiresAt = Date.now() + ACADEMY_VIP_STATUS_TTL_MS
  const expiresAt = isVip && Number.isFinite(untilMs)
    ? Math.min(ttlExpiresAt, Math.max(Date.now(), untilMs))
    : ttlExpiresAt
  setBoundedAcademyCache(academyVipStatusCache, key, {
    value: {
      isVip: isVip,
      untilISO: untilISO,
    },
...
function ql7InvalidateVipStatus(accountId) {
  const key = academyRuntimeAccountKey(accountId)
  if (key) academyVipStatusCache.delete(key)
}

async function ql7FetchVipStatus(accountId, options) {
  if (!accountId) return { isVip: false, untilISO: null }
  const key = academyRuntimeAccountKey(accountId)
  const force = !!(options && options.force)
  const cached = academyVipStatusCache.get(key)
  if (!force && cached && cached.expiresAt > Date.now()) return cached.value

  const pending = academyVipStatusInflight.get(key)
  ...
  const request = (async function () {
    ...
    if (!r.ok) throw new Error((j && j.error) || 'vip_status_error')
    const value = { isVip: !!(j && j.isVip), untilISO: (j && j.untilISO) || null }
    ql7PrimeVipStatus(accountId, value)
    return value
  })()

  academyVipStatusInflight.set(key, request)
  try {
    return await request
  } catch (e) {
    return { isVip: false, untilISO: null }
  } finally {
    if (academyVipStatusInflight.get(key) === request) academyVipStatusInflight.delete(key)
  }
}

function ql7PrimeAcademyExamState(blockId, accountId, isVip, snapshot) {
  const key = academyExamStateKey(blockId, accountId, isVip)
  setBoundedAcademyCache(academyExamStateCache, key, {
    value: snapshot,
    expiresAt: Date.now() + ACADEMY_EXAM_STATE_TTL_MS,
  })
}

async function ql7FetchAcademyExamState(blockId, accountId, isVip, options) {
  const key = academyExamStateKey(blockId, accountId, isVip)
  const force = !!(options && options.force)
  const cached = academyExamStateCache.get(key)
  if (!force && cached && cached.expiresAt > Date.now()) return cached.value

  const pending = academyExamStateInflight.get(key)
  if (pending) return pending

  const request = (async function () {
    const headers = {}
    if (accountId) headers['x-forum-user-id'] = String(accountId)
    headers['x-forum-vip'] = isVip ? '1' : '0'

    const res = await fetch(
      '/api/academy/exam?blockId=' + encodeURIComponent(blockId),
      {
        method: 'GET',
        headers: headers,
        cache: 'no-store',
    ...
    if (res.status === 401) {
      const value = { unauthorized: true, data: null }
      ql7PrimeAcademyExamState(blockId, accountId, isVip, value)
      return value
    }
    ...
    const value = { unauthorized: false, data: data }
    ql7PrimeAcademyExamState(blockId, accountId, isVip, value)
    return value
  })()

  academyExamStateInflight.set(key, request)
  try {
    return await request
  } finally {
    if (academyExamStateInflight.get(key) === request) academyExamStateInflight.delete(key)
  }
}
...
export default function AcademyExamBlock({ blockId }) {
  ...
  useEffect(function () {
    ...
    const id = setInterval(function () {
      ...
      setNow(nextNow)
      if (nextNow >= cooldownUntil) clearInterval(id)
    }, 1000)
    return function () {
      clearInterval(id)
    }
  ...
  async function fetchExamState(blockIdArg, accId, isVipArg, options) {
    ...
    try {
      ...
      const snapshot = await ql7FetchAcademyExamState(
        blockIdArg,
        accId,
        isVipArg,
  ...
  // Все чекпоинты Академии разделяют один VIP probe. Событие обновляет их
  // локальное отображение, а single-flight ниже оставляет только один запрос.
  useEffect(
    function () {
      if (typeof window === 'undefined') return undefined

      function handleVipRefresh(event) {
        const detail = (event && event.detail) || {}
        const acc = detail.accountId || ql7ReadAccountId()
        if (!acc) return
        setAccountId(acc)

        if (typeof detail.isVip === 'boolean') {
          const nextVip = !!detail.isVip
          ql7PrimeVipStatus(acc, { isVip: nextVip, untilISO: detail.untilISO || null })
          setVip(nextVip)
          fetchExamState(blockId, acc, nextVip, { silent: true })
          return
        }

        ql7InvalidateVipStatus(acc)
        ;(async function () {
          const st = await ql7FetchVipStatus(acc, { force: true })
          const nextVip = !!(st && st.isVip)
          setVip(nextVip)
          await fetchExamState(blockId, acc, nextVip, { silent: true })
        })()
      }

      window.addEventListener('vip:refresh', handleVipRefresh)
      return function () {
        window.removeEventListener('vip:refresh', handleVipRefresh)
      }
    },
    [blockId]
  )
  ...
  const handleVipClick = async function () {
    try {
      ...
      if (paymentMethod.method === 'qcoin') {
        ql7PrimeVipStatus(acc, { isVip: true, untilISO: null })
        setVip(true)
        try {
          window.dispatchEvent(new CustomEvent('vip:refresh', {
            detail: { accountId: acc, isVip: true },
          }))
        } catch (e) {}
        return
      }
  ...
  const handleAnswer = async function (letter) {
    ...
    try {
      ...
      ql7PrimeAcademyExamState(blockId, accountId, vip, {
        unauthorized: false,
        data: data,
      })
  ...
  return (
    <section className="QL7-exam" data-ql7-visual-scope="panel">
