...
import { extractVideoFrames } from '../forum/features/media/utils/moderationPrep'
import { buildModerationFormData, ensureModerationResponse } from '../forum/features/moderation/utils/http'
import useForumToast from '../forum/shared/hooks/useForumToast'
import useForumModerationUi from '../forum/features/moderation/hooks/useForumModerationUi'
import { prepareForumVideoPreviewIfNeeded } from '../../lib/forumClientVideoOptimizer'
import { preparePreparedVideoArtifacts } from '../forum/features/media/services/moderatePreparedVideoForUpload'
...
function makeEmptyCreative() {
  return {
    ...
    videoPoster: null,
    videoPosterPreviewUrl: null,
    imagePreviewUrl: null,
    videoPreparedOnSelect: false,
...
function CampaignCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      data-ql7-render-managed="1" data-ql7-visual-scope="card"
    >
...
export default function AdsHome({ onExitCabinet }) {
  const { t } = useI18n()
  const moderationToast = useForumToast()
  const { toastI18n: moderationToastI18n, reasonKey: moderationReasonKey } = useForumModerationUi({
    t,
    toast: moderationToast,
  })
  ...
  const creativePreviewAbortRef = useRef(null)
  const creativeSelectionGenerationRef = useRef(0)
  const adsVideoModerationJobsRef = useRef(new Set())
  const adsVideoModerationTimersRef = useRef(new Set())
  const [creativePreparingVideo, setCreativePreparingVideo] = useState(false)
...
const [campaignMetrics, setCampaignMetrics] = useState({})
  ...
  const cancelCreativePipeline = () => {
    creativeSelectionGenerationRef.current += 1
    try { creativePreviewAbortRef.current?.abort?.() } catch {}
    try { creativeUploadAbortRef.current?.abort?.() } catch {}
    creativePreviewAbortRef.current = null
    creativeUploadAbortRef.current = null
    setCreativePreparingVideo(false)
  ...
  useEffect(() => () => {
    try { creativePreviewAbortRef.current?.abort?.() } catch {}
    try { creativeUploadAbortRef.current?.abort?.() } catch {}
    for (const timer of adsVideoModerationTimersRef.current) {
      try { clearTimeout(timer) } catch {}
    }
    adsVideoModerationTimersRef.current.clear()
  }, [])
  ...
  // Campaign creation is never awaited on moderation. Sampling starts only after
  // campaignCreate returned a real id, so HEVC/transcode/upload stays untouched.
  const runAdsVideoModeration = async ({ campaignId, source, fallbackSource = '', mediaUrl = '', retryCount = 0 } = {}) => {
    const entityId = String(campaignId || '').trim()
    const actorId = String(accountId || '').trim()
    const moderationMediaUrl = String(mediaUrl || fallbackSource || (typeof source === 'string' ? source : '') || '').trim()
    if (!entityId || !actorId || !source || !moderationMediaUrl) return
    const jobKey = `ads:${entityId}:${moderationMediaUrl}`
    if (adsVideoModerationJobsRef.current.has(jobKey)) return
    adsVideoModerationJobsRef.current.add(jobKey)
    ...
    try {
      let frames = []
      const sourceCandidates = [source, fallbackSource]
        .filter(Boolean)
      ...
      if (frames.length >= 5) {
        ...
        const clientRequestId = `ads-video-${entityId}`
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          try {
            ...
            resultStatus = String(checked?.videoModeration?.status || '').trim().toLowerCase() || 'pending'
            resultReason = String(checked?.reason || checked?.videoModeration?.reason || 'unknown')
            lockedUntil = Number(checked?.videoModeration?.lockedUntil || 0) || 0
            break
          } catch {
      ...
      if (resultStatus === 'rejected' || resultStatus === 'gone') {
        await reloadCabinet({ silent: true })
        if (resultStatus === 'rejected') {
          try { moderationToastI18n('warn', 'forum_image_blocked') } catch {}
          try { moderationToastI18n('info', moderationReasonKey(resultReason)) } catch {}
          void lockedUntil
        }
        return
      }
    } finally {
      adsVideoModerationJobsRef.current.delete(jobKey)
    }

    const nextRetry = Math.min(12, Number(retryCount || 0) + 1)
    const waitMs = Math.min(60000, 12000 + (nextRetry * 5000))
    const timer = setTimeout(() => {
      adsVideoModerationTimersRef.current.delete(timer)
      void runAdsVideoModeration({ campaignId: entityId, source, fallbackSource, mediaUrl: moderationMediaUrl, retryCount: nextRetry })
    }, waitMs)
    adsVideoModerationTimersRef.current.add(timer)
  }
  ...
  useEffect(() => {
    for (const campaign of (Array.isArray(campaigns) ? campaigns : [])) {
      const campaignId = String(campaign?.id || campaign?.campaignId || '').trim()
      if (!campaignId) continue
      const creativeVideos = (Array.isArray(campaign?.creatives) ? campaign.creatives : [])
        .filter((item) => String(item?.mediaType || '').toLowerCase() === 'video')
      const videos = creativeVideos.length
        ? creativeVideos
        : (String(campaign?.mediaType || '').toLowerCase() === 'video' ? [campaign] : [])
      videos.forEach((video) => {
        const mediaUrl = String(video?.mediaUrl || '').trim()
        const hasVideoModerationStatus = !!video && Object.prototype.hasOwnProperty.call(video, 'moderationStatus')
        const hasCampaignModerationStatus = !!campaign && Object.prototype.hasOwnProperty.call(campaign, 'moderationStatus')
        const explicitStatus = String(
          hasVideoModerationStatus
            ? video?.moderationStatus
            : (hasCampaignModerationStatus ? campaign?.moderationStatus : ''),
        ).trim().toLowerCase()
        // Missing moderationStatus is the stable marker for a campaign that predates
        // video moderation. If the property exists but is malformed, stay fail-closed.
        const status = hasVideoModerationStatus || hasCampaignModerationStatus
          ? (['pending', 'approved'].includes(explicitStatus) ? explicitStatus : 'pending')
          : 'approved'
        if (!mediaUrl || status !== 'pending') return
        void runAdsVideoModeration({ campaignId, source: mediaUrl, mediaUrl })
      })
    }
    // runAdsVideoModeration intentionally owns de-duplication/retry state.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accountId, campaigns])
  ...
  async function uploadMediaForCreative(cr, { signal } = {}) {
    ...
    if (videoFile) {
      ...
      return { mediaUrl, mediaType, posterUrl: String(res?.posterUrl || ''), videoApprovalToken: String(res?.videoApprovalToken || '') }
    }
  ...
  const handleCreateCampaign = async () => {
    setNewError(null)
    if (creativePreparingVideo) return
    if (!hasActivePkg) {
      setNewError(
        TX(
          t,
    ...
    try {
      ...
      if (cleanedCreative.videoFile && !cleanedCreative.videoPreparedOnSelect) {
        const { ok, seconds } = await checkVideoDuration(cleanedCreative.videoFile)
        if (!ok) {
          setCreating(false)
          setNewError(
            TX(
              ...
              `Видео длиннее 3 минут (${Math.round(seconds)} сек) — сократи, пожалуйста.`
            )
          )
          return
        }
      ...
      const { mediaUrl, mediaType, posterUrl, videoApprovalToken } = await uploadMediaForCreative(
        cleanedCreative,
      ...
      revokeCreativePreviewUrl(creative.videoPreviewUrl)
      revokeCreativePreviewUrl(creative.videoPosterPreviewUrl)
      revokeCreativePreviewUrl(creative.imagePreviewUrl)
      setNewName('')
...
// Общие lifetime-метрики по всем кампаниям для верхних таблеток.
// Этот контур намеренно не зависит от range: нижний переключатель периода не должен менять верхнюю сводку.
useEffect(() => {
    ...
    try {
      ...
      await Promise.all(
        campaigns.map(async (c) => {
          ...
          const starts = [c.startsAt, c.createdAt, c.updatedAt]
            .map((value) => new Date(value || 0).getTime())
            .filter((value) => Number.isFinite(value) && value > 0)
          const fromMs = starts.length
            ? Math.max(0, Math.min(...starts) - 86400_000)
            : Date.UTC(2024, 0, 1)
          const from = new Date(fromMs).toISOString()
          ...
          try {
            ...
            console.error(
              '[ADS] lifetime analytics error for campaign',
              id,
              e,
            )
          }
      ...
      console.error('[ADS] lifetime analytics error', e)
    }
  }
...
// Метрики по каждой кампании для списка остаются привязанными к нижнему period selector.
useEffect(() => {
  if (!Array.isArray(campaigns) || campaigns.length === 0) {
    setCampaignMetrics({})
    return
  ...
  const loadCampaignMetrics = async () => {
    try {
      ...
      await Promise.all(
        campaigns.map(async (c) => {
          ...
          try {
            ...
            metricsById[id] = { impressions: imps, clicks, ctr }
          } catch (e) {
            console.error('[ADS] campaign metrics error for campaign', id, e)
          }
        }),
      )
      ...
      console.error('[ADS] campaign metrics error', e)
    }
  }
  ...
  const revokeCreativePreviewUrl = (value) => {
  ...
  const handleVideoFileChange = async (e) => {
    ...
    setNewError(null)
    creativeSelectionGenerationRef.current += 1
    const generation = creativeSelectionGenerationRef.current
    try { creativePreviewAbortRef.current?.abort?.() } catch {}
    const controller = new AbortController()
    creativePreviewAbortRef.current = controller
    ...
    let createdUrl = null
    let releaseHeavyActivity = null
    const ensureHeavyActivity = () => {
      if (!releaseHeavyActivity) releaseHeavyActivity = enterVideoHeavyActivity()
    }
    const isStale = () => controller.signal.aborted || creativeSelectionGenerationRef.current !== generation

    try {
      const prepared = await prepareForumVideoPreviewIfNeeded(file, {
        maxDurationSeconds: MAX_VIDEO_SECONDS,
        ...
        onProgress: (event) => {
          ...
          if (!progressStarted) {
            ...
            setCreativePreparingVideo(true)
          }
          setCreativeProgress((previous) => {
            const mapped = mapVideoPrepareProgress(event, previous.percent)
            return { active: true, phase: mapped.phase, percent: mapped.percent }
      ...
      const needsPreparedPreview = Boolean(prepared?.needsPreparedPreview)
      let previewFile = file
      let durationSec = null
      let decoderPath = ''

      if (needsPreparedPreview) {
        previewFile = prepared?.file
        durationSec = Number(prepared?.result?.durationSec || 0)
        decoderPath = String(prepared?.decoderPath || '')
        if (!(previewFile instanceof Blob)) throw new Error('HEVC_PREVIEW_MISSING_FILE')
      } else {
        const checked = await checkVideoDuration(file)
        if (isStale()) return
        if (!checked.ok) {
          const err = new Error('VIDEO_TOO_LONG')
          err.code = 'VIDEO_TOO_LONG'
          err.seconds = checked.seconds
          throw err
        }
        durationSec = Number(checked.seconds || 0)
      }

      createdUrl = URL.createObjectURL(previewFile)
      if (isStale()) {
        revokeCreativePreviewUrl(createdUrl)
        createdUrl = null
        return
      }

      let videoPoster = null
      let videoPosterPreviewUrl = null
      if (needsPreparedPreview) {
        ensureHeavyActivity()
        const artifacts = await preparePreparedVideoArtifacts({
          file: previewFile,
          surface: 'ads',
          userId: String(accountId || ''),
          signal: controller.signal,
          mirrorX: false,
          sourceName: previewFile?.name || file?.name || 'ad-video.mp4',
          onProgress: (event) => {
            if (isStale()) return
            setCreativeProgress((previous) => {
              const mapped = mapVideoPrepareProgress(event, previous.percent)
        ...
        if (isStale()) return
        videoPoster = artifacts?.poster || null
        if (!(videoPoster?.blob instanceof Blob) || !videoPoster.blob.size) throw new Error('VIDEO_POSTER_MISSING')
        videoPosterPreviewUrl = URL.createObjectURL(videoPoster.blob)
      }

      updateCreative({
        videoFile: previewFile,
        videoPreviewUrl: createdUrl,
        videoPoster,
        videoPosterPreviewUrl,
        videoPreparedOnSelect: needsPreparedPreview,
        videoDurationSec: durationSec,
        videoDecoderPath: decoderPath,
      })
      createdUrl = null

      if (needsPreparedPreview) {
        setCreativeProgress((previous) => {
          const mapped = mapVideoReadyProgress(previous.percent)
          return { active: true, phase: mapped.phase, percent: mapped.percent }
        })
      }
    } catch (error) {
      if (createdUrl) revokeCreativePreviewUrl(createdUrl)
      if (isStale() || error?.name === 'AbortError') return
      const seconds = Number(error?.details?.durationSeconds || error?.details?.durationSec || error?.seconds || 0)
      if (error?.code === 'VIDEO_TOO_LONG' || error?.message === 'VIDEO_TOO_LONG') {
        setNewError(
          TX(
            t,
            'ads_new_err_video_too_long',
        ...
        setNewError(TX(t, 'ads_new_err_video_prepare', 'Не удалось подготовить видео. Попробуй другой файл.'))
      }
    } finally {
      try { releaseHeavyActivity?.() } catch {}
      if (creativePreviewAbortRef.current === controller) creativePreviewAbortRef.current = null
      if (creativeSelectionGenerationRef.current === generation) {
        setCreativePreparingVideo(false)
        if (progressStarted) {
          setTimeout(() => {
            if (creativeSelectionGenerationRef.current !== generation) return
            setCreativeProgress({ active: false, phase: 'preparing', percent: 0 })
          }, 350)
        }
      }
  ...
  return (
    <div className="page-content ads-page-root" data-ql7-render-managed="1" data-ql7-visual-scope="route">
      {moderationToast.view}
      <style jsx global>{`
        ...
        .qft_toast.ok{ border-color:rgba(70,220,130,.5); }
        .qft_toast.warn{ border-color:rgba(255,200,80,.5); }
        .qft_toast.err{ border-color:rgba(255,90,90,.5); }
        .ads-analytics-preview{ position:relative; }
        .ads-video-moderation-badge{
          ...
          pointer-events:none; user-select:none; animation:adsVideoModerationPulse 1.9s ease-in-out infinite;
        }
        @keyframes adsVideoModerationPulse{ 0%,100%{opacity:.92;filter:brightness(.96)} 50%{opacity:1;filter:brightness(1.16)} }
        @media (prefers-reduced-motion: reduce){ .ads-video-moderation-badge{animation:none;} }
      `}</style>
      <main className="page-center ads-home">
        ...
        {/* ===== HEADER-УРОВЕНЬ ===== */}
        <section className="panel ads-panel ads-header-panel" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
        ...
        <section className="panel ads-panel" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
          ...
          <div className="ads-form-grid">
            <div className="ads-form-left">
                ...
                <label className="ads-field mini">
                  ...
                  <div className="ads-file-control">
                    ...
                    <input
                      ...
                      disabled={creating || creativePreparingVideo}
                    />
                  </div>
                </label>
              ...
              <div className="ads-create-progress">
                <MediaPipelineProgress
                  ...
                  theme="ads"
                  onCancel={(creating || creativePreparingVideo) ? cancelCreativePipeline : undefined}
            ...
            <div className="ads-form-preview">
              <div className="ads-preview-header">
                <span className="qcoinLabel ql7QCoinGoldLabel">
                  {TX(
                    t,
              ...
              <div className="ads-preview-body">
                {primaryCreative.videoPreviewUrl ? (
                  <video
                    ...
                    poster={primaryCreative.videoPosterPreviewUrl || undefined}
...
{filteredCampaigns.map((c, idx) => {
 ...
 >
                          ...
                          selectedCampaign.mediaUrl ? (
                            <>
                            {String((Array.isArray(selectedCampaign?.creatives) ? selectedCampaign.creatives.find((item) => String(item?.mediaUrl || '') === String(selectedCampaign?.mediaUrl || ''))?.moderationStatus : '') || selectedCampaign?.moderationStatus || '').toLowerCase() === 'pending' && (
                              <span className="ads-video-moderation-badge" aria-label="moderation">moderation</span>
                            )}
                            <video
                              src={selectedCampaign.mediaUrl}
                              poster={selectedCampaign.posterUrl || undefined}
                              autoPlay
                              muted
                              loop
                              playsInline
