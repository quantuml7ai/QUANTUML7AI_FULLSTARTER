...
const [campaignMetrics, setCampaignMetrics] = useState({})
  ...
  async function uploadMediaForCreative(cr, { signal } = {}) {
    ...
    if (videoFile) {
      ...
      const res = await uploadR2MediaFile({
        ...
        signal,
        videoPoster: cr.videoPoster || null,
        videoPolicy: {
          mode: 'video-required',
      ...
      return { mediaUrl, mediaType, posterUrl: String(res?.posterUrl || '') }
    }
  ...
  const handleCreateCampaign = async () => {
    ...
    try {
      ...
      const { mediaUrl, mediaType, posterUrl } = await uploadMediaForCreative(
        cleanedCreative,
      ...
      const createdCampaignId = String(j?.campaignId || j?.campaign?.id || j?.campaign?.campaignId || '').trim()
      if (cleanedCreative.videoFile && createdCampaignId) {
        void runAdsVideoModeration({
          campaignId: createdCampaignId,
          source: cleanedCreative.videoFile,
          fallbackSource: mediaUrl,
          mediaUrl,
        })
      }
...
useEffect(() => {
  ...
  const handleVideoFileChange = async (e) => {
    ...
    try {
      ...
      const videoPoster = await createNativeVideoPoster({
        source: previewFile,
        mirrorX: false,
        signal: controller.signal,
        sourceName: previewFile?.name || file?.name || 'ad-video',
      })
      if (isStale()) return
      const videoPosterPreviewUrl = URL.createObjectURL(videoPoster.blob)
  ...
  return (
    <div className="page-content ads-page-root" data-ql7-visual-scope="route">
      ...
      <main className="page-center ads-home">
        ...
        {/* ===== HEADER-УРОВЕНЬ ===== */}
        <section className="panel ads-panel ads-header-panel" data-ql7-visual-scope="panel">
        ...
        <section className="panel ads-panel" data-ql7-visual-scope="panel">
