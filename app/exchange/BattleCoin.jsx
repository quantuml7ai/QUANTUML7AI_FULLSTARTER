...
import {
  BATTLECOIN_MARKET_STREAM_BASE,
  BATTLECOIN_MARKET_STREAM_ENABLED,
  BATTLECOIN_MARKET_STREAM_RECONCILE_MS,
  battleCoinMarketReconnectDelay,
  readBattleCoinTickerPrice,
} from '../../lib/battlecoin/battlecoin-market-runtime'
...
export function isIOSBrowserRuntime(navigatorLike = typeof navigator !== 'undefined' ? navigator : null) {
...
export function shouldReprimeGuestMarketScroller({
  auth = false,
  loading = true,
  symbolCount = 0,
...
const BATTLECOIN_SELECTED_PRICE_POLL_MS = 1000
const BATTLECOIN_SELECTED_PRICE_TIMEOUT_MS = 3500

// Text-only live price leaf: it updates one text node, never the BattleCoin root.
// The surrounding premium DOM stays owned by BattleCoin so styled-jsx scoping is preserved.
export function BattleCoinLivePriceText({
  symbol,
  initialPrice,
  streamEnabled = BATTLECOIN_MARKET_STREAM_ENABLED,
}) {
  const normalizedSymbol = String(symbol || '').toUpperCase().trim()
  const initialNumeric = Number(initialPrice || 0)
  const [price, setPrice] = useState(
    Number.isFinite(initialNumeric) && initialNumeric > 0 ? initialNumeric : 0
  )
  const requestInFlightRef = useRef(false)
  const abortRef = useRef(null)
  const streamRevisionRef = useRef(0)
  const providerEventAtRef = useRef(0)

  useEffect(() => {
    const nextInitial = Number(initialPrice || 0)
    if (!Number.isFinite(nextInitial) || nextInitial <= 0) return
    setPrice((current) => (current > 0 ? current : nextInitial))
  }, [initialPrice, normalizedSymbol])

  const refresh = useCallback(async (reason = 'interval') => {
    if (!normalizedSymbol || requestInFlightRef.current) return
    if (reason === 'interval' && document.visibilityState !== 'visible') return

    requestInFlightRef.current = true
    const streamRevisionAtStart = streamRevisionRef.current
    const controller = new AbortController()
    abortRef.current?.abort()
    abortRef.current = controller
    const timeoutId = window.setTimeout(
      () => controller.abort(),
      BATTLECOIN_SELECTED_PRICE_TIMEOUT_MS
    )

    try {
      const response = await fetch(
        `/api/battlecoin/state?scope=market&symbol=${encodeURIComponent(normalizedSymbol)}`,
      ...
      if (
        ...
        streamRevisionAtStart !== streamRevisionRef.current ||
        !payload?.ok ||
        String(row?.symbol || '') !== normalizedSymbol ||
        !Number.isFinite(nextPrice) ||
        nextPrice <= 0
      ) {
        return
      }
      setPrice((current) => (Object.is(current, nextPrice) ? current : nextPrice))
    } catch (error) {
      if (error?.name !== 'AbortError') {
        // Keep last-known-good text. The next visible tick revalidates it.
      }
    } finally {
  ...
  useEffect(() => {
    ...
    const streamIsConnected = () => (
      Boolean(socket) && socket.readyState === window.WebSocket.OPEN
    )
    const scheduleStreamReconnect = () => {
      ...
      clearReconnect()
      const delay = battleCoinMarketReconnectDelay(reconnectAttempt)
      reconnectAttempt += 1
    ...
    function connectStream() {
      ...
      try {
        const nextSocket = new window.WebSocket(
          `${BATTLECOIN_MARKET_STREAM_BASE}/${normalizedSymbol.toLowerCase()}@ticker`,
        )
        socket = nextSocket
        nextSocket.onopen = () => {
        ...
        nextSocket.onmessage = (event) => {
          ...
          try {
            ...
            const nextPrice = readBattleCoinTickerPrice(payload, normalizedSymbol)
            if (!nextPrice) return
            const providerEventAt = Number(payload?.E || 0)
            if (
              !Number.isFinite(providerEventAt) ||
              providerEventAt <= 0 ||
              providerEventAt < providerEventAtRef.current
            ) return
            providerEventAtRef.current = providerEventAt
        ...
        const onDisconnected = () => {
          ...
          if (!stopped) {
            void refresh('stream-disconnect')
            scheduleStreamReconnect()
          }
        }
        nextSocket.onerror = onDisconnected
        nextSocket.onclose = onDisconnected
      } catch {
        socket = null
        scheduleStreamReconnect()
      }
    ...
    providerEventAtRef.current = 0
    void refresh('mount')
    connectStream()
    const intervalId = window.setInterval(
      () => {
        if (!streamIsConnected()) void refresh('interval')
      },
      BATTLECOIN_SELECTED_PRICE_POLL_MS
    )
    const reconcileIntervalId = streamEnabled
      ? window.setInterval(() => {
        if (streamIsConnected()) void refresh('stream-reconcile')
      }, BATTLECOIN_MARKET_STREAM_RECONCILE_MS)
      : null
    const refreshVisible = () => {
      if (document.visibilityState === 'visible') {
        connectStream()
        if (!streamIsConnected()) void refresh('visibility')
      } else {
        clearReconnect()
        closeSocket()
      }
    ...
    const refreshFocus = () => {
      connectStream()
      if (!streamIsConnected()) void refresh('focus')
    }
    const refreshOnline = () => {
      connectStream()
      if (!streamIsConnected()) void refresh('online')
    }
    ...
    return () => {
      ...
      if (reconcileIntervalId) window.clearInterval(reconcileIntervalId)
      clearReconnect()
      closeSocket()
      abortRef.current?.abort()
  ...
  }, [normalizedSymbol, refresh, streamEnabled])

  const visiblePrice = price > 0 ? price : initialNumeric
  return formatNumber(visiblePrice, 6)
}
...
const fetchState = useCallback(
  async (scope = 'full') => {
    if (scope === 'light' && lightRequestInFlightRef.current) return
    if (scope === 'light') {
      lightRequestInFlightRef.current = true
      try { lightDotRef.current?.classList.add('is-pinging') } catch {}
    }
    ...
    try {
      ...
      const mayApplyLiveState = requestSeq >= stateAppliedSeqRef.current
      if (mayApplyLiveState) {
        stateAppliedSeqRef.current = requestSeq
      ...
      if (scope === 'full') setSymbols(Array.isArray(j.symbols) ? j.symbols : [])
      ...
      // A transient live refresh must never replace last-known-good BattleCoin data.
      if (scope === 'full') {
      ...
      if (scope === 'light') {
        lightRequestInFlightRef.current = false
        try { lightDotRef.current?.classList.remove('is-pinging') } catch {}
      }
    }
  },
  [hasActiveOrder]
 ...
 useEffect(() => {
  ...
  // iOS can create the native overflow layer before the first guest market payload
  // has fully painted. The authenticated path naturally receives a later FULL
  // state pass after auth:ok; guest does not. Re-prime only the guest iOS overflow
  // layer after populated DOM has painted, without refetching data or owning touch.
  useEffect(() => {
    if (auth) {
      setMarketScrollPriming(false)
      return undefined
    }
    if (guestMarketPrimeDoneRef.current) return undefined
    if (!shouldReprimeGuestMarketScroller({
      auth,
      loading,
      symbolCount: symbols.length,
    })) return undefined

    guestMarketPrimeDoneRef.current = true
    setMarketScrollPriming(true)
  ...
  }, [auth, loading, symbols.length])

  // --- authoritative live order polling: only while an OPEN order exists ---
  useEffect(() => {
    if (!hasActiveOrder) return undefined

    const refreshLight = () => {
      if (document.visibilityState !== 'visible') return
      void fetchState('light')
    }

    refreshLight()
    const intervalId = window.setInterval(refreshLight, 2500)
    const refreshVisible = () => {
      if (document.visibilityState === 'visible') refreshLight()
    }
    const refreshFocus = () => { refreshLight() }
    const refreshOnline = () => { refreshLight() }
  ...
  }, [fetchState, hasActiveOrder])

  // --- countdown display clock: update only the existing timer DOM node ---
  // Business settlement remains the same handleSettle() + server-side auto-settle path.
  const renderTimeLeftSec = hasActiveOrder
    ? Math.max(0, Math.floor((Number(activeOrder?.expiresAt || 0) - Date.now()) / 1000))
    : 0
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      const left = Number(activeOrder?.expiresAt || 0) - Date.now()
      const seconds = Math.max(0, Math.floor(left / 1000))
      const timerNode = timerValueRef.current

      if (timerNode) {
        const nextText = formatTimer(seconds)
        if (timerNode.textContent !== nextText) timerNode.textContent = nextText
        timerNode.classList.toggle('is-pulsing', seconds > 0 && seconds <= 10)
      }

      if (seconds <= 0 && !settleRequestedRef.current) {
    ...
    // handleSettle is intentionally the same existing business operation; the effect is
    // keyed only by the identity/deadline of the current order.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeOrder?.expiresAt, activeOrder?.orderId, activeOrder?.status, hasActiveOrder])
...
const filteredOrders = useMemo(() => {
  ...
  const timerPulsing = renderTimeLeftSec > 0 && renderTimeLeftSec <= 10
  const handleVipClick = async () => {
    try {
      ...
      const paymentMethod = await openPaymentMethodPopover({ accountId: acc, purpose: 'vip' })
  ...
  return (
    <section id="ql7-exchange-battlecoin" className="panel battlecoin-panel" data-ql7-visual-scope="panel">
      {/* ---------------- Header ---------------- */}
...
/>
        ...
        <div className={`battlecoin-header-right ${isVip ? 'is-vip-active' : ''}`}>         
  ...
  {tf(t, 'battlecoin_vip_cta', 'Unlock VIP x100')}
      ...
      <div className="battlecoin-layout">
        ...
        <div className="battlecoin-left">
          {/* Control panel */}
          <div className="battlecoin-card battlecoin-control-card" data-ql7-visual-scope="card">
       ...
       {tf(
         ...
         'Stake • Leverage • Direction'
              ...
              <div className="card-status-dot">
                <span ref={lightDotRef} className="live-dot" />
   ...
   <span className="control-extra">
     ...
     <span className="ls-sub">
       ...
       {tf(t, 'battlecoin_short_bearish', 'Bearish')}
          ...
          {/* Active order */}
          <div className="battlecoin-card battlecoin-active-card" data-ql7-visual-scope="card">
            <div className="card-header">
              <div className="card-title-wrap">
                <h3 className="card-title">
                  {tf(
...
<div className="info-symbol">
  ...
  <span>{activeOrder.symbol}</span>
        ...
        <div className="battlecoin-right">
          {/* Market list */}
          <div className="battlecoin-card market-card" data-ql7-visual-scope="card">
            <div className="card-header">
              <div className="card-title-wrap">
                <h3 className="card-title">
                  {tf(
     ...
     <span className="card-subtitle">
       {tf(
         ...
         'Realtime prices • click to target symbol'
          ...
          {/* History */}
          <div className="battlecoin-card history-card" data-ql7-visual-scope="card">
...
/* Активный VIP может ужиматься,
   чтобы всегда оставаться рядом с балансом */
.battlecoin-header-right.is-vip-active .battlecoin-vip-block {
  min-width: 0;
  max-width: 100%;
}

.battlecoin-header-right.is-vip-active .battlecoin-vip-pill {
  width: auto;
  min-width: 0;
  max-width: 100%;
  flex: 0 1 auto;
...
/* Если VIP активен:
   баланс + VIP всегда остаются в одной строке */
.battlecoin-header-right.is-vip-active {
  grid-template-columns: minmax(0, 1fr) minmax(0, max-content);
  column-gap: clamp(4px, 1.8vw, 8px);
}

.battlecoin-header-right.is-vip-active .battlecoin-balance-block {
  min-width: 0;
}
...
.battlecoin-header-right.is-vip-active .battlecoin-vip-pill {
  ...
  padding: 4px clamp(5px, 1.8vw, 9px);
  min-height: clamp(32px, 9vw, 42px);
  white-space: nowrap;
}

/* Уменьшаем золотую искру */
.battlecoin-header-right.is-vip-active .vip-spark {
  flex: 0 0 auto;
  width: clamp(8px, 2.8vw, 12px);
  height: clamp(8px, 2.8vw, 12px);
}

/* Надпись VIP сама уменьшается */
.battlecoin-header-right.is-vip-active .qcoinLabel {
  flex: 0 1 auto;
  min-width: 0;
  font-size: clamp(12px, 3.9vw, 17px);
  letter-spacing: clamp(0px, 0.1vw, 0.4px);
  white-space: nowrap;
}

/* x100 тоже уменьшается */
.battlecoin-header-right.is-vip-active .vip-val {
  flex: 0 1 auto;
  min-width: 0;
...
/* Когда VIP НЕ активен — кнопка Unlock VIP
   сохраняет прежнее поведение */
.battlecoin-header-right:not(.is-vip-active) .battlecoin-vip-pill,
.battlecoin-header-right:not(.is-vip-active) .battlecoin-vip-cta {
  width: fit-content;
  min-width: 128px;
}
