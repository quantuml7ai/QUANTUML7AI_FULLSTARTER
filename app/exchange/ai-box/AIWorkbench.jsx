...
import { subscribeVisualActivity } from '../../../lib/visual-runtime/visualActivityRegistry'
...
function useTypewriter(text, enabled, speed = 14, charsPerTick = 1, scopeRef = null) {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      if (cancelled || !running || index >= text.length || timerId) return
    ...
    const start = () => {
      if (cancelled || running || index >= text.length) return
      running = true
      if (index === 0) tick()
      else schedule()
    }
  ...
  }, [charsPerTick, enabled, scopeRef, speed, text])

  return value
}

const QuotaBadge = memo(function QuotaBadge({ entitlement, readEntitlement, t }) {
  ...
  useEffect(() => {
    ...
    const paintClock = () => {
      if (document.visibilityState === 'visible') {
        const current = typeof readEntitlement === 'function'
          ? readEntitlement()
          : entitlement
        const nextText = formatQuotaClock(current?.remainingSec)
        if (clockRef.current && clockRef.current.textContent !== nextText) {
          clockRef.current.textContent = nextText
        }
      }
      timerId = window.setTimeout(paintClock, 1000)
    }

    paintClock()
    return () => window.clearTimeout(timerId)
  }, [entitlement, metered, readEntitlement])
  ...
  return (
    ...
    >
      ...
      <span className="ai-access-copy">
        ...
        <small ref={clockRef}>{formatQuotaClock(entitlement.remainingSec)}</small>
      </span>
    </div>
  )
})
...
export default function AIWorkbench({
  ...
  const reasonsAutoScrollRef = useRef(true)
 ...
 const typedReasons = useTypewriter(
  reasonsFullText,
  canAnalyze,
  42,
  15,
...
// Каждый новый ответ AI снова начинает печататься с автоскроллом.
useEffect(() => {
  reasonsAutoScrollRef.current = true
}, [reasonsFullText])

// Определяет, находится ли пользователь около нижней границы.
const handleReasonsScroll = useCallback((event) => {
  ...
  // Если пользователь вернулся вниз — автоскролл снова включается.
  // Если ушёл выше — отключается.
  reasonsAutoScrollRef.current = distanceToBottom <= 8
}, [])

// Мгновенно отключает автоскролл при прокрутке колёсиком вверх.
const handleReasonsWheel = useCallback((event) => {
  if (event.deltaY < 0) {
    reasonsAutoScrollRef.current = false
  }
}, [])

// Отключает автоскролл при ручной прокрутке пальцем.
const handleReasonsTouchMove = useCallback(() => {
  reasonsAutoScrollRef.current = false
}, [])
...
useEffect(() => {
  ...
  if (!element || !reasonsAutoScrollRef.current) {
    return undefined
  }

  const frameId = window.requestAnimationFrame(() => {
    if (reasonsAutoScrollRef.current) {
  ...
  return (
    <section id="ql7-exchange-ai-box" className="ai-workbench" ref={workbenchRef} data-ai-workbench="v3" data-ql7-visual-scope="panel">
        ...
        {canAnalyze && !purchaseOpen && (
          <article className="ai-recommendation-layer" data-ai-recommendation="visible">
            <div className="ai-status-rail">
              ...
              <QuotaBadge entitlement={entitlement} readEntitlement={readEntitlement} t={t} />
            </div>
              ...
              {!aiData || !Number.isFinite(aiData.price) ? (
                ...
                <>
                  ...
                  <div className="ai-reason-shell">
                    ...
                    <div
                        className="ai-reason-scroll"
                        ref={reasonsRef}
                        onScroll={handleReasonsScroll}
                        onWheel={handleReasonsWheel}
                        onTouchMove={handleReasonsTouchMove}
                    >
                      <ul>
