...
import { getStoredWalletSession } from '../../../lib/walletSessionClient.js'
...
export const BATTLE_CHAT_RECONNECT_INITIAL_MS = 1800
export const BATTLE_CHAT_RECONNECT_MAX_MS = 30000
...
export function battleChatReconnectDelay(attempt = 0) {
  const safeAttempt = Math.max(0, Math.floor(Number(attempt) || 0))
  return Math.min(
    BATTLE_CHAT_RECONNECT_MAX_MS,
    BATTLE_CHAT_RECONNECT_INITIAL_MS * (2 ** Math.min(safeAttempt, 16)),
  )
}
...
export async function postBattleChatMessage({ text, clientMutationId, locale = '' }) {
  const res = await fetch('/api/battlecoin/chat/messages', {
    ...
    body: JSON.stringify({ text, clientMutationId, locale: String(locale || '') }),
  })
  return readJson(res)
}
