...
import { copyToClipboard } from '../../../lib/forumShareManager.js'
import { AvatarBadgeOverlay } from '../../forum/features/profile/components/VipFlipBadge.jsx'
import { battleChatAuthorAvatar, battleChatAuthorName } from './battleChatIdentity.js'
import { compactBattleChatCount, formatBattleChatTime } from './battleChatFormatters.js'
  ...
  return (
    ...
    >
      <div className={styles.avatarColumn}>
        <div className={`${styles.avatarWrap} ql7AvatarBadgeHost`}>
          <Image
            src={avatar}
            alt=""
            width={68}
            height={68}
            className={`${styles.avatar} ql7AvatarMediaClip`}
            unoptimized={avatar.startsWith('http')}
          />
          <AvatarBadgeOverlay vipActive={authorVip} />
        </div>
        <button
          type="button"
      ...
      <div className={[styles.messageBubble, emojiOnly ? styles.messageBubbleEmoji : ''].filter(Boolean).join(' ')}>
        <div className={[styles.messageBodyLine, canTranslate ? styles.messageBodyLineTranslated : ''].filter(Boolean).join(' ')}>
          <p className={[styles.messageText, emojiOnly ? styles.messageTextEmoji : ''].filter(Boolean).join(' ')}>
            <TypewriterText
              as="span"
              className={[emojiOnly ? styles.emojiBurst : '', emojiTone].filter(Boolean).join(' ')}
              text={shownText}
