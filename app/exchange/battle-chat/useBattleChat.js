...
import { runAuthorizedClientAction } from '../../../lib/authActionGateClient.js'
import {
...
export function useBattleChat(locale = '') {
  ...
  useEffect(() => {
    ...
    const closeEventSource = () => {
      if (!es) return
      try { es.removeEventListener('battlecoin-chat-ready', handleReady) } catch {}
    ...
    const scheduleReconnect = () => {
      ...
      const delay = battleChatReconnectDelay(reconnectAttempt)
      reconnectAttempt += 1
    ...
    const connect = () => {
      ...
      try {
        ...
        es.addEventListener('battlecoin-chat-ready', handleReady)
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      setError(err?.message || 'battlecoin_chat_error')
      return { ok: false, error: err?.message || 'battlecoin_chat_error', data: err?.data || null }
    } finally {
      if (mountedRef.current) setSending(false)
    }
  }, [acceptPayload, locale])
