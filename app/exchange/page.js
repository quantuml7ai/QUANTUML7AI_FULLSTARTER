...
/* ================================= data ================================= */
async function fetchDepth(sym,limit=20,{ signal }={}){
  const r=await fetch(`${BINANCE}/api/v3/depth?symbol=${sym}&limit=${limit}`,{cache:'no-store',signal})
  return r.json()
}
    ...
    <Panel>
      <div className="ex-hero-panel">
        ...
        <div className="ex-badge">
          <span className="qcoinLabel ql7QCoinGoldLabel">{title}</span>
        </div>
...
function OrderBook({ symbol }) {
  ...
  useEffect(() => {
    ...
    const requestedSymbol = String(symbol || '').toUpperCase().trim()
    ...
    async function load() {
      ...
      try {
        const j = await fetchDepth(requestedSymbol, 50, { signal: requestController?.signal })
        if (alive && revision === requestRevision && requestedSymbol === String(symbol || '').toUpperCase().trim()) setData(j)
      } catch {
        // Preserve the previous snapshot on aborts and transient provider failures.
      } finally {
      ...
      <div className="ob-cols">
        ...
        <div className="ob-col">
          {bids.slice(0, 20).map((r, i) => (
            <div key={'b' + i} className="ob-row ob-row-bid" data-ql7-visual-scope="row" data-ql7-render-managed="1">
              <div
        ...
        <div className="ob-col">
          {asks
            ...
            .reverse()
            .map((r, i) => (
              <div key={'a' + i} className="ob-row ob-row-ask" data-ql7-visual-scope="row" data-ql7-render-managed="1">
                <div
                  className="ob-row-fill"
                  style={{ width: `${(r.qty / maxQty) * 100}%` }}
                />
...
/* Heavy exchange widgets own their own live state. Memoizing them here keeps
   the AI quota clock from reconciling BattleCoin / TradingView / OrderBook
   when their props did not change. Context updates still pass through React. */
const StableTVTicker = React.memo(TVTicker)
const StableBattleCoin = React.memo(BattleCoin)
const StableTVChart = React.memo(TVChart)
const StableOrderBook = React.memo(OrderBook)

/* ================================= Page ================================= */
function isIOSExchangeFlatCompositorABRuntime(){
...
export default function ExchangePage(){
  ...
  // R11 forensic A/B retained only for the fixed HeroAvatar image filter on physical
  // iOS /exchange. R13 disables backdrop-filter at source declarations across the ecosystem
  // without a document-wide universal selector; market/auth/API/polling ownership stay untouched.
  useEffect(() => {
    if (!isIOSExchangeFlatCompositorABRuntime()) return undefined
    const root = document.documentElement
    const attr = 'data-ql7-exchange-ios-flat-compositor-ab'
  ...
  const { entitlement, readEntitlement, refresh: refreshEntitlement, canAnalyze } = useAIEntitlement()
  ...
  const handlePayClick = async () => {
    ...
    try {
      ...
      if (paymentMethod.method === 'qcoin') {
        setOpenUnlimit(false)
        setPaymentBusy(false)
        await refreshEntitlement({ reason: 'qcoin-vip-purchase' })
        return
      }
  ...
  return (
    <div className="wrap" data-ql7-visual-scope="route">
      <BadgeTitle/>

      <StableTVTicker symbol={symbol}/>
      <StableBattleCoin />
      <StableTVChart symbol={symbol} tf={tf}/>
