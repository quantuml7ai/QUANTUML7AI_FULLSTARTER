...
export default function CryptoNewsLens() {
  ...
  const [translationAnimationKey, setTranslationAnimationKey] = useState(0)
  const [translationAnimationEnabled, setTranslationAnimationEnabled] = useState(false)
  ...
  const [visualActivityRunning, setVisualActivityRunning] = useState(false)
  ...
  useEffect(() => {
    const node = lensRootRef.current
    if (!node) return undefined
    return subscribeVisualActivity(node, {
      onRun: () => setVisualActivityRunning(true),
      onPause: () => setVisualActivityRunning(false),
      loop: true,
    })
  }, [])
  ...
  useEffect(() => {
    if (!autoPlayEnabled || !visualActivityRunning) return
    ...
    {activeItem?.previewKind === 'video' && (
      <div className="videoBadge">▶</div>
              ...
              <TypewriterText
                as="h3"
                className="titleText"
                text={isTranslated && translatedTitle ? translatedTitle : activeItem.title}
              ...
              {activeItem.summary && (
                <TypewriterText
                  as="p"
                  className="summary"
                  text={isTranslated && translatedSummary ? translatedSummary : activeItem.summary}
                  animate={translationAnimationEnabled}
                  animationKey={translationAnimationKey}
                />
              )}
