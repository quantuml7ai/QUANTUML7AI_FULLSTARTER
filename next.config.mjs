...
const trustIdentityContentLanguageHeaderPairs = [
  ['/en/trust-and-identity', 'en'],
  ['/ru/trust-and-identity', 'ru'],
  ['/uk/trust-and-identity', 'uk'],
  ['/es/trust-and-identity', 'es'],
  ['/tr/trust-and-identity', 'tr'],
  ['/ar/trust-and-identity', 'ar'],
  ['/zh/trust-and-identity', 'zh'],
]

// QL7_TRUST_CONTENT_LANGUAGE_MUTABLE_NEXT_ROUTES_V2
// Next normalizes custom-route objects in place; allocate fresh mutable objects on every call.
function buildTrustIdentityContentLanguageHeaders() {
  return trustIdentityContentLanguageHeaderPairs.map(([source, lang]) => ({
    source,
    headers: [
      { key: 'Content-Language', value: lang },
    ],
  }))
}
const nextConfig = {
  env: {
    SUPPORT_ACTIVE: process.env.SUPPORT_ACTIVE === '1' ? '1' : '0',
  },
...
images: {
  ...
  async headers() {
    ...
    return [
      ...buildTrustIdentityContentLanguageHeaders(),
