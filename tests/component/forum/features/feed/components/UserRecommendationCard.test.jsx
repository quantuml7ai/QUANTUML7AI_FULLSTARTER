...
describe('UserRecommendationCard', () => {
  it('renders avatar, neon nickname, stars count and vip marker', () => {
    ...
    expect(screen.getByTestId('mock-avatar-badge-overlay')).toHaveAttribute('data-vip-active', '1')
    expect(screen.getByTestId('mock-avatar-badge-overlay')).toHaveAttribute('data-info-visible', '1')
    expect(screen.getByTestId('mock-avatar-badge-overlay')).toHaveStyle({ pointerEvents: 'none' })
    expect(screen.getByTestId('mock-avatar-info-hint')).toHaveTextContent('i')
    expect(container.querySelector('style')?.textContent).toMatch(/\.recommendationCard\s*\{[\s\S]*?overflow: visible;/u)
    expect(container.querySelector('style')?.textContent).toMatch(/\.recommendationCardAvatarShell\s*\{[\s\S]*?z-index: 3;/u)
  })
