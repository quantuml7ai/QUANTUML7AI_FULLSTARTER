...
describe('UserRecommendationsRail', () => {
  ...
  it('renders fifteen cards without creating per-card or per-rail stylesheets', () => {
    const createRail = (key) => React.createElement(UserRecommendationsRail, {
        key,
        t: recommendationT,
        railState: createRecommendationRailState(createRecommendationUsers(15)),
        onOpenUserPosts: vi.fn(),
        hideScrollbar: true,
        desktopArrows: true,
      })
    const { container, rerender } = render(createRail('rail-1'))

    expect(container.querySelectorAll('.recommendationCard')).toHaveLength(15)
    expect(container.querySelectorAll('style')).toHaveLength(0)
    ...
    expect(container.querySelectorAll('.recommendationCard')).toHaveLength(45)
    expect(container.querySelectorAll('style')).toHaveLength(0)
  })
