...
function Surface() {
  return h(React.Fragment, null,
    ...
    h('div', { 'data-ql7-render-managed': '1', 'data-ql7-visual-scope': 'card', 'data-testid': 'managed-card' },
      h('span', null, 'Premium'),
    ),
  )
}
...
describe('REV6 generic visual scope hydration boundary', () => {
  ...
  test('hydrates generic visual scopes without inventing data-ql7-visual-state', async () => {
    ...
    const managed = host.querySelector('[data-testid="managed-card"]')
    expect(managed).toBeTruthy()
    expect(managed.hasAttribute('data-ql7-render-mode')).toBe(false)
    expect(['hot', 'paused']).toContain(managed.getAttribute('data-ql7-motion-mode'))
    expect(errors.filter((line) => /hydration|extra attributes|data-ql7-visual-state|data-ql7-render-mode|data-ql7-motion-mode/iu.test(line))).toEqual([])
