...
describe('QL7 Support V14 premium surface component', () => {
  it('renders the canonical premium card with semantic SVG evidence', () => {
    ...
    const card = buildQl7SupportCard({ ...result.surface, caseId: 'component:qcoin' })
    ...
    const verifiedBadge = card.badges.find((badge) => badge?.id === 'verified')
    expect(verifiedBadge).toMatchObject({ id: 'verified', tone: 'success', icon: 'confirmed' })
    const verifiedBadgeMarkup = `class="ql7SupportBadgeLabel">${verifiedBadge.label}</span>`
    expect(html.split(verifiedBadgeMarkup).length - 1).toBe(1)
