...
function signedLearningApproval({ candidateId, proposal, keyId, signingKey, issuedAt }) {
  const proposalHash = shaJson(proposal)
  const body = {
    schema: 'ql7.support.learning-approval',
    schemaVersion: '5.1.0',
    approverId: 'test-reviewer',
    keyId,
    decision: 'approve',
    candidateId,
    proposalHash,
  ...
  const receiptHash = crypto.createHash('sha256')
    .update(JSON.stringify(stableApprovalValue(body)))
    .digest('hex')
  const signature = crypto.createHmac('sha256', signingKey).update(receiptHash).digest('hex')
...
describe('QL7 Support premium persisted pipeline', () => {
  ...
  test('requires review and evaluation before deployment and supports rollback', async () => {
    ...
    const candidate = await recordQl7SupportLearningSignal({
      ...
      consent: true,
      consentReceiptId: 'consent:test-learning:user-1:2026-07-24',
      clock,
    })
    ...
    const proposal = {}
    const keyId = 'test-learning-approval-key'
    const signingKey = 'test-learning-manual-approval-secret-0123456789'
    const approval = signedLearningApproval({
    ...
    const deployment = await deployQl7SupportLearningCandidate({
      database,
      candidateId: candidate._id,
      evaluation: { passed: true, version: 'v-test' },
      mode: 'canary',
      proposal,
      approvals: [approval],
      signingKeys: { [keyId]: signingKey },
      clock,
    })
