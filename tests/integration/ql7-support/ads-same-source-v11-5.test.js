...
const { readQl7SupportAdsDiagnosticV9 } = await import('../../../lib/ql7-support/adsSupportReadAdapterV9.js')

describe('QL7 Support V11.5 same-source advertising evidence', () => {
  ...
  test('uses the advertising cabinet read source and keeps an active package distinct from zero campaigns', async () => {
    ...
    const result = await readQl7SupportAdsDiagnosticV9({ userId: 'wallet-1', aliases: ['telegram-1'], analysis: { messageAct: 'personal_status_request', subIntent: 'ads_packages_self_status', operation: 'check_status' }, now: new Date('2026-07-28T00:00:00.000Z') })
  ...
  test('reads campaign analytics only when metrics were explicitly requested', async () => {
    ...
    const result = await readQl7SupportAdsDiagnosticV9({ userId: 'wallet-1', analysis: { messageAct: 'personal_status_request', subIntent: 'ads_campaigns_metrics', operation: 'show_metrics' }, now: new Date('2026-07-28T00:00:00.000Z') })
