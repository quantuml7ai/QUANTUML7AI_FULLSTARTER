...
vi.mock('../../../lib/webPush.js', () => ({
  sendBackgroundPush: vi.fn(async () => ({ sent: 0 })),
}))
...
describe('QL7 Support canonical runtime integration', () => {
  ...
  test('maps a VIP provider timeout into a user-safe result and aggregate admin report', () => {
    const diagnostic = buildQl7SupportDiagnosticFailureResult({ error: Object.assign(new Error('provider adapter stack'), { name: 'AbortError' }), topic: 'vip', caseId: 'case-canonical' })
    const bridge = buildQl7SupportEmailBridgePayload({ fromUserId: 'user-canonical', text: 'Проверь мой VIP. Bearer abc.def.ghi', messageId: 'message-canonical', locale: 'ru', topic: 'vip', caseId: 'case-canonical', diagnosticResult: diagnostic, ecosystemRating: { value: 72, band: 'established', confidence: 80 } })
