...
describe('QL7 Support canonical production-shaped integration', () => {
  it('delivers the Polish VIP status fixture without unrelated benefit leakage', async () => {
    const row = await executeQl7SupportScenario({
      id: 'vip_status:pl:2',
      locale: 'pl',
      input: 'Is my VIP active?',
      fixtureTopic: 'vip',
      seed: 'combat-10k:20260915-073101:integration:2',
      expected: {
        topic: 'vip',
        requiredAdapter: 'vip',
        tableSchema: 'ql7.table.vip',
        verifiedBadge: 1,
      },
    })

    expect(row.oracle.ok).toBe(true)
    expect(row.productionDelivery.qualityGate).toMatchObject({
      decision: 'allow',
      coherenceFailures: [],
    })
    expect(row.productionDelivery.qualityGate.localeNaturalness.unexpectedLanguageSpans).toEqual([])
    expect(row.productionDelivery.qualityGate.domainIsolation).toMatchObject({
      decision: 'allow',
      forbiddenGeneratedEntityIds: [],
    })
    expect(row.result.text).toBe('Zweryfikowane dane dotyczące VIP Plus pochodzą z zatwierdzonego źródła.')
    expect(row.result.factProjection).toMatchObject({ status: 'active', issues: [] })
    expect(row.result.surface.table.rows.map((item) => item.key)).toEqual(['status', 'tier'])
    expect(row.result.surface.table.columns.map((item) => item.label)).toEqual(['Pole', 'Wartość'])
    expect(row.result.surface.table.rows.find((item) => item.key === 'status')?.value).toBe('aktywny')
    const visibleSurface = {
      text: row.result.text,
      title: row.result.surface.title,
      summary: row.result.surface.summary,
      badges: row.result.surface.badges,
      table: row.result.surface.table,
      actions: row.result.surface.actions,
    }
    expect(JSON.stringify(visibleSurface)).not.toMatch(/priority support|\bField\b|\bValue\b|\bActive\b/u)
  })
