...
vi.mock('../../../../lib/mongo/profile-primary.cjs', () => ({
  default: {
    getUserProfile: vi.fn(async (id) => ({
      nickname: routeHarness.state.profileNick[id] || '',
      icon: routeHarness.state.profileAvatar[id] || '',
    })),
  },
}))
...
vi.mock('../../../../lib/mongo/forum-primary.cjs', () => ({
  default: {
    getFollowersCount: vi.fn(async (id) => Number(routeHarness.state.followers[id] || 0)),
  },
}))
