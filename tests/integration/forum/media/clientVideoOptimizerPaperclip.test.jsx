...
import { preparePreparedVideoArtifacts } from '../../../../app/forum/features/media/services/moderatePreparedVideoForUpload.js'
...
function createProps(overrides = {}) {
  return {
    ...
    videoModerationActorId: 'viewer-1',
    showVideoLimitOverlay: vi.fn(),
    readVideoDurationSecFn: vi.fn(async () => 119.8),
    prepareVideoPreviewFn: vi.fn(async (file) => ({
      needsPreparedPreview: false,
      file,
      metadata: { classified: true, hevcSource: false },
      optimized: false,
...
describe('paperclip deferred video gateway integration', () => {
  beforeEach(() => {
    ...
    vi.mocked(preparePreparedVideoArtifacts).mockImplementation(async ({ file }) => {
      const posterBlob = new Blob([new Uint8Array(8)], { type: 'image/webp' })
      return { file, status: 'approved', moderationReceipt: 'receipt', mediaSha256: 'a'.repeat(64), poster: { blob: posterBlob } }
    })
  })
  ...
  test('non-HEVC selection creates a local fullscreen/composer preview without optimization, presign or upload', async () => {
    const restoreUrl = installObjectUrlStubs()
    const source = new File([new Uint8Array(835)], 'legacy-preview.mp4', { type: 'video/mp4' })
    const props = createProps()
  ...
  test('HEVC selection prepares first, previews the verified File, and does not upload before Send', async () => {
    const restoreUrl = installObjectUrlStubs()
    const source = new File([new Uint8Array(835)], 'camera-hevc.mov', { type: 'video/quicktime' })
    const prepared = new File([new Uint8Array(512)], 'camera-hevc_FFMP.mp4', { type: 'video/mp4' })
    const prepareVideoPreviewFn = vi.fn(async (_file, options) => {
      options.onProgress?.({ stage: 'encoding', attempt: 1, maxAttempts: 3, progress: 0.5 })
      options.onProgress?.({ stage: 'verifying', progress: 0.8 })
      return {
        needsPreparedPreview: true,
        file: prepared,
        optimized: true,
        decoderPath: 'ql7-hevc-wasm-fallback-v7-prod',
        result: { durationSec: 26.9 },
      }
    })
    const props = createProps({ prepareVideoPreviewFn })
    ...
    try {
      ...
      expect(prepareVideoPreviewFn).toHaveBeenCalledWith(source, expect.objectContaining({
        maxDurationSeconds: 300,
        signal: expect.any(AbortSignal),
        onProgress: expect.any(Function),
      }))
      expect(props.beginMediaPipeline).toHaveBeenCalledWith('Processing', expect.any(AbortController))
      expect(props.endMediaPipeline).toHaveBeenCalledTimes(1)
      expect(props.setMediaPhase).toHaveBeenCalledWith('Prepared')
      expect(props.pendingVideoRef.current).toBe('blob:paperclip-preview')
      ...
      expect(props.readVideoDurationSecFn).not.toHaveBeenCalled()
      expect(uploadR2MediaFile).not.toHaveBeenCalled()
      const meta = props.pendingVideoBlobMetaRef.current.get('blob:paperclip-preview')
      expect(meta).toEqual(expect.objectContaining({
        previewUrl: 'blob:paperclip-preview',
        source: 'paperclip_hevc_prepared_preview',
        filename: 'camera-hevc_FFMP.mp4',
        contentType: 'video/mp4',
        durationSec: 26.9,
        preparedOnSelect: true,
        hevcPreparedPreview: true,
        decoderPath: 'ql7-hevc-wasm-fallback-v7-prod',
        preparedFile: prepared,
        posterBlob: expect.any(Blob),
        originalFilename: 'camera-hevc.mov',
  ...
  test('Send reuses the exact prepared HEVC File without refetching or reconverting the blob preview', async () => {
    const prepared = new File([new Uint8Array(512)], 'camera-hevc_FFMP.mp4', { type: 'video/mp4' })
    const localMeta = {
      previewUrl: 'blob:paperclip-preview',
      source: 'paperclip_hevc_prepared_preview',
    ...
    vi.mocked(uploadR2MediaFile).mockResolvedValue({
      url: 'https://cdn.test/prepared.mp4',
      preparation: { policyId: 'ql7-client-video-streaming-v4', optimized: false, reusedVerifiedFile: true },
    })
    ...
    const result = await resolveComposerMediaPayload({
      ...
      pendingVideoBlobMetaRef: { current: new Map([['blob:paperclip-preview', localMeta]]) },
      emitDiag: vi.fn(),
      forumVideoMaxSeconds: 300,
    ...
    expect(beginMediaPipeline).toHaveBeenCalledWith('Uploading', null, { continueExisting: true })
    expect(uploadR2MediaFile).toHaveBeenCalledWith(expect.objectContaining({ file: prepared }))
    expect(result).toEqual(expect.objectContaining({ failed: false, videoUrlToSend: 'https://cdn.test/prepared.mp4' }))
  })

  test('prepared HEVC Send survives React state/ref lag without cloning the exact File or restarting Processing', async () => {
    const prepared = new File([new Uint8Array(384)], 'desktop-hevc_FFMP.mp4', { type: 'video/mp4' })
    const exactMeta = {
      previewUrl: 'blob:desktop-hevc-preview',
      source: 'paperclip_hevc_prepared_preview',
      filename: prepared.name,
      contentType: prepared.type,
      durationSec: 18.4,
    ...
    vi.stubGlobal('fetch', fetchMock)
    vi.mocked(uploadR2MediaFile).mockResolvedValue({
      url: 'https://cdn.test/desktop-prepared.mp4',
      preparation: { bypassReason: 'prepared-artifacts' },
    })
    ...
    const result = await resolveComposerMediaPayload({
      ...
      beginMediaPipeline,
      mediaCancelRef: { current: false },
      pendingVideoRef: { current: 'blob:desktop-hevc-preview' },
      pendingVideoInfoRef: { current: exactMeta },
      pendingVideoBlobMetaRef: { current: new Map() },
      emitDiag: vi.fn(),
      forumVideoMaxSeconds: 300,
      forumVideoCameraRecordEpsilonSec: 2,
    ...
    expect(fetchMock).not.toHaveBeenCalled()
    expect(beginMediaPipeline).toHaveBeenCalledTimes(1)
    expect(beginMediaPipeline).toHaveBeenCalledWith('Uploading', null, { continueExisting: true })
    expect(uploadR2MediaFile).toHaveBeenCalledWith(expect.objectContaining({ file: prepared }))
    expect(result.videoUrlToSend).toBe('https://cdn.test/desktop-prepared.mp4')
  })

  test('preserves the legacy camera/trim ref fallback when React state is still empty', async () => {
    const cameraBlob = new Blob([new Uint8Array(320)], { type: 'video/webm' })
    const beginMediaPipeline = vi.fn(() => new AbortController())
    const fetchMock = vi.fn(async (url) => ({
      blob: vi.fn(async () => cameraBlob),
      ok: true,
      url,
    }))
    vi.stubGlobal('fetch', fetchMock)
    vi.mocked(uploadR2MediaFile).mockResolvedValue({
      url: 'https://cdn.test/camera.mp4',
      preparation: { optimized: true },
    })
    ...
    const result = await resolveComposerMediaPayload({
      ...
      beginMediaPipeline,
      mediaCancelRef: { current: false },
      pendingVideoRef: { current: 'blob:camera-preview' },
      pendingVideoInfoRef: { current: { source: 'camera', durationSec: 8.2 } },
      pendingVideoBlobMetaRef: { current: new Map() },
      emitDiag: vi.fn(),
      forumVideoMaxSeconds: 300,
      forumVideoCameraRecordEpsilonSec: 2,
    ...
    expect(beginMediaPipeline).toHaveBeenCalledWith('Processing')
    expect(fetchMock).toHaveBeenCalledWith('blob:camera-preview', expect.objectContaining({ signal: expect.anything() }))
    expect(uploadR2MediaFile).toHaveBeenCalledWith(expect.objectContaining({ file: cameraBlob }))
    expect(result.videoUrlToSend).toBe('https://cdn.test/camera.mp4')
  })

  test('verified current preview ref wins over stale React state and preserves the prepared File identity', async () => {
    const prepared = new File([new Uint8Array(416)], 'current-hevc_FFMP.mp4', { type: 'video/mp4' })
    const exactMeta = {
      previewUrl: 'blob:current-hevc-preview',
      source: 'paperclip_hevc_prepared_preview',
      filename: prepared.name,
      contentType: prepared.type,
      durationSec: 22.1,
    ...
    vi.stubGlobal('fetch', fetchMock)
    vi.mocked(uploadR2MediaFile).mockResolvedValue({
      url: 'https://cdn.test/current-prepared.mp4',
      preparation: { bypassReason: 'prepared-artifacts' },
    })

    const result = await resolveComposerMediaPayload({
      pendingVideo: 'blob:stale-previous-preview',
      pendingAudio: null,
      beginMediaPipeline,
      mediaCancelRef: { current: false },
      pendingVideoRef: { current: 'blob:current-hevc-preview' },
      pendingVideoInfoRef: { current: exactMeta },
      pendingVideoBlobMetaRef: { current: new Map([['blob:current-hevc-preview', exactMeta]]) },
      emitDiag: vi.fn(),
      forumVideoMaxSeconds: 300,
      forumVideoCameraRecordEpsilonSec: 2,
      showVideoLimitOverlay: vi.fn(),
    ...
    expect(beginMediaPipeline).toHaveBeenCalledWith('Uploading', null, { continueExisting: true })
    expect(uploadR2MediaFile).toHaveBeenCalledWith(expect.objectContaining({ file: prepared }))
    expect(result.videoUrlToSend).toBe('https://cdn.test/current-prepared.mp4')
  })
  ...
  test('send converts the selected local preview through the shared gateway and reports real progress', async () => {
    ...
    vi.mocked(uploadR2MediaFile).mockImplementation(async (options) => {
      ...
      options.onPrepareProgress?.({ stage: 'sampling', progress: 0.5 })
      options.onPrepareProgress?.({ stage: 'moderating', progress: 0.5 })
...
describe('composer image preview UI', () => {
  ...
  test('parks prepared HEVC unlocked at 94% and resumes the same visual lifecycle at Uploading', async () => {
    const { result } = renderHook(() => useMediaPipelineController({
      t: (key) => key,
      pendingImgs: [],
      pendingAudio: null,
      pendingVideo: 'blob:prepared-hevc',
    }))

    let processingController
    act(() => {
      processingController = result.current.beginMediaPipeline('Processing')
      result.current.setMediaPct(94)
      result.current.setMediaPhase('Prepared')
      result.current.endMediaPipeline()
    })

    await waitFor(() => {
      expect(result.current.mediaPipelineOn).toBe(false)
      expect(result.current.mediaBarOn).toBe(true)
      expect(result.current.mediaPct).toBe(94)
      expect(result.current.mediaPhase).toBe('Prepared')
    })
    expect(result.current.formatMediaPhase('Prepared')).toBe('media_pipeline_ready')

    let uploadController
    act(() => {
      uploadController = result.current.beginMediaPipeline('Uploading', null, { continueExisting: true })
    })

    expect(uploadController).toBeInstanceOf(AbortController)
    expect(uploadController).not.toBe(processingController)
    expect(result.current.mediaPhase).toBe('Uploading')
    expect(result.current.mediaPct).toBeGreaterThanOrEqual(94)
    expect(result.current.mediaPipelineOn).toBe(true)
  })
