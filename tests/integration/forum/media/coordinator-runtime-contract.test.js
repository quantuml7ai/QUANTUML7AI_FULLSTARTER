...
describe('coordinator runtime contract', () => {
  test('R24 keeps the authoritative autoplay/playback function bodies source-identical across platform EOLs', () => {
    ...
    const hashSlice = (startToken, endToken) => {
      ...
      const normalizedSource = src.slice(start, end).replace(/\r\n/g, '\n')
      return crypto.createHash('sha256').update(normalizedSource, 'utf8').digest('hex').toUpperCase()
    }
