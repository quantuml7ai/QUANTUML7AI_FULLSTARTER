...
describe('coordinator runtime contract', () => {
  ...
  test('R25 keeps R24 autoplay stable and limits playMedia changes to the audited src-budget handoff', () => {
    ...
    expect(playMedia.split(r43ManualOnlyGuard)).toHaveLength(2)
    const nativePreservedPlayMedia = playMedia.replace(r43ManualOnlyGuard, '')
    expect(nativePreservedPlayMedia.split(r25BudgetHandoff)).toHaveLength(2)
    expect(hashSource(nativePreservedPlayMedia)).toBe(
      '9CF811D828654AD2328019BB2288A03B5AD5A65CB8D309110EC0B051E24960D2',
    )
    expect(hashSource(nativePreservedPlayMedia.replace(r25BudgetHandoff, ''))).toBe(
  ...
  test('coordinator keeps native near-prewarm ordering while external iframes are manual-only', () => {
    const src = read('app/forum/features/media/hooks/useForumMediaCoordinator.js')
    expect(src).toContain('const mediaDomOrder = new WeakMap();')
    expect(src).toContain('const getNearQueuePlacement = (el, dir = 1) => {')
    expect(src).toContain('const isManualOnlyExternalFrame = (el) => {')
    expect(src).toContain("trace('play_skip_external_manual_only', el)")
    expect(src).toContain("traceCandidate('external_manual_only_skip_prewarm', el, { reason })")
    expect(src).toContain('if (isManualOnlyExternalFrame(el)) return false;')
  })
  ...
  test('external iframe contour blocks prewarm and autoplay until explicit manual intent', () => {
    const src = read('app/forum/features/media/hooks/useForumMediaCoordinator.js')
    const start = src.indexOf('const prepareExternalMedia =')
    const end = src.indexOf('const waitExternalIframeLoad =', start)
    expect(start).toBeGreaterThanOrEqual(0)
    expect(end).toBeGreaterThan(start)
    const block = src.slice(start, end)
    const manualGate = block.indexOf('if (isManualOnlyExternalFrame(el) && !hasManualExternalIntent(el))')
    const originWarm = block.indexOf('warmExternalProviderOrigin(nextSrc)')
    const activeAttach = block.indexOf("el.setAttribute('src', nextSrc)")
    expect(manualGate).toBeGreaterThanOrEqual(0)
    expect(originWarm).toBeGreaterThan(manualGate)
    expect(activeAttach).toBeGreaterThan(manualGate)
    expect(block).toContain("trace('iframe_manual_only_skip_prepare', el, { reason })")
    expect(src).toContain("traceCandidate('external_manual_only_skip_prewarm', el, { reason })")
    expect(src).toContain('if (isManualOnlyExternalFrame(el)) continue;')
  })


test('external poster dismissal follows the trusted local manual-play state, not provider telemetry', () => {
  const player = read('app/forum/features/media/components/ExternalVideoPlayer.jsx')
  const ads = read('app/forum/ForumAds.js')

  expect(player).toContain('const posterVisible = pausedState')
  expect(player).not.toContain('actualPlayingState')
  expect(player).toContain('playWithSoundFromSurface')
  expect(player).toContain("setPausedState(false)\n    const unmuted = commandExternalVideo(frame, 'unmute')")
  expect(player).toContain('data-forum-external-poster-play-button="1"')
  expect(player).toContain('.ql7ExternalPosterPlayButton:hover > img')
  expect(player).toContain('.ql7ExternalPosterPlayButton:active > img')
  expect(player).toContain("backgroundSize: 'contain'")

  expect(ads).toContain('const [externalPosterVisible, setExternalPosterVisible] = useState(true);')
  expect(ads).toContain('setExternalPosterVisible(false);')
  expect(ads).not.toContain('externalFramePlaying')
  expect(ads).toContain('className="forum-ad-external-play-button"')
  expect(ads).toContain('.forum-ad-external-play-button:hover > img')
  expect(ads).toContain('.forum-ad-external-play-button:active > img')
})
