...
describe('coordinator runtime contract', () => {
  ...
  test('native post-video can prewarm near the viewport before activation and primes the first frame through coordinator', () => {
    ...
    expect(src).toContain('getOwnerViewportGapPx(el) <= getNativePrimeGapLimit()')
    ...
    expect(src).toContain('Math.min(2300, Math.round(viewportH * 2.05))')
    expect(src).toContain('Math.min(1500, Math.round(viewportH * 1.32))')
    expect(src).toContain('const hardProtected = isKeep || playing || visiblePx > 0 || holdActive;')
