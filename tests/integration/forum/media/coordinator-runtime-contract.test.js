...
describe('coordinator runtime contract', () => {
  ...
  test('post-video restore delegates network kicks to the coordinator load gate', () => {
    ...
    expect(src).toContain('const postRestorePreloadMode = String(el.dataset?.__restorePreloadMode || \'\').toLowerCase()')
    expect(src).toContain('el.preload = isPostFeedVideo ? postRestorePreload : (shouldAutoPreload ? \'auto\' : \'metadata\')')
  ...
  test('native post-video can prewarm near the viewport before activation and primes the first frame through coordinator', () => {
    ...
    expect(src).toContain('getOwnerVisiblePx(el) > Math.max(56, Math.round(getStartVisiblePx(el) * 0.24))')
    expect(src).toContain('return getOwnerViewportGapPx(el) <= getNativeEarlyPrimeGapLimit();')
    expect(src).toContain("preload: keepWarm ? 'auto' : 'metadata'")
    expect(src).toContain('function restoreNativeSrcIfAllowed(media, reason = \'restore\', opts = {})')
    expect(src).toContain("trace('native_restore_block_src', media, {")
    ...
    expect(src).toContain('const getNativeWarmOwnerLostDelay =')
    expect(src).toContain("scheduleHardUnload(prev, getNativeWarmOwnerLostDelay(prev), 'native_warm_owner_lost')")
    expect(src).toContain("unloadReason !== 'native_warm_owner_lost'")
    expect(src).toContain('const warmupOnlyPrime =')
    expect(src).toContain('const allowHiddenWarmupPrime =')
    expect(src).toContain('const foreignPlaybackBlocksPrime = foreignPlaying && !allowHiddenWarmupPrime;')
    expect(src).toContain('media.dataset.__nativePrimeHoldUntil')
    expect(src).toContain('const POST_NATIVE_SRC_CAP =')
    expect(src).toContain('const clampPostNativeWarmBuffer =')
    expect(src).toContain('const enforcePostNativeSrcCap =')
    expect(src).toContain('if (ios || coarse || /Android/i.test(ua)) return 3;')
    expect(src).toContain('Math.min(1900, Math.round(viewportH * 1.62))')
    expect(src).toContain('Math.min(1500, Math.round(viewportH * 1.24))')
