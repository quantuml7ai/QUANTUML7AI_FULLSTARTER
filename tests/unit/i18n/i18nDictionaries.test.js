...
describe('Academy premium single-card localizations', () => {
  it('ships the premium title, answer, info and pagination labels in all seven languages', () => {
    const keys = [
      'academy_title_primary',
      'academy_title_secondary',
      'academy_card_answer_label',
      'academy_card_info_aria',
      'academy_exam_stage_label',
      'academy_pagination_first_aria',
      'academy_pagination_prev_aria',
      'academy_pagination_next_aria',
      'academy_pagination_last_aria',
      'academy_pagination_page',
    ]
    expect(I18N_SUPPORTED_LANGS).toEqual(['ru', 'en', 'zh', 'uk', 'ar', 'tr', 'es'])
    for (const lang of I18N_SUPPORTED_LANGS) {
      for (const key of keys) {
        expect(String(DICTS[lang][key] || '').trim()).not.toBe('')
      }
    }
  })
})
