...
import { calibrateQl7SupportRouteV13 } from '../../../lib/ql7-support/semantics/routeCalibration.js'
...
import { collectQl7SemanticSignals, getQl7SemanticBankCoverage } from '../../../lib/ql7-support/language/semanticBanks.js'
import { getQl7HumanVariationCoverage, pickQl7HumanVariation } from '../../../lib/ql7-support/language/humanVariationBanks.js'
...
import { getQl7SupportSurfaceTitleV13 } from '../../../lib/ql7-support/presentation/registry.js'
import {
...
import { getQl7SupportTopicActionV9 } from '../../../lib/ql7-support/topicActionRegistryV9.js'
...
describe('QL7 Support V14 final runtime', () => {
  ...
  it('persists exact safety durations', () => {
    expect(run('ты идиот', { priorLedger: { safety: { directInsultCount: 0 } } }).composerPolicy.allowed).toBe(true)
    expect(run('ты идиот', { priorLedger: { safety: { directInsultCount: 1 } } }).composerPolicy.cooldownMs).toBe(60000)
  ...
  it('builds choice surface for ambiguity', () => {
    const r = run('метрики')
    expect(r.surface.options).toHaveLength(4)
    expect(r.surface.other).toMatchObject({ id: 'other', semantic: { topic: 'other' } })
    expect(r.contentPlan.waitingFor).toBe('signed_choice')
  })
  ...
  it('keeps noise, vague Ads and vague crypto in clarification instead of opening adapters', () => {
    ...
    expect(noise.surface.surfaceKind).toBe('choices')
    ...
    expect(ads.surface.options[0].semantic.topic).toBe('ads_campaigns')
    ...
    expect(btc.surface.options[0].semantic.topic).toBe('exchange_ai')
  })
  ...
  it('keeps provider vague ads, noise and QCoin incidents conservative and localized', () => {
    ...
    expect(vagueAd.contentPlan.choices.options.map((row) => row.label)).toContain('Annonsstatistik')
    ...
    expect(explicitAd.replyPlan.text).toMatch(/verifierad källa/iu)
    ...
    expect(noise.analysis.messageAct).toBe('spam_or_noise')
    expect(noise.analysis.requiresAdapter).toBe(false)
    expect(noise.replyPlan.text).toMatch(/jasno/iu)
    ...
    expect(incident.replyPlan.text).toMatch(/Proveravam ledger/iu)
    expect(incident.replyPlan.text).not.toMatch(/Izaberite|razjasnimo|cilj|vrednost|obim/iu)
  })

  it('answers Exchange AI knowledge without Ads collision or market adapter and dedupes provider humor', () => {
    ...
    const humor = executeQl7SupportTurnRuntime({
      mode: 'test',
      requestId: 'fa:joke-duplicate-regression',
      userTurnId: 'fa:joke-duplicate-regression',
      selectedLocale: 'fa',
      text: 'Tell me a joke',
      now: '2026-08-05T00:00:00.000Z',
    })
    ...
    const hiVague = executeQl7SupportTurnRuntime({
      mode: 'test',
      requestId: 'hi:qcoin-vague-dedupe',
      userTurnId: 'hi:qcoin-vague-dedupe',
      selectedLocale: 'hi',
      text: 'qcoin',
      now: '2026-08-05T00:00:00.000Z',
    })
    expect(hiVague.analysis.messageAct).toBe('ambiguous_request')
    expect(hiVague.critic.ok).toBe(true)
    expect(hiVague.critic.issues).not.toContain('provider_fragment_duplicate')
  })
  ...
  it('collects business/operator contacts before handoff and then builds an operator case', () => {
    const first = run('хочу живого оператора по партнерству, есть коммерческое предложение и пилот для Quantum L7 AI', {
      priorLedger: { activeTopic: 'partnership', business: { intakeTurns: 1, operatorRequestTurns: 0 } },
    })
    ...
    const withContact = run('мой email partner@example.invalid и telegram @partner_l7_ai, можно передать оператору', {
    ...
    expect(withContact.text).toMatch(/Обращение сформировано и передано оператору/iu)
  })
  ...
  it('supports DM-only operator handoff when the user refuses external contacts', () => {
    const r = run('оператор нужен, но без контактов, пишите мне только в DM Quantum Messenger', {
      priorLedger: { activeTopic: 'contact', business: { intakeTurns: 2, operatorRequestTurns: 1, contactPrompted: true } },
    })
  ...
  it('returns bounded social support to QL7 choices after several supportive turns', () => {
    const r = run('мне плохо и хочется просто поговорить, меня травят и я держусь без курения третий день', {
      priorLedger: { activeTopic: 'support_system', social: { supportiveTurns: 3 } },
    })
    expect(r.analysis.messageAct).toBe('emotional_support')
    expect(r.contentPlan.supportiveBoundary).toBe(true)
    expect(r.surface.surfaceKind).toBe('choices')
    expect(r.surface.options).toHaveLength(4)
    expect(r.surface.other).toBeTruthy()
    expect(r.text).toMatch(/разговорные функции намеренно ограничены/iu)
  })
  ...
  it('ships expanded semantic banks for all 32 locales', () => {
    ...
    expect(coverage.version).toBe('15.3.2')
  ...
  it('ships 32-locale human variation banks for support, humor, business and warnings', () => {
    const coverage = getQl7HumanVariationCoverage()
    expect(coverage).toMatchObject({ ok: true, localeCount: 32, version: '15.3.1' })
    expect(coverage.minVariantsPerLocale).toBeGreaterThanOrEqual(41600)
    expect(coverage.totalVariants).toBeGreaterThan(1300000)
    expect(coverage.rows.every((row) => row.categoryCounts.emotional > 0)).toBe(true)
    expect(coverage.rows.every((row) => row.categoryCounts.humor > 0)).toBe(true)
    expect(coverage.rows.every((row) => row.categoryCounts.businessCollectBrief > 0)).toBe(true)
    expect(coverage.rows.every((row) => row.categoryCounts.strictWarning > 0)).toBe(true)
    ...
    const ruHumor = new Set(Array.from({ length: 16 }, (_, index) => pickQl7HumanVariation('ru', 'humor', { seed: `humor:${index}` })))
    expect(ruHumor.size).toBeGreaterThanOrEqual(8)
    ...
    expect(deSupport.text).toMatch(/Ich|QL7|Schritt/u)
    ...
    const frBusiness = executeQl7SupportTurnRuntime({
      ...
      priorLedger: { activeTopic: 'partnership', business: { intakeTurns: 1 } },
      now: '2026-08-02T00:00:00.000Z',
    })
    expect(frBusiness.contentPlan.relationshipIntent.stage).toBe('collect_contact')
    expect(frBusiness.text).toMatch(/partenariat|objectif|email|Telegram|DM|contact/u)
    ...
    for (const locale of providerLocales) {
      for (const category of ['businessCollectBrief', 'businessCollectContact']) {
        for (let index = 0; index < 12; index += 1) {
          const text = pickQl7HumanVariation(locale, category, {
            seed: `provider:${locale}:${category}:${index}`,
          })
          expect(text.length, `${locale}:${category}:${index}`).toBeGreaterThan(8)
          expect(hasAdjacentRepeatedPhrase(text), `${locale}:${category}:${index}:${text}`).toBe(false)
        }
      }
    }
  }, 60000)

  it('ships the 46-domain 32-locale knowledge corpus at the required combat scale', () => {
  ...
  it('answers topic recall without leaking into stale QCoin adapter', () => {
    ...
    expect(r.ledger.activeTopic).toBe('qcoin')
  })
  ...
  it('uses provider-locale banks for data intent and profanity context', () => {
  ...
  it('builds a Russian operator SMTP report with DM action, rating criteria and moderation columns', () => {
    ...
    expect(operatorCase.rating.criteria).toHaveLength(6)
  ...
  it('keeps entry greetings diverse and separates fresh versus continuing threads', () => {
    for (const locale of QL7_SUPPORT_ALL_LOCALES) {
      expect(listQl7SupportEntryGreetingsV11(locale).length, locale).toBeGreaterThanOrEqual(1500)
      const fresh = selectQl7SupportEntryGreetingV11({ locale, entryMode: 'fresh', seed: `fresh:${locale}`, now: Date.parse('2026-08-01T09:00:00.000Z') })
      const cont = selectQl7SupportEntryGreetingV11({ locale, entryMode: 'continue', seed: `continue:${locale}`, now: Date.parse('2026-08-01T09:00:00.000Z') })
      expect(cont.id).toContain('continue')
      expect(fresh.id).not.toContain('continue')
      expect(cont.text).not.toBe(fresh.text)
    }
  }, 60000)

  it('does not use QL7 Support as a repeated greeting card title', () => {
    expect(getQl7SupportSurfaceTitleV13({ role: 'greeting', topic: 'support_system', locale: 'ru' })).toBe('Рад помочь')
    expect(getQl7SupportSurfaceTitleV13({ role: 'greeting', topic: 'support_system', locale: 'en' })).toBe('Ready to help')
  })

  it('adds product actions to every ecosystem explanation surface', () => {
    for (const topic of QL7_SUPPORT_ECOSYSTEM_TOPICS) {
      const action = getQl7SupportTopicActionV9(topic, { locale: 'ru', seed: `unit:${topic}` })
  ...
  it('detects self-harm crisis language without locking emergency input', () => {
    ...
    expect(r.operatorCase).toBeTruthy()
    expect(r.stateEvents.map((event) => event.state)).toContain('attention_required')
    expect(r.stateEvents.some((event) => event.rawStates?.includes('operator_pending'))).toBe(true)
  })
  ...
  it('builds an AI-Recomendation crypto surface with quota, market table and no-financial-advice warning', () => {
    ...
    expect(r.surface.title).toBe('AI-Recomendation')
    expect(r.surface.semanticRole).toBe('ai_recommendation')
    expect(r.surface.tables[0].schema).toBe('ql7.table.ai.recommendation')
  ...
  it('uses production banks for varied AI recommendation and quota replies without raw routes', () => {
    ...
    for (let index = 0; index < 16; index += 1) {
      ...
      expect(r.analysis.topic).toBe('exchange_ai')
      expect(r.text).toMatch(/BTCUSDT|5m|HOLD|72%/u)
      expect(r.text).toMatch(/financial advice|Educational/u)
  ...
  it('proves live-read coverage for user geo, ads, balance, subscriptions, complaints, MetaMarket, BattleCoin and Telegram without leaking identifiers', async () => {
    ...
    const snapshot = await readQl7LiveUserSnapshot({
      ...
      services: {
        identityGraph: {
          buildQl7IdentityGraphV8: async () => ({ lookupIds: [wallet, telegram, `telegram:${telegram}`] }),
          publicQl7IdentityGraphProjectionV8: () => ({ lookupIdsMasked: ['0x1111...111111', '123...789'] }),
        },
...
describe('QL7 Support V6 gold topic calibration', () => {
  const cases = [
  ...
  it('repairs production route candidates for the seven observed clusters', () => {
    ...
    for (const [text, topic, expected] of cases) {
      const route = calibrateQl7SupportRouteV13({ text, route: { topic, messageAct: 'informational_question' }, analysis: { topic } })
...
describe('QL7 Support V8 Ads package status calibration', () => {
  const cases = [
  ...
  it('repairs a production support_system route for Spanish Ads package status', () => {
    const route = calibrateQl7SupportRouteV13({
  ...
  it('renders verified-empty Ads package evidence with the correct table, badge and action', () => {
    const result = executeQl7SupportTurnRuntime({
      mode: 'test',
      requestId: 'v8:ads-package:es',
      userTurnId: 'v8:user:ads-package:es',
      selectedLocale: 'es',
      text: 'Muestra el estado del paquete publicitario ELITE',
      now: '2026-07-31T00:00:00.000Z',
      adapterReceipts: [{
        id: 'fixture:v8:ads-packages',
        ...
        durationMs: 1,
        writeCount: 0,
        evidenceHash: 'v8-ads-package-evidence',
        checkedAt: '2026-07-31T00:00:00.000Z',
        verified: true,
      }],
    })
...
describe('QL7 Support V10 adversarial resilience calibration', () => {
  it('keeps quoted threats as reported speech and blocks protected boundary adapters', () => {
    const quoted = executeQl7SupportTurnRuntime({
      mode: 'test',
      requestId: 'v10:quoted',
      userTurnId: 'v10:quoted:user',
      selectedLocale: 'ru',
    ...
    const protectedBoundary = executeQl7SupportTurnRuntime({
      mode: 'test',
      requestId: 'v10:boundary',
      userTurnId: 'v10:boundary:user',
      selectedLocale: 'en',
...
describe('V11 fact and boundary calibration', () => {
  it('keeps VIP text and table on the same inactive fact', async () => {
    ...
    const result = executeQl7SupportTurnRuntime({ mode:'test', locale:'en', text:'Show VIP status', requestId:'v11-vip' }, { vip:{ adapter:'vip', executed:true, writeCount:0, resultKind:'verified', result:{ active:true, status:'inactive', tier:'VIP' }, checkedAt:'2026-08-03T00:00:00.000Z' } })
    expect(result.factProjection.status).toBe('inactive')
    expect(result.text.toLowerCase()).toContain('inactive')
  ...
  it('does not punish an uncertain first insult', async () => {
    const { executeQl7SupportTurnRuntime } = await import('../../../lib/ql7-support/runtime/executeTurn.js')
    const result = executeQl7SupportTurnRuntime({ mode:'test', locale:'en', selectedLocale:'en', text:'st.u.pid...', requestId:'v11-uncertain', now:'2026-08-03T00:00:00.000Z' }, {})
    ...
    expect(result.ledger.safety.pendingBoundaryClarification.active).toBe(true)
  })
  it('apologizes and restores the topic when the user denies an uncertain insult', async () => {
    const { executeQl7SupportTurnRuntime } = await import('../../../lib/ql7-support/runtime/executeTurn.js')
    const priorLedger = {
      activeTopic:'qcoin', activeGoal:'check balance', lastMaterialTurnId:'v11-material',
      safety:{ directInsultCount:0, confirmedDirectInsultCount:0, history:[], pendingBoundaryClarification:{ active:true, resumeTopic:'qcoin', resumeGoal:'check balance', lastMaterialTurnId:'v11-material' } },
    }
    const result = executeQl7SupportTurnRuntime({ mode:'test', locale:'en', selectedLocale:'en', text:'No, I meant the app, not you', requestId:'v11-denial', priorLedger, now:'2026-08-03T00:00:00.000Z' }, {})
    expect(result.safety.category).toBe('insult_denied')
    expect(result.safety.escalationLevel).toBe(0)
    expect(result.safety.cooldownMs).toBe(0)
    expect(result.ledger.activeTopic).toBe('qcoin')
    expect(result.ledger.safety.pendingBoundaryClarification.active).toBe(false)
  ...
  it('uses the existing first measure when a confirmed insult continues after clarification', async () => {
    ...
    const result = executeQl7SupportTurnRuntime({ mode:'test', locale:'en', selectedLocale:'en', text:'you idiot', requestId:'v11-continued', priorLedger, now:'2026-08-03T00:00:00.000Z' }, {})
...
describe('V11 adversarial normalization and security escalation calibration', () => {
  ...
  it('restores joined direct insults to the existing safety state machine', () => {
    ...
    for (const [locale,text] of rows) {
      const result=executeQl7SupportTurnRuntime({mode:'test',selectedLocale:locale,locale,text,requestId:`joined:${locale}`,priorLedger:{safety:{directInsultCount:1}},now:'2026-08-03T00:00:00.000Z'}, {})
      expect(result.safety.category).toBe('direct_insult')
