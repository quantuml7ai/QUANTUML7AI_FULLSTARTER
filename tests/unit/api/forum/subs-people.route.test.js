...
vi.mock('../../../../app/api/forum/_db.js', () => ({
  ...
  getUsersPublicMini: vi.fn(async (ids) => {
    ...
    if (harness.state.usersDelayMs > 0) {
      await new Promise((resolve) => setTimeout(resolve, harness.state.usersDelayMs))
    }
...
describe('/api/forum/subs/people Mongo identity compatibility', () => {
  ...
  test('waits for canonical profile hydration instead of replacing a slower followers page with anonymous rows', async () => {
    harness.state.pages['0xraw'] = {
      ids: ['0xfriend'],
      users: [],
      counts: { followers: 1, following: 0 },
    ...
    harness.state.counts['0xraw'] = { followers: 1, following: 0 }
    harness.state.users['0xfriend'] = { userId: '0xfriend', nickname: 'Friend', icon: '/friend.png' }
    harness.state.usersDelayMs = 1850
    ...
    expect(json.users).toEqual([
      expect.objectContaining({ userId: '0xfriend', nickname: 'Friend', icon: '/friend.png' }),
    ])
  })
