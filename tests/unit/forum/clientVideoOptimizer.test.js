...
describe('forumClientVideoOptimizer V4 policy', () => {
  test('declares the calibrated 27 MiB preferred target and 30 MiB hard ceiling', () => {
    ...
    expect(FORUM_CLIENT_VIDEO_OPTIMIZER_MIN_TARGET_BYTES).toBe(1 * MIB)
  })
  ...
  test('uses duration-sensitive budgets instead of inflating short compatible sources', () => {
    ...
    expect(shortTarget).toBeGreaterThanOrEqual(1 * MIB)
  ...
  test('keeps compact 1 MiB videos transcode-plan eligible without a 3 MiB minimum-size gate', () => {
    const alreadySafe = selectForumVideoEncodingProfile({
      durationSeconds: 10,
      sourceBytes: 1 * MIB,
      displayWidth: 1920,
      displayHeight: 1080,
    ...
    expect(alreadySafe.targetOutputBytes).toBe(1 * MIB)
    expect(alreadySafe.bitratePlan.videoBitrate).toBeGreaterThanOrEqual(alreadySafe.minimumVideoBitrate)

    for (const durationSeconds of [15, 30, 120, 300]) {
      const plan = selectForumVideoEncodingProfile({
        durationSeconds,
        sourceBytes: 1 * MIB,
        displayWidth: 1920,
        displayHeight: 1080,
      ...
      expect(plan.targetOutputBytes).toBeGreaterThan(1 * MIB)
      expect(plan.targetOutputBytes).toBeLessThanOrEqual(FORUM_CLIENT_VIDEO_OPTIMIZER_OUTPUT_MAX_BYTES)
      expect(plan.bitratePlan.videoBitrate).toBeGreaterThanOrEqual(plan.minimumVideoBitrate)
    }
  })
