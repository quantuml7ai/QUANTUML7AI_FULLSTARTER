...
function collectionApi(rows = [], name = '', telemetry = null) {
  return {
    find(query = {}) {
      if (telemetry) telemetry.find[name] = Number(telemetry.find[name] || 0) + 1
    ...
    async findOne(query = {}) {
      if (telemetry) telemetry.findOne[name] = Number(telemetry.findOne[name] || 0) + 1
...
function fakeDb({ rows, mediaRows = null, profileGeo, coreTopics = [], corePosts = [], telemetry = null }) {
  return {
    collection(name) {
      if (name === 'forum_geo_feed_index') return collectionApi(rows, name, telemetry)
      if (name === 'forum_media_feed_index') return collectionApi(mediaRows || rows, name, telemetry)
      if (name === 'forum_core_posts') return collectionApi(corePosts, name, telemetry)
      if (name === 'forum_core_topics') return collectionApi(coreTopics, name, telemetry)
      if (name === 'profiles') return collectionApi(profileGeo ? [{ _id: 'profile:viewer', accountId: 'viewer', _geoCurrent: profileGeo }] : [], name, telemetry)
      return collectionApi([], name, telemetry)
    },
  }
}
...
describe('forum server feed geo rings', () => {
  ...
  test('does not repeat the thread-counter lookup after feed or media geo ring hydration', async () => {
    const profileGeo = {
      known: true,
      country: 'FR',
      region: 'IDF',
    ...
    const row = makePost({
      id: 'budget-post',
      scopeKeys: ['city:fr:idf:paris', 'region:fr:idf', 'country:fr', 'global'],
      views: 3,
      ts: 7,
    })
    row.mediaKind = 'video'
    const corePost = {
      _id: 'post:budget-post',
      id: 'budget-post',
      postId: 'budget-post',
      topicId: 'topic-1',
      text: 'budget-post',
    ...
    const telemetry = { find: {}, findOne: {} }
    const db = fakeDb({ rows: [row], profileGeo, corePosts: [corePost], telemetry })
    pageReader.__setTestDb(db)
    profileGeoPrimary.__setTestDb(db)

    const page = await pageReader.readForumFeedPage({
      input: {
        mode: 'geo',
        sort: 'views',
        seed: 'query-budget',
    ...
    expect(page.items.map((item) => item.postId)).toEqual(['budget-post'])
    expect(Number(telemetry.find.forum_thread_index || 0)).toBe(2)

    const mediaPage = await pageReader.readForumMediaFeedPage({
      input: {
        mode: 'geo',
        mediaKind: 'all',
        sort: 'views',
        seed: 'query-budget-media',
        accountId: 'viewer',
        pageSize: 1,
      },
    })

    expect(mediaPage.items.map((item) => item.postId)).toEqual(['budget-post'])
    expect(Number(telemetry.find.forum_thread_index || 0)).toBe(4)
  })
})
