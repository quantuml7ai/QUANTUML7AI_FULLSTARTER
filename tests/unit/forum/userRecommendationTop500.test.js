...
function validDoc(users = []) {
  ...
  const ranked = users.map((row, index) => ({
    ...
    nickname: row.nickname || `User ${row.canonicalAccountId}`,
    avatar: row.avatar || `/${row.canonicalAccountId}.png`,
    rank: index + 1,
  ...
  return {
    ...
    buildWeek: pool.weeklyBuildKey(built),
    nextBuildAt: new Date(pool.nextWeeklyBuildAtMs(built)).toISOString(),
    leaseToken: null,
    leaseUntil: null,
...
describe('forum recommendation Top-500 policy', () => {
  it('uses an exact Monday UTC weekly key and next boundary', () => {
    expect(pool.REBUILD_MS).toBe(7 * 24 * 60 * 60 * 1000)
    expect(pool.weeklyBuildKey(Date.parse('2026-08-16T23:59:59.000Z'))).toBe('2026-08-10')
    expect(pool.weeklyBuildKey(Date.parse('2026-08-17T00:00:00.000Z'))).toBe('2026-08-17')
    expect(pool.nextWeeklyBuildAtMs(Date.parse('2026-08-12T10:00:00.000Z'))).toBe(Date.parse('2026-08-17T00:00:00.000Z'))
  })
  ...
  it('validates a complete schema-10 weekly materialized pool', () => {
    const doc = validDoc([
  ...
  it('accepts a completed empty week so active devices do not trigger endless rebuilds', () => {
    const doc = validDoc([])
    expect(pool.validatePoolDocument(doc, { requireUsers: false })).toEqual({ ok: true, issues: [] })
  })
  ...
  it('permits one same-week schema migration but rejects an already-current weekly snapshot', async () => {
    const targetWeek = pool.weeklyBuildKey(NOW)
    const state = { doc: validDoc([{ canonicalAccountId: 'a' }]) }
    const collection = {
      async updateOne(filter, update) {
        if (update?.$setOnInsert) return { modifiedCount: 0 }
        const leaseFree = !state.doc.leaseUntil || Date.parse(state.doc.leaseUntil) <= NOW
        const weeklyCurrent = state.doc.buildWeek === targetWeek
          && state.doc.schemaVersion === pool.SCHEMA_VERSION
          && state.doc.profileResolution?.presentationHydration === 'weekly-materialized'
        if (!leaseFree || weeklyCurrent) return { modifiedCount: 0 }
        state.doc = { ...state.doc, ...(update?.$set || {}) }
        return { modifiedCount: 1 }
      },
      async findOne() { return state.doc },
    ...
    expect((await pool.acquireLease({ db, nowMs: NOW })).acquired).toBe(false)

    state.doc.schemaVersion = 9
    state.doc.profileResolution.presentationHydration = 'live-per-response'
    expect((await pool.acquireLease({ db, nowMs: NOW })).acquired).toBe(true)

    state.doc.schemaVersion = pool.SCHEMA_VERSION
    state.doc.profileResolution.presentationHydration = 'weekly-materialized'
    state.doc.buildWeek = targetWeek
    state.doc.leaseToken = null
    state.doc.leaseUntil = null
    expect((await pool.acquireLease({ db, nowMs: NOW })).acquired).toBe(false)
  })
  ...
  it('delivers stored weekly presentation without live profile, follower or VIP hydration', async () => {
    const materialized = validDoc([
      { canonicalAccountId: 'a', nickname: 'Weekly A', avatar: '/weekly-a.png', metrics: { followers: 9, vipAtBuild: true } },
      { canonicalAccountId: 'b', nickname: 'Weekly B', avatar: '/weekly-b.png', metrics: { followers: 2 } },
    ])
    const db = {
      collection(name) {
        if (name === pool.COLLECTION) return { async findOne() { return materialized } }
        if (name === 'forum_media_locks') return { find() { return cursor([]) } }
        if (name === 'forum_subscription_sets') return { async findOne() { return null } }
        throw new Error('unexpected_live_delivery_collection:' + name)
      },
    }

    const result = await pool.getSnapshot({
      explicitDb: db,
      requestIdentityResolved: true,
      nowMs: NOW,
    })

    expect(result.poolReady).toBe(true)
    expect(result.rebuildDue).toBe(false)
    expect(result.users).toEqual([
      expect.objectContaining({ canonicalAccountId: 'a', nickname: 'Weekly A', avatar: '/weekly-a.png', followersCount: 9, isVip: true }),
      expect.objectContaining({ canonicalAccountId: 'b', nickname: 'Weekly B', avatar: '/weekly-b.png', followersCount: 2, isVip: false }),
    ])
  })

  it('resolves only request identities and excludes the canonical viewer from the snapshot', async () => {
    const materialized = validDoc([
      { canonicalAccountId: 'viewer-canonical', metrics: { followers: 1 } },
      { canonicalAccountId: 'exclude-canonical', metrics: { followers: 1 } },
    ...
    try {
      ...
      expect(result.users.map((user) => user.canonicalAccountId)).toEqual(['author-a'])
      expect(identityCalls).toEqual([
        { raw: 'viewer-alias', phase: 'delivery-request-canonical' },
        { raw: 'exclude-alias', phase: 'delivery-request-canonical' },
      ])
    } finally {
      pool.__setTestCanonicalResolver(null)
    }
  ...
  it('excludes an active media safety lock during build without creating an extra rebuild trigger', async () => {
    ...
    try {
      ...
      expect(doc.moderation.nextBuildScheduledForMediaUnlock).toBe(false)
      expect(Date.parse(doc.nextBuildAt)).toBe(pool.nextWeeklyBuildAtMs(NOW))
      expect(pool.validatePoolDocument(doc)).toEqual({ ok: true, issues: [] })
    } finally {
  ...
  it('keeps the weekly member but suppresses a newly active MediaLock in one snapshot batch', async () => {
    const buildDb = dbForBuild({ followers: [{ _id: 'followers:u', count: 1, members: ['f1'] }] })
    pool.__setTestCanonicalResolver(async (raw) => ({ canonicalAccountId: raw, aliasSet: [raw], conflictWarnings: [] }))
    pool.__setTestProfileReader(async (id) => ({ nickname: 'Weekly-' + id, icon: '/weekly-' + id + '.png' }))
    pool.__setTestMediaLockReader(async (_id, phase) => phase === 'delivery-snapshot-media-lock'
      ? { locked: true, untilMs: NOW + 60_000 }
      : { locked: false, untilMs: NOW - 1000 })
    try {
      const doc = await pool.calculatePool({ db: buildDb, nowMs: NOW })
      expect(doc.users).toHaveLength(1)
      const deliveryDb = {
        collection(name) {
          if (name === pool.COLLECTION) return { async findOne() { return doc } }
          if (name === 'forum_subscription_sets') return { async findOne() { return null } }
          if (name === 'forum_media_locks') return { find() { return cursor([]) } }
          throw new Error('unexpected_collection:' + name)
        },
