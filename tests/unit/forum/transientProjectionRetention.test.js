...
describe('forum transient projection retention', () => {
  ...
  test('bounds a transient projection view without truncating the source history', () => {
    const source = Array.from({ length: 100 }, (_, index) => ({ id: `video-${index}` }))
    const bounded = boundForumTransientProjectionItems(source, 60)

    expect(source).toHaveLength(100)
    expect(bounded).toHaveLength(60)
    expect(bounded[0].id).toBe('video-40')
    expect(bounded.at(-1).id).toBe('video-99')
    expect(boundForumTransientProjectionItems(source, 0)).toBe(source)
    expect(boundForumTransientProjectionItems(source, 120)).toBe(source)
  })
