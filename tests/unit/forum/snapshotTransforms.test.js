...
describe('forum snapshot transforms', () => {
  ...
  test('promotes authoritative full-snapshot rows out of transient projection ownership', () => {
    const result = applyForumFullSnapshot(
      {
        ...
        posts: [{
          ...
          topicId: 'topic-1',
          text: 'transient',
          __ql7TransientProjectionOnly: true,
          __ql7TransientProjectionOwners: ['forum:thread:topic-1:post-1'],
        }],
      },
      {
        topics: [],
        posts: [{ id: 'post-1', topicId: 'topic-1', text: 'canonical' }],
      },
      emptyTombstones,
    )

    expect(result.posts).toHaveLength(1)
    expect(result.posts[0].text).toBe('canonical')
    expect(result.posts[0].__ql7TransientProjectionOnly).toBeUndefined()
    expect(result.posts[0].__ql7TransientProjectionOwners).toBeUndefined()
  })
