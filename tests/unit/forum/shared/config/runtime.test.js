...
describe('readForumRuntimeConfig', () => {
  ...
  it('keeps the recommendation rail at exactly 15 while cadence remains configurable', async () => {
    ...
    expect(runtime.userRecommendations).not.toHaveProperty('batchesPerRequest')
    expect(runtime.userRecommendations).not.toHaveProperty('prefetchRailsAhead')
    expect(runtime.userRecommendations).not.toHaveProperty('rotateSec')
  })
