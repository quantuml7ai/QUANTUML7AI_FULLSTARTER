...
describe('quest Mongo primary repository', () => {
  ...
  test('writes new wallet quest progress only under the canonical EIP-55 principal', async () => {
    const checksum = '0x51be760fA3775263D2C2496824f23Ca31d829e6A'
    const lower = checksum.toLowerCase()

    await questPrimary.writeCard(lower, '3', {
      done: ['1'],
      taskCount: 1,
    })

    const rows = memoryDb.collection('quest_progress').rows
    expect(rows.has(`quest:progress:${checksum}:3`)).toBe(true)
    expect(rows.has(`quest:progress:${lower}:3`)).toBe(false)
    expect(rows.get(`quest:progress:${checksum}:3`)).toMatchObject({
      userId: checksum,
      userRef: checksum,
      sourceRedisKey: `quest:progress:${checksum}`,
    })
    await expect(questPrimary.readProgress(lower)).resolves.toMatchObject({
      3: { done: ['1'], taskCount: 1 },
    })
  })

  test('reuses legacy lowercase wallet progress without creating a second business object', async () => {
    const checksum = '0x51be760fA3775263D2C2496824f23Ca31d829e6A'
    const lower = checksum.toLowerCase()
    const legacyId = `quest:progress:${lower}:4`

    await memoryDb.collection('quest_progress').updateOne(
      { _id: legacyId },
      { $set: questPrimary.cardDoc({
        userId: lower,
        cardId: '4',
        card: { done: ['1'], taskCount: 2 },
      }) },
      { upsert: true },
    )

    await questPrimary.writeCard(checksum, '4', {
      done: ['1', '2'],
      taskCount: 2,
    })
    ...
    expect(rows.has(`quest:progress:${checksum}:4`)).toBe(false)
    expect(rows.get(legacyId)).toMatchObject({
      progressId: `quest:progress:${checksum}:4`,
      userId: checksum,
      userRef: checksum,
      sourceRedisKey: `quest:progress:${checksum}`,
      done: ['1', '2'],
