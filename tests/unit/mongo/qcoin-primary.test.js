...
import transactionContextModule from '../../../lib/mongo/transaction-context.cjs'
...
const transactionContext = transactionContextModule?.default || transactionContextModule
...
function matches(doc, filter = {}) {
  ...
  if (Array.isArray(filter.$and)) return filter.$and.every((item) => matches(doc, item))
...
describe('qcoin Mongo primary repository', () => {
  ...
  test('debits a purchase only when the authoritative QCoin balance is sufficient', async () => {
    const uid = 'purchase-user'
    const idempotencyKey = 'qcp:test:sufficient:debit'
    await qcoinPrimary.incrementBalance({ uid, amount: 25, eventKind: 'seed' })
    const receipt = decisionReceipt.createDecisionReceipt({
      operationId: 'operation:qcoin-purchase-sufficient',
      envelopeHash: 'hash:qcoin-purchase-sufficient',
    ...
    const result = await qcoinPrimary.debitBalanceIfSufficient({
      uid,
      amount: 19.99,
      eventKind: 'qcoin_vip_purchase',
      route: '/api/pay/qcoin-purchase',
      economicRouteId: 'qcoin.entitlement.purchase.debit',
      sourceEventId: 'qcp:test:sufficient',
      idempotencyKey,
      decisionReceipt: receipt,
    })

    expect(result.balance).toBeCloseTo(5.01, 8)
    await expect(qcoinPrimary.readAccount(uid)).resolves.toMatchObject({ balance: 5.01 })
    expect(memoryDb.collection('qcoin_ledger').rows.size).toBe(2)
  })

  test('refuses an insufficient purchase without a debit or purchase ledger row', async () => {
    const uid = 'purchase-user-low'
    const idempotencyKey = 'qcp:test:insufficient:debit'
    await qcoinPrimary.incrementBalance({ uid, amount: 10, eventKind: 'seed' })
    const receipt = decisionReceipt.createDecisionReceipt({
      operationId: 'operation:qcoin-purchase-insufficient',
      envelopeHash: 'hash:qcoin-purchase-insufficient',
      routeId: 'qcoin.entitlement.purchase.debit',
      operationType: 'debit',
      actorAccountId: uid,
      targetAccountId: uid,
    ...
    await expect(qcoinPrimary.debitBalanceIfSufficient({
      uid,
      amount: 19.99,
      eventKind: 'qcoin_vip_purchase',
      route: '/api/pay/qcoin-purchase',
      economicRouteId: 'qcoin.entitlement.purchase.debit',
      sourceEventId: 'qcp:test:insufficient',
      idempotencyKey,
      decisionReceipt: receipt,
    })).rejects.toMatchObject({
      code: 'INSUFFICIENT_QCOIN',
      balance: 10,
      required: 19.99,
    })

    await expect(qcoinPrimary.readAccount(uid)).resolves.toMatchObject({ balance: 10 })
    expect(memoryDb.collection('qcoin_ledger').rows.size).toBe(1)
  })
  ...
  test('reads legacy alias balance together with new canonical referral rewards without double crediting', async () => {
    ...
    expect(memoryDb.collection('qcoin_accounts').rows.has('account:0x2222222222222222222222222222222222222222')).toBe(false)
    expect(memoryDb.collection('qcoin_accounts').rows.get('account:777001')).toMatchObject({
      userId: '0x2222222222222222222222222222222222222222',
  ...
  test('updates a legacy lowercase physical QCoin row while writing canonical EIP-55 identity fields', async () => {
    ...
    expect(memoryDb.collection('qcoin_accounts').rows.has(`legacy:${lower}`)).toBe(true)
    expect(memoryDb.collection('qcoin_accounts').rows.has(`account:${checksum}`)).toBe(false)
    expect(memoryDb.collection('qcoin_accounts').rows.get(`legacy:${lower}`)).toMatchObject({
      balance: 98.25,
      userId: checksum,
      accountId: checksum,
    })

    await qcoinPrimary.writeState(lower, {
      startedAt: 1000,
      lastActiveAt: 2000,
      lastConfirmAt: 2000,
      presenceOfflineAt: 0,
    ...
    await qcoinPrimary.markPresenceOffline({ uid: lower, now: 3000 })

    expect(memoryDb.collection('qcoin_accounts').rows.has(`account:${checksum}`)).toBe(false)
    expect(memoryDb.collection('qcoin_accounts').rows.get(`legacy:${lower}`)).toMatchObject({
      balance: 98.5,
      seconds: 20,
      presenceOfflineAt: 3000,
      userId: checksum,
      accountId: checksum,
    })
  })
  ...
  test('never chooses or sums another wallet balance even if a legacy alias list is contaminated', async () => {
    const walletA = '0x7777777777777777777777777777777777777777'
    const walletB = '0x8888888888888888888888888888888888888888'
    qcoinPrimary.__setTestAliasResolver(async () => ({
      canonical: walletB,
      aliases: [walletA, walletB],
    }))

    await memoryDb.collection('qcoin_accounts').updateOne(
      { _id: `account:${walletA}` },
      { $set: { uid: walletA, userId: walletA, accountId: walletA, balance: 22_000_000 } },
      { upsert: true },
    )
    await memoryDb.collection('qcoin_accounts').updateOne(
      { _id: `account:${walletB}` },
      { $set: { uid: walletB, userId: walletB, accountId: walletB, balance: 0.25 } },
      { upsert: true },
    )

    await expect(qcoinPrimary.readAccount(walletB)).resolves.toMatchObject({
      accountId: walletB,
      balance: 0.25,
    })
    const result = await qcoinPrimary.incrementBalance({
      uid: walletB,
      amount: 1,
      eventKind: 'identity_isolation_proof',
      idempotencyKey: 'identity:isolation:wallet-b',
    })
    expect(result.balance).toBe(1.25)
    expect(memoryDb.collection('qcoin_accounts').rows.get(`account:${walletA}`).balance).toBe(22_000_000)
    expect(memoryDb.collection('qcoin_accounts').rows.get(`account:${walletB}`).balance).toBe(1.25)
  })

  test('rejects an economic mutation when canonical identity resolution reports a conflict', async () => {
    qcoinPrimary.__setTestAliasResolver(async () => ({
      canonical: '0x8888888888888888888888888888888888888888',
      aliases: ['6783588404'],
      conflicted: true,
    }))

    await expect(qcoinPrimary.incrementBalance({
      uid: '6783588404',
      amount: 1,
      eventKind: 'must_not_write',
    })).rejects.toMatchObject({ code: 'QCOIN_IDENTITY_LINK_CONFLICT' })
    expect(memoryDb.collection('qcoin_accounts').rows.size).toBe(0)
    expect(memoryDb.collection('qcoin_ledger').rows.size).toBe(0)
  })

  test('binds a signed debit receipt to the absolute economic amount', async () => {
    const uid = 'receipt-user'
    const idempotencyKey = 'battlecoin:open:receipt-user:1:debit'
    await qcoinPrimary.incrementBalance({ uid, amount: 10, eventKind: 'seed' })
    const receipt = decisionReceipt.createDecisionReceipt({
      operationId: 'operation:debit-proof',
      envelopeHash: 'hash:debit-proof',
    ...
    await expect(qcoinPrimary.incrementBalance({
      ...
      idempotencyKey,
      decisionReceipt: receipt,
    })).resolves.toMatchObject({ balance: 7.75 })
  })

  test('memoizes canonical identity only inside one Mongo operation context', async () => {
    let calls = 0
    qcoinPrimary.__setTestAliasResolver(async (uid) => {
      calls += 1
      return { canonical: uid, aliases: [`telegram:${uid}`] }
    })

    await transactionContext.withMongoOperationContext(async () => {
      await Promise.all([
        qcoinPrimary.resolveCanonicalAccountId('777001'),
        qcoinPrimary.resolveAccountIdCandidates('777001'),
        qcoinPrimary.resolveCanonicalAccountId('777001'),
      ])
    })
    expect(calls).toBe(1)

    await transactionContext.withMongoOperationContext(() => (
      qcoinPrimary.resolveCanonicalAccountId('777001')
    ))
    expect(calls).toBe(2)
  })
