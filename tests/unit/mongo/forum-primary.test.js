...
const CANONICAL_SUBSCRIPTION_VIEWER = '0x1111111111111111111111111111111111111111'
const CANONICAL_SUBSCRIPTION_AUTHOR = '0x2222222222222222222222222222222222222222'
...
describe('forum Mongo primary repository', () => {
  ...
  test('native video poster belongs to posts, survives Mongo read and snapshot, and never contaminates topic creation', async () => {
    const posterUrl = 'https://media.example.test/forum/video-posters/poster.webp'
    const { topic } = await forumPrimary.createTopic({
      title: 'Poster topic',
      description: 'No poster field on topic',
      userId: 'alice',
      nickname: 'Alice',
    })
    expect(topic).not.toHaveProperty('posterUrl')

    const { post } = await forumPrimary.createPost({
      topicId: topic.id,
      text: 'native video',
      userId: 'alice',
      nickname: 'Alice',
    ...
    expect(post).toMatchObject({ topicId: topic.id, posterUrl })
    await expect(forumPrimary.getPost(post.id)).resolves.toMatchObject({ posterUrl })

    const snapshot = await forumPrimary.readSnapshot({ sinceRev: 0 })
    expect(snapshot.topics.find((row) => row.id === topic.id)).not.toHaveProperty('posterUrl')
    expect(snapshot.posts.find((row) => row.id === post.id)).toMatchObject({ posterUrl })
  })
  ...
  test('reuses a legacy lowercase reaction row while canonicalizing its logical owner', async () => {
    const checksum = '0x51be760fA3775263D2C2496824f23Ca31d829e6A'
    const lower = checksum.toLowerCase()
    const { topic } = await forumPrimary.createTopic({ title: 'T', userId: 'alice' })
    const { post } = await forumPrimary.createPost({ topicId: topic.id, text: 'hello', userId: 'alice' })

    await memoryDb.collection('forum_post_reactions').updateOne(
      { _id: `reaction:${post.id}:${lower}` },
      { $set: { postId: post.id, userId: lower, kind: 'like', updatedAt: '2026-08-30T00:00:00.000Z' } },
      { upsert: true },
    )
    await memoryDb.collection('forum_core_posts').updateOne(
      { _id: `post:${post.id}` },
    ...
    const result = await forumPrimary.setPostReactionState({
      postId: post.id,
      userId: checksum,
      state: 'dislike',
    })

    expect(result).toMatchObject({ likes: 0, dislikes: 1, changed: true, likeDelta: -1 })
    const rows = memoryDb.collection('forum_post_reactions').rows
    expect(rows.size).toBe(1)
    expect(rows.has(`reaction:${post.id}:${lower}`)).toBe(true)
    expect(rows.has(`reaction:${post.id}:${checksum}`)).toBe(false)
    expect(rows.get(`reaction:${post.id}:${lower}`)).toMatchObject({
      userId: checksum,
      kind: 'dislike',
    })
  })
  ...
  test('writes one canonical subscription set while legacy linked aliases remain readable', async () => {
    await memoryDb.collection('account_aliases').updateOne(
      { _id: 'alias:telegram:777001' },
      {
        $set: {
          ...
          aliasId: 'telegram:777001',
          aliasValue: '777001',
          accountId: CANONICAL_SUBSCRIPTION_VIEWER,
          canonicalAccountId: CANONICAL_SUBSCRIPTION_VIEWER,
        },
      },
      { upsert: true },
    )
    await memoryDb.collection('account_aliases').updateOne(
      { _id: 'alias:telegram:888002' },
      {
        $set: {
          ...
          aliasValue: '888002',
          accountId: CANONICAL_SUBSCRIPTION_AUTHOR,
          canonicalAccountId: CANONICAL_SUBSCRIPTION_AUTHOR,
        },
      },
      { upsert: true },
    )

    await expect(forumPrimary.toggleSubscription(CANONICAL_SUBSCRIPTION_VIEWER, CANONICAL_SUBSCRIPTION_AUTHOR))
      .resolves
      .toMatchObject({ ok: true, subscribed: true, followersCount: 1, followingCount: 1 })

    await expect(forumPrimary.listSubscriptions('telegram:777001')).resolves.toEqual([CANONICAL_SUBSCRIPTION_AUTHOR])
    await expect(forumPrimary.listSubscriptions('tma:777001')).resolves.toEqual([CANONICAL_SUBSCRIPTION_AUTHOR])
    await expect(forumPrimary.listSubscriptions('777001')).resolves.toEqual([CANONICAL_SUBSCRIPTION_AUTHOR])
    ...
    expect(page.ids).toEqual([CANONICAL_SUBSCRIPTION_AUTHOR])
    expect(memoryDb.collection('forum_subscription_sets').rows.has(`followingZ:${CANONICAL_SUBSCRIPTION_VIEWER}`)).toBe(true)
    expect(memoryDb.collection('forum_subscription_sets').rows.has(`followersZ:${CANONICAL_SUBSCRIPTION_AUTHOR}`)).toBe(true)
    expect(memoryDb.collection('forum_subscription_sets').rows.has('viewer:telegram:777001')).toBe(false)
    expect(memoryDb.collection('forum_subscription_sets').rows.has('followers:telegram:888002')).toBe(false)
  })

  test('canonical subscription state overrides stale legacy alias rows after unsubscribe', async () => {
    await memoryDb.collection('account_aliases').updateOne(
      { _id: 'alias:telegram:777001' },
      {
        $set: {
    ...
    // Historical relation exists only under a legacy alias-owned set.
    await memoryDb.collection('forum_subscription_sets').updateOne(
      { _id: 'viewer:telegram:777001' },
      {
        $set: {
          members: ['telegram:888002'],
          rows: [{ member: 'telegram:888002', score: 10 }],
        },
      },
      { upsert: true },
    )

    // The first canonical mutation must see that legacy relation and unsubscribe.
    await expect(forumPrimary.toggleSubscription('telegram:777001', 'telegram:888002'))
      .resolves
      .toMatchObject({ ok: true, subscribed: false, followersCount: 0, followingCount: 0 })

    // The old legacy document intentionally still exists until Mongo compaction.
    expect(memoryDb.collection('forum_subscription_sets').rows.has('viewer:telegram:777001')).toBe(true)

    // A canonical Z document now exists and is authoritative over stale legacy rows.
    const canonical = memoryDb.collection('forum_subscription_sets').rows.get(`followingZ:${CANONICAL_SUBSCRIPTION_VIEWER}`)
    expect(canonical).toBeTruthy()
    expect(canonical.members || []).toEqual([])

    await expect(forumPrimary.listSubscriptions('telegram:777001')).resolves.toEqual([])
    await expect(forumPrimary.listSubscriptions('tma:777001')).resolves.toEqual([])
    await expect(
      forumPrimary.listSubscriptionPeoplePage({
        userId: 'telegram:777001',
        mode: 'following',
        limit: 10,
      }),
    ).resolves.toMatchObject({ ids: [], totalCount: 0 })
  })
...
describe('forum post mutation journal and canonical media lock authority', () => {
  let memoryDb
  ...
  test('returns only compact public edit/delete deltas after the requested revision', async () => {
    const { topic } = await forumPrimary.createTopic({ title: 'T', description: 'D', userId: 'alice', nickname: 'Alice' })
    const { post } = await forumPrimary.createPost({ topicId: topic.id, text: 'before', userId: 'alice', nickname: 'Alice' })
    const baseline = await forumPrimary.readCounter('forum:rev', 0)

    const edited = await forumPrimary.updatePostText(post.id, 'after')
    expect(edited.rev).toBeGreaterThan(baseline)
    const deleted = await forumPrimary.deletePostBranchHard(post.id)
    expect(deleted.rev).toBeGreaterThan(edited.rev)

    const delta = await forumPrimary.readPublicPostMutationChangesSince({ sinceRev: baseline, limit: 16 })
    expect(delta.rev).toBe(deleted.rev)
    expect(delta.cursorRev).toBe(deleted.rev)
    expect(delta.hasMore).toBe(false)
    expect(delta.events).toEqual([
      expect.objectContaining({ kind: 'post_edit', id: post.id, rev: edited.rev, data: { text: 'after' } }),
      expect.objectContaining({ kind: 'post_deleted', id: post.id, rev: deleted.rev, deletedPostIds: [post.id] }),
    ])
    expect(JSON.stringify(delta.events)).not.toContain('userId')
    expect(JSON.stringify(delta.events)).not.toContain('accountId')
  })

  test('stores one durable canonical media lock record and updates it in place', async () => {
    const firstUntil = Date.now() + 60_000
    await forumPrimary.setMediaLockRecord({
      accountId: 'account-1',
      lockedUntil: firstUntil,
      reason: 'violence',
      source: 'forum_report_threshold',
      sourcePostId: 'post-1',
    })
    await expect(forumPrimary.getMediaLockRecord('account-1')).resolves.toMatchObject({
      accountId: 'account-1',
      lockedUntil: firstUntil,
      reason: 'violence',
      source: 'forum_report_threshold',
    ...
    await forumPrimary.setMediaLockRecord({ accountId: 'account-1', lockedUntil: secondUntil, reason: 'porn', source: 'video_postcommit_moderation' })
    const rows = await memoryDb.collection('forum_media_locks').find({}).toArray()
    expect(rows).toHaveLength(1)
    await expect(forumPrimary.getMediaLockRecord('account-1')).resolves.toMatchObject({ lockedUntil: secondUntil, reason: 'porn' })
  })
})
