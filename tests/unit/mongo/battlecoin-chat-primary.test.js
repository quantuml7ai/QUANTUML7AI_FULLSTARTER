...
describe('Battle Chat Mongo primary adapter', () => {
  ...
  test('falls back to anonymous public identity for technical nicknames', async () => {
    ...
    const stored = await db.collection('battlecoin_chat_messages').findOne({ _id: sent.message.id })
    expect(stored?.authorIdentityIds).toEqual(['0xtech'])
  })

  test('uses the strictest alias sender state when canonical and legacy rows coexist', async () => {
    await db.collection('battlecoin_chat_sender_state').insertOne({
      _id: 'sender:canonical',
    ...
    await db.collection('battlecoin_chat_sender_state').insertOne({
      ...
      nextAllowedAtMs: 180_000,
      messageWindowId: 2,
      messageWindowCount: 3,
      sessionCount: 9,
      updatedAtMs: 1000,
    })
    ...
    const result = await battleChatPrimary.sendBattleChatMessage({
      actor: {
        accountId: 'wallet-linked',
        identityIds: ['wallet-linked', 'telegram:777001', '777001'],
      },
      text: 'Must stay rate limited',
      clientMutationId: 'strictest-sender-state',
      now: 120_000,
    })
    ...
    expect(db.collection('battlecoin_chat_messages').rows).toHaveLength(0)    
  })

  test('reuses legacy alias sender state so canonical identity cannot bypass rate limits', async () => {
    await db.collection('battlecoin_chat_sender_state').insertOne({
      _id: 'sender:legacy-telegram',
      accountId: 'telegram:777001',
      nextAllowedAtMs: 0,
      messageWindowId: 0,
      messageWindowCount: 0,
    ...
    const sent = await battleChatPrimary.sendBattleChatMessage({
      actor,
      text: 'Canonical sender state',
      clientMutationId: 'canonical-sender-state',
      now: 120_000,
    })

    expect(sent.ok).toBe(true)
    const rows = db.collection('battlecoin_chat_sender_state').rows
