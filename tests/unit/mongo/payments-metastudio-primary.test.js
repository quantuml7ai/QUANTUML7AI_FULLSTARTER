...
describe('payments and metastudio Mongo primary repositories', () => {
  ...
  test('reuses a legacy lowercase wallet MetaStudio registration for an EIP-55 write', async () => {
    const canonical = '0x51be760fA3775263D2C2496824f23Ca31d829e6A'
    const legacy = canonical.toLowerCase()
    const registrations = memoryDb.collection('metastudio_registrations')
    const latest = memoryDb.collection('metastudio_registration_latest')

    await registrations.updateOne(
      { _id: `legacy:${legacy}` },
      { $set: { accountId: legacy, seenCount: 6, registeredAt: '2026-01-01T00:00:00.000Z', createdAt: '2026-01-01T00:00:00.000Z' } },
      { upsert: true },
    )
    await latest.updateOne(
      { _id: `legacy-latest:${legacy}` },
      { $set: { accountId: legacy, latestAt: '2026-01-01T00:00:00.000Z' } },
      { upsert: true },
    )

    await metastudioPrimary.upsertRegistration(canonical, {
      source: 'game-page',
      updatedAt: '2026-01-03T00:00:00.000Z',
    }, { legacyAccountIds: [legacy] })

    expect(registrations.rows.size).toBe(1)
    expect(latest.rows.size).toBe(1)
    expect(registrations.rows.has(canonical)).toBe(false)
    expect(registrations.rows.get(`legacy:${legacy}`)).toMatchObject({
      accountId: canonical,
      seenCount: 7,
      registeredAt: '2026-01-01T00:00:00.000Z',
    })
    expect(latest.rows.get(`legacy-latest:${legacy}`)).toMatchObject({
      accountId: canonical,
