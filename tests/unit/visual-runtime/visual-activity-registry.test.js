...
describe('QL7 visual activity V3 performance-safe registry', () => {
  ...
  test('bounds disconnected strong-reference fallback retention to one second without body observers', () => {
    expect(registry).toContain('const DISCONNECTED_SWEEP_MS = 1000')
    expect(registry).toContain('for (const node of nodes)')
    expect(registry).toContain('counters.disconnectedSweepReleases += 1')
    expect(registry).toContain('disconnectedSweepMs: DISCONNECTED_SWEEP_MS')
    const sweepStart = registry.indexOf('function sweepDisconnected()')
    const sweepEnd = registry.indexOf('export function installVisualRegistryDocumentController', sweepStart)
    const sweep = registry.slice(sweepStart, sweepEnd)
    expect(sweep).not.toContain('for (const node of [...nodes])')
  })
