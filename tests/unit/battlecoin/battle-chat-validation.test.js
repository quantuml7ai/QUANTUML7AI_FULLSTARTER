...
describe('Battle Chat validation contract', () => {
  test('keeps fixed production constants', () => {
    ...
    expect(validation.BATTLE_CHAT_MESSAGE_MAX_GRAPHEMES).toBe(300)
    expect(validation.BATTLE_CHAT_SEND_COOLDOWN_MS).toBe(10_000)
    expect(validation.BATTLE_CHAT_SESSION_HARD_LIMIT).toBe(1000)
    expect(validation.BATTLE_CHAT_QUICK_EMOJIS).toEqual(['❤️', '😄', '😡'])
  })

  test('allows market text and quick emoji without links', () => {
    const result = validation.validateBattleChatText('BTC/USDT LONG x10 looks strong 😄')
    expect(result).toMatchObject({ ok: true, text: 'BTC/USDT LONG x10 looks strong 😄' })
  ...
  test('enforces 300 grapheme limit', () => {
    const ok = validation.validateBattleChatText('🔥'.repeat(300))
    const bad = validation.validateBattleChatText('🔥'.repeat(301))
