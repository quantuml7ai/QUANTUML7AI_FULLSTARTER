...
describe('QL7 Support premium smoke wiring', () => {
  test('wires live, architecture closure, self-test and 24768-scenario runners without package drift', () => {
    for (const file of [
      'scripts/ql7-support/prelab-architecture-closure.mjs',
    ...
    const architecture = read('scripts/ql7-support/prelab-architecture-closure.mjs')
    expect(architecture).toContain('ql7-support-god-mode-static-audit.mjs')
    const matrix = read('scripts/ql7-support/synthetic-scenario-matrix.mjs')
    expect(matrix).toContain("['en','ru','uk','es','tr','ar','zh','he','de','fr','it','pt','pl','nl','sv','no','da','fi','cs','sk','hu','ro','bg','sr','hr','sl','el','ka','az','kk','ja','ko']")
  ...
  test('keeps live smoke read-only while proving the unauthenticated send boundary', () => {
    const live = read('scripts/ql7-support/live-smoke.mjs')
    expect(live).toContain("const baseUrl = arg('base-url'")
    expect(live).toContain("method: 'POST'")
    expect(live).toContain("to: 'ql7-support'")
    expect(live).toContain('support-send-requires-session-or-disabled')
    expect(live).toContain('[400, 401, 403, 404, 429].includes(unauthorizedSend.status)')
    expect(live).toContain('read-only-no-admin-write-probe')
    expect(live).toContain('no business write is authorized')
  ...
  test('exposes verified persisted operator states and premium card surface', () => {
    ...
    expect(stateRoute).toContain("export const dynamic='force-dynamic'")
    expect(stateRoute).toContain("fetchCache='force-no-store'")
    expect(stateRoute).toMatch(/['"]cache-control['"]\s*:\s*['"][^'"]*\bno-store\b[^'"]*\bmax-age=0\b[^'"]*['"]/u)
    ...
    expect(card).toContain("import DmMediaRenderer from './DmMediaRenderer.jsx'")
    ...
    expect(media).toContain("const VIDEO_STYLE = Object.freeze({ minHeight: 315, maxHeight: 'min(64.8vh, 585px)'")
