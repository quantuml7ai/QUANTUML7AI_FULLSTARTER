...
describe('QL7 Support canonical isolation and completion contracts', () => {
  test('global Deep Translate is independent from QL7 Support and keeps every ecosystem consumer', () => {
    ...
    const supportService = read('lib/ql7-support/nativeTranslationService.js')
    ...
    expect(supportServer).toContain("from './nativeTranslationService.js'")
  ...
  test('worker, admin context, browser evidence and learning deployment stay isolated and guarded', () => {
    ...
    expect(browser).toContain('support-browser-${viewportWidth}x${viewportHeight}.png')
    expect(browser).toContain('Number(args.width || 360)')
    expect(browser).toContain('Number(args.height || 800)')
  })
