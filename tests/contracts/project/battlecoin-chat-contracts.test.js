...
describe('Battle Chat isolated contour contracts', () => {
  ...
  test('backs off only failed SSE reconnects and preserves Mongo catch-up cadence', () => {
    const hook = read('app/exchange/battle-chat/useBattleChat.js')
    const client = read('app/exchange/battle-chat/battleChatClient.js')

    expect(client).toContain('BATTLE_CHAT_FALLBACK_FOREGROUND_MS = 9000')
    expect(client).toContain('BATTLE_CHAT_FALLBACK_HIDDEN_MS = 45000')
    expect(client).toContain('BATTLE_CHAT_RECONNECT_INITIAL_MS = 1800')
    expect(client).toContain('BATTLE_CHAT_RECONNECT_MAX_MS = 30000')
    expect(hook).toContain("es.addEventListener('battlecoin-chat-ready', handleReady)")
    expect(hook).toContain('const handleReady = () => {\n      reconnectAttempt = 0')
    expect(hook).toContain('const delay = battleChatReconnectDelay(reconnectAttempt)')
    expect(hook).toContain('reconnectAttempt += 1')
    expect(hook).toContain('if (closed || reconnectTimer) return')
    expect(hook).not.toContain('window.setTimeout(connect, 1800)')
    expect(hook).toContain('hidden ? BATTLE_CHAT_FALLBACK_HIDDEN_MS : BATTLE_CHAT_FALLBACK_FOREGROUND_MS')
  })
  ...
  test('Battle Chat UI keeps premium composer and message contract', () => {
    ...
    expect(row).toContain('`${styles.avatarWrap} ql7AvatarBadgeHost`')
    expect(row).toContain('width={68}')
    expect(row).toContain('isAuthorVip')
    expect(row).toContain('<AvatarBadgeOverlay vipActive={authorVip} />')
    expect(row).toContain('`${styles.avatar} ql7AvatarMediaClip`')
