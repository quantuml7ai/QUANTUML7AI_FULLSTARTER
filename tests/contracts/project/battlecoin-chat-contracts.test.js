...
describe('Battle Chat isolated contour contracts', () => {
  ...
  test('send and reaction mutations require verified auth on server and client gate', () => {
    ...
    expect(client).toContain('readAuthorizedAccountId')
    expect(client).toContain("headers['x-auth-account-id'] = authorizedAccountId")
    expect(auth).toContain('actorFromAuthorizedAccountHeader')
    expect(auth).toContain("readHeader(req, 'x-auth-account-id')")
    expect(auth).toContain('resolveCanonicalAccountId(claimedAccountId)')
    expect(auth.indexOf('await actorFromAuthorizedAccountHeader(req)')).toBeLessThan(auth.indexOf('await actorFromTelegramMiniApp(req, body)'))
  ...
  test('Battle Chat UI keeps premium composer and message contract', () => {
    ...
    expect(row).toContain('styles.messageBodyLineEmoji')
    ...
    expect(css).toContain('.messageRowEmoji')
    expect(css).toContain('.messageBubbleEmoji')
    expect(css).toContain('.messageBodyLineEmoji')
    expect(css).toContain('animated emoji effects')
    expect(css).toContain('max-height: clamp(88px, 8.8vw, 112px)')
