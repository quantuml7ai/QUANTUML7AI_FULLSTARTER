...
describe('QL7 final-MP4 pre-commit video moderation contracts', () => {
  test('samples 5-10 frames and poster from one monotonic Mediabunny session on the exact final MP4', () => {
    ...
    expectAll(precommit, [
      ...
      'new mediabunny.CanvasSink(videoTrack, sinkOptions)',
      'sink.canvasesAtTimestamps(requests.map((request) => request.timestamp))',
      ...
      "signal?.addEventListener?.('abort', disposeOnAbort",
      "signal?.removeEventListener?.('abort', disposeOnAbort)",
      'input.dispose?.()',
    ])
    expect(precommit).not.toContain('for (let attempt = 0; attempt < 3; attempt += 1)')
    expect(precommit).not.toContain('lastNetworkError')
    expect(precommit.split("fetch('/api/forum/moderate'").length - 1).toBe(1)
    expect(precommit).not.toContain("document.createElement('video')")
    expect(precommit).not.toContain('video.currentTime')
    expect(precommit).not.toContain('extractVideoFrames')
  })

  test('keeps conversion + artifact moderation in one heavy window and forbids decoder authority after prepared HEVC Send', () => {
    ...
    expectAll(gateway, [
      'getForumVerifiedVideoUploadProof',
      'getPreparedVideoArtifacts(file, artifactContext)',
      "sourceProof?.source === 'optimizer-output' && !videoArtifacts",
      "throw new Error('R2 prepared video requires exact-file pre-upload artifacts')",
      "sourceProof?.source === 'optimizer-output' && videoArtifacts",
      "bypassReason: 'prepared-artifacts'",
      'releaseHeavy = enterVideoHeavyActivity()',
      'await prepareForumVideoForUpload({',
      'videoArtifacts = await preparePreparedVideoArtifacts({',
      'try { releaseHeavy?.() } catch {}',
      'posterRecord = videoModeration?.poster || null',
    ...
    expect(gateway).not.toContain('createNativeVideoPoster')
    expect(gateway).not.toContain('extractVideoFrames')
    expectAll(picker, [
      'const prepared = await prepareVideoPreviewFn(vf',
      'if (preparedOnSelect) {',
      'preparedArtifacts = await preparePreparedVideoArtifacts({',
      'posterBlob: preparedOnSelect ? preparedArtifacts?.poster?.blob || null : null',
      'try { releaseHeavyActivity?.() } catch {}',
    ])
    expectBefore(picker, 'const prepared = await prepareVideoPreviewFn(vf', 'preparedArtifacts = await preparePreparedVideoArtifacts({')
  ...
  test('video Porn hard-block relaxation is scoped to video and preserves review/non-Porn safety', () => {
    const route = read('app/api/forum/moderate/route.js')
    ...
    expectAll(route, [
      ...
      'nsfwDecision(nsfw || {}, { video: isVideoModeration })',
      'if (Porn >= 0.34)',
      'if (ps >= 0.82 && Porn >= 0.16)',
      'if (Hentai >= 0.72)',
      'if (Hentai >= 0.45)',
      'if (goreScore >= 0.78)',
      'if (violenceScore >= 0.62)',
      'return Math.max(3, Math.ceil(count / 3))',
    ])
  })
