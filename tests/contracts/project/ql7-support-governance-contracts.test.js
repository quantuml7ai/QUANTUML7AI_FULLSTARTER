...
describe('QL7 Support Planetary AI V5 governance closeout', () => {
  ...
  test('keeps generated project documentation synchronized with Support structure', () => {
    ...
    for (const expectedPath of [
      ...
      'docs/ql7-support-planetary-ai-v5.md',
      'docs/ql7-support-v5-compliance-audit.md',
  ...
  test('keeps the acceptance evidence runner executable and honest about runtime-only gates', () => {
    const source = read('scripts/ql7-support/acceptance-audit.mjs')
    expect(source).toContain("scenarioCount: matrixRows.length")
    expect(source).toContain("'acceptance-summary.json'")
    expect(source).toContain("acceptanceComplete: failed.length === 0 && partial.length === 0 && runtimeRequired.length === 0")
    expect(source).toContain("criticalSkipped: [...partial, ...runtimeRequired].map")
    expect(source).toMatch(/\['ACCEPT-031',\s*codexEvidence\s*\?\s*'pass'\s*:\s*'runtime_required',\s*'canonical pnpm test:codex evidence'\]/)
    expect(source).not.toContain('acceptanceComplete: true')
  })
})
