...
describe('forum client video gateway V4 contracts', () => {
  ...
  test('forces every upload video through full transcode unless it already has optimizer-output proof', () => {
    const src = read('lib/forumClientVideoOptimizer.js')

    expect(src).toContain("FORUM_CLIENT_VIDEO_OPTIMIZER_MIN_TARGET_BYTES = 1 * MIB")
    expect(src).toContain("if (existingProof?.source === 'optimizer-output')")
    expect(src).toContain('await optimizeForumVideoOnDevice(sourceFile, options)')
    expect(src).toContain('forceTranscode: true')
    expect(src).toContain("source: 'optimizer-output'")

    expect(src).not.toContain("source: 'canonical-source'")
    expect(src).not.toContain("bypassReason: 'canonical-source'")
    expect(src).not.toContain("source: 'canonical-remux-copy'")
    expect(src).not.toContain("bypassReason: 'canonical-remux-copy'")
    expect(src).not.toContain('await optimizeForumVideoFastStart(file')
    expect(src).not.toContain("import { optimizeForumVideoFastStart } from './forumVideoTrim'")
  })
  ...
  test('places fail-closed preparation before every presigned URL request', () => {
    ...
    expect(src).toMatch(/import\s*\{[^}]*prepareForumVideoForUpload[^}]*\}\s*from\s*['"]\.\.\/\.\.\/\.\.\/\.\.\/\.\.\/lib\/forumClientVideoOptimizer['"]/)
  ...
  test('connects deferred paperclip, camera/trim Blob and ads creative to the shared gateway', () => {
    ...
    expect(paperclip).toContain('const preparedOnSelect = !!prepared?.needsPreparedPreview')
    expectBefore(paperclip, 'const preparedOnSelect = !!prepared?.needsPreparedPreview', 'const previewFile = preparedOnSelect ? prepared?.file : vf')
