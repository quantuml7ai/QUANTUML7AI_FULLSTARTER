...
const GENERATED_RUNTIME_EVIDENCE_DIRS = Object.freeze([
  '.ql7-patch-backups',
  'report/QL7_SUPPORT_PATCH',
])

function isGeneratedRuntimeEvidenceDirectory(dir) {
  const rel = relative(root, dir).replace(/\\/g, '/').replace(/^\.\//, '')
  return GENERATED_RUNTIME_EVIDENCE_DIRS.some(
...
function* walk(dir) {
  for (const entry of readdirSync(dir)) {
    ...
    if (isGeneratedRuntimeEvidenceDirectory(full)) continue
...
describe('project text encoding contract', () => {
  test('distinguishes valid uppercase Cyrillic from mojibake byte artifacts', () => {
    // This contract audits source-managed project text. Runtime patch evidence and
    // patch backups are deliberately outside that population: they can contain
    // captured stderr in an arbitrary terminal code page and are independently
    // integrity-checked by the patch evidence/rollback contracts. Keep the
    // exclusion exact so a normal project directory named `report` is still
    // audited.
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, 'report', 'QL7_SUPPORT_PATCH'))).toBe(true)
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, 'report', 'QL7_SUPPORT_PATCH', 'run-id'))).toBe(true)
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, '.ql7-patch-backups'))).toBe(true)
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, '.ql7-patch-backups', 'marker', 'run-id'))).toBe(true)
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, 'report', 'source-controlled.txt'))).toBe(false)
    expect(isGeneratedRuntimeEvidenceDirectory(join(root, 'lib', 'report', 'QL7_SUPPORT_PATCH'))).toBe(false)
