...
describe('QL7 forum native video poster-first light prewarm R24', () => {
  test('removes synthetic first-frame priming and keeps a bounded metadata-only runway', () => {
    ...
    expect(media).toContain("reasonKey === 'resident_cap_overflow'")
    expect(media).toContain("reason !== 'resident_cap_overflow'")
    expect(media).toContain('__nativeSrcCapBlockedUntil')
    expect(media).toContain('const readAttachedMediaSrc = (el) => {')
    expect(media).toContain('const selfHealDisconnectedMediaRegistry =')
    expect(media).toContain("releaseDetachedManagedVideoPipeline(owner, 'detached_registry_release')")
    expect(media).toContain("releaseDetachedManagedVideoPipeline(detachedPrewarm, 'detached_native_prewarm_release')")
    expect(media).toContain("trace('load_kick_skip_detached', media, { channel })")
    expect(media).toContain('rewarmBlockedUntil > now')
    expect(media).toContain('const budget = POST_NATIVE_SRC_CAP;')
    expect(media).toContain("trace('observe_native_visible_prewarm', media, { visiblePx, gapPx });")
    expect(media).not.toContain("trace('observe_native_visible_restore', media, { visiblePx, gapPx });")
    ...
    expect(prewarm).toContain('if (!media.isConnected)')
