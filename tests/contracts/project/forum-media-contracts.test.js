...
describe('forum media contracts', () => {
  ...
  test('every fresh Forum runtime resets native, audio and iframe media to muted', () => {
    ...
    const bootStart = lifecycle.indexOf(';(() => {')
    const bootEnd = lifecycle.indexOf('})()', bootStart)
    const boot = lifecycle.slice(bootStart, bootEnd)

    expect(bootStart).toBeGreaterThanOrEqual(0)
    expect(bootEnd).toBeGreaterThan(bootStart)
    expect(boot).toContain('Every fresh Forum runtime starts from one shared muted state')
    expect(boot).toContain('writeMutedPrefToDocument(true, false)')
    expect(boot).toContain('writeMutedPrefToStorage(true)')
    expect(boot).not.toContain('readMutedPrefFromStorage()')
    expect(boot).not.toContain('__isIOSWebKitMediaRuntime()')

    expect(coordinator).toContain('el instanceof HTMLVideoElement || el instanceof HTMLAudioElement')
    expect(coordinator).toContain('el.defaultMuted = effectiveWant')
    expect(coordinator).toContain("if (kind === 'youtube' || kind === 'tiktok' || kind === 'iframe')")
    expect(coordinator).toContain("commandExternalVideo(el, effectiveWant ? 'mute' : 'unmute')")
  })
  ...
  test('iOS/WebKit audible autoplay rejection promotes forum and site ads to shared mute', () => {
    ...
    expect(coordinator).toContain('return !!isIOSUi;')
    expect(coordinator).not.toContain('isIOSUi && !isTelegramMiniAppMediaRuntime')
    expect(coordinator).toContain("trace('audible_autoplay_pause_to_global_mute_resume'")
    ...
    const policyPauseStart = coordinator.indexOf('if (isAudibleAutoplayPolicyPause) {')
    const policyPauseEnd = coordinator.indexOf('const isNativeAutoplayPause =', policyPauseStart)
    expect(policyPauseStart).toBeGreaterThanOrEqual(0)
    expect(policyPauseEnd).toBeGreaterThan(policyPauseStart)
    const policyPause = coordinator.slice(policyPauseStart, policyPauseEnd)
    expect(policyPause).toContain('setMutedPref(true, fallbackSource)')
    expect(policyPause.indexOf('setMutedPref(true, fallbackSource)')).toBeLessThan(
      policyPause.indexOf("scheduleNativePauseRecovery(target, owner, 'audible_autoplay_policy_muted_resume')"),
    )
...
expect(src).toMatch(
  ...
  test('Android Chromium native playback stays on native video without a canvas fallback', () => {
    const videoMediaSrc = read('app/forum/features/media/components/VideoMedia.jsx')
    const adsSrc = read('app/forum/ForumAds.js')
    const splashSrc = read('components/ForumBootSplash.jsx')

    for (const source of [videoMediaSrc, adsSrc, splashSrc]) {
      expect(source).not.toContain('AndroidChromiumVideoCanvas')
      expect(source).not.toContain('androidChromiumCanvasReady')
      expect(source).not.toContain('data-android-chromium-canvas-ready')
      expect(source).not.toContain('data-android-chromium-video-canvas')
    }
    expect(videoMediaSrc).toContain('<video')
    expect(adsSrc).toContain('<video')
    expect(splashSrc).toContain('<video')
    expect(fs.existsSync(path.join(root, 'components/AndroidChromiumVideoCanvas.jsx'))).toBe(true)

    const unexpectedRuntimeOwners = []
    const pending = ['app', 'components', 'lib'].map((relative) => path.join(root, relative))
    while (pending.length > 0) {
      ...
      for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
        ...
        if (relative === 'components/AndroidChromiumVideoCanvas.jsx') continue
        const source = fs.readFileSync(absolute, 'utf8')
        if (/AndroidChromiumVideoCanvas|androidChromiumCanvasReady|data-android-chromium-(?:canvas-ready|video-canvas)/.test(source)) {
          unexpectedRuntimeOwners.push(relative)
        }
      }
    }
    expect(unexpectedRuntimeOwners).toEqual([])
  })

  test('camera recording overlay remains byte-semantically frozen while playback canvas is removed', () => {
    expect(sha256Source('app/forum/features/media/components/VideoOverlay.jsx')).toBe(
      '2f092100e21307d8d4ad3b89da5752a95047e8b91124dfe6cae67aee6f1e7d06',
    )
    expect(sha256Source('app/forum/features/ui/components/ForumOverlayStack.jsx')).toBe(
      'b1a69590c7b16f9b174a818fd23acd1b1311a689f897714fdd090481d5049c39',
    )
  })
