...
describe('App entry contracts', () => {
  ...
  it('keeps a canonical, real ICO favicon available at the conventional public path', () => {
    const rootLayout = readRepoFile('app/layout.js')
    const favicon = fs.readFileSync(path.join(process.cwd(), 'public/favicon.ico'))

    expect(rootLayout).toContain("icon: withAssetVersion('/favicon.ico')")
    expect(rootLayout).toContain("shortcut: withAssetVersion('/favicon.ico')")
