...
describe('Forum hook contracts', () => {
  ...
  it('keeps front-camera video mirroring metadata wired through fullscreen preview and safe upload fallback', () => {
    ...
    expect(videoOverlaySource).toContain('data-front-camera-mirror={previewMirror ?')
    expect(videoOverlaySource).toContain('transform: previewMirror ?')
    expect(videoOverlaySource).not.toContain('.voPreviewVideo[data-front-camera-mirror="1"]::-webkit-media-controls-panel')
    expect(composerPreviewSource).toContain('data-front-camera-mirror={pendingVideoMirror ?')
    expect(composerPreviewSource).not.toContain('enableVideoControlsOnTap')
    expect(forumStylesSource).toContain('@media (hover:hover) and (pointer:fine)')
    expect(forumStylesSource).toContain('.videoCard video[data-front-camera-mirror="1"]::-webkit-media-controls-panel')
    expect(forumStylesSource).toContain('.voPreviewVideo[data-front-camera-mirror="1"]::-webkit-media-controls-panel')
