...
describe('forum transient server projection retention contract', () => {
  ...
  it('bounds only the duplicated media-feed projection while preserving local history and player policy', () => {
    expect(videoRuntime).toContain('QL7_MEDIA_FEED_PROJECTION_OWNER')
    expect(videoRuntime).toContain('QL7_MEDIA_FEED_PROJECTION_PAGE_WINDOW = 3')
    expect(videoRuntime).toContain('QL7_MEDIA_FEED_PROJECTION_MIN_ITEMS = 60')
    expect(videoRuntime).toContain('const nextServerVideoPosts = reset')
    expect(videoRuntime).toContain('serverVideoPostsRef.current = nextServerVideoPosts')
    expect(videoRuntime).toContain('setServerVideoPosts(nextServerVideoPosts)')
    expect(videoRuntime).toContain('boundForumTransientProjectionItems(nextServerVideoPosts, projectionLimit)')
    expect(videoRuntime).toContain('publishServerVideoItems(projectionPosts, { reset: true })')
    ...
    expect(retention).toContain('boundForumTransientProjectionItems')
