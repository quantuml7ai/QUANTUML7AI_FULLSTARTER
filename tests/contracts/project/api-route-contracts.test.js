...
describe('API route contracts', () => {
  ...
  it('keeps wallet sessions bound to the latest account token and hard logout cleanup', () => {
    ...
    expect(runtime).toContain('verifyStoredWalletSession({ emitAuthTransition: false })')
    expect(runtime).toContain('sessionWatchdogInFlightRef')
    expect(client).toContain('const emitAuthTransition = options?.emitAuthTransition !== false')
    expect(client).toContain('dispatchWalletSessionVerified(session)')
    expect(client).toContain("dispatchAuthReady(session, 'auth:ok')")
    ...
    expect(authNav).toContain('const emitAuthTransition = options?.emitAuthTransition !== false')
    expect(authNav).toContain('verifyStoredWalletSession({ emitAuthTransition })')
    expect(authNav).toContain('verifySession({ force: true })')
    expect(authNav).toContain('verifySession({ minAgeMs: WALLET_SESSION_FOCUS_REVERIFY_MS, emitAuthTransition: false })')
