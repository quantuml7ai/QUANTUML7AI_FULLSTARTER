...
describe('canonical human identity Stage 1-4 contract', () => {
  ...
  it('keeps Forum writes and video receipts on the post-auth actor without mutating profile alias presentation state', () => {
    const queue = readRepoFile('app/forum/features/feed/hooks/useForumMutationQueue.js')
    const createPost = readRepoFile('app/forum/features/feed/hooks/useForumCreatePostAction.js')
    const ownerActions = readRepoFile('app/forum/features/feed/hooks/useForumMutationActions.js')
    const forumApi = readRepoFile('app/forum/services/forumApi.js')
    const forumRoot = readRepoFile('app/forum/ForumRoot.jsx')
    const profileCache = readRepoFile('app/forum/features/profile/utils/profileCache.js')

    const queueActor = functionSlice(
      queue,
      'const userId =',
      'const resp = await api?.mutate?.',
    )
    expect(queueActor.indexOf('authRef?.current?.asherId')).toBeLessThan(
      queueActor.indexOf('authRef?.current?.accountId'),
    )

    const dmBranch = functionSlice(createPost, 'if (isDm) {', 'let r = null')
    expect(dmBranch).toContain("resolveProfileAccountId(r.asherId || r.accountId || '')")

    const forumVideoAuth = functionSlice(
      createPost,
      'let r = null',
      'const media = await resolveComposerMediaPayload({',
    )
    expect(forumVideoAuth).toContain("uid = String(r.asherId || r.accountId || '').trim()")
    expect(forumVideoAuth).not.toContain('resolveProfileAccountId')
    expect(createPost).toContain("videoModerationSurface: 'forum'")
    expect(createPost).toContain('videoModerationActorId: uid')
    expect(ownerActions).toContain("const uid = authAsherId || authAccountId || ''")

    expect(forumRoot).toContain("videoModerationSurface: dmMode ? 'dm' : 'forum'")
    expect(forumRoot).toContain(": String(auth?.asherId || auth?.accountId || ''),")
    expect(forumRoot).toContain("remembered?.actorId || auth?.asherId || auth?.accountId || viewerId || ''")

    expect(profileCache).toContain('if (!from || !to || from === to) return')
    expect(profileCache).not.toContain('localStorage.removeItem(PROFILE_ALIAS_PREFIX + key)')

    const forumActorReader = functionSlice(
      forumApi,
      'export function readForumActorId()',
      'function withForumActor(',
    )
    expect(forumActorReader.indexOf('window.__ASHER_ID__')).toBeLessThan(
      forumActorReader.indexOf('window.__AUTH_ACCOUNT__'),
    )
  })
