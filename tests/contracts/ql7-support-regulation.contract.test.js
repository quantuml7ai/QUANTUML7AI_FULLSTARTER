...
describe('QL7 Support canonical regulation contracts', () => {
  it('exposes all 32 locales, expanded banks and production run profiles', () => {
    ...
    expect(QL7_SUPPORT_SIMULATION_PROFILES).toContain('product-breadth')
    expect(QL7_SUPPORT_MUTATION_FAMILIES).toHaveLength(29)
  }, 60000)

  it('uses material product-breadth prompts instead of multiplying a greeting fixture', () => {
    const rows = Array.from({ length: 2000 }, (_, index) => buildQl7SupportScenario(index, {
      profile: 'product-breadth',
      seed: 'contract-product-breadth',
    }))
    const normalized = new Set(rows.map((row) => row.input.normalize('NFKC').toLocaleLowerCase().replace(/\s+/gu, ' ').trim()))
    const lanes = new Set(rows.map((row) => row.lab?.breadthLane).filter(Boolean))
    const locales = new Set(rows.map((row) => row.locale))
    const humanTopics = new Set(rows.map((row) => row.lab?.expectedHumanTopic).filter(Boolean))
    expect(normalized.size).toBeGreaterThan(900)
    expect(lanes.size).toBeGreaterThanOrEqual(8)
    expect(locales.size).toBe(32)
    expect(humanTopics.size).toBeGreaterThanOrEqual(80)
  })
