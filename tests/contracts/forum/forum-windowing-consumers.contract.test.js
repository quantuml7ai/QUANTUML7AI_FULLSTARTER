...
describe('forum windowing consumer contract', () => {
  ...
  test('shared hook owns remount footprint continuity for every windowing consumer', () => {
    const windowing = fs.readFileSync(path.join(process.cwd(), 'app/forum/shared/hooks/useForumWindowing.js'), 'utf8')

    for (const token of [
      'confirmedFootprintsRef',
      'remountCandidatesRef',
      'remountReservationsRef',
      'readItemNaturalLayoutFootprint',
      ...
      expect(windowing).toContain(token)
    }

    // The continuity mechanism must stay generic: no VideoFeed-specific DOM
    // selector or consumer-owned height reservation belongs in the shared hook.
    expect(windowing).not.toContain('data-ql7-video-feed-grid')
    for (const file of consumers) {
      const src = fs.readFileSync(path.join(process.cwd(), file), 'utf8')
      expect(src, file).not.toContain('remount_footprint_reserved')
      expect(src, file).not.toContain('REMOUNT_FOOTPRINT_RECHECK_MS')
    }
  })
