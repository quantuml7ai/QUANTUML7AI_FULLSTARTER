...
describe('forum runtime stability protected contracts', () => {
  ...
  test('keeps one DM notification retry owner while preserving initial and Support reads', () => {
    ...
    expect(lifecycle).toContain('loadDmThread(uid, null, { force: true, refresh: true, bypassThrottle: true, resetHistory: true })')
  ...
  test('binds load-more and managed visuals to a real inner forum scroller only when one is an ancestor', () => {
    ...
    const videoFeed = read('app/forum/features/media/components/VideoFeedPane.jsx')
    expect(videoFeed).toContain('rootMargin="0px"')
    expect(videoFeed).toContain('requireExitBeforeReplay={true}')
    expect(videoFeed).toContain('onExit={releaseVideoFeedTail}')
    expect(videoFeed).toContain('useVideoFeedTailWatchdog({')
    expect(videoFeed).toContain('isLoadMoreSentinelInsideViewport(snapshot?.tailElementRef?.current)')
  })
