...
describe('MetaMarket contracts', () => {
  ...
  it('keeps all MetaMarket user VIP badges docked to avatars and info hints click-through', () => {
    const runtime = readRepoFile('components/MetaMarket.jsx')
    const globalStyles = readRepoFile('app/globals.css')
    expect(runtime.match(/<AvatarBadgeOverlay/gu) || []).toHaveLength(4)
    expect(runtime.match(/<AvatarBadgeOverlay[^>]*showInfo/gu) || []).toHaveLength(2)
    expect(runtime).toContain('mmRecipientAvatar ql7AvatarBadgeHost')
    expect(runtime).toContain('mmOwnerAvatar ql7AvatarBadgeHost')
    expect(runtime).toContain('mmOwnerAvatar avaMini ql7AvatarBadgeHost')
    expect(runtime).toContain('mmTransactionTargetAvatar ql7AvatarBadgeHost')
    expect(runtime).toContain('mmTransactionTargetAvatarImg ql7AvatarMediaClip')
    ...
    expect(runtime).toMatch(/\.mmHistoryUser\s*\{\s*border-radius: 14px;/u)
    expect(runtime).toMatch(/\.mmHistoryUser\s*\{\s*overflow: visible;/u)
    expect(runtime).toMatch(/\.mmTransactionTargetAvatar\s*\{[\s\S]*?border-radius: 10px;/u)
    expect(runtime).toMatch(/\.mmTransactionTarget\s*\{[\s\S]*?border-radius: 14px;/u)    
    expect(globalStyles).toContain('.ql7AvatarBadgeOverlay {')
