...
describe('MetaMarket contracts', () => {
  ...
  it('keeps the TopBar MetaMarket launch behind the shared auth continuation gate', () => {
    ...
    expect(topBar).toContain(
      "actionKey: 'topbar-metamarket-open'",
    )

    expect(topBar).toContain(
      "source: 'topbar-metamarket'",
    )
    ...
    const handlerStart = topBar.indexOf(
      'const openMetaMarketFromBrand = () => {',
    )

    const handlerEnd = topBar.indexOf(
      '\n  return (',
      handlerStart,
    )
    ...
    const openIndex = handler.indexOf(
      "new CustomEvent('metamarket:open'",
    )

    expect(gateIndex).toBeGreaterThan(-1)
    expect(openIndex).toBeGreaterThan(gateIndex)
  })
  ...
  it('keeps MetaMarket avatars free from info and VIP overlays and orders gift recipients label-avatar-nickname', () => {
    const runtime = readRepoFile('components/MetaMarket.jsx')
    expect(runtime).not.toContain('AvatarBadgeOverlay')
    expect(runtime).not.toContain('ql7AvatarBadgeHost')
    expect(runtime).not.toContain('ql7AvatarMediaClip')
    expect(runtime).not.toContain('<VipFlipBadge')
    expect(runtime).not.toContain('mmContextVip')
    expect(runtime).not.toContain('mmOwnerVip')
    expect(runtime).not.toContain('mmTransactionVip')
    expect(runtime).toContain('onClick={(clickEvent) => openUserInfoFromHistory(counterparty, event.txId, clickEvent)}')
    expect(runtime).toContain('onClick={(event) => openUserInfoFromOwner(user, event)}')
    expect(runtime.match(/className="nick-badge nick-animate"/gu) || []).toHaveLength(2)
    expect(runtime).toMatch(/mmGiftContextLabel[\s\S]*?mmRecipientAvatar[\s\S]*?mmGiftContextNickname/u)
    expect(runtime).toMatch(/mmTransactionTargetLabel[\s\S]*?mmTransactionTargetAvatar[\s\S]*?mmTransactionTargetName/u)
    expect(runtime).toContain("tx('metamarket_context_gift_to')")
    expect(runtime).toContain("tx('metamarket_transaction_gift_target')")
    for (const dict of Object.values(dictionaries)) {
      expect(dict.metamarket_context_gift_to).not.toContain('{nickname}')
      expect(dict.metamarket_transaction_gift_target).not.toContain('{nickname}')
    }
    expect(runtime).toMatch(/\.mmRecipientAvatar,\s*\.mmOwnerAvatar\s*\{[\s\S]*?border-radius: 10px;/u)
    expect(runtime).toMatch(/\.mmTransactionTargetAvatar\s*\{[\s\S]*?border-radius: 10px;/u)
    expect(runtime).toMatch(/\.mmTransactionTarget\s*\{[\s\S]*?grid-template-columns: max-content 24px minmax\(0, auto\);/u)
  })

  it('keeps new gifts as a per-opening snapshot with an inline red SVG ribbon', () => {
    const runtime = readRepoFile('components/MetaMarket.jsx')
    const notificationSync = readRepoFile('components/AndroidNotificationBadgeSync.jsx')
    expect(runtime).toContain('giftOpenSnapshotRef')
    expect(runtime).toContain('giftOpenSnapshotReady')
    expect(runtime).toContain('!append && !giftOpenSnapshotRef.current.resolved')
    expect(runtime).toContain("if (typeof window === 'undefined' || !giftOpenSnapshotReady) return")
    expect(runtime).toMatch(/if \(typeof window === 'undefined' \|\| !giftOpenSnapshotReady\) return[\s\S]*?source: 'metamarket_gifts',[\s\S]*?count: 0,/u)
    expect(runtime).toContain('}, [giftOpenSnapshotReady])')
    expect(runtime).toContain('writeSeenGiftAt(viewerId, newestIncomingAt)')
    expect(runtime).not.toContain('}, 1400)')
    expect(runtime).toContain('unseenGiftItemIds.has')
    expect(runtime).toContain('const orderedOwnedItems = useMemo(() => {')
    expect(runtime).toContain('return [...newGiftItems, ...existingItems]')
    expect(runtime).toContain('orderedOwnedItems.map((owned) => {')
    expect(runtime).toContain('className="mmNewGiftBadge" role="img"')
    expect(runtime).toContain('<svg viewBox="0 0 64 64"')
    expect(runtime).toContain('<text x="22" y="27" transform="rotate(-45 22 22)">New</text>')
    expect(runtime).toContain('.mmNewGiftRibbonFace {')
    expect(runtime).toContain('className="mmNewGiftRibbonGlint"')
    expect(runtime).toContain('@keyframes mmNewGiftSunGlint')
    expect(runtime).toContain('animation: mmNewGiftSunGlint 7.2s ease-in-out infinite;')
    expect(runtime).toMatch(/\.mmOwnedCard\.isNewGift\s*\{[\s\S]*?z-index: 2;[\s\S]*?overflow: visible;[\s\S]*?isolation: isolate;/u)
    expect(runtime).toMatch(/\.mmOwnedCard > \.mmNewGiftBadge\s*\{[\s\S]*?position: absolute;[\s\S]*?left: -1px;[\s\S]*?top: -1px;[\s\S]*?z-index: 6;[\s\S]*?width: 64px;[\s\S]*?height: 64px;[\s\S]*?transform: none;/u)
    expect(runtime).not.toContain('animation: mmNewGiftGlow')
    expect(runtime).not.toMatch(/\.mmNewGiftRibbonGlint\s*\{[^}]*(?:mix-blend-mode|will-change):/u)
    expect(runtime).toContain('.mmNewGiftBadge text {')
    expect(runtime).toContain('fill: #fff;')
    expect(notificationSync).toContain("if (window.__QL7_PENDING_NOTIFICATION_SOURCE__ !== source) return")
    expect(notificationSync).not.toMatch(/const dispatchNotice = \(\) => \{\s*window\.__QL7_PENDING_NOTIFICATION_SOURCE__ = source/u)
  })

  it('keeps transaction invariants visible in the server domain', () => {
    const transactions = readRepoFile('app/api/metamarket/_transactions.js')
    const db = readRepoFile('app/api/metamarket/_db.js')
    const locks = readRepoFile('app/api/metamarket/_locks.js')
    ...
    expect(transactions.match(/return await withMongoTransaction\(async \(\) =>/gu)).toHaveLength(3)
    expect(transactions).toContain('economicStarted.authorization.envelope.idempotencyKey')
    expect(transactions).toContain('economicStarted.authorization.envelope.sourceEventId')
    expect(locks).toContain('const DEFAULT_LOCK_TTL_SEC = 30')
  })

  it('keeps internal economic errors private and the transaction modal inside mobile bounds', () => {
    const format = readRepoFile('app/api/metamarket/_format.js')
    const buyRoute = readRepoFile('app/api/metamarket/buy/route.js')
    const sellRoute = readRepoFile('app/api/metamarket/sell/route.js')
    const giftRoute = readRepoFile('app/api/metamarket/gift/route.js')
    const readRoutes = [
    ...
    expect(format).toContain("return PUBLIC_METAMARKET_ERRORS.has(code) ? code : 'transaction_failed'")
    for (const route of [buyRoute, sellRoute, giftRoute, ...readRoutes]) {
      expect(route).toContain('publicErrorCode(error)')
      expect(route).not.toContain('jsonError(errorCode(error)')
    }
    expect(runtime).toContain('normalizeMetaMarketPublicError')
    expect(runtime).toContain('.mmTransactionModal *')
    expect(runtime).toContain('box-sizing: border-box')
    expect(runtime).toContain('overflow-wrap: anywhere')
    expect(runtime).toContain("[dir='rtl'] .mmTransactionAssetInfo")
    expect(runtime).toContain('grid-template-columns: 68px minmax(0, 1fr)')
  })
