...
describe('QL7 Support canonical architecture closure contract',()=>{
 it('excludes only root temporary checkouts while retaining active and similarly named sources',()=>{
  const audit=read('scripts/ql7-support/prelab-architecture-closure.mjs')
  const start=audit.indexOf('function walk(dir)')
  const end=audit.indexOf('const sourceExt=',start)
  expect(start).toBeGreaterThanOrEqual(0)
  expect(end).toBeGreaterThan(start)
  const root=path.resolve('audit-walk-fixture')
  const tree=new Map([
   [root,[['.codex_tmp',true],['.codex_tmp_active',true],['app',true],['scripts',true]]],
   [path.join(root,'.codex_tmp_active'),[['active.js',false]]],
   [path.join(root,'app'),[['page.js',false],['.codex_tmp',true]]],
   [path.join(root,'app','.codex_tmp'),[['nested-active.js',false]]],
   [path.join(root,'scripts'),[['proof.mjs',false]]],
  ])
  const visited=[]
  const fakeFs={
   existsSync:(dir)=>tree.has(dir)||dir===path.join(root,'.codex_tmp'),
   readdirSync:(dir)=>{
    visited.push(dir)
    if(!tree.has(dir))throw new Error('temporary checkout was scanned')
    return tree.get(dir).map(([name,isDirectory])=>({name,isDirectory:()=>isDirectory}))
   },
  }
  const files=runInNewContext(`${audit.slice(start,end)};walk(ROOT)`,{fs:fakeFs,path,ROOT:root})
  expect(Array.from(files,(file)=>path.relative(root,file).replaceAll(path.sep,'/')).sort()).toEqual([
   '.codex_tmp_active/active.js','app/.codex_tmp/nested-active.js','app/page.js','scripts/proof.mjs',
  ])
  expect(visited).not.toContain(path.join(root,'.codex_tmp'))
 })
