...
export function getMediaPrefixByKind(kind = '', contentType = '') {
  ...
  if (normalizedKind === 'forum_video_poster' || normalizedKind === 'video_poster') return 'forum/video-posters'
  ...
  if (normalizedKind === 'ads_video_poster' || normalizedKind === 'ad_video_poster') return 'ads/video-posters'
...
export function createAnimatedPosterKey(sourceKey = '') {
  const clean = String(sourceKey || '').trim().replace(/^\/+/, '')
  if (!clean) return ''
  if (/\.[a-z0-9]+$/i.test(clean)) return clean.replace(/\.[a-z0-9]+$/i, '-poster.webp')
  return `${clean}-poster.webp`
}
