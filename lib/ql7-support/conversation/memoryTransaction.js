...
export async function commitQl7SupportMemoryTransaction({
  ...
  if (result.ok) {
    return Object.freeze({ ...result, memoryHash: result.memoryHash || compacted.memoryHash })
  }
  ...
  if (stored &&
    ...
    stored.memoryHash === (result.memoryHash || compacted.memoryHash)) {
    return Object.freeze({
      ok: true,
      duplicate: true,
      expectedVersion: Math.max(0, Number(expectedVersion || 0)),
      nextVersion: Number(compacted.memoryVersion || 0),
      memoryHash: result.memoryHash || compacted.memoryHash,
    })
  }
