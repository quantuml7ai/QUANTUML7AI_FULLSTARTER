export const QL7_SUPPORT_TEMPORAL_CONTEXT_VERSION = '12.0.0'

function str(value) { return String(value ?? '').trim() }

export function normalizeQl7SupportTimeZone(value = '') {
...
export function getQl7SupportLocalTimeParts({ now = Date.now(), timeZone = 'UTC', locale = 'en-US' } = {}) {
  const zone = normalizeQl7SupportTimeZone(timeZone)
...
export function selectQl7SupportGreetingDaypart(input = {}) {
  const parts = getQl7SupportLocalTimeParts(input)
  return Object.freeze({
    ...parts,
