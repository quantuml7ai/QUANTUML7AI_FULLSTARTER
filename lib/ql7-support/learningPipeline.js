...
import {validateQl7SupportLearningApprovals} from './learning/approvalReceipt.js'
import {evaluateQl7SupportPromotion} from './learning/promotionPolicy.js'
import {evaluateQl7SupportScientificLearningGovernance} from './learning/scientificGovernance.js'
import {buildQl7CalibrationCandidateReceipt} from './learning/calibrationCandidateReceipt.js'
import {deleteQl7SupportUserLearningData as deleteLearningDataAuthority, deleteQl7SupportUserLinkedLearningData} from './learning/datasetDeletion.js'
export {deleteQl7SupportUserLinkedLearningData}
...
export async function recordQl7SupportLearningSignal({
  ...
  if (consent !== true || !str(consentReceiptId)) throw new Error('learning_consent_required')
  ...
  await database.collection(QL7_SUPPORT_FEEDBACK_EVENTS).insertOne({
    ...
    signalType: str(signalType),
    consent: true,
    consentReceiptId: str(consentReceiptId),
    poisoningRisk: risk,
    createdAt: at,
    storagePrimary: 'mongo',
  })
  const candidateReceipt = buildQl7CalibrationCandidateReceipt({candidateId,actorIdHash:userIdHash,consentReceiptId:str(consentReceiptId),clusterId:str(projectedEvidence.clusterId),replayId:str(projectedEvidence.replayId),locale:str(sourceLocale),domainId:str(topic),actual:redactedActual,expectedInvariants:[redactedExpected],createdAtServerUtc:at})
  const doc = {
    _id: candidateId,
    candidateId,
    calibrationCandidateReceipt: candidateReceipt,
    userIdHash,
    sourceCaseIdHash,
    sourceLocale: str(sourceLocale),
    topic: str(topic),
...
export async function evaluateQl7SupportLearningCandidate({ database, candidateId, metrics = {}, baselineVersion = 'active', candidateVersion = '', clock = Date.now } = {}) {
  ...
  if (candidate.consent !== true || !str(candidate.consentReceiptId)) throw new Error('learning_consent_required')
  const at = nowIso(clock)
  const normalized = {
...
export async function deployQl7SupportLearningCandidate({ database, candidateId, evaluation = {}, mode = 'canary', canaryPercent = 1, deploymentScope = '', approvals = [], signingKeys = {}, proposal = {}, ablation = {}, counterfactual = {}, shadow = {}, clock = Date.now } = {}) {
  const candidate = await database.collection(QL7_SUPPORT_LEARNING_CANDIDATES).findOne({ _id: str(candidateId) })
  if (!candidate || candidate.status !== 'approved_for_eval') throw new Error('learning_candidate_not_approved')
  ...
  const sensitive = /safety|economic|quarantine|composer/iu.test(`${candidate?.signalType || ''} ${candidate?.clusterId || ''} ${JSON.stringify(proposal || {})}`)
  const proposalHash = sha(proposal || candidate?.proposedChange || { candidateId: candidate._id })
  const approval = validateQl7SupportLearningApprovals({ approvals, candidateId: candidate._id, proposalHash, sensitive, signingKeys })
  const scientificGovernance = evaluateQl7SupportScientificLearningGovernance({ owner: 'ql7-support.learning', features: evaluation?.features || [], baselineScore: Number(evaluation?.baselineScore || 0), candidateScore: Number(evaluation?.candidateScore || 0), weightChanges: proposal?.changes || [], evidenceIds: evaluation?.evidenceIds || [], constraints: proposal?.constraints || {}, ablation, counterfactualPairs: counterfactual?.pairs || [], drift: evaluation?.drift || {} })
  const promotion = evaluateQl7SupportPromotion({ proposal, approvals: approval.approvals, ablation, counterfactual, shadow, canary: { ok: mode !== 'active' }, hardFailures: Number(evaluation?.hardFailures || evalRun?.hardFailures || 0) + Number(scientificGovernance.hardFailures.length) })
  if (!approval.ok || mode === 'active' || promotion.autonomousPromotion !== false) throw new Error('learning_manual_approval_or_canary_required')
  const at = nowIso(clock)
  ...
  const doc = {
    ...
    approvalSetHash: approval.approvalSetHash,
    proposalHash,
    autonomousPromotion: false,
    scientificGovernanceHash: scientificGovernance.receiptHash,
...
export async function promoteQl7SupportCanary({ database, deploymentId, observed = {}, deploymentScope = '', approvals = [], signingKeys = {}, proposal = {}, ablation = {}, counterfactual = {}, shadow = {}, clock = Date.now } = {}) {
  ...
  const candidate = await database.collection(QL7_SUPPORT_LEARNING_CANDIDATES).findOne({ _id: deployment.candidateId })
  if (!candidate || candidate.consent !== true || !str(candidate.consentReceiptId)) throw new Error('learning_consent_required')
  const sensitive = /safety|economic|quarantine|composer/iu.test(`${candidate?.signalType || ''} ${JSON.stringify(proposal || {})}`)
  const proposalHash = sha(proposal || { candidateId: deployment.candidateId, deploymentId: deployment._id })
  const approval = validateQl7SupportLearningApprovals({ approvals, candidateId: deployment.candidateId, proposalHash, sensitive, signingKeys })
  const scientificGovernance = evaluateQl7SupportScientificLearningGovernance({ owner: 'ql7-support.learning.promotion', features: observed?.features || [], baselineScore: Number(observed?.baselineScore || 0), candidateScore: Number(observed?.candidateScore || 0), weightChanges: proposal?.changes || [], evidenceIds: observed?.evidenceIds || [], constraints: proposal?.constraints || {}, ablation, counterfactualPairs: counterfactual?.pairs || [], drift: observed?.drift || observed || {} })
  const promotion = evaluateQl7SupportPromotion({ proposal, approvals: approval.approvals, ablation, counterfactual, shadow, canary: { ok: safe, ...observed }, hardFailures: Number(observed?.hardFailures || 0) + Number(scientificGovernance.hardFailures.length) })
  if (!approval.ok || !promotion.allowed) throw new Error('learning_promotion_requires_signed_manual_approval')
  const at = nowIso(clock)
  await database.collection(QL7_SUPPORT_LEARNING_DEPLOYMENTS).updateOne({ _id: deployment._id }, { $set: { status: 'active', canaryPercent: 100, promotedAt: at, observed: clone(observed) || {}, approvalSetHash: approval.approvalSetHash, proposalHash, scientificGovernanceHash: scientificGovernance.receiptHash, scientificGovernance, autonomousPromotion: false } })
  await database.collection(QL7_SUPPORT_DEPLOYMENT_STATE).updateOne(
...
export async function deleteQl7SupportUserLearningData(args = {}) { return deleteLearningDataAuthority(args) }
