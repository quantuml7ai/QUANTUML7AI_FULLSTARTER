...
import { hasVerifiedQl7SupportReceiptV13 } from '../data/adapterReceipt.js'
...
export const QL7_SUPPORT_RESPONSE_CRITIC_VERSION_V13='13.0.0'
export const QL7_SUPPORT_MACHINE_PHRASES_V13=Object.freeze([
...
const V13_FORBIDDEN=Object.freeze([
...
function normalizedSentenceV13(s){return norm(s)}
...
export function critiqueQl7SupportResponseV13({text='',card=null,locale='en',plan={},receipts=[],expectedLocale=''}={}){
  const failures=[]; const source=[text,card?.title,card?.summary].map(ql7Str).filter(Boolean).join('\n'); const lang=ql7Locale(locale); const count=ql7CountGraphemes(text,lang); const sentences=ql7Sentences(text)
  if(!ql7Str(text)) failures.push({code:'missing_response_text',severity:'critical'})
  if(count>400) failures.push({code:'excessive_length',severity:'critical',actual:count,max:400})
  if(sentences.length>6) failures.push({code:'excessive_sentence_count',severity:'high',actual:sentences.length,max:6})
  for(const [code,pattern,severity] of V13_FORBIDDEN) if(pattern.test(source)) failures.push({code,severity})
  for(const pattern of QL7_SUPPORT_MACHINE_PHRASES_V13) if(pattern.test(source)) failures.push({code:'machine_phrase_leakage',severity:'critical'})
  const keys=sentences.map(normalizedSentenceV13).filter(Boolean); if(new Set(keys).size!==keys.length) failures.push({code:'semantic_repetition',severity:'high'})
  if(plan?.requiresReceipt && /(?:проверка завершена|check is complete|comprobación terminó|kontrol tamamlandı|اكتمل التحقق|检查已完成|הבדיקה הושלמה)/iu.test(source) && !hasVerifiedQl7SupportReceiptV13(receipts)) failures.push({code:'claim_without_adapter_receipt',severity:'critical'})
  if(expectedLocale && ql7Locale(expectedLocale)!==lang) failures.push({code:'locale_mismatch',severity:'critical',expected:ql7Locale(expectedLocale),actual:lang})
  if(plan?.kind==='information' && /(?:что вы ожидали|what did you expect|expected result versus actual|последнее действие|last action)/iu.test(source)) failures.push({code:'informational_answer_became_incident_intake',severity:'high'})
  return Object.freeze({version:QL7_SUPPORT_RESPONSE_CRITIC_VERSION_V13,ok:failures.length===0,locale:lang,graphemes:count,sentenceCount:sentences.length,failures:Object.freeze(failures)})
}
