export const QL7_SUPPORT_ADMIN_REPORT_RU_VERSION = '12.0.0'

function str(value) { return String(value ?? '').trim() }
function visible(value) { return str(value).replace(/q[\s._-]*l[\s._-]*7(?:[\s._-]*support)?/giu, 'поддержка') }
function esc(value) {
  return str(value).replace(/[&<>"']/g, (ch) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[ch])
}

export function composeQl7SupportAdminReportRu({ caseId = '', topic = '', identity = {}, tone = {}, recommendation = '' } = {}) {
  return Object.freeze({
    version: QL7_SUPPORT_ADMIN_REPORT_RU_VERSION,
    locale: 'ru',
    subject: `Поддержка: ${str(topic) || 'case'} ${str(caseId).slice(0, 24)}`,
    body: [
      'Сводка обращения в поддержку.',
...
export function renderQl7SupportAdminReportRuHtml(report = {}) {
  const safeBody = visible(report.body || 'Сводка обращения в поддержку.')
  const safeSubject = visible(report.subject || 'Поддержка: обращение на проверку')
...
</style></head><body bgcolor="#07111f" style="margin:0;background:#07111f!important;color:#ffffff!important"><main class="wrap" style="background:#07111f;color:#ffffff"><section class="panel" style="color:#ffffff!important"><h1>Отчёт поддержки оператору</h1><span class="badge">privacy-safe</span><span class="badge">русский SMTP report</span><div class="message" bgcolor="#092848" style="white-space:pre-wrap;border:1px solid #9edfff;border-radius:14px;padding:16px;background:#092848!important;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;font-weight:650;line-height:1.55;overflow-wrap:anywhere">${esc(safeBody)}</div></section></main></body></html>
`
}
