...
export function normalizeQl7MetricKeyV9(value = '') {
  return ALIASES.get(key(value)) || ''
}

export function isQl7MetricAllowedV9(value = '') {
  return Boolean(normalizeQl7MetricKeyV9(value))
}

export function localizeQl7MetricV9(value = '', locale = 'en') {
  const canonical = normalizeQl7MetricKeyV9(value)
  const lang = str(locale).toLowerCase().split(/[-_]/u)[0] || 'en'
  return METRICS[canonical]?.labels?.[lang] || METRICS[canonical]?.labels?.en || ''
}

export function ql7MetricFormatV9(value = '') {
  const canonical = normalizeQl7MetricKeyV9(value)
  return METRICS[canonical]?.format || 'text'
}

export function normalizeQl7MetricRowV9(row = {}, locale = 'en') {
  const rawKey = str(row?.key || row?.label)
  const canonical = normalizeQl7MetricKeyV9(rawKey)
  if (!canonical) return null
  const value = row?.value
  if (value === null || value === undefined || value === '') return null
  return Object.freeze({
    key: canonical,
    label: localizeQl7MetricV9(canonical, locale),
    value,
    format: str(row?.format) || ql7MetricFormatV9(canonical),
    tone: str(row?.tone || 'neutral'),
    visibility: str(row?.visibility || 'both'),
  })
}

export function normalizeQl7MetricRowsV9(rows = [], locale = 'en') {
  const seen = new Set()
  return (Array.isArray(rows) ? rows : [])
    .map((row) => normalizeQl7MetricRowV9(row, locale))
    .filter(Boolean)
    .filter((row) => {
...
export const QL7_SUPPORT_METRIC_REGISTRY_V9 = METRICS
