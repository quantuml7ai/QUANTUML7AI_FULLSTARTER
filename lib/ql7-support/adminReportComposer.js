...
function humanReadableScalar(value = '') {
  const clean = redactQl7SupportSecrets(str(value))
...
export function composeQl7SupportAdminReport({
  ...
  return Object.freeze({
    ...
    title: 'QL7 Support - отчёт оператору',
    subject: `QL7 Support: обращение ${str(input.caseId || input.messageId || 'на проверку')}`,
...
export function renderQl7SupportAdminReportText(report = {}) {
  ...
  return [
    report.title || 'QL7 Support - отчёт оператору',
    '',
...
export function renderQl7SupportAdminReportHtml(report = {}) {
  ...
  <div class="hero"><h1>${esc(report.title || 'QL7 Support - отчёт оператору')}</h1><div class="badges"><span class="badge">${esc(humanize(report.priority || 'normal'))}</span><span class="badge">${esc(report.case?.topic || 'Support')}</span><span class="badge">${esc(report.case?.caseStatus || report.case?.diagnosticStatus || 'открыт')}</span></div></div>
