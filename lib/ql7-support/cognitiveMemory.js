...
import {countQl7SupportGraphemes} from './limits.js'

export const QL7_SUPPORT_CANONICAL_MESSAGE_COLLECTION = 'dm_messages'
export const QL7_SUPPORT_COGNITIVE_COLLECTIONS = Object.freeze({
  turns: 'ql7_support_turn_decisions',
  outcomes: 'ql7_support_dialogue_outcomes',
  actions: 'ql7_support_action_outcomes',
  translations: 'ql7_support_translation_outcomes',
  quality: 'ql7_support_response_quality',
  personality: 'ql7_support_personality_state',
  adaptation: 'ql7_support_user_adaptation',
  calibrations: 'ql7_support_calibration_snapshots',
  simulationRuns: 'ql7_support_simulation_runs',
  simulationFailures: 'ql7_support_simulation_failures',
})
...
export async function ensureQl7SupportCognitiveIndexes(database) {
  if (!database?.collection) return { ok: false, reason: 'database_unavailable' }
  if (!globalThis[INDEX_KEY]) {
    const c = QL7_SUPPORT_COGNITIVE_COLLECTIONS
...
export async function readQl7SupportLearningOptOut(database, userId = '') {
  ...
  const [preference, legacyProfile, adaptation] = await Promise.all([
    ...
    database.collection('ql7_support_communication_profiles').findOne({ _id: uid }).catch(() => null),
    database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.adaptation).findOne({ userIdHash: uidHash }).catch(() => null),
  ])
  return preference?.learningOptOut === true || preference?.optOut === true || legacyProfile?.optOut === true || adaptation?.learningOptOut === true
}

export async function readQl7SupportPersonalityState({ database, userId = '' } = {}) {
  if (!database?.collection || !str(userId)) return null
  return database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.personality)
    .findOne({ userIdHash: userHash(userId) })
    .catch(() => null)
}

export async function writeQl7SupportPersonalityState({ database, userId = '', state = null, evidenceType = '' } = {}) {
  if (!database?.collection || !str(userId) || !state || typeof state !== 'object') return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  ...
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.personality).updateOne(
    { userIdHash },
    { $set: safeState, $setOnInsert: { _id: `personality:${userIdHash}`, userIdHash, createdAt: now } },
    { upsert: true },
  )
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.adaptation).updateOne(
    { userIdHash },
    { $set: { locale: safeState.locale, confidence: safeState.confidence, sampleSize: safeState.sampleSize, personalityVersion: safeState.version, updatedAt: now }, $setOnInsert: { _id: `adaptation:${userIdHash}`, userIdHash, learningOptOut: false, createdAt: now } },
    { upsert: true },
  )
  return { ok: true, userIdHash }
}

export async function recordQl7SupportCognitiveTurn({ database, userId = '', messageId = '', caseId = '', requestContext = {}, replyPlan = {}, diagnosticResult = null, languageInput = {}, actionIds = [], personalityState = null, modelVersion = 'canonical', calibrationVersion = 'baseline' } = {}) {
  if (!database?.collection || !str(userId) || !str(messageId)) return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  const userIdHash = userHash(userId)
  const caseIdHash = sha(str(caseId))
  const turnId = `turn:${sha(`${messageId}:${userIdHash}:${caseIdHash}`).slice(0, 40)}`
  const replyText = str(replyPlan?.text)
  const row = {
    _id: turnId,
    turnId,
    canonicalMessageCollection: QL7_SUPPORT_CANONICAL_MESSAGE_COLLECTION,
    ...
    replyGraphemes: countQl7SupportGraphemes(replyText, languageInput?.detectedLanguage || 'en'),
  ...
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.turns).updateOne({ _id: turnId }, { $setOnInsert: row }, { upsert: true })
  return { ok: true, turnId, userIdHash, caseIdHash }
}

export async function resolveLatestQl7SupportCognitiveTurn({ database, userId = '', caseId = '', messageId = '' } = {}) {
  if (!database?.collection || !str(userId)) return null
  ...
  return database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.turns).findOne(filter, { sort: { createdAt: -1 } }).catch(() => null)
}

export async function recordQl7SupportOutcome({ database, userId = '', turnId = '', outcomeType = '', value = '', metadata = {} } = {}) {
  if (!database?.collection || !str(userId) || !str(turnId) || !str(outcomeType)) return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  const userIdHash = userHash(userId)
  const id = `outcome:${sha(`${turnId}:${outcomeType}`).slice(0, 48)}`
  const now = new Date().toISOString()
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.outcomes).updateOne(
    { _id: id },
  ...
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.turns).updateOne(
...
export async function recordQl7SupportActionOutcome({ database, userId = '', turnId = '', actionId = '', routeId = '', outcomeType = '', metadata = {} } = {}) {
  if (!database?.collection || !str(userId) || !str(turnId) || !str(actionId) || !str(outcomeType)) return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  const userIdHash = userHash(userId)
  const id = `action:${sha(`${turnId}:${actionId}:${outcomeType}`).slice(0, 48)}`
  const now = new Date().toISOString()
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.actions).updateOne(
    { _id: id },
...
export async function recordQl7SupportTranslationOutcome({ database, userId = '', turnId = '', locale = '', provider = '', status = '', sourceLocale = '', machineFieldsPreserved = true, userVisibleCoverage = null } = {}) {
  if (!database?.collection || !str(userId) || !str(turnId)) return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  const userIdHash = userHash(userId)
  const id = `translation:${sha(`${turnId}:${locale}`).slice(0, 48)}`
  const now = new Date().toISOString()
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.translations).updateOne(
    { _id: id },
...
export async function recordQl7SupportResponseQuality({ database, userId = '', turnId = '', modelVersion = 'canonical', calibrationVersion = 'baseline', metrics = {} } = {}) {
  if (!database?.collection || !str(userId) || !str(turnId)) return { ok: false, skipped: true }
  if (await readQl7SupportLearningOptOut(database, userId)) return { ok: true, skipped: true, reason: 'learning_opt_out' }
  await ensureQl7SupportCognitiveIndexes(database)
  const userIdHash = userHash(userId)
  const id = `quality:${sha(turnId).slice(0, 48)}`
  const now = new Date().toISOString()
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.quality).updateOne(
    { _id: id },
...
export async function recordQl7SupportSimulationRun({ database, run = {} } = {}) {
  if (!database?.collection || !str(run?.runId)) return { ok: false, skipped: true }
  await ensureQl7SupportCognitiveIndexes(database)
  const now = new Date().toISOString()
  const safeRun = cleanObject(run)
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.simulationRuns).updateOne(
    { runId: str(run.runId) },
    { $set: { ...safeRun, runId: str(run.runId), updatedAt: now, storagePrimary: 'mongo' }, $setOnInsert: { _id: `simulation:${sha(run.runId).slice(0, 40)}`, createdAt: now } },
    { upsert: true },
  )
  return { ok: true, runId: str(run.runId) }
}

export async function recordQl7SupportSimulationFailure({ database, runId = '', failure = {} } = {}) {
  ...
  const id = `sim-failure:${sha(`${runId}:${failureId}`).slice(0, 48)}`
  const now = new Date().toISOString()
  await database.collection(QL7_SUPPORT_COGNITIVE_COLLECTIONS.simulationFailures).updateOne(
    { _id: id },
...
export async function deleteQl7SupportCognitiveMemoryForUser({ database, userId = '' } = {}) {
