...
const SUPPORT_AUTO_REPLIES = Object.freeze({
  new: {
    en: [
      '{topic} line opened. The context is preserved in this thread, and the next verified update will return here.',
      'Your {topic} case is active. Add concrete details here when needed; they will stay attached to the same service line.',
      'Message accepted for {topic}. The thread will stay ordered around facts, source checks, and the next confirmed step.',
    ],
    ru: [
      'Линия «{topic}» открыта. Контекст сохранён в этой ветке, следующий проверенный ответ вернётся сюда.',
      'Кейс «{topic}» активен. Конкретные детали добавляйте здесь — они останутся в той же сервисной линии.',
      'Сообщение принято по направлению «{topic}». Ветка будет держаться вокруг фактов, проверки источников и следующего подтверждённого шага.',
    ],
    uk: [
      'Лінію «{topic}» відкрито. Контекст збережено в цій гілці, наступна перевірена відповідь повернеться сюди.',
      'Кейс «{topic}» активний. Конкретні деталі додавайте тут — вони залишаться у тій самій сервісній лінії.',
      'Повідомлення прийнято за напрямом «{topic}». Гілка триматиметься навколо фактів, перевірки джерел і наступного підтвердженого кроку.',
    ],
    es: [
      'Línea de {topic} abierta. El contexto queda preservado en este hilo y la próxima actualización verificada volverá aquí.',
      'Tu caso de {topic} está activo. Añade detalles concretos aquí cuando haga falta; quedarán unidos a la misma línea de servicio.',
      'Mensaje aceptado para {topic}. El hilo se mantendrá ordenado alrededor de hechos, fuentes y el siguiente paso confirmado.',
    ],
    tr: [
      '{topic} hattı açıldı. Bağlam bu konuşmada korunacak ve sonraki doğrulanmış güncelleme buraya dönecek.',
      '{topic} dosyanız aktif. Gerektiğinde somut ayrıntıları buraya ekleyin; aynı servis hattında kalacaklar.',
      'Mesaj {topic} için kabul edildi. Konuşma gerçekler, kaynak kontrolleri ve sonraki onaylı adım etrafında düzenli kalacak.',
    ],
    ar: [
      'تم فتح مسار {topic}. سيبقى السياق محفوظاً في هذه المحادثة، وسيعود التحديث المؤكد التالي هنا.',
      'حالة {topic} نشطة. أضف التفاصيل الدقيقة هنا عند الحاجة؛ ستبقى ضمن خط الخدمة نفسه.',
      'تم قبول الرسالة بخصوص {topic}. ستبقى المحادثة مرتبة حول الحقائق وفحص المصادر والخطوة المؤكدة التالية.',
    ],
    zh: [
      '{topic} 服务线已打开。上下文会保存在本会话中，下一条已确认更新会回到这里。',
      '你的 {topic} 案件已激活。需要时请在这里补充具体细节，它们会保留在同一条服务线。',
      '消息已按 {topic} 接收。本线程会围绕事实、来源核验和下一步确认动作保持清晰。',
    ],
  },
  followup: {
    en: [
