...
export const QL7_SUPPORT_NATIVE_LOCALES = Object.freeze(['en', 'ru', 'uk', 'es', 'tr', 'ar', 'zh'])
...
export function detectQl7SupportLanguage(text = '', selectedLocale = '') {
  ...
  if (scores.latin > 0) {
    ...
    if (selected && QL7_SUPPORT_NATIVE_LOCALES.includes(selected)) return selected
    return 'en'
  }
  return selected || 'en'
}

function translationHash({ text, source, target, provider }) {
  return crypto.createHash('sha256')
    .update([source, target, provider, text].join('\u0000'))
    .digest('hex')
    .slice(0, 24)
}
...
function providerOutputMatchesTarget({ originalText = '', translatedText = '', sourceLanguage = '', targetLanguage = '' } = {}) {
...
function normalizeProviderResult(result, originalText = '', { sourceLanguage = '', targetLanguage = '' } = {}) {
  const value = result && typeof result === 'object' ? result : {}
  const text = str(value.text)
  const provider = str(value.provider)
  const providerWarning = str(value.warning)
  const targetValid = providerOutputMatchesTarget({
    originalText,
    translatedText: text,
    sourceLanguage,
    targetLanguage,
  ...
  return {
    ok: !!text && text !== str(originalText) && provider !== 'fallback_original' && targetValid,
    text: text || str(originalText),
    provider: provider || 'unavailable',
    warning: targetValid ? providerWarning : (providerWarning || 'provider_output_language_mismatch'),
  }
}
...
export async function prepareQl7SupportLanguageInput({
  ...
  const translationRequired = detectedLanguage !== QL7_SUPPORT_CANONICAL_LOCALE
  if (!translationRequired || typeof translate !== 'function') {
    return {
      originalText,
      redactedText,
      canonicalText: redactedText,
      detectedLanguage,
      canonicalLanguage: translationRequired ? detectedLanguage : QL7_SUPPORT_CANONICAL_LOCALE,
      translationRequired,
      translationStatus: translationRequired ? 'provider_unavailable' : 'native',
      translationProvider: translationRequired ? 'unavailable' : 'native',
      translationEvidenceHash: '',
    }
  }

  const result = normalizeProviderResult(await translate({
    text: redactedText,
  ...
  return {
    ...
    translationRequired,
    translationStatus: result.ok ? 'translated' : 'provider_failed',
    translationProvider: result.provider,
    translationWarning: result.warning,
    translationEvidenceHash: result.ok
      ? translationHash({ text: result.text, source: detectedLanguage, target: 'en', provider: result.provider })
      : '',
  }
}
...
export async function localizeQl7SupportReply({
  ...
  if (!sourceText || target === 'en' || typeof translate !== 'function') {
    return {
      text: sourceText,
      targetLanguage: target,
      translationStatus: target === 'en' ? 'native' : 'provider_unavailable',
      translationProvider: target === 'en' ? 'native' : 'unavailable',
      translationEvidenceHash: '',
    }
  }
  const result = normalizeProviderResult(await translate({
  ...
  return {
    ...
    translationWarning: result.warning,
    translationEvidenceHash: result.ok
      ? translationHash({ text: result.text, source: 'en', target, provider: result.provider })
      : '',
  }
}
