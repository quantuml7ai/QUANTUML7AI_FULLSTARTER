...
export const QL7_SUPPORT_LEARNING_CONTROL_VERSION = 'ql7-learning-control'
...
export function calculateQl7SupportCognitiveMaturity(metrics = {}, { evalRunId = '', corpusHash = '', modelVersion = 'canonical', asOf = new Date().toISOString() } = {}) {
  const keys = [
  ...
  return Object.freeze({
    version: QL7_SUPPORT_LEARNING_CONTROL_VERSION,
    status,
...
export function buildQl7SupportCalibrationCandidate({ cluster = {}, evidence = [], proposedChanges = {}, baseline = {}, candidate = {} } = {}) {
  const rows = Array.isArray(evidence) ? evidence : []
  return Object.freeze({
    candidateId: `cal:${hash({ cluster, proposedChanges, at: Date.now() }).slice(0, 36)}`,
    version: QL7_SUPPORT_LEARNING_CONTROL_VERSION,
    status: 'candidate',
...
export function evaluateQl7SupportPromotionGate({ baseline = {}, candidate = {}, criticalFailures = [] } = {}) {
...
export const QL7_SUPPORT_LEARNING_COMPATIBILITY_MAP = Object.freeze({
  ...
  legacySignals: 'ql7_support_learning_signals',
  legacyProfiles: 'ql7_support_communication_profiles',
})

export function buildQl7SupportGovernedLearningState({ stage = 'observation', candidate = null, evaluation = null, deployment = null } = {}) {
  ...
  return Object.freeze({
    ...
    compatibleCollections: QL7_SUPPORT_LEARNING_COMPATIBILITY_MAP,
