...
function constantTimeEqualText(left = '', right = '') {
  const a = crypto.createHash('sha256').update(str(left)).digest()
  const b = crypto.createHash('sha256').update(str(right)).digest()
...
function makePlan({
  ...
  return {
    ...
    recipients: ids,
    sourcePayloadKind: 'typed_event_source_statement',
    approvedTemplateId: type === 'critical_security'
      ? 'security.critical-source'
      : 'broadcast.admin-source',
    sourcePayloadPreview: str(message).slice(0, 160),
...
export function authorizeQl7SupportBroadcastRequest({ fromUserId = '', rawFromIds = [], token = '', keyId = '', env = process.env } = {}) {
  const configuredKeyId = str(env.QL7_SUPPORT_BROADCAST_KEY_ID || 'broadcast-primary')
  if (!keyId || !constantTimeEqualText(keyId, configuredKeyId)) {
    const error = new Error('ql7_support_broadcast_key_id_invalid')
    error.status = 403
    throw error
  }
  return assertBroadcastCommandAllowed({ fromUserId, rawFromIds, token, env })
}
...
function assertBroadcastCommandAllowed({ fromUserId, rawFromIds = [], token, env = process.env } = {}) {
  ...
  if (!expected || !constantTimeEqualText(token, expected)) {
