...
import serverSecret from '../security/ql7-server-secret.cjs'

export const QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11 = 11
export const QL7_SUPPORT_CHOICE_TTL_MS_V11 = 30 * 60 * 1000
...
function canonicalPayload(payload = {}) {
  return JSON.stringify({
    v: Number(payload.v || QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11),
...
async function resolveChoiceKey(secretOverride = '') {
  ...
  if (direct.length >= 32) return crypto.createHash('sha256').update(`ql7-support-choice-v11:${direct}`).digest()
  const derived = await serverSecret.deriveForumRuntimeSecret('ql7-support-choice:v11')
...
async function decodeAndVerifyToken(token = '', secretOverride = '') {
  ...
  if (Number(payload?.v) !== QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11) return { ok: false, error: 'choice_token_version' }
...
export async function attachQl7SupportSignedChoicesV11({
  card = null,
  database = null,
  userId = '',
  ownerCaseId = '',
  issuedAt = Date.now(),
  ttlMs = QL7_SUPPORT_CHOICE_TTL_MS_V11,
  secret = '',
} = {}) {
  ...
  if (!card || !uid || !owner || (!options.length && !other)) return { card: clone(card), pendingChoice: null, signed: false }

  const issued = nowMs(issuedAt)
  const expires = issued + Math.max(60_000, Math.min(24 * 60 * 60 * 1000, Number(ttlMs) || QL7_SUPPORT_CHOICE_TTL_MS_V11))
  const choiceSetId = `choice-v11:${sha(`${uid}:${owner}:${issued}:${crypto.randomBytes(16).toString('hex')}`).slice(0, 32)}`
  ...
  const signOption = async (option, kind = 'option') => {
    ...
    const payload = {
      v: QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11,
      choiceSetId,
      ownerCaseId: owner,
  ...
  unsigned.choiceContractVersion = QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11
  unsigned.options = signedOptions
  unsigned.other = signedOther
  unsigned.expiresAt = new Date(expires).toISOString()

  const pendingChoice = {
    version: QL7_SUPPORT_CHOICE_CONTRACT_VERSION_V11,
    choiceSetId,
    ownerCaseId: owner,
    userHash,
    issuedAt: new Date(issued).toISOString(),
  ...
  if (database?.collection) {
    await database.collection('ql7_support_cases').updateOne(
      ...
      { $set: { pendingChoiceV11: pendingChoice, nextState: 'waiting_choice', updatedAt: new Date().toISOString() } },
    )
  }
  return { card: unsigned, pendingChoice, signed: true }
}

export async function verifyQl7SupportChoiceTokenV11({ token = '', userId = '', secret = '', now = Date.now() } = {}) {
  ...
  if (!str(payload.choiceSetId) || !str(payload.ownerCaseId) || !str(payload.optionId)) return { ok: false, error: 'choice_payload_incomplete' }
  return { ok: true, payload: Object.freeze({ ...payload }) }
}

export async function consumeQl7SupportChoiceV11({
  database = null,
  userId = '',
  supportChoice = null,
  secret = '',
  ...
  const verified = await verifyQl7SupportChoiceTokenV11({ token: supportChoice.signedToken, userId, secret, now })
  if (!verified.ok) return verified
  const payload = verified.payload
  ...
  const pending = ownerCase?.pendingChoiceV11
  ...
  const updated = await database.collection('ql7_support_cases').updateOne(
    { _id: payload.ownerCaseId, userId: str(userId), 'pendingChoiceV11.choiceSetId': payload.choiceSetId, 'pendingChoiceV11.consumedAt': null },
    { $set: {
      'pendingChoiceV11.consumedAt': nowIso,
      'pendingChoiceV11.selectedOptionId': payload.optionId,
      'pendingChoiceV11.selectedTargetCaseId': payload.targetCaseId,
  ...
  if (Number(updated?.matchedCount || 0) !== 1) {
    ...
    if (str(reread?.pendingChoiceV11?.selectedOptionId) !== str(payload.optionId)) return { ok: false, error: 'choice_consume_race' }
  }
  return {
    ok: true,
    selected: true,
...
export function hasQl7SupportChoiceSelectionAttemptV11(value = {}) {
...
export function sanitizeQl7SupportChoiceTransportV11(value = {}) {
