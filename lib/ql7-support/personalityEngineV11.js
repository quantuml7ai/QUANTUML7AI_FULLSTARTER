...
export const QL7_SUPPORT_PERSONALITY_VERSION_V11 = 'ql7-support-personality-v11.0.3'
export const QL7_SUPPORT_IMMUTABLE_CONSTITUTION_V11 = Object.freeze({
  truthFirst: true,
  privacyFirst: true,
...
export function normalizeQl7SupportPersonalityTraitsV11(value = {}) {
  const out = {}
  for (const [key, fallback] of Object.entries(DEFAULT_TRAITS)) out[key] = clamp01(value?.[key], fallback)
  return Object.freeze(out)
}

export function buildQl7SupportPersonalityEvidenceV11({ outcomeType = '', actionId = '', value = '', metadata = {} } = {}) {
...
export function buildQl7SupportPersonalityStateV11({ previous = null, evidence = [], locale = 'en', now = new Date().toISOString() } = {}) {
  const prior = normalizeQl7SupportPersonalityTraitsV11(previous?.traits || previous)
  ...
  const state = {
    version: QL7_SUPPORT_PERSONALITY_VERSION_V11,
    locale: str(locale || previous?.locale || 'en').slice(0, 24),
    traits: Object.freeze(traits),
    sampleSize,
    confidence,
    constitutionHash: hash(JSON.stringify(QL7_SUPPORT_IMMUTABLE_CONSTITUTION_V11)),
    updatedAt: String(now),
  }
  return Object.freeze(state)
}

export function selectQl7SupportResponseModeV11({ messageAct = '', topic = '', diagnosticResult = null, tone = {}, hasCard = false, requestedDetail = false, personalityState = null } = {}) {
  const act = str(messageAct)
  const traits = normalizeQl7SupportPersonalityTraitsV11(personalityState?.traits || personalityState || {})
...
export function getQl7SupportPersonalityDefaultsV11() { return DEFAULT_TRAITS }
