import {getQl7SupportFactLabel} from './language/factPresentationLexicon.js'
...
const FACT_ENUM_KEYS=Object.freeze({active:'active',inactive:'inactive',available:'available',exhausted:'exhausted'})
function lang(locale='en'){const k=str(locale).toLowerCase().split(/[-_]/u)[0];return QL7_SUPPORT_ALL_LOCALES.includes(k)?k:'en'}
function localizedFactEnum(value='',locale='en'){
 const factKey=FACT_ENUM_KEYS[str(value).toLowerCase()]
 if(!factKey)return''
 try{return getQl7SupportFactLabel(lang(locale),factKey).value}catch{return''}
}
export function localizeQl7EvidenceValue(value,locale='en'){
 const key=lang(locale)
 if(typeof value==='boolean')return ENUMS[String(value)][key]||ENUMS[String(value)].en
 const raw=str(value)
 const factValue=localizedFactEnum(raw,key)
 if(factValue)return factValue
 if(ENUMS[raw.toLowerCase()])return ENUMS[raw.toLowerCase()][key]||ENUMS[raw.toLowerCase()].en
 if(/^0x[a-f0-9]{40}$/iu.test(raw))return `${raw.slice(0,6)}…${raw.slice(-4)}`
 if(/^(?:wallet|telegram|tg|tguid):/iu.test(raw))return '••••••'
 return raw
}
export function sanitizeQl7EvidenceRows(rows=[],locale='en'){
 ...
 for(const row of Array.isArray(rows)?rows:[]){
  ...
  const value=localizeQl7EvidenceValue(row.value,locale)
...
export function decideQl7EvidenceStatus({claim='',checks=[],facts=[],unknowns=[],anomalies=[]}={}){
...
export function assertNoQl7MachineLeak(value){
 const projection=value&&typeof value==='object'?value:{}
 const visible={
  title:projection.title,
  summary:projection.summary,
  status:projection.status,
  rows:Array.isArray(projection.tableSchema?.rows)?projection.tableSchema.rows:[],
  nextActions:Array.isArray(projection.nextActions)?projection.nextActions:[],
 }
 const text=JSON.stringify(visible)
 return !/(?:(?:"(?:key|label)"\s*:\s*"[^"]*(?:adapter|analyzer|collection|query|filter|rawaccount|userid|accountid|subjectid|runid|signature|token|secret|password|internal|sourcecontract|businesscollection|supportcollection)[^"]*")|"details"\s*:|\bpending\b|\btrue\b|\bfalse\b|wallet:0x|mongodb|collectionName|adapterId|rawAccountId)/iu.test(text)
}
