...
function actorReceiptId(actor = {}) {
  const accountId = str(actor.canonicalAccountId || actor.accountId || actor.userId)
  if (!accountId) return ''
  const body = JSON.stringify({
    accountIdHash: tokenHash(accountId),
  ...
  return `actor-receipt:${tokenHash(body)}`
}
...
async function resolveWalletActor({ req, body, database, clock }) {
  ...
  return { ...actor, actorReceiptId: actorReceiptId(actor) }
}
