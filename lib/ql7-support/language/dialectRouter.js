...
export const QL7_SUPPORT_LANGUAGE_DIALECT_ROUTER_VERSION = '12.0.0'
...
export function routeQl7SupportLanguageDialect({ text = '', selectedLocale = '', scenarioClass = '', dialectPack = '' } = {}) {
  const detectedLanguage = detectQl7SupportLanguage(text, selectedLocale)
  const locale = QL7_SUPPORT_SIMULATION_LANGUAGES.includes(detectedLanguage) ? detectedLanguage : (QL7_SUPPORT_SIMULATION_LANGUAGES.includes(str(selectedLocale)) ? str(selectedLocale) : 'en')
  ...
  return Object.freeze({
    version: QL7_SUPPORT_LANGUAGE_DIALECT_ROUTER_VERSION,
    locale,
    ...
    supported: QL7_SUPPORT_SIMULATION_LANGUAGES.includes(locale),
  })
}
