...
import {QL7_SUPPORT_SEMANTIC_CONCEPT_TERM_COUNT, auditQl7SupportSemanticConceptBank} from './semanticConceptBank.js'
import {buildQl7CanonicalInputVariants} from './mutationLattice.js'

export const QL7_SUPPORT_SEMANTIC_BANK_VERSION = '15.5.0'

export const QL7_SUPPORT_SEMANTIC_EXPANSION_POLICY = Object.freeze({
  version: 'controlled-mutation-expansion',
...
const TOPIC_ALIASES = Object.freeze({
  ...
  exchange: ['quantum exchange', 'exchange', 'market chart', 'order book', 'orderbook', 'биржа', 'бирже', 'аналитика на бирже', 'аналитикой на бирже', 'стакан', 'график рынка', 'біржа', 'біржі', 'аналітика на біржі', 'ринковий графік', 'bolsa crypto', 'borsa grafiği', 'سجل الأوامر', '交易所', 'ספר פקודות'],
...
const TOPIC_ALIAS_VARIANTS = Object.freeze(Object.fromEntries(
  Object.entries(TOPIC_ALIASES).map(([topic, aliases]) => [
    topic,
    Object.freeze(Array.from(new Set(ql7Arr(aliases).flatMap((alias) => termVariants('topicAlias', alias))))),
  ]),
))
...
const BANKS = Object.freeze({
  ...
  ru: freezeBank({
    ...
    profanity: ['бля', 'блядь', 'блять', 'сука', 'хуй', 'нахуй', 'пиздец', 'ебаный', 'ёбаный', 'мудак', 'дебил', 'идиот', 'долбоеб', 'долбоёб', 'пидор', 'пидорас', 'пидар', 'тупой', 'урод'],
  ...
  uk: freezeBank({
    ...
    profanity: ['бля', 'блядь', 'блять', 'сука', 'хуй', 'нахуй', 'пиздець', 'їбаний', 'єбаний', 'мудак', 'дебіл', 'ідіот', 'довбойоб', 'підор', 'пидор', 'тупий'],
...
function safetyCanonicalForms(text = '', locale = 'und') {
  const source = canonical(text)
  const lattice = buildQl7CanonicalInputVariants(text, { locale, maxVariants: 24 })
  const latticeForms = lattice.hypotheses.map((row) => canonical(row.text)).filter(Boolean)
  const folded = latticeForms.map((value) => canonical(value.replace(/[@0346acekmoptxy]/gu, (char) => SAFETY_INPUT_FOLD[char] || char)))
  return Object.freeze(Array.from(new Set([source, ...latticeForms, ...folded].filter(Boolean))))
}

function isLooseScript(term = '') {
  return /[\p{Script=Han}\p{Script=Hangul}\p{Script=Arabic}\p{Script=Hebrew}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Thai}\p{Script=Devanagari}]/u.test(term)
}
...
export function collectQl7SemanticSignals(text = '', locale = 'en') {
  ...
  const topicWeights = []
  for (const [topic, aliases] of Object.entries(TOPIC_ALIAS_VARIANTS)) {
...
export function collectQl7SafetySignals(text = '', locale = '') {
  const sources = safetyCanonicalForms(text, locale || 'und')
