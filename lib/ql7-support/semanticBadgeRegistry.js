...
export const QL7_SEMANTIC_BADGES=Object.freeze({
...
export function normalizeQl7SemanticBadgeKey(value=''){
 const key=str(value).toLowerCase().replace(/[\s-]+/gu,'_')
 const aliased=KEY_ALIASES[key]||key
 return QL7_SEMANTIC_BADGES[aliased]?aliased:''
}
...
function badgeSeconds(badge={}){
 ...
 return secondsFromTimer(badge?.label||badge?.value)
}

function inferRailBadgeKey(badge={}){
 const icon=normalizeQl7SemanticBadgeKey(badge?.icon||badge?.iconKey||badge?.semanticIcon)
 if(SPECIAL_KEYS.has(icon))return icon
 const tone=normalizeQl7SemanticBadgeKey(badge?.tone)
 ...
 if(/^(?:blocked|restricted|pause|paused|пауза|на паузе|pausa|duraklama|إيقاف|暂停|השהיה)/iu.test(label))return badgeSeconds(badge)?'cooldown':'blocked'
 return''
}

export function localizeQl7SemanticBadgeLabel(key='',locale='en',{seconds=0,fallback=''}={}){
 const normalized=normalizeQl7SemanticBadgeKey(key)
 const language=localeKey(locale)
 const label=SPECIAL_LABELS[language]?.[normalized]||str(fallback)||QL7_SEMANTIC_BADGES[normalized]?.label||''
 const timer=['stop','blocked','cooldown'].includes(normalized)?numeric(seconds):0
 return timer?`${label} ${formatTimer(timer)}`:label
}

export function localizeQl7SupportBadgeRailLabel(badge={},locale='en'){
 const raw=str(badge?.label||badge?.value)
 const key=inferRailBadgeKey(badge)
 if(!SPECIAL_KEYS.has(key))return raw
 return localizeQl7SemanticBadgeLabel(key,locale,{seconds:badgeSeconds(badge),fallback:raw})
}

export function resolveQl7SemanticBadge({semanticIcon='',purpose='',status='',topic='',emotion='',locale='en',seconds=0}={}){
 let key=normalizeQl7SemanticBadgeKey(semanticIcon)
 if(!QL7_SEMANTIC_BADGES[key]){
 ...
 const base=QL7_SEMANTIC_BADGES[key]
 return Object.freeze({key,...base,label:localizeQl7SemanticBadgeLabel(key,locale,{seconds,fallback:base.label})})
}
export function listQl7SemanticBadgeKeys(){return Object.freeze(Object.keys(QL7_SEMANTIC_BADGES))}

export function resolveQl7SemanticBadgeWithEntityBudget(input={}, {cardTitle=''}={}){const b=resolveQl7SemanticBadge(input);const n=v=>String(v||'').normalize('NFKC').toLowerCase().replace(/[^\p{L}\p{N}]+/gu,' ').trim();return Object.freeze({...b,label:n(cardTitle)&&n(b.label)===n(cardTitle)?'':b.label,entityMentionSuppressed:Boolean(n(cardTitle)&&n(b.label)===n(cardTitle))})}
