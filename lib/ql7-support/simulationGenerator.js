...
import {
  QL7_SUPPORT_SIMULATION_AXES,
  QL7_SUPPORT_SIMULATION_LANGUAGES,
  QL7_SUPPORT_SIMULATION_TOPICS,
  getQl7SupportSimulationSeeds,
  getQl7SupportSimulationTopicLabel,
} from './simulationOntology.js'
import {QL7_SUPPORT_ECOSYSTEM_TOPICS} from './ecosystemCatalog.js'
import {countQl7SupportGraphemes, trimQl7SupportGraphemes} from './limits.js'
...
function repeatToLength(text, target, locale) { let out = str(text) || 'help'; while (countQl7SupportGraphemes(out, locale) < target) out += ` ${text || 'help'}`; return trimQl7SupportGraphemes(out, target, locale) }
...
function replaceTopic(value, topic) { return str(value).replaceAll('{topic}', getQl7SupportSimulationTopicLabel(topic)) }

const COMPACT_TOPIC_ANCHORS = Object.freeze({
...
const SIMULATION_TOPIC_SIGNAL_PATTERNS = Object.freeze([
...
const SIMULATION_PARTNERSHIP_SIGNAL = /business\s+partnership|commercial\s+partnership|partnership\s+concerning|asociaci[oó]n\s+comercial|iş\s+ortaklığı|делов\p{L}*\s+партн[её]р|ділов\p{L}*\s+партнер|商业合作|合作|شراكة|שותפות/iu
...
function compactTopicAnchor(topic = '') {
  return COMPACT_TOPIC_ANCHORS[str(topic)] || str(topic).replaceAll('_', ' ') || 'QL7 Support'
}
...
const SIMULATION_KEYBOARD_REPAIR = Object.freeze({ 'й': 'q', 'ц': 'w', 'у': 'e', 'к': 'r', 'е': 't', 'н': 'y' })
function repairSimulationKeyboardLayout(value = '') {
  return str(value).replace(/[\p{L}\p{N}_-]+/gu, (token) => {
    ...
    return token.replace(/[йцукен]/giu, (char) => {
      const replacement = SIMULATION_KEYBOARD_REPAIR[char.toLowerCase()]
      return char === char.toUpperCase() ? replacement?.toUpperCase() || char : replacement || char
    })
  })
}

function strongTopicSignals(text = '') {
  const repaired = repairSimulationKeyboardLayout(text)
  ...
  for (const [topic, anchor] of Object.entries(COMPACT_TOPIC_ANCHORS)) {
  ...
  for (const [topic, pattern] of SIMULATION_TOPIC_SIGNAL_PATTERNS) {
    if (sources.some((source) => pattern.test(source))) add(topic)
  }
  return Object.freeze(topics)
}

const SIMULATION_WEAK_TOPIC_SIGNAL_PATTERNS = Object.freeze([
...
function weakTopicSignals(text = '') {
  const repaired = repairSimulationKeyboardLayout(text)
  const sources = Array.from(new Set([str(text), str(repaired)].filter(Boolean)))
  const topics = []
  for (const [topic, pattern] of SIMULATION_WEAK_TOPIC_SIGNAL_PATTERNS) {
    if (sources.some((source) => pattern.test(source)) && !topics.includes(topic)) topics.push(topic)
  }
  return Object.freeze(topics)
}

function visibleTopicSignals(text = '') {
  return Object.freeze(Array.from(new Set([
    ...strongTopicSignals(text),
    ...weakTopicSignals(text),
  ])))
}

function oraclePrimaryTopic(oracle = {}, fallback = '') {
...
function ambiguityOracle({ visibleTopics = [], previousTopic = '', previousTopics = [], requestedLength = 600, reason = 'topic_anchor_not_recoverable' } = {}) {
...
function buildTextualOracle({
  ...
  const strongTopics = strongTopicSignals(text)
  const visibleTopics = Object.freeze(Array.from(new Set([...strongTopics, ...weakTopicSignals(text)])))
  const primaryAnchorRecovered = strongTopics.includes(topic)
  const partnership = SIMULATION_PARTNERSHIP_SIGNAL.test(text)
...
function decorateProfile(text, axes, key) {
  ...
  if (axes.humor === 'playful_complaint') out = `My ${getQl7SupportSimulationTopicLabel(axes.topic)} seems to have gone on vacation. ${out}`
...
function axisProjection(key, config) {
  const axis = {}
  for (const [name, values] of Object.entries(QL7_SUPPORT_SIMULATION_AXES)) axis[name] = pick(values, `${key}:axis:${name}`)
...
function oracleContextTopics(oracle = {}, fallback = '') {
  return Object.freeze(Array.from(new Set([
...
function buildConversationTurns({ key, config, initialTopic, locale, initialText, initialOracle, turnCount, transition, axes }) {
  ...
  let previousSemanticTopic = oraclePrimaryTopic(initialOracle, initialTopic)
  let previousTopics = oracleContextTopics(initialOracle, previousSemanticTopic || initialTopic)
  ...
  for (let turnIndex = 0; turnIndex < turnCount; turnIndex += 1) {
    ...
    if (turnIndex > 0) {
      if (transition === 'abrupt_switch') {
        ...
        text = pick(getQl7SupportSimulationSeeds(alternate, locale, axes.intent), `${key}:turn:${turnIndex}:switch`)
      } else if (transition === 'return') {
        expectedTopic = turnIndex % 2 === 1 ? alternate : initialTopic
        text = turnIndex % 2 === 1
          ? pick(getQl7SupportSimulationSeeds(alternate, locale, axes.intent), `${key}:turn:${turnIndex}:away`)
    ...
    text = trimQl7SupportGraphemes(text, config.maxLength, locale)
    ...
    turns.push(Object.freeze({
      ...
      locale,
      input: text,
      inputGraphemes: countQl7SupportGraphemes(text, locale),
      oracle,
    }))
    previousOracle = oracle
    previousSemanticTopic = oraclePrimaryTopic(oracle, previousSemanticTopic || expectedTopic)
    previousTopics = oracleContextTopics(oracle, previousSemanticTopic || expectedTopic)
  }
  return Object.freeze(turns)
}

export function normalizeQl7SupportSimulationConfig(input = {}) {
  ...
  const selectedTopics = (Array.isArray(input.topics) && input.topics.length ? input.topics : QL7_SUPPORT_ECOSYSTEM_TOPICS).filter((value) => QL7_SUPPORT_SIMULATION_TOPICS.includes(value))
  const selectedLanguages = (Array.isArray(input.languages) && input.languages.length ? input.languages : (mode === 'quick' ? ['en','ru','uk','es','tr','ar','zh','he'] : QL7_SUPPORT_SIMULATION_LANGUAGES)).filter((value) => QL7_SUPPORT_SIMULATION_LANGUAGES.includes(value))
  ...
  return Object.freeze({
    mode,
    seed: str(input.seed) || 'ql7-baseline',
...
export function buildQl7SupportSimulationScenario(index = 0, rawConfig = {}) {
  const config = normalizeQl7SupportSimulationConfig(rawConfig)
  const key = `${config.seed}:${index}`
  const axes = axisProjection(key, config)
  const topic = axes.topic
  const locale = axes.language
  const seedText = pick(getQl7SupportSimulationSeeds(topic, locale, axes.intent), `${key}:text`)
  ...
  if (length > countQl7SupportGraphemes(text, locale)) text = repeatToLength(text, length, locale)
  else text = trimQl7SupportGraphemes(text, length, locale)
  ...
  const scenarioId = `ql7:${crypto.createHash('sha256').update(key).digest('hex').slice(0, 32)}`
  return Object.freeze({
    scenarioId,
    index,
    seed: config.seed,
    topic,
    topicLabel: getQl7SupportSimulationTopicLabel(topic),
    locale,
    intent: axes.intent,
...
export function *generateQl7SupportSimulationScenarios(rawConfig = {}) {
  const config = normalizeQl7SupportSimulationConfig(rawConfig)
  for (let index = 0; index < config.scenarioCount; index += 1) {
    if (index % config.shardCount !== config.shardIndex) continue
    yield buildQl7SupportSimulationScenario(index, config)
  }
}
