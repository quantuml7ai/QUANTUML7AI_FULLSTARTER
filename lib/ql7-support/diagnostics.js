...
import {
  QL7_SUPPORT_ADS_READ_COLLECTIONS_V9,
  readQl7SupportAdsDiagnosticV9,
} from './adsSupportReadAdapterV9.js'
...
export async function runQl7SupportAdsDiagnostic({
  ...
  try {
    const v9 = await readQl7SupportAdsDiagnosticV9({
      database,
      userId: uid,
      aliases,
      analysis,
    ...
    const doc = await recordDiagnosticRun(database, {
      ...
      userId: uid,
      caseId,
      subjectId: adsProbeIds(analysis)[0] || str(v9.campaigns?.[0]?.campaignId || v9.campaigns?.[0]?.id || v9.packages?.[0]?.id || v9.packages?.[0]?.packageId),
      now,
      businessCollectionsRead: BUSINESS_ADS_COLLECTIONS,
