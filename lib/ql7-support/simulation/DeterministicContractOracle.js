...
const tables=(surface={})=>arr(surface.tables).length?arr(surface.tables):(surface.table?[surface.table]:[])
...
if(expected.tableSchema&&!tables(surface).some((t)=>t?.schema===expected.tableSchema))failures.push({code:'required_table_missing',expected:expected.tableSchema,severity:'high'});
...
const rows=tables(result.surface).flatMap((t)=>t?.rows||[]);
if(fact?.topic==='vip'&&fact.verified){const status=String(rows.find((r)=>r?.key==='status')?.value||'').toLowerCase();
const expectedStatus=String(localizeQl7EvidenceValue(fact.status,result.surface?.locale||result.localePolicy?.locale||'en')||fact.status||'').toLowerCase();
if(status&&status!==expectedStatus)failures.push({code:'verified_fact_text_table_mismatch',severity:'critical'})}if(result.safety?.category==='insult_uncertain'&&(!result.composerPolicy?.allowed||Number(result.safety?.cooldownMs||0)>0))failures.push({code:'uncertain_insult_wrong_cooldown',severity:'critical'});
