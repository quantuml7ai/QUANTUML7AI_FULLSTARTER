...
export async function readQl7LiveUserSnapshot({
  ...
  const identityModule = services.identityGraph || await import('../identityGraph.js')
  const vipDiagnostic = services.vipDiagnostic || (await import('../vipResolver.js')).runQl7SupportVipDiagnostic
  const adsDiagnostic = services.adsDiagnostic || (await import('../adsSupportReadAdapter.js')).readQl7SupportAdsDiagnostic
  const ratingCalculator = services.ratingCalculator || (await import('../ecosystemRating.js')).calculateQl7EcosystemRating
  const buildIdentity = identityModule.buildQl7IdentityGraph || identityModule.build
  const publicIdentity = identityModule.publicQl7IdentityGraphProjection || identityModule.publicProjection
  ...
  const receiptMap = {
    ...
    ads_packages: receipt('ads_packages', adsPackageResult, ads?.status === 'unavailable' ? 'unavailable' : Number(adsPackageResult.activeCampaignCount) === 0 ? 'verified_empty' : 'verified', 'ads_read_adapter'),
    ads_campaigns: receipt('ads_campaigns', { campaigns }, ads?.status === 'unavailable' ? 'unavailable' : campaigns.length ? 'verified' : 'verified_empty', 'ads_read_adapter'),
    forum: receipt('forum', {
      ...activity,
  ...
  for (let index = 0; index < questions.length; index += 1) {
    ...
    const productionTurn = await executeQl7SupportProductionTurn({
      mode: 'replay',
      ...
      verifiedActorId: canonicalAccountId,
      actor: {
        valid: true,
        canonicalAccountId,
        accountIdMasked: mask(canonicalAccountId),
        locale,
        authMode: 'laboratory_verified_identity',
        actorReceiptId: `actor-receipt:live-read:${hash({ canonicalAccountId, index }).slice(0, 24)}`,
      },
    ...
    surfaces.push({
      ...
      surface: productionDelivery.surface,
      ...
      analysis: {
        topic: result.analysis?.topic || result.analysis?.primaryTopic || '',
        messageAct: result.analysis?.messageAct || '',
        subIntent: result.analysis?.subIntent || '',
        mode: result.analysis?.mode || '',
      },
      semanticRole: result.contentPlan?.semanticRole || productionDelivery.surface?.semanticRole || '',
  ...
  return Object.freeze({
    schema: 'ql7.support.live-user-snapshot',
    readOnly: true,
    writeCount: 0,
