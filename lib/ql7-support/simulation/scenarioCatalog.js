...
import { QL7_SUPPORT_PROVIDER_SAFETY_LEXICON } from '../language/safetyLexicon.provider.js'
import { getQl7SupportCanonicalDomain } from '../knowledge/domainRegistry.js'
import { buildQl7SupportKnowledge32ScenarioSeed } from './corpora/knowledge32V14.js'
...
const QL7_SUPPORT_ALL_SAFETY_LEXICONS = Object.freeze({ ...QL7_SUPPORT_NATIVE_SAFETY_LEXICON, ...QL7_SUPPORT_PROVIDER_SAFETY_LEXICON })
...
const CRITICAL = Object.freeze([
 ...
 {id:'insult_first',text:{en:'You are an idiot',ru:'Ты идиот',uk:'Ти ідіот',es:'Eres idiota',tr:'Sen aptalsın',ar:'أنت غبي',zh:'你是白痴',he:'אתה אידיוט'},priorLedger:{safety:{directInsultCount:0}},expected:{surfaceKind:'safety',cooldownMs:0,allowed:true}},
 {id:'insult_second',text:{en:'You are an idiot',ru:'Ты идиот',uk:'Ти ідіот',es:'Eres idiota',tr:'Sen aptalsın',ar:'أنت غبي',zh:'你是白痴',he:'אתה אידיוט'},priorLedger:{safety:{directInsultCount:1}},expected:{surfaceKind:'safety',cooldownMs:60000,allowed:false}},
...
function criticalScenario(index, locale, seed){const base=CRITICAL[index%CRITICAL.length];const providerLocale=QL7_SUPPORT_PROVIDER_LOCALES.includes(locale);return {...base,id:`${base.id}:${locale}:${index}`,locale,input:base.text[locale]||base.text.en,providerEquivalent:providerLocale,seed:`${seed}:${index}`,expected:{...base.expected,selectedLocale:locale,providerEquivalent:providerLocale}}}
...
function businessScenario(index,locale,seed){
 const rows=[
  {input:'Бро, есть бизнес предложение по партнерству Quantum L7 AI, хочу обсудить интеграцию бренда, мой email partner@example.invalid и tg @partner_l7_ai',priorLedger:{activeTopic:'partnership',business:{intakeTurns:2,operatorRequestTurns:0,contactPrompted:true}},expected:{topic:'partnership',messageAct:'business_proposal',operatorCase:true}},
  {input:'Хочу живого оператора по партнерству, у нас коммерческое предложение и бюджет на пилот, контакт @quantum_partner_demo',priorLedger:{activeTopic:'partnership',business:{intakeTurns:2,operatorRequestTurns:1,contactPrompted:true}},expected:{topic:'partnership',messageAct:'human_operator_request',operatorCase:true}},
  {input:'Оператор пожалуйста, без контактов, пишите мне только в DM Quantum Messenger',priorLedger:{activeTopic:'contact',business:{intakeTurns:2,operatorRequestTurns:1,contactPrompted:true}},expected:{topic:'contact',messageAct:'human_operator_request',operatorCase:true}},
  {input:'Инвест идея: хотим вложиться в развитие Quantum L7 AI, интересуют условия, сроки и формат участия',priorLedger:{activeTopic:'investment',business:{intakeTurns:1,operatorRequestTurns:0,contactPrompted:false}},expected:{topic:'investment',messageAct:'business_proposal',surfaceKind:'structured'}},
  {input:'мой контакт investor@example.invalid, можно передать оператору, тема инвестиции и стратегическое сотрудничество',priorLedger:{activeTopic:'investment',business:{intakeTurns:2,operatorRequestTurns:1,contactPrompted:true}},expected:{topic:'investment',operatorCase:true}},
 ]
 const row=rows[index%rows.length]
 return {id:`business:${locale}:${index}`,locale,input:row.input,seed:`${seed}:${index}`,priorLedger:row.priorLedger,profile:{nickname:`Business Simulation ${index}`,locale,stats:{posts:17,topics:3,likes:93,reportsOnPosts:index%3}},actor:{accountId:`business:${index}`,accountIdMasked:`business…${index}`,locale},expected:row.expected}
}
function socialBoundaryScenario(index,locale,seed){
 ...
 return {id:`social-boundary:${locale}:${index}`,locale,input:rows[index%rows.length],seed:`${seed}:${index}`,priorLedger:{activeTopic:'support_system',social:{supportiveTurns:3},topicBranches:[{topic:'forum',at:'2026-07-30T12:00:00.000Z'}]},expected:{messageAct:'emotional_support',surfaceKind:'choices',choice:true}}
}
function conversationScenario(index,locale,seed){
 const rows=[
  ...
  {input:'you are stupid support, but I still need help',priorLedger:{safety:{directInsultCount:index%2}},expected:{surfaceKind:'safety'}},
  {input:'I want to cooperate with Quantum L7 AI, maybe partnership or investment, please guide me',priorLedger:{activeTopic:'partnership',business:{intakeTurns:1}},expected:{topic:'investment',surfaceKind:'structured'}},
 ...
 const row=rows[index%rows.length]
 return {id:`conversation:${locale}:${index}`,locale,input:row.input,seed:`${seed}:${index}`,priorLedger:row.priorLedger||{},expected:{...row.expected,selectedLocale:locale}}
}
function cryptoAiScenario(index,locale,seed){
 const rows=[
  {input:'Покажи актуальную цену BTC и AI рекомендацию на 5m',fixtureState:'verified',expected:{topic:'exchange_ai',messageAct:'ai_recommendation_request',requiredAdapter:'exchange_ai',tableSchema:'ql7.table.ai.recommendation',surfaceKind:'structured'}},
  {input:'what is eth price and ai box recommendation 1h',fixtureState:'verified',expected:{topic:'exchange_ai',messageAct:'ai_recommendation_request',requiredAdapter:'exchange_ai',tableSchema:'ql7.table.ai.recommendation'}},
  {input:'прогноз sol на 15m, квота вроде есть',fixtureState:'verified',expected:{topic:'exchange_ai',messageAct:'ai_recommendation_request',requiredAdapter:'exchange_ai',tableSchema:'ql7.table.ai.recommendation'}},
  {input:'AI quota закончилась, хочу прогноз bitcoin',fixtureState:'quota_exhausted',expected:{topic:'exchange_ai',messageAct:'ai_recommendation_request',requiredAdapter:'exchange_ai',tableSchema:'ql7.table.ai.recommendation',actionRouteId:'vip'}},
 ]
 const row=rows[index%rows.length]
...
function v11BoundaryScenario(index,locale,seed,kind='insult-direct'){
 ...
 if(kind==='insult-denial-recovery'){
  ...
  return {id:`${kind}:${locale}:${index}`,locale,input,seed:`${seed}:${index}`,priorLedger:{activeTopic:'qcoin',activeGoal:'check_balance',lastMaterialTurnId:'user:qcoin:before-boundary',safety:{confirmedDirectInsultCount:0,directInsultCount:0,pendingBoundaryClarification:{active:true,resumeTopic:'qcoin',resumeGoal:'check_balance',lastMaterialTurnId:'user:qcoin:before-boundary'}}},expected:{messageAct:'boundary_denial',surfaceKind:'compact',cooldownMs:0,allowed:true,selectedLocale:locale}}
 }
 if(kind==='insult-continuation'){
  ...
  return {id:`${kind}:${locale}:${index}`,locale,input:`${target}${glue}${insult}`,seed:`${seed}:${index}`,priorLedger:{activeTopic:'qcoin',activeGoal:'check_balance',lastMaterialTurnId:'user:qcoin:before-boundary',safety:{confirmedDirectInsultCount:0,directInsultCount:0,pendingBoundaryClarification:{active:true,resumeTopic:'qcoin',resumeGoal:'check_balance',lastMaterialTurnId:'user:qcoin:before-boundary'}}},expected:{surfaceKind:'safety',cooldownMs:0,allowed:true,selectedLocale:locale}}
 }
...
export function buildQl7SupportScenario(index,{profile='mixed',seed='ql7-v14',locale='',mutationFamily=''}={}){
...
export function ql7SupportCorpusHash(profile='mixed'){return ql7StableHash(JSON.stringify({profile,topics:QL7_SUPPORT_ECOSYSTEM_TOPICS,native:QL7_SUPPORT_NATIVE_LOCALES,provider:QL7_SUPPORT_PROVIDER_LOCALES,mutations:QL7_SUPPORT_MUTATION_FAMILIES,critical:CRITICAL.map(x=>x.id)}))}
