...
const LOCAL_ENVIRONMENT_DIRS=new Set(['node_modules','.next','.git','report','reports','public','.ql7-patch-backups','.agents','.codex_tmp','.pytest_cache','.venv-ql7','.vscode','coverage'])
function walk(root){const out=[];function rec(dir){for(const ent of fs.readdirSync(dir,{withFileTypes:true})){const full=path.join(dir,ent.name),rel=norm(path.relative(root,full));if(LOCAL_ENVIRONMENT_DIRS.has(ent.name))continue;if(rel==='report/QL7_SUPPORT_PATCH'||rel.startsWith('report/QL7_SUPPORT_PATCH/'))continue;if(/^QL7_SUPPORT_NATIVE_NEURAL_INTELLIGENCE_MASTER_SPEC_RU_FINAL(?:\(\d+\))?\.md$/u.test(ent.name))continue;if(ent.name.startsWith('.env'))continue;if(ent.isDirectory())rec(full);else if(EXT.has(path.extname(ent.name)))out.push(rel)}}rec(root);return out.sort()}
...
const IMPORT_RE=/(?:\b(?:import|export)\s*[^'"]*?\bfrom\s*|\bimport\s*(?:\(\s*)?|\brequire\s*\()\s*['"]([^'"]+)['"]/gu
...
const LOCAL_CALIBRATION_SUPPORT_FILES=new Set([
 'lib/ql7-support/knowledge/retrieval/neuralReranker.js',
 'lib/ql7-support/learningGovernance.js',
 'lib/ql7-support/neural/criticContract.js',
 'lib/ql7-support/neural/generationContract.js',
 'lib/ql7-support/neural/nativeCriticAdapter.js',
 'lib/ql7-support/neural/nativeGenerationAdapter.js'
])
const FACADE=new Set()
const TARGET=new Set(['app/api/forum/mutate/route.js','app/api/dm/send/route.js','app/api/battlecoin/chat/messages/route.js','app/api/academy/exam/route.js','app/api/quest/progress/route.js','app/api/referral/hit/route.js','app/api/qcoin/drop/route.js','app/api/qcoin/heartbeat/route.js','app/api/qcoin/topup/webhook/route.js','app/api/pay/webhook/route.js','lib/qcoinEntitlementPurchase.js','lib/mongo/qcoin-primary.cjs','lib/mongo/subscriptions-primary.cjs','lib/mongo/metamarket-primary.cjs','lib/mongo/battlecoin-primary.cjs','lib/adsCore.js','lib/subscriptions.js','components/Ql7SupportRuntimeBridge.jsx'])
const PROD_ROOTS=[
