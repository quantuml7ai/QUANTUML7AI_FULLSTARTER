...
import { normalizeQl7HumanInputV11_6 } from './inputNormalizationV11_6.js'
...
export const QL7_PREMIUM_MICRO_INTENT_COUNT_V11_6=DOMAINS.length*OPERATIONS.length
export const QL7_PREMIUM_MICRO_INTENTS_V11_6=Object.freeze(DOMAINS.flatMap(([domain,topic])=>OPERATIONS.map(([goal,operation],index)=>Object.freeze({id:`${domain}.${goal}`,domain,topic,goal,operation,ordinal:index+1}))))
...
export function classifyQl7PremiumMicroIntentV11_6(text='', { previousTopic='' }={}) {
  const prepared=normalizeQl7HumanInputV11_6(text)
  ...
  const row=QL7_PREMIUM_MICRO_INTENTS_V11_6.find(x=>x.domain===domain&&x.goal===goal) || QL7_PREMIUM_MICRO_INTENTS_V11_6.find(x=>x.domain===domain)
...
export function getQl7PremiumMicroIntentCatalogStatsV11_6(){return Object.freeze({domains:DOMAINS.length,operations:OPERATIONS.length,microIntents:QL7_PREMIUM_MICRO_INTENTS_V11_6.length})}
export function listQl7PremiumDomainsV11_6(){return Object.freeze(DOMAINS.map(([domain,topic])=>({domain,topic})))}
