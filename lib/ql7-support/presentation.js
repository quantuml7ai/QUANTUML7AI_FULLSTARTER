export const QL7_SUPPORT_CORE_LOCALES = Object.freeze(['en','ru','uk','es','tr','ar','zh'])
export const QL7_SUPPORT_PURPOSES = Object.freeze([
...
export const QL7_SUPPORT_THEME_BY_PURPOSE = Object.freeze({
...
function hasRenderableEvidence(card={},metrics=[]){
  if(safeArray(metrics).some((metric)=>hasMeaningfulEvidence(metric?.value,metric?.key||metric?.label)))return true
  if(safeArray(card?.table?.rows).some((row)=>hasMeaningfulEvidence(row,'table')))return true
  if(safeArray(card?.facts).some((item)=>hasMeaningfulEvidence(item,'facts')))return true
  if(safeArray(card?.checks).some((item)=>hasMeaningfulEvidence(item,'checks')))return true
  return false
}

function effectiveStatusCode(statusCode='',card={},metrics=[]){
  const code=s(statusCode)
  return UNAVAILABLE_STATUS_RE.test(code)&&hasRenderableEvidence(card,metrics)?'healthy':code
}

export function isQl7SupportCoreLocale(value=''){
  const key=s(value).toLowerCase().split(/[-_]/u)[0]
  return QL7_SUPPORT_CORE_LOCALES.includes(key)
}

export function ql7SupportRenderLocale(value=''){
  const key=s(value).toLowerCase().replace(/_/gu,'-')
  return /^[a-z]{2,3}(?:-[a-z0-9]{2,8})*$/u.test(key)?key:'en'
}

export function ql7SupportDirection(value=''){
  const key=ql7SupportRenderLocale(value).split('-')[0]
  return ['ar','he','fa','ur'].includes(key)?'rtl':'ltr'
}

export function ql7SupportLocale(value=''){
  const key=s(value).toLowerCase().split(/[-_]/u)[0]
  return QL7_SUPPORT_CORE_LOCALES.includes(key)?key:'en'
}

export function ql7SupportLabels(locale='en'){
  const key=ql7SupportLocale(locale)
  return {...(L[key]||L.en),...(S[key]||S.en)}
}

export function localizeQl7Status(code='',locale='en'){
  const key=machineKey(code)
  const labels=ql7SupportLabels(locale)
  return labels[key]||labels.unavailable||'Status unavailable'
}

export function localizeQl7PresentationMetric(key='',locale='en'){
...
export function localizeQl7ContentType(value='',locale='en'){
  const key=machineKey(value)
  const dictionaryLocale=ql7SupportLocale(locale)
  return CONTENT_TYPE_LABELS[dictionaryLocale]?.[key]||CONTENT_TYPE_LABELS.en[key]||s(value)||'—'
}

export function inferQl7Purpose(input={}){
  const kind=s(input.purpose||input.kind)
  if(QL7_SUPPORT_PURPOSES.includes(kind))return kind
...
export function semanticDedupe(values=[]){
  const seen=new Set()
...
function normalizeSections(card,labels){
  const normalizeItems=(items,id)=>semanticDedupe(items).map((item,index)=>{
...
function normalizeMetricObject(metrics,locale){
  const rows=[]
  if(Array.isArray(metrics)){
    for(const item of metrics){
      const key=s(item?.key||item?.label)
      if(!key||!hasValue(item?.value))continue
      rows.push({key,label:s(item?.label)||localizeQl7PresentationMetric(key,locale),value:item.value,format:s(item?.format)||inferFormat(item.value),tone:s(item?.tone)||'neutral',visibility:s(item?.visibility)||'both'})
    }
  }else if(metrics&&typeof metrics==='object'){
    for(const [key,value] of Object.entries(metrics)){
      if(!hasValue(value))continue
      rows.push({key,label:localizeQl7PresentationMetric(key,locale),value,format:inferFormat(value),tone:'neutral',visibility:'both'})
    }
  }
  return rows
}

function normalizeLegacyTableMetrics(table,locale){
  ...
  safeArray(table.rows).forEach((row,rowIndex)=>{
    for(const column of columns){
      ...
      rows.push({
        ...
        label:s(column.label)||localizeQl7PresentationMetric(key,locale),
        value,
...
function normalizeSnapshot(snapshot,locale){
  ...
  return{
    ...snapshot,
    contentType:hasValue(snapshot.contentType)?localizeQl7ContentType(snapshot.contentType,locale):snapshot.contentType,
  }
}

export function adaptQl7SupportCardForRender(card={}){
  if(Number(card?.version)===3||card?.schema==='ql7.support.card')return card
  const renderLocale=ql7SupportRenderLocale(card?.locale)
  const dictionaryLocale=ql7SupportLocale(card?.locale)
  const purpose=inferQl7Purpose(card)
  const labels=ql7SupportLabels(dictionaryLocale)
  const metrics=dedupeMetrics([
    ...normalizeMetricObject(card?.metrics,dictionaryLocale),
    ...normalizeLegacyTableMetrics(card?.table,dictionaryLocale),
  ])
  const statusCode=effectiveStatusCode(s(card?.status?.code||card?.status||card?.branch),card,metrics)
  return{
    ...card,
    version:3,
    schema:'ql7.support.card',
    purpose,
    visualTheme:s(card?.visualTheme)||QL7_SUPPORT_THEME_BY_PURPOSE[purpose]||'knowledge-blue',
    severity:s(card?.severity||'info'),
    locale:renderLocale,
    direction:ql7SupportDirection(renderLocale),
    status:statusCode?{
      code:statusCode,
      label:localizeQl7Status(statusCode,dictionaryLocale),
      tone:/unavailable|error|inconsistent|expired|blocked/iu.test(statusCode)?'warning':'neutral',
      icon:'status',
    }:null,
    sections:normalizeSections(card,labels),
    metrics,
    timeline:normalizeTimeline(card?.timeline),
    snapshot:normalizeSnapshot(card?.snapshot,dictionaryLocale),
