...
export const QL7_SUPPORT_SAFE_LEARNING_CALIBRATION_VERSION = '12.0.0'

export const QL7_SUPPORT_SAFE_LEARNING_POLICY = Object.freeze({
...
export function normalizeQl7SupportLearningSample(sample = {}, index = 0) {
  ...
  return Object.freeze({
    ...
    poisoningRisk,
    quarantined: poisoningRisk >= QL7_SUPPORT_SAFE_LEARNING_POLICY.maxPoisoningRiskPerUsableSample || sample.quarantined === true,
    consent: sample.consent === true,
  })
}

export function summarizeQl7SupportLearningEvidence(samples = []) {
  const normalized = safeArray(samples).map((sample, index) => normalizeQl7SupportLearningSample(sample, index))
...
export function evaluateQl7SupportSafeLearningGate({
  samples = [],
  candidateMetrics = {},
  completedStages = [],
  candidate = {},
  policy = QL7_SUPPORT_SAFE_LEARNING_POLICY,
} = {}) {
  const evidence = summarizeQl7SupportLearningEvidence(samples)
  ...
  return Object.freeze({
    version: QL7_SUPPORT_SAFE_LEARNING_CALIBRATION_VERSION,
...
export function getQl7SupportSafeLearningCalibrationStats() {
  return Object.freeze({
    version: QL7_SUPPORT_SAFE_LEARNING_CALIBRATION_VERSION,
    policy: QL7_SUPPORT_SAFE_LEARNING_POLICY,
...
export function buildQl7SupportSafeLearningAnswer({ locale = 'en', seed = '' } = {}) {
  ...
  return Object.freeze({
    ...
    cardSpec: Object.freeze({
      kind: 'canonical_learning_response',
      purpose: 'explanation',
      locale: lang,
