...
export const QL7_SUPPORT_FACT_PROJECTION_VERSION = '16.0.0'
...
export function selectQl7SupportFactReceipt(topic = '', rows = []) {
  ...
  return eligible.find((row)=>String(row?.topicId||row?.domainId||row?.result?.topicId||'')===cleanTopic) || eligible.find((row)=>Array.isArray(row?.capabilities)&&row.capabilities.includes(cleanTopic)) || eligible.find((row) => !pattern || pattern.test(receiptText(row))) || null
}
...
export function buildQl7SupportFactProjection({ topic = '', receipts = [], receipt = null } = {}) {
  ...
  const facts = Object.freeze({
    ...
    campaigns: Array.isArray(data.campaigns)
      ? data.campaigns
      : Array.isArray(data.campaignRows)
        ? data.campaignRows
        : [],
    amount: data.amount,
