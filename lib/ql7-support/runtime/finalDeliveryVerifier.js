...
function visibleSurfaceStrings(surface = {}) {
  ...
  for (const table of ql7Arr(surface.tables)) {
    ...
    for (const row of ql7Arr(table?.rows)) {
      rows.push(row?.label, row?.displayValue ?? row?.value, row?.explanation)
    }
  }
  // `table` is the compatibility projection of `tables[0]`. It is not a
  // second visual surface, so scanning both copies creates false duplicate
  // language hits and does not describe what the user actually sees.
  if (surface.table && !ql7Arr(surface.tables).length) {
