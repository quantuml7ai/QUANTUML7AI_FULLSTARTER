...
import {finalizeQl7SupportDeterministicUnderstanding} from '../semantics/deterministicUnderstandingCoordinator.js'
import crypto from 'node:crypto'

export const QL7_SUPPORT_PRODUCTION_TURN_VERSION = '18.0.0'
...
export async function resolveQl7SupportProductionUnderstanding(input = {}) {
  ...
  if (input.eventEnvelope || input.entryEvent || analysis.deterministicUnderstandingReceipt?.receiptId) return input
  ...
  return {
    ...
    authoritativeAnalysis: true,
    baseAnalysisTrust: true,
    canonicalUnderstandingReceiptId: canonical.deterministicUnderstandingReceipt?.receiptId || '',
  }
}
...
export async function executeQl7SupportProductionTurn(input = {}, adapters = {}) {
  ...
  return Object.freeze({
    ...
    composerPolicy: runtime.composerPolicy,
    deterministicUnderstandingReceipt: runtime.analysis?.deterministicUnderstandingReceipt || null,
    // Compatibility only: historical consumers may still read this field name.
    neuralUnderstandingReceipt: runtime.analysis?.deterministicUnderstandingReceipt || null,
    delivery,
  })
}
