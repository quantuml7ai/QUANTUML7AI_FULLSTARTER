...
export const QL7_SUPPORT_SEMANTIC_BATTLE_EXPANSION_VERSION = '12.0.0'
...
export const QL7_SUPPORT_BATTLE_INPUT_MODES = Object.freeze([
  'synonym_chain',
  'typo_noise',
  'slang_voice',
  'regional_dialect',
...
export function getQl7SupportBattleExpansionStats() {
  const coveredTopics = QL7_SUPPORT_SIMULATION_TOPICS.filter((topic) => Array.isArray(TOPIC_SYNONYMS[topic]) && TOPIC_SYNONYMS[topic].length >= 4)
  ...
  return Object.freeze({
    version: QL7_SUPPORT_SEMANTIC_BATTLE_EXPANSION_VERSION,
    topicCount: QL7_SUPPORT_SIMULATION_TOPICS.length,
    coveredTopicCount: coveredTopics.length,
    languageCount: QL7_SUPPORT_SIMULATION_LANGUAGES.length,
    inputModeCount: QL7_SUPPORT_BATTLE_INPUT_MODES.length,
    synonymTerms,
    styleTerms,
    semanticCombinationFloor: coveredTopics.length * QL7_SUPPORT_SIMULATION_LANGUAGES.length * QL7_SUPPORT_BATTLE_INPUT_MODES.length,
    readyForBattlePreflight: coveredTopics.length === QL7_SUPPORT_SIMULATION_TOPICS.length && synonymTerms >= 250,
  })
}

export function buildQl7SupportBattleSemanticInput({ text = '', topic = '', locale = 'en', pairIndex = 0, scenarioId = '', turnIndex = 0 } = {}) {
  const cleanTopic = QL7_SUPPORT_SIMULATION_TOPICS.includes(str(topic)) ? str(topic) : 'support_system'
  const key = `${scenarioId}:${pairIndex}:${turnIndex}:${cleanTopic}:${locale}`
  const mode = pick(QL7_SUPPORT_BATTLE_INPUT_MODES, `${key}:mode`)
...
export function assertQl7SupportBattleExpansionReady() {
  const stats = getQl7SupportBattleExpansionStats()
  return Object.freeze({
    ok: stats.readyForBattlePreflight,
    stats,
    missingTopics: Object.freeze(QL7_SUPPORT_SIMULATION_TOPICS.filter((topic) => !TOPIC_SYNONYMS[topic] || TOPIC_SYNONYMS[topic].length < 4)),
  })
}
