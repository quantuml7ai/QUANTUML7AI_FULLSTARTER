...
export const QL7_SUPPORT_ROUTE_CALIBRATION_VERSION = '13.0.2'
...
export function calibrateQl7SupportRoute({ text = '', route = {}, analysis = {} } = {}) {
  const source = normalizeCalibrationText(text)
  const originalTopic = normalizeTopic(route.topic || analysis.topic || 'support_system')
  ...
  if (topic === originalTopic || explicitCorrection(source)) {
    ...
    if (catalogTopic && (explicitCorrection(source) || topic === 'support_system')) {
      topic = normalizeTopic(catalogTopic)
  ...
  return Object.freeze({
    ...
    v13Calibration: Object.freeze({ version: QL7_SUPPORT_ROUTE_CALIBRATION_VERSION, originalTopic, topic, changed: topic !== originalTopic, correction, reason }),
  })
}
