...
import { getQl7SupportEmailOutboxHealthV8, processQl7SupportEmailOutbox, reconcileQl7SupportEmailOutboxV8 } from './emailOutboxWorker.js'
...
export async function runQl7SupportScheduler(input = {}) {
  ...
  const emailReconciliation = input?.dryRun === false
    ? await reconcileQl7SupportEmailOutboxV8({ database })
    : { ok: true, skipped: true, reason: 'dry_run' }
  const emailOutbox = input?.dryRun === false
    ? await processQl7SupportEmailOutbox({
      database,
      workerId: input?.workerId || `scheduler:${process.pid}`,
      maxItems: input?.emailMaxItems || 25,
      maxAttempts: input?.emailMaxAttempts || 5,
    })
    : { ok: true, skipped: true, reason: 'dry_run' }
  const emailHealth = input?.dryRun === false
    ? await getQl7SupportEmailOutboxHealthV8({ database })
    : { ok: true, skipped: true, reason: 'dry_run' }
  return {
    ok: true,
