...
import {getQl7SupportEmailOutboxHealth, processQl7SupportEmailOutbox, reconcileQl7SupportEmailOutbox} from './emailOutboxWorker.js'
import {recoverQl7SupportDeliveryCommits} from './server.js'
import canonicalUserId from '../identity/canonical-user-id.cjs'

const DAY_MS = 24 * 60 * 60 * 1000
const DEFAULT_LIMIT = 250
const MAX_LIMIT = 1000
const SCHEDULER_INDEX_KEY = '__ql7SupportSchedulerIndexes'
...
export async function runQl7SupportVipScheduler({
  ...
  for (const row of rows) {
    const userId = canonicalUserId.normalizePrincipalSyntax(row?.accountId || row?.userId || row?.uid || '')
...
export async function runQl7SupportAdsScheduler({
  ...
  if (includeExpiry) {
    for (const pkg of packages) {
      const userId = canonicalUserId.normalizePrincipalSyntax(pkg?.accountId || pkg?.userId || pkg?.uid || '')
  ...
  if (includeWeeklyMetrics) {
    for (const campaign of campaigns) {
      const userId = canonicalUserId.normalizePrincipalSyntax(campaign?.accountId || campaign?.userId || campaign?.uid || '')
...
export const QL7_SUPPORT_SCHEDULER_JOB_IDS = Object.freeze(['vip','ads','email_reconciliation','email_outbox','email_health','delivery_recovery'])
function schedulerJobs(input={}){const requested=Array.isArray(input.jobs)?input.jobs.map(str).filter(Boolean):QL7_SUPPORT_SCHEDULER_JOB_IDS;const invalid=requested.filter(id=>!QL7_SUPPORT_SCHEDULER_JOB_IDS.includes(id));if(invalid.length)throw new Error(`ql7_support_scheduler_job_unregistered:${invalid.join(',')}`);return new Set(requested)}
export async function runQl7SupportScheduler(input = {}) {
  if (!isQl7SupportActive()) return { ok: true, skipped: true, reason: 'ql7_support_disabled', supportActive: false }
  const jobs=schedulerJobs(input), skipped={ok:true,skipped:true,reason:'job_not_requested'}
  const [vip, ads] = await Promise.all([
    jobs.has('vip')?runQl7SupportVipScheduler(input):skipped,
    jobs.has('ads')?runQl7SupportAdsScheduler(input):skipped,
  ])
  const database = input?.dryRun === false ? await db() : null
  const emailReconciliation = input?.dryRun === false && jobs.has('email_reconciliation')
    ? await reconcileQl7SupportEmailOutbox({ database }) : skipped
  const emailOutbox = input?.dryRun === false && jobs.has('email_outbox')
    ? await processQl7SupportEmailOutbox({database,workerId: input?.workerId || `scheduler:${process.pid}`,maxItems: input?.emailMaxItems || 25,maxAttempts: input?.emailMaxAttempts || 5}) : skipped
  const emailHealth = input?.dryRun === false && jobs.has('email_health')
    ? await getQl7SupportEmailOutboxHealth({ database }) : skipped
  const deliveryRecovery = input?.dryRun === false && jobs.has('delivery_recovery')
    ? await recoverQl7SupportDeliveryCommits({database,workerId: input?.workerId || `scheduler:${process.pid}`,maxItems: input?.deliveryRecoveryMaxItems || 25}) : skipped
  return {
    ok: true,
