...
import {getQl7SupportSurfaceCopy} from '../language/supportSurfaceCopyRegistry.js'

export function buildQl7SupportBadges({ plan = {}, locale = 'en' } = {}) {
  const p = getQl7NativeCopy(locale).badges
  const surfaceCopy = getQl7SupportSurfaceCopy(locale)
  const out = []
  if (plan.resultKind === 'verified') out.push({ id: 'verified', label: surfaceCopy.status.verified, tone: 'success', icon: 'confirmed' })
