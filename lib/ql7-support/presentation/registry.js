...
export const QL7_SUPPORT_PRESENTATION_REGISTRY_VERSION = '13.0.0'
const canonical_TOPICS=new Set(['partnership','investment','accessibility','learning_governance'])
function normalizeTopic(value){const raw=ql7Str(value).toLowerCase().replace(/[\s-]+/g,'_');return canonical_TOPICS.has(raw)?raw:normalizeQl7SupportTopic(raw||'support_system')}
...
export function resolveQl7SupportPresentation({topic='',kind='',messageAct='',verified=false,unavailable=false,operator=false,severity='',tone={}}={}){
  const normalizedTopic=normalizeTopic(topic||'support_system')
  ...
  return Object.freeze({
    version:QL7_SUPPORT_PRESENTATION_REGISTRY_VERSION,
...
export function getQl7SupportSurfaceTitle({role='information',topic='support_system',locale='en'}={}){
  const normalizedTopic=normalizeTopic(topic||'support_system')
