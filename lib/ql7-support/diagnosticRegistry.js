...
import {runQl7SupportVipDiagnostic} from './vipResolver.js'
import {readQl7SupportAiBoxAnalysis} from './data/aiBoxSupportReadAdapter.js'
...
export async function runQl7SupportPremiumDiagnostic(input = {}) {
  ...
  if (topic === 'vip') {
    const result = await runQl7SupportVipDiagnostic(input)
    return { ...result, playbook: 'vip_entitlement_identity_graph', sourceContract: getQl7SupportSourceContract(topic), businessCollectionsWritten: [], readOnly: true }
  }
  if (topic === 'qcoin') {
  ...
  if (topic === 'exchange_ai') {
    const entitlement = await runRegisteredAdapter({ ...input, topic })
    if (entitlement?.status === 'unavailable') return { ok: false, ...entitlement, businessCollectionsWritten: [], readOnly: true }
    try {
      const aiBoxReceipt = await readQl7SupportAiBoxAnalysis({ analysis: input.analysis || {}, actor: input.actor || { id: input.userId }, deps: input.aiBoxDeps || {}, now: input.now || new Date() })
      return {
        ok: true,
        ...entitlement,
        branch: 'exchange_ai_analysis_ready',
        status: 'healthy',
        playbook: 'exchange_ai_shared_analysis_service',
        sourceContract: getQl7SupportSourceContract(topic),
        aiBoxReceipt,
        aiBoxAnalysis: aiBoxReceipt.result,
        ...aiBoxReceipt.result,
      ...
      return {
        ok: false,
        ...entitlement,
        branch: 'exchange_ai_source_unavailable',
        status: 'unavailable',
        playbook: 'exchange_ai_shared_analysis_service',
        sourceContract: getQl7SupportSourceContract(topic),
        error: str(error?.message || 'exchange_ai_analysis_unavailable'),
        businessCollectionsWritten: [],
        readOnly: true,
      }
    }
