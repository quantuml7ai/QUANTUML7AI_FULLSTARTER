...
export const QL7_SUPPORT_ACTION_REGISTRY_V11 = frozen({
  ...
  metastudio: frozen({ actionType: 'route', href: '/game?ql7Action=metastudio#metastudio', tab: 'metastudio' }),
  metaverse: frozen({ actionType: 'route', href: '/game' }),
...
export const QL7_SUPPORT_ROUTE_HREFS_V9 = frozen(Object.fromEntries(Object.entries(QL7_SUPPORT_ACTION_REGISTRY_V11).filter(([, value]) => value.actionType === 'route').map(([key, value]) => [key, value.href])))
export const QL7_SUPPORT_SAFE_ROUTE_IDS_V9 = frozen(Object.keys(QL7_SUPPORT_ACTION_REGISTRY_V11))
const QL7_SUPPORT_ROUTE_NAVIGATION_HREFS_V11 = frozen({
...
export function isQl7SupportSafeRouteIdV9(value=''){return Object.prototype.hasOwnProperty.call(QL7_SUPPORT_ACTION_REGISTRY_V11,str(value))}
export function getQl7SupportRouteHrefV9(value=''){return str(QL7_SUPPORT_ACTION_REGISTRY_V11[str(value)]?.href)}
export function getQl7SupportRouteNavigationHrefV11(value=''){
  const routeId=str(value)
  return str(QL7_SUPPORT_ROUTE_NAVIGATION_HREFS_V11[routeId]||QL7_SUPPORT_ACTION_REGISTRY_V11[routeId]?.href)
}
export function getQl7SupportActionDescriptorV11(value=''){const row=QL7_SUPPORT_ACTION_REGISTRY_V11[str(value)];return row?frozen({...row,detail:row.detail?{...row.detail}:undefined}):null}
export function getQl7SupportTopicRouteIdV9(topic=''){const raw=str(topic).toLowerCase();if(raw==='qrm'||raw==='crypto_radar')return raw;return TOPIC_ROUTE_ID[normalizeQl7SupportTopic(topic)]||''}
export function getQl7SupportTopicActionLabelV9(locale='en',seed='',routeId=''){const lang=normalizeQl7SupportLocale(locale);const row=LABELS[lang]||LABELS.en;const special=routeId==='qcoin'?row.qcoin:routeId==='wallet'||routeId==='payments'?row.wallet:routeId==='vip'?row.vip:routeId==='metamarket'?row.metamarket:routeId.startsWith('ads_')?row.ads:routeId==='exchange_ai'?row.exchangeAi:routeId.startsWith('exchange')||['battlecoin','battle_chat','futures'].includes(routeId)?row.exchange:routeId==='gameverse'||routeId==='metastudio'||routeId==='metaverse'?row.gameverse:routeId==='academy'||routeId==='academy_exam'?row.academy:routeId==='forum'||routeId.startsWith('forum_')||['search','geodetect','media','quantum_family','moderation','profile','push','quests'].includes(routeId)?row.forum:routeId==='messenger'?row.messenger:routeId==='auth'||routeId==='telegram'?row.auth:['privacy','security','account_deletion'].includes(routeId)?row.privacy:['contact','support_system','learning_governance'].includes(routeId)?row.support:'';return special||pick(row.default,`${seed}:${lang}`)||LABELS.en.default[0]}
export function getQl7SupportTopicActionV9(topic='',{locale='en',seed='',kind='primary'}={}){const normalizedTopic=normalizeQl7SupportTopic(topic);const routeId=getQl7SupportTopicRouteIdV9(normalizedTopic);const descriptor=getQl7SupportActionDescriptorV11(routeId);if(!routeId||!descriptor)return null;return frozen({id:`open-${normalizedTopic}`,routeId,...descriptor,label:getQl7SupportTopicActionLabelV9(locale,`${seed}:${normalizedTopic}`,routeId),iconKey:'arrow-right',kind})}
