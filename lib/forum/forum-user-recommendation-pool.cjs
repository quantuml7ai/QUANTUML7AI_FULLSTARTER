...
function mediaLockIdentityVariants(raw) {
  const out = new Set()
  const add = (value) => { const id = str(value); if (id) { out.add(id); out.add(id.toLowerCase()) } }
  const value = str(raw)
  if (!value) return []
  add(value)
  const wallet = walletAddress(value)
  if (wallet) { add(wallet); add(`wallet:${wallet}`) }
  const tg = telegramValue(value) || (/^\d+$/.test(value) ? value : '')
  if (tg) {
    for (const candidate of [tg, `tg:${tg}`, `tguid:${tg}`, `telegram:${tg}`, `telegramid:${tg}`, `telegram:id:${tg}`, `tg:uid:${tg}`]) add(candidate)
  }
...
async function readMediaLockStates(canonicalIds, aliasesByCanonical = null, phase = 'runtime', referenceNowMs = Date.now()) {
  const ids = uniq(canonicalIds)
  ...
  // Unit/integration tests install test identity/profile readers. Do not require
  // external Redis credentials in that synthetic environment; production remains
  // fail-closed when no authoritative media-lock source is configured.
  if ((testCanonicalResolver || testProfileReader || testDatabase) && (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN)) {
    return out
  }
  if (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN) {
    throw new Error('recommendation_media_lock_source_unavailable')
  }

  const redis = await recommendationRedis()
  ...
  for (const canonical of ids) {
    ...
    for (const alias of aliases) {
      for (const variant of mediaLockIdentityVariants(alias)) {
        const key = `${MEDIA_LOCK_KEY_PREFIX}${variant}`
  ...
  const keys = Array.from(keyOwners.keys())
  for (const group of chunks(keys, MEDIA_LOCK_QUERY_CHUNK)) {
    let values
    if (typeof redis.mget === 'function') values = await redis.mget(...group)
    else values = await Promise.all(group.map((key) => redis.get(key)))
    group.forEach((key, index) => {
      const untilMs = Math.max(0, timestampMs(values?.[index]))
      if (!untilMs) return
      for (const canonical of keyOwners.get(key) || []) {
        const current = out.get(canonical) || { locked: false, untilMs: 0 }
        if (untilMs > current.untilMs) out.set(canonical, { locked: untilMs > Date.now(), untilMs })
      }
    })
  }
  ...
  for (const canonical of ids) {
    const current = out.get(canonical) || { locked: false, untilMs: 0 }
    out.set(canonical, { locked: current.untilMs > nowMs, untilMs: current.untilMs > nowMs ? current.untilMs : 0 })
  }
  return out
}

async function selectModerationEligibleRanked(ranked, aliasesByCanonical, nowMs) {
  ...
  while (checked < source.length && selected.length < target) {
    ...
    const states = await readMediaLockStates(group.map((row) => row.canonicalAccountId), aliasesByCanonical, 'build-media-lock', nowMs)
...
async function calculatePool({ db: explicitDb, nowMs = Date.now() } = {}) {
  ...
  const moderation = await selectModerationEligibleRanked(ranked, aliasesByCanonical, nowMs)
...
async function hydrateCards(db, rows, bannedRawIds = []) {
  ...
  const graphCanonicals = uniq([
    ...rowIds.map((id) => canonicalFromResolution(resolution, id)),
    ...bannedIds.map((id) => canonicalFromResolution(resolution, id)),
  ])
  const forumCanonicalMap = await resolveForumCanonicalMap(graphCanonicals, 'delivery-canonical')
  const currentCanonical = (raw) => {
    const graphCanonical = canonicalFromResolution(resolution, raw)
    return str(forumCanonicalMap.get(graphCanonical) || graphCanonical)
  }
  const bannedCanonical = new Set(bannedIds.map((id) => currentCanonical(id).toLowerCase()).filter(Boolean))
  const aliasesByCanonical = canonicalAliasMap(resolution, forumCanonicalMap, graphCanonicals)
  const deliveryCanonicalIds = uniq(rowIds.map((id) => currentCanonical(id)))

  // Live follower gate is read from the authoritative relation sets, not the
  // denormalized forum_subscription_counts projection.
  const exactFollowers = await readExactFollowerCounts(db, deliveryCanonicalIds, aliasesByCanonical)
  const mediaLocks = await readMediaLockStates(deliveryCanonicalIds, aliasesByCanonical, 'delivery-media-lock')
  ...
  const vipCanonical = new Set(vipRows.map((row) => str(metricForumCanonical.get(str(row?.accountId || str(row?._id).replace(/^vip:/, ''))) || currentCanonical(row?.accountId || str(row?._id).replace(/^vip:/, '')))).filter(Boolean))

  const authoritativeProfiles = await readAuthoritativeProfiles(deliveryCanonicalIds, 'delivery-profile')
  const out = []
...
async function getPage({ viewerId = '', excludeIds = [], cursor = '', batchSize = DEFAULT_BATCH_SIZE, batchCount = DEFAULT_BATCH_COUNT, feedMode = 'video', sort = 'random' } = {}) {
  ...
  const requestGraphIds = uniq([
    canonicalFromResolution(requestIdentity, viewerId),
    ...(Array.isArray(excludeIds) ? excludeIds : []).map((id) => canonicalFromResolution(requestIdentity, id)),
  ])
  const requestForumCanonical = await resolveForumCanonicalMap(requestGraphIds, 'request-canonical')
  const toRequestCanonical = (raw) => {
    const graphCanonical = canonicalFromResolution(requestIdentity, raw)
    return str(requestForumCanonical.get(graphCanonical) || graphCanonical)
  }
  const canonicalViewerId = toRequestCanonical(viewerId)
  const canonicalExcludeIds = uniq((Array.isArray(excludeIds) ? excludeIds : []).map((id) => toRequestCanonical(id)))
