...
function weeklyBuildKey(nowMs = Date.now()) {
...
function nextWeeklyBuildAtMs(nowMs = Date.now()) {
  return Date.parse(`${weeklyBuildKey(nowMs)}T00:00:00.000Z`) + REBUILD_MS
}
...
async function selectModerationEligibleRanked(ranked, aliasesByCanonical, nowMs, explicitDb = null) {
  ...
  return {
    ...
    nextBuildAtMs: nextWeeklyBuildAtMs(nowMs),
  }
}
...
async function calculatePool({ db: explicitDb, nowMs = Date.now() } = {}) {
  ...
  const users = moderation.selected.slice(0, TOP_LIMIT).map((row, index) => {
    const profile = authoritativeProfiles.get(row.canonicalAccountId) || {}
    return {
      ...row,
      nickname: str(profile.nickname),
  ...
  const buildWeek = weeklyBuildKey(nowMs)
  return {
    _id: POOL_ID,
    ...
    builtAt,
    buildWeek,
    nextBuildAt: iso(nextWeeklyBuildAtMs(nowMs)),
    leaseToken: null,
    leaseUntil: null,
    users,
    ...
    profileResolution: {
      ...
      presentationFieldsStoredInPool: 2,
    },
    updatedAt: builtAt,
  }
}
...
async function acquireLease({ db: explicitDb, force = false, nowMs = Date.now() } = {}) {
  ...
  const currentWeekReadyFilter = {
    $or: [
      { buildWeek: { $ne: buildWeek } },
      { schemaVersion: { $ne: SCHEMA_VERSION } },
      { 'profileResolution.presentationHydration': { $ne: 'weekly-materialized' } },
    ],
  }
  const filter = {
    _id: POOL_ID,
    $and: [
      force ? {} : currentWeekReadyFilter,
  ...
  return { acquired: Number(result?.modifiedCount || 0) === 1, leaseToken, leaseUntil: iso(nowMs + LEASE_MS), buildWeek, db }
}
...
async function rebuildPool({ force = false, reason = 'runtime', nowMs = Date.now(), explicitDb = null } = {}) {
  ...
  if (!lease.acquired) {
    ...
    const alreadyBuilt = str(current?.buildWeek) === lease.buildWeek
      && Math.trunc(num(current?.schemaVersion)) === SCHEMA_VERSION
      && str(current?.profileResolution?.presentationHydration) === 'weekly-materialized'
    return {
      ok: true,
      rebuilt: false,
      reason: alreadyBuilt ? 'already_built_this_week' : 'lease_not_acquired',
      buildWeek: str(current?.buildWeek),
      targetBuildWeek: lease.buildWeek,
      poolVersion: str(current?.version),
      poolSize: Array.isArray(current?.users) ? current.users.length : 0,
    }
  }
  try {
    const next = await calculatePool({ db: lease.db, nowMs })
    const candidateValidation = validatePoolDocument(next, { requireUsers: false })
...
async function getSnapshot({
  viewerId = '',
  excludeIds = [],
  requestIdentityResolved = false,
  nowMs = Date.now(),
  explicitDb = null,
} = {}) {
  const db = explicitDb || await database()
  const pool = await readPool({ db })
  const targetBuildWeek = weeklyBuildKey(nowMs)
  const poolReady = !!(
    pool
    && str(pool.version)
    && Array.isArray(pool.users)
    && Math.trunc(num(pool.schemaVersion)) === SCHEMA_VERSION
    && str(pool?.profileResolution?.presentationHydration) === 'weekly-materialized'
  )
  const rebuildDue = !poolReady || str(pool?.buildWeek) !== targetBuildWeek
  ...
  if (!poolReady) {
    return {
      ...
      poolSize: 0,
      poolBuiltAt: '',
      buildWeek: str(pool?.buildWeek),
      targetBuildWeek,
      nextBuildAt: str(pool?.nextBuildAt),
      rebuildDue: true,
      poolReady: false,
    }
  }

  const requestIds = uniq([viewerId, ...(Array.isArray(excludeIds) ? excludeIds : [])])
  let canonicalMap = new Map(requestIds.map((id) => [id, id]))
  if (!requestIdentityResolved && requestIds.length) {
    canonicalMap = await resolveForumCanonicalMap(requestIds, 'delivery-request-canonical')
  }
  const canonicalViewerId = str(canonicalMap.get(str(viewerId)) || viewerId)
  const canonicalExcludeIds = uniq((Array.isArray(excludeIds) ? excludeIds : [])
    .map((id) => str(canonicalMap.get(str(id)) || id)))

  // The weekly document already owns followers, profile and VIP presentation.
  // Delivery keeps only two compact live safety gates: one ban-set read and one
  // batch MediaLock read. It never re-ranks or hydrates 15 cards individually.
  const poolIds = uniq(pool.users.map((row) => row?.canonicalAccountId))
  const [bannedIds, mediaLocks] = await Promise.all([
    readBannedIds(db),
    readMediaLockStates(poolIds, null, 'delivery-snapshot-media-lock', nowMs, db),
  ])
  const bannedMap = bannedIds.length
    ? await resolveForumCanonicalMap(bannedIds, 'delivery-banned-canonical')
    : new Map()
  const excluded = new Set(uniq([
    canonicalViewerId,
    ...canonicalExcludeIds,
    ...bannedIds.map((id) => str(bannedMap.get(id) || id)),
  ]).map((id) => identityKey(id)))

  const emitted = new Set()
  const users = []
  for (const row of pool.users) {
    const canonical = str(row?.canonicalAccountId)
    const key = identityKey(canonical)
    const lock = mediaLocks.get(canonical) || { locked: false }
    if (!canonical || !key || emitted.has(key) || excluded.has(key) || lock.locked) continue
    if (Math.trunc(num(row?.metrics?.followers)) < MIN_FOLLOWERS) continue
    if (!str(row?.nickname) || !str(row?.avatar)) continue
    emitted.add(key)
    users.push({
      userId: canonical,
      ...
      nickname: str(row.nickname),
      avatar: str(row.avatar),
      followersCount: Math.max(0, Math.trunc(num(row?.metrics?.followers))),
      isVip: row?.metrics?.vipAtBuild === true,
    })
  }
...
function validatePoolDocument(pool, { requireUsers = true } = {}) {
  ...
  if (Math.trunc(num(pool?.schemaVersion)) >= 3) {
    ...
    if (str(pool?.profileResolution?.presentationHydration) !== 'weekly-materialized') issues.push('authoritative_profile_hydration_mode')
    if (Math.trunc(num(pool?.profileResolution?.presentationFieldsStoredInPool, -1)) !== 2) issues.push('presentation_fields_stored')
  }
  ...
  if (Math.trunc(num(pool?.schemaVersion)) >= SCHEMA_VERSION) {
    ...
    if (pool?.moderation?.nextBuildScheduledForMediaUnlock !== false) issues.push('moderation_extra_rebuild_trigger')
    ...
    const privacyCompacted = Number.isFinite(Date.parse(str(pool?.lastPrivacyDeleteAt)))
    if (!privacyCompacted && Math.trunc(num(pool?.eligibility?.poolSize, -1)) !== users.length) issues.push('eligibility_pool_size_metadata')
    if (!privacyCompacted && eligibleExact && users.length !== Math.min(TOP_LIMIT, eligibleCount)) issues.push('eligibility_pool_completeness')
    if (!privacyCompacted && !eligibleExact && users.length !== TOP_LIMIT) issues.push('eligibility_truncated_pool_must_be_full')
    ...
    if (!privacyCompacted && eligibleExact && Math.trunc(num(pool?.eligibility?.omittedByTopLimit, -1)) !== Math.max(0, eligibleCount - users.length)) issues.push('eligibility_omitted_count')
  ...
  users.forEach((row, index) => {
    ...
    previousScore = score
    const privacyCompacted = Number.isFinite(Date.parse(str(pool?.lastPrivacyDeleteAt)))
    if (!privacyCompacted && Math.trunc(num(row?.rank)) !== index + 1) issues.push(`rank:${index}`)
    if (!str(row?.nickname)) issues.push(`nickname:${index}`)
    if (!str(row?.avatar)) issues.push(`avatar:${index}`)
    if (Object.prototype.hasOwnProperty.call(row || {}, 'icon')) issues.push(`profile_icon_alias:${index}`)
  ...
  if (Number.isFinite(built) && str(pool.buildWeek) !== weeklyBuildKey(built)) issues.push('build_week')
  if (Number.isFinite(built) && Number.isFinite(to) && Math.abs(to - built) > 10_000) issues.push('window_to_built_at')
  if (!Number.isFinite(next) || !Number.isFinite(built) || Math.abs(next - nextWeeklyBuildAtMs(built)) > 1000) issues.push('weekly_rebuild_boundary')
