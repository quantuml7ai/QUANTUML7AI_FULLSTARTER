...
const { buildFeedSelectionPlan } = require('../geo/geo-rings.cjs');
...
async function readForumThreadPage({ input = {} } = {}) {
  const body = normalizeBody(input);
  const pageSize = clampPageSize('thread', body.pageSize || body.limit);
  const cursorPayload = body.cursor ? await decodeSignedCursor(DOMAIN_THREAD, body.cursor) : null;
  const plan = buildThreadPagePlan({ mode: body.mode || body.branchMode || 'children', topicId: body.topicId, parentId: body.parentId, rootPostId: body.rootPostId, pageSize, cursor: cursorPayload });
  const db = await database();
  const rows = await filterRowsWithExistingCorePosts(db, await findRows(db, plan));
  const returnedRows = rows.slice(0, pageSize);
  const hydratedRows = await hydrateRowsWithReplyMetadata(db, await hydratePostRowsWithThreadReplyCounts(db, await hydratePostRowsWithCoreCounters(db, returnedRows)));
  ...
  const nextCursor = await makeNextCursor({ domain: DOMAIN_THREAD, plan, rows, returnedRows, pageSize, sort: 'new' });
  return sanitizePublicForumPayload({ ok: true, kind: 'ql7-forum-thread-page', source: 'mongo_projection_index', count: items.length, items, nextCursor, hasMore: Boolean(nextCursor), queryShape: { collection: plan.collection, hint: plan.hint, limit: plan.limit, cursorDomain: plan.cursorDomain } });
}
