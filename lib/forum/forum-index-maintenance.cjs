...
function mediaFromPost(post = {}) {
  const candidates = [];
  for (const key of ['videoUrl', 'imageUrl', 'audioUrl', 'url']) {
...
async function maintainForumIndexesForPostEdited({ postId, text } = {}) {
  ...
  const cleanSnippet = snippet(cleanText);
  const post = await readPost(db, pid) || { id: pid, text: cleanText };
  const media = mediaFromPost({ ...post, text: cleanText });
  const postTextSet = {
    'post.text': cleanText,
  ...
  const searchTextSet = {
    ...postTextSet,
    'text.snippet': snippet(cleanText, 320),
    'text.tokens': tokenize(cleanText),
    'text.qHash': qHash(cleanText),
  };
  await Promise.all([
    db.collection('forum_thread_index').updateMany({ postId: pid }, { $set: postTextSet }),
    db.collection('forum_user_post_index').updateMany({ postId: pid }, { $set: postTextSet }),
    db.collection('forum_search_index').updateMany({ postId: pid }, { $set: searchTextSet }),
    db.collection('forum_geo_feed_index').updateMany({ postId: pid }, { $set: postTextSet }),
    db.collection('forum_media_feed_index').updateMany({ postId: pid }, { $set: postTextSet }),
    db.collection('forum_reply_inbox_index').updateMany({ postId: pid }, { $set: postTextSet }),
  ]);
