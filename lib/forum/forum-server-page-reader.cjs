...
async function readForumFeedPage({ request = null, input = {} } = {}) {
  ...
  // readRingPriorityPage already hydrated thread reply totals before ranking.
  // Repeating that child lookup here adds Mongo work without changing the payload.
  const hydratedRows = await hydrateRowsWithReplyMetadata(db, await hydrateRowsWithTopicPostTotals(db, await hydrateRowsWithTopicCounters(db, ringResult.returnedRows)))
