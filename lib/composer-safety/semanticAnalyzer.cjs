...
function extractFeatures(text,{locale='und',targeted=false,quotedRanges=[],conversationReferences=[],context={}}={}){
 ...
 const naturalDestroyAction=Boolean(unquotedRiskHints.hits?.destroy?.some?.(row=>row?.vocabulary==='natural_intent'))
 const naturalCyberAction=Boolean(unquotedRiskHints.hits?.cyber?.some?.(row=>row?.vocabulary==='natural_intent'))
 const cyberDefense=cyberAttack&&bool(CYBER_DEFENSE_RE,lower)&&!naturalCyberAction&&!commitment&&!incite
 const prevention=bool(PREVENTION_RE,lower)&&!commitment&&!incite
 const quotedHarmContinuation=Boolean(quoteSpans.length>0&&(unquotedRiskHints.commitment||unquotedHints.temporal)&&(
  riskHints.kill||riskHints.attack||riskHints.destroy||riskHints.cyber||riskHints.war||riskHints.riot||explosive||terror
 ))
 ...
 const harassment=bool(HARASSMENT_RE,lower),degrading=bool(DEGRADING_RE,lower)
 ...
 return Object.freeze({version:VERSION,source:rawSource,normalizedHash:hash(rawSource),locale:String(locale||'und'),tokenCount:ts.length,contextKind:String(context?.conversationKind||context?.operationType||''),localeHints,riskHints,moderationHints:{version:moderationHints.version,bankTermCount:moderationHints.bankTermCount,scannedLocaleCount:moderationHints.scannedLocaleCount,profanity:moderationHints.profanity,euphemisms:moderationHints.euphemisms,directInsults:moderationHints.directInsults,threats:moderationHints.threats},canonicalSafety:{version:canonicalSafety.version,locale:canonicalSafety.locale,fingerprint:canonicalSafety.fingerprint,insultHits:canonicalSafety.insults.length,threatHits:canonicalSafety.threats.length,targetHits:canonicalSafety.targets.length,denialHits:canonicalSafety.denials.length,productHits:canonicalSafety.product.length,quoteHits:canonicalSafety.quotes.length},quoteScope:{hasQuote:quoteSpans.length>0||canonicalQuote,reportedSpeech:reported,ranges:quoteSpans},targetScope:{targeted:Boolean(targeted||second||third),harmTargeted:targetFrame.personalThreatTarget===true,targetKind:targetFrame.targetKind,firstPerson:Boolean(first||implicitFirstHarm),secondPerson:second,thirdPerson:third,semanticFrame:targetFrame},context:{newsHistoricalEducational:news,victimReport:victim,counterSpeech:counter,selfDefense,fiction,negation,denial:decisionDenial,canonicalDenialObserved:canonicalDenial,idiom,protectiveAdvice,positiveExclamation,cyberDefense,prevention,conversationReferenceCount:Array.isArray(conversationReferences)?conversationReferences.length:0},signals:{directKill,operational,commitment,explosive,terror,mass,attack,incite,war,warIncitement,cyberAttack,naturalCyberAction,property,naturalDestroyAction,praise,profanity,insult,harassment,degrading,quotedHarmContinuation,product,frustration},actionability:{operationalInstruction:Boolean(operational||commitment||(naturalDestroyAction&&(!decisionDenial||commitment||implicitFirstHarm))||quotedHarmContinuation),hasTarget:Boolean(targeted||second||third),hasHarmTarget:targetFrame.personalThreatTarget===true,specificity:(explosive||terror||property||mass||warIncitement||cyberAttack)?'high':directKill?'medium':'low',capabilityClaim:Boolean(quotedHarmContinuation||((!decisionDenial||commitment||implicitFirstHarm)&&(naturalDestroyAction||naturalCyberAction||((first||implicitFirstHarm||commitment||incite)&&(operational||(directKill&&targetFrame.personalThreatTarget===true)||property||attack||warIncitement||terror||explosive))))),temporalIntent},features:Object.freeze(f)})
}
function scoreCandidates(features,{priorConfirmedWarnings=0}={}){const s=features.signals,c=features.context,t=features.targetScope,a=features.actionability,rows=[];const add=(id,score,evidence=[],counterEvidence=[])=>rows.push({id,score,evidence,counterEvidence});
 ...
 const safeCounter=[c.victimReport&&'victim_report',c.counterSpeech&&'counter_speech',c.newsHistoricalEducational&&'news_education',c.selfDefense&&'self_defense',c.fiction&&'fiction',c.idiom&&'safe_idiom',c.denial&&'local_denial',c.cyberDefense&&'cyber_defense',c.prevention&&'prevention',(features.quoteScope.hasQuote||features.quoteScope.reportedSpeech)&&'quoted_scope'].filter(Boolean)
 const suppressSevere=Boolean(safeCounter.length&&!a.operationalInstruction&&!a.capabilityClaim)
 ...
 if((c.cyberDefense||c.prevention)&&!a.operationalInstruction&&!a.capabilityClaim)add('clean_respectful',.993,[c.cyberDefense?'cyber_defense_context':'prevention_context'],[])
 if(s.quotedHarmContinuation)add('dangerous_operational_intent',.997,['quoted_harm','unquoted_continuation'],safeCounter)
 if(s.terror&&(s.operational||s.commitment||s.incite))add('terrorism_operational_intent',suppressSevere?.42:.995,['terror_entity',s.explosive?'explosive_entity':'terror_action',s.operational?'operational_instruction':s.incite?'incitement':'operational_commitment'],safeCounter)
 if(s.explosive&&!s.terror&&(s.operational||s.commitment||s.incite))add('dangerous_operational_intent',suppressSevere?.42:.992,['explosive_entity',s.operational?'operational_instruction':s.incite?'incitement':'operational_commitment'],safeCounter)
 ...
 if(s.property&&!c.cyberDefense&&(t.firstPerson||s.commitment||s.incite||s.cyberAttack||s.naturalDestroyAction))add('property_destruction_incitement',suppressSevere?.35:.97,[s.cyberAttack?'cyber_attack_intent':'property_harm',t.firstPerson?'first_person':s.incite?'incitement':s.naturalDestroyAction?'natural_action_form':'operational_commitment'],safeCounter)
 ...
 if(s.harassment)add('harassment',.93,['harassment_pattern',a.hasTarget?'target_scope':'target_implied'],[])
 if(s.degrading)add('degrading_hate_like_language',.92,['degrading_pattern',a.hasTarget?'target_scope':'target_implied'],[])
 ...
 if((features.riskHints.kill||features.riskHints.attack||features.riskHints.destroy||features.riskHints.cyber||features.riskHints.war||features.riskHints.riot)&&!c.cyberDefense&&!c.prevention)add('risk_ambiguous',.79,['localized_risk_neighborhood','context_or_target_unresolved'],safeCounter)
...
function analyzeComposerSemantics(text,options={}){
 ...
 const defensiveSafe=Boolean((features.context.cyberDefense||features.context.prevention)&&!features.actionability.operationalInstruction&&!features.actionability.capabilityClaim)
 if(sharedSevere&&!scopedSafe&&!defensiveSafe)selected=sharedSevere
 else if(severeTop&&!scopedSafe&&!defensiveSafe&&!features.context.fiction&&!features.context.idiom&&!features.context.selfDefense)selected=severeTop
 else if(safe&&(['victim_report','counter_speech'].includes(safe.id)||features.context.idiom||features.context.fiction||!severeTop))selected=safe
