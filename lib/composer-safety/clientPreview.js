...
export function classifyComposerPreview(text='', { locale='und', targeted=false } = {}) {
  ...
  const materialActionability = Boolean(
    operational || commitment || warIncitement ||
    (kill && harmTarget && (first || implicitFirstHarm)) ||
    (property && (first || commitment || cyberAttack)) ||
    (incite && (kill || attack))
  )
  ...
  if (idiom && !materialActionability) return row('clean_respectful', .90, ['safe_idiom'])
  ...
  if (terror && explosive && (operational || commitment)) return row('terrorism_operational_intent', .97, ['terror','explosive', operational ? 'operational' : 'commitment'])
  ...
  if (property && (first || commitment || cyberAttack)) return row('property_destruction_incitement', .94, [cyberAttack ? 'cyber_attack' : 'property_harm','commitment'])
  ...
  if (cyberAttack) return row('uncertain_hostility', .90, ['cyber_risk','security_review'])
