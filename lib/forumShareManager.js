...
export function getShareTargets({ url, text, labels }) {
  ...
  const waTextEncoded = safeUrl ? encodeURIComponent(safeUrl) : '';
  const fbWebUrl = safeUrl
    ? `https://www.facebook.com/sharer/sharer.php?u=${urlEncoded}`
    : '';
  const xWebUrl = safeUrl ? `https://twitter.com/intent/tweet?text=${textEncoded}` : '';
  const igWebUrl = safeUrl ? `https://www.instagram.com/?url=${urlEncoded}` : '';
  ...
  return [
    ...
    },
    {
      ...
      icon: '/friends/wa.png',
      label: l('wa', 'WhatsApp'),
      appUrl: safeUrl ? `whatsapp://send?text=${waTextEncoded}` : '',
      webUrl: safeUrl ? `https://api.whatsapp.com/send?text=${waTextEncoded}` : '',
      url: safeUrl ? `https://wa.me/?text=${waTextEncoded}` : '',
    },
    {
      key: 'viber',
    ...
    },
    {
      ...
      icon: '/friends/fb.png',
      label: l('fb', 'Facebook'),
      appUrl: fbWebUrl ? `fb://facewebmodal/f?href=${encodeURIComponent(fbWebUrl)}` : '',
      webUrl: fbWebUrl,
      url: fbWebUrl,
    },
    {
      ...
      icon: '/friends/x.png',
      label: l('x', 'X'),
      appUrl: safeUrl ? `twitter://post?message=${textEncoded}` : '',
      webUrl: xWebUrl,
      url: xWebUrl,
    },
  ];
...
export function openAppOrWeb(target, opts = {}) {
  const appUrl = String(target?.appUrl || '').trim();
  const webUrl = String(target?.webUrl || target?.url || '').trim();
  const delay = Math.max(300, Math.min(2500, Number(opts?.delayMs) || 1100));

  if (!appUrl && !webUrl) return false;
  if (typeof window === 'undefined') return false;

  if (!appUrl) return safeOpen(webUrl);
  ...
  const onVisibilityChange = () => {
    if (document.visibilityState === 'hidden') appLaunchDetected = true;
  };
  ...
  window.setTimeout(() => {
    ...
    if (!webUrl || appLaunchDetected) return;
    try {
      if (!popup.closed) popup.location.href = webUrl;
    } catch {}
  }, delay);
