...
export function buildCanonicalPostPath(postId) {
  ...
  return `/forum/p/${encodeURIComponent(id)}`;
}
...
export function buildShareTitle(_post, labels) {
  ...
  const generic = String(lab.generic || 'Forum post').trim() || 'Forum post';
  return generic;
}

export function buildShareText(post, opts = {}) {
  const maxLen = Number(opts?.maxLen || 200);
  const plain = toPlainText(post?.text || post?.bodyTextPlain || post?.body || '');
  const snippet = truncateOnWord(plain, maxLen);
  const title =
...
export function buildShareSnippet(post, opts = {}) {
  const maxLen = Number(opts?.maxLen || 220);
  const plain = toPlainText(post?.text || post?.bodyTextPlain || post?.body || '');
  return truncateOnWord(plain, maxLen);
}
...
export function getShareTargets({ url, text, labels }) {
  ...
  const textEncoded = safeUrl
    ? encodeURIComponent((safeText ? `${safeText} ${safeUrl}` : safeUrl).trim())
    : '';
  ...
  return [
    {
      ...
      url:
        safeUrl
          ? `https://t.me/share/url?url=${urlEncoded}${safeText ? `&text=${textOnlyEncoded}` : ''}`
          : '',
    },
    {
      key: 'ig',
    ...
    },
    {
      ...
      icon: '/friends/wa.png',
      label: l('wa', 'WhatsApp'),
      url: safeUrl ? `https://wa.me/?text=${textEncoded}` : '',
    },
    {
      key: 'viber',
      icon: '/friends/viber.png',
      label: l('viber', 'Viber'),
      url: safeUrl ? `viber://forward?text=${textEncoded}` : '',
    },
    {
      key: 'fb',
      icon: '/friends/fb.png',
...
export async function tryNativeShare({ title, text, url }) {
  try {
    ...
    if (t) payload.text = t.slice(0, 1000);
