...
const K = {
  ...
  runtimeState: (userId) => `notif:${shortUserId(userId)}`,
...
export function notificationImpulseChannel(rawUserId) {
  return `push:events:${shortUserId(rawUserId)}`
}
...
export async function publishNotificationImpulse(rawUserId, {
  ...
  const subscriptions = await listWebPushSubscriptions(userId)
  let webSent = 0
  if (subscriptions.length > 0 && configureVapid()) {
    await Promise.all(subscriptions.map(async (record) => {
      const id = String(record?.id || '').trim()
      if (!record?.subscription) {
        if (id) await removeSubscriptionById(userId, id)
        return
      }
      if (String(record.userId || '') !== userId) {
        if (id) await removeSubscriptionById(userId, id, { deleteRecord: false })
        return
      }
      try {
        ...
        if ((status === 404 || status === 410) && id) await removeSubscriptionById(userId, id)
      }
    }))
  }
...
function normalizePushStateDocument(raw = {}) {
  ...
  return {
    ...
    storagePrimary: String(record.storagePrimary || 'mongo'),
    redisRole: String(record.redisRole || 'accelerator-only'),
  }
}
...
function publicPushState(state = {}) {
  ...
  return {
    ...
    storagePrimary: normalized.storagePrimary,
    redisRole: normalized.redisRole,
  }
}
...
async function removeSubscriptionById(userId, id, { deleteRecord = true } = {}) {
  await Promise.allSettled([
    pushPrimary.removePushSubscription(userId, id, { deleteRecord }),
    redis.srem(K.userSubscriptions(userId), id),
    deleteRecord ? redis.del(K.subscription(id)) : Promise.resolve(),
  ])
}

async function listLegacyPushSubscriptions(userId) {
  const ids = await redis.smembers(K.userSubscriptions(userId)).catch(() => [])
  if (!Array.isArray(ids) || !ids.length) return []
  let raws = null
  if (typeof redis.mget === 'function') {
    raws = await redis.mget(...ids.map((id) => K.subscription(id))).catch(() => null)
  }
  const values = Array.isArray(raws)
    ? raws
    : await Promise.all(ids.map((id) => redis.get(K.subscription(id)).catch(() => null)))
  return ids
    .map((id, index) => {
      ...
      return {
        ...
        storagePrimary: 'legacy-redis-subscription',
      }
    })
    .filter(Boolean)
}

async function migrateLegacyPushSubscriptionsToMongo(userId, legacyRecords = []) {
  const records = Array.isArray(legacyRecords) ? legacyRecords : []
  if (!records.length) return []
  const migrated = []
  for (const record of records) {
    const id = String(record?.id || '').trim()
    const subscription = normalizeSubscription(record?.subscription)
    if (!id || !subscription?.endpoint) continue
    const saved = await pushPrimary.savePushSubscription(userId, id, subscription, {
      lang: String(record?.lang || 'en').trim().slice(0, 8) || 'en',
      userAgent: String(record?.userAgent || '').trim().slice(0, 240),
      nativeShell: record?.nativeShell === true,
    }, {
      ttlMs: SUBSCRIPTION_TTL_SECONDS * 1000,
    ...
    await Promise.allSettled([
      redis.srem(K.userSubscriptions(userId), id),
      redis.del(K.subscription(id)),
    ])
  }
  return migrated
}

async function listWebPushSubscriptions(userId) {
  const primary = await pushPrimary
    .listPushSubscriptions(userId, { limit: MAX_SUBSCRIPTIONS_PER_USER })
    .catch(() => [])
  if (Array.isArray(primary) && primary.length) return uniqueWebPushSubscriptions(primary)
  const legacy = await listLegacyPushSubscriptions(userId)
  const migrated = await migrateLegacyPushSubscriptionsToMongo(userId, legacy)
  return uniqueWebPushSubscriptions(migrated.length ? migrated : legacy)
}

function uniqueWebPushSubscriptions(records = []) {
  const byEndpoint = new Map()
  for (const record of Array.isArray(records) ? records : []) {
    const endpoint = String(record?.subscription?.endpoint || '').trim()
    const fallback = String(record?.id || '').trim()
    const key = endpoint || fallback
    if (!key) continue
    const previous = byEndpoint.get(key)
    if (!previous) {
      byEndpoint.set(key, record)
      continue
    }
    const previousUpdated = Number(previous?.updatedAt || previous?.createdAt || 0)
    const nextUpdated = Number(record?.updatedAt || record?.createdAt || 0)
    if (nextUpdated > previousUpdated) byEndpoint.set(key, record)
  }
...
export async function savePushSubscription(rawUserId, rawSubscription, metadata = {}) {
  ...
  await pushPrimary.savePushSubscription(userId, id, subscription, {
...
export async function removePushSubscription(rawUserId, endpoint) {
  ...
  const primaryRecords = await pushPrimary.listPushSubscriptions(userId, { limit: MAX_SUBSCRIPTIONS_PER_USER }).catch(() => [])
  const primaryRecord = Array.isArray(primaryRecords) ? primaryRecords.find((row) => String(row?.id || '') === id) : null
  if (primaryRecords.length && !primaryRecord) return false
...
async function readStoredPushState(userId) {
  const primary = await pushPrimary.readNotificationState(userId).catch(() => null)
  if (primary) return normalizePushStateDocument(primary)
  const raw = await redis.get(K.state(userId)).catch(() => null)
  if (raw == null) return null
  return normalizePushStateDocument({
    ...(parseRecord(raw) || {}),
    storagePrimary: 'legacy-redis-state',
    redisRole: 'migration-source',
  })
}

function compactPushRuntimeSnapshot(state = {}) {
  const normalized = normalizePushStateDocument(state)
  return {
    counts: normalized.counts,
    totalCount: normalized.totalCount,
    ...
    redisRole: 'runtime-cache',
  }
}

async function writeRuntimePushSnapshot(userId, state) {
  const snapshot = compactPushRuntimeSnapshot(state)
  await redis.set(K.runtimeState(userId), JSON.stringify(snapshot), { ex: RUNTIME_STATE_TTL_SECONDS }).catch(() => {})
  return snapshot
}

async function writeStoredPushState(userId, state) {
  const input = normalizePushStateDocument({
    ...state,
    updatedAt: Date.now(),
  })
  const normalized = normalizePushStateDocument(
    await pushPrimary.writeNotificationState(userId, input),
  )
  await writeRuntimePushSnapshot(userId, normalized)
...
async function readPushStateForUser(userId, { force = false } = {}) {
  ...
  if (stored) {
    if (stored.storagePrimary === 'legacy-redis-state') {
      const migrated = await writeStoredPushState(userId, stored)
      await redis.del(K.state(userId)).catch(() => {})
      writePushStateCache(userId, migrated)
      return migrated
    }
...
export async function sendBackgroundPush(rawUserId, {
  ...
  const subscriptions = await listWebPushSubscriptions(userId)
  const webPushEnabled = subscriptions.length > 0 && configureVapid()
  ...
  let sent = 0
  if (webPushEnabled) await Promise.all(subscriptions.map(async (record) => {
    const id = String(record?.id || '').trim()
    if (!record?.subscription) {
      if (id) await removeSubscriptionById(userId, id)
      return
