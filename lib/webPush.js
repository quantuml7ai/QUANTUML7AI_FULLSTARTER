...
async function publishMetaMarketGiftImpulse(userId, state) {
  const count = clampNotificationCount(state?.counts?.[NOTIFICATION_SOURCES.METAMARKET_GIFTS])
  await redis.publish(notificationImpulseChannel(userId), JSON.stringify({
