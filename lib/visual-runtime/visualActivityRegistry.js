...
const DISCONNECTED_SWEEP_MS = 1000
...
const counters = {
  ...
  disconnectedSweepRuns: 0,
  disconnectedSweepReleases: 0,
}
...
export function isIOSExchangeMotionContinuityRuntime(
  ...
  if (!ios) return false
  return /(?:^|\/)exchange(?:\/|$)/i.test(String(pathname || ''))
}
...
function desiredState(record) {
  ...
  // Physical iOS /exchange is a motion-continuity exception. WebKit async scrolling
  // becomes unstable when nested Exchange scopes are pause/resume-owned while the
  // finger is moving. Keep motion hot here without restoring the retired FULL/CHEAP
  // render governor, prepaint observer, filters, shadows or resource prewarm.
  if (isIOSExchangeMotionContinuityRuntime()) return 'running'
...
function sweepDisconnected() {
  counters.disconnectedSweepRuns += 1
  // Set iteration tolerates deleting the current entry, so avoid allocating a
  // full [...nodes] copy on every fallback sweep. Explicit React ref owners still
  // unregister synchronously; this one-second pass only bounds stale dataset-only
  // scopes that disappeared without an owner cleanup callback.
  for (const node of nodes) {
    if (node?.isConnected) continue
    counters.disconnectedSweepReleases += 1
...
export function getVisualActivitySnapshot() {
  ...
  return {
    ...
    runtimeCostGuards: {
      ...
      disconnectedSweepRuns: counters.disconnectedSweepRuns,
      disconnectedSweepReleases: counters.disconnectedSweepReleases,
      disconnectedSweepMs: DISCONNECTED_SWEEP_MS,
    },
    lifecycle: {
