...
function isCssAnimation(animation) {
  try {
    ...
    if (typeof CSSTransition === 'function' && animation instanceof CSSTransition) return true
  } catch {}
  const name = String(animation?.constructor?.name || '')
  return name === 'CSSAnimation' || name === 'CSSTransition'
}

function pauseOwnedWaapi(record) {
  if (!record.pauseJs || typeof record.node?.getAnimations !== 'function') return
  const owned = []
  for (const animation of safeCall(() => record.node.getAnimations({ subtree: true })) || []) {
    if (!animation || isCssAnimation(animation) || animation.playState !== 'running') continue
    try {
      animation.pause()
  ...
  if (owned.length) runtimePausedWaapi.set(record.node, owned)
}
...
function applyRecordState(record) {
  ...
  if (next === 'paused') pauseOwnedWaapi(record)
  record.state = next
  if (record.pauseCss) {
  ...
  if (next === 'running') resumeOwnedWaapi(record)
...
export function unregisterVisualScope(node) {
  ...
  if (node?.dataset?.ql7VisualState) delete node.dataset.ql7VisualState
}
...
export function registerDatasetScope(node) {
  ...
  return registerVisualScope(node, datasetOptions(node))
}
