...
import { buildQl7SupportSmtpLivePolicyReceipt } from './ql7-support/operator/smtpPolicy.js'
...
function redact(value = '') {
  return str(value)
    ...
    .replace(/q[\s._-]*l[\s._-]*7(?:[\s._-]*support)?/giu, 'support')
}
...
function humanizeKnownValue(value = '') {
  ...
  const mapped = {
    ...
    ql7_support_dm: 'Support direct conversation',
...
export function buildSupportEmailReport({ source = 'contact_form', name = '', email = '', message = '', meta = {}, report = null } = {}) {
  ...
  return {
    title: str(input.title) || (str(source) === 'ql7_support_dm' ? 'Support case report' : 'Quantum L7 AI contact request'),
...
export function renderSupportEmailHtml(report = {}) {
  ...
  return `<!doctype html><html lang="${esc(report.locale || 'en')}" dir="ltr"><head><meta charset="utf-8"><style>
    ...
    <h1>${esc(report.title || 'Support report')}</h1>
...
export async function sendSupportEmail({ source = 'contact_form', name = '', email = '', replyTo = '', subject = '', message = '', meta = {}, report = null, smtpContext = null } = {}) {
  ...
  const rendered = {
    subject: str(subject) || str(emailReport.subject) || (str(source) === 'ql7_support_dm' ? `Support case ${emailReport?.case?.caseId || emailReport?.case?.messageId || ''}`.trim() : `Quantum L7 AI contact from ${safeName}`),
  ...
  const livePolicy = buildQl7SupportSmtpLivePolicyReceipt({
    source, emailReport, rendered: emailReport?.rendered || rendered, config: cfg, requestedReplyTo: replyTo || safeEmail, smtpContext: smtpContext || {},
  })
  if (!livePolicy.ok) {
    return { ok: false, skipped: false, reason: 'smtp_live_policy_rejected', policyReceipt: livePolicy }
  }
  ...
  const finalReplyTo = looksLikeEmail(livePolicy.replyTo) ? `"${safeName}" <${livePolicy.replyTo}>` : undefined
  ...
  return { ok: true, skipped: false, messageId: str(info?.messageId), policyReceipt: livePolicy }
}
