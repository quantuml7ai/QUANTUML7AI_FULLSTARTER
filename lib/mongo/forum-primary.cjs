...
function normalizeComposerPolicyBinding(value) {
  if (!value || typeof value !== 'object') return {}
  const decisionId = str(value.composerPolicyDecisionId)
  const actorHash = str(value.composerPolicyActorHash)
  const intentHash = str(value.composerPolicyIntentHash)
  if (!decisionId && !actorHash && !intentHash) return {}
  if (!decisionId || !/^[a-f0-9]{64}$/u.test(actorHash) || !/^[a-f0-9]{64}$/u.test(intentHash)) {
    throw new Error('forum_composer_policy_binding_invalid')
  }
  return {
    composerPolicyDecisionId: decisionId,
    composerPolicyActorHash: actorHash,
    composerPolicyIntentHash: intentHash,
  }
}
function stripComposerPolicyBinding(value) {
  const output = clone(value) || {}
  delete output.composerPolicyDecisionId
  delete output.composerPolicyActorHash
  delete output.composerPolicyIntentHash
  return output
}
...
async function ensureIndexes(database) {
  await Promise.allSettled([
    ...
    database.collection('forum_core_change_events').createIndex({ kind: 1, rev: 1 }),
    database.collection('forum_media_locks').createIndex({ accountId: 1 }, { unique: true, sparse: true }),
...
function addIdentityVariants(target, raw) {
  ...
  if (walletId) {
    target.add(walletId)
    target.add(walletId.toLowerCase())
    target.add(`wallet:${walletId.toLowerCase()}`)
    return
  }

  const telegramId = canonicalUserId.normalizeTelegramId(value)
  if (telegramId) {
    target.add(telegramId)
    target.add(`telegram:${telegramId}`)
    target.add(`telegramid:${telegramId}`)
    target.add(`telegram:id:${telegramId}`)
    target.add(`tguid:${telegramId}`)
    target.add(`tg:${telegramId}`)
    target.add(`tg:uid:${telegramId}`)
    target.add(`tma:${telegramId}`)
  }
}

async function getLinkedIdentityIds(database, userId) {
  const seed = canonicalUserId.normalizePrincipalSyntax(userId)
  const ids = new Set()
  addIdentityVariants(ids, userId)
  addIdentityVariants(ids, seed)
  if (!seed) return []

  const seedVariants = Array.from(ids)
  const walletIds = unique(seedVariants.map((id) => canonicalUserId.normalizeWalletId(id)).filter(Boolean))
  const telegramIds = unique(seedVariants.map((id) => canonicalUserId.normalizeTelegramId(id)).filter(Boolean))

  // New source of truth: profiles explicitly link walletId <-> telegramId.
  const profiles = await database.collection('profiles').find({
    $or: [
      { principalId: { $in: seedVariants } },
      { accountId: { $in: seedVariants } },
      { canonicalAccountId: { $in: seedVariants } },
      { userId: { $in: seedVariants } },
  ...
  // Legacy READ fallback only. Stage 1-4 never writes account_aliases.
  const aliasSeed = Array.from(ids)
  const walletRegexClauses = aliasSeed
    .map((id) => canonicalUserId.normalizeWalletId(id))
    .filter(Boolean)
    .flatMap((walletId) => {
      const escaped = walletId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
      const exact = new RegExp(`^(?:wallet:)?${escaped}$`, 'i')
      return [
        { accountId: exact },
...
function publicPostMutationChange(doc = {}) {
  const parsed = doc?.parsed && typeof doc.parsed === 'object'
    ? doc.parsed
    : (() => { try { return doc?.raw ? JSON.parse(String(doc.raw)) : {} } catch { return {} } })()
  const kind = str(parsed?.kind || doc?.kind)
  const id = str(parsed?.id || doc?.id)
  const rev = num(parsed?.rev ?? doc?.rev, 0)
  const ts = num(parsed?.ts ?? doc?.ts, 0)

  if (kind === 'post_edit') {
    if (!id || typeof parsed?.data?.text !== 'string') return null
    return { kind: 'post_edit', id, rev, ts, data: { text: String(parsed.data.text) } }
  }

  if (kind === 'post_deleted') {
    const deletedPostIds = unique([
      ...(Array.isArray(parsed?.deletedPostIds) ? parsed.deletedPostIds : []),
      ...(Array.isArray(parsed?.deleted) ? parsed.deleted : []),
      id,
    ])
    if (!id && !deletedPostIds.length) return null
    return { kind: 'post_deleted', id: id || deletedPostIds[0], rev, ts, deletedPostIds }
  }

  return null
}

async function readPublicPostMutationChangesSince({ sinceRev = 0, limit = 128 } = {}) {
  const database = await db()
  const since = Math.max(0, Math.floor(num(sinceRev, 0)))
  const take = Math.max(1, Math.min(512, Math.floor(num(limit, 128))))
  const currentRev = Math.max(0, Math.floor(await readCounter('forum:rev', 0)))
  if (since <= 0 || currentRev <= since) {
    return { rev: currentRev, cursorRev: currentRev, events: [], hasMore: false }
  }

  const docs = await database.collection('forum_core_change_events')
    .find({ rev: { $gt: since }, kind: { $in: ['post_edit', 'post_deleted'] } })
    .sort({ rev: 1, ts: 1 })
    .limit(take + 1)
    .toArray()
  const hasMore = docs.length > take
  const selected = hasMore ? docs.slice(0, take) : docs
  const events = selected.map(publicPostMutationChange).filter(Boolean)
  // Cursor follows the scanned journal rows, not only successfully normalized events,
  // so one malformed historical row can never stall pagination forever.
  const scannedRev = selected.reduce((max, doc) => Math.max(max, num(doc?.rev ?? doc?.parsed?.rev, 0)), since)
  const cursorRev = hasMore ? Math.max(since, scannedRev) : currentRev
  return { rev: currentRev, cursorRev, events, hasMore }
}

function mediaLockRecordId(accountId) {
  const id = str(accountId)
  return id ? `media-lock:${id}` : ''
}

async function getMediaLockRecord(accountId) {
  const id = str(accountId)
  if (!id) return null
  const database = await db()
  const doc = await database.collection('forum_media_locks').findOne({ _id: mediaLockRecordId(id) }).catch(() => null)
  if (!doc) return null
  return {
    accountId: str(doc.accountId || id),
    lockedUntil: Math.max(0, num(doc.lockedUntil, 0)),
    reason: str(doc.reason),
    source: str(doc.source),
    sourcePostId: str(doc.sourcePostId),
    policyVersion: str(doc.policyVersion),
...
async function setMediaLockRecord({ accountId, lockedUntil, reason = '', source = '', sourcePostId = '', policyVersion = 'forum-media-lock-canonical-v1' } = {}) {
  const database = await db()
  const id = await resolveForumPrincipal(database, accountId)
  const until = Math.max(0, Math.floor(num(lockedUntil, 0)))
  if (!id || !until) return null
  const iso = nowIso()
  await database.collection('forum_media_locks').updateOne(
    { _id: mediaLockRecordId(id) },
    {
      $set: {
        accountId: id,
        lockedUntil: until,
        reason: str(reason),
        source: str(source),
        sourcePostId: str(sourcePostId),
        policyVersion: str(policyVersion) || 'forum-media-lock-canonical-v1',
        updatedAt: iso,
        storagePrimary: 'mongo',
      },
...
async function createTopic({ title, description, userId, nickname, icon, ts, _geoOrigin = null, composerPolicyBinding = null }) {
  const database = await db()
  const principalId = await resolveForumPrincipal(database, userId)
  if (!principalId) throw new Error('missing_user_id')
  ...
  const t = {
    ...
    storagePrimary: 'mongo',
    updatedAt: nowIso(),
    ...normalizeComposerPolicyBinding(composerPolicyBinding),
  }
  ...
  const publicTopic = stripComposerPolicyBinding(t)
  await writeChange({ rev, kind: 'topic', id: topicId, data: publicTopic, ts: t.ts })
  await patchSnapshot({ rev, patch: { topics: { [topicId]: publicTopic } } })
  return { topic: publicTopic, rev }
}

async function createPost({ topicId, parentId, text, userId, nickname, icon, posterUrl = '', videoModerationStatus = '', ts, _geoOrigin = null, composerPolicyBinding = null }) {
  const database = await db()
  const principalId = await resolveForumPrincipal(database, userId)
  if (!principalId) throw new Error('missing_user_id')
  ...
  const p = {
    ...
    nickname: str(nickname),
    icon: str(icon),
    ...(str(posterUrl) ? { posterUrl: str(posterUrl) } : {}),
    ...(['pending', 'approved'].includes(str(videoModerationStatus).toLowerCase())
      ? { videoModerationStatus: str(videoModerationStatus).toLowerCase() }
      : {}),
    isAdmin: '0',
    likes: 0,
    dislikes: 0,
  ...
  const publicPost = stripComposerPolicyBinding(p)
  await writeChange({ rev, kind: 'post', id: postId, data: publicPost, ts: p.ts })
  await patchSnapshot({
    rev,
    patch: {
      topics: topic ? { [tid]: topic } : {},
      posts: { [postId]: publicPost },
    },
  })
  return { post: publicPost, rev }
}

async function setPostVideoModerationStatus(postId, status) {
  const database = await db()
  const pid = str(postId)
  const nextStatus = str(status).toLowerCase()
  if (!pid || !['pending', 'approved'].includes(nextStatus)) throw new Error('bad_video_moderation_status')
  const updatedAt = nowIso()
  const result = await database.collection('forum_core_posts').findOneAndUpdate(
    { _id: `post:${pid}` },
    { $set: { videoModerationStatus: nextStatus, updatedAt, storagePrimary: 'mongo' } },
    { returnDocument: 'after' },
  ).catch(() => null)
  const post = readPostFromDoc(result?.value || result)
    || readPostFromDoc(await database.collection('forum_core_posts').findOne({ _id: `post:${pid}` }).catch(() => null))
  if (!post) return null
  const rev = await nextRev()
  await writeChange({ rev, kind: 'post', id: pid, data: post, ts: now() })
...
async function updatePostText(postId, text, { composerPolicyBinding = null } = {}) {
  const database = await db()
  const pid = str(postId)
  if (!pid) return { post: null, notFound: true }
  const result = await database.collection('forum_core_posts').findOneAndUpdate(
    { _id: `post:${pid}` },
    { $set: { text: str(text), tsEdited: now(), updatedAt: nowIso(), storagePrimary: 'mongo', ...normalizeComposerPolicyBinding(composerPolicyBinding) } },
    { returnDocument: 'after' },
  )
  const post = readPostFromDoc(result?.value || result)
...
async function findReactionForIdentity(database, postId, userId) {
  const pid = str(postId)
  const principalId = await resolveForumPrincipal(database, userId)
  if (!pid || !principalId) return { principalId: '', existing: null }

  const linked = await getLinkedIdentityIds(database, principalId).catch(() => [principalId])
  const identityIds = unique([principalId, userId, ...linked])
  const keys = identityIds.map((id) => `reaction:${pid}:${id}`)
  const rows = await database.collection('forum_post_reactions')
    .find({
      $or: [
        { _id: { $in: keys } },
        { postId: pid, userId: { $in: identityIds } },
      ],
    })
...
async function setReaction({ postId, userId, kind }) {
  const pid = str(postId)
  if (!pid || !str(userId)) throw new Error('bad_react_args')

  const database = await db()
  const { principalId: uid, existing } = await findReactionForIdentity(database, pid, userId)
  if (!uid) throw new Error('bad_react_args')

  const target = kind === 'dislike' ? 'dislike' : 'like'
  const key = str(existing?._id) || `reaction:${pid}:${uid}`
  ...
  // If a legacy alias-keyed reaction already exists, update that physical row
  // in place until Mongo compaction. Its logical owner becomes canonical now,
  // avoiding both counter duplication and a second reaction document.
...
async function reconcileForumUserStats(database, userIds = []) {
  ...
  for (const seed of seeds) {
    ...
    const writeId = await resolveForumPrincipal(database, seed)
    if (writeId) {
      await database.collection('forum_user_stats').updateOne(
        { _id: writeId },
        {
          $set: {
            canonicalAuthorId: writeId,
            stats,
            updatedAt: nowIso(),
            storagePrimary: 'mongo',
          },
...
function preferredIdentityKey(ids = [], fallback = '') {
  ...
  return canonicalUserId.normalizePrincipalSyntax(list[0] || fallback)
}

async function resolveForumPrincipal(database, raw) {
  const seed = canonicalUserId.normalizePrincipalSyntax(raw)
  if (!seed) return ''
  const linked = await getLinkedIdentityIds(database, seed).catch(() => [seed])
  return preferredIdentityKey(linked, seed) || seed
}
...
async function readSubscriptionRowsForIdentity(userId, mode = 'followers') {
  ...
  const principalOwnerId = preferredIdentityKey(identityIds, uid)
  const canonicalDocId = principalOwnerId
    ? `${safeMode === 'following' ? 'followingZ' : 'followersZ'}:${principalOwnerId}`
    : ''
  const canonicalDocs = canonicalDocId
    ? docs.filter((doc) => str(doc?._id) === canonicalDocId)
    : []

  // Transitional authority rule:
  // before the first canonical mutation we union legacy rows so old Mongo stays
  // readable. Once the canonical Z document exists, it is authoritative and
  // stale alias-owned rows can no longer resurrect an unsubscribed relation.
  const sourceDocs = canonicalDocs.length ? canonicalDocs : docs
  const byKey = new Map()
...
function subscriptionWriteIds(primary, aliases = []) {
  // Stage 1-4: permanent writes use one principal only. Aliases remain a READ concern.
  const candidate = preferredIdentityKey([primary, ...(Array.isArray(aliases) ? aliases : [])], primary)
...
async function writeSubscriptionRelationDocs({ ownerIds = [], mode = 'followers', members = [], rows = null } = {}) {
  const safeMode = mode === 'following' ? 'following' : 'followers'
  const ownerId = subscriptionWriteIds('', ownerIds)[0] || ''
  if (!ownerId) return

  const cleanMembers = unique(members.map((member) => preferredIdentityKey([member], member)).filter(Boolean))
  const cleanRows = Array.isArray(rows)
    ? rows
      .map((row) => ({ ...row, member: preferredIdentityKey([row?.member], row?.member) }))
      .filter((row) => row.member)
    : rankedRows(cleanMembers)

  if (safeMode === 'following') {
    await writeSetDoc(`followingZ:${ownerId}`, cleanMembers, cleanRows)
    return
  }

  await Promise.all([
    writeSetDoc(`followersZ:${ownerId}`, cleanMembers, cleanRows),
    setCountDoc(`followers:${ownerId}`, cleanMembers.length),
  ])
}
...
async function toggleSubscription(viewerId, authorId) {
  ...
  const [viewer, author] = await Promise.all([
    resolveForumPrincipal(database, viewerId),
    resolveForumPrincipal(database, authorId),
  ])
  ...
  const followingMembers = await Promise.all(
    viewerRows.rows.map((row) => canonicalSubscriptionMember(database, row.member)),
  )
  const followerMembers = await Promise.all(
    followerRows.rows.map((row) => canonicalSubscriptionMember(database, row.member)),
  )

  const following = new Set(unique(followingMembers).filter((id) => id && id !== viewer))
  const followers = new Set(unique(followerMembers).filter((id) => id && id !== author))
  const subscribed = !following.has(author)
...
async function setBannedUser(accountId, banned) {
  const database = await db()
  const id = await resolveForumPrincipal(database, accountId)
  if (!id) return { rev: await readCounter('forum:rev', 0) }

  const doc = await readSetDoc('banned_users')
  const set = new Map()
  for (const value of unique(doc?.members)) {
    const key = preferredIdentityKey([value], value)
    if (key) set.set(key, key)
  }

  if (banned) set.set(id, id)
  else set.delete(id)

  const members = Array.from(set.values())
  await writeSetDoc('banned_users', members)
  const rev = await nextRev()
  await writeChange({ rev, kind: banned ? 'ban' : 'unban', id, ts: now() })
  await patchSnapshot({ rev, patch: { banned: members } })
  return { rev }
}
...
async function reportPost({ postId, reporterId, reason, thresholds = {} } = {}) {
  const pid = str(postId)
  const rawReporterId = str(reporterId)
  const rsn = str(reason).toLowerCase()
  if (!pid || !rawReporterId) {
  ...
  const database = await db()
  const rid = await resolveForumPrincipal(database, rawReporterId)
  if (!rid) {
    const err = new Error('bad_request')
    err.status = 400
    throw err
...
module.exports = {
  ...
  readCounter,
  readSnapshot,
  readPublicPostMutationChangesSince,
  getMediaLockRecord,
  setMediaLockRecord,
  rebuildSnapshot,
  reportPost,
