...
function buildThreadPagePlan(input = {}) {
  ...
  const sort = normalizeUserBranchSort(input.sort || 'new')
  const threadCursorFilter = cursorFilter(sort, input.cursor, idField)
  const threadSort = sortObject(sort, idField)
  const threadHint = sort === 'new' ? 'ql7_thread_children_new_v1' : ''
  if (mode === 'topic_roots') {
    return {
      collection: FORUM_INDEX_COLLECTIONS.thread,
      query: mergeQuery({ topicId, parentId: null }, boolDeletedFilter(), threadCursorFilter),
      sort: threadSort,
      limit: pageSize,
      hint: threadHint,
  ...
  return {
    collection: FORUM_INDEX_COLLECTIONS.thread,
    query: mergeQuery({ topicId, parentId }, boolDeletedFilter(), threadCursorFilter),
    sort: threadSort,
    limit: pageSize,
    hint: threadHint,
    cursorDomain: 'forum-thread-v1',
