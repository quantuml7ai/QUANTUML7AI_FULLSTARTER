...
function examIdentityClauses(userId, blockId, aliases = []) {
  const uid = str(userId)
  const block = str(blockId)
  ...
  return { uid, block, ids, clauses }
}

async function readExamState({ userId, blockId, aliases = [] } = {}) {
  const { uid, block, clauses } = examIdentityClauses(userId, blockId, aliases)
  if (!uid || !block) return {}
...
async function writeExamState({ userId, blockId, state = {}, aliases = [] } = {}) {
  const doc = buildDoc({ userId, blockId, state })
  const database = await db()
  const { clauses } = examIdentityClauses(userId, blockId, aliases)
  ...
  const existing = rows.find((row) => (
    str(row?._id) === doc._id ||
    str(row?.examId) === doc.examId ||
    (str(row?.userId) === doc.userId && str(row?.blockId) === doc.blockId)
  )) || rows[0] || null
  const setDoc = { ...doc }
  delete setDoc._id
  await database.collection(COLLECTION).updateOne(
    existing?._id ? { _id: existing._id } : { _id: doc._id },
