...
function normalizeComposerPolicyBinding(value) {
  ...
  if (!decisionId || !/^[a-f0-9]{64}$/u.test(actorHash) || !/^[a-f0-9]{64}$/u.test(intentHash)) {
    throw new Error('battlecoin_chat_composer_policy_binding_invalid')
  }
  return {
...
async function profileFor(accountId) {
  ...
  if (!id) return {}
  try {
    if (typeof profilePrimary.getUserProfile === 'function') {
      return await profilePrimary.getUserProfile(id)
    }
...
function hydrateAuthorSnapshot(accountId, storedAuthor, profile = {}, vipState = {}) {
  ...
  if (
    liveAuthor?.kind === 'profile' &&
    (
      storedAuthor?.kind === 'anonymous' ||
      !str(storedAuthor?.nickname) ||
      isTechnicalBattleChatNickname(storedAuthor?.nickname)
    )
  ) {
    return liveAuthor
  }
...
async function hydrateMessages(database, docs = [], viewerAccountId = '', viewerIdentityIds = []) {
  ...
  const viewerIds = identitySet([viewerAccountId, ...viewerIdentityIds])
  if (viewerIds.length && ids.length) {
    const likes = await database.collection(LIKES).find({
      messageId: { $in: ids },
      accountId: { $in: viewerIds },
      status: 'liked',
...
async function listBattleChatMessages({ limit = 100, viewerAccountId = '', viewerIdentityIds = [] } = {}) {
  ...
  const messages = await hydrateMessages(database, ordered, viewerAccountId, viewerIdentityIds)
...
async function listBattleChatDelta({ since = '', viewerAccountId = '', viewerIdentityIds = [], limit = 100 } = {}) {
  ...
  const docs = await database.collection(MESSAGES)
    ...
    .toArray()
    .catch(() => [])
  const messages = await hydrateMessages(database, docs || [], viewerAccountId, viewerIdentityIds)
...
async function readSenderState(database, accountId, identityIds = []) {
  const id = str(accountId)
  if (!id) return null
  const ids = identitySet([id, ...identityIds])
  const rows = await database.collection(SENDER_STATE)
    .find({ accountId: { $in: ids } })
    .sort({ updatedAtMs: -1, createdAtMs: -1 })
    .limit(100)
    .toArray()
    .catch(() => [])
  if (!rows.length) return null

  const physical = rows.find((row) => str(row?.accountId) === id) || rows[0]
  const nextAllowedAtMs = Math.max(0, ...rows.map((row) => num(row?.nextAllowedAtMs, 0)))
  const sessionCount = Math.max(0, ...rows.map((row) => Math.floor(num(row?.sessionCount, 0))))
  const messageWindowId = Math.max(-1, ...rows.map((row) => Math.floor(num(row?.messageWindowId, -1))))
  const messageWindowCount = Math.max(
    0,
    ...rows
      .filter((row) => Math.floor(num(row?.messageWindowId, -1)) === messageWindowId)
      .map((row) => Math.floor(num(row?.messageWindowCount, 0))),
  )

  // Transitional aliases can leave several physical protective-state rows.
  // Reuse one row for writes, but never let a permissive canonical row weaken a
  // stricter legacy cooldown/rate/session state while compaction is pending.
  return {
    ...physical,
    nextAllowedAtMs,
    sessionCount,
...
async function writeSenderState(database, accountId, patch = {}, identityIds = []) {
  const id = str(accountId)
  if (!id) return null
  const existing = await readSenderState(database, id, identityIds)
...
async function assertSenderAllowed(database, accountId, now, identityIds = []) {
  const state = await readSenderState(database, accountId, identityIds)
...
async function sendBattleChatMessage({ actor, text, clientMutationId = '', composerPolicyBinding = null, now = Date.now() } = {}) {
  ...
  if (mutationId) {
    ...
    if (existing) {
      const [message] = await hydrateMessages(database, [existing], accountId, actor?.identityIds || [])
      return { ok: true, duplicate: true, message, syncToken: syncTokenFromDoc(existing), storagePrimary: 'mongo' }
    }
  }

  const allowed = await assertSenderAllowed(database, accountId, now, actor?.identityIds || [])
  ...
  const policyBinding = normalizeComposerPolicyBinding(composerPolicyBinding)
  ...
  const [message] = await hydrateMessages(database, [doc], accountId, actor?.identityIds || [])
  return {
    ok: true,
    message,
...
async function toggleBattleChatLike({ actor, messageId, like = true, now = Date.now() } = {}) {
  const accountId = str(actor?.accountId)
  const actorIds = identitySet([accountId, ...(actor?.identityIds || [])])
  ...
  if (like) {
    const existing = actorIds.length
      ? await database.collection(LIKES).findOne({ messageId: id, accountId: { $in: actorIds }, status: 'liked' }).catch(() => null)
      : null
    if (existing) {
      delta = 0
    } else {
    ...
    const result = await database.collection(LIKES).deleteMany({ messageId: id, accountId: { $in: actorIds } }).catch(() => ({ deletedCount: 0 }))
    if (result?.deletedCount) delta = -Math.max(1, Math.floor(num(result.deletedCount, 0)))
  }
  ...
  const [publicRow] = await hydrateMessages(database, [nextDoc], accountId, actor?.identityIds || [])
  return {
    ok: true,
    message: publicRow,
    liked: !!like,
