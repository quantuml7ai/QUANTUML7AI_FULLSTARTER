...
async function resolveDmPrincipal(raw) {
  const syntax = canonicalUserId.normalizePrincipalSyntax(raw)
  if (!syntax) return ''
  if (testDatabase) return syntax
  const canonical = await profilePrimary.resolveCanonicalAccountId(syntax).catch(() => '')
  return canonical || syntax
}

async function resolveDmPrincipals(values = []) {
  const out = []
  for (const value of unique(values)) {
    const principal = await resolveDmPrincipal(value)
...
function operationOptions(options = {}, base = {}) {
  return options?.session ? { ...base, session: options.session } : base
}

async function db(options = {}) {
  if (options?.database && typeof options.database.collection === 'function') return options.database
...
async function nextMsgId(options = {}) {
  const database = await db(options)
  const result = await database.collection('dm_counters').findOneAndUpdate(
    ...
    operationOptions(options, { upsert: true, returnDocument: 'after' }),
  )
  const value = num(result?.value?.value ?? result?.value, 0)
  if (value > 0) return String(value)
  const doc = await database.collection('dm_counters').findOne(
    { _id: 'dm:seq' },
...
async function saveMessage(msg, options = {}) {
  ...
  const [from, to] = await Promise.all([
    resolveDmPrincipal(msg?.from),
    resolveDmPrincipal(msg?.to),
  ])
  if (!from || !to) throw new Error('missing_user_id')
  ...
  await database.collection('dm_messages').updateOne(
    ...
    operationOptions(options, { upsert: true }),
  )
  return clean
}
...
async function setMessageVideoModerationStatus(msgId, status) {
  const id = str(msgId)
  const nextStatus = str(status).toLowerCase()
  if (!id || !['pending', 'approved'].includes(nextStatus)) throw new Error('bad_video_moderation_status')
  const database = await db()
  const doc = await database.collection('dm_messages').findOne({
    $or: [{ _id: `message:${id}` }, { messageId: id }, { id }],
  })
  if (!doc) return null
  const raw = doc?.raw && typeof doc.raw === 'object' ? jsonClone(doc.raw) : jsonClone(doc)
  if (!raw) return null
  const attachments = (Array.isArray(raw.attachments) ? raw.attachments : []).map((entry) => {
    ...
    return isVideo ? { ...entry, moderationStatus: nextStatus } : entry
  })
  raw.attachments = attachments
  const updatedAt = nowIso()
  await database.collection('dm_messages').updateOne(
    { messageId: id },
    { $set: { attachments: jsonClone(attachments), raw, updatedAt, storagePrimary: 'mongo' } },
  )
  return { ...raw, id, attachments }
}
...
async function addAliasesFor(primary, aliases = [], options = {}) {
  // Stage 1-4 stop-the-bleeding shim. Legacy dm_aliases remains READ-only
  // until the manifest-driven Mongo compaction.
  void primary
  void aliases
  void options
  return false
...
async function expandAliasIds(ids = [], options = {}) {
  const base = new Set(unique(ids))
  const seed = Array.from(base)
  if (!seed.length) return seed
  const database = await db(options)

  // New link source: profile walletId <-> telegramId. Keep this read-only.
  if (!testDatabase) {
    for (const value of seed) {
      const linked = await profilePrimary.getLinkedIdentityIds(value).catch(() => [])
      for (const id of linked || []) {
        const clean = str(id)
        if (clean) base.add(clean)
      }
    }
  }

  const expandedSeed = Array.from(base)
  const docs = await database.collection('dm_aliases').find({
    $or: [
      { uid: { $in: expandedSeed } },
      { userId: { $in: expandedSeed } },
      { canonicalId: { $in: expandedSeed } },
      { alias: { $in: expandedSeed } },
      { aliasId: { $in: expandedSeed } },
    ],
  }, operationOptions(options)).limit(1000).toArray()
...
async function addMessageIndexes({ msg, fromIds = [], toIds = [], score } = {}, options = {}) {
  const id = str(msg?.id)
  if (!id) throw new Error('bad_id')
  const database = await db(options)
  void fromIds
  void toIds
  const from = await resolveDmPrincipal(msg?.from)
  const to = await resolveDmPrincipal(msg?.to)
  const fromList = from ? [from] : []
  const toList = to ? [to] : []
  ...
  if (mailboxOps.length) await database.collection('dm_mailbox_entries').bulkWrite(
    mailboxOps,
    operationOptions(options, { ordered: false }),
  )
  ...
  if (threadOps.length) await database.collection('dm_thread_entries').bulkWrite(
    threadOps,
    operationOptions(options, { ordered: false }),
  )
...
async function setBlock(blockerId, blockedIds = []) {
  const blocker = await resolveDmPrincipal(blockerId)
  const blocked = await resolveDmPrincipals(blockedIds)
...
async function markSeenForPairs(meIds = [], peerIds = [], lastSeenTs = 0) {
  const meList = await resolveDmPrincipals(meIds)
  const peerList = await resolveDmPrincipals(peerIds)
...
async function listIncomingMessageIdsForSeenWindow(meIds = [], peerIds = [], afterTs = 0, untilTs = 0) {
  const meList = unique(meIds)
  const peerList = unique(peerIds)
  const after = num(afterTs, 0)
  const until = num(untilTs, 0)
  if (!meList.length || !peerList.length || !until || until <= after) return []
  const database = await db()
  const rows = await database.collection('dm_messages').find({
    from: { $in: peerList },
    to: { $in: meList },
    ts: { $gt: after, $lte: until },
  }).sort({ ts: 1 }).limit(500).toArray().catch(() => [])
  return unique((rows || []).map((row) => row?.messageId || row?.id || row?._id))
}
...
async function deleteDialog({ meIds = [], withIds = [], deletedAt = Date.now() } = {}) {
  ...
  const iso = nowIso()
  const canonicalMeList = await resolveDmPrincipals(meList)
  const canonicalWithList = await resolveDmPrincipals(withList)
  const tombstoneOps = []
  for (const uid of canonicalMeList) {
    for (const peerId of canonicalWithList) {
      tombstoneOps.push({
        updateOne: {
          filter: { uid, peerId },
  ...
  for (const uid of canonicalWithList) {
    for (const peerId of canonicalMeList) {
      tombstoneOps.push({
        updateOne: {
          filter: { uid, peerId },
          update: { $max: { deletedAt: at, deletedTs: at, ts: at }, $set: { uid, userId: uid, peerId, updatedAt: iso, storagePrimary: 'mongo' }, $setOnInsert: { _id: `deleted:${uid}:${peerId}`, createdAt: iso } },
...
module.exports = {
  ...
  getMessage,
  isBlockedBy,
  listIncomingMessageIdsForSeenWindow,
  markDelivered,
