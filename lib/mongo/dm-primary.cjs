  ...
  for (const uid of toList) {
    mailboxOps.push({
      updateOne: {
        ...
        update: { $set: { uid, box: 'inbox', messageId: id, score: s, updatedAt: at, storagePrimary: 'mongo' }, $setOnInsert: { createdAt: at } },
        upsert: true,
      },
    })
  }
  for (const uid of fromList) {
    mailboxOps.push({
      updateOne: {
        ...
        update: { $set: { uid, box: 'outbox', messageId: id, score: s, updatedAt: at, storagePrimary: 'mongo' }, $setOnInsert: { createdAt: at } },
        upsert: true,
      },
    })
  }
    ...
    for (const b of toList) {
      ...
      threadOps.push({
        updateOne: {
          ...
          update: { $set: { threadKey: key, messageId: id, score: s, updatedAt: at, storagePrimary: 'mongo' }, $setOnInsert: { createdAt: at } },
          upsert: true,
        },
      })
    }
