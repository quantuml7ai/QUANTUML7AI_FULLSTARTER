...
function idsToSupportHashes(ids = []) { return uniq(ids).map((id) => sha(lc(id))) }
...
function makeBasePlans(ids = [], deletedTopicIds = [], deletedPostIds = []) {
  ...
  return [
    ...
    { name: 'ql7_support_learning_candidates', filter: exactAny(['userId', 'uid', 'accountId', 'canonicalAccountId'], ids) },
    { name: 'support_email_outbox', filter: exactAny(['userId', 'uid', 'accountId', 'canonicalAccountId'], ids) },
    { name: 'ql7_support_turn_decisions_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_dialogue_outcomes_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_action_outcomes_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_translation_outcomes_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_response_quality_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_personality_state_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
    { name: 'ql7_support_user_adaptation_v11', filter: { userIdHash: { $in: idsToSupportHashes(ids) } } },
