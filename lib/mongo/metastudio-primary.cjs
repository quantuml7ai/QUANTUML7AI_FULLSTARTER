...
function registrationIdentityIds(accountId, legacyAccountIds = []) {
  const ids = new Set()
  for (const raw of [accountId, ...(Array.isArray(legacyAccountIds) ? legacyAccountIds : [])]) {
    const value = str(raw)
    if (!value) continue
    ids.add(value)
    const syntax = canonicalUserId.normalizePrincipalSyntax(value)
...
async function findRegistration(accountId, legacyAccountIds = []) {
  const ids = registrationIdentityIds(accountId, legacyAccountIds)
  if (!ids.length) return null
  const database = await db()
  return database.collection('metastudio_registrations').findOne({
    $or: [{ accountId: { $in: ids } }, { _id: { $in: ids } }],
  }).catch(() => null)
}

async function readRegistration(accountId, legacyAccountIds = []) {
  const doc = await findRegistration(accountId, legacyAccountIds)
  return doc ? clone(doc) : null
}

async function upsertRegistration(accountId, patch = {}, { legacyAccountIds = [] } = {}) {
  const id = canonicalUserId.normalizePrincipalSyntax(accountId)
  if (!id) throw new Error('metastudio_account_id_required')
  const database = await db()
  const existing = await findRegistration(id, legacyAccountIds)
  ...
  const identityIds = registrationIdentityIds(id, legacyAccountIds)
  const existingLatest = await database.collection('metastudio_registration_latest').findOne({
    $or: [{ accountId: { $in: identityIds } }, { _id: { $in: identityIds } }],
  }).catch(() => null)
  await database.collection('metastudio_registration_latest').updateOne(
    existingLatest?._id ? { _id: existingLatest._id } : { accountId: id },
    {
      $set: {
        accountId: id,
