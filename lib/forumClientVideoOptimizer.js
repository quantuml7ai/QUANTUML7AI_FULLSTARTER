...
import { executeForumVideoMobileWorker } from './forumClientVideoWorkerBridge'
const MIB = 1024 * 1024
...
export const FORUM_CLIENT_VIDEO_OPTIMIZER_MIN_TARGET_BYTES = 1 * MIB
...
export function selectForumVideoEncodingProfile({
  ...
  // A compact source may legitimately be smaller than the bitrate budget
  // required by every calibrated streaming profile. Keep the compact target
  // when it is already safe; only if every profile rejected it, retry once
  // against the canonical duration-based policy target without lowering any
  // quality floor. sourceBytes: 0 disables only the compact-source shrink.
  const sourceSize = Number(sourceBytes || 0)
  if (Number.isFinite(sourceSize) && sourceSize > 0) {
    const policyTargetOutputBytes = calculateForumVideoAdaptiveTargetBytes({
      durationSeconds,
      sourceBytes: 0,
      preferredTargetBytes,
      minTargetBytes,
    ...
    if (policyTargetOutputBytes > targetOutputBytes) {
      return selectForumVideoEncodingProfile({
        durationSeconds,
        sourceBytes: 0,
        displayWidth,
        displayHeight,
...
async function classifyPreviewVideoCodec(mediabunny, file, options) {
  const input = createInput(mediabunny, file, options.inputCacheBytes)

  try {
    const readable = await input.canRead()
    if (!readable) return { classified: false, hevcSource: false, reason: 'unreadable-by-mediabunny' }

    const videoTrack = await input.getPrimaryVideoTrack()
    if (!videoTrack) return { classified: false, hevcSource: false, reason: 'no-video-track' }

    const videoCodec = await videoTrack.getCodec()
    const videoCodecString = await videoTrack.getCodecParameterString()
    const hevcSource = isQl7HevcCodec(videoCodec, videoCodecString)
    ...
    // Selection-time classification must never make a legacy non-HEVC preview path stricter.
    // Full fail-closed validation still happens in prepareForumVideoForUpload() on Send/Create.
    return {
      classified: false,
      hevcSource: false,
...
async function inspectSource(mediabunny, file, options) {
  ...
  try {
    ...
    const videoCodec = await videoTrack.getCodec()
    const videoCodecString = await videoTrack.getCodecParameterString()
    const videoDecoderConfig = await videoTrack.getDecoderConfig().catch(() => null)
    const hevcSource = isQl7HevcCodec(videoCodec, videoCodecString || videoDecoderConfig?.codec)
    const nativeVideoDecodable = hevcSource && videoDecoderConfig
      ? await probeQl7NativeVideoDecoder(videoDecoderConfig)
      : await videoTrack.canDecode()
    const colorSpace = await videoTrack.getColorSpace().catch(() => null)
    const hevcFallbackEligible = isQl7HevcFallbackEligible({
      codec: videoCodec,
    ...
    return {
      ...
      videoCodecString,
      videoDecoderConfig,
      nativeVideoDecodable,
      videoDecodable: nativeVideoDecodable,
      hevcSource,
      hevcFallbackEligible,
      hevcFallbackActive: false,
      colorSpace,
      sourceFrameRate,
      audioCodec,
      audioDecodable,
...
export async function prepareForumVideoForUpload({
  ...
  // Upload bypass is valid only for a file that has already completed the full
  // decode -> normalize -> AVC/AAC encode -> verification pipeline in this runtime.
  // Container/source compatibility alone is never sufficient.
  if (existingProof?.source === 'optimizer-output') {
    return {
      file,
  ...
  // Every video upload must pass the full transcoder at least once.
  // Do not short-circuit canonical MP4 sources or copy-remux results: those paths
  // do not prove browser-decodable frames for post-commit moderation and do not
  // normalize MOV/HEVC/H.265 sources into the calibrated AVC/AAC output contract.
...
async function acquireHevcFallbackWhenRequired(mediabunny, metadata) {
  if (metadata?.nativeVideoDecodable || !metadata?.hevcSource) return { metadata, lease: null }
  if (!metadata?.hevcFallbackEligible || !metadata?.videoDecoderConfig) {
    return { metadata, lease: null }
  }

  const lease = acquireQl7HevcFallbackConfig(mediabunny, metadata.videoDecoderConfig, {
  ...
  return {
    ...
    metadata: {
      ...metadata,
      videoDecodable: true,
      hevcFallbackActive: true,
    },
  }
}

export async function prepareForumVideoPreviewIfNeeded(file, options = {}) {
  ensureBrowserRuntime()
  const opts = normalizeOptions(options)
  ...
  // R22 iPhone boundary: Photos/Telegram can hand the web input an AVC export
  // even when the library asset was HEVC. On Apple mobile we therefore inspect
  // the actual selected bytes before deciding whether a raw preview is safe.
  // Other runtimes preserve the lightweight HEVC-only classification path.
  let classification = null
  let pressure = null
  if (runtime.appleMobile) {
    classification = await inspectSource(mediabunny, file, opts)
    pressure = classifyForumVideoProcessingPressure(classification, Number(file.size || 0))
  } else {
    classification = await classifyPreviewVideoCodec(mediabunny, file, opts)
  }
  throwIfAborted(signal)

  const appleAvcPressure = runtime.appleMobile === true && pressure?.appleAvcPressureCandidate === true
  if (!classification.hevcSource && !appleAvcPressure) {
    return {
      ...
      pressure,
      optimized: false,
      decoderPath: 'deferred-existing-path',
    }
  }

  const optimized = await optimizeForumVideoOnDevice(file, opts)
  return {
    ...
    optimized: true,
    result: optimized,
    decoderPath: optimized.decoderPath || 'native-mediabunny',
  }
}
...
export async function optimizeForumVideoOnDevice(file, options = {}) {
  ...
  try {
    const mediabunny = await loadMediabunny()
    let metadata = await inspectSource(mediabunny, file, opts)
    const runtime = detectForumVideoRuntime()
    const pressure = classifyForumVideoProcessingPressure(metadata, Number(file.size || 0))
    const executor = selectForumVideoExecutor({ runtime, pressure })
    const mobileWorker = executor === 'mobile-worker-opfs'

    if (executor === 'mobile-safe-unavailable') {
      fail(
        'VIDEO_OPTIMIZER_MOBILE_SAFE_RUNTIME_REQUIRED',
        'This mobile device cannot safely process this high-pressure video locally.',
        { runtime, pressure },
      )
    }

    if (!mobileWorker) {
      const fallback = await acquireHevcFallbackWhenRequired(mediabunny, metadata)
      metadata = fallback.metadata
      fallbackLease = fallback.lease
    }
    ...
    if (!metadata.videoDecodable && !mobileWorker) {
      fail(
    ...
    let resolvedHardwareAcceleration = opts.hardwareAcceleration
    ...
    let videoEncodable = await mediabunny.canEncodeVideo('avc', videoEncoderConfig)

    // Apple WebCodecs can expose AVC encode support under one acceleration hint
    // while rejecting another. Probe bounded alternatives before declaring the
    // local streaming path unavailable; the output codec/size/quality plan is
    // unchanged.
    if (!videoEncodable && mobileWorker && runtime.appleMobile) {
      for (const candidate of ['prefer-hardware', 'prefer-software']) {
        if (candidate === resolvedHardwareAcceleration) continue
        const candidateConfig = { ...videoEncoderConfig, hardwareAcceleration: candidate }
        if (!(await mediabunny.canEncodeVideo('avc', candidateConfig))) continue
        resolvedHardwareAcceleration = candidate
        videoEncoderConfig = candidateConfig
        videoEncodable = true
        break
      }
    }
    ...
    const plan = {
      ...
      runtime: {
        ...
        appleMobile: runtime.appleMobile,
        androidMobile: runtime.androidMobile,
        telegramWebView: runtime.telegramWebView,
      },
      pressure,
      decoderPath: mobileWorker
        ? 'mobile-worker-probe'
        : (metadata.hevcFallbackActive ? QL7_HEVC_FALLBACK_ID : 'native-mediabunny'),
    }
    ...
    let resultBlob = null
    let decoderPath = metadata.hevcFallbackActive ? QL7_HEVC_FALLBACK_ID : 'native-mediabunny'
    const attempts = []
    ...
    if (mobileWorker) {
      emitProgress(onProgress, {
        ...
        internalStage: 'worker-start',
      })
      const mobileResult = await executeForumVideoMobileWorker({
        file,
        signal,
        onProgress,
        spec: {
          ...
          hardwareAcceleration: resolvedHardwareAcceleration,
          appleMobile: runtime.appleMobile,
          appleAvcPressure: runtime.appleMobile === true && pressure?.appleAvcPressureCandidate === true,
          transientCodecRetries: runtime.appleMobile === true && pressure?.appleAvcPressureCandidate === true ? 2 : 0,
          stepSeconds: runtime.appleMobile ? QL7_MOBILE_VIDEO_APPLE_STEP_SECONDS : QL7_MOBILE_VIDEO_STEP_SECONDS,
          inputCacheBytes: runtime.appleMobile
            ? Math.min(opts.inputCacheBytes, QL7_MOBILE_VIDEO_APPLE_INPUT_CACHE_BYTES)
            : opts.inputCacheBytes,
        },
      })
      resultBlob = new Blob([mobileResult.buffer], { type: 'video/mp4' })
      attempts.push(...mobileResult.attempts)
      decoderPath = mobileResult.decoderPath || 'native-mediabunny'
    } else {
    ...
    const result = {
      ...
      executor: mobileWorker ? QL7_MOBILE_VIDEO_EXECUTOR_ID : 'desktop-buffer',
      decoderPath,
    }
