...
export async function prepareForumVideoForUpload({
  ...
  if (mp4Candidate) {
    ...
    const {
      flatFastStart,
      ...canonicalChecksExceptFastStart
    } = canonical.checks || {}

    const copyRemuxEligible =
      flatFastStart === false &&
      Object.values(canonicalChecksExceptFastStart).every(Boolean)

    if (copyRemuxEligible) {
      const remuxed = await optimizeForumVideoFastStart(file, {
        signal,
        strictFlatFaststart: false,
        onProgress: (progress, detail = {}) => {
          emitProgress(onProgress, {
            stage: 'remuxing',
            progress: Math.max(0, Math.min(1, Number(progress || 0))),
            pipeline: String(detail?.stage || ''),
          })
        },
      })

      if (
        remuxed?.flatFaststart === true &&
        remuxed?.blob instanceof Blob &&
        Number(remuxed.blob.size || 0) > 0
      ) {
        const preparedName = String(
          filename || file?.name || 'video.mp4'
        ).replace(/\.[^.]+$/, '.mp4')

        const preparedFile =
          typeof File === 'function'
            ? new File([remuxed.blob], preparedName, {
                type: 'video/mp4',
                lastModified: Number(file?.lastModified || Date.now()),
              })
            : remuxed.blob

        const remuxCanonical = await inspectCanonicalUploadCandidate(
          preparedFile,
          metadata,
          options,
        )

        if (remuxCanonical.ok) {
          const proof = registerVerifiedVideoUpload(preparedFile, {
            source: 'canonical-remux-copy',
            durationSec: metadata.durationSeconds,
            width: metadata.displayWidth,
            height: metadata.displayHeight,
            frameRate: metadata.sourceFrameRate,
            checks: remuxCanonical.checks,
            atoms: remuxCanonical.atoms,
