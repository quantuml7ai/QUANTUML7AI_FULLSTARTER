...
async function readDuration(input, videoTrack) {
  let duration = null
  try {
    duration = await videoTrack.getDurationFromMetadata({ skipLiveWait: true })
  } catch {}
...
async function inspectSource(mediabunny, file, options) {
  ...
  try {
    ...
    const durationSeconds = await readDuration(input, videoTrack)
