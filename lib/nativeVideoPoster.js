...
export async function createNativeVideoPosterFromCanvas({
  canvas,
  sourceName = '',
  timeSec = 0,
  durationSec = null,
  ...
  if (!canvas || typeof document === 'undefined') throw new Error('Native video poster canvas is required')
  const dims = resolveNativeVideoPosterDimensions(canvas.width, canvas.height, maxEdge)
  ...
  const blob = await encodePosterCanvas(target)
  ...
  const name = posterFilename(sourceName || 'video', mime)
  ...
  return {
    ...
    timeSec: Number.isFinite(Number(timeSec)) ? Number(timeSec) : 0,
    sizeBytes: blob.size,
