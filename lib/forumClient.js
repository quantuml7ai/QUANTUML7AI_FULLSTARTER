// ###########################################################################
// lib/forumClient.js — prod-ready client for /api/forum/* and /api/news
// Compatible with the new Edge routes (strict auth, credentials: 'include')
// ###########################################################################
...
// Centralized error classifier for UI
...
export const ForumAPI = {
  ...
  async createTopic({ title, category = '', tags = [], text = '', allowLinks = false, user } = {}) {
    // server expects: { title, category, tags[], text, allowLinks? }
  ...
  async listPostsLegacy({ topic, page = 1, limit = 50, sort = 'new' } = {}) {
    if (!topic) throw new Error('topic required')
    return fetchJSON(`/api/forum/posts${q({ topic, page, limit, sort })}`, { method: 'GET' })
  },
  ...
  async view({ type, id } = {}) {
    if (!type || !id) throw new Error('type & id required')
