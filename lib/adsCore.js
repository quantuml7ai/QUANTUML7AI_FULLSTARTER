...
// те же ключи, что и в lib/subscriptions.js
const ACC_KEY = (addr) => `acc:${addr}`         // HASH { tg_id: <uid> }
const TG_UID_KEY = (uid) => `tg:uid:${uid}`     // STRING "<0x...>"

// Нормализация accountId: поддерживаем tg-префиксы и любое написание адреса
...
// - если это числовой uid → ищем привязанный кошелёк в tg:uid:<uid>
async function resolveAccountIds(raw) {
  ...
  if (primary.startsWith('0x')) {
    try {
      // сначала по нормализованному адресу
      let tgId = await redis.hget(ACC_KEY(primary), 'tg_id')

      // если связь сохранялась по "сырому" checksum-адресу — пробуем и его
      if (!tgId && rawStr.toLowerCase().startsWith('0x') && rawStr.toLowerCase() !== primary) {
        tgId = await redis.hget(ACC_KEY(rawStr))
      }
...
export async function getPackagesWithCurrent(accountIdRaw) {
  ...
  if (accountId) {
    // ВНИМАНИЕ: для чтения используем сырой accountId,
    // внутри getPackagesForAccount / findActivePackageForAccount
    // уже работает resolveAccountIds
...
export async function getCabinet(accountIdRaw) {
  ...
  // Логируем активность по нормализованному ID (кошелёк или uid)
  await touchAccount(accountId)

  // А вот пакеты/кампании читаем по ВСЕМ связанным id
