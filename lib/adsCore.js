...
// Поддерживает "tguid:<id>" / "tg:<id>" / просто число / кошелёк 0x...
...
async function resolveAccountIds(raw) {
  ...
  if (primary.startsWith('0x')) {
    try {
      ...
      // если в acc: лежит checksum-адрес – пробуем и по «сырому»
  ...
  if (/^\d+$/.test(primary)) {
    try {
      ...
      // старый вариант ключа на всякий случай
      if (!addr) {
...
const ADS_PLANS = [
  {
    ...
    maxMediaPerCampaign: envNum('NEXT_PUBLIC_ADS_STARTER_MAX_MEDIA', 3),
  ...
  },
  {
    ...
    maxMediaPerCampaign: envNum('NEXT_PUBLIC_ADS_PRO_MAX_MEDIA', 10),
  ...
  },
  {
    ...
    maxMediaPerCampaign: envNum('NEXT_PUBLIC_ADS_ELITE_MAX_MEDIA', 50),
...
// Ручная/telegram-выдача пакета – может быть вызвана с кошельком или с tg uid.
// Пакет потом читается и так, и так, за счёт resolveAccountIds.
export async function grantAdsPackageForAccount({
  ...
  const raw = {
    id,
    accountId,                    // тут может лежать и tg uid, и кошелёк
...
// Собираем все пакеты по всем связанным идентификаторам (wallet + tg uid).
...
// 1. Пакеты + текущий пакет (для веба и телеги)
...
// Кабинет: текущий пакет + кампании
...
export async function createPurchase({ accountId: rawAccountId, pkgType }) {
  ...
  const invoice = {
    id: internalId,
    externalId: null,
    accountId,                  // здесь может быть И tg uid, и кошелёк
    pkgType: plan.internalName,
    amount: plan.price,
...
export async function applyPaidInvoice(invoice, nowDate = new Date()) {
  ...
  const pkg = {
    id: pkgId,
    invoiceId: invoice.id,
    accountId: invoice.accountId,      // тут опять может быть tg uid ИЛИ кошелёк
    pkgType: plan.internalName,
    status: 'active',
    maxCampaigns: plan.maxCampaigns,
    usedCampaigns: 0,
...
// 5. Links для ForumAds (учитываем мультикреатив)
export async function getLinksForForum() {
  ...
  const lines = Array.from(dedupMap.values())
...
export async function registerEvent(body, req) {
  try {
    ...
    // 2) Иначе пытаемся сопоставить по url_hash
    const hashKey =
    ...
    if (!targetCampaignIds.length && hashKey) {
      ...
      // Медленный путь для старых кампаний без urlHash
...
export async function getAnalyticsForCampaign({
  ...
  for (let ts = startBucket; ts <= endBucket; ts += step) {
    ...
    series.push({
      ts,
      label: formatBucketLabelUtc(ts, gb),
      impressions,
      clicks,
    })
  }
...
export async function deleteCampaign(campaignIdRaw) {
  ...
  // usedCampaigns не откатываем специально

  return { ok: true, deleted: true }
}

// 11. Вебхук NOWPayments (IPN)
