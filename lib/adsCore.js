...
async function resolveAccountIds(raw) {
  ...
  // --- Кошелёк → uid (+ его вариации) ---
  if (primary.startsWith('0x')) {
    const addrNorm = primary.toLowerCase()
    ids.add(addrNorm)

    // если в rawStr был checksum-адрес — добавляем его тоже
    if (rawStr && rawStr !== addrNorm) {
      ids.add(rawStr)
    }
    ...
    try {
      ...
      for (const addr of candidates) {
        if (!addr) continue
        const v = await redis.hget(`acc:${addr}`, 'tg_id')
        if (v) {
          tgId = v
          break
        }
  ...
  // --- Telegram UID → кошелёк ---
  if (/^\d+$/.test(primary)) {
    try { 
      let addr = await redis.get(`tg:uid:${primary}`)
      ...
      if (addr) {
        const addrNorm = String(addr).trim().toLowerCase()
        ids.add(addrNorm)
        ids.add(addr)
      }
    } catch {
...
export async function grantAdsPackageForAccount({
  ...
  await redis.sadd(`ads:packages:${accountId}`, id)
...
async function updatePackageUsedCampaigns(pkgKey, delta) {
  ...
  const script = `
    ...
    local pkg = cjson.decode(raw)
    local used = tonumber(pkg.usedCampaigns or 0)
    local maxC = tonumber(pkg.maxCampaigns or 0)
    local nextUsed = used + delta
    if nextUsed < 0 then nextUsed = 0 end
    if maxC > 0 and nextUsed > maxC then return -1 end
    pkg.usedCampaigns = nextUsed
    pkg.updatedAt = now
    redis.call('SET', key, cjson.encode(pkg))
    return nextUsed
  `

  try {
    const res = await redis.eval(script, [pkgKey], [String(delta), now])
    if (res === -1) return { ok: false, error: 'LIMIT_REACHED' }
    if (res === -2) return { ok: false, error: 'NO_PKG' }
    return { ok: true, used: Number(res) }
  } catch {
    // fallback below
  }
...
export async function getLinksForForum(req) {
  ...
  const viewerGeo = getViewerGeoFromRequest(req)
  const now = nowMs()
  const active = []
