...
async function getPackagesForAccount(accountIdRaw) {
  const accountId = normalizeAccountId(accountIdRaw)
  if (!accountId) return []
  const ids = await smembers(`ads:packages:${accountId}`)
  const out = []
  for (const id of ids) {
    const pkg = await getJSON(`ads:package:${id}`)
    if (pkg) out.push(pkg)
...
async function getCampaignsForAccount(accountIdRaw) {
  const accountId = normalizeAccountId(accountIdRaw)
  if (!accountId) return []
  const ids = await smembers(`ads:campaigns:${accountId}`)
  const out = []
  for (const id of ids) {
...
export async function getPackagesWithCurrent(accountIdRaw) {
  ...
  if (accountId) {
    const pkgs = await getPackagesForAccount(accountId)
    const active = await findActivePackageForAccount(accountId)
    if (active) {
...
export async function getCabinet(accountIdRaw) {
  ...
  const allPkgs = await getPackagesForAccount(accountId)
  const effectivePkg =
    pkg || (allPkgs[0] ? await hydratePackage(allPkgs[0]) : null)

  const campaigns = await getCampaignsForAccount(accountId)
