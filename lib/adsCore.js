...
async function resolveAccountIds(raw) {
  ...
  if (primary.startsWith('0x')) ids.add(primary.toLowerCase())
  ...
  for (const seed of aliasSeeds) {
    try {
      ...
      for (const alias of aliases || []) {
        const value = profilePrimary.stripPrefix(alias?.aliasValue || alias?.value || alias?.alias || '')
        if (!value) continue
        if (/^\d+$/.test(value)) addTgVariants(value)
        else ids.add(value)
        ids.add(String(value).toLowerCase())
      }
    } catch {
