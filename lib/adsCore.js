...
import { putR2Object, isR2PublicUrl } from './storage/r2.js'
import { createMediaObjectKey } from './storage/mediaKeys.js'
import adsPrimary from './mongo/ads-primary.cjs'
import profilePrimary from './mongo/profile-primary.cjs'
import { notifyQl7AdsActivated } from './ql7-support/integration/productEventBridge.js'
import economicWriterGuard from './economic-integrity/writerGuard.cjs'
...
// Local/dev ad selection fallback:
// used ONLY to decide which Mongo campaign can be served in local/dev when Vercel/Cloudflare geo headers are absent.
// It is NOT used by analytics/event geo, so metrics keep real/unknown geo as-is.
const ADS_LOCAL_DEV_SERVE_GEO_COUNTRY = (
  process.env.ADS_LOCAL_DEV_SERVE_GEO_COUNTRY ||
  process.env.ADS_DEV_GEO_COUNTRY ||
  process.env.NEXT_PUBLIC_ADS_LOCAL_DEV_SERVE_GEO_COUNTRY ||
  'UA'
).trim().toUpperCase()

const ADS_LOCAL_DEV_SERVE_GEO_REGION = (
  process.env.ADS_LOCAL_DEV_SERVE_GEO_REGION ||
  process.env.ADS_DEV_GEO_REGION ||
  process.env.NEXT_PUBLIC_ADS_LOCAL_DEV_SERVE_GEO_REGION ||
  ''
).trim().toUpperCase()

const ADS_LOCAL_DEV_SERVE_GEO_CITY = (
  process.env.ADS_LOCAL_DEV_SERVE_GEO_CITY ||
  process.env.ADS_DEV_GEO_CITY ||
  process.env.NEXT_PUBLIC_ADS_LOCAL_DEV_SERVE_GEO_CITY ||
  ''
).trim()
...
async function resolveWriteAccountId(raw) {
  const syntax = normalizeAccountId(raw)
...
// - через Mongo profile identity map добираем wallet / telegram aliases
// - добавляем legacy tg/tguid варианты, чтобы старые рекламные ключи читались без смены UX
async function resolveAccountIds(raw) {
  ...
  const addTgVariants = (uid) => {
    if (!uid) return
    ids.add(uid)
    ids.add(`tg:${uid}`)
    ids.add(`tguid:${uid}`)
    ids.add(`telegram:${uid}`)
    ids.add(`telegramid:${uid}`)
    ids.add(`telegram:id:${uid}`)
    ids.add(`tg:uid:${uid}`)
  }

  if (primary.startsWith('0x')) {
    ids.add(primary.toLowerCase())
    ids.add(`wallet:${primary.toLowerCase()}`)
  }
  if (rawStr && rawStr !== primary) ids.add(rawStr)
  if (/^\d+$/.test(primary)) addTgVariants(primary)

  try {
    const canonical = await profilePrimary.resolveCanonicalAccountId(primary)
    if (canonical) {
      const canonicalValue = String(canonical)
      ids.add(canonicalValue)
      const canonicalWallet = canonicalUserId.normalizeWalletId(canonicalValue)
      if (canonicalWallet) {
        ids.add(canonicalWallet.toLowerCase())
        ids.add(`wallet:${canonicalWallet.toLowerCase()}`)
      }
    }
  } catch {
  ...
  const aliasSeeds = Array.from(ids)
  for (const seed of aliasSeeds) {
    try {
      const aliases = await profilePrimary.listAliasesForAccount(seed)
      for (const alias of aliases || []) {
        const rawValue = alias?.aliasValue || alias?.value || alias?.alias || alias?.aliasId || ''
        const value = normalizeAccountId(profilePrimary.stripPrefix(rawValue) || rawValue)
        if (!value) continue
        if (/^\d+$/.test(value)) addTgVariants(value)
        else {
          ids.add(value)
          if (/^0x[a-f0-9]{40}$/i.test(value)) ids.add(`wallet:${value.toLowerCase()}`)
        }
      }
    } catch {
      /* alias lookup is best-effort */
    }
  }
...
// ---- Mongo-primary обёртки с прежней Redis key model ----

async function setJSON(key, value) {
  return adsPrimary.setJSON(key, value)
}

async function getJSON(key) {
  return adsPrimary.getJSON(key)
}

async function setString(key, value) {
  return adsPrimary.setString(key, value)
}

async function getString(key) {
  return adsPrimary.getString(key)
}

async function incr(key) {
  return adsPrimary.incr(key)
}

async function sadd(key, member) {
  return adsPrimary.sadd(key, member)
}

async function srem(key, member) {
  return adsPrimary.srem(key, member)
}
async function smembers(key) {
  const arr = await adsPrimary.smembers(key)
  return Array.isArray(arr) ? arr : []
}

async function delKey(key) {
  return adsPrimary.delKey(key)
}
...
async function touchAccount(accountIdRaw) {
  const accountId = await resolveWriteAccountId(accountIdRaw)
  if (!accountId) return null

  const identityIds = await resolveAccountIds(accountIdRaw)
  const candidateKeys = Array.from(new Set([
    `ads:account:${accountId}`,
    ...identityIds.map((id) => `ads:account:${id}`),
  ]))
  ...
  for (const candidateKey of candidateKeys) {
    ...
    key = candidateKey
    existing = candidate
    if (candidateKey === `ads:account:${accountId}`) break
  }
  const now = nowIso()
...
export async function grantAdsPackageForAccount({
  ...
  const accountId = await resolveWriteAccountId(rawAccountId)
  if (!accountId) {
    throw new Error('grantAdsPackageForAccount: accountId is required')
  }
  economicWriterGuard.assertEconomicWriterReceipt(decisionReceipt, {
  ...
  if (requestedPackageId && !/^pkg_[A-Za-z0-9_.:-]{8,180}$/.test(requestedPackageId)) {
    throw new Error('grantAdsPackageForAccount: invalid packageId')
  }
  const id = requestedPackageId || `pkg_${accountId}_${now}`
  ...
  await sadd(`ads:packages:${accountId}`, id)

  const hydrated = await hydratePackage(raw, plan)
  if (notify) {
    await notifyQl7AdsActivated({
      userId: accountId,
      packageName: plan.label || plan.name || plan.internalName,
      campaign: note || id,
      invoiceId: id,
      activatedAt: startsAt,
    }).catch((error) => {
      console.warn('[ql7-support:ads-activated]', error?.message || error)
    })
  }
...
export async function getAdsReadOnlySnapshot(accountIdRaw) {
  const accountId = normalizeAccountId(accountIdRaw)
  if (!accountId) {
    return { ok: false, error: 'NO_ACCOUNT', readOnly: true, source: 'adsCore' }
  }

  // This is the canonical read path used by the advertising cabinet, without
  // touchAccount() or any other business write. Support diagnostics must use
  // this function instead of independently scanning Mongo projections.
  const linkedAccountIds = await resolveAccountIds(accountIdRaw)
  const activePackage = await findActivePackageForAccount(accountIdRaw)
  const packageRows = await getPackagesForAccount(accountIdRaw)
  const packages = []
  for (const row of packageRows) packages.push(await hydratePackage(row))
  const campaigns = await getCampaignsForAccount(accountIdRaw)
  const currentPackage = activePackage || packages[0] || null
...
export async function getCabinet(accountIdRaw) {
  ...
  // Cabinet activity tracking remains a UI/business concern. Support and
  // acceptance diagnostics use getAdsReadOnlySnapshot() and never call this
  // write-bearing branch.
  await touchAccount(accountId)

  const snapshot = await getAdsReadOnlySnapshot(accountIdRaw)
  return {
    ok: snapshot.ok,
...
const successBase = `${SITE_URL}/ads` // или другой путь, который реально существует
...
const payload = {
  ...
  success_url: `${successBase}?status=success`,
  cancel_url: `${successBase}?status=cancel`,
...
export async function applyPaidInvoice(invoice, nowDate = new Date()) {
  ...
  const accountId = await resolveWriteAccountId(invoice.accountId)
  if (!accountId) return null
  ...
  await sadd(`ads:packages:${accountId}`, pkgId)
  await setString(`ads:package:byInvoice:${invoice.id}`, pkgId)

  const hydrated = await hydratePackage(pkg)
  await notifyQl7AdsActivated({
    userId: accountId,
    packageName: plan.label || plan.name || plan.internalName,
    campaign: invoice.meta?.campaign || invoice.meta?.campaignId || pkgId,
    invoiceId: invoice.id,
    activatedAt: startsAt,
  }).catch((error) => {
    console.warn('[ql7-support:ads-activated]', error?.message || error)
...
function normalizeTargetCountries(raw) {
...
function normalizeTargetRegions(raw, allowedCountries) {
  const out = {}
  if (!raw || typeof raw !== 'object') return out
  const allowed = new Set(allowedCountries || [])

  for (const [countryKey, regions] of Object.entries(raw)) {
    const country = String(countryKey || '')
      .trim()
      .toUpperCase()
...
async function updatePackageUsedCampaigns(pkgKey, delta) {
  const now = nowIso()
  try {
    return await adsPrimary.updatePackageUsedCampaigns(pkgKey, delta, now)
  } catch {
    // fallback below
  }

  const freshPkg = await getJSON(pkgKey)
  if (!freshPkg) return { ok: false, error: 'NO_PKG' }

  const maxC = Number(freshPkg.maxCampaigns || 0)
  const used = Number(freshPkg.usedCampaigns || 0)
  let nextUsed = used + Number(delta || 0)
  if (nextUsed < 0) nextUsed = 0
  if (maxC > 0 && nextUsed > maxC) {
    return { ok: false, error: 'LIMIT_REACHED' }
  }

  freshPkg.usedCampaigns = nextUsed
  freshPkg.updatedAt = now
...
export async function createCampaign({
  ...
  // Для поиска активного пакета — используем сырой ID, чтобы legacy READ compatibility
  // внутри resolveAccountIds() могла увидеть старые Mongo representations.
  const pkg = await findActivePackageForAccount(rawAccountId ?? accountId)
  ...
  const normalizedTargetCountries = normalizeTargetCountries(targetCountries)
  if (!normalizedTargetCountries.length) {
    return { ok: false, error: 'TARGET_COUNTRIES_REQUIRED' }
  }
  const normalizedTargetRegions = normalizeTargetRegions(
    targetRegions,
    ...
    rawCreatives = [
      {
        ...
        mediaType,
        posterUrl,
        ...(String(mediaType || '').trim().toLowerCase() === 'video' ? { videoModerationApproved } : {}),
      },
    ]
  }
  ...
  for (const raw of rawCreatives) {
    ...
    const rawPosterUrl = String(raw.posterUrl || '').trim()
    const finalPosterUrl = isR2PublicUrl(rawPosterUrl) && /\.(?:webp|jpe?g)(?:[?#].*)?$/i.test(rawPosterUrl) ? rawPosterUrl : ''

    if (!finalMediaUrl) {
      finalMediaType = 'none'
    }
    if (finalMediaType === 'video' && raw.videoModerationApproved !== true) {
      return { ok: false, error: 'VIDEO_MODERATION_APPROVAL_REQUIRED' }
    }
    ...
    normalizedCreatives.push({
      ...
      posterUrl: finalMediaType === 'video' ? finalPosterUrl : '',
  ...
  const spend = normalizedTargetCountries.length
  const pkgKey = `ads:package:${pkg.id}`
  const reserve = await updatePackageUsedCampaigns(pkgKey, spend)
  if (!reserve.ok) {
    return { ok: false, error: reserve.error || 'LIMIT_REACHED' }
  }
  ...
  const campaign = {
    ...
    // ВАЖНО: сохраняем таргетинг (нормализованный)
  ...
  try {
    ...
    await updatePackageUsedCampaigns(pkgKey, -spend)
    throw e
  }
...
function normalizedIdentitySet(list = []) {
  return new Set((Array.isArray(list) ? list : [])
    .map((value) => normalizeAccountId(value) || String(value || '').trim().toLowerCase())
    .filter(Boolean))
}

async function campaignOwnedByAccount(campaign, rawAccountId) {
  if (!campaign || !rawAccountId) return false
  const [actorIds, ownerIds] = await Promise.all([
    resolveAccountIds(rawAccountId),
    resolveAccountIds(campaign.accountId),
  ])
  const actorSet = normalizedIdentitySet(actorIds)
  const ownerSet = normalizedIdentitySet(ownerIds)
  for (const id of actorSet) if (ownerSet.has(id)) return true
  return false
}

function isCampaignVideoCreative(creative) {
  if (!creative || typeof creative !== 'object') return false
  const mediaType = String(creative.mediaType || '').trim().toLowerCase()
  const mediaUrl = String(creative.mediaUrl || '').trim()
  return mediaType === 'video' || /\.(?:mp4|webm|mov|m4v|mkv)(?:[?#].*)?$/i.test(mediaUrl)
}

function campaignVideoItems(campaign) {
  const creatives = Array.isArray(campaign?.creatives)
    ? campaign.creatives.filter(isCampaignVideoCreative)
    : []
  if (creatives.length) return creatives
  return isCampaignVideoCreative(campaign)
    ? [{
...
function findCampaignVideoItem(campaign, mediaUrl = '') {
  const wanted = String(mediaUrl || '').trim()
  const videos = campaignVideoItems(campaign)
  if (!videos.length) return null
  if (wanted) {
    const exact = videos.find((creative) => String(creative?.mediaUrl || '').trim() === wanted)
...
function normalizeVideoModerationStatus(raw, fallback = 'pending') {
  const value = String(raw || '').trim().toLowerCase()
  return ['pending', 'approved'].includes(value) ? value : fallback
}

function hasOwnVideoModerationStatus(value) {
  return !!value && Object.prototype.hasOwnProperty.call(value, 'moderationStatus')
}

function resolveCampaignVideoModerationStatus(campaign, creative = campaign) {
  // Campaigns created after video moderation integration always persist an explicit
  // moderationStatus. Older campaigns do not. Missing metadata is therefore a
  // legacy pre-moderation marker and must not be reclassified as pending forever.
  if (hasOwnVideoModerationStatus(creative)) {
    return normalizeVideoModerationStatus(creative?.moderationStatus, 'pending')
  }
  if (creative !== campaign && hasOwnVideoModerationStatus(campaign)) {
    return normalizeVideoModerationStatus(campaign?.moderationStatus, 'pending')
  }
  return 'approved'
}

function computeCampaignVideoModerationStatus(campaign) {
  const videos = campaignVideoItems(campaign)
  if (!videos.length) return 'approved'
  return videos.every((creative) => resolveCampaignVideoModerationStatus(campaign, creative) === 'approved')
    ? 'approved'
    : 'pending'
}

export async function getCampaignVideoModerationStatus(campaignId, mediaUrl = '') {
  const id = String(campaignId || '').trim()
  if (!id) return { ok: false, error: 'NO_CAMPAIGN_ID', status: 'gone' }
  const campaign = await getJSON(`ads:campaign:${id}`)
  if (!campaign) return { ok: false, error: 'NOT_FOUND', status: 'gone' }
  const wanted = String(mediaUrl || '').trim()
  if (wanted) {
    const creative = findCampaignVideoItem(campaign, wanted)
    if (!creative) return { ok: false, error: 'VIDEO_NOT_FOUND', status: 'gone' }
    return {
      ok: true,
      status: resolveCampaignVideoModerationStatus(campaign, creative),
      campaignStatus: normalizeVideoModerationStatus(campaign?.moderationStatus, computeCampaignVideoModerationStatus(campaign)),
    }
  }
  return {
    ok: true,
    status: normalizeVideoModerationStatus(campaign?.moderationStatus, computeCampaignVideoModerationStatus(campaign)),
  }
}

export async function getCampaignForVideoModeration({ campaignId, accountId, mediaUrl = '' }) {
  const id = String(campaignId || '').trim()
  if (!id) return { ok: false, error: 'NO_CAMPAIGN_ID' }
  const campaign = await getJSON(`ads:campaign:${id}`)
  if (!campaign) return { ok: false, error: 'NOT_FOUND' }
  if (!(await campaignOwnedByAccount(campaign, accountId))) return { ok: false, error: 'FORBIDDEN' }
  const video = findCampaignVideoItem(campaign, mediaUrl)
  if (!video) return { ok: false, error: 'VIDEO_NOT_FOUND' }
  return { ok: true, campaign, video }
}

export async function setCampaignVideoModerationStatus({ campaignId, accountId, mediaUrl = '', status }) {
  const state = await getCampaignForVideoModeration({ campaignId, accountId, mediaUrl })
  if (!state.ok) return state
  const nextStatus = String(status || '').trim().toLowerCase()
  if (!['pending', 'approved'].includes(nextStatus)) return { ok: false, error: 'BAD_MODERATION_STATUS' }
  const campaign = { ...state.campaign }
  const wanted = String(state.video?.mediaUrl || mediaUrl || '').trim()

  if (Array.isArray(campaign.creatives) && campaign.creatives.length) {
    campaign.creatives = campaign.creatives.map((creative) => {
      if (!isCampaignVideoCreative(creative)) return creative
      return String(creative?.mediaUrl || '').trim() === wanted
        ? { ...creative, moderationStatus: nextStatus }
        : creative
    })
  } else if (isCampaignVideoCreative(campaign)) {
    campaign.moderationStatus = nextStatus
  }

  campaign.moderationStatus = computeCampaignVideoModerationStatus(campaign)
  campaign.updatedAt = nowIso()
  await setJSON(`ads:campaign:${campaign.id || campaignId}`, campaign)
  return {
    ok: true,
    campaign,
    itemStatus: nextStatus,
    campaignStatus: campaign.moderationStatus,
  }
}

export async function deleteCampaignForVideoModeration({ campaignId, accountId, mediaUrl = '' }) {
  const state = await getCampaignForVideoModeration({ campaignId, accountId, mediaUrl })
  if (!state.ok) return state
  return deleteCampaign(campaignId)
}

export function getViewerGeoFromRequest(req) {
...
function isLocalAdsServeDevRequest(req) {
  const headers = req?.headers || req || {}
  const host = String(getHeaderValue(headers, 'host') || '').toLowerCase()
  return (
    process.env.NODE_ENV !== 'production' ||
    process.env.ADS_LOCAL_DEV_SERVE_GEO_FALLBACK === '1' ||
...
function getAdSelectionGeoFromRequest(req) {
  const geo = getViewerGeoFromRequest(req)
  const currentCountry = String(geo?.country || 'ZZ').toUpperCase()

  if (currentCountry && currentCountry !== 'ZZ') {
    return geo
  }

  if (!isLocalAdsServeDevRequest(req)) {
    return geo
  }

  if (!ADS_LOCAL_DEV_SERVE_GEO_COUNTRY || ADS_LOCAL_DEV_SERVE_GEO_COUNTRY === 'ZZ') {
    return geo
  }

  return {
    country: ADS_LOCAL_DEV_SERVE_GEO_COUNTRY,
    region: ADS_LOCAL_DEV_SERVE_GEO_REGION,
    city: ADS_LOCAL_DEV_SERVE_GEO_CITY,
  }
}

export function campaignMatchesGeo(campaign, viewerGeo) {
  if (!campaign) return false

  // Строгая безопасность: если таргетинг не задан — НЕ показываем
  ...
  const viewerCountry = String(viewerGeo?.country || '').toUpperCase()
  if (!viewerCountry || viewerCountry === 'ZZ') return false
  if (!targetCountries.includes(viewerCountry)) return false

  const regionsByCountry = campaign.targetRegions || {}
  const regionList = Array.isArray(regionsByCountry[viewerCountry])
    ? regionsByCountry[viewerCountry]
    : []

  if (regionList.length) {
    const viewerRegion = String(viewerGeo?.region || '').toUpperCase()
    if (!viewerRegion) return false
    if (!regionList.includes(viewerRegion)) return false
  }

  return true
}

export async function getLinksForForum(req) {
  const ids = await smembers('ads:campaigns:all')
  if (!ids.length) {
    return { ok: true, linksString: '' }
  }
  const viewerGeo = getAdSelectionGeoFromRequest(req)
  const now = nowMs()
  const active = []
  ...
  for (const id of ids) {
    ...
    if (!campaignMatchesGeo(c, viewerGeo)) continue
    active.push(c)
  }
  ...
  const pushLine = (clickUrl, mediaUrl, posterUrl = '', moderationStatus = '', campaignId = '') => {
    if (!clickUrl) return
    ...
    if (mediaUrl) {
      ...
      line = status || id
        ...
        : `${clickUrl}|${mediaUrl}${posterUrl ? `|${posterUrl}` : ''}`
    }
    lines.push(line)
  }
    ...
    if (Array.isArray(c.creatives) && c.creatives.length) {
      ...
      for (const cr of c.creatives) {
        pushLine(
          ...
          cr.mediaUrl,
          cr.posterUrl,
          cr.mediaType === 'video' ? resolveCampaignVideoModerationStatus(c, cr) : '',
          c.id || c.campaignId,
        )
      }
    } else {
      ...
      pushLine(
        ...
        c.mediaUrl,
        c.posterUrl,
        c.mediaType === 'video' ? resolveCampaignVideoModerationStatus(c) : '',
        c.id || c.campaignId,
      )
    }
  }
...
export async function registerEvent(body, req) {
  try {
    ...
    const hasViewerCountry =
      country && String(country).trim() && String(country).toUpperCase() !== 'ZZ'
    const viewerGeo = hasViewerCountry
      ? {
    ...
    if (!targetCampaignIds.length && hashKey) {
      ...
      for (const id of indexedIds) {
        ...
        if (viewerGeo && !campaignMatchesGeo(c, viewerGeo)) continue
...
export async function serveAd(req) {
  const ids = await smembers('ads:campaigns:all')
  if (!ids.length) {
  ...
  if (Array.isArray(campaign.creatives) && campaign.creatives.length) {
    ...
    const outCampaign = {
      ...
      posterUrl: cr.posterUrl || campaign.posterUrl || '',
      ...(isCampaignVideoCreative(cr)
  ...
  if (isCampaignVideoCreative(campaign)) {
    return {
      ok: true,
      campaign: {
        ...campaign,
...
export async function uploadMedia(file, opts = {}) {
  ...
  const contentType = String(file.type || '').split(';')[0].trim().toLowerCase() || 'application/octet-stream'
  const isVideo = /^video\//i.test(contentType) || /\.(mp4|webm|mov|m4v|mkv)$/i.test(rawName)
  const prefix = isVideo ? 'ads/videos' : 'ads/images'

  const key = forceUniqueName
    ? createMediaObjectKey({
        prefix,
        filename: rawName,
        contentType,
        fallbackName: isVideo ? 'ad-video' : 'ad-image',
        fallbackExt: isVideo ? 'mp4' : 'webp',
      })
...
export async function startCampaign(campaignIdRaw) {
  ...
  const pkg = campaign.packageId ? await getPackageById(campaign.packageId) : null
  if (pkg?.expiresAt) {
    const expiresTs = new Date(pkg.expiresAt).getTime()
    if (Number.isFinite(expiresTs) && expiresTs <= Date.now()) {
      campaign.status = 'expired'
      campaign.endsAt = pkg.expiresAt
      campaign.updatedAt = now
      await setJSON(key, campaign)
      return { ok: false, error: 'PACKAGE_EXPIRED' }
    }
    campaign.endsAt = pkg.expiresAt
  } else if (!campaign.endsAt || new Date(campaign.endsAt).getTime() <= Date.now()) {
...
export async function deleteCampaign(campaignIdRaw) {
  ...
  const accountIds = await resolveAccountIds(campaign.accountId)
  ...
  for (const accountId of accountIds) await srem(`ads:campaigns:${accountId}`, id)
  if (packageId) {
