...
import { notifyQl7AdsActivated } from './ql7-support/events.js'
...
async function resolveAccountIds(raw) {
  ...
  try {
    ...
    if (canonical) {
      ids.add(String(canonical))
      ids.add(String(canonical).toLowerCase())
    }
  } catch {
...
export async function applyPaidInvoice(invoice, nowDate = new Date()) {
  ...
  await sadd(`ads:packages:${invoice.accountId}`, pkgId)
...
export async function createCampaign({
  ...
  // Для поиска активного пакета — используем сырой ID (до normalize), чтобы resolveAccountIds сработал правильно
  ...
  const creativesWithMeta = normalizedCreatives.map((c, idx) => {
    ...
    return {
      ...
      moderationStatus: c.mediaType === 'video' ? 'pending' : 'approved',
      urlHash: hStr,
      createdAt: now,
    }
  })
  ...
  const campaign = {
    ...
    moderationStatus: creativesWithMeta.some((creative) => creative.mediaType === 'video') ? 'pending' : 'approved',
