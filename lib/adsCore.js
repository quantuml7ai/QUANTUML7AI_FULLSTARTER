...
function normalizeAccountId(raw) {
  ...
  // ВАЖНО: сначала сигнатуры с префиксами,
  // а в итоге хотим получить "канонический" id:
  //  - для телеги → просто цифры
  //  - для кошелька → 0x... в lowercase
  ...
  // адреса храним в lowercase
  if (s.toLowerCase().startsWith('0x')) {
    return s.toLowerCase()
  }

  // всё остальное (uid и прочее) — как есть
  return s
}
...
async function resolveAccountIds(raw) {
  ...
  // базовые варианты: канон + то, что реально прилетело
  ids.add(primary)
  if (rawStr && rawStr !== primary) {
    ids.add(rawStr)
  }

  // хелпер для добавления всех tg-вариантов
  const addTgVariants = (uid) => {
    if (!uid) return
    const s = String(uid).trim()
    if (!s) return
    ids.add(s)                      // "6737..."
    ids.add(`tg:${s}`)              // "tg:6737..."
    ids.add(`tguid:${s}`)           // "tguid:6737..."
    ids.add(`telegramid:${s}`)      // "telegramid:6737..."
  }
  ...
  if (primary.startsWith('0x')) {
    ...
    try {
      let tgId = await redis.hget(ACC_KEY(addrNorm), 'tg_id')

      // вдруг было записано под "сырым" адресом (checksum)
      if (!tgId &&
          rawStr.toLowerCase().startsWith('0x') &&
          rawStr.toLowerCase() !== addrNorm) {
  ...
  // --- Числовой uid → все tg-префиксы + кошелёк ---
  if (/^\d+$/.test(primary)) {
    addTgVariants(primary)

    try {
      let addr = await redis.get(TG_UID_KEY(primary)) // "0x...."

      // b/c у тебя ещё есть старые ключи вида telegramid:<id>
      if (!addr) {
        addr = await redis.get(`telegramid:${primary}`)
      }

      if (addr) {
        const normAddr = String(addr).trim().toLowerCase()
        if (normAddr) {
          ids.add(normAddr)
        }
      }
