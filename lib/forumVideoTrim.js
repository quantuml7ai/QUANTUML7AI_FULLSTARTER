...
function pushFastStartDiag(event, payload = {}) {
  try {
    if (typeof window === "undefined") return;
    const rec = {
      event: String(event || "faststart"),
      ts: Date.now(),
      ...payload,
    };
    for (const key of ["__ql7FastStartDiag", "__ql7VideoFastStartDiag"]) {
      const prev = Array.isArray(window[key]) ? window[key] : [];
      window[key] = [...prev.slice(-99), rec];
    }
  } catch {}
}

function makeAbortError(message = "faststart_aborted") {
...
function readBlobDuration(blob, timeoutMs = 22000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (err, dur) => {
      ...
      try { v.removeEventListener("loadeddata", onMeta); } catch {}
      try { v.removeEventListener("durationchange", onMeta); } catch {}
...
export async function optimizeForumVideoFastStart(inputBlob, opts = {}) {
  ...
  const allowTranscodeForFaststart = opts?.allowTranscode === true;
  const strictFlatFaststart = opts?.strictFlatFaststart !== false;
  const abortFaststartOnSignal = opts?.abortFaststartOnSignal !== false;
  ...
  if (allowTranscodeForFaststart) {
    pushFastStartDiag("faststart_transcode_disabled", {
      reason: "v67_copy_remux_only",
      sourceMime,
      sourceBytes,
    });
  }
  if (signal?.aborted) {
    pushFastStartDiag("faststart_signal_abort_seen", { sourceMime, sourceBytes, alreadyAborted: true });
    throw makeAbortError("faststart_aborted");
  }
  if (!canFaststartCopy) {
    const reason = "faststart_unsupported_input";
    pushFastStartDiag("fallback_error", { reason, sourceMime, sourceBytes });
    if (strictFlatFaststart) throw new Error(reason);
    return { blob: inputBlob, mime: sourceMime || "video/webm", changed: false, pipeline: "passthrough_non_mp4", flatFaststart: false };
  }
  if (sourceBytes <= 0 || sourceBytes > maxTranscodeBytes) {
    const reason = sourceBytes <= 0 ? "faststart_empty_blob" : "faststart_too_large";
    pushFastStartDiag("fallback_error", { reason, sourceMime, sourceBytes, maxTranscodeBytes });
    if (strictFlatFaststart) throw new Error(reason);
    return { blob: inputBlob, mime: sourceMime || "video/mp4", changed: false, pipeline: reason, flatFaststart: false };
  }
  if (typeof Worker === "undefined") {
    const reason = "faststart_no_worker";
    pushFastStartDiag("fallback_error", { reason, sourceMime, sourceBytes });
    if (strictFlatFaststart) throw new Error(reason);
    return { blob: inputBlob, mime: sourceMime || "video/mp4", changed: false, pipeline: "no_worker", flatFaststart: false };
  }

  try {
    const outResult = await new Promise((resolve, reject) => {
      let settled = false;
      let timeoutId = 0;
      const worker = new Worker("/workers/forum-trim-worker.js");
      let abortCleanup = null;
      ...
      const onAbort = () => {
        pushFastStartDiag("faststart_signal_abort_seen", {
          sourceMime,
          sourceBytes,
          abortFaststartOnSignal,
        });
        if (abortFaststartOnSignal) finish(makeAbortError("faststart_aborted"));
      };
      if (signal) {
        if (signal.aborted) {
          onAbort();
      ...
      worker.onmessage = (ev) => {
        const msg = ev?.data || {};
        if (msg?.type === "progress") {
          try { onProgress(Math.max(0, Math.min(1, Number(msg.progress || 0))), msg); } catch {}
          pushFastStartDiag("faststart_worker_progress", {
        ...
        if (msg?.type === "done") {
          if (!msg.ok) {
            const err = new Error(String(msg.error || "faststart_worker_failed"));
            pushFastStartDiag("faststart_worker_done_error", {
              error: err.message,
          ...
          const buffer = msg?.buffer;
          if (!(buffer instanceof ArrayBuffer) || !buffer.byteLength) {
            const err = new Error("faststart_worker_empty");
            pushFastStartDiag("faststart_worker_done_error", { error: err.message, sourceMime, sourceBytes });
            finish(err);
            return;
          }
          finish(null, {
            blob: new Blob([buffer], { type: "video/mp4" }),
            pipeline: String(msg.pipeline || "worker_local_flat_faststart_remux_copy"),
            audit: msg?.audit || null,
          });
        }
      };
      worker.onerror = (e) => {
        const err = new Error(String(e?.message || "faststart_worker_error"));
        pushFastStartDiag("faststart_worker_done_error", { error: err.message, sourceMime, sourceBytes });
        finish(err);
      };

      timeoutId = setTimeout(() => {
        const err = new Error("faststart_worker_timeout");
        pushFastStartDiag("faststart_worker_done_error", { error: err.message, sourceMime, sourceBytes });
        finish(err);
      ...
      try {
        ...
        const err = e || new Error("faststart_worker_post_failed");
        pushFastStartDiag("faststart_worker_done_error", {
          error: String(err?.message || err || "faststart_worker_post_failed"),
          sourceMime,
          sourceBytes,
        });
        finish(err);
    ...
    if (signal?.aborted) throw makeAbortError("faststart_aborted");

    const pipeline = String(outResult?.pipeline || "worker_local_flat_faststart_remux_copy");
    const outBlob = pipeline === "already_flat_faststart" ? inputBlob : (outResult?.blob || outResult);
    ...
    if (!outOk || !driftOk) {
      const reason = !outOk ? "faststart_bad_duration" : "faststart_duration_drift";
      pushFastStartDiag("fallback_error", {
        reason,
        pipeline,
        sourceMime,
        sourceBytes,
        inputDurationSec: Number.isFinite(inDur) ? inDur : null,
        outputDurationSec: Number.isFinite(outDur) ? outDur : null,
      });
      if (strictFlatFaststart) throw new Error(reason);
      return { blob: inputBlob, mime: sourceMime || "video/mp4", changed: false, pipeline: reason, flatFaststart: false };
    }

    pushFastStartDiag("faststart_success", {
      pipeline,
      sourceMime,
      sourceBytes,
      outputBytes: Number(outBlob?.size || 0) || null,
      audit: outResult?.audit || null,
      changed: pipeline !== "already_flat_faststart",
    });
    return {
      blob: outBlob,
      mime: "video/mp4",
    ...
    if (isAbortLike(e)) {
      pushFastStartDiag("faststart_aborted", { sourceMime, sourceBytes });
      throw makeAbortError("faststart_aborted");
    }
    const reason = String(e?.message || e || "fallback_error");
    pushFastStartDiag("fallback_error", { reason, sourceMime, sourceBytes });
    if (strictFlatFaststart) throw new Error(reason);
    return { blob: inputBlob, mime: sourceMime || "video/mp4", changed: false, pipeline: "fallback_error", flatFaststart: false };
  }
}
