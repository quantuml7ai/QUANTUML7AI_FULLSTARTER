...
export const SITE_ORIGIN = 'https://www.quantuml7ai.com'
...
export function toAbsoluteSiteUrl(rawUrl, origin = SITE_ORIGIN) {
  const value = String(rawUrl || '').trim()
  if (!value) return String(origin || '').trim()
  if (/^https?:\/\//i.test(value)) return value
  const base = String(origin || SITE_ORIGIN).trim() || SITE_ORIGIN
