...
import { isVideoHeavyActivityActive, subscribeVideoHeavyActivity } from '../lib/videoPipelineProgress'
...
const DEFAULT_FIRST_DROP_DELAY_MS = 180_000
...
function buildFxPack (tier) {
  ...
  return { confetti, sparks }
}

const cn = (...parts) => parts.filter(Boolean).join(' ')

export default function QCoinDropFX ({
  firstDropDelayMs = DEFAULT_FIRST_DROP_DELAY_MS,
  ...
  // Защищает расписание от повторных auth:ok/auth:success,
  // которые могут приходить после фоновой проверки wallet-session.
  const activeAccountKeyRef = useRef('')
  const scheduleArmedRef = useRef(false)
  ...
  useEffect(() => {
    ...
    const armFirstDrop = (accountId) => {
      const account = String(accountId || '').trim()
      if (!account) return

      const accountKey = normalizeAccountKey(account)
      const sameAccount = activeAccountKeyRef.current === accountKey

      /*
       * Фоновая verifyStoredWalletSession может повторно публиковать
       * auth:ok/auth:success для уже активного аккаунта.
       * Такие подтверждения не имеют права сбрасывать текущий таймер.
       */
      if (sameAccount && scheduleArmedRef.current) return

      activeAccountKeyRef.current = accountKey
      scheduleArmedRef.current = true

      // Новый реальный аккаунт получает собственное расписание.
      ...
      spawnAtRef.current =
        now + Math.max(0, Number(firstDropDelayMs) || 0)

      setScheduleVersion((value) => value + 1)
    }

    const onAuth = () => {
      armFirstDrop(readUnifiedAccountId())
    }

    const onLogout = () => {
      activeAccountKeyRef.current = ''
      scheduleArmedRef.current = false
    ...
    // Уже авторизованный пользователь после загрузки страницы.
    armFirstDrop(readUnifiedAccountId())
  ...
  useEffect(() => {
    ...
    if (!uid) {
      ...
      const prevTimer = spawnTimerRef.current
      animFrameRef.current = null
      spawnTimerRef.current = null
      try { if (prevRaf) cancelAnimationFrame(prevRaf) } catch {}
      try { if (prevTimer) clearTimeout(prevTimer) } catch {}
      return
    }
    ...
    let rafId = 0
    let timerId = 0
    let videoHeavyActive = isVideoHeavyActivityActive()
    ...
    const scheduleFrame = () => {
      if (stopped || !documentVisible || videoHeavyActive) return
      rafId = requestAnimationFrame(loop)
      animFrameRef.current = rafId
    }

    const scheduleSpawnWake = () => {
      if (stopped || !documentVisible || videoHeavyActive) return
      ...
      if (!spawnAtRef.current) spawnAtRef.current = now + firstDropDelayMs
      const delay = Math.max(0, spawnAtRef.current - now)
    ...
    const loop = (ts) => {
      ...
      let { w, h } = worldRef.current
      if (!w || !h) {
        initWorld()
        ;({ w, h } = worldRef.current)
      }
    ...
    const onVisibilityChange = () => {
      ...
      if (coinRef.current) scheduleFrame()
      else scheduleSpawnWake()
    }
    document.addEventListener('visibilitychange', onVisibilityChange, { passive: true })
    const unsubscribeVideoHeavy = subscribeVideoHeavyActivity((active) => {
      const nextActive = !!active
      if (nextActive === videoHeavyActive) return
      videoHeavyActive = nextActive
      clearScheduledWork()
      lastTimeRef.current = 0
      if (videoHeavyActive || stopped || !documentVisible) return
      if (coinRef.current) scheduleFrame()
      else scheduleSpawnWake()
    })
    ...
    return () => {
      stopped = true
      unsubscribeVideoHeavy?.()
      document.removeEventListener('visibilitychange', onVisibilityChange)
      clearScheduledWork()
    }
  }, [uid, scheduleVersion, firstDropDelayMs, intervalMs, minSize, maxSize, initWorld])
  ...
  const handleCollect = async () => {
    const coin = coinRef.current
    if (!coin || coin.exploding || !uid) return
  ...
  return (
    <>
      {fx && (
        <div className={cn('qdrop-fx', `tier-${fx.tier}`)} data-ql7-visual-scope="overlay" aria-hidden="true">
      ...
      {coin && (
        <div className="qdrop-root" data-ql7-visual-scope="overlay" style={{ pointerEvents: 'none' }}>
          <div
      ...
      <style jsx>{`
        /* Механика падения и сбора сохранена; тяжёлые декоративные тени облегчены. */
        ...
        .qdrop-coin-btn.is-exploding { pointer-events: none; }
        ...
        .qcoinLabel {
          ...
          color: transparent;
          animation: qcoinShine 1.15s ease-out 1;
          text-shadow: 0 0 0.16rem rgba(255, 215, 0, 0.24);
          white-space: nowrap;
          min-height: 1.2em;
        }
        ...
        .qdrop-fx-badge { position: absolute; left: 50%; top: calc(50% + 92px); transform: translateX(-50%); opacity: 0; animation: qfxBadge 1.05s ease-out forwards; }
        .qdrop-fx-badge-inner { display: inline-block; padding: 8px 14px; border-radius: 999px; border: 1px solid rgba(255, 220, 140, 0.42); background: radial-gradient(circle at 20% 0%, rgba(255,255,255,0.18), transparent 45%), rgba(10, 14, 24, 0.9); color: rgba(255, 245, 225, 0.98); font-weight: 950; letter-spacing: 0.08em; text-transform: uppercase; }
        ...
        .qdrop-fx-spark { position: absolute; left: 0; top: 0; width: var(--sz); height: var(--sz); border-radius: 999px; background: radial-gradient(circle at 30% 30%, #fff, rgba(255, 232, 160, 0.92), rgba(255, 170, 80, 0.1)); transform: translate(-50%, -50%) translate3d(0,0,0) scale(0.6); opacity: 0; animation: qfxSpark var(--d) ease-out var(--dl) forwards; }
