...
import { metaMarketT, metaMarketTitle, normalizeMetaMarketPublicError } from './metamarket/metaMarketI18n'
import {
...
const MetaMarketUserInfoPopover = dynamic(() => import('../app/forum/features/profile/components/UserInfoPopover'), { ssr: false })
const MetaMarketSubscriptionsPopover = dynamic(() => import('../app/forum/features/subscriptions/components/SubscriptionsPopover'), { ssr: false })
const MetaMarketForumStyles = dynamic(() => import('../app/forum/styles/ForumStyles'), { ssr: false })
...
function MetaMarketImage({ item, alt, className = '', width = 256, height = 256 }) {
  ...
  const animated = !!item?.animated || /(?:\.gif|[-_]animated(?:[-_][^/?#]+)?\.(?:webp|avif|png))(?:$|[?#])/i.test(String(currentSrc || ''))
  const imageError = () => {
  ...
  if (animated) {
    return (
      <ViewportAnimatedImage
        className={className}
        animatedSrc={currentSrc}
        staticSrc={item?.posterPath || item?.posterUrl || ''}
        alt={alt || ''}
        width={width}
        height={height}
        loading="lazy"
  ...
  return (
    // eslint-disable-next-line @next/next/no-img-element -- MetaMarket static catalog assets preserve their existing direct source/error fallback behavior.
    <img
      className={className}
      src={currentSrc}
      alt={alt || ''}
...
export default function MetaMarket({
  ...
  const giftOpenSnapshotRef = useRef({ viewerId: String(viewerId || '').trim(), resolved: false })
  ...
  const [giftOpenSnapshotReady, setGiftOpenSnapshotReady] = useState(false)
  const [transaction, setTransaction] = useState(null)
  const [infoOverlay, setInfoOverlay] = useState(null)
  const [userInfoState, setUserInfoState] = useState({ open: false, userId: '', preview: null })
  const [subscriptionsState, setSubscriptionsState] = useState({ open: false, userId: '', initialMode: 'followers' })
  ...
  const orderedOwnedItems = useMemo(() => {
    if (unseenGiftItemIds.size === 0 || ownedItems.length < 2) return ownedItems
    const newGiftItems = []
    const existingItems = []
    ownedItems.forEach((item) => {
      const itemId = String(item?.itemId || '').trim()
      if (unseenGiftItemIds.has(itemId)) newGiftItems.push(item)
      else existingItems.push(item)
    })
    return [...newGiftItems, ...existingItems]
  }, [ownedItems, unseenGiftItemIds])
  ...
  const closeUserInfoPopover = useCallback(() => {
    setUserInfoState({ open: false, userId: '', preview: null })
    userInfoAnchorRef.current = null
  }, [])

  const closeSubscriptionsPopover = useCallback(() => {
    setSubscriptionsState({ open: false, userId: '', initialMode: 'followers' })
  }, [])
  ...
  const loadHistory = useCallback(async ({ append = false, cursor = '' } = {}) => {
    ...
    try {
      ...
      if (giftOpenSnapshotRef.current.viewerId !== normalizedViewerId) {
        giftOpenSnapshotRef.current = { viewerId: normalizedViewerId, resolved: false }
        setGiftOpenSnapshotReady(false)
      }
      if (!append && !giftOpenSnapshotRef.current.resolved) {
        ...
        giftOpenSnapshotRef.current = { viewerId: normalizedViewerId, resolved: true }
        setGiftOpenSnapshotReady(true)
        if (incoming.length) {
          const newestIncomingAt = incoming.reduce((latest, event) => (
            Math.max(latest, Number(event?.createdAt || 0))
          ), seenAt)
          writeSeenGiftAt(viewerId, newestIncomingAt)
        }
      }
    } catch (error) {
      if (isAbortError(error)) return
  ...
  useEffect(() => {
    ...
    const onKey = (event) => {
      ...
      if (subscriptionsState.open) {
        closeSubscriptionsPopover()
        return
      }
      if (userInfoState.open) {
  ...
  }, [closeInfoOverlay, closeMetaMarket, closeSubscriptionsPopover, closeUserInfoPopover, infoOverlay, portalReady, subscriptionsState.open, transactionCloseLocked, userInfoState.open])
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !giftOpenSnapshotReady) return
  ...
  const openMetaMarketUserInfo = useCallback((user, sourceKind, sourceId, anchor) => {
    ...
    userInfoAnchorRef.current = anchor
    setUserInfoState((current) => (
      current.open && current.userId === userId
        ? { open: false, userId: '', preview: null }
        : { open: true, userId, preview }
    ))
  }, [])

  const openUserInfoFromOwner = useCallback((user, event) => {
    openMetaMarketUserInfo(user, 'metamarket-owner', ownersItemId, event?.currentTarget || null)
  }, [openMetaMarketUserInfo, ownersItemId])

  const openUserInfoFromHistory = useCallback((user, sourceId, event) => {
    openMetaMarketUserInfo(user, 'metamarket-history', sourceId, event?.currentTarget || null)
  }, [openMetaMarketUserInfo])

  const openSubscriptionsFromUserInfo = useCallback(({ userId, initialMode = 'followers' } = {}) => {
  ...
  const openUserInfoFromSubscriptions = useCallback((userId, anchor, userPreview = null) => {
    const targetUserId = String(userId || '').trim()
    if (!targetUserId || !anchor) return
    closeSubscriptionsPopover()
    userInfoAnchorRef.current = anchor
    setUserInfoState({ open: true, userId: targetUserId, preview: userPreview })
  }, [closeSubscriptionsPopover])

  const openForumProfileBranchFromMetaMarket = useCallback((mode, payload = {}) => {
    if (typeof window === 'undefined') return
    const userId = String(payload?.userId || '').trim()
    if (!userId) return
    const detail = {
      mode: mode === 'topics' ? 'topics' : 'posts',
      userId,
      nickname: String(payload?.nickname || userInfoState.preview?.nickname || '').trim(),
    }
    try {
      window.sessionStorage?.setItem('ql7_forum_profile_branch_after_navigation_v1', JSON.stringify(detail))
    } catch {}
    const onForum = /^\/forum(?:\/|$)/u.test(window.location.pathname || '')
    closeMetaMarket()
    if (onForum) {
      try { window.dispatchEvent(new CustomEvent('forum:profile-branch-open', { detail })) } catch {}
      return
    }
    const params = new URLSearchParams({ profileMode: detail.mode, profileUser: detail.userId })
    if (detail.nickname) params.set('profileNick', detail.nickname)
    window.location.assign(`/forum?${params.toString()}`)
  }, [closeMetaMarket, userInfoState.preview?.nickname])

  const openDirectMessageFromMetaMarket = useCallback(({ userId } = {}) => {
    if (typeof window === 'undefined') return
    const targetUserId = String(userId || '').trim()
    if (!targetUserId) return
    const onForum = /^\/forum(?:\/|$)/u.test(window.location.pathname || '')
    closeMetaMarket()
    if (!onForum) {
      const params = new URLSearchParams({ inbox: '1', dmUser: targetUserId })
      window.location.assign(`/forum?${params.toString()}`)
    }
  }, [closeMetaMarket])
    ...
    try {
      ...
      setTransaction((prev) => {
        ...
        return {
          ...
          quotePending: false,
          quoteConfirmed: false,
          errorCode: normalizeMetaMarketPublicError(error?.code || error?.message, 'network_error'),
        }
      })
    }
  ...
  const updateTransactionQuantity = useCallback(async (nextQuantity) => {
    ...
    try {
      ...
      if (isAbortError(error)) return
      setTransaction((prev) => prev && prev.idempotencyKey === current.idempotencyKey
        ? { ...prev, quantity, quotePending: false, quoteConfirmed: false, status: 'error', errorCode: normalizeMetaMarketPublicError(error?.code || error?.message, 'network_error') }
        : prev)
    }
  ...
  const submitTransaction = useCallback(async () => {
    ...
    try {
      ...
      setTransaction((prev) => prev ? {
        ...prev,
        status: 'error',
        errorCode: normalizeMetaMarketPublicError(error?.code || error?.message),
      } : prev)
    }
  ...
  }, [activeCollection, collectionItems.length, collectionTitle, historyOpen, mode, ownersItem, tx])
  ...
  const needsForumSurfaceStyles = typeof window !== 'undefined'
    && !/^\/forum(?:\/|$)/u.test(window.location.pathname || '')

  return createPortal(
    <div
      className="mmOverlay"
      data-ql7-visual-scope="market"
    ...
    >
      ...
      >
        ...
        <header className="mmHeader">
          ...
          {showContextStrip && <div className="mmContextStrip">
            {giftFlowActive && recipientState?.userId && (
              <span className="mmGiftContext">
                <span className="mmGiftContextLabel">{tx('metamarket_context_gift_to')}</span>
                <span className="mmRecipientAvatar">
                  {recipientState?.icon || recipientState?.avatar ? (
                    <AvatarEmoji
                      userId={recipientState.userId}
                      pIcon={recipientState.icon || recipientState.avatar}
                      className="mmRecipientAvatarImg"
                    />
                  ) : (
                    <span className="mmAnonymousAvatar" style={{ backgroundImage: `url("${ANONYMOUS_AVATAR_URL}")` }} aria-hidden="true" />
                  )}
                </span>
                <span className="mmGiftContextNickname" translate="no">{userDisplayName(recipientState)}</span>
              </span>
            )}
            {!(giftFlowActive && recipientState?.userId) && (
        ...
        <main className="mmBody">
          ...
          {stateStatus !== 'loading' && stateStatus !== 'error' && historyOpen && (
            <div className="mmHistoryPane">
              ...
              {historyStatus !== 'loading' && historyStatus !== 'error' && historyEvents.map((event) => {
                ...
                return (
                  <article key={event.txId} className={`mmHistoryRow mmHistory-${action}`} data-ql7-visual-scope="row" data-ql7-render-managed="1">
                    ...
                    {counterparty?.userId ? (
                      ...
                      >
                        <span className="mmOwnerAvatar">
                          {counterparty.icon || counterparty.avatar ? (
                            <AvatarEmoji
                              ...
                              pIcon={counterparty.icon || counterparty.avatar}
                              className="mmOwnerAvatarImg"
                            />
                          ) : (
                            <span className="mmAnonymousAvatar" style={{ backgroundImage: `url("${ANONYMOUS_AVATAR_URL}")` }} aria-hidden="true" />
                          )}
                        ...
                        <span className="mmHistoryUserName">
                          <span className="nick-badge nick-animate" translate="no">
          ...
          {stateStatus !== 'loading' && stateStatus !== 'error' && !historyOpen && mode === 'market' && !selectedCollectionId && !ownersItemId && (
            <div className="mmCollectionList">
              {marketCollections.length ? marketCollections.map((collectionLike) => {
                ...
                return (
                  <article key={collection.id} className={`mmCollectionCard mmTheme-${collection.theme || 'cyan'}`} data-ql7-visual-scope="card" data-ql7-render-managed="1">
                    <button
                      type="button"
          ...
          {stateStatus !== 'loading' && stateStatus !== 'error' && !historyOpen && mode === 'market' && selectedCollectionId && !ownersItemId && (
            <div className="mmItemGrid">
              ...
              {collectionStatus !== 'loading' && collectionStatus !== 'error' && collectionItems.map((item) => {
                ...
                return (
                  <article key={canonical.itemId} className={`mmItemCard mmRarity-${canonical.rarity || 'common'}`} data-ql7-visual-scope="card" data-ql7-render-managed="1">
          ...
          {stateStatus !== 'loading' && stateStatus !== 'error' && !historyOpen && mode === 'collections' && (
            <div className="mmOwnedList">
              ...
              {ownedStatus !== 'loading' && ownedStatus !== 'error' && orderedOwnedItems.map((owned) => {
                ...
                const ribbonId = isNewGift
                  ? `mm-new-gift-${String(owned.itemId || '').replace(/[^a-zA-Z0-9_-]/g, '-')}`
                  : ''
                return (
                  <article key={owned.itemId} className={`mmOwnedCard mmRarity-${canonical.rarity || 'common'} ${isNewGift ? 'isNewGift' : ''}`} data-ql7-visual-scope="card" data-ql7-render-managed="1">
                    {isNewGift && (
                      <span className="mmNewGiftBadge" role="img" aria-label={tx('metamarket_new_gift_badge')} translate="no">
                        <svg viewBox="0 0 64 64" aria-hidden="true" focusable="false">
                          <defs>
                            <linearGradient id={`${ribbonId}-edge`} x1="0" y1="1" x2="1" y2="0">
                              <stop offset="0" stopColor="#650526" />
                              <stop offset="0.48" stopColor="#ff315b" />
                              <stop offset="1" stopColor="#7c092d" />
                            </linearGradient>
                            <linearGradient id={`${ribbonId}-face`} x1="0" y1="1" x2="1" y2="0">
                              <stop offset="0" stopColor="#9f1239" />
                              <stop offset="0.28" stopColor="#f43f5e" />
                              <stop offset="0.62" stopColor="#e11d48" />
                              <stop offset="1" stopColor="#be123c" />
                            </linearGradient>
                            <linearGradient id={`${ribbonId}-glint`} x1="0" y1="0" x2="1" y2="0">
                              <stop offset="0" stopColor="#fff" stopOpacity="0" />
                              <stop offset="0.34" stopColor="#bef5ff" stopOpacity="0.46" />
                              <stop offset="0.5" stopColor="#fff" stopOpacity="1" />
                              <stop offset="0.66" stopColor="#ffe08a" stopOpacity="0.72" />
                              <stop offset="1" stopColor="#fff" stopOpacity="0" />
                            </linearGradient>
                            <clipPath id={`${ribbonId}-clip`}>
                              <path d="M-1 30 30-1h18L-1 48Z" />
                            </clipPath>
                          </defs>
                          <path className="mmNewGiftRibbonFold" d="M-1 29 29-1h19l-8 8-33 33-8 9Z" />
                          <path className="mmNewGiftRibbonEdge" fill={`url(#${ribbonId}-edge)`} d="M-1 28 28-1h22l-4 5 4 4-5 3 3 5L-1 53Z" />
                          <path className="mmNewGiftRibbonFace" fill={`url(#${ribbonId}-face)`} d="M-1 31 31-1h17l-4 5 4 4-5 3 3 4L-1 49Z" />
                          <path className="mmNewGiftRibbonCrown" d="m3 31 28-28" />
                          <g clipPath={`url(#${ribbonId}-clip)`}>
                            <g className="mmNewGiftRibbonGlint">
                              <rect x="-7" y="18" width="10" height="38" rx="5" fill={`url(#${ribbonId}-glint)`} transform="rotate(-45 0 37)" />
                            </g>
                          </g>
                          <path className="mmNewGiftRibbonGem" d="m7 39 2-2 2 2-2 2Zm32-32 2-2 2 2-2 2Z" />
                          <text x="22" y="27" transform="rotate(-45 22 22)">New</text>
                        </svg>
                      </span>
                    )}
                    <button
                  ...
                  <span className="mmTransactionAssetInfo">
                    ...
                    {transaction.action === 'gift' && recipientState?.userId && (
                      <span className="mmTransactionTarget">
                        <span className="mmTransactionTargetLabel">{tx('metamarket_transaction_gift_target')}</span>
                        <span className="mmTransactionTargetAvatar">
                          {recipientState?.icon || recipientState?.avatar ? (
                            <AvatarEmoji
                              userId={recipientState.userId}
                              pIcon={recipientState.icon || recipientState.avatar}
                              className="mmTransactionTargetAvatarImg"
                            />
                          ) : (
                            <span className="mmAnonymousAvatar" style={{ backgroundImage: `url("${ANONYMOUS_AVATAR_URL}")` }} aria-hidden="true" />
                          )}
                        </span>
                        <span className="mmTransactionTargetName" translate="no">{userDisplayName(recipientState)}</span>
                      </span>
                    )}
                  </span>
                </div>
        ...
        <style jsx global>{`
          ...
          .mmOwnedCard > .mmNewGiftBadge {
            ...
            transform: none;
            filter:
              drop-shadow(0 3px 3px rgba(38, 2, 16, .58))
              drop-shadow(0 0 4px rgba(255, 49, 91, .36));
          }
          .mmNewGiftBadge svg {
            display: block;
            width: 100%;
          ...
          .mmNewGiftRibbonFold {
            fill: #5d0622;
            stroke: rgba(255, 116, 143, .46);
            stroke-width: .7;
          }
          .mmNewGiftRibbonFoldRight {
            fill: #73072b;
          }
          .mmNewGiftRibbonFace {
            stroke: rgba(255, 196, 207, .72);
            stroke-width: .7;
          }
          .mmNewGiftRibbonCrown {
            fill: none;
          ...
          .mmNewGiftRibbonGem {
            fill: rgba(255, 229, 150, .92);
          }
          .mmNewGiftRibbonGlint {
            opacity: 0;
            transform: translate3d(-20px, 20px, 0);
            transform-origin: center;
            animation: mmNewGiftSunGlint 7.2s ease-in-out infinite;
          }
          .mmNewGiftBadge text {
            fill: #fff;
            font: 1000 15px/1 ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
            letter-spacing: .065em;
            text-anchor: middle;
            paint-order: stroke fill;
            stroke: rgba(76, 3, 29, .62);
            stroke-width: 1.35px;
          }
          @keyframes mmNewGiftSunGlint {
            0%, 58% { opacity: 0; transform: translate3d(-20px, 20px, 0); }
            62% { opacity: .24; }
            68% { opacity: 1; }
            76% { opacity: .52; }
            82%, 100% { opacity: 0; transform: translate3d(76px, -76px, 0); }
          }
          .mmOwnersHero,
          .mmHistoryRow {
            border-radius: 20px;
          ...
          .mmTransactionTarget {
            ...
            max-width: 100%;
            display: inline-grid;
            grid-template-columns: max-content 24px minmax(0, auto);
            align-items: center;
            justify-self: start;
            gap: 7px;
            border-radius: 14px;
          ...
          .mmTransactionTargetLabel {
            padding-left: 3px;
          }
          .mmTransactionTargetName {
            font-weight: 1000;
          }
          ...
          [dir='rtl'] .mmTransactionAssetInfo {
            padding-left: 0;
            padding-right: 12px;
            border-left: 0;
          ...
          [dir='rtl'] .mmTransactionAssetInfo em {
            justify-self: end;
          }
          @media (max-width: 640px) {
            .mmTransactionLayer {
      ...
      {needsForumSurfaceStyles && <MetaMarketForumStyles />}
      <MetaMarketSubscriptionsPopover
        open={subscriptionsState.open}
        userId={subscriptionsState.userId}
        initialMode={subscriptionsState.initialMode}
        onClose={closeSubscriptionsPopover}
        onOpenUserInfo={openUserInfoFromSubscriptions}
        t={t}
      />
      <MetaMarketUserInfoPopover
        anchorRef={userInfoAnchorRef}
        open={userInfoState.open}
        onClose={closeUserInfoPopover}
        rawUserId={userInfoState.userId}
        userPreview={userInfoState.preview}
        t={t}
        onOpenUserPosts={(payload) => openForumProfileBranchFromMetaMarket('posts', payload)}
        onOpenUserTopics={(payload) => openForumProfileBranchFromMetaMarket('topics', payload)}
        onOpenSubscriptions={openSubscriptionsFromUserInfo}
        onOpenDirectMessage={openDirectMessageFromMetaMarket}
      />
    </div>,
    document.body,
  )
