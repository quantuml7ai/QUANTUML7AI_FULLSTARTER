...
function useSmoothedPercent(active, targetPercent, phase, scopeRef) {
...
export default function MediaPipelineProgress({
  ...
  const displayPercent = useSmoothedPercent(active, percent, phase, rootRef)
  if (!active) return null

  const phaseKey = String(phase || 'preparing').toLowerCase()
  const prepared = phaseKey === 'prepared'
  const ready = phaseKey === 'ready' || displayPercent >= 99.95
  const visualPhaseKey = prepared ? 'verifying' : phaseKey
  const currentIndex = ready
    ? stageOrder.length - 1
    : Math.max(0, stageOrder.indexOf(visualPhaseKey))
  const phaseLabel = labels[phaseKey] || labels[visualPhaseKey] || labels.processing || phaseKey

  return (
    <section
      ref={rootRef}
      className={`ql7PipelineProgress ${className}`.trim()}
      data-ql7-visual-scope="pending"
      data-theme={theme}
      data-phase={phaseKey}
      data-ready={ready ? '1' : '0'}
      data-prepared={prepared ? '1' : '0'}
      role="status"
      aria-live="polite"
    ...
    >
      <div className="ql7PipelineProgress__head">
        ...
        {!ready && !prepared && typeof onCancel === 'function' ? (
          <button
            type="button"
      ...
      <ol className="ql7PipelineProgress__stages" aria-label={labels.stagesAria || phaseLabel}>
        {stageOrder.map((stage, index) => {
          const done = ready || (prepared ? index <= currentIndex : index < currentIndex)
          const current = !ready && !prepared && index === currentIndex
          return (
      ...
      <style jsx>{`
        ...
        .ql7PipelineProgress[data-prepared='1'] {
        ...
        .ql7PipelineProgress[data-prepared='1'] .ql7PipelineProgress__fill {
          background: linear-gradient(90deg, #31dc88, #8affc8, #48a9ff);
        }
        .ql7PipelineProgress[data-prepared='1'] .ql7PipelineProgress__fill {
          animation: none;
        }
        .ql7PipelineProgress[data-prepared='1'] .ql7PipelineProgress__scan {
          display: none;
        }
