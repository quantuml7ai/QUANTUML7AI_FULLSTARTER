...
import { openPaymentWindow } from '../app/forum/features/qcoin/utils/paymentWindow'

const WALLET_Z_INDEX = 2147482400
const MIN_TOPUP_QCOIN = 20
const TOPUP_STEP_QCOIN = 1

function clampTopupAmount(value) {
  const parsed = Math.floor(Number(value))
  if (!Number.isFinite(parsed)) return MIN_TOPUP_QCOIN
  return Math.max(MIN_TOPUP_QCOIN, parsed)
}

function formatTopupUsd(value) {
  const amount = clampTopupAmount(value)
...
const FALLBACKS = {
  ...
  quantum_wallet_balance_label: 'Balance',
  quantum_wallet_deposit: 'Top up',
  quantum_wallet_deposit_aria: 'QCoin top-up module is preparing',
  quantum_wallet_topup_title: 'QCoin top-up',
  quantum_wallet_topup_kicker: 'QCoin / USD',
  quantum_wallet_topup_subtitle: 'You are preparing to buy QCoin for your Quantum Wallet balance.',
  quantum_wallet_topup_amount_label: 'QCoin amount',
  quantum_wallet_topup_rate_label: 'Rate',
  quantum_wallet_topup_rate_value: '1 QCoin = 1 USD',
  quantum_wallet_topup_total_label: 'Payment total',
  quantum_wallet_topup_min_hint: 'Minimum purchase is 20 QCoin.',
  quantum_wallet_topup_pay: 'Pay',
  quantum_wallet_topup_decline: 'Decline',
  quantum_wallet_topup_processing: 'Creating secure payment',
  quantum_wallet_topup_error_auth: 'Authorization is required for QCoin top-up.',
  quantum_wallet_topup_error_amount: 'Enter at least 20 QCoin.',
  quantum_wallet_topup_error_create: 'Could not create payment. Try again later.',
  quantum_wallet_topup_close_aria: 'Close QCoin top-up',
  quantum_wallet_topup_minus_aria: 'Decrease QCoin amount',
  quantum_wallet_topup_plus_aria: 'Increase QCoin amount',
  quantum_wallet_action_info_aria: 'Open module information',
  quantum_wallet_action_exchange_description: 'Quantum Exchange information is being prepared.',
  quantum_wallet_action_zigzag_description: 'Quantum Zigzag information is being prepared.',
  quantum_wallet_action_quest_description: 'Quantum Quests information is being prepared.',
  quantum_wallet_action_meta_description: 'MetaMarket information is being prepared.',
...
function QuantumTitle() {
  return (
    <svg className="qw-title-svg" viewBox="0 0 520 76" aria-hidden="true" preserveAspectRatio="xMidYMid meet">
      ...
      <path className="qw-title-orbit qw-title-orbit-a" d="M92 45 C158 12, 364 10, 430 42" stroke="url(#qwTitleGradient)" strokeWidth="1.4" fill="none" strokeLinecap="round" />
      <path className="qw-title-orbit qw-title-orbit-b" d="M76 55 C184 70, 320 70, 442 52" stroke="url(#qwTitleGradient)" strokeWidth="1" fill="none" strokeLinecap="round" />
      <path className="qw-title-circuit" d="M62 58h58l18-12h34M350 58h58l22-15h38" stroke="url(#qwTitleGradient)" strokeWidth="1.6" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <circle className="qw-title-node qw-title-node-a" cx="134" cy="46" r="2.3" fill="#67e8f9" />
      <circle className="qw-title-node qw-title-node-b" cx="388" cy="44" r="2.1" fill="#facc15" />
...
export default function QuantumWallet({ onClose, userKey = '', vipActive = false }) {
  ...
  const [topupAmount, setTopupAmount] = useState(MIN_TOPUP_QCOIN)
  const [topupInput, setTopupInput] = useState(String(MIN_TOPUP_QCOIN))
  const [topupSubmitting, setTopupSubmitting] = useState(false)
  const [topupError, setTopupError] = useState('')
  const topupPendingRef = useRef(null)
  ...
  const topupStorageKey = useMemo(() => {
    const account = userKey ? String(userKey).trim().toLowerCase() : 'anonymous'
    return `ql7:qcoin-topup:pending:${account || 'anonymous'}`
  }, [userKey])

  const forgetPendingTopup = useCallback(() => {
    topupPendingRef.current = null
    try {
      if (typeof window !== 'undefined') window.sessionStorage.removeItem(topupStorageKey)
    } catch {}
  }, [topupStorageKey])

  const readPendingTopup = useCallback(() => {
    if (topupPendingRef.current) return topupPendingRef.current
    try {
      if (typeof window === 'undefined') return null
      const raw = window.sessionStorage.getItem(topupStorageKey)
      if (!raw) return null
      const parsed = JSON.parse(raw)
      if (parsed && typeof parsed === 'object') {
        topupPendingRef.current = parsed
        return parsed
      }
    } catch {}
    return null
  ...
  const rememberPendingTopup = useCallback((payload = {}) => {
    const pending = {
      invoiceId: payload.invoiceId ? String(payload.invoiceId) : '',
      orderId: payload.orderId ? String(payload.orderId) : '',
      amountQcoin: payload.amountQcoin || payload.qcoinAmount || topupAmount,
      accountId: userKey || '',
      url: payload.url ? String(payload.url) : '',
      createdAt: Date.now(),
    }
    topupPendingRef.current = pending
    try {
      if (typeof window !== 'undefined') window.sessionStorage.setItem(topupStorageKey, JSON.stringify(pending))
    } catch {}
    return pending
  }, [topupAmount, topupStorageKey, userKey])

  const releaseExternalTopupLock = useCallback(() => {
    const pending = readPendingTopup()
    if (pending?.invoiceId || pending?.orderId) {
      setTopupSubmitting(false)
    }
  }, [readPendingTopup])

  const cancelPendingTopup = useCallback((reason = 'user_cancel') => {
    const pending = readPendingTopup()
    forgetPendingTopup()
    if (!userKey || (!pending?.invoiceId && !pending?.orderId)) return
    let body = ''
    try {
      body = JSON.stringify({
        accountId: userKey,
        ...
        reason: typeof reason === 'string' ? reason : 'user_cancel',
      })
    } catch {
      body = '{}'
    }
    try {
      fetch('/api/qcoin/topup/cancel', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-forum-user-id': userKey,
          'x-auth-account-id': userKey,
        },
        body,
        keepalive: body.length > 0 && body.length < 60000,
      }).catch(() => {})
    } catch {}
  }, [forgetPendingTopup, readPendingTopup, userKey])

  const closeWallet = useCallback(() => {
    if (topupOpen) cancelPendingTopup('wallet_close')
    setStack(['home'])
    setActionInfo(null)
    setTopupSubmitting(false)
    setTopupOpen(false)
    setTopupError('')
    onClose?.()
  }, [cancelPendingTopup, onClose, topupOpen])
  ...
  const goBack = useCallback(() => {
    if (topupOpen) {
      cancelPendingTopup('wallet_back')
      setTopupSubmitting(false)
      setTopupOpen(false)
      setTopupError('')
      return
  ...
  }, [actionInfo, cancelPendingTopup, topupOpen])

  const openTopup = useCallback(() => {
    setActionInfo(null)
    setTopupError('')
    setTopupAmount((current) => {
      const next = clampTopupAmount(current)
  ...
  const closeTopup = useCallback((reason = 'user_cancel') => {
    cancelPendingTopup(reason)
    setTopupSubmitting(false)
    setTopupOpen(false)
    setTopupError('')
  }, [cancelPendingTopup])

  const commitTopupAmount = useCallback((value) => {
    const next = clampTopupAmount(value)
    setTopupAmount(next)
    setTopupInput(String(next))
  }, [])

  const adjustTopupAmount = useCallback((delta) => {
    setTopupAmount((current) => {
      const next = clampTopupAmount(current + delta)
      setTopupInput(String(next))
      return next
    })
  }, [])
  ...
  const submitTopup = useCallback(async () => {
    ...
    const amount = clampTopupAmount(topupInput)
    setTopupAmount(amount)
    setTopupInput(String(amount))
    setTopupError('')

    if (!userKey) {
      setTopupError('quantum_wallet_topup_error_auth')
      return
    }
    if (amount < MIN_TOPUP_QCOIN) {
      setTopupError('quantum_wallet_topup_error_amount')
      return
    }

    setTopupSubmitting(true)
    try {
      const res = await fetch('/api/qcoin/topup/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-forum-user-id': userKey,
      ...
      rememberPendingTopup({
        ...
        amountQcoin: data.amountQcoin || amount,
        url: data.url,
      })
      setTopupSubmitting(false)
      ...
      openPaymentWindow(data.url)
    } catch {
      setTopupError('quantum_wallet_topup_error_create')
      setTopupSubmitting(false)
    }
  }, [rememberPendingTopup, topupInput, topupSubmitting, userKey])
  ...
  const openMetaMarket = useCallback(() => {
    try {
      window.dispatchEvent(new CustomEvent('metamarket:open', {
        detail: {
          source: 'quantum-wallet',
          initialMode: 'market',
          giftFlow: false,
          viewerId: userKey,
          vipActive: !!vipActive,
  ...
  useEffect(() => {
    const release = () => releaseExternalTopupLock()
    const onVisibility = () => {
      if (document.visibilityState === 'visible') releaseExternalTopupLock()
    }
  ...
  }, [actionInfo, closeTopup, closeWallet, topupOpen])
  ...
  const actions = useMemo(() => [
    ...
    { key: 'exchange', label: tx('quantum_wallet_exchange'), icon: 'exchange', active: true, primary: true, onClick: goExchange, infoKey: 'quantum_wallet_action_exchange_description' },
    { key: 'zigzag', label: tx('quantum_wallet_zigzag'), icon: 'zigzag', active: false, locked: true, infoKey: 'quantum_wallet_action_zigzag_description' },
    { key: 'quest', label: tx('quantum_wallet_quest'), icon: 'quest', active: false, quest: true, onClick: openQuests, infoKey: 'quantum_wallet_action_quest_description' },
    { key: 'chain', label: tx('quantum_wallet_chain_l7'), sublabel: tx('quantum_wallet_blockchain'), icon: 'chain', active: false },
    { key: 'meta', label: tx('quantum_wallet_meta_market'), icon: 'meta', active: true, meta: true, onClick: openMetaMarket, title: tx('metamarket_open_from_wallet_aria'), infoKey: 'quantum_wallet_action_meta_description' },
  ], [goExchange, openMetaMarket, openQuests, tx])

  const openActionInfo = useCallback((action, event) => {
    event?.preventDefault?.()
    event?.stopPropagation?.()
    if (!action?.infoKey) return
    setActionInfo({
      key: action.key,
      title: action.sublabel ? `${action.label} ${action.sublabel}` : action.label,
      infoKey: action.infoKey,
    })
  }, [])
  ...
  return createPortal(
    <div
      className="qw-overlay" data-ql7-visual-scope="wallet" data-ql7-visual-open="1"
      role="presentation"
    ...
    >
      ...
      >
        ...
        <div className="qw-content">
          {activeLayer === 'info' ? (
            <div className="qw-info-panel">
              <QuantumTitle />
              <div className="qw-info-card" data-ql7-render-managed="1" data-ql7-visual-scope="card">
            ...
            <div className="qw-home" aria-label={tx('quantum_wallet_layer_home')}>
              ...
              <div className="qw-stage" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
                <QuantumReactor />
              ...
              <div className="qw-balance" data-ql7-render-managed="1" data-ql7-visual-scope="panel">
                ...
                <div className={`qw-balance-value ${qcoin.paused ? 'is-paused' : 'is-live'}`} suppressHydrationWarning>
                  <span className="qw-balance-number" translate="no">{balanceText}</span>
                  <span className="qw-balance-divider" aria-hidden="true" />
                  <button
                    type="button"
                    className="qw-deposit"
                    aria-label={tx('quantum_wallet_deposit_aria')}
                    title={tx('quantum_wallet_deposit_aria')}
                    onClick={openTopup}
                  >
                    {tx('quantum_wallet_deposit')}
                  </button>
                </div>
              </div>

              <div className="qw-actions" data-ql7-render-managed="1" data-ql7-visual-scope="panel" aria-label={tx('quantum_wallet_layer_home')}>
                {actions.map((action) => {
                  const disabled = !action.active
                  return (
                    <div key={action.key} className={`qw-action-shell qw-action-shell-${action.key}`}>
                      <button
                        type="button"
                        className={`qw-action qw-action-${action.key} ${action.active ? 'is-active' : 'is-disabled'} ${action.locked ? 'is-locked' : ''}`}
                        disabled={disabled}
                        aria-disabled={disabled}
                        title={action.title || (action.quest ? tx('quantum_wallet_quest') : action.active ? tx('quantum_wallet_go_exchange') : tx('quantum_wallet_disabled_hint'))}
                      ...
                      {action.infoKey && (
                        <button
                          type="button"
                          className="qw-action-info-btn"
                          aria-label={tx('quantum_wallet_action_info_aria')}
                          title={tx('quantum_wallet_action_info_aria')}
                          onClick={(event) => openActionInfo(action, event)}
                        >
                          i
                        </button>
                      )}
        ...
        {actionInfo && (
          <div className="qw-action-info-layer" role="presentation">
            <section
              className="qw-action-info-popover"
              role="dialog"
              aria-modal="false"
              aria-labelledby="qw-action-info-title"
              onPointerDown={(event) => event.stopPropagation()}
            >
              <button
                type="button"
                className="qw-round qw-action-info-close"
                aria-label={tx('quantum_wallet_close')}
                title={tx('quantum_wallet_close')}
              ...
              <h2 id="qw-action-info-title">{actionInfo.title}</h2>
              <span className="qw-action-info-rail" aria-hidden="true" />
              <div className="qw-action-info-body">
                {splitParagraphs(tx(actionInfo.infoKey, actionInfo.infoKey)).map((paragraph, index) => (
                  <p key={`${actionInfo.key}-${index}`}>{paragraph}</p>
                ))}
              </div>
            </section>
          </div>
        ...
        {topupOpen && (
          <div className="qw-topup-layer" role="presentation">
            <section
              className="qw-topup-popover"
              role="dialog"
              aria-modal="false"
              aria-labelledby="qw-topup-title"
              onPointerDown={(event) => event.stopPropagation()}
            >
              <button
                type="button"
                className="qw-round qw-topup-close"
                aria-label={tx('quantum_wallet_topup_close_aria')}
                title={tx('quantum_wallet_topup_close_aria')}
                onClick={() => closeTopup('close_button')}
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M7 7l10 10M17 7 7 17" />
                </svg>
              ...
              <div className="qw-topup-head">
                <span className="qw-topup-kicker">{tx('quantum_wallet_topup_kicker')}</span>
                <h2 id="qw-topup-title">{tx('quantum_wallet_topup_title')}</h2>
                <p>{tx('quantum_wallet_topup_subtitle')}</p>
              </div>

              <span className="qw-topup-rail" aria-hidden="true" />

              <div className="qw-topup-selector">
                <button
                  type="button"
                  className="qw-topup-step"
                  aria-label={tx('quantum_wallet_topup_minus_aria')}
                  title={tx('quantum_wallet_topup_minus_aria')}
                  onClick={() => adjustTopupAmount(-TOPUP_STEP_QCOIN)}
                  disabled={topupSubmitting || topupAmount <= MIN_TOPUP_QCOIN}
                  aria-disabled={topupSubmitting || topupAmount <= MIN_TOPUP_QCOIN}
                >
                  -
                </button>
                <label className="qw-topup-input-shell">
                  <span>{tx('quantum_wallet_topup_amount_label')}</span>
                  <input
                    value={topupInput}
                    inputMode="numeric"
                    pattern="[0-9]*"
                    ...
                    onBlur={() => commitTopupAmount(topupInput)}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        event.preventDefault()
                        commitTopupAmount(topupInput)
                      }
                    }}
                  />
                </label>
                <button
                  type="button"
                  className="qw-topup-step"
                  aria-label={tx('quantum_wallet_topup_plus_aria')}
                  title={tx('quantum_wallet_topup_plus_aria')}
                  onClick={() => adjustTopupAmount(TOPUP_STEP_QCOIN)}
                  disabled={topupSubmitting}
                  aria-disabled={topupSubmitting}
                >
                  +
                </button>
              </div>

              <div className="qw-topup-details" aria-live="polite">
                <div className="qw-topup-detail">
                  <span>{tx('quantum_wallet_topup_rate_label')}</span>
                  <strong>{tx('quantum_wallet_topup_rate_value')}</strong>
                </div>
                <div className="qw-topup-detail">
                  <span>{tx('quantum_wallet_topup_total_label')}</span>
                  <strong>{formatTopupUsd(topupAmount)}</strong>
                </div>
                <div className="qw-topup-detail">
                  <span>{tx('quantum_wallet_topup_amount_label')}</span>
                  <strong>{topupAmount.toLocaleString('en-US')} QCoin</strong>
                </div>
              </div>

              <div className="qw-topup-hint">{tx('quantum_wallet_topup_min_hint')}</div>
              {topupError && (
                <div className="qw-topup-error" role="alert">
                  {tx(topupError)}
                </div>
              )}

              <div className="qw-topup-actions">
                <button
                  type="button"
                  className="qw-topup-btn is-secondary"
                  onClick={() => closeTopup('decline_button')}
                >
                  {tx('quantum_wallet_topup_decline')}
                </button>
                <button
                  type="button"
                  className="qw-topup-btn is-primary"
                  onClick={submitTopup}
                  disabled={topupSubmitting}
                >
                  {topupSubmitting ? tx('quantum_wallet_topup_processing') : tx('quantum_wallet_topup_pay')}
                </button>
              </div>
            </section>
          </div>
      ...
      <style jsx global>{`
        ...
        .qw-shell {
          ...
          animation: qwShellIn .16s cubic-bezier(.2,.82,.2,1) both;
        }
...
/* Quantum Wallet v14: premium static-safe controls, title rail, action details. */
.qw-overlay {
...
.qw-field-particle:nth-of-type(n+23) {
  display: none !important;
}

@media (min-width: 641px) {
  .qw-shell.qw-layer-info {
    --qw-info-viewport-edge: clamp(22px, 5dvh, 54px);
    height: min(860px, calc(100dvh - (var(--qw-info-viewport-edge) * 2))) !important;
    max-height: calc(100dvh - (var(--qw-info-viewport-edge) * 2)) !important;
  }

  .qw-shell.qw-layer-info .qw-content {
    height: calc(100% - clamp(48px, 7vw, 64px)) !important;
    max-height: none !important;
  }

  .qw-shell.qw-layer-info .qw-info-panel {
    height: 100% !important;
  }
}
...
.qw-title-svg {
  overflow: visible !important;
  filter:
    drop-shadow(0 0 10px rgba(103,232,249,.22))
    drop-shadow(0 0 16px rgba(250,204,21,.10)) !important;
}
...
.qw-title-back {
  ...
  stroke: rgba(2,8,20,.92) !important;
  stroke-width: 7px !important;
}

.qw-title-main {
  fill: url(#qwTitleGradient) !important;
  stroke: rgba(236,254,255,.56) !important;
  stroke-width: .7px !important;
  stroke-dasharray: none !important;
  stroke-dashoffset: 0 !important;
  filter: url(#qwTitleGlow) !important;
  paint-order: stroke fill !important;
  animation: qwWalletTitleBreathe 6.8s ease-in-out infinite !important;
}

.qw-title-orbit {
  opacity: .46;
  stroke-dasharray: 42 210;
  animation: qwWalletTitleOrbit 8.5s ease-in-out infinite;
}
...
.qw-title-circuit {
  opacity: .62;
  stroke-dasharray: 58 180;
  animation: qwWalletTitleCircuit 7.6s ease-in-out infinite;
}

.qw-title-node {
  transform-origin: center;
  animation: qwWalletTitleNode 4.8s ease-in-out infinite;
}
...
.qw-shell.qw-layer-home .qw-balance-value {
  ...
  padding: clamp(7px, 1.35vw, 10px) clamp(8px, 1.8vw, 13px) !important;
  box-sizing: border-box !important;
...
.qw-balance-divider {
  ...
  border-radius: 999px;
  background: linear-gradient(180deg, transparent, rgba(103,232,249,.78), rgba(250,204,21,.54), transparent);
  box-shadow: 0 0 10px rgba(103,232,249,.24);
}

.qw-deposit {
  min-width: clamp(66px, 15vw, 92px);
  min-height: clamp(30px, 6vw, 38px);
  border: 1px solid rgba(250,204,21,.46);
  border-radius: 999px;
  padding: 0 clamp(8px, 2vw, 13px);
  background:
    radial-gradient(circle at 22% 0%, rgba(255,247,173,.32), transparent 44%),
    linear-gradient(135deg, rgba(250,204,21,.22), rgba(103,232,249,.12), rgba(15,23,42,.82));
  color: rgba(255,247,205,.96);
...
.qw-action-shell {
  position: relative;
  min-width: 0;
  min-height: clamp(76px, 11.2vw, 98px);
  display: grid;
}

.qw-action-shell .qw-action {
  width: 100% !important;
  height: 100% !important;
}
...
.qw-action-info-btn {
  ...
  padding: 0;
  background:
    radial-gradient(circle at 30% 15%, rgba(255,255,255,.20), transparent 38%),
    linear-gradient(145deg, rgba(8,19,38,.94), rgba(1,5,14,.88));
  color: rgba(226,246,255,.95);
...
.qw-action-info-btn:hover {
  filter: brightness(1.1);
  border-color: rgba(250,204,21,.52);
}

.qw-action-info-layer {
  position: absolute;
  inset: 0;
  z-index: 22;
  display: grid;
  ...
  border-radius: inherit;
  background:
    radial-gradient(circle at 50% 32%, rgba(103,232,249,.14), transparent 38%),
    rgba(1,5,14,.58);
  -webkit-backdrop-filter: none;
  backdrop-filter: none;
}

.qw-action-info-popover {
  position: relative;
  width: min(430px, 100%);
  height: 100%;
  max-height: 100%;
  ...
  background:
    radial-gradient(circle at 12% 12%, rgba(103,232,249,.18), transparent 38%),
    radial-gradient(circle at 88% 8%, rgba(250,204,21,.16), transparent 34%),
    linear-gradient(145deg, rgba(7,18,36,.98), rgba(1,5,14,.98));
  box-shadow:
...
.qw-action-info-rail {
  height: 1px;
  border-radius: 999px;
  background: linear-gradient(90deg, transparent, rgba(103,232,249,.62), rgba(250,204,21,.42), transparent);
...
.qw-action-info-body {
  ...
  padding-right: 4px;
  scrollbar-width: thin;
  scrollbar-color: rgba(103,232,249,.56) rgba(15,23,42,.36);
}

.qw-action-info-body p {
  margin: 0;
  padding: clamp(12px, 2.2vw, 15px);
  border: 1px solid rgba(103,232,249,.18);
  border-radius: 18px;
  background:
    linear-gradient(120deg, rgba(103,232,249,.08), rgba(250,204,21,.055), rgba(192,132,252,.06)),
    rgba(2,8,20,.54);
...
.qw-topup-layer {
  ...
  border-radius: inherit;
  background:
    radial-gradient(circle at 50% 26%, rgba(250,204,21,.14), transparent 34%),
    radial-gradient(circle at 20% 72%, rgba(103,232,249,.13), transparent 42%),
    rgba(1,5,14,.62);
  -webkit-backdrop-filter: none;
  backdrop-filter: none;
}
...
.qw-topup-popover {
  ...
  background:
    radial-gradient(circle at 18% 8%, rgba(103,232,249,.18), transparent 38%),
    radial-gradient(circle at 88% 0%, rgba(250,204,21,.16), transparent 36%),
    linear-gradient(145deg, rgba(6,18,35,.98), rgba(1,5,14,.99));
  box-shadow:
    0 28px 74px rgba(0,0,0,.62),
    0 0 34px rgba(250,204,21,.13),
    inset 0 0 0 1px rgba(255,255,255,.05);
  overflow: hidden;
}

.qw-topup-popover::before {
  content: '';
  position: absolute;
  inset: 1px;
  border-radius: inherit;
  pointer-events: none;
  background:
    linear-gradient(90deg, transparent, rgba(103,232,249,.13), transparent),
    repeating-linear-gradient(90deg, rgba(103,232,249,.055) 0 1px, transparent 1px 74px);
  opacity: .58;
}
...
.qw-topup-head h2 {
  ...
  line-height: 1.05;
  text-shadow:
    0 0 16px rgba(103,232,249,.36),
    0 0 22px rgba(250,204,21,.18);
}

.qw-topup-head p {
  margin: 0;
  color: rgba(213,235,246,.88);
...
.qw-topup-rail {
  ...
  height: 1px;
  border-radius: 999px;
  background: linear-gradient(90deg, transparent, rgba(103,232,249,.66), rgba(250,204,21,.55), transparent);
  box-shadow: 0 0 14px rgba(103,232,249,.18);
}
...
.qw-topup-selector {
  ...
  border-radius: 999px;
  background:
    linear-gradient(90deg, rgba(103,232,249,.12), rgba(250,204,21,.075), rgba(192,132,252,.10)),
    rgba(2,8,20,.72);
...
.qw-topup-step {
  ...
  border-radius: 999px;
  background:
    radial-gradient(circle at 35% 15%, rgba(255,255,255,.18), transparent 36%),
    linear-gradient(145deg, rgba(8,31,48,.92), rgba(2,8,20,.94));
...
.qw-topup-step:hover:not(:disabled) {
  border-color: rgba(250,204,21,.58);
  filter: brightness(1.14);
}

.qw-topup-step:disabled {
  opacity: .45;
  cursor: not-allowed;
}

.qw-topup-input-shell {
  min-width: 0;
  display: grid;
  place-items: center;
  border: 1px solid rgba(250,204,21,.28);
  border-radius: 999px;
  background:
    radial-gradient(circle at 50% 0%, rgba(250,204,21,.14), transparent 46%),
    rgba(1,5,14,.76);
}

.qw-topup-input-shell span {
  color: rgba(154,230,255,.96);
  font-size: 9px;
  font-weight: 1000;
  letter-spacing: .06em;
...
.qw-topup-input-shell input {
  width: 100%;
  min-width: 0;
  border: 0;
  outline: 0;
  background: transparent;
  color: rgba(255,248,208,.98);
...
.qw-topup-detail {
  ...
  border-radius: 16px;
  background:
    linear-gradient(135deg, rgba(103,232,249,.08), rgba(250,204,21,.045)),
    rgba(2,8,20,.62);
  text-align: center;
}

.qw-topup-detail span {
  min-width: 0;
  color: rgba(154,230,255,.9);
  font-size: 9px;
  font-weight: 1000;
  letter-spacing: .04em;
  line-height: 1.1;
...
.qw-topup-hint {
  ...
  color: rgba(213,235,246,.82);
  background: rgba(2,8,20,.44);
}

.qw-topup-error {
  border: 1px solid rgba(248,113,113,.38);
  color: rgba(255,216,216,.95);
...
.qw-topup-btn:hover:not(:disabled) {
  transform: translateY(-1px);
  filter: brightness(1.1);
}

.qw-topup-btn:disabled {
  opacity: .66;
  cursor: wait;
}

.qw-topup-btn.is-secondary {
  border: 1px solid rgba(103,232,249,.30);
  background: linear-gradient(135deg, rgba(8,31,48,.94), rgba(2,8,20,.88));
}

.qw-topup-btn.is-primary {
  border: 1px solid rgba(250,204,21,.54);
  background:
    radial-gradient(circle at 30% 0%, rgba(255,247,173,.30), transparent 42%),
    linear-gradient(135deg, rgba(250,204,21,.24), rgba(103,232,249,.14), rgba(46,16,101,.68));
...
@media (max-width: 640px) {
  ...
  .qw-action-shell {
    min-height: clamp(66px, 17vw, 78px);
  }
...
@keyframes qwWalletTitleBreathe {
  0%, 100% {
    opacity: .9;
...
@keyframes qwWalletTitleCircuit {
  0%, 100% {
