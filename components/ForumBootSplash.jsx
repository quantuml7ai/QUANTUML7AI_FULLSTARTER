...
import AndroidChromiumVideoCanvas from './AndroidChromiumVideoCanvas'
...
function shouldShowSplashNow() {
  ...
  return window.__QL7_FORUM_SPLASH_DONE__ !== '1'
}

function attachSplashSource(el) {
  if (!el) return false
  const expectedSrc = FORUM_SPLASH_VIDEO_SRC
  try {
    if (el.dataset.ql7SplashSrcAttached === '1' && el.getAttribute('src') === expectedSrc) {
      return false
    }
    el.dataset.ql7SplashSrcAttached = '1'
    el.setAttribute('src', expectedSrc)
    return true
  } catch {
    return false
  }
...
function isIOSWebKitSplashRuntime() {
...
const splashStyle = useMemo(() => ({
  ...
  zIndex: FORUM_SPLASH_Z_INDEX,
  display: 'flex',
  alignItems: FORUM_SPLASH_ALIGN_Y,
  justifyContent: FORUM_SPLASH_ALIGN_X,
  padding: FORUM_SPLASH_PADDING,
  background: FORUM_SPLASH_BACKDROP,
  backdropFilter: `blur(${FORUM_SPLASH_BACKDROP_BLUR})`,
  WebkitBackdropFilter: `blur(${FORUM_SPLASH_BACKDROP_BLUR})`,

  /*
   * Splash itself receives pointer events,
   * but browser must not interpret touch gestures as scrolling.
   */
  pointerEvents: 'auto',
  touchAction: 'none',
  ...
  const frameStyle = useMemo(() => ({
    position: 'relative',
    width: frameWidth,
    minWidth: FORUM_SPLASH_MIN_WIDTH,
    maxWidth: FORUM_SPLASH_MAX_WIDTH,
    maxHeight: FORUM_SPLASH_MAX_HEIGHT,
    aspectRatio: FORUM_SPLASH_ASPECT_RATIO,
    left: FORUM_SPLASH_OFFSET_X,
    top: FORUM_SPLASH_OFFSET_Y,
    borderRadius: FORUM_SPLASH_BORDER_RADIUS,
    border: FORUM_SPLASH_BORDER,
    boxShadow: FORUM_SPLASH_BOX_SHADOW,
  ...
  const videoStyle = useMemo(() => ({
    ...
    width: '100%',
    height: '100%',
    objectFit: FORUM_SPLASH_OBJECT_FIT,
    objectPosition: FORUM_SPLASH_OBJECT_POSITION,
    background: FORUM_SPLASH_VIDEO_BACKGROUND,
    opacity: `calc(${FORUM_SPLASH_VIDEO_OPACITY} * .9999)`,
  ...
  const finish = useCallback((immediate = false) => {
    ...
    try {
      if (typeof window !== 'undefined') window.__QL7_FORUM_SPLASH_DONE__ = '1'
    } catch {}
  ...
  const startPlayback = useCallback(async () => {
    ...
    const attached = attachSplashSource(el)

    try {
      if (attached || !el.currentSrc) el.load?.()
    } catch {}
  ...
  setSoundAutoplayBlocked(true)
    ...
    if (FORUM_SPLASH_ALLOW_MUTED_FALLBACK === 1) {
      try {
        ...
        const blockedMutedAutoplay =
          isIOSWebKitSplashRuntime() &&
          String(err?.name || '') === 'NotAllowedError'

        if (blockedMutedAutoplay) {
          ...
          try {
            ...
            delete el.dataset.ql7SplashSrcAttached
            el.load?.()
          } catch {}
...
const unlockSplashSound = useCallback(() => {
  ...
  try {
    el.volume = Math.max(
      0,
      Math.min(1, Number(FORUM_SPLASH_VOLUME || 1))
    )
    ...
    /*
     * ВАЖНО:
     * play() вызывается непосредственно из пользовательского click,
     * не после timeout/await. Именно это нужно WebKit для user activation.
     */
    const p = el.play?.()
    ...
    if (p && typeof p.then === 'function') {
      p.then(() => {
        ...
        /*
         * Не удалось даже из user gesture —
         * не ломаем Splash, возвращаем стабильный muted playback.
         */
...
useEffect(() => {
  ...
  /*
   * Save ONLY styles that we temporarily modify.
   */
  const prevBodyOverflow = document.body.style.overflow
  const prevHtmlOverscrollBehavior = document.documentElement.style.overscrollBehavior

  /*
   * Hard block native/manual page scrolling.
   *
   * Important:
   * overflow:hidden blocks USER scrolling and scrollbar dragging,
   * but we do NOT freeze scrollTop and do NOT call scrollTo().
   */
  document.body.style.overflow = 'hidden'
  document.documentElement.style.overscrollBehavior = 'none'

  /*
   * Wheel / mouse wheel / desktop trackpad.
   */
  ...
  /*
   * Finger scrolling on mobile/tablet.
   */
  ...
  /*
   * Middle mouse button autoscroll.
   */
  const preventMiddleMouse = (event) => {
  ...
  window.addEventListener('mousedown', preventMiddleMouse, {
    capture: true,
  })
  ...
  return () => {
    /*
     * Restore exactly what existed before BootSplash.
     */
    ...
    window.removeEventListener('mousedown', preventMiddleMouse, {
      capture: true,
    })

    if (window.__forumBootSplashActive === '1') {
  ...
  return (
    <>
      <div
        ...
        data-runtime-owner="forum-boot-splash"
        aria-hidden="true"
        style={splashStyle}
      >
        <div className="forum-boot-splash__frame" style={frameStyle}>
          <video
            ref={videoRef}
            className="forum-boot-splash__video"
            data-ql7-splash-src={FORUM_SPLASH_VIDEO_SRC}
            playsInline
            preload="none"
            controls={false}
          ...
          />
          <AndroidChromiumVideoCanvas videoRef={videoRef} fit={FORUM_SPLASH_OBJECT_FIT} />
...
.forum-boot-splash {
  ...
  /*
   * Prevent touch pan and scroll chaining while BootSplash is active.
   */
  touch-action: none;
