...
function readUnifiedAccountId() {
  ...
  return String(readAuthorizedAccountId() || '').trim() || null
}
function readAccountFromAuthEvent(event) {
  return String(
...
export default function InviteFriendHost() {
  ...
  const openRuntime = useCallback((accountOverride, reason = 'manual') => {
    const currentUid = readUnifiedAccountId()
    const effectiveUid = currentUid || accountOverride || (reason === 'auto' ? uid : null)
  ...
  useEffect(() => {
    ...
    const handleAuth = (event) => {
      const acc = readAccountFromAuthEvent(event)
      if (!acc) return

      const now = Date.now()
      setUid(acc)
      setRuntime((current) => (current && current.accountId !== acc ? null : current))
      setLastAuthAt(now)
      window.localStorage.setItem(LS_LAST_AUTH, String(now))
