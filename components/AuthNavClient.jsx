...
/** Жёстко открываем в этой же вкладке (в TMA — WebApp.openLink) */
...
/** Детект webview/TMA (подстраховка) */
...
/** URL старта твоего OAuth-селектора */
function buildAuthStartUrl() {
  const base = (typeof window !== 'undefined' ? window.location.origin : '')
  const target = `${base}/auth`           // именно страница /auth (твой мини-модал)
  try {
...
/** Достаём accountId как у тебя в проекте */
...
export default function AuthNavClient() {
  ...
  // ------------- состояние cookie-сессии сайта (OAuth / Email / Apple / Google)
  const [siteAuthed, setSiteAuthed] = useState(false)
  const [siteUser, setSiteUser]     = useState(null) // { userId, email, name }
  const [checking, setChecking]     = useState(false)

  // виджет/метод авторизации web3modal (для подписи на кнопке, если надо)
  ...
  // подпись метода из web3modal (для лейбла)
  useEffect(() => {
    try {
  ...
  // если отключили кошелёк — не трогаем cookie-сессию, просто перерисуемся
  useEffect(() => {
  ...
  // --- Возврат из Telegram (startapp=auth_<code>) → обмен и обновление куки
  useEffect(() => {
    try {
  ...
  // ===== Выход (и из cookie-сессии, и из кошелька при наличии)
  async function signOut() {
    try {
      await fetch('/api/auth/logout', { method: 'POST', credentials: 'include' })
    } catch {}
    // чистим твои ключи, если были
    try {
  ...
  // ===== Меню действий при нажатии на кнопку, когда уже авторизован
  async function openAccountMenu() {
    // максимально простое и кроссплатформенное меню без сторонних либ
    ...
    if (action === '1') {
      // перевход: в TMA/webview ведём на твою страницу /auth в той же вкладке,
      // в обычных браузерах — открываем Web3Modal (кошельки) или /auth (если нужен OAuth)
  ...
  async function onAuthClick() {
    ...
    // обычный браузер — модалка web3modal (кошельки + OAuth по твоей конфигурации)
