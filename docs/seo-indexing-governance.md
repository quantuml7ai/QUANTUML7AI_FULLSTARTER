...
`/contact` is intentionally non-indexed and disabled from the public runtime:
the page route performs a server-side redirect into the authenticated
`/forum?ql7SupportOpen=1&inbox=messages&dmUser=ql7-support` support thread.
The SMTP contact API remains available only as server logic for existing
transport/reporting paths and is not a public page entry.
