...
const sampler = read('app/forum/features/media/services/moderatePreparedVideoForUpload.js')
...
requireText(gateway, 'posterRecord = videoModeration?.poster || null', 'POSTER_GATEWAY_EXACT_ARTIFACT_MISSING')
forbidText(gateway, 'createNativeVideoPoster({', 'POSTER_GATEWAY_SECOND_DECODER_PRESENT')
requireText(sampler, 'new mediabunny.CanvasSink(videoTrack, sinkOptions)', 'POSTER_CANVAS_SINK_MISSING')
requireText(sampler, 'sink.canvasesAtTimestamps(requests.map((request) => request.timestamp))', 'POSTER_SINGLE_SAMPLE_SESSION_MISSING')
requireText(sampler, 'createNativeVideoPosterFromCanvas({', 'POSTER_SAME_SESSION_ENCODER_MISSING')
...
forbidText(resolver, 'readCachedNativeVideoPoster', 'POSTER_RESOLVER_STRING_CACHE_REGRESSION')
...
requireText(preview, 'pendingVideoPosterBlob', 'POSTER_COMPOSER_EXACT_BLOB_MISSING')
forbidText(preview, 'createNativeVideoPoster', 'POSTER_COMPOSER_SECOND_DECODER_PRESENT')
requireText(preview, 'frontCameraMirror={pendingVideoMirror}', 'POSTER_COMPOSER_VIDEO_MIRROR_MISSING')
requireText(overlay, 'previewPosterBlob', 'POSTER_COMPOSER_OVERLAY_EXACT_BLOB_MISSING')
forbidText(overlay, 'createNativeVideoPoster', 'POSTER_OVERLAY_SECOND_DECODER_PRESENT')
