...
const immutablePublic = process.env.QL7_SUPPORT_IMMUTABLE_PUBLIC === '1'
const immutableEnv = process.env.QL7_SUPPORT_IMMUTABLE_ENV === '1'
const generatedAt = process.env.QL7_PROJECT_DOCS_GENERATED_AT || new Date().toISOString()
...
function isGeneratedLocalArtifact(relPath) {
  ...
  if (normalized === 'report/QL7_SUPPORT_PATCH' || normalized.startsWith('report/QL7_SUPPORT_PATCH/')) return true
  if (ignoredTopDirs.includes(topDir)) return true
  if (parts.some((part) => ignoredDirNames.has(part) || /^\.codex_tmp/i.test(part) || /^\.tmp-/i.test(part) || /^\.?venv(?:-|$)/i.test(part))) return true

  const base = parts[parts.length - 1]
  if (/^QL7_SUPPORT_PLANETARY_AI_V5_.*\.(?:ps1|zip|json|patch)$/i.test(base)) return true
  if (/^QL7_TRUST_IDENTITY_MULTILINGUAL_SEO_FINAL_BASELINE_V\d+.*\.(?:ps1|zip|json|txt)$/i.test(base)) return true
  if (/^final_baseline_trust_identity_v\d+_.*\.json$/i.test(base)) return true
...
function readRepoFiles() {
  ...
  if (gitFiles) {
    for (const relPath of gitFiles) {
      ...
      if (immutablePublic && (normalized === 'public' || normalized.startsWith('public/'))) continue
      if (immutableEnv && /^\.env(?:$|\.)/i.test(normalized)) continue
    ...
    function walk(currentDir) {
      ...
      for (const entry of entries) {
        ...
        if (immutablePublic && (relPath === 'public' || relPath.startsWith('public/'))) continue
        if (immutableEnv && /^\.env(?:$|\.)/i.test(relPath)) continue
  ...
  return Array.from(new Set(out)).sort((a, b) => a.localeCompare(b, 'ru', { sensitivity: 'base', numeric: true }))
}
...
function main() {
  ...
  lines.push('')
  lines.push(`Сгенерировано автоматически: ${generatedAt}`)
  ...
  lines.push("- `scripts/` — локальные migration/audit/smoke/image утилиты; не production runtime и не часть коммитимого дерева после V15.")
  lines.push("- `.ql7-backups/`, `.ql7-local-archive/`, `.ql7-tmp/`, `.ql7-patch-backups/`, `reports/`, `account-deletion-reports/`, `migration-reports/`, `mongo-migration-reports/` и корневые `QL7_SUPPORT_PLANETARY_AI_V5_*` — локальные patch/report/evidence артефакты.")
  if (immutablePublic) lines.push('- `public/**` — внешний immutable asset namespace, исключённый из Support closeout inventory; его фактическое содержимое защищается отдельным before/after hash guard и не нормализуется этим tooling.')
  if (immutableEnv) lines.push('- `.env*` — локальный immutable secret/config namespace, исключённый из Support closeout inventory; содержимое не читается и не нормализуется project-doc tooling.')
  lines.push("")
  lines.push("## Охват")
  lines.push('')
  lines.push('Этот файл описывает весь авторский репозиторий проекта, а не только форумный домен.')
  lines.push(immutablePublic
    ? 'Включены все прикладные страницы, layouts, API-маршруты, клиентские компоненты, серверные и инфраструктурные модули, audit-артефакты и корневые конфиги. `public/**` является внешним immutable namespace для этого Support closeout и намеренно не инвентаризируется.'
