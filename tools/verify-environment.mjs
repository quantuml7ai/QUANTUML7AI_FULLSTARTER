...
const canonicalSiteUrlKey = 'NEXT_PUBLIC_SITE_URL'
const canonicalSiteUrl = 'https://www.quantuml7ai.com'

function readEnvAssignments(filePath, key) {
...
function validateCanonicalSiteUrl(filePath, label, { required = false } = {}) {
  const assignments = readEnvAssignments(filePath, canonicalSiteUrlKey)

  if (required && assignments.length === 0) {
    issues.push(`${label} must declare ${canonicalSiteUrlKey}=${canonicalSiteUrl}.`)
    return
  }

  if (assignments.length > 1) {
    issues.push(
      `${label} declares ${canonicalSiteUrlKey} more than once (lines ${assignments
        .map(({ lineNumber }) => lineNumber)
        .join(', ')}). Keep exactly one canonical value.`,
    )
    return
  }
  ...
  const [{ value, lineNumber }] = assignments
  let parsed
  try {
    parsed = new URL(value)
  } catch {
    issues.push(`${label}:${lineNumber} ${canonicalSiteUrlKey} must be an absolute HTTPS origin.`)
    return
  }
  ...
  if (!hasOnlyOrigin || parsed.origin !== canonicalSiteUrl) {
    issues.push(
      `${label}:${lineNumber} ${canonicalSiteUrlKey} must be exactly ${canonicalSiteUrl}.`,
    )
  }
}
...
validateCanonicalSiteUrl(path.join(repoRoot, '.env.local.example'), '.env.local.example', {
  required: true,
})
validateCanonicalSiteUrl(path.join(repoRoot, '.env.local'), '.env.local')
