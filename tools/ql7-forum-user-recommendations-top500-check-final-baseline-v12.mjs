...
const videoState = read('app/forum/features/media/hooks/useVideoFeedState.js')
...
const checks = {
  ...
  moderationLiveGate: pool.includes("readMediaLockStates(deliveryCanonicalIds, aliasesByCanonical, 'delivery-media-lock', deliveryNowMs, db)") && pool.includes('if (lock.locked) continue') && unitPolicy.includes('excludes an active lock again at live delivery'),
  ...
  profileLiveHydration: pool.includes("const AUTHORITATIVE_PROFILE_SOURCE = 'profilePrimary.getUserProfile'") && pool.includes("presentationHydration: 'live-per-response'") && pool.includes("readAuthoritativeProfiles(deliveryCanonicalIds, 'delivery-profile')") && pool.includes("phase === 'delivery-profile'") && pool.includes('profile = await profilePrimary.getUserProfile(canonicalAccountId)') && pool.includes('currentFollowers < MIN_FOLLOWERS'),
  ...
  bufferedSupplyFetchDedup: hook.includes('const requiredAdditionalBatchCount = Math.max(') && hook.includes('const latentFirstRailBatchCount = railSlots.length === 0 && unusedBufferedBatchCount > 0 ? 1 : 0') && hook.includes('const futureReserveBatchCount = Math.max(0, unusedBufferedBatchCount - latentFirstRailBatchCount)') && hook.includes('desiredMissingCount + prefetchRailsAhead - futureReserveBatchCount') && hook.includes('.slice(0, 1)') && hook.includes('.slice(safeStart, safeEnd + 1)') && !hook.includes('.slice(safeStart, safeEnd + prefetchRailsAhead + 1)') && hook.includes('if (!initialPrimeNeeded && requiredAdditionalBatchCount <= 0) return') && hook.includes('requiredAdditionalBatchCount,') && hookTest.includes('redundant_guest_recommendation_fetch') && hookTest.includes('expect(guestGetCount).toBe(1)'),
  earlyOneRailPrime: hook.includes("initialPrimeNeeded ? 'prime' : 'refill'") && hook.includes('const requestBatchCount = initialPrimeNeeded') && hook.includes('? 1') && !hook.includes('if (!desiredRailSlotKeys.length) return') && hookTest.includes('primes one rail before any slot exists and refills the future reserve without blocking it') && hookTest.includes("expect(primeUrl.searchParams.get('batches')).toBe('1')") && hookTest.includes("expect(reserveUrl.searchParams.get('batches')).toBe('3')"),
  boundedCursorRefill: hook.includes('Math.min(batchesPerRequest, requiredAdditionalBatchCount)') && hook.includes('normalizeId(state.nextCursor)') && hookTest.includes('uses prefetchRailsAhead as an unassigned reserve and carries the cursor into refill') && hookTest.includes("expect(refillUrl.searchParams.get('batches')).toBe('2')") && hookTest.includes("expect(refillUrl.searchParams.get('cursor')).toBe('cursor-after-prime')"),
  stableRecommendationFeedContext: !videoState.includes('function buildVideoFeedContextSignature') && !videoState.includes('buildVideoFeedContextSignature(videoFeed)') && hookTest.includes('keeps the recommendation context key stable when a server video page append only grows the feed'),
  effectDependencyParity: hook.includes('initialPrimeNeeded,\n    prefetchRailsAhead,') && hook.includes('requiredAdditionalBatchCount,\n    desiredMissingCount,\n    videoFeedOpen,'),
  ...
  singleRequestIdentityBoundary: pool.includes("resolveForumCanonicalMap(seeds, 'identity-seed-authority')") && !pool.includes("'request-canonical'") && !pool.includes("'delivery-canonical'") && pool.includes('const canonicalViewerId = canonicalFromResolution(requestIdentity, viewerId)') && unitPolicy.includes('resolves raw request identity once and excludes the canonical viewer before delivery') && routeTest.includes('GET resolves viewer/excludes and never performs a rebuild itself'),
  parallelLiveDeliveryGates: pool.includes('const [exactFollowers, mediaLocks, vipCanonical, authoritativeProfiles] = await Promise.all([') && pool.includes('readExactFollowerCounts(db, deliveryCanonicalIds, aliasesByCanonical)') && pool.includes("readMediaLockStates(deliveryCanonicalIds, aliasesByCanonical, 'delivery-media-lock', deliveryNowMs, db)") && pool.includes('readLiveVipCanonicalSet(db, aliasesByCanonical, currentCanonical)') && pool.includes("readAuthoritativeProfiles(deliveryCanonicalIds, 'delivery-profile')"),
  deliveryProfileFastPath: pool.includes("phase === 'delivery-profile'") && pool.includes('profile = await profilePrimary.getUserProfile(canonicalAccountId)') && contractPolicy.includes('expect(source).toContain("phase === \'delivery-profile\'")'),
...
const payload = {
  ...
  policy: {
    ...
    authTransition: 'atomic generation reset + AbortController + canonical self-defense + early one-rail prime + bounded cursor refill',
    metricWindowDays: 7,
