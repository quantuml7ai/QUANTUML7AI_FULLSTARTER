...
function main() {
  ...
  const publicDirs = immutablePublic ? [] : topDirs(path.join(repoRoot, 'public'))
  ...
  lines.push(immutablePublic
    ? '- `public/**` — внешний immutable asset namespace для Support closeout; содержимое не инвентаризируется и не изменяется этим tooling.'
  ...
  if (immutablePublic) {
    lines.push(...mdLinesForOwnership(
      'public/** (external immutable namespace)',
      'public/**',
      'Статические ассеты реального репозитория. Support closeout не перечисляет и не изменяет их содержимое; целостность проверяется before/after hash guard установщика.',
      [],
      ['app', 'components', 'app/forum', 'styles/public URLs'],
      ['Содержимое намеренно не зависит от урезанного архивного представления.']
    ))
  } else {
